...
def __is_banded(sn: str):
    """某个cookie是否被禁用"""
    value = rds.client.get(f"res:zdm:banded:{sn}")
    return value == "1"


def __banded_cookie(sn: str, freeze_time: int = 7200):
    """禁用某个cookie"""
    key = f"res:zdm:banded:{sn}"
...
def get_ck_list(origin_key_list: List[str]) -> Tuple[List[Tuple[str, str]], dict]:
    """
    sn_list_key = f"res:cookie:*:zdm:{plat_type}:*"
...
def __add_use_time(biz_id: str, cookie_pin: str, expire: int = 60):
    ...
    key = f"res:zdm:use:{biz_id}:{cookie_pin}:{time_stamp}"
    rds.client.set(key, time_stamp)
    rds.client.expire(key, expire)
    record_key = f"res:zdm:record:{cookie_pin}"
...
def get_cookie(biz_id: str, keys: List[str] = []) -> Optional[ResCookie]:  # noqa
    """
    根据业务编码获取cookie
    :param biz_id: 业务编号，如：zdm_price
    :param keys: 需要提取的cookie字段，如果为空则获取所有
    :return:
    config: plat_type: str, app, h5
            expire: int, 使用过期时间
            use_times: 最多使用的次数
            channel: 来源
    ...
    plat_type = config.get("plat_type", "app")

    cookie_queue_key = f'res:cookie:{channel}:zdm:{plat_type}:queue'
    ...
    while True:
        ...
        if __is_over_times(biz_id, sn, use_times):
            continue
        cookie_key = f'res:cookie:{channel}:zdm:{plat_type}:{sn}'
...
def release_cookie(sn: str):
    ...
    把移除的cookie释放，解冻
    """
    rds.client.delete(f"res:zdm:banded:{sn}")
