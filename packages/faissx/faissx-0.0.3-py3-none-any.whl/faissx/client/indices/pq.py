...
class IndexPQ(FAISSxBaseIndex):
    ...
    def __init__(self, d: int, M: int = 8, nbits: int = 8,
        ...
        # Validate that dimension is a multiple of M (required by FAISS)
        if d % M != 0:
        ...
        if client is not None and client.mode == "remote":
            # Remote mode
            logger.info(f"Creating remote IndexPQ on server {client.server}")
            self._create_remote_index(client, d, M, nbits)
        else:
            # Local mode
            logger.info(f"Creating local IndexPQ index {self.name}")
            self._create_local_index(d, M, nbits, metric_type)
    ...
    def _create_local_index(
        self, d: int, M: int, nbits: int, metric_type: Any
    ) -> None:
        """
        Create a local FAISS PQ index.
        ...
        try:
            ...
            if gpu_available:
                ...
                try:
                    ...
                    logger.info(f"Using GPU-accelerated PQ index for {self.name}")
            ...
            raise RuntimeError(f"Failed to create IndexPQ: {e}")

    def _create_remote_index(
        self, client: Any, d: int, M: int, nbits: int
    ) -> None:
        """
        Create a remote PQ index on the server.
        ...
        try:
            ...
            try:
                ...
                # For PQ indices, some servers consider them always trained
                self.is_trained = True
    ...
    def _train_local(self, vectors: np.ndarray) -> None:
        ...
        if self._local_index is None:
            # Fallback initialization if local index wasn't properly created
            try:
                import faiss as local_faiss
                metric_type_val = 0 if self.metric_type == "L2" else 1
                self._local_index = local_faiss.IndexPQ(self.d, self.M, self.nbits, metric_type_val)
                logger.warning("Created fallback local index during training")
            except Exception as e:
                raise RuntimeError(f"Cannot create fallback local index: {e}")
    ...
    def _add_remote_batch(self, client: Any, vectors: np.ndarray) -> None:
        ...
        try:
            ...
            # For some server implementations that don't properly implement PQ
    ...
    def search(self, x: np.ndarray, k: int) -> Tuple[np.ndarray, np.ndarray]:
        ...
        internal_k = min(k, max(1, self.ntotal))
    ...
    def _search_remote_batch(
        ...
        try:
            # Include any required parameters in the search request
    ...
    def _range_search_local(
        ...
        if self._local_index is not None:
            if hasattr(self._local_index, "range_search"):
                start_time = time.time()
                result = self._local_index.range_search(query_vectors, radius)
                logger.info(f"Local range search completed in {time.time() - start_time:.2f}s")
                return result
        ...
        return lims, np.array([], dtype=np.float32), np.array([], dtype=np.int64)
    ...
    def _range_search_remote(
        ...
        try:
            ...
            logger.info(f"Remote range search completed in {time.time() - start_time:.2f}s")
