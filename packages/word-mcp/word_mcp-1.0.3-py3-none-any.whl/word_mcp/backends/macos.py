...
class MacOSWordBackend(WordBackend):
    ...
    async def replace_selection_text(self, text: str, instance_id: Optional[str] = None) -> Dict[str, Any]:
        ...
        try:
            # Escape the text for AppleScript - handle Chinese characters properly
            escaped_text = text.replace('"', '\\"').replace('\\', '\\\\').replace('\r', '\\r').replace('\n', '\\n')
            
            # Use the simpler approach: append newline directly to the replacement text
            applescript = f'''
            tell application "Microsoft Word"
                if (count of documents) > 0 then
                    try
                        ...
                        -- Set the content of the selection to the new text with explicit newline
                        set content of text object of currentSel to "{escaped_text}\\r"
