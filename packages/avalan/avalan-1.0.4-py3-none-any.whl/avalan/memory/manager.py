...
class MemoryManager:
    ...
    async def create_instance(
        ...
        *args,
        ...
        recent_memory = RecentMessageMemory() \
                        if with_recent_message_memory else None
    ...
    def recent_messages(self) -> Optional[list[EngineMessage]]:
        return self._recent_message_memory.data \
               if self._recent_message_memory else None
    ...
    async def append_message(self, engine_message: EngineMessage) -> None:
        assert isinstance(engine_message, EngineMessage) \
    ...
    async def continue_session(
        ...
        *args,
        ...
        if (
            ...
            self._permanent_message_memory and
            self._recent_message_memory
        ):
            messages = await \
                self._permanent_message_memory.get_recent_messages(
                    participant_id=self._participant_id,
                    session_id=session_id,
