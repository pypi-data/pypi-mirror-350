...
class GradioProviderConfig(BaseModel):
    ...
    api: Optional[GradioProviderApi] = Field(
        None, description="A list of APIs available for testing."
    )
    ...
    def validate_url_or_hf_space(cls, value: str) -> str:
        """
        Validates that the provided URL is either:
        - A well-formed URL
        - A valid Hugging Face Space identifier (org/app)

        Raises a ValueError if neither format is met.
        """
