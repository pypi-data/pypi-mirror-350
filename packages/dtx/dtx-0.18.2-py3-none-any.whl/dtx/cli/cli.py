...
from dtx.core.models.template.prompts.base import PromptsRepoType
...
class AgentScanner:
    ...
    def _parse_arguments(self):
        ...
        run_parser.add_argument(
            ...
            help="Choose an Agent (optional if plan provided)",
        )
    ...
    def _run_tests(self):
        ...
        agent_type, url = self._resolve_agent_and_url(plan=plan)

        agent = self._run_tests_initialize_agent(plan, agent_type, url)
        collector = self._run_tests_initialize_collector(self.args.no_rich)
        runner = self._run_tests_initialize_runner(
            agent_type, url, self.args.max_prompts, self.args.tactics
        )

        runner.run(plan=plan, agent=agent, collector=collector)
        if self.args.output:
            runner.save_yaml(self.args.yml)
            runner.save_json(self.args.json)
        else:
    ...
    def _redteam_interactive_quick(self):
        ...
        # Step 6: Save the output YAML
        runner.save_yaml(output_file)
