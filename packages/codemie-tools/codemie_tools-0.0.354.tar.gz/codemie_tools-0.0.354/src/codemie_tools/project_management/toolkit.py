...
class ProjectManagementToolkit(BaseToolkit):
    ...
    def get_tools(self) -> list:
        tools = []
        if self.jira_config:
            jira_tool = ProjectManagementToolkit.create_jira_tool(self.jira_config)
    ...
    @classmethod
    def create_jira_tool(cls, jira_config: JiraConfig) -> GenericJiraIssueTool:
        jira = Jira(
            url=jira_config.url,
        ...
        return GenericJiraIssueTool(jira=jira)
    ...
    def get_toolkit(cls, configs: Dict[str, Any]):
        ...
        return cls(jira_config=jira_config, confluence_config=confluence_config)
