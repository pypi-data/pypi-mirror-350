...
class FileUtils():
    def __init__(self, client: RestPkiClient, file_model: FileModel):
    ...
    def open(self, buff_size=4096):
        content_base64 = self.__file.content
        content_raw = Utils._base64_decode(content_base64)\
            if content_base64 is not None else None
        if content_raw is not None:
            stream = Utils._get_raw_stream(content_raw)
        else:
            stream = BytesIO()
            with self.__client.get_file_from_url(self.__file.url) as response:
                for chunk in response.stream(buff_size):
                    stream.write(chunk)
                stream.seek(0, 0)
        return stream
    ...
    def __get_content_base64(self):
        ...
        self.write_to(buffer)
        buffer.seek(0, 0)
        return Utils._base64_encode_string(buffer.read())
