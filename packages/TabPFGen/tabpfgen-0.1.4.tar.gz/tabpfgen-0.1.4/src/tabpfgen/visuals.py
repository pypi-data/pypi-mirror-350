...
def visualize_classification_results(
    X_orig, y_orig, X_synth, y_synth, feature_names=None
):
    """
    ...
    for c in classes:
        ...
        mask_orig = y_orig == c
        plt.scatter(
            X_tsne_orig[mask_orig, 0],
            X_tsne_orig[mask_orig, 1],
            alpha=0.6,
            marker="o",
            label=f"Original Class {c}",
        )
        # Synthetic data
        mask_synth = y_synth == c
        plt.scatter(
            X_tsne_synth[mask_synth, 0],
            X_tsne_synth[mask_synth, 1],
            alpha=0.6,
            marker="x",
            label=f"Synthetic Class {c}",
        )
    ...
    for i, label in enumerate(classes):
        ...
        plt.hist(
            ...
            color=plt.cm.Set3(i + len(classes)),
            density=True,
        )

    plt.title(
        f"Distribution of Top Feature:\n{feature_names[top_feature_idx]}",
        fontsize=10,
        pad=16,
    )
    plt.legend()
...
def visualize_regression_results(X_orig, y_orig, X_synth, y_synth, feature_names=None):
    ...
    plt.scatter(
        X_orig[:, top_feature_idx], y_orig, alpha=0.5, label="Original", color="blue"
    )
    plt.scatter(
        X_synth[:, top_feature_idx], y_synth, alpha=0.5, label="Synthetic", color="red"
    )
    ...
    plt.hist(
        residuals_orig, bins=30, alpha=0.5, label="Original", density=True, color="blue"
    )
    plt.hist(
        residuals_synth,
        bins=30,
