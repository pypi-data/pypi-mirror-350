...
def setup_api(agent_id: str, config_manager: ConfigManager, read: bool = True):
    ...
    config = config_manager.current
    if not config:
        raise RuntimeError(f"No configuration found for profile {config_manager.current_profile}. "
...
def resolve_agent_query(agent_query: str, api: Client):
    ...
    try:
        ...
        print("Tried to use fuzzy matching without fuzzywuzzy installed. "
    ...
    if len(matches) == 0:
        print(f"Could not resolve agent query: {agent_query}. Using default user agent ID.")
        return

    if len(matches) == 1 or len([m for m in matches if m[1] == 100]):
        agent_name, score = matches[0]
        # quick route, no menu required
        print(f"Using agent {agent_name} for API calls. (Query: {agent_query}, Score: {score}%)")
