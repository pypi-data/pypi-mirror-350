...
class CustomDataset(Dataset):
    ...
    def get_true_attributes(self):
        ...
        for file in self.attributes.files:
            ...
            flattened = [item for sublist in self.attributes[file].tolist() for item in sublist]
    ...
    def __getitem__(self, idx):
        ...
        # 将类别转换为标签
        attributes = self.attributes[class_name]
        label = self.class_to_label[class_name]

        return image, [torch.tensor(attribute).float() for attribute in attributes], label
...
if __name__ == '__main__':
    ...
    train_data = CustomDataset(img_dir=r'D:\Code\2-ZSL\Zero-Shot-Learning\data\B\seen\train',
                               att_path=r'D:\Code\2-ZSL\Zero-Shot-Learning\data\B\seen\seen_att.npz',
                               transform=transform)
    train_data.get_true_attributes()
    print()
