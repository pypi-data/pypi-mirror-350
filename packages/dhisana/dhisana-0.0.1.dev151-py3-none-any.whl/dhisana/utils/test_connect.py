...
async def _rpc(session, url, headers, method, params=None):
    ...
    return await session.post(url, headers=merged_headers, data=json.dumps(body))
...
async def test_mcp_server(
    ...
    """
    Sanity check for MCP server connectivity and basic capabilities.

    Steps:
    1. Attempt POST to tools/list at <base_url>/mcp.
    2. If it fails (e.g., 404/405/410), fetch .well-known/mcp-configuration and retry at discovered endpoint.
    3. Optionally test prompts/list (common MCP extension).
    ...
    """
    headers: Dict[str, str] = {header_name: header_value} if header_name and header_value else {}
    result: Dict[str, Any] = {
        "success": False,
        "endpoint": None,
        "tools": None,
    ...
    async with aiohttp.ClientSession(timeout=aiohttp.ClientTimeout(total=10)) as session:
        ...
        initial_endpoint = base_url

        # Step 1: Try tools/list directly
        resp = await _rpc(session, initial_endpoint, headers, "tools/list")
        if resp.status == 200:
            data = await _safe_json(resp) or {}
            result.update({
                "success": True,
                "endpoint": initial_endpoint,
                "tools": len(data.get("result", {}).get("tools", []))
            })
        else:
            # Step 2: Discover MCP endpoint from config
            config_url = urljoin(root_url, ".well-known/mcp-configuration")
            config_resp = await session.get(config_url, headers=headers)

            if config_resp.status != 200:
                result["detail"] = f"tools/list @ {initial_endpoint} → {resp.status}; config fetch @ {config_url} → {config_resp.status}"
                return result

            config = await _safe_json(config_resp) or {}
            discovered_endpoint = (
                config.get("endpoints", {}).get("mcp") or
                config.get("endpoint")  # fallback for older style
            )

            if not discovered_endpoint:
                result["detail"] = "MCP discovery document found but no 'mcp' endpoint specified"
                return result

            full_endpoint = urljoin(root_url, discovered_endpoint.lstrip("/"))
            resp2 = await _rpc(session, full_endpoint, headers, "tools/list")

            if resp2.status != 200:
                result["detail"] = f"tools/list @ {full_endpoint} → {resp2.status}"
                return result

            data = await _safe_json(resp2) or {}
            result.update({
                "success": True,
        ...
        # Step 3: Optionally try prompts/list
        if result["success"]:
            prompts_resp = await _rpc(session, result["endpoint"], headers, "prompts/list")
            if prompts_resp.status == 200:
                pdata = await _safe_json(prompts_resp) or {}
                result["prompts"] = len(pdata.get("result", {}).get("prompts", []))
    ...
    for tool in tool_config:
        ...
        if tool_name == "mcpServer":
            ...
            if not server_url or not header_name or not header_value:
                ...
                results[tool_name] = await test_mcp_server(server_url, header_name, header_value)
            continue
