...
def archive_image(metadata_id: int) -> None:
    ...
    if not img_metadata["archived"]:
        ...
        raise RuntimeError(f"Image with metadata ID {metadata_id} is already archived.")
...
def create_image_metadata(
    ...
    ), f"""Can only create metadata for images located in {
        ", ".join([x for x in list(ALLOWED_OCI_SOURCE_TYPES_OPERATIONS.keys())])
    }"""
    ...
    payload = {
        ...
        "config": {
            "ADABOARD_API_URL": {
                ...
                "real_value": adalib_config.SERVICES["adaboard-api"]["external"],
            },
            ...
            "HARBOR_TARGET_REPOSITORY_NAME": {
                ...
                "real_value": response_metadata["oci_image_name"]["repository"],
            },
            "HARBOR_TARGET_TAG": {
...
def get_image_metadata_id(project_name: str, repository_name: str, tag: str) -> int:
    """
...
def get_project_stats(project_name: str) -> list[dict[str, str | int]]:
    ...
    return [{"repository": x["name"], "count": x["image_amount"]} for x in all_images]
...
def get_publish_logs(metadata_id: int, source: str = "publish") -> list[dict[str, str]]:
    """
    ...
    assert source in ALLOWED_PUBLISH_LOG_SOURCES, (
    ...
    try:
        ...
        publish_id = [x["run_id"] for x in mdata if x["metadata_id"] == metadata_id][0]
    except IndexError:
        raise ValueError(
...
def get_repositories_by_type(type_id: str, include_archived: bool = False) -> list[str]:
    """
...
def request_harbor(
    ...
    **kwargs,
    ...
    constructed_url = f"{adalib_config.SERVICES['harbor']['url']}/api/v2.0/{path}"
...
def restore_image(metadata_id: int) -> None:
    ...
    if img_metadata["archived"]:
        ...
        raise RuntimeError(f"Image with metadata ID {metadata_id} is already visible.")
