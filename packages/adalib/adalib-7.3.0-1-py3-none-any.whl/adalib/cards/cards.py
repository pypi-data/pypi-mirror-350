...
def _create_keywords_if_not_exist(keywords: list[str]) -> dict[str, int]:
    ...
    all_kws = {
        ...
        for x in adaboard.request_adaboard(path="keywords", method=requests.get).json()
    }
    for kw in keywords:
...
def create_card(  # noqa: C901
    ...
    assert cardtype.lower() in ALLOWED_CARD_TYPES, (
        "Card type must be 'notebook' or 'voila'."
    )
    assert notebook_file.lower().endswith(".ipynb"), (
        "Notebook file must be a .ipynb file."
    )
    assert source.lower() in ALLOWED_CARD_SOURCES, (
        "Files source must be 'lab' or 'local'."
    )
    validate_acl("card", False, acl_type_view, acl_type_launch, acl_type_edit)
    assert all(issubclass(type(x), dict) for x in aux_files), (
        "Auxiliary files must be a list of dictionaries."
    )
    assert all(("source" in d and "target" in d) for d in aux_files), (
    ...
    if source.lower() == "lab":
        ...
        assert props["size"] <= MAX_FILE_SIZE, (
        ...
        assert os.path.getsize(notebook_file) <= MAX_FILE_SIZE, (
            f"Notebook file size exceeds the maximum allowed ({MAX_FILE_SIZE} bytes). Please reduce the size of the file and try again."
        )
    ...
    if not include_kernel:
        nb_file_dict = json.loads(base64.b64decode(nb_file_bin).decode("utf-8"))
        nb_file_dict["metadata"]["kernelspec"] = {
            "display_name": "",
            "name": "",
        }
        nb_file_bin = base64.b64encode(json.dumps(nb_file_dict).encode("utf-8")).decode(
            "utf-8"
        )
    ...
    if aux_files[0]["source"]:
        ...
        if source.lower() == "lab":
            ...
            for aux_file in all_aux_files:
                # Check file size before actually pulling it from the Lab
                assert aux_file["size"] <= MAX_FILE_SIZE, (
                ...
                aux_file_name = (
                    f"{aux_file['target']}{os.path.basename(aux_file['source'])}"
                )
                tmp = {
        ...
        # In the local machine, paths are absolute; nesting is allowed
            ...
            for aux_file in all_aux_files:
                ...
                assert os.path.getsize(aux_file["source"]) <= MAX_FILE_SIZE, (
                    f"Auxiliary file size exceeds the maximum allowed ({MAX_FILE_SIZE} bytes). Please reduce the size of the file and try again."
                )
    ...
    acl_config = [
        ...
        },
        {
            ...
            "userlist": (acl_list_launch if acl_type_launch == "userlist" else []),
            "grouplist": (acl_list_launch if acl_type_launch == "grouplist" else []),
        },
        {
...
def create_card_group(
    ...
    for card_id in cards_id:
        group_cards.append({"card_id": card_id, "order_id": cards_id.index(card_id)})
...
def create_url_card(
    ...
    try:
        ...
        url_subtype_id = [
            x["urlcardsubtype_id"] for x in url_subtypes if x["name"] == url_subtype
        ][0]
    except IndexError as e:
        raise IndexError(
...
def edit_card(  # noqa: C901
    ...
    :rtype: None
    """
    assert cardtype is None or cardtype.lower() in ALLOWED_CARD_TYPES, (
        "Card type must be 'notebook' or 'voila'."
    )
    assert notebook_file is None or notebook_file.lower().endswith(".ipynb"), (
        "Notebook file must be a .ipynb file."
    )
    assert source.lower() in ALLOWED_CARD_SOURCES, (
        "Files source must be 'lab' or 'local'."
    ...
    assert aux_files is None or all(issubclass(type(x), dict) for x in aux_files), (
        "Auxiliary files must be a list of dictionaries."
    )
    ...
    acl_config = [
        {
            "acl_action": "card_view",
            "acl_type": (
                acl_type_view if acl_type_view else old_config["acls"][0]["acl_type"]
            ),
            "userlist": (
                acl_list_view if acl_list_view else old_config["acls"][0]["userlist"]
            ),
            "grouplist": (
                acl_list_view if acl_list_view else old_config["acls"][0]["grouplist"]
            ),
        },
        {
            "acl_action": "card_launch",
        ...
        },
        {
            "acl_action": "card_edit",
            "acl_type": (
                acl_type_edit if acl_type_edit else old_config["acls"][2]["acl_type"]
            ),
            "userlist": (
                acl_list_edit if acl_type_edit else old_config["acls"][2]["userlist"]
            ),
            "grouplist": (
                acl_list_edit if acl_type_edit else old_config["acls"][2]["grouplist"]
            ),
        },
    ]
    ...
    old_coauthors = [coauthor["user_id"] for coauthor in old_config["coauthors"]]
    old_config["coauthors"] = old_coauthors
    old_reviewers = [reviewer["user_id"] for reviewer in old_config["reviewers"]]
    ...
    if notebook_file:
        ...
        content = [x for x in old_config["content"] if x["contenttype_id"] == 1]

    # Next, the auxiliary files
    # Start by collecting or ditching the old ones
    if keep_aux_files:
        old_aux_files = [x for x in old_config["content"] if x["contenttype_id"] == 2]
    ...
    payload = {k: v if v is not None else old_config[k] for k, v in card_config.items()}
    adaboard.request_adaboard(f"cards/{card_id}", method=requests.put, json=payload)
    return None
...
def edit_card_group(
    ...
    # Build request payload combining new and old options
    payload = {
        k: v if v is not None else old_config[k] for k, v in card_group_config.items()
    }
    adaboard.request_adaboard(
        path=f"cards/{group_id}",
        method=requests.put,
...
def edit_url_subtype(
    ...
    old_config = [x for x in tmp if x["urlcardsubtype_id"] == url_subtype_id][0]

    # Build request payload combining new and old options
    payload = {
        k: v if v is not None else old_config[k] for k, v in url_subtype_config.items()
    }
    adaboard.request_adaboard(
...
def get_cards(card_type: str = "all") -> list[list[int | str]]:
    ...
    """
    card_types = [
        x.get("key") for x in adaboard.request_adaboard(path="cardtypes").json()
    ]
    card_types.append("all")
    assert card_type in card_types, f"Card type must be one of {', '.join(card_types)}"
...
def launch_card(card_id: int, start_lab: bool = True, lab_config: dict = {}) -> str:
    """
    ...
    if lab_config:
        assert list(lab_config.keys()) == ["options"] or list(lab_config.keys()) == [
            "profile_id"
        ], (
        ...
        if list(lab_config.keys()) == ["options"]:
            ...
            for id in lab_config["options"]:
                assert id in config_opts_ids, "Invalid configuration option ID."
            ...
            assert lab_config["profile_id"] in config_profiles_ids, (
    ...
    nb_file_dict = json.loads(base64.b64decode(response["content"]).decode("utf-8"))

    # Check if the kernel is custom
    kernel_pattern = re.compile(r"^(kp|kt)(\d+)$")
    kernel_match = kernel_pattern.match(nb_file_dict["metadata"]["kernelspec"]["name"])
    if kernel_match:
        ...
        if kernel_match.group(1) == "kp":
            ...
            try:
                [x for x in kernels if x["metadata_id"] == kernel_metadata_id][0]
            except IndexError:
        ...
        if id_dict:
            payload = {
                "display_name": nb_file_dict["metadata"]["kernelspec"]["display_name"],
...
def toggle_card_review(card_id: int) -> None:
    ...
    adaboard.request_adaboard(path=f"cards/{card_id}/review", method=requests.put)
