...
def create_schedule(
    ...
    assert card_id or notebook_file, "Either card_id or notebook_file must be provided."
    if notebook_file:
    ...
    content = {}
    if card_id:
        tmp = adaboard.request_adaboard(f"cards/{card_id}?incl_content=true").json()
...
def edit_schedule(  # noqa: C901
    ...
    old_acls = [
        {
            ...
            "userlist": [
                y["user_id"] for y in x["acl"]["userlist"] if "user_id" in y.keys()
            ],
            "grouplist": [
                y["user_id"] for y in x["acl"]["grouplist"] if "user_id" in y.keys()
            ],
        }
        for x in tmp["acls"]
    ]
    ...
    acl_config = [
        ...
        },
        {
            "acl_action": "schedule_view_logs",
            "acl_type": (
                acl_type_logs if acl_type_logs else old_config["acls"][1]["acl_type"]
            ),
            "userlist": (
                acl_list_logs if acl_list_logs else old_config["acls"][1]["userlist"]
            ),
            "grouplist": (
                acl_list_logs if acl_list_logs else old_config["acls"][1]["grouplist"]
            ),
        },
        {
            "acl_action": "schedule_edit",
            ...
            "userlist": (
                acl_list_edit if acl_list_edit else old_config["acls"][2]["userlist"]
            ),
            "grouplist": (
                acl_list_edit if acl_list_edit else old_config["acls"][2]["grouplist"]
            ),
        },
        {
            "acl_action": "schedule_edit_privileged",
    ...
    payload = {
        k: v if v is not None else old_config[k] for k, v in schedule_config.items()
    }
...
def get_card_schedules(card_id: int, include_inactive: bool = False) -> list[dict]:
    """
    ...
    cards_schedules = [
        __extract_schedule_data(x) for x in all_schedules if x["card_id"] == card_id
    ]
...
def get_runs_overview(schedule_id: int = None, owner_id: int = None) -> list[dict]:
    """
...
def get_user_schedules(user_id: str = "", include_inactive: bool = False) -> list[dict]:
    """
