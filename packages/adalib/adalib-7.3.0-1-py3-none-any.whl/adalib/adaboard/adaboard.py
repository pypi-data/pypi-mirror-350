...
def build_request_url(path: str) -> str:
    ...
    url_scheme = "https://" if adalib_config.ENVIRONMENT == "external" else "http://"
    api_extension = "/adaboard/api" if adalib_config.ENVIRONMENT == "external" else ""
    return (
        url_scheme
...
def get_user(
    ...
    if include_notification_preferences:
        notification_preferences = request_adaboard(path="nosy/preferences/").json()
        notification_preferences = [
            _unravel_notification(x) for x in notification_preferences["preferences"]
        ]
