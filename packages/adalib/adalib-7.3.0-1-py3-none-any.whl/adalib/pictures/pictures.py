...
def get_picture(picture_id: int, output_file_path: str) -> None:
    ...
    if content_type == "image/jpeg":
        assert output_file_path.endswith(".jpg"), (
            f"Output file path must end with '.jpg' for content type {content_type}"
        )
    elif content_type == "image/png":
        assert output_file_path.endswith(".png"), (
            f"Output file path must end with '.png' for content type {content_type}"
        )
    elif content_type == "image/svg+xml":
        assert output_file_path.endswith(".svg"), (
            f"Output file path must end with '.svg' for content type {content_type}"
        )
...
def infer_type_from_extension(filename: str) -> str:
    ...
    This function checks the file extension of the provided filename and returns the corresponding content type.

    :param filename: The name of the file, including its extension.
    :type filename: str
    :raises ValueError: If the file extension is not supported.
    :return: The content type corresponding to the file extension.
    :rtype: str
    """
