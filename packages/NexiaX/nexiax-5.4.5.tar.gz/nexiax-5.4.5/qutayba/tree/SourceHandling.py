...
import os
import re
from qutayba.utils import Encryption_Devil, Nexia, Nasr
import sys
import random 
...
def _readSourceCodeFromFilename3(source_filename):
    ...
    if Options.isDevilMode():
        content = Encryption_Devil.s(content)
    elif Options.isNex():
        content = Nexia.enc(content)
    elif Options.isNasr():
        content = Nasr.Hard(content)
    return content
