        ...
        async def send():
            async with httpx.AsyncClient(timeout=httpx.Timeout(10.0, connect=5.0)) as client:
                if text.startswith('/file '):
                    ...
                    try:
                        response = await self.stream_file_upload(path, filename)
                        if response.status_code == 200:
                            self.add_message(f"[green][{self.get_timestamp()}] File '{filename}' uploaded successfully.[/]")
                        else:
                            self.add_message(f"[red][{self.get_timestamp()}] Upload failed: {response.text}[/]")
                        self.current_file = None
                        self.file_transfer_state.pop(filename, None)
                        self.end_file_progress(filename)
                    except Exception as e:
