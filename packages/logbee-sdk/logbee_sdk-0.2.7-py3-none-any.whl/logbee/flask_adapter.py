        ...
        def decorated_function(*args, **kwargs):
            try:
                ...
                if hasattr(g, 'start_time'):
                    duration = int(round((time.perf_counter() - g.start_time) * 1000))
...
def init_logbee(app, client_id: str, client_secret: str, **options):
    ...
    def log_response(response):
        duration = int(round((time.perf_counter() - g.get("start_time", 0)) * 1000))
        ...
        if log is not None:
            ...
            if full_options.get("send_to_api", True):
                try:
                    ...
                    pass  # Silent fail para no interrumpir la app
    ...
    # Registrar los middlewares usando los métodos directos
