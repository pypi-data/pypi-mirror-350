...
def init_logbee(app, client_id: str, client_secret: str, **options):
    ...
    def log_response(response):
        ...
        # Debug logging
        print(f"🔍 DEBUG LOGBEE:")
        print(f"   Status: {response.status_code}")
        print(f"   send_to_api: {full_options.get('send_to_api', True)}")
        print(f"   capture_all_requests: {full_options.get('capture_all_requests', True)}")
        print(f"   min_status_code_to_capture: {full_options.get('min_status_code_to_capture', 0)}")
        print(f"   client_id: {full_options.get('client_id', 'NOT_SET')[:10]}...")
        print(f"   Log body: {log.body is not None}")
        print(f"   Log headers: {log.headers is not None}")
        ...
        if full_options.get("send_to_api", True):
            ...
            print(f"   ⏸️  send_to_api está deshabilitado")
