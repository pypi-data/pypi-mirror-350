...
def build_log_data(
    ...
    if options.get("mask_sensitive_data"):
    ...
    return LogData(
        ...
        body=body if options.get("capture_body") else None,
        headers=raw_data.get("headers") if options.get("capture_headers") else None,
        query=raw_data.get("query") if options.get("capture_query_params") else None,
...
def send_log_to_api(log: LogData, client_id: str = None, client_secret: str = None, timeout: int = 30):
    """
...
def send_error_to_logbee(error: Exception, context: Dict[str, Any] = None, client_id: str = None, client_secret: str = None, timeout: int = 30):
    """
    ...
    # Send to API
    return send_log_to_api(log_data, client_id=client_id, client_secret=client_secret, timeout=timeout)
