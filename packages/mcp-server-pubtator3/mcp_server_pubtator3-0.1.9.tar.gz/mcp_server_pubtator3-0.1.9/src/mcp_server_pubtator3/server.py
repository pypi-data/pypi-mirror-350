...
""",
        ...
        types.Tool(
            name="get_paper_text",
            description="""Retrieve text of an article from Pubtator Central using either a PMID or PMCID.

Full texts are available in BioC XML or BioC JSON formats.
If the requested paper is not available in full text, the system responds with a message explaining why (e.g., not in PMC, access restricted).
Input parameters:
- pmid: A PubMed ID (PMID) to retrieve an article.
- pmcid: A PubMed Central ID (PMCID) to retrieve an article.
...
Returns:
- The full text in the selected format, or
...
""",
            inputSchema={
                ...
                "properties": {
                    "pmid": {
                        "type": "string",
                        "description": "A single PubMed ID (PMID)."
                    },
                    "pmcid": {
                        "type": "string",
                        "description": "A single PubMed Central ID (PMCID)."
                    },
                    "format": {
                        "type": "string",
