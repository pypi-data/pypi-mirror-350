...
class PubtatorClient:
    ...
    async def find_entity(self, query:str, bioconcept:Optional[str]=None, limit:int=10):
        """
    ...
    async def search_pubtator(self, query: str, relation: Optional[str] = "ANY", limit: int = 10):
        """
    ...
    async def get_paper_text(
        ...
        """
        get the abstract or full text of a publication in a specified format.
        ...
        if pmid and pmcid:
            raise ValueError("Only one of pmid or pmcid should be supplied.")

        if not pmid and not pmcid:
            raise ValueError("Either pmid or pmcid must be provided.")

        if pmid:
            url = f"{self.base_url}publications/export/{format}?pmids={pmid}"
            if full is not None and format in ("biocxml", "biocjson"):
                if full:
                    url += f"&full=true"
        else:
            url = f"{self.base_url}publications/pmc_export/{format}?pmcids={pmcid}"
