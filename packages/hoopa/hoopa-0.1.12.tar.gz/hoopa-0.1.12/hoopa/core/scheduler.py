...
class Scheduler:
    ...
    async def add(self, requests: typing.Union[Request, typing.List[Request]]):
        ...
        for request in requests:
            if request.dont_filter:
                request_stats[request.priority] = (
                    request_stats.get(request.priority, 0) + 1
                )
    ...
    async def set_result(
        ...
        await self.stats.inc_value(
            f"queue/response_count/priority_{request.priority}/{response.ok}", 1
        )

    async def check_scheduler(self, spider_ins):
        await self.scheduler_queue.check_status(
            spider_ins, self.engine.spider.setting["RUN_FOREVER"]
        )
