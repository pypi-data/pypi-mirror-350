...
class SpiderMiddleware(MiddlewareManager):
    ...
    def _get_mw_list_from_engine(cls, engine):
        return engine.setting.get("SPIDER_MIDDLEWARES") + engine.setting.get(
    ...
    def _add_middleware(self, mw):
        ...
        if hasattr(mw, "process_response") and callable(
            getattr(mw, "process_response")
        ):
            self.methods["process_response"].appendleft(mw.process_response)
            self.names["process_response"].appendleft(mw.__class__.__name__)

        if hasattr(mw, "process_exception") and callable(
            getattr(mw, "process_exception")
        ):
    ...
    async def process_input(self, request: Request, response: Response, spider_ins):
        for method in self.methods["process_request"]:
            middleware_response = await run_function(
                method, request, response, spider_ins
            )
            if response is not None and not isinstance(middleware_response, bool):
                raise Exception(
                    f"<Middleware {method.__name__}: must return None, got {type(response)}"
                )
    ...
    async def process_output(
        self, request: Request, response: Response, result, spider_ins
    ):
        for method in self.methods["process_response"]:
            middleware_response = await run_function(
                method, request, response, result, spider_ins
            )
            if middleware_response is not None and not isinstance(
                middleware_response, (Response, Request)
            ):
                raise InvalidOutput(
                    f"<Middleware {method.__name__}: must return None, Request or Item, "
                    f"got {type(response)}"
                )
    ...
    async def process_exception(self, request, response, error, spider_ins):
        for method in self.methods["process_exception"]:
            middleware_response = await run_function(
                method, request, response, error, spider_ins
            )
            if middleware_response is not None and not isinstance(
                middleware_response, bool
            ):
                raise InvalidOutput(
        ...
        try:
            ...
            if results:
                ...
                for result in results:
                    process_response_result = await self.process_output(
                        request, response, result, spider_ins
                    )
            ...
            await self.process_exception(
                request, response, Error(e, traceback.format_exc()), spider_ins
            )
