...
from hoopa.exceptions import Error, InvalidCallback, InvalidCallbackResult
...
from hoopa.utils.concurrency import (iterate_in_threadpool, run_function,
                                     run_function_no_concurrency)
from hoopa.utils.helpers import (create_instance_and_init, get_timestamp,
                                 load_object, split_list)
...
class Engine:
    ...
    async def _process_async_callback(
        self, request: Request, response: Response, callback_results: AsyncGeneratorType
    ):
        ...
        async for callback_result in callback_results:
            if isinstance(callback_result, Request):
                ...
                await self.pipeline_manager.process_pipelines(
                    request, response, callback_result, self.spider
                )
            elif isinstance(callback_result, list) and all(
                isinstance(item, Item) for item in callback_result
            ):
                # 如果是item list，也进行处理
                await self.pipeline_manager.process_pipelines(
                    request, response, callback_result, self.spider
    ...
    async def _process_callback(self, request, response):
        ...
        try:
            if process_func is not None:
                callback_results = await self.spider_middleware.scrape_response(
                    process_func, request, response, self.spider
                )
                ...
                raise Exception(
                    f"<Parse invalid callback result type: {request.callback}>"
                )
        except Exception as e:
            response.ok = 0
            response.error = Error(e, traceback.format_exc())
            logger.error(
                f"{request} {response} callback error \n{response.error.stack}"
            )
    ...
    async def handle_download_callback(self, request: Request):
        ...
        # 加载request中间件, 并调用下载器
        response = await self.downloader_middleware.download(
            self.downloader.fetch, request, self.spider
        )
    ...
    async def start_worker(self):
        ...
        while True:
            ...
            asyncio.run_coroutine_threadsafe(
                self._process_task(request_item), loop=self.loop
            )
    ...
    async def consumer(self):
        ...
        while self.spider.run:
            ...
            # 休眠
            sleep_second = max(
                self.spider.download_delay - (time.time() - last_time), 0.01
            )
    ...
    async def finish(self):
        ...
        spider_stats_sorted_keys = sorted(
            spider_stats.items(), key=operator.itemgetter(0)
        )
        blank = " " * 4
        body = "\nstats\n"
