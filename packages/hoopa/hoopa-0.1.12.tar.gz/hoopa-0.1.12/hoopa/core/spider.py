...
from hoopa.settings import Setting, const
...
class RedisSpider(BaseSpider, ABC):
    ...
    async def open_spider(self):
        ...
        self.setting.set(
            "DUPEFILTER_SETTING", self.setting.get("REDIS_SETTING"), "default"
        )
    ...
    async def process_item(self, item: Item):
        ...
        with await self.redis_pool as conn:
            result = await conn.hmset(
                f"{self.name}:{item.item_name}", item_md5, item_str_json
            )
        if result:
