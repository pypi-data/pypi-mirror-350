...
class PipelineManager(MiddlewareManager):
    ...
    async def process_item(self, item, spider_ins):
        ...
        for method in methods:
            ...
            if not isinstance(_item, Item) and (
                not (
                    isinstance(_item, list)
                    and all(isinstance(item, Item) for item in _item)
                )
            ):
                raise InvalidOutput(
    ...
    async def process_exception(self, request, response, item, exception, spider_ins):
        for method in self.methods["process_exception"]:
            middleware_response = await run_function(
                method, request, response, item, exception, spider_ins
            )
    ...
    async def process_pipelines(self, request, response, item, spider_ins):
        ...
        try:
            ...
            return await self.process_exception(
                request, response, item, Error(e, traceback.format_exc()), spider_ins
            )
