...
class DownloaderMiddleware(MiddlewareManager):
    ...
    def _get_mw_list_from_engine(cls, engine):
        return engine.setting.get("DOWNLOADER_MIDDLEWARES") + engine.setting.get(
    ...
    async def process_request(self, request: Request, spider_ins):
        ...
        for method in methods:
            ...
            if response is not None and not isinstance(response, (Response, Request)):
                raise InvalidOutput(
                    f"<Middleware {method.__name__}: must return None, Response or Request"
                )
            if response:
                return response
    ...
    async def process_exception(self, request, exception, spider_ins):
        for method in self.methods["process_exception"]:
            middleware_response = await run_function(
                method, request, exception, spider_ins
            )
            if middleware_response is not None and not isinstance(
                middleware_response, (Response, Request)
            ):
    ...
    async def download(self, download_func, request, spider_ins):
        ...
        if not response:
            ...
            try:
                ...
                return await self.process_exception(
                    request, Error(e, traceback.format_exc()), spider_ins
                )

        if response:
            # 加载response中间件
            process_response_result = await self.process_response(
                request, response, spider_ins
            )
