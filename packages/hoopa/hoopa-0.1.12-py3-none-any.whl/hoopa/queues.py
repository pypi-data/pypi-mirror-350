...
from hoopa.utils.helpers import get_mac_pid, get_priority_list, get_timestamp
...
class BaseQueue:
    ...
    async def set_result(
        self, request: Request, response: Response, task_request: Request
    ):
        """
        保存结果
        @param request:
...
class MemoryQueue(BaseQueue):
    ...
    async def clean_scheduler(
        self, waiting=True, pending=True, failure=True, data=True
    ):
        """
        清空队列
        """
...
class RedisQueue(BaseQueue):
    ...
    async def set_heart_beat(self):
        ...
        while True:
            ...
            if run_time:
                ...
                requests_per_second_10s = round(
                    (requests_count - last_time_requests_count) / 10, 1
                )
        ...
        try:
            ...
            with await self.pool as conn:
                for p_item in priority_list:
                    _min, _max = p_item
                    eval_result = await conn.eval(
                        lua,
                        keys=[self._waiting_key, self._pending_key, _min, _max],
                        args=[],
                    )
                    if eval_result:
                        self.task_count += 1
                        return Request.unserialize(
                            eval_result, self.serialization_module
                        )
    ...
    async def add(self, requests):
        ...
        """
        with await self.pool as conn:
            add_counts = await conn.eval(
                lua, keys=[self._spider_name] + priority_list, args=str_requests
            )
        return add_counts
        ...
        if now_time - self._last_check_pending_task_time > 10:
            ...
            with await self.pool as conn:
                ...
                if to_waiting_list:
                    ...
                    logger.info(
                        f"pendings: {len(pending_list)}, del_pending: {result[1]}, add_waitings: {result[0]}"
                    )
