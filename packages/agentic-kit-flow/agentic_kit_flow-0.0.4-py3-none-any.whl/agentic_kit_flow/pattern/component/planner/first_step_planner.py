...
class FirstStepCotPlanner(CotPlanner):
    """根据任务，生成第一步planner"""

    default_prompt = '''
# 你是一个任务调度分配专家
1. 对于以下任务，请制定第一步需要做什么
2. 你可以将证据或结果存储到变量#E中，以便后续工具调用。（计划，#E1，计划，#E2，计划，……）
3. 对于那些不需要借助外部工具tool来解决的问题，你可以自己回答
4. 每个步骤step要尽量的详细和尽量小工作量
5. 每个步骤step要尽量的详细描述任务内容，需要调用的工具tool和调用参数
6. 每个plan字段要尽量的详细描述任务内容，需要调用的工具tool和调用参数

# 可供调用的工具可以是以下几种之一，工具调用请严格遵守参数要求：
{tools}

# 回答输出格式要求：
## 您必须仅输出整个计划的第一步，格式为符合以下模式的 YAML 字典：
plans:
  - step_name: "Step#1"
    plan: "first plan"
...
* 输出：
    plans:
      - step: "鉴于托马斯工作了x小时，将问题转化为代数表达式，并通过WolframAlpha求解."
