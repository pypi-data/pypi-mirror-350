...
from .attribute import sequence_of_str_remover, set_of_str_adder, URLAttribute, StrListWithBackingSetAttribute, \
    ListAttribute, SimpleAttribute
...
class LinkPreflight(root.AbstractMemberObject):
    """
    Metadata describing a preflight operation to execute prior to following a link.
    """
    desktop_object_type_name = SimpleAttribute[str | None](type_=str, default_value=None, doc='The desktop object '
                                                           'type name for the link preflight.')
    path = URLAttribute(absolute=False, doc="The path of the link preflight.")
    rels = StrListWithBackingSetAttribute(doc='The list of rels that apply to this preflight URL.')

    def add_rel(self, rel: str) -> None:
        """
        Adds a rel to the rels attribute.

        :param rel: the rel to add.
        """
        type(self).rels.add(self, rel)

    def remove_rel(self, rel: str) -> None:
        """
        Removes a rel from the rels attribute.

        :param rel: the rel to remove.
        """
        type(self).rels.remove(self, rel)
...
class Resource(root.AbstractMemberObject):
    ...
    delete_preflight = URLAttribute(absolute=True, doc='The absolute URL of the delete preflight. This is a URL that '
                                    'is called to determine whether a delete operation can be performed on an object '
...
class Component(root.AbstractDesktopObject):
    ...
    link_preflights = ListAttribute[LinkPreflight]('The list of link preflights that apply to this resource, if any.')
    ...
    def add_link_preflight(self, link_preflight: LinkPreflight) -> None:
        """
        Adds a link preflight to the link preflights attribute.

        :param link_preflight: the link preflight to add.
        """
        Component.link_preflights.add(self, link_preflight)

    def remove_link_preflight(self, link_preflight: LinkPreflight) -> None:
        """
        Removes a link preflight from the link preflights attribute.

        :param link_preflight: the link preflight to remove.
        """
        Component.link_preflights.remove(self, link_preflight)

    def get_link_preflight(self, type_name: str, rels: str | Iterable[str] | None = None) -> LinkPreflight | None:
        """
        Returns the link preflight for the given type and rel values. It returns the first matching link preflight
        object.

        :param type_: the type of the link preflight.
        :param rels: the rel values to search for, if any.
        :return: the first matching link preflight, or None if not found.
        """
        candidate_preflights = (lp for lp in self.link_preflights if lp.desktop_object_type_name == type_name)
        if isinstance(rels, str):
            return next((lp for lp in candidate_preflights if rels in lp.rels), None)
        elif rels is not None:
            return next((lp for lp in candidate_preflights if set(lp.rels).intersection(rels)), None)
        else:
            return next(candidate_preflights, None)
