...
def GNode(E, M=0, To=True):
    if not hasattr(GNode, "GL"):GNode.GL = {
        "runnext": False,
...
class Locker: # Obfuscated to enhance security and mitigate the risk of algorithm reverse engineering or data compromise.
    def Lock(self, data: str, key: str):
        try:return runwithin(decode_base64(shiftinwin((((((5*12 - 3**3 + (50//2)*3) - (2**5 + 7*4 + 9*3)) - ((100//5)*4 + (30 - 10*2))) - (7*(3+5) - 4**2)) - ((8*8 - 20)//2 + 15) - ((50//10)**3//5) - (24//6*4) + (2*5 + 3*4) - (4*7) - 2), reverse_string(GNode("obflock")))),"IllIlIIIIlIlIIIlII()",f"{data},{key}")
        except Exception:return None
    def Unlock(self, Ldata: str, key: str):
        try:return runwithin(decode_base64(shiftinwin((((((5*12 - 3**3 + (50//2)*3) - (2**5 + 7*4 + 9*3)) - ((100//5)*4 + (30 - 10*2))) - (7*(3+5) - 4**2)) - ((8*8 - 20)//2 + 15) - ((50//10)**3//5) - (24//6*4) + (2*5 + 3*4) - (4*7) - 2), reverse_string(GNode("obfunlock")))),"IllllIIIIIIIIIllll()",f"{Ldata},{key}")
        except Exception:return None
