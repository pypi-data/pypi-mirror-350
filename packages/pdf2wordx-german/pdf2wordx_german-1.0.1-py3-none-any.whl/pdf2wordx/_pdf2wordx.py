...
if module_file is None:
    raise RuntimeError("Module __file__ attribute is missing.")
BASE_PATH = os.path.dirname(module_file)
ICO_PATH = os.path.join(BASE_PATH, "favicon.ico")
LOG_PATH = os.path.join(os.path.expanduser("~"), "pdf2wordx-german.log")
...
class App(win.Window):
    def __init__(self, root: Tk, width: int = 300, height: int = 300, bgColor: str = 'green', title: str = 'Window', resizable: list = [False, False]) -> None:
        ...
        self.width = width
        self.root = root
        ico_path = os.path.join(os.path.dirname(sys.modules[__name__].__file__), 'favicon.ico')
