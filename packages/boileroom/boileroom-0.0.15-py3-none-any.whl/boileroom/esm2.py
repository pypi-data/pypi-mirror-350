...
class ESM2(EmbeddingAlgorithm):
    ...
    def __init__(self, config: dict = {}) -> None:
        ...
        self._residue_index: Optional[torch.Tensor] = None
        self._chain_index: Optional[torch.Tensor] = None
    ...
    def embed(self, sequences: Union[str, List[str]]) -> ESM2Output:
        ...
        if any(":" in seq for seq in sequences):
            ...
            self._linker_map, self._residue_index, self._chain_index = self._store_multimer_properties(
                sequences, glycine_linker
            )
            ...
            tokenized["attention_mask"] = (self._linker_map == 1).to(torch.int32)
        else:
            # Monomer logic
            tokenized = self.tokenizer(
                sequences,
    ...
    def _store_multimer_properties(
        sequences: List[str], glycine_linker: str
        ...
        return linker_map, residue_index, chain_index
    ...
    def _convert_outputs(
        ...
        if self._linker_map is not None:
            ...
            result = self._mask_linker_region(embeddings, hidden_states, self._chain_index, self._residue_index)
            embeddings, hidden_states, self._chain_index, self._residue_index = result
        else:  # only MONOMERs
            self._chain_index = np.zeros((embeddings.shape[0], embeddings.shape[1]), dtype=np.int32)
        ...
        return ESM2Output(
            ...
            residue_index=self._residue_index,
        )

    def _mask_linker_region(
        self, embeddings: np.ndarray, hidden_states: np.ndarray, chain_index: np.ndarray, residue_index: np.ndarray
