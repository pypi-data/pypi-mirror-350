"""DH MCP server module for providing MCP tools for the DH API."""
...
logger = get_logger("DH MCP Server")

mcp = FastMCP("DH MCP", log_level="INFO")
...
async def getVoices(
        isSys: Annotated[Optional[bool], Field(description="是否是系统音色，true获取系统音色，false获取克隆音色, 空查询所有音色", default=None)]
) -> MCPVoicesResponse:
    """
...
async def getFigures(
        systemFigure: Annotated[Optional[bool], Field(description="是否是平台公共人像，true返回平台公共人像，false返回定制人像，空查询全部", default=None)]
) -> MCPFiguresResponse:
    """
    ...
    """
    logger.info(f'getFigures {systemFigure}')
    try:
        client = await getDhClient()
...
async def uploadFiles(
    ...
    Return:
        fileID: 文件ID
    """
    logger.info(f'uploadFiles {localFilePath}, {providerType}, {fileName}')
    try:
        client = await getDhClient()
...
async def generateDh123Video(
    ...
    Args:
        ...
        title: 标题
    """
    logger.info(f'generateDh123Video {inputText} {voiceId} {videoUrl} {title}')
    try:
        ...
        logger.info(f'generateDh123Video error')
...
async def getDh123VideoStatus(
    ...
    Args:
        taskId: 视频任务ID
    """
    logger.info(f'getDh123VideoStatus {taskId}')
    try:
        client = await getDhClient()
        logger.info(f'getDh123VideoStatus {taskId}')
        ...
        logger.info(f'getDh123VideoStatus error')
...
async def generateDhVideo(
    ...
    """
    try:
        logger.info(f'generateDhVideo {figureId} {voiceId} {driveType}')
        ...
        logger.info(f'generateDhVideo error')
        return MCPVideoGenerateResponse(error=str(e))
...
async def getDhVideoStatus(
    ...
    try:
        client = await getDhClient()
        logger.info(f'getDhVideoStatus {taskId}')
        ret = await client.get_video_status(taskId)
        return ret
    except Exception as e:
        logger.info(f'getDhVideoStatus error')
        return MCPVideoStatusResponse(error=str(e))
...
async def generateText2Audio(
    ...
    """
    logger.info(f'generateText2Audio {text} {voiceId}')
    try:
        ...
        logger.info(f'generateText2Audio error')
        return MCPVideoGenerateResponse(error=str(e))
...
async def getText2AudioStatus(
    ...
    Args:
        taskId: 任务ID
    """
    logger.info(f'getText2AudioStatus {taskId}')
    try:
        client = await getDhClient()
        ret = await client.get_text2audio_status(taskId)
        return ret
    except Exception as e:
        logger.info(f'getText2AudioStatus error')
...
async def generateLite2dGeneralVideo(
    ...
    try:
        ...
        logger.info(f'generateLite2dGeneralVideo {req.model_dump()}')
        ret = await client.generate_lite2d_video(req)
        return ret
    except Exception as e:
        logger.info(f'generateLite2dGeneralVideo error')
...
async def getLite2dGeneralStatus(
    ...
    Returns:
        任务状态
    """
    logger.info(f'getLite2dGeneralStatus {figureId} {systemFigure} {trainSuccess}')
    try:
        client = await getDhClient()
        ret = await client.get_lite2d_general_status(figureId, systemFigure, trainSuccess)
        return ret
    except Exception as e:
        logger.info(f'getLite2dGeneralStatus error')
        return MCPVideoStatusResponse(error=str(e))
...
async def generateVoiceCloneLite(
    ...
    Return:
        perId: 音色克隆任务的ID
    """
    logger.info(f'generateVoiceClone {name}, {uploadAudioId}, {exampleText} {gender}')
    try:
        client = await getDhClient()
        ...
        logger.info(f'generateVoiceClone error')
...
async def getVoiceCloneStatus(
    ...
    Returns:
        任务状态
    """
    logger.info(f'getVoiceCloneStatus {perId}')
    try:
        client = await getDhClient()
        ret = await client.voice_clone_status(perId)
        return ret
    except Exception as e:
        logger.info(f'getVoiceCloneStatus error')
        return MCPVoiceCloneStatusResponse(error=str(e))

def start():
    """Start the DH MCP """
    _transport = os.environ.get("DH_MCP_TRANSPORT", "stdio")
    try:
