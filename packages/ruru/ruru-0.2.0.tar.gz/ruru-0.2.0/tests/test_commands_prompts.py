...
class TestPromptCommands:
    ...
    def test_download_command_help(self, cli_runner: CliRunner):
        """Test that download command shows help."""
        result = cli_runner.invoke(prompts_group, ["download", "--help"])
        assert result.exit_code == 0
        assert "Download prompts with filtering options" in result.output

    @patch("ruru.commands.prompts.get_authenticated_client")
    @patch("builtins.open", new_callable=mock_open)
    @patch("ruru.commands.prompts.Path")
    def test_download_command_json_success(
        self, mock_path, mock_file, mock_get_client, cli_runner: CliRunner
    ):
        """Test successful download command with JSON format."""
        # Setup mocks
        mock_client = Mock()
        mock_get_client.return_value = mock_client
        mock_client.download_prompts.return_value = {
            "prompts": [
                {
                    "id": "123",
                    "name": "test-prompt",
        ...
        mock_path.cwd.return_value = mock_output_dir
        mock_output_dir.mkdir = Mock()
        mock_json_path = Mock()
        mock_output_dir.__truediv__ = Mock(return_value=mock_json_path)

        # Run command
        result = cli_runner.invoke(
            prompts_group, ["download", "--project", "test-project", "--no-content"]
        )

        assert result.exit_code == 0
        assert "Downloaded 1 prompt(s)" in result.output

        # Verify client was called
        mock_client.download_prompts.assert_called_once_with(
    ...
    @patch("builtins.open", new_callable=mock_open)
    @patch("ruru.commands.prompts.Path")
    def test_download_command_zip_success(
        self, mock_path, mock_file, mock_get_client, cli_runner: CliRunner
    ):
        """Test successful download command with ZIP format."""
        # Setup mocks
        mock_client = Mock()
        mock_get_client.return_value = mock_client
        mock_client.download_prompts_zip.return_value = b"fake zip content"
        ...
        mock_output_dir.mkdir = Mock()
        mock_zip_path = Mock()
        mock_output_dir.__truediv__ = Mock(return_value=mock_zip_path)

        # Run command
        result = cli_runner.invoke(
            prompts_group, ["download", "--format", "zip", "--tags", "python,ai"]
        )

        assert result.exit_code == 0
        assert "Downloaded ZIP file" in result.output

        # Verify client was called
        mock_client.download_prompts_zip.assert_called_once_with(
            project_name=None, directory=None, tags=["python", "ai"]
        )

    @patch("ruru.commands.prompts.get_authenticated_client")
    def test_download_command_no_results(self, mock_get_client, cli_runner: CliRunner):
        """Test download command with no results."""
        # Setup mocks
        mock_client = Mock()
        mock_get_client.return_value = mock_client
        mock_client.download_prompts.return_value = {
        ...
        # Run command
        result = cli_runner.invoke(prompts_group, ["download"])

        assert result.exit_code == 0
        assert "No prompts found matching the criteria" in result.output

    def test_download_project_command_help(self, cli_runner: CliRunner):
        """Test that download-project command shows help."""
        result = cli_runner.invoke(prompts_group, ["download-project", "--help"])
        assert result.exit_code == 0
        assert "Download all prompts from a specific project" in result.output

    @patch("ruru.commands.prompts.get_authenticated_client")
    @patch("builtins.open", new_callable=mock_open)
    @patch("ruru.commands.prompts.Path")
    def test_download_project_command_success(
        self, mock_path, mock_file, mock_get_client, cli_runner: CliRunner
    ):
        """Test successful download-project command."""
        # Setup mocks
        mock_client = Mock()
        mock_get_client.return_value = mock_client
        mock_client.download_prompts_by_project.return_value = {
            "prompts": [
                {
                    "id": "123",
                    "name": "test-prompt",
        ...
        assert result.exit_code == 0
        assert "Downloaded 1 prompt(s) from project 'test-project'" in result.output

        # Verify client was called
        mock_client.download_prompts_by_project.assert_called_once_with(
            project_name="test-project", include_content=False
        )

    def test_download_directory_command_help(self, cli_runner: CliRunner):
        """Test that download-directory command shows help."""
        result = cli_runner.invoke(prompts_group, ["download-directory", "--help"])
        assert result.exit_code == 0
        assert "Download all prompts from a specific directory" in result.output

    @patch("ruru.commands.prompts.get_authenticated_client")
    @patch("builtins.open", new_callable=mock_open)
    @patch("ruru.commands.prompts.Path")
    def test_download_directory_command_success(
        self, mock_path, mock_file, mock_get_client, cli_runner: CliRunner
    ):
        """Test successful download-directory command."""
        # Setup mocks
        mock_client = Mock()
        mock_get_client.return_value = mock_client
        mock_client.download_prompts_by_directory.return_value = {
            "prompts": [
                {
                    "id": "123",
                    "name": "test-prompt",
        ...
        # Run command
        result = cli_runner.invoke(
            prompts_group, ["download-directory", "src/*", "--no-content"]
        )

        assert result.exit_code == 0
        assert "Downloaded 1 prompt(s) from directory 'src/*'" in result.output

        # Verify client was called
        mock_client.download_prompts_by_directory.assert_called_once_with(
            directory="src/*", include_content=False
        )

    def test_download_tags_command_help(self, cli_runner: CliRunner):
        """Test that download-tags command shows help."""
        result = cli_runner.invoke(prompts_group, ["download-tags", "--help"])
        assert result.exit_code == 0
        assert "Download all prompts matching specific tags" in result.output

    @patch("ruru.commands.prompts.get_authenticated_client")
    @patch("builtins.open", new_callable=mock_open)
    @patch("ruru.commands.prompts.Path")
    def test_download_tags_command_success(
        self, mock_path, mock_file, mock_get_client, cli_runner: CliRunner
    ):
        """Test successful download-tags command."""
        # Setup mocks
        mock_client = Mock()
        mock_get_client.return_value = mock_client
        mock_client.download_prompts_by_tags.return_value = {
            "prompts": [
                {
                    "id": "123",
                    "name": "test-prompt",
        ...
        # Run command
        result = cli_runner.invoke(
            prompts_group, ["download-tags", "python,ai", "--no-content"]
        )

        assert result.exit_code == 0
        assert "Downloaded 1 prompt(s) with tags 'python, ai'" in result.output

        # Verify client was called
        mock_client.download_prompts_by_tags.assert_called_once_with(
            tags=["python", "ai"], include_content=False
        )
