...
class TestConfigManager:
    ...
    def test_get_api_key_from_keyring(self, mock_keyring, isolated_env, monkeypatch):
        """Test getting API key from keyring."""
        # Ensure no API key in environment
        monkeypatch.delenv("RURU_API_KEY", raising=False)

        # Mock the _create_env_config to return a config that doesn't have RURU_API_KEY
        def mock_env_config(key, default=None, cast=None):
            if key == "RURU_API_KEY":
                return default
            return default

        monkeypatch.setattr(
            "ruru.config.ConfigManager._create_env_config", lambda self: mock_env_config
        )
