...
from moxn_types.content import SignedURLContentRequest
...
class RefreshEngine(Generic[T]):
                ...
                async with self._semaphore:
                    response = await self._backend.get_signed_content_url(
                        SignedURLContentRequest(
                            file_path=content.file_path, ttl_seconds=content.ttl_seconds
                        )
                    )
    ...
    async def _refresh_batch(self, items: list[T]) -> list[T]:
        ...
        try:
            reqs = [
                SignedURLContentRequest(
                    file_path=i.file_path, ttl_seconds=i.ttl_seconds
                )
                for i in items
