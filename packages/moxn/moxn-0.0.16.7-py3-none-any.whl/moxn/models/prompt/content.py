...
from moxn.base_models.blocks.document import (
...
class PromptContent(BaseModel):
    ...
    def append_document(
        self,
        document: PDFContentFromSource | dict,
        name: str = "",
        description: str = "",
        author=Author.HUMAN,
        role=MessageRole.USER,
        ...
        if isinstance(document, dict):
            _document: PDFContentFromSource = (
                PDFContentFromSourceAdapter.validate_python(document)
            )
        else:
            _document = document
