...
class RelationshipDetector:
    ...
    def __init__(self):
        ...
        logger.info(f"Initialized RelationshipDetector with {len(self.generic_fields_lower)} generic fields for FK suffix matching: "
    ...
    def detect_foreign_key_relationships(self, entities_map: Dict[str, Dict]) -> List[Dict[str, Any]]:
        ...
        Generic fields are USED as suffixes to identify FKs (e.g., user_id, company_key),
        but relationships between two generic fields are filtered out to avoid meaningless connections.
        ...
        logger.info(f"Using generic field suffixes for FK detection: {', '.join(self.generic_fields_lower)}")
        logger.info(f"Will filter out relationships where BOTH source and target fields are generic")

        for source_qnk, source_info in entities_map.items():
            source_subgraph = source_info.get('subgraph')
            source_pks = {pk.lower() for pk in source_info.get('primary_keys', [])}
            ...
            for field in source_info.get('fields', []):
                ...
                # Only skip primary keys (not generic fields, as they're used for FK pattern matching)
                if field_name_lower in source_pks:
    ...
    def detect_shared_field_relationships(self, entities_map: Dict[str, Dict]) -> List[Dict[str, Any]]:
        ...
        Filters out generic fields and primary keys because shared relationships
        on generic fields (like 'id', 'name', 'status') are not meaningful business relationships.

        This is different from FK detection where generic fields are USED to identify patterns.
        ...
        for form in forms_to_check:
            ...
            if target_qnk and target_qnk != source_qnk:
                ...
                if not to_field_name:
                    logger.debug(f"Skipping relationship from {source_qnk}.{field_name} to {target_qnk} - no valid target field found")
                    continue

                # ADDITIONAL CHECK: Skip if both source and target fields are generic
                # This prevents meaningless relationships between generic fields
                source_field_is_generic = field_name.lower() in self.generic_fields_lower
                target_field_is_generic = to_field_name.lower() in self.generic_fields_lower

                if source_field_is_generic and target_field_is_generic:
                    logger.debug(f"Skipping relationship from {source_qnk}.{field_name} to {target_qnk}.{to_field_name} - both fields are generic")
                    continue
    ...
    def _analyze_shared_fields(self, qnk1: str, qnk2: str, fields1: Set[str],
        ...
        for field_lower in common_fields:
            # Skip generic fields and primary keys for shared field relationships
            # (shared relationships on generic fields like 'id', 'status' are not meaningful)
            if (field_lower in self.generic_fields_lower or
                    field_lower in entity1_pks or field_lower in entity2_pks):
                logger.debug(f"Skipping shared field '{field_lower}' between {qnk1} and {qnk2} - generic field or primary key")
                continue
    ...
    def _determine_and_validate_target_field_name(self, pk_template: str, guessed_generic_id: Optional[str],
        ...
        For FK relationships, generic fields ARE allowed as target fields when they represent
        legitimate primary keys or are explicitly specified in templates.
        ...
        candidate_fields.extend(['id', 'key', 'identifier', 'uid'])
