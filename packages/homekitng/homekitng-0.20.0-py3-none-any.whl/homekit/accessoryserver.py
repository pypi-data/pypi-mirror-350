...
class AccessoryRequestHandler(BaseHTTPRequestHandler):
    ...
    def _post_pair_verify(self):
        ...
        if state == States.M1:
            ...
            accessory_ltsk = SigningKey(self.server.data.accessory_ltsk)
            accessory_signature = accessory_ltsk.sign(accessory_info).signature
        ...
        if state == States.M3:
            ...
            ios_device_ltpk = VerifyKey(ios_device_ltpk_bytes, encoder=RawEncoder)
            ...
            try:
                ios_device_ltpk.verify(ios_device_info, ios_device_sig)
    ...
    def _post_pair_setup(self):
        ...
        if state == States.M5:
            ...
            verify_key = VerifyKey(ios_device_ltpk, encoder=RawEncoder)
            try:
                verify_key.verify(ios_device_info, ios_device_sig)
            except BadSignatureError:
                self.send_error_reply(States.M6, Errors.Authentication)
                self.log_error('error in step #6 %s %s', d_res, self.server.sessions)
                return
            ...
            if self.server.data.accessory_ltsk is None or self.server.data.accessory_ltpk is None:
                accessory_ltsk = SigningKey.generate()
                accessory_ltpk = accessory_ltsk.verify_key
                self.server.data.set_accessory_keys(
                    accessory_ltpk.encode(encoder=RawEncoder),
                    accessory_ltsk.encode(encoder=RawEncoder),
                )
            else:
                accessory_ltsk = SigningKey(self.server.data.accessory_ltsk, encoder=RawEncoder)
                accessory_ltpk = accessory_ltsk.verify_key  # Reconstruct from stored LTSK
            ...
            # 3) construct accessory_info
            accessory_info = accessory_x + self.server.data.accessory_pairing_id_bytes + bytes(accessory_ltpk)
            ...
            sub_tlv = [
                tlv8.Entry(TlvTypes.Identifier, self.server.data.accessory_pairing_id_bytes),
                tlv8.Entry(TlvTypes.PublicKey, bytes(accessory_ltpk)),
                tlv8.Entry(TlvTypes.Signature, accessory_signature)
            ]
            sub_tlv_b = tlv8.encode(sub_tlv)
