...
def get_available_program_path():
    ...
    if "ANTHROPIC_API_KEY" in os.environ and "None" not in os.environ["ANTHROPIC_API_KEY"]:
        return Path(__file__).parent.parent / "examples" / "anthropic.toml"
    elif "OPENAI_API_KEY" in os.environ and "None" not in os.environ["OPENAI_API_KEY"]:
        return Path(__file__).parent.parent / "examples" / "openai.toml"
    else:
        return None
...
def run_prompt_option(program_path, prompt, timeout=45):
    ...
    Returns:
        Tuple of (return_code, stdout, stderr)
    """
    cmd = [sys.executable, "-m", "llmproc.cli", str(program_path), "--prompt", prompt]
...
def run_non_interactive_option(program_path, input_text="Hello from stdin\n", timeout=45):
    ...
    Returns:
        Tuple of (return_code, stdout, stderr)
    """
    cmd = [sys.executable, "-m", "llmproc.cli", str(program_path), "--non-interactive"]
...
@pytest.mark.llm_api
@pytest.mark.essential_api
def test_non_interactive_option():
...
def test_stdin_pipe_with_n_flag():
    ...
    # Create command with -n flag (shorthand for --non-interactive)
    cmd = [
        sys.executable,
        "-m",
        "llmproc.cli",
...
def test_program_linking():
    ...
    program_path = Path(__file__).parent.parent / "examples" / "program-linking" / "main.toml"
...
def test_help_option():
    ...
    cmd = [sys.executable, "-m", "llmproc.cli", "--help"]
    return_code, stdout, stderr = run_cli_command(cmd)
