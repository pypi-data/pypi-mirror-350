...
def get_yield_curve(output: str = 'dataframe', column_name: str = None):
    """
    Function to retrieve the yield curve data — Estimated Term Structure of Interest Rates and Implied Inflation

    Parameters:
        - output (str): Desired output type ('dataframe' or 'graph'). Default is 'dataframe'.
        - column_name (str):The name of the column to plot when `output='graph'`.
          Must be one of the following:
            - 'ETTJ_IPCA'          : Estimated Term Structure of Interest Rates (inflation-indexed).
            - 'ETTJ_Prefixados'    : Estimated Term Structure of Interest Rates (fixed rate).
            - 'Inflação_implícita' : Implied Inflation.

            Required if `output` is set to 'graph'; ignored otherwise.
...
def get_federal_bonds(bond:str, output: str = 'dataframe', column_name: str = None, bond_date:str = None):
    """
    Collects reference rates and unit prices of federal public bonds
