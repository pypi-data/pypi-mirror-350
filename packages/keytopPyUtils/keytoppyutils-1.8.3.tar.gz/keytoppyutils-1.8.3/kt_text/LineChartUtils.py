...
class LineChartUtils:
    ...
    def generate_line_chart(self):
        ...
        if self.title_name != '':
            ax.set_title(self.title_name, color='#666666', fontsize=12, fontweight='bold', pad=20)  # 设置标题颜色
        else:
            plt.subplots_adjust(bottom=0.1,  top=0.9)

        if self.x_label_name != '':
            ax.set_xlabel(self.x_label_name, color='#666666', fontsize=8, labelpad=5)  # 设置X轴标签颜色
        else:
            plt.subplots_adjust(bottom=0.1, top=0.9)

        if self.y_label_name != '':
            plt.subplots_adjust(left=0.1, right=0.9)
            ax.set_ylabel(self.y_label_name, color='#666666', fontsize=8, labelpad=5)  # 设置Y轴标签颜色
        else:
