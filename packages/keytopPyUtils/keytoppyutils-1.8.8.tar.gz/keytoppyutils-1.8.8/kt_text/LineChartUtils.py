...
class LineChartUtils:
    ...
    def generate_line_chart(self):
        ...
        for key in self.y_keys:
            ax.plot(x, grouped_data[key], label=key, color=self.y_keys[key], linestyle='-',linewidth=0.5,alpha=0.7)

        if self.title_name != '':
            ax.set_title(self.title_name, color='#666666', fontsize=10, fontweight='bold', pad=20)  # 设置标题颜色
        else:
        ...
        if self.y_label_name != '':
            ...
            ax.set_ylabel(self.y_label_name, color='#666666', fontsize=8, labelpad=6)  # 设置Y轴标签颜色
        else:
