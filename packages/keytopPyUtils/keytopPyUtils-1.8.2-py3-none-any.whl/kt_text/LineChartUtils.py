...
class LineChartUtils:
    ...
    def generate_line_chart(self):
        ...
        for key in self.y_keys:
            ax.plot(x, grouped_data[key], label=key, color=self.y_keys[key], linestyle='-',linewidth=0.85,alpha=0.8)

        if self.title_name != '':
            ax.set_title(self.title_name, color='#777777', fontsize=12, fontweight='bold', pad=22)  # 设置标题颜色
        else:
            plt.subplots_adjust(bottom=0.1,  top=0.9)

        if self.x_label_name != '':
            ax.set_xlabel(self.x_label_name, color='#777777', fontsize=8, labelpad=5)  # 设置X轴标签颜色
        else:
            plt.subplots_adjust(bottom=0.1, top=0.9)

        if self.y_label_name != '':
            plt.subplots_adjust(left=0.1, right=0.9)
            ax.set_ylabel(self.y_label_name, color='#777777', fontsize=8, labelpad=5)  # 设置Y轴标签颜色
        else:
