...
class HgvsMutationTokenizer:
    ...
    def tokenize(mutation: GeneMutation) -> str:
        ...
        return " ".join(sorted(filtered_tokens))

    @staticmethod
    def _filter_low_value_tokens(tokens: set) -> set:
        """Filter out single-letter amino acid codes that provide limited search value"""
        # Get single-letter amino acid codes to filter out
        single_letter_aa = set(HgvsMutationTokenizer.AA_MAP.values())
        
        filtered = set()
        for token in tokens:
            # Keep token if it's not a standalone single-letter amino acid
            if token not in single_letter_aa:
                filtered.add(token)
            # Also filter out single nucleotides A, C, G, T when standalone
            elif token not in {'A', 'C', 'G', 'T'}:
                filtered.add(token)
