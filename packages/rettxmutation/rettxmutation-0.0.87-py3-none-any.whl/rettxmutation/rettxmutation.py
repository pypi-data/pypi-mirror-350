...
class RettXDocumentAnalysis:
    ...
    def tokenize(self,
        ...
        Args:
            ...
            return_array (bool, optional): Whether to return an array of tokens. Defaults to False.
        ...
        """
        return MutationTokenizator.tokenize(
            gene_mutation,
    ...
    def get_unique_tokens(self,
        ...
        """
        Get unique tokens from a list of GeneMutation objects.
        
        Args:
            gene_mutation_list (List[Mutation]): List of mutations to extract unique tokens from

        Returns:
            List[str]: List of unique tokens
        """
        return MutationTokenizator.get_unique_tokens(
            gene_mutation_list=gene_mutation_list,
            lowercase=lowercase,
            min_token_length=min_token_length
        )

    def get_unique_tokens_as_string(self,
                                    gene_mutation_list: List[str],
                                    lowercase: bool = False,
                                    min_token_length: int = None,
                                    separator: str = " ") -> str:
        """
        Get unique tokens from a list of GeneMutation objects as a single string.
        Args:
            gene_mutation_list (List[Mutation]): List of mutations to extract unique tokens from
            separator (str, optional): Separator to use between tokens. Defaults to " ".
        Returns:
            str: Space-separated unique tokens
        """
        return MutationTokenizator.get_unique_tokens_as_string(
            gene_mutation_list=gene_mutation_list,
            lowercase=lowercase,
            min_token_length=min_token_length,
            separator=separator
