...
class HgvsMutationTokenizer:
    ...
    @staticmethod
    def tokenize(mutation: GeneMutation, return_array: bool = False, lowercase: bool = False):
        """
        Tokenize a GeneMutation object.
        
        Args:
            mutation: The GeneMutation object to tokenize
            return_array: If True, returns list of tokens; if False, returns space-separated string
            lowercase: If True, converts all tokens to lowercase
            
        Returns:
            str or List[str]: Tokens as string (default) or list of strings
        """
        tokens = set()
        ...
        # Filter out low-value single-letter tokens
        filtered_tokens = HgvsMutationTokenizer._filter_low_value_tokens(tokens)
    ...
    @staticmethod
    def _filter_low_value_tokens(tokens: set) -> set:
        """Filter out single-character tokens that provide limited search value"""
        filtered = set()
        for token in tokens:
            # Keep only tokens with 2 or more characters
            if len(token) >= 2:
                filtered.add(token)
    ...
    def parse_transcript_variant(variant: str) -> set:
        ...
        # Extract simplified notation without "c." prefix
        simplified_match = re.search(r'c\.(.+)', variant)
        if simplified_match:
            tokens.add(simplified_match.group(1))  # Add "916C>T" from "c.916C>T"
    ...
    def parse_protein_consequence(protein_str: str) -> set:
        ...
        # Add the original string as a token
        tokens.add(protein_str)
        
        # Handle full format with accession: NP_001104262.1:p.(Arg318Cys)
        full_match = re.match(r'^([^:]+):p\.\(?([^)]+)\)?', protein_str)
        if full_match:
            accession, protein_change = full_match.groups()
            
            # Add accession tokens
            tokens.add(accession)  # NP_001104262.1
            accession_base = re.sub(r'\.\d+$', '', accession)  # Remove version number
            tokens.add(accession_base)  # NP_001104262
            
            # Add protein change in various formats
            tokens.add(f"p.({protein_change})")  # p.(Arg318Cys)
            tokens.add(f"p.{protein_change}")    # p.Arg318Cys
            tokens.add(f"p{protein_change}")     # pArg318Cys
            tokens.add(protein_change)           # Arg318Cys
            
            # Parse individual components from protein_change for substitutions
            subst_match = re.match(r'([A-Za-z]{1,3})(\d+)([A-Za-z]{1,3})', protein_change)
            if subst_match:
                from_aa, position, to_aa = subst_match.groups()
                tokens.add(position)                    # 318
                tokens.add(f"{from_aa}{position}")      # Arg318 or R306
                tokens.add(f"{position}{to_aa}")        # 318Cys or 306C
        
        # Remove prefix and parentheses: NP_004983.1:p.(R306C) -> R306C
        clean_str = re.sub(r'^.*?p\.\(?([^)]+)\)?', r'\1', protein_str)
        tokens.add(clean_str)
        
        # Also add the simplified notation without "p." and parentheses from the original string
        simplified_match = re.search(r'p\.\(?([^)]+)\)?', protein_str)
        if simplified_match:
            simplified = simplified_match.group(1)
            tokens.add(simplified)  # Add "R306C" or "Arg306Cys" directly
        ...
        if subst_match:
            ...
            if len(from_aa) == 3:  # 3-letter to 1-letter
                ...
                # Add 3-letter codes as they're more informative - these will be kept by filter
        ...
        if fs_match:
            ...
            if len(aa) == 3:
                ...
                tokens.add(aa)  # 3-letter code is informative - will be kept by filter
            elif len(aa) == 1:
                # Convert to 3-letter for standalone token
                aa_3 = next((k for k, v in HgvsMutationTokenizer.AA_MAP.items() if v == aa), aa)
                if aa_3 != aa:
                    tokens.add(aa_3)  # 3-letter code will be kept by filter
            return tokens
        ...
        if del_match:
            ...
            if len(from_aa) == 3:
                ...
                # Add 3-letter codes as they're more informative - will be kept by filter
                tokens.update([from_aa, to_aa])
            elif len(from_aa) == 1:
                ...
                to_3 = next((k for k, v in HgvsMutationTokenizer.AA_MAP.items() if v == to_aa), to_aa)
                if from_3 != from_aa:
                    tokens.update([from_3, to_3])  # 3-letter codes will be kept by filter
            return tokens
