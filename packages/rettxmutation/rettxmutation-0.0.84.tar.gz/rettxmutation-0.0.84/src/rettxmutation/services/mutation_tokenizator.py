...
class MutationTokenizator:
    ...
    @classmethod
    def tokenize(cls, mutation_string: str, include_original: bool = True, 
                 lowercase: bool = False, min_token_length: int = None) -> List[str]:
        """
        ...
        Returns:
            List[str]: List of unique tokens generated from the mutation string, sorted by length (descending)
        """
        ...
        return sorted(filtered_tokens, key=len, reverse=True)
    ...
    @classmethod
    def get_unique_tokens(cls, mutation_strings: List[str], lowercase: bool = False,
                         min_token_length: int = None) -> List[str]:
        """
        Get unique tokens from a list of mutation strings.
        
        Args:
            mutation_strings (List[str]): List of mutation strings to tokenize
            lowercase (bool): Whether to convert to lowercase for normalization
            min_token_length (int): Minimum token length
        ...
        for mutation_string in mutation_strings:
            tokens = cls.tokenize(mutation_string, lowercase=lowercase,
