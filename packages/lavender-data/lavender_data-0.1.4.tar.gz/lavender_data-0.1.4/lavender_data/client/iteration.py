...
class DataMarkerMixin:
    _api: LavenderDataClient

    def complete(self, index: int, metadata: Optional[dict] = None):
        # TODO async
...
class LavenderDataLoader(DataMarkerMixin, IterableMixin):
    def __init__(
        self,
...
class AsyncLavenderDataLoader(DataMarkerMixin, IterableMixin):
    def __init__(
        self,
        ...
        self._executor: Optional[ThreadPoolExecutor] = None  # to be serializable
    ...
    def _submit_next(self):
        ...
        queue_size = self._prefetch_factor
        if self._executor is None:
            self._executor = ThreadPoolExecutor(queue_size)

        while len(self._futures) < queue_size:
            try:
    ...
    def __next__(self):
        ...
        while True:
            ...
            already_arrived = (
                [data for i, data in self._arrived if i == next_index]
                if self._in_order
                else self._arrived
            )
            if len(already_arrived) > 0:
                data = already_arrived[0]
                self._arrived = [a for a in self._arrived if a[0] != next_index]
            ...
            if self._stopped and len(self._futures) == 0:
                raise StopIteration
            ...
            if not self._in_order or arrived_index == next_index:
