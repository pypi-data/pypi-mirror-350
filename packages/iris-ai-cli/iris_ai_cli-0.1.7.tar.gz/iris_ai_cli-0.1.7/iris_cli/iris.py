...
class GroqCLI:
    ...
    def __init__(self, api_key: Optional[str] = None):
        ...
        self.routing_model = "meta-llama/llama-4-scout-17b-16e-instruct"
...
"""
    ...
    def configure_model(self, model_name: str = "meta-llama/llama-4-scout-17b-16e-instruct") -> None:
    ...
    def route_query(self, query: str) -> str:
        ...
        routing_prompt = f"""
You are an assistant that determines whether a system tool is required to answer a user's query. The available tools include functions for retrieving system information, git status, directory contents, environment variables, executing Python scripts, and creating Python scripts.

**Rules:**

1. If the user's query involves accessing system information (e.g., memory, disk, processes), git repository status, directory contents, environment variables, executing a Python script, or creating a Python script, respond with **"TOOL: SYSTEM"**.
2. If the query can be answered without any system TOOLs—using only general knowledge or simple terminal commands that don’t rely on the specified tools—respond with **"NO TOOL"**.

**Examples:**

- **Query:** "What is the current memory usage?"  
  **Response:** TOOL: SYSTEM  
  *(Requires system information access)*

- **Query:** "List the files in the current directory."  
  **Response:** TOOL: SYSTEM  
  *(Requires directory contents listing)*

- **Query:** "Show me the git status of this repository."  
  **Response:** TOOL: SYSTEM  
  *(Requires git status information)*

- **Query:** "Execute this C:\Mydrive\BrillianceLabsIncubator\AI_engine\ai-lab\iris_cli\open_camera.py"  
  **Response:** TOOL: SYSTEM  
  *(Requires script execution)*

- **Query:** "Create and save a Python script for printing the numbers from 1 to n"  
  **Response:** TOOL: SYSTEM  
  *(Requires script creation)*

- **Query:** "Take the screenshot of the current window"  
  **Response:** TOOL: SYSTEM  
  *(Requires script creation or system interaction)*

- **Query:** "Fetch me all the environment variables in this system"  
  **Response:** TOOL: SYSTEM  
  *(Requires environment variable access)*

- **Query:** "List me all environment variables"  
  **Response:** TOOL: SYSTEM  
  *(Requires environment variable access)*

- **Query:** "Is there any llm key in the environment variables"  
  **Response:** TOOL: SYSTEM  
  *(Requires checking environment variables)*

- **Query:** "What is the capital of France?"  
  **Response:** NO TOOL  
  *(General knowledge, no tools needed)*

  **Query:** "What is my openai key"  
  **Response:** TOOL: SYSTEM
  *(Requires checking environment variables)*

Please analyze the user's query and respond with either **"TOOL: SYSTEM"** or **"NO TOOL"** based on whether a system tool is required.
"""
...
def main():
    ...
    try:
        ...
        if len(sys.argv) > 1:
            ...
            groq_cli = GroqCLI()
            groq_cli.configure_model("meta-llama/llama-4-scout-17b-16e-instruct")
