...
def dict2netcdf_old(data_dict, output_path, group=None, parent_group=None):
    # {{{
    """
    将多层字典转换为 NetCDF 文件
    ...
    if parent_group is None:
        with nc.Dataset(output_path, 'w', format='NETCDF4') as rootgrp:
            dict2netcdf(data_dict, output_path, group=None, parent_group=rootgrp)
        return
    ...
    for key, value in data_dict.items():
        ...
        if isinstance(value, Mapping):
            dict2netcdf(value, output_path, group=key, parent_group=current_group)
            continue
        ...
        if isinstance(value, (list, np.ndarray)):
            ...
            for i, dim_size in enumerate(arr.shape):
                ...
                if dim_name not in current_group.dimensions:
            ...
            if arr.dtype.kind == 'U':
                ...
                var = current_group.createVariable(key, str_type, tuple(f'{key}_dim_{i}' for i in range(arr.ndim)))
                var[:] = np.char.encode(arr, 'utf-8')
            else:
                # 处理数值数组
                var = current_group.createVariable(key, arr.dtype, tuple(f'{key}_dim_{i}' for i in range(arr.ndim)))
                var[:] = arr
            continue
    ...
    for key, value in data_dict.items():
        ...
        if isinstance(value, (list, np.ndarray)):
            ...
            if arr.dtype.kind == 'U':
                ...
                # 处理数值数组
                var = current_group.createVariable(key, arr.dtype, tuple(gdim_dict[f'{key}_dim_{i}'] for i in range(arr.ndim)), zlib=compress)
                var[:] = arr
            continue
