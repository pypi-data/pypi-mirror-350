...
from remotivelabs.topology.frame import FrameName
from remotivelabs.topology.namespace import NamespaceName
from remotivelabs.topology.signal import SignalName
...
class Restbus:
    ...
    async def add(self, *frames: tuple[NamespaceName, list[FrameConfig]], start: bool = False):
        """
        ...
        await self._restbus_stub.Add(
            restbus_api_pb2.AddRequest(
                ...
                startOption=restbus_api_pb2.StartOption.START_AFTER_INIT if start else restbus_api_pb2.StartOption.STOPPED,
            )
        )
    ...
    async def update_signals(self, *signals: tuple[NamespaceName, list[SignalConfig]]) -> None:
        """
