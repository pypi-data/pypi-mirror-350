...
from remotivelabs.topology.broker.restbus.signal_config import FrameConfig, SignalConfig
from remotivelabs.topology.frame import Frame, FrameInfo, FrameName, FrameSubscription
from remotivelabs.topology.namespace import NamespaceName
...
from remotivelabs.topology.secoc import SecocProperty
from remotivelabs.topology.signal import Signal, SignalInfo, SignalName, WriteSignal

_logger = logging.getLogger(__name__)


class GenericNamespace(Namespace[Union[FrameFilter], Callable[[Frame], Awaitable[None]]]):
    """
    ...
    def create_input_handler(
        self, filters: Sequence[FrameFilter], callback: Callable[[Frame], Awaitable[None]]
        ...
        Args:
            - filters: A sequence of FrameFilter objects to select relevant frames or signals.
...
class RestbusConfig:
    ...
    restbus_filters: list[FrameFilter]
...
class Restbus:
    ...
    async def open(self) -> Self:
        ...
        frames: dict[FrameName, FrameConfig] = {}
            ...
            for frame_info in frame_infos:
                for filter in config.restbus_filters:
                    if filter.filter_frames(frame_info):
                        ...
                        frame_config = FrameConfig(name=frame_info.name, cycle_time=cycle_time)
    ...
    async def add(self, *frame_configs: FrameConfig, start: bool = False):
        """
    ...
    async def update_signals(self, *signal_configs: SignalConfig) -> None:
        """
        ...
        """
        await self._broker_client.restbus.update_signals((self._namespace, list(signal_configs)))
...
class TransactionContext:
    ...
    def update(self, *signal_config: SignalConfig) -> None:
        self.updates.extend(signal_config)

    def set_update_bit(self, *signals: SignalName) -> None:
        self.updates.extend([SignalConfig.set_update_bit(name) for name in signals])
