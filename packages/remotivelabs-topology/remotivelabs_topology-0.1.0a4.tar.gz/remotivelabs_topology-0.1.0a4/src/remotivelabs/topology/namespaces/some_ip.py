...
import remotivelabs.topology.broker.conv.grpc_converters as conv
from remotivelabs.topology.broker.client import BrokerClient
from remotivelabs.topology.frame import Frame, FrameSubscription
...
from remotivelabs.topology.namespaces.input_handlers import (
    InputHandler,
    SomeIPEventHandler,
...
class SomeIPNamespace(
    ...
    If used standalone (outside `BehavioralModel`) this class must be used as an async context manager in order to active the namespace.
    """
    ...
    def create_input_handler(
        ...
        if all(isinstance(f, SomeIPRequestFilter) for f in filters):
            return (self.name, SomeIPRequestHandler(filters=filters, cb=callback, decode_named_values=self._decode_named_values))  # type: ignore[arg-type]
        return (self.name, SomeIPEventHandler(filters=filters, cb=callback, decode_named_values=self._decode_named_values))  # type: ignore[arg-type]
