...
from src.terrajinja.sbp.vcd.network_isolated_v2 import SbpVcdNetworkIsolatedV2
from .helper import stack, has_resource, has_resource_with_properties


class TestSbpVcdNetworkIsolatedV2:
    ...
    def test_cidr_valid(self, stack, cidr, gateway, prefix_length, static_ip_pool):
        ...
        SbpVcdNetworkIsolatedV2(stack, f"valid_cidr_{cidr}", cidr, ["", ""], name="name", owner_id="id")
        synthesized = Testing.synth(stack)

        has_resource(synthesized, "vcd_network_isolated_v2")
        has_resource_with_properties(synthesized, "vcd_network_isolated_v2", {"gateway": gateway})
        has_resource_with_properties(synthesized, "vcd_network_isolated_v2", {"prefix_length": prefix_length})
    ...
    def test_new_var(self, stack):
        ...
        has_resource(synthesized, "vcd_network_isolated_v2")
        has_resource_with_properties(synthesized, "vcd_network_isolated_v2", {"gateway": "40.40.0.1"})
        has_resource_with_properties(synthesized, "vcd_network_isolated_v2", {"guest_vlan_allowed": True})
    ...
    def test_override_gateway(self, stack):
        ...
        has_resource(synthesized, "vcd_network_isolated_v2")
        has_resource_with_properties(synthesized, "vcd_network_isolated_v2", {"gateway": "10.10.10.10"})


    def test_cidr_empty(self, stack):
        with pytest.raises(AddressValueError) as context:
            SbpVcdNetworkIsolatedV2(stack, "empty_cidr", "", ["", ""], name="name", owner_id="id")

        assert "Address cannot be empty" == str(context.value)

    def test_cidr_invalid(self, stack):
        with pytest.raises(AddressValueError) as context:
            SbpVcdNetworkIsolatedV2(stack, "invalid_cidr", "invalid_cidr", ["", ""], name="name", owner_id="id")
