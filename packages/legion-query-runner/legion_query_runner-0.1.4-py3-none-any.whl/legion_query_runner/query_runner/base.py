...
from legion_query_runner import settings
from legion_query_runner.util import utils

from .utils.requests_session import (
...
def with_ssh_tunnel(query_runner, details):
    ...
    try:
        server = open_tunnel(bastion_address, remote_bind_address=remote_address, **auth)
        server = server.__enter__()
        if not hasattr(server, 'local_bind_address') or not server.local_bind_address:
            raise ValueError("SSH tunnel server did not provide local bind address")
...
class QueryRunner:
    """A unified adapter for different query runners that provides a consistent interface."""
    
    def __init__(self, db_type: str, configuration: Dict[str, Any]):
        """
        Initialize the query runner adapter.
        
        Args:
            db_type: Type of database ('mysql', 'pg', 'redshift', etc.)
            configuration: Database connection configuration
        """
        self.runner = get_query_runner(db_type, configuration)
        if not self.runner:
    ...
    def run_query(self, query: str, user: Optional[Any] = None) -> Dict[str, Any]:
        """
        ...
        Returns:
            Dict containing 'columns' and 'rows' if successful
            
        Raises:
            Exception: If the query fails or SSH tunnel fails
        """
        data, error = self.runner.run_query(query, user)
        if error:
            raise Exception(error)
        if not data:
            return {'columns': [], 'rows': []}
    ...
    def get_schema(self, get_stats: bool = False) -> List[Dict[str, Any]]:
        """
        Get the database schema.
        
        Args:
            get_stats: Whether to include table statistics
            
        Returns:
            List of table definitions with their columns
        """
        return self.runner.get_schema(get_stats)
    ...
    def get_table_columns(self, table_name: str) -> List[str]:
        ...
        for table in schema:
            if table['name'] == table_name:
                return [col['name'] if isinstance(col, dict) else col for col in table['columns']]
        return []
    
    def get_table_types(self, table_name: str) -> Dict[str, Optional[str]]:
        """
        ...
        Returns:
            Dict mapping column names to their types (None if column is a string)
        """
        schema = self.get_schema()
        for table in schema:
            if table['name'] == table_name:
                return {
                    ...
                    col.get('type', TYPE_STRING) if isinstance(col, dict) else None 
