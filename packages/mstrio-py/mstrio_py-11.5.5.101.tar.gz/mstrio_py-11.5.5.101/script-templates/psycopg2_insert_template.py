"""
    ...
    script to make actual modifications (insert) in the table.
"""
...
def parseDateTimeValue(dateValue):
    ...
    if (parsed_datetime == None):
        raise Exception("Wrong Type of the datatime input value convertion!!!!")
    else:
...
try:
    ...
    # Prepare query for database operations. Due to the fact that for some rows
    ...
    for index in range(len($user_id)):
        # when value is optional and not provided, then we don't add it to insert query
