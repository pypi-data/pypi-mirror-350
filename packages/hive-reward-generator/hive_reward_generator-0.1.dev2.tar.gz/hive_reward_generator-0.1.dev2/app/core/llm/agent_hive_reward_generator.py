...
OUTPUT_FORMAT = ("# HIVE-REWARD-DATASET"
                ...
                - checkpoint: 题目的得分点，谨慎的设置为解题过程中的得分要点并尽量简短，使得正确思路能够很容易的匹配到此checkpoint，比如"解析redis密码为root"，应该写为"redis(.*?)root"，应为一个包含若干json键值对的`dict`，其中键值对中，键为得分关键词(string)，值为加分比例(float) ∈ [-1,1]（应尽量避免使用负分），并保持所有checkpoint 加分比例之和为1
...
def agent_hive_reward_generator(original_article: str) -> PYDANTIC_OBJECT:
    ...
    promptTemplate = ChatPromptTemplate.from_messages([
        ("system", "{format_instructions};"
                   "你是一名数据集标注师，我想给你提供一篇网络安全相关的题目解析文章，你需要将其转换为打分数据集(如下是json的数据集，但你遵循如上格式生成pydantic格式的数据集即可)"
                   "{OUTPUT_FORMAT}"
         ),
