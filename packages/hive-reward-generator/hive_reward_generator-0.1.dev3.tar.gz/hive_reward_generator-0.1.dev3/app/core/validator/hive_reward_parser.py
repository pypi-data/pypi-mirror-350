...
def calc_normal_mode_reward(checkpoint: dict, response: str) -> float:
    ...
    for keyword, score in checkpoint.items():
        if keyword in response:
            return score
        loguru.logger.warning("normal->"+keyword+" got zero")
        return 0.0
...
def calc_regex_mode_reward(checkpoint: dict, response: str) -> float:
    ...
    for restr, score in checkpoint.items():
        if re.match(restr, response):
            return score
        loguru.logger.warning("regex->"+restr+" get zero")
        return 0.0
        #return score if re.search(restr, response) else 0.0


def parse_hive_reward(hive_reward_data: json, article: str) -> float:
    """
    根据输入的hive-reward.json和response进行打分
    :param hive_reward_data: hive-reward.json 文件内容
    :param article: 文章内容
    :return: 打分，∈[-1, 0]，越接近0说明文章越符合hive-reward.json的要求
    """
    print(article)
    print(hive_reward_data)
    global WARNING_FLAG
    ...
    for i in range(len(data['checkpoint'])):
        if data['matchingmethod'][i] == 'regex':
            total_score += calc_regex_mode_reward(data['checkpoint'][i], article)
            continue
        elif data['matchingmethod'][i] == 'normal':
            pass
        else:
            WARNING_FLAG = True
        if  WARNING_FLAG:
        ...
        total_score += calc_normal_mode_reward(data['checkpoint'][i], article)
    return total_score
