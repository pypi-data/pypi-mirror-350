...
def build_select_query(
    ...
    all_params = table_params + column_placeholders + where_params + join_params + having_params
...
class JoinQuery:
    ...
    def placeholder_pair(self) -> Tuple[str, List[Any]]:
        ...
        table_expr, table_params = _format_table_name(self.table_name, validate=not self.ignore_forbidden_chars)
        if self.alias:
            table_expr = f"{table_expr} AS {self.alias}"

        return f"{self.join_type} JOIN {table_expr} ON {condition_str}", table_params + params
