            ...
            for table in doc["spec"]["tables"]:
                if "data" in table and "file" in table["data"]:
                    ...
                    if original_filename in file_mapping:
                        # Extract just the filename from the path, as the API expects just a filename
...
def prompt_for_file_paths(file_references: List[Tuple[str, str, str]]) -> Dict[str, str]:
    ...
    for table_name, _, file_name in file_references:
        print(f"  • Table '{table_name}' requires file '{file_name}'")
    ...
    for table_name, _, file_name in file_references:
        while True:
            ...
            if user_input and os.path.isfile(user_input):
                ...
                print(f"Error: '{user_input}' is not a valid file path. Please try again.")
...
def main(
    ...
    if not username or not password:
        typer.echo(
            "Error: Username and password are required. Provide them as arguments or environment variables.",
            err=True,
        )
        raise typer.Exit(code=1)
