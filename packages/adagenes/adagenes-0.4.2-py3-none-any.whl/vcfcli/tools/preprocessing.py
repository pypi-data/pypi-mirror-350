...
def get_variant_request_type(var):
    ...
    if re.compile(gencode.exp_refseq_transcript_pt).match(var):
        ...
        return "refseq_transcript", groups
...
def identify_query_parameters(query, gene_name_q=None, genompos_q=None):
    """
                ...
                if "variant_data" in query[var]:
                    if "type" in query[var]["variant_data"]:
                        ...
                        if query[var]["variant_data"]["type"]=="insertion":
                            var = vcfcli.tools.parse_indel_location(var,"insertion")
                            indels = indels + [var]
                        elif query[var]["variant_data"]["type"]=="deletion":
                            var = vcfcli.tools.parse_indel_location(var,"deletion")
                            indels = indels + [var]
