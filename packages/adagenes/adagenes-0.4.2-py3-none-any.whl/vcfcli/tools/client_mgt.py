...
def split_bframe(bframe, size_set1=None, size_set2=None):
    ...
    try:
        ...
        bframe_sub = vcfcli.BiomarkerFrame(data=bframe_sub_data)

        bframe1_sub_data = {key: bframe.data[key] for key in keys_set2}
        bframe1_sub = vcfcli.BiomarkerFrame(data=bframe1_sub_data)
...
def merge_bframes(bframe, bframe1):
    """
    Adds a dataset from a biomarker file to an existing biomarker frame

    :param bframe:
    :param infile:
    :return:
    """
    if bframe.genome_version != bframe1.genome_version:
        print("Error merging biomarker frames: Genome versions do not match")
        exit(1)
    bframe_comb = vcfcli.merge_dictionaries(bframe.data,bframe1.data)
    bframe.data = bframe_comb

    # TODO: merge meta data
...
def read_file(
    ...
    try:
        ...
        print(traceback.format_exc())
    return reader.read_file(infile_src,genome_version=genome_version,sep=sep,mapping=mapping)
...
def get_reader(infile_src, file_type=None, genome_version=None):
    ...
    if (file_type is None) or (file_type==""):
        file_type = vcfcli.get_file_type(infile_src)
        print("File type detected: ",file_type)

    if file_type == 'vcf':
        return vcfcli.VCFReader(genome_version=genome_version)
    if file_type == 'maf':
        return vcfcli.MAFReader(genome_version=genome_version)
    elif file_type == 'json':
        return vcfcli.JSONReader(genome_version=genome_version)
    elif file_type == 'avf':
        return vcfcli.AVFReader(genome_version=genome_version)
    elif file_type == 'tsv':
        return vcfcli.TSVReader(genome_version=genome_version)
    elif file_type == 'xlsx':
        return vcfcli.XLSXReader(genome_version=genome_version)
    elif file_type == 'txt':
        return vcfcli.TXTReader(genome_version=genome_version)
    elif file_type == 'csv':
        print("Generated reader: CSV")
        return vcfcli.CSVReader(genome_version=genome_version)
...
def get_writer(outfile_src, file_type=None):
    ...
    if (file_type is None) or (file_type==""):
        file_type = vcfcli.get_file_type(outfile_src)

    if file_type == 'vcf':
        return vcfcli.VCFWriter()
    elif file_type == 'json':
        return vcfcli.JSONWriter()
    elif file_type == 'tsv':
        return vcfcli.TSVWriter()
    elif file_type == 'csv':
        return vcfcli.CSVWriter()
    elif file_type == 'maf':
        return vcfcli.MAFWriter()
    elif file_type == 'xlsx':
        return vcfcli.XLSXWriter()
    elif file_type == 'avf':
        return vcfcli.AVFWriter()
