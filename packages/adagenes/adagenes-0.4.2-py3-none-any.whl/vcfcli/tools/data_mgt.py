...
def get_newest_variant_data_filename(id:str) -> str:
    """
    Searches for the newest annotated VCF file for a given ID. Returns the newest plain VCF file if no annotated file is found
...
def locate_latest_variant_file(base_path: str, filetype='vcf') -> str:

    file_mgt_file = base_path + "/" + config.__UPLOAD_FILE__
    if exists(file_mgt_file):
        df = pd.read_csv(base_path + "/" + config.__UPLOAD_FILE__,header=0,sep='\t')

        for i in range(1, df.shape[0]+1):
            latest_file = df.iloc[ df.shape[0]-i,0]
            format = df.iloc[ df.shape[0]-i,:].loc["input_format"]
            if format == filetype:
                print("return latest variant file ",latest_file, ",",df)
                return latest_file, df

    print("no latest file found ",file_mgt_file)
    return None
