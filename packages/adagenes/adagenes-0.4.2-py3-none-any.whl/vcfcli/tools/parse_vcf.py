...
import vcfcli
from vcfcli.tools.data_io import get_current_datetime
...
def generate_variant_data_section_all_variants(variant_data):
    for var in variant_data:
        ...
        chr, ref_seq, pos, ref, alt = vcfcli.parse_genome_position(var)
    return variant_data
...
def generate_variant_data_section(vcf_obj,qid=None):
    ...
    if config.variant_data_key not in vcf_obj:
        ...
        if qid is not None:
            chrom, ref_seq, pos, ref, alt = vcfcli.parse_genome_position(qid)
