...
def recognize_biomarker_types(bframe: vcfcli.BiomarkerFrame) -> vcfcli.BiomarkerFrame:
    """
...
class TypeRecognitionClient(client.Client):
    ...
    def process_data(self, biomarker_data):
        is_bframe = False
        if isinstance(biomarker_data, vcfcli.BiomarkerFrame):
            data = biomarker_data.data
            is_bframe = True
            bframe = biomarker_data
        else:
            data = biomarker_data
            bframe = vcfcli.BiomarkerFrame
            bframe.data = data

        #biomarker_data = vcfcli.tools.biomarker_types.get_biomarker_type(biomarker_data)
        bframe = vcfcli.tools.identify_biomarkers(bframe)
