...
from vcfcli.conf import vcf_config
from vcfcli.tools.parse_vcf import generate_variant_data_section,generate_vcf_columns
import traceback
import pandas as pd
...
class VCFWriter(writer.Writer):
        ...
        if save_headers:
            ...
            if len(json_obj.header_lines) > 0:
                ...
                if genome_version is not None:
                    print(vcfcli.conf.vcf_config.genome_version_line.format(genome_version),file=outfile)
                print(vcfcli.conf.vcf_config.base_info_line, file=outfile)
    ...
    def to_single_vcf_line(self, vcf_obj,mapping=None, labels=None, sort_features=True, sorted_features=None):
        ...
        try:
            ...
            #    columns = vcfcli.tools.parse_dataframes.get_tsv_labels(mapping=mapping,labels=labels)
