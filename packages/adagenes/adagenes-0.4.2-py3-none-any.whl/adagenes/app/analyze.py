...
import adagenes.app.parse_data
...
def load_file(qid, data_dir, filter = True, start_row=None, end_row=None, genome_version=None):
    """
    ...
    infile = adagenes.app.io.find_newest_file(data_dir, filter=filter, genome_version=genome_version)
...
def liftover(qid, genome_version, output_genome_version, data_dir=None, output_format='vcf'):
    ...
    #outfile = outfile.replace(" ", "_")
    #outfile = outfile.replace(":", "-")
    outfile = adagenes.app.tools.update_filename_with_current_datetime(infile, action="processed")
...
def apply_filter_sort(qid, filter_model, sort_model, data_dir=None, genome_version=None):
    ...
    data_dir = data_dir + "/" + qid
    active_filter = adagenes.app.io.get_active_filter(data_dir)

    active_sort = adagenes.app.io.get_active_sort(data_dir)
...
def apply_filters(qid, filter_model, sort_model=None, data_dir=None, output_format='vcf', genome_version=None):
    ...
    for column, filter_data in filter_model.items():
        ...
        if use_filtered_file is False:
            infile = adagenes.app.io.find_newest_file(data_dir, filter=use_filtered_file, genome_version=genome_version)
        else:
        ...
        if annotation_key not in previous_actions:
            ...
            outfile = adagenes.app.tools.update_filename_with_current_datetime(infile_name, action="filter",
                                                                               increase_count=True)
    ...
    if count_filters == 0:
        ...
        symlink_file = adagenes.app.tools.update_filename_with_current_datetime(most_annotated_file, action="processed", increase_count=True)
    ...
    if sort_model:
        ...
        if sort_found is False:
            ...
            try:
                ...
                outfile = adagenes.app.tools.update_filename_with_current_datetime(infile_name, action="sort")
...
def generate_table_data_from_bframe(bframe: ag.BiomarkerFrame, output_format=None):
    """
    ...
    if output_format == "vcf":
        ...
        for header_line in bframe.header_lines:
            #pass
            column_defs, columns = adagenes.app.parse_data.get_column_definition(header_line, column_defs, columns)
...
def analyze_uploaded_file(qid, data_dir = None,
    ...
    if annotate is not None:
        if len(list(annotate.keys())) > 0:
            ...
            adagenes.app.annotate.annotate_qid_chunks_parallel(qid, annotate, genome_version="hg38",
                                                               data_dir=data_dir)
            #pass
    ...
    bframe, file_type = load_file(qid, data_dir, filter=filter_file, start_row=start_row, end_row=end_row, genome_version=output_genome_version)
