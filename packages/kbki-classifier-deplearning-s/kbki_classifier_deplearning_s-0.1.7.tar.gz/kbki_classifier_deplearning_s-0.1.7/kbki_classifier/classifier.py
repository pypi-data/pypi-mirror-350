...
class KBKIClassifier:
    ...
    def _load_models(self):
        ...
        # Patch metadata model_paths to fix incorrect paths automatically
        self._fix_metadata_paths()

        self.model_paths = self.metadata['model_paths']

    def _fix_metadata_paths(self):
        """
        Perbaiki path di self.metadata['model_paths'] supaya sesuai struktur folder baru.
        Contohnya:
        - ganti "single_class" jadi "models/models/single_class"
        - ganti prefix "/content/models/" jadi "models/models/"
        """
        for key in self.metadata["model_paths"]:
            path = self.metadata["model_paths"][key]
            if path == "single_class":
                self.metadata["model_paths"][key] = str(self.model_dir / "models" / "single_class")
            else:
                # Ganti prefix /content/models/ dengan path yang benar
                if isinstance(path, str) and path.startswith("/content/models/"):
                    new_path = path.replace("/content/models/", str(self.model_dir / "models") + "/")
                    self.metadata["model_paths"][key] = new_path
                else:
                    # kalau sudah benar, biarkan saja
                    self.metadata["model_paths"][key] = path
    ...
    def predict(self, text):
        ...
        if model_path == "single_class":
            with open(self.model_dir / f"models/{p_category}/single_class.pkl", 'rb') as f:
        ...
        with open(self.model_dir / f"models/{p_category}/vectorizer.pkl", 'rb') as f:
            vectorizer = pickle.load(f)
        
        with open(self.model_dir / f"models/{p_category}/feature_names.pkl", 'rb') as f:
        ...
        pred_label = prediction['predict'][0, 0]
        pred_prob = float(prediction[pred_label][0, 0])
