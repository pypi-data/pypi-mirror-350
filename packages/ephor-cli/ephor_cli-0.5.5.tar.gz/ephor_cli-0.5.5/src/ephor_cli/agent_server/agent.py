...
def _create_llm_instance(model: Model, temperature: float = 0.2):
    ...
    if model.provider == LLMProvider.ANTHROPIC:
        return ChatAnthropic(model=model.name, temperature=temperature, max_tokens=8192)
    elif model.provider == LLMProvider.OPENAI:
        return ChatOpenAI(model=model.name, temperature=temperature, max_tokens=8192)
    elif model.provider == LLMProvider.GOOGLE:
        return ChatGoogleGenerativeAI(
            model=model.name, temperature=temperature, max_output_tokens=8192
        )
...
class Agent:
    def __init__(
        ...
        self.model = _create_llm_instance(model_obj, temperature)
