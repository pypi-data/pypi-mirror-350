#!/usr/bin/env python
"""
Tool for flashing .hex files to the ABIDrive via the STM built-in USB DFU mode.
"""
...
import fibre
import abidrive
from abidrive.utils import Event, OperationAbortedException
from abidrive.dfuse import *
...
class FirmwareFromGithub(Firmware):
    ...
    def __init__(self, release_json, asset_json):
        ...
        self.fw_version = abidrive.version.version_str_to_tuple(release_json['tag_name'])
    ...
    def get_as_hex(self):
        ...
        if self.hex is None:
            ...
            response = requests.get('https://api.github.com/repos/alireza0x0f/ABIDrive/releases/assets/' + str(self.github_asset_id),
...
def get_all_github_firmwares():
    response = requests.get('https://api.github.com/repos/alireza0x0f/ABIDrive/releases')
    if response.status_code != 200:
...
def put_into_dfu_mode(device, cancellation_token):
    ...
    if not hasattr(device, "enter_dfu_mode"):
        print("The firmware on device {:08X} cannot soft enter DFU mode.\n"
              ...
              "dfu-util, see the abidrive documentation.\n"
...
def get_hw_version_in_dfu_mode(dfudev):
    ...
    This is written on all ABIDrives sold since Summer 2018.
    """
    otp_sector = [s for s in dfudev.sectors if s['name'] == 'OTP Memory' and s['addr'] == 0x1fff7800][0]
...
def unlock_device(serial_number, cancellation_token):
    print("Looking for ABIDrive in DFU mode...")
    print("If the program hangs at this point, try to set the DFU switch to \"DFU\" and power cycle the ABIDrive.")
    ...
    print(" 1. Put the DFU switch on the ABIDrive to \"DFU\"")
    print(" 2. Power-cycle the ABIDrive")
    print(" 3. Run \"abidrivetool dfu\" (or any third party DFU tool)")
    print(" 4. Put the DFU switch on the ABIDrive to \"RUN\"")
...
def update_device(device, firmware, logger, cancellation_token):
    ...
    if hw_version < (3, 5, 0):
        ...
        print("Warning: DFU mode is not supported on ABIDrives earlier than v3.5 unless you perform a hardware mod.")
        if not abidrive.utils.yes_no_prompt("Do you still want to continue?", False):
    ...
    print("Found ABIDrive {} ({}) with firmware {}{}".format(
                serial_number,
    ...
    if firmware is None:
        if hw_version == (0, 0, 0):
            if dfudev is None:
                suggestion = 'You have to manually flash an up-to-date firmware to make automatic checks work. Run `abidrivetool dfu --help` for more info.'
            else:
                suggestion = 'Please contact info@abirobotics.com with your order number for help.'
    ...
    if firmware.fw_version <= fw_version:
        ...
        if firmware.fw_version < fw_version:
            ...
            print("You are about to flash firmware {} which is the same version as the firmware on the device ({}).".format(
                    get_fw_version_string(firmware.fw_version),
                    get_fw_version_string(fw_version)))
        if not abidrive.utils.yes_no_prompt("Do you want to flash this firmware anyway?", False):
            raise OperationAbortedException()
    ...
    if dfudev is None:
        ...
        if do_backup_config:
            abidrive.configuration.backup_config(device, None, logger)
    elif not abidrive.utils.yes_no_prompt("The configuration cannot be backed up because the device is already in DFU mode. The configuration may be lost after updating. Do you want to continue anyway?", True):
        raise OperationAbortedException()

    # Put the device into DFU mode if it's not already in DFU mode
    if dfudev is None:
        find_abidrive_cancellation_token = Event(cancellation_token)
        put_into_dfu_mode(device, find_abidrive_cancellation_token)
        stm_device = find_device_in_dfu_mode(serial_number, cancellation_token)
        find_abidrive_cancellation_token.set()
        dfudev = DfuDevice(stm_device)
    ...
    if hw_version is None:
        logger.error("Could not determine hardware version. Flashing precompiled "
                     ...
                     "https://abirobotics.com/abidrive/developer-guide for details.")
        # Jump to application
    ...
    if not found_in_dfu:
        ...
        device = abidrive.find_any(abidrive.default_usb_search_path, serial_number,
                        cancellation_token, timeout=30)

        if do_backup_config:
            temp_config_filename = abidrive.configuration.get_temp_config_filename(device)
            abidrive.configuration.restore_config(device, None, logger)
...
def launch_dfu(args, logger, cancellation_token):
    ...
    logger.info("Waiting for ABIDrive...")

    devices = [None, None]

    # Start background thread to scan for ABIDrives in DFU mode
    def find_device_in_dfu_mode_thread():
        devices[0] = find_device_in_dfu_mode(serial_number, find_abidrive_cancellation_token)
        find_abidrive_cancellation_token.set()
    ...
    # Scan for ABIDrives not in DFU mode
    # We only scan on USB because DFU is only implemented over USB
    devices[1] = abidrive.find_any(abidrive.default_usb_search_path, serial_number,
        find_abidrive_cancellation_token)
    find_abidrive_cancellation_token.set()
