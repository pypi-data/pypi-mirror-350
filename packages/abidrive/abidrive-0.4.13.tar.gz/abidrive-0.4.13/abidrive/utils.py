...
try:
    ...
    print("Refer to install instructions at http://abirobotics.com/ABIdrive/#downloading-and-installing-tools")
    sys.stdout.flush()
    pass
...
def dump_errors(odrv, clear=False, printfunc = print):
    ...
    system_error_codes = {v: k for k, v in abidrive.enums.__dict__ .items() if k.startswith("ABIDRIVE_ERROR_")}
    ...
    for name, axis in axes:
        ...
        module_decode_map = [
            ('axis', axis, 'error', {v: k for k, v in abidrive.enums.__dict__ .items() if k.startswith("AXIS_ERROR_")}),
            ('motor', axis, 'motor.error', {v: k for k, v in abidrive.enums.__dict__ .items() if k.startswith("MOTOR_ERROR_")}),
            ('sensorless_estimator', axis, 'sensorless_estimator.error', {v: k for k, v in abidrive.enums.__dict__ .items() if k.startswith("SENSORLESS_ESTIMATOR_ERROR")}),
            ('encoder', axis, 'encoder.error', {v: k for k, v in abidrive.enums.__dict__ .items() if k.startswith("ENCODER_ERROR_")}),
            ('controller', axis, 'controller.error', {v: k for k, v in abidrive.enums.__dict__ .items() if k.startswith("CONTROLLER_ERROR_")}),
        ]
