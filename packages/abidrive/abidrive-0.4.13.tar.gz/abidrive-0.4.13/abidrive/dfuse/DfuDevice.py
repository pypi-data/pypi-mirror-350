...
import array
import time
from abidrive.dfuse.DfuState import DfuState
import math
...
def make_exception(status):
    if status[0] == 11: # errVENDOR
        suffix = " - Try running \"abidrivetool unlock\" and then try \"abidrivetool dfu\" again."
    else:
        suffix = ""
    ...
    def get_state(self):
        ...
        if len(msg) == 0:
            raise Exception("Could not get device state. Firmware upgrade will abort. "
                            "Please try again. If abidrivetool can't find the device "
                            "anymore after this, follow the instructions in "
                            "https://abirobotics.com/ABIdrive/abidrivetool#device-firmware-update "
