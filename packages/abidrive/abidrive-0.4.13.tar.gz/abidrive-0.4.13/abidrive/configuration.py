...
import abidrive
from abidrive.utils import OperationAbortedException, yes_no_prompt
...
def get_temp_config_filename(device):
    serial_number = abidrive.get_serial_number_str_sync(device)
    safe_serial_number = ''.join(filter(str.isalnum, serial_number))
    return os.path.join(tempfile.gettempdir(), 'abidrive-config-{}.json'.format(safe_serial_number))

def backup_config(device, filename, logger):
    """
    Exports the configuration of an ABIDrive to a JSON file.
