    ...
    def train(
        ...
        if val_set:
            ...
            self.best_model, histories = trainer.train(
                self.model, train_set, val_set, collate_fn, sampler, show_progress, amp, eps
            )
        else:
            trainer = Trainer(
                epochs,
            ...
            self.best_model, histories = trainer.train(
                self.model, train_set, collate_fn, show_progress, amp, eps
            )
        return histories
...
class FastModelWrapper(ModelWrapper):
    ...
    def train(
        ...
        return super().train(train_set, val_set, collate_fn, epochs, optimizer, scheduler, lr, T_max,
                                ...
                                drop_last, checkpoint_per_rounds, checkpoint_name, show_progress, amp=amp, eps=eps)
...
class SplitModelWrapper(FastModelWrapper):
    ...
    def train(
        ...
        return super().train(X_train, y_train, val_data, collate_fn, epochs, optimizer, scheduler, lr, T_max,
                            batch_size, eval_batch_size, sampler, num_workers, num_eval_workers, pin_memory,
                            pin_memory_device, persistent_workers, early_stopping_rounds, print_per_rounds, 
                            drop_last, checkpoint_per_rounds, checkpoint_name, show_progress, amp, eps)
...
class ClassifyModelWrapper(ModelWrapper):
    ...
    def train(
        ...
        eval_batch_size: int = 128,
        sampler: Sampler = None,
        weight: Union[torch.Tensor, np.ndarray, List] = None,
        num_workers: int = 0,
        num_eval_workers: int = 0,
        pin_memory: bool = False,
        pin_memory_device: str = "",
        ...
        if val_set:
            ...
            self.best_model, histories = trainer.train(
                self.model, train_set, val_set, collate_fn, show_progress, sampler, weight, amp, eps, monitor
            )
        else:
            ...
            self.best_model, histories = trainer.train(
                self.model, train_set, collate_fn, show_progress, sampler, weight, amp, eps, monitor
            )
        return histories
...
class FastClassifyModelWrapper(ClassifyModelWrapper):
    ...
    def train(
        ...
        batch_size: int = 64,
        eval_batch_size: int = 128,
        sampler: Union[Sampler, bool] = None,
        weight: Union[torch.Tensor, np.ndarray, List] = None,
        num_workers: int = 0,
        num_eval_workers: int = 0,
        pin_memory: bool = False,
        ...
        return super().train(train_set, val_set, collate_fn, epochs, optimizer, scheduler, lr, T_max,
                            ...
                            pin_memory_device, persistent_workers, early_stopping_rounds, print_per_rounds, 
                            drop_last, checkpoint_per_rounds, checkpoint_name, show_progress, amp, eps, monitor)
...
class RegressModelWrapper(ModelWrapper):
    ...
    def train(
        ...
        if val_set:
            ...
            self.best_model, histories = trainer.train(
                self.model, train_set, val_set, collate_fn, sampler, show_progress, amp, eps, monitor
            )
        else:
            ...
            self.best_model, histories = trainer.train(
                self.model, train_set, collate_fn, sampler, show_progress, amp, eps, monitor
            )
        return histories
...
class FastRegressModelWrapper(RegressModelWrapper):
    ...
    def train_evaluate(
        ...
        return super().train_evaluate(train_set, val_set, collate_fn, epochs, optimizer, scheduler, lr, T_max,
                            batch_size, eval_batch_size, sampler, num_workers, num_eval_workers, pin_memory,
                            pin_memory_device, persistent_workers, early_stopping_rounds, print_per_rounds, 
                            drop_last, checkpoint_per_rounds, checkpoint_name, show_progress, amp, eps, monitor, verbose)
