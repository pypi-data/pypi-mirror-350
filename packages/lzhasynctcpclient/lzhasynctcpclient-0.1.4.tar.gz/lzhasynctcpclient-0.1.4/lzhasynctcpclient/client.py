...
logger = logging.getLogger("AsyncTcpClient")
formatter = logging.Formatter("%(asctime)s,%(levelname)+7s,[%(filename)s:%(lineno)d %(funcName)s],%(message)s", datefmt='%Y-%m-%d %H:%M:%S')
...
class AsyncTcpClient:
        ...
        def run():
            ...
            while not self._stop_event.is_set():
                try:
                    ...
                    logger.error(f"[{self._name}] Event loop error: {e}")
                    logger.info(f"[{self._name}] Reconnecting in {self._reconnect_interval} seconds...")
                    import time
    ...
    def start(self):
        if self._thread and not self._thread.is_alive():
            self._thread.start()
            logger.info(f"[{self._name}] Thread started")

    def stop(self):
        self._stop_event.set()
        if self._loop:
            self._loop.call_soon_threadsafe(self._loop.stop)
        logger.info(f"[{self._name}] Stopped client")
    ...
    def send(self, message: str):
        try:
            ...
            logger.error(f"[{self._name}] Failed to send message: {e}")
    ...
    async def _main_loop(self):
        while not self._stop_event.is_set():
            try:
                logger.info(f"[{self._name}] Connecting to {self._host}:{self._port}")
                self._reader, self._writer = await asyncio.open_connection(self._host, self._port)
                logger.info(f"[{self._name}] Connected to server")
                if self._on_connect:
                    self._on_connect()
                ...
                logger.warning(f"[{self._name}] Connection failed: {e}")
            finally:
                if self._writer:
                    self._writer.close()
    ...
    async def _recv_loop(self):
        try:
            ...
            logger.warning(f"[{self._name}] Connection closed by server")
        except Exception as e:
    ...
    async def _heartbeat_loop(self):
        try:
            ...
            logger.error(f"[{self._name}] Heartbeat error: {e}")
            raise
