...
class Oso:
    ...
    def authorize(
        ...
        parityHandle: Optional[ParityHandle] = None,
    ) -> bool:
        """Check a permission:
        ...
        otherwise false.
        """
        if parityHandle is not None and not isinstance(parityHandle, ParityHandle):
            raise TypeError("parityHandle must be an instance of ParityHandle")

        if isinstance(context_facts, ParityHandle):
            raise ValueError(
                "Received a ParityHandle as the 'context_facts' argument. "
                "ParityHandle must be passed as a keyword argument, e.g. parityHandle=parityHandle1"
            )
        ...
        result = self.api.post_authorize(data, parityHandle)
    ...
    def authorize_local(
        ...
        result = self.api.post_authorize_query(data, parityHandle)
        return result.sql
