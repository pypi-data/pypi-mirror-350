...
def sendmail(subject, text, receivers, sender="", attachments=None, 
             trace=False, method='s-nail'):
    # method for sending mails
    if trace: print("Sending mail to {}".format(receivers))
    ...
    if "\n" in text and "\\n" not in text:
        text = text.replace("\n", "\\n")
    receivers = " " + (receivers if isinstance(receivers, str) 
                       else " ".join(receivers))
    sender = sender and " -r {}".format(sender)
    attachments = attachments and " " + \
        " ".join("-a {}".format(a) for a in attachments) or ""
    command = 'echo -e "' + text + '" '

    try:
        if not shutil or not shutil.which(method):
            raise Exception('{} not available!'.format(method))
    except Exception as e:
        if trace: print('sendmail(...): {}; defaults to system mail'.format(e))
        method = 'mail'

    command += '| {} -s "{}" '.format(method,subject)
