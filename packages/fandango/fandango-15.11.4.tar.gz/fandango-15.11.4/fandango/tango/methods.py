    ...
    try:
        ...
        di = Struct(
            [
                (k, v)
                for k, v in zip(
                    ("name", "ior", "version", "server", "host", "started", "stopped"),
                    vals[1],
                )
            ]
        )
...
@Cached(depth=100, expire=30)
def get_alias_dict(exp="*",host=''):
    """
    returns an {alias:device} dictionary with all matching alias from Tango DB
    :param exp:
    """
    tangodb = get_database(host)
    return {k: tangodb.get_device_alias(k) 
            for k in tangodb.get_device_alias_list(exp)}
...
def parse_tango_model(name, use_host=False, fqdn=None, *args, **kwargs):
    ...
    The Accepted URI format is:
    
    [tango://][<host1:port1>][,<host2>:port2][/][domain/family/member][/attribute][?query1=value1&query2=value2][#fragment];[...model2...]
    
    If present, #fragment will attempt for a member of the returned object value (value, quality, time, type, format, name)
    otherwise, query will be used to access class, proxy or property values.
        ?property=<property_name>
        ?info=server_host
        ?get_config=label
    
    A model containing just host and port; will match the database server, this
    specific model can be used to query class or service properties.
    
    Servers, Classes, and Objects will be queried using an specific syntax
        ?dserver/<class>/<instance>
        ?class=<class_name>&property=<property_name>
        ?service=<service>&property=<property_name>
        ?free=<free system property>&property=<property_name>
        
    According to URL specs, queries should precede fragments: https://en.wikipedia.org/wiki/URI_fragment
    ...
    r.fragment = (name.split("#", 1)[1] if "#" in name else ("")).split('?')[0]
    try:
        r.query = dict(t.split('=',1) for t in 
            (name.split("?", 1)[1] if "?" in name else ("")).split('&')
            )
        r.property = r.query.get('property','')
    except:
        r.query = {}
        r.property = ""        
    ...
    if not r.get("device"):
        #print('Matching a database model for free/class properties')
        m = re.match(fandango.tango.rehost_short, name)
        if not m:
            m = re.match(fandango.tango.rehost_short + fandango.tango.reattr, name)
        if not m:
            # print('%s does not match a tango model' % name)
            return None
        gd = m.groupdict()
        if 'host' not in gd:
            return None
        r.tango_class = gd.get('attribute','')
        r.authority = r.tango_host = gd.get("host",None)
        r.device = r.devicemodel = r.attribute = ''
...
def get_device_labels(target, filters="", brief=True):
    """
    Returns an {attr:label} dict for all attributes of this device with labels.
    Or those attribute/labels matching the passed filter.
    Filters will be a regular expression to apply to attr or label.
    If brief is True (default) attributes are obtained offline.
    """
    labels = {}
    if isString(target):
        d = get_device(target)
    ...
    for a in attrlist:
        ...
        if ((filters and (matchCl(filters,a) or matchCl(filters,l))) or
            (not filters and (not brief or l!=a))):
            labels[a] = l
...
def set_attribute_config(device, attribute, config, events=True, verbose=False):
    ...
    config may be a dictionary or an AttributeInfoEx object
    """
    ...
    if isinstance(config, (PyTango.AttributeInfo,PyTango.AttributeInfoEx)):
        dp.set_attribute_config(attribute)
...
def get_attribute_polling(target):
    try:
        dp, attr = get_device(target),target.split('/')[-1]
        return dp.get_attribute_poll_period(attr)
    except:
        return None


def check_attribute_events(
    model, ev_type=None, verbose=False, asynch=False,
    keep=False, wait=0.01, min_tango_version = 700):
    """
    This method expects model and a list of event types.
    If empty, CHANGE and ARCHIVE events are tested.
    
    min_tango_version >= 800 can be used to discard notifd events
    ...
    try:
        ...
        if check_device(dp):
            ...
            if min_tango_version < 100:
                min_tango_version = min_tango_version * 100

            if dp.get_tango_lib_version() < min_tango_version:
                # Old tango events not supported
                return False
            ...
            for ev_type in list(result.keys()):
                try:
                    ...
                    def hook(self, *args, **kwargs):
                        ...
                        if verbose:
                            #print not working on event hook?
                            print("%s.%s.event_received: %s" % (
                                attr, ev_type, args))
...
def get_black_box(device, n=100):
    ...
    data = sorted((t for t in map(parse_black_box_row, bb)),
                   key = (lambda _:_["time"]))
    j, t, l = 0, 0, ""
    for i, row in enumerate(data[1:]):
        r = row['time'] - data[i]['time']  # time between commands
        if r > t:
            j, t, l = i, r, row['row']
        row["rtime"] = r
        row["load"] = float(r) if r > 0 else 0
    print("slower command (%f s) was: %s" % (t, l))
    return {_['time']:_ for _ in data}


parse_black_box = get_black_box  # backwards compatibility

def get_subdevices(pattern = None):
    dbd = get_database_device()
    q = "select device,count,value from property_device where name like '__SubDevices'"
    if pattern:
        q += " and device like '{}'".format(pattern.replace('*','%'))
    q += " order by device,count"
    r = dbd.DbMySqlSelect(q)[1]
    subdevices = defaultdict(list)
    i = 0
    while i < len(r):
        subdevices[r[i]].append(r[i+2])
        i+=3
    return subdevices
...
class AttrInvalid(Exception):
    pass

class AttrValueExpired(Exception):
    pass
...
def check_attribute(model, attr='', readable=True, timeout=0, brief=False, 
    ...
    try:
        ...
        if readable and value.quality == AttrQuality.ATTR_INVALID:
            raise AttrInvalid(attr)
        elif expire and value.time.totime() < (time.time() - expire):
            raise AttrValueExpired(attr,value)
        else:
            if brief:
...
def read_attribute(model, attr='', timeout=0, full=False):
    """
    Helper for check_attribute(attr,brief=True)
