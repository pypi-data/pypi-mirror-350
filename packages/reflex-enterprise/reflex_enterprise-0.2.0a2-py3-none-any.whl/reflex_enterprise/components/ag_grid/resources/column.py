...
from reflex.utils.types import _isinstance, typehint_issubclass
from reflex.vars import FunctionVar
from reflex.vars.base import Var
from reflex.vars.function import ArgsFunctionOperation

from ..resource import AGEditors, AGFilters, value_func_factory
...
class ColumnDef(
    ...
    @staticmethod
    def get_js_expressions(camel_case: bool = False) -> dict[str, Any]:
        """Get the list of fields that are JS expressions."""
        return {
            to_camel_case(key) if camel_case else key: value.type_
            for key, value in ColumnDef.__fields__.items()
    ...
    def dict(self, *args, **kwargs):
        ...
        for key, exp_types in self.get_js_expressions(camel_case=True).items():
            if key in return_dict:
                ...
                elif not _isinstance(return_dict[key], exp_types):
                    console.warn(
                        f"Unexpected type for JS expression: {type(return_dict[key])}. Expected one of: {exp_types}."
                    )
        return return_dict
