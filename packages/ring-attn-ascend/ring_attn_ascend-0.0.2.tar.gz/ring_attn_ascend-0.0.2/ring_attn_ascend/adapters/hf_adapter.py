...
def create_ring_flash_attention_forward(
    ...
    def _flash_attention_forward(
        ...
        """
        assert use_top_left_mask == False, "only use_top_left_mask==False is supported yet"
        ...
        assert is_causal, "only causal attention is supported yet."
        batch_size = query_states.size(0)
        assert batch_size == 1, "varlen data should be processed in advance."

        attn_mask = torch.triu(torch.ones([2048, 2048], device=query_states.device), diagonal=1).bool()
