...
class PlatformIdProvider(HardwareIdProvider):
    ...
    def _get_linux_id(self):
        return self._read_from_file("/var/lib/dbus/machine-id") or \
               self._read_from_file("/etc/machine-id")

    def _get_bsd_id(self):
        return self._read_from_file("/etc/hostid") or \
               self._execute_command("kenv -q smbios.system.uuid")
