...
class Result(Generic[T, E]):
    ...
    def unwrap(self) -> T:
        ...
        if self.is_err():
            raise self.err if isinstance(self.err, Exception) else ValueError(self.err)
        return self.value
    ...
    def expect(self, message: str) -> T:
        ...
        if self.is_err():
            raise RuntimeError(f"{message}: {self.err}")
        return self.value

    def map(self, func: Callable[[T], U]) -> "Result[U, E]":
        """链式转换成功值（类似 Functor）"""
        return Result.ok(func(self.value)) if self.is_ok() else Result.err(self.err)

    def and_then(self, func: Callable[[T], "Result[U, E]"]) -> "Result[U, E]":
        """Monadic 绑定操作（类似 flatMap）"""
        return func(self.value) if self.is_ok() else Result.err(self.err)

    def match(self, ok_handler: Callable[[T], U], err_handler: Callable[[E], U]) -> U:
        """模式匹配处理"""
        return ok_handler(self.value) if self.is_ok() else err_handler(self.err)

    def __repr__(self) -> str:
        """序列化支持"""
        return f"Result(value={self.value}, err={self.err})"

    def to_dict(self) -> Dict[str, Optional[Union[T, E]]]:
        """转换为 API 友好格式"""
        return {"ok": self.value, "err": self.err}

    @classmethod
    def ok(cls, value: T) -> "Result[T, E]":
        """构造成功结果"""
        return cls(value=value, err=None)

    @classmethod
    def err(cls, error: E) -> "Result[T, E]":
        """构造失败结果"""
        return cls(value=None, err=error)
