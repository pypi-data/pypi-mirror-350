...
class USBConnection(Connection):
    ...
    def __init__(self, vendor_id: int, product_id: int, serial_number: str = None):
        ...
        self.read_connection_state = mp.Value('i', ConnectionState.INIT)
    ...
    def is_connected(self):
        return (self.read_connection_state.value == ConnectionState.CONNECTED)
    ...
    def start(self):
        ...
        self.usb_task_read = mp.Process(target=usb_task_read, args=(out_conn_read, usb_device_hash, self.read_connection_state, tx_ids, rx_queues, self.global_receive_queue, self.node_update_queue))
