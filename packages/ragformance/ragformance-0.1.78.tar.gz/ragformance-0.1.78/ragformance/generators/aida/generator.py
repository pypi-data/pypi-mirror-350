...
# Assuming helper functions/classes will be imported from the original script
# or moved here. For now, let's assume they are in 'capella_pdf_qa_generator_script'
# which would be the renamed or refactored original file.
from .capella_pdf_qa_generator import (
    build_network,
    build_name_index,
    PyPDFium2Loader, # If used directly, else it's within a helper
    RecursiveCharacterTextSplitter, # If used directly
    HuggingFaceEmbeddings, # If used directly
    Chroma, # If used directly
    setup_entity_extractor,
    setup_qa_llm,
    generate_qa_set,
    # Pydantic models used by generate_qa_set and setup_qa_llm if they are not directly returned
    EntityList,
    QA,
    QASet,
)
...
class AidaGenerator(RAGformanceGeneratorInterface):
    def run(self, config: Dict) -> Tuple[List[DocModel], List[AnnotatedQueryModel]]:
        ...
        """
        seed_questions_path = Path(config["seed_questions_path"])
        data_dir = Path(config["data_dir"])
        output_dir = Path(config["output_dir"])
        openrouter_key = config["llm_api_key"] # Assuming generic key name
        openrouter_base_url = config["llm_base_url"] # Assuming generic base url
        hf_embed_model_name = config["hf_embed_model"] # Renamed from hf_embed_model for clarity with HuggingFaceEmbeddings
        capella_path = Path(config["capella_xml_path"]) 
        entity_model_name = config["entity_model_name"]
        qa_model_name = config["qa_model_name"]

        chunk_size = config.get("chunk_size", 750)
        chunk_overlap = config.get("chunk_overlap", 100)
        persist_dir = Path(config.get("persist_dir", "chroma_index"))
        k_pdf = config.get("k_pdf", 5)
        k_capella = config.get("k_capella", 8)
        ...
        for pdf_file_path in sorted(data_dir.glob("*.pdf")):
            ...
            pages.extend(loaded)
            print(f"[AidaGenerator.run]   Loaded {len(loaded)} pages from {pdf_file_path.name}")
        ...
        for idx, chunk_doc in enumerate(raw_chunks):
            ...
            corpus_docs_intermediate.append(type(chunk_doc)(page_content=chunk_doc.page_content, metadata=meta))

        print(f"[AidaGenerator.run]   Split into {len(corpus_docs_intermediate)} chunks")

        # 4) Build & persist Chroma index
        emb_model_service = HuggingFaceEmbeddings(model_name=hf_embed_model_name) # Changed variable name
        vectordb = Chroma.from_documents(corpus_docs_intermediate, embedding=emb_model_service, persist_directory=str(persist_dir))
        vectordb.persist()
        print(f"[AidaGenerator.run]   Chroma index persisted at {persist_dir}")

        # 5) Setup LLMs & parsers
        extract_prompt_template, extract_response_model,         entity_model_name_out, entity_api_key_out, entity_api_base_out = setup_entity_extractor(
        ...
        with queries_jsonl_path.open("w", encoding="utf-8") as qf:
            for qi, seed_query_text in enumerate(seeds):
                qa_set_as_dict, src_map = generate_qa_set( # generate_qa_set returns a dict and src_map
                        ...
                        if src_map and original_sources:
                            for src_tag_raw in original_sources:
                                ...
                                if src_tag_cleaned in src_map:
                                    ...
                                    if mapped_info.get("kind") == "pdf_chunk_id": # Assuming we can add 'pdf_chunk_id' to src_map
                                         relevant_docs_list.append({"corpus_id": mapped_info["pdf_chunk_id"], "score": 1})
