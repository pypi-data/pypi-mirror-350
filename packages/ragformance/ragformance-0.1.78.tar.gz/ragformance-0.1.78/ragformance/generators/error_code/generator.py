...
from .question_generation import (
    ...
    # generate_med_question, # Kept for future, but current main.py comments it out
)
...
class ErrorCodeGenerator(RAGformanceGeneratorInterface):
    def run(self, config: Dict) -> Tuple[List[DocModel], List[AnnotatedQueryModel]]:
        ...
        """
        folder_path = config["data_path"] # This path should contain output.md and page_X.md files
        output_path = config["output_path"]
        prefix_id = config["corpus_id_prefix"]
        title = config["document_title"]
        api_key = config["llm_api_key"]
        api_url = config["llm_base_url"] 
        api_model = config["llm_model_name"]
        
        error_keywords = config.get("error_keywords", "error, information, or alarm codes (e.g., E09)")
        max_token_context = config.get("max_token_context", 64000)
        question_tags = config.get("question_tags", [])
        question_category = config.get("question_category", "error code")

        if not os.path.exists(output_path):
            os.makedirs(output_path)

        # --- Logic adapted from the original error_code/main.py run function ---

        # Assuming output.md is the main manifest or full manual text
        manual_file_path = os.path.join(folder_path, "output.md")
        if not os.path.exists(manual_file_path):
            raise FileNotFoundError(f"Expected 'output.md' not found in data_path: {folder_path}")
        ...
        if not page_numbers or len(page_numbers) == 0:
            print("No relevant page numbers found for error codes. Skipping generation.")
            return [], []
        ...
        merged_pages_text, initial_corpus_dicts = merge_pages(page_numbers, folder_path, prefix_id, title)
        ...
        if not extracted_error_data:
            ...
            return [], [] # Or return initial_corpus_dicts as DocModels if that's desired
        ...
        # Placeholder for medium questions (currently commented out in original main.py)
        # med_queries, med_query_augmentation = generate_med_question(...)
        # easy_queries_dicts.extend(med_queries)
        # query_augmentation_list.extend(med_query_augmentation)
        # page_numbers.extend(med_page_numbers) # Need to handle page_numbers for med questions if added
        ...
        final_queries_dicts = add_augmented_question(easy_queries_dicts, augmented_question_variations)
        ...
        # If medium questions add more pages, this needs to be updated. For now, it's based on error pages.
        final_corpus_docs: List[DocModel] = [DocModel(**doc) for doc in initial_corpus_dicts]
        ...
        for query_dict in final_queries_dicts:
            ...
            final_annotated_queries.append(AnnotatedQueryModel(
                _id=query_dict["_id"],
                query_text=query_dict["query_text"],
                relevant_document_ids=query_dict.get("relevant_document_ids", []),
                ref_answer=query_dict.get("ref_anwser", ""), # Typo "ref_anwser" in original generate_easy_question
