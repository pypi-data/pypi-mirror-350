...
def question_variation(question, nombre_variantes=5, API_KEY=None, API_URL="https://openrouter.ai/api/v1/chat/completions", API_MODEL="qwen/qwen3-32b:free"):
    """
...
def generate_easy_question(tabular_data, category="None", tags=["None"], prefix_id=""):
    ...
    for idx, (error_code, (text, page_num)) in enumerate(tabular_data.items(), start=1):
        json_obj = {
            ...
            "relevant_document_ids": [{"corpus_id": f'DOC_{prefix_id}_p{page_num}', "score":1}],
        ...
        query_augmentation.append({"_id": f"{prefix_id}_EASY_{idx}", "query_text": f"Que signifie le code d'erreur {error_code} ?"})
        output.append(json_obj)
    
    return output, query_augmentation


def generate_med_question(tabular_data, document, category="None", tags=["None"], prefix_id="", API_KEY=None, API_URL="https://openrouter.ai/api/v1/chat/completions", API_MODEL="qwen/qwen3-32b:free"):
    output, query_augmentation = [], []
        ...
        if response.status_code == 200:
            ...
            try:
                ...
                json_obj = {
                    ...
                    "relevant_document_ids": [{"corpus_id": f'DOC_{prefix_id}_p{i}', "score":1} for i in np.unique(all_pages)],
                    "ref_answer": message["result"], # Corrected typo from ref_anwser to ref_answer
                    "metadata": {
                        "category": category,
                        "difficulty": "Medium",
                        "tags": tags
                ...
                query_augmentation.append({"_id": f"{prefix_id}_MED_{idx}", "query_text": f"Quels sont les étapes pour résoudre le problème lié au code erreur {error_code} ?"})
                output.append(json_obj)
            except:
                print(f"Error on json file:", message)
                continue
        else:
