...
def _get_embedding_model():
    if not _SENTENCE_TRANSFORMERS_AVAILABLE:
        raise ImportError(
            ...
            "Please install ragformance with the [visualization] option:\n"
            "    pip install ragformance[visualization]"
        )
...
def visualize_semantic_F1(
    ...
    model_quadrants = defaultdict(lambda: {
    ...
    for a in answers:
        query_id = a.query.id
        relevant_docs = set(doc.corpus_id for doc in a.query.relevant_document_ids)
        ...
        semantic_score = np.mean([
            max(0, min(1, cosine_sim(query_embedding, doc_embedding)))
    ...
    display_data = grouped_data if not is_single_model else {"Overall": list(grouped_data.values())[0]}
    display_quadrants = model_quadrants if not is_single_model else {"Overall": list(model_quadrants.values())[0]}
    ...
    for model_key, points in display_data.items():
        ...
        fig.update_layout(
            title=f"Semantic Similarity of queries, number of documents retrieved as color – {model_key}" if not is_single_model else "Semantic Similarity of queries, number of documents retrieved as color",
