...
# parsing_engine functions will be inlined below
import pandas as pd
...
from typing import List, Dict, Any, Tuple # Added Dict, Any, Tuple
...
# --- Functions inlined from parsing_engine.py ---
...
def _extract_tag_content(text: str, tag: str) -> str:
    ...
    """
    if not text: return "" # Added guard for None or empty text
    try:
...
def parse_qa_pairs_from_response(raw_response: str) -> list[dict[str, Any]]:
    ...
    Priority: <output_json>, ```json, best-effort bracket.
    """
    ...
    # Fallback to simpler QA pair extraction if the above fail or return non-list/empty.
    # This part is more aligned with the original intent for simpler XML-like Q&A.
    qa_pairs = []
    ...
    if qa_pairs: # If <qa_pair> yielded results, use them.
        return qa_pairs

    # Fallback to individual <question> and <answer> tags if no <qa_pair> found
    # This is kept from original logic as a further fallback.
    questions = re.findall(r"<question>(.*?)</question>", raw_response, re.DOTALL)
    answers = re.findall(r"<answer>(.*?)</answer>", raw_response, re.DOTALL)
    
    if questions and answers: # Only proceed if both lists have content
        for q_text, a_text in zip(questions, answers): # zip will stop at the shorter list
            qa_pairs.append({"question": q_text.strip(), "answer": a_text.strip()})
        return qa_pairs
        
    return [] # Return empty list if no structured data found

# --- End of inlined functions from parsing_engine.py ---
...
def _chunk_document_fast(
    ...
    Returns:
        List[Tuple[str, str]]: A list of chunk tuples (chunk_id, chunk_text).
    """
...
def generate_questions(
    chunks: List[Tuple[str, str]], # Expect list of (chunk_id, chunk_text)
    file_name_md: str, # Original markdown file name e.g. "doc.md"
    doc_summary: str, # Summary of the document
    ...
    for chunk_id_str, chunk_text in chunks:
        ...
        if not raw_response_content:
            print(f"No response from LLM for chunk {chunk_id_str} in file {file_name_md}")
            continue
        ...
        for qa_pair in parsed_qa_list:
            ...
            if not question or not answer:
                print(f"Missing question or answer in parsed pair for chunk {chunk_id_str}, file {file_name_md}")
                continue
...
def parquet_to_jsonl(parquet_folder_path: str) -> Tuple[List[DocModel], List[AnnotatedQueryModel]]:
    ...
    for file_name in os.listdir(parquet_folder_path):
        if file_name.endswith(".parquet"):
            ...
            for row in df.itertuples(index=False):
                ...
                chunk_id = str(getattr(row, "chunk_id", "")) # Ensure string
                chunk_text = getattr(row, "chunk_text", "")
                question_text = getattr(row, "question", "")
                answer_text = getattr(row, "answer", "")

                if not chunk_id or not chunk_text:
                    print(f"Skipping row due to missing chunk_id or chunk_text in {file_name}")
                    continue
    ...
    # Deduplicate corpus_list based on '_id', keeping the first encountered item
    if corpus_list:
        seen_ids = set()
        deduplicated_corpus = []
    ...
    # Define paths for JSONL files within the same folder as parquets
...
# Run function is removed, its logic is now in AlphaGenerator
