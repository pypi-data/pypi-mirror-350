...
from langchain_community.document_loaders import DirectoryLoader, TextLoader # Added TextLoader
...
def _validate_input_files(datapath_str: str) -> bool:
    ...
    if datapath_obj.is_dir():
        ...
        valid_files_in_dir = [f for f in files_in_dir if f.lower().endswith((".txt", ".md"))]
...
# _validate_additional_arguments was removed.
    ...
    for node_a, rel, node_b in result:
        ...
        for node in [node_a, node_b]: # Check both nodes
            if getattr(node, "type", None) == NodeType.CHUNK:
                current_corpus_text = getattr(node, "properties", {}).get("page_content", "")
                ...
                doc_node_for_title = node_a if getattr(node_b, "type", None) == NodeType.DOCUMENT else (node_b if getattr(node_a, "type", None) == NodeType.DOCUMENT else None)
                if not doc_node_for_title: # If CHUNK is paired with another CHUNK, search for related DOCUMENT
                    try:
                        # This is a conceptual search; actual KG traversal might differ
                        related_docs = [n for n_id in kg. G.neighbors(node.id) for n in [kg.G.nodes[n_id]['data']] if getattr(n, "type", None) == NodeType.DOCUMENT]
                        if related_docs: doc_node_for_title = related_docs[0]
                    except: pass # KG structure might not support this easily

                if doc_node_for_title:
                     current_corpus_title = getattr(doc_node_for_title, "properties", {}).get("document_metadata", {}).get("source", "")
            ...
            for node in [node_a, node_b]:
                 if getattr(node, "type", None) == NodeType.DOCUMENT:
                    ...
                    if current_corpus_text is None: # If CHUNK text wasn't found
                         current_corpus_text = getattr(node, "properties", {}).get("page_content", "") # Some docs might have raw text
                    if current_corpus_title is None:
                         current_corpus_title = getattr(node, "properties", {}).get("document_metadata", {}).get("source", "")
                    break 
        
        if current_corpus_id and current_corpus_id not in corpus_id_set:
            corpus_items.append({
                "_id": current_corpus_id, 
                "title": current_corpus_title if current_corpus_title else "Unknown Title", 
                "text": current_corpus_text if current_corpus_text else ""
            })
            corpus_id_set.add(current_corpus_id)
        elif not current_corpus_id:
            print(f"Warning: Could not determine a corpus ID for a node pair in KG during corpus extraction.")
            
    if not corpus_items:
        print("Warning: No corpus items extracted from the knowledge graph. This might happen if the graph is empty or only contains DOCUMENT nodes without extractable CHUNK information via selected relationships.")
...
def _extract_chunk_id(chunk_str: str, kg) -> Optional[str]:
    if kg is None:
        print(f"Warning: Knowledge graph is None. Cannot extract chunk ID for: {chunk_str[:50]}...")
        return None
    ...
    print(f"Warning: Could not find chunk_id for content starting with: {search_str_key}")
    return None
...
def run_ragas_core( # Renamed from run
    ...
    if datapath_obj.is_dir():
        loader = DirectoryLoader(datapath, glob="**/[!.]*", show_progress=True, use_multithreading=False, silent_errors=True) # Process all files except hidden
    elif datapath_obj.is_file():
        loader = TextLoader(datapath)
    else:
        raise ValueError(f"Path {datapath} is not a valid directory or file.") # Should be caught by _validate
    ...
    print(f"Generating RAGAS testset with TestsetGenerator.generate_with_langchain_docs and args: {generation_args}")
    dataset = ragas_testset_generator.generate_with_langchain_docs(docs, **generation_args)
    
    kg_to_use = None
    if hasattr(ragas_testset_generator, 'knowledge_graph') and ragas_testset_generator.knowledge_graph:
    ...
    for data_item in dataset.to_list(): 
        ...
        ground_truth = data_item.get("ground_truth", "") # RAGAS 0.1.x uses "ground_truth"
            ...
            "references": [
                {
                    "corpus_id": _extract_chunk_id(
                        chunk_str=context, kg=kg_to_use 
                    ),
        ...
        query_item["references"] = [ref for ref in query_item["references"] if ref["corpus_id"] is not None]
