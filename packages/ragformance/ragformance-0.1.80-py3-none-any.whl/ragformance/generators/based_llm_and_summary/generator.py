...
from ragformance.generators.based_llm_and_summary.config import BasedLLMSummaryGeneratorConfig
...
class BasedLLMSummaryGenerator(RAGformanceGeneratorInterface):
    def run(self, config: Dict) -> Tuple[List[DocModel], List[AnnotatedQueryModel]]:
        ...
        if config_model.use_proxy:
            ...
            if config_model.proxy_url: os.environ["PROXY"] = config_model.proxy_url
            if config_model.no_proxy_list: os.environ["NO_PROXY"] = config_model.no_proxy_list
            if config_model.ca_bundle_path: os.environ["CA_BUNDLE"] = config_model.ca_bundle_path
