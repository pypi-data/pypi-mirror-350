...
from ragformance.generators.structural_generator.config import StructuralGeneratorConfig
from ragformance.models.corpus import DocModel
from ragformance.models.answer import AnnotatedQueryModel

# Assuming the core logic is in .main
from .main import run as run_structural_core_logic 
# run_structural_core_logic is expected to return Tuple[List[Dict], List[Dict]]
# and handle its own file writing.
...
class StructuralGenerator(RAGformanceGeneratorInterface):
    def run(self, config: Dict) -> Tuple[List[DocModel], List[AnnotatedQueryModel]]:
        ...
        # The original 'run' function in main.py takes 'file_name', not 'file_path'.
        ...
        for query_dict in raw_queries_list:
            ...
            annotated_queries.append(AnnotatedQueryModel(
                _id=query_dict["_id"],
                query_text=query_dict["text"],
                relevant_document_ids=query_dict.get("references", []), # format_query uses "references"
                ref_answer=query_dict.get("ref_answer", ""), 
                metadata=query_dict.get("metadata", {})
            ))
            
        print(f"[StructuralGenerator.run] Structural generation complete. Files saved in {output_path_str}")
