...
class SayHiMCPServer:
    ...
    def _register_tools(self):
        ...
        @self.mcp.tool()
        async def hi_alice(input_data: GreetingInput) -> str:
            """向Alice打招呼
            ...
            Args:
                input_data: 包含你的名字的输入参数

            Returns:
                Alice的热情回应
            """
            return f"Hi {input_data.my_name}, i'm alice! Nice to meet you! 😊"

        @self.mcp.tool()
        async def hi_bob(input_data: GreetingInput) -> str:
            """向Bob打招呼
            ...
            Returns:
                Bob的友好回应
            """
            return f"Hi {input_data.my_name}, i'm bob. Good to see you! 👋"

        @self.mcp.tool()
        async def hi_charlie(input_data: GreetingInput) -> str:
            ...
            """
            raise Exception(f"Charlie遇到了一个错误！用户名：{input_data.my_name} 😵")

        @self.mcp.tool()
        async def hi_all(input_data: GreetingInput) -> str:
            """向所有人打招呼
...
你好 {input_data.my_name}！现在我将引导你与每位同学打招呼。请按照以下步骤使用MCP工具：

1. 🌟 首先调用 hi_alice 工具
   - 参数: my_name="{input_data.my_name}"
