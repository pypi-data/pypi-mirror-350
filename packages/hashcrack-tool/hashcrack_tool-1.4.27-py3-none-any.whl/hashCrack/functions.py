...
def show_menu(default_os, hash_file=None):
    ...
    # Display loaded hash file status with improved validation
    if hash_file and os.path.isfile(hash_file) and os.path.getsize(hash_file) > 0:
        ...
        # Read first few characters to show hash preview
        try:
            ...
            print(colored(f" [✓][HASH FILE] {file_name} loaded ({file_size} bytes) - Error reading preview", 'yellow', attrs=['bold']))
    else:
        if hash_file:
            # File was specified but has issues
            ...
            print(colored(" [✕][HASH FILE] No hash file loaded", 'yellow', attrs=['bold']))
