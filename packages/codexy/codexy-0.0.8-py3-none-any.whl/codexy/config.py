...
class StoredConfig(TypedDict, total=False):
    ...
    full_auto_error_mode: Optional[str]
    memory: Optional[MemoryConfig]
    notify: Optional[bool]
    history: Optional[HistoryConfig]
...
def _discover_project_doc_path(start_dir: Path) -> Optional[Path]:
    """
...
def _load_project_doc(doc_path: Optional[Path]) -> str:
...
def load_config(
    ...
    # --- Project Doc Loading ---
    project_doc = ""
    actual_project_doc_path: Optional[Path] = None
...
def save_config(
    ...
    if (
        config.get("model")
        and config["model"] != DEFAULT_AGENTIC_MODEL
        and config["model"] != DEFAULT_FULL_CONTEXT_MODEL
    ):
