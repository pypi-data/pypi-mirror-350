...
CONFIG_DELAY: int = 2 * 60  # For WARP to apply network configurations, in seconds
...
async def is_dns_ready(
    *,
    ...
    for attempt_number in range(1, attempts + 1):
        ...
        await asyncio.sleep(seconds_per_attempt)
    LOGGER.error(
        "DNS resolution failed after %s attempts on domain %s",
        attempts,
        host_to_test_dns,
    )
    return False
...
async def warp_cli_connect() -> None:
    ...
    LOGGER.info(
        "Connected",
        extra={
            "extra": {"status": await warp_cli_status()},
        },
    )
