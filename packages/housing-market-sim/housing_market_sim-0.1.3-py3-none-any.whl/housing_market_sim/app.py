...
# ✅ 引入静态建议模块（顶部已导入）
from housing_market_sim.static_summaries import STATIC_RECOMMENDATIONS_ZH, STATIC_RECOMMENDATIONS_EN
...
# ✅ 用于从包内或本地读取图标为 base64
import os
import base64
from importlib import resources

def img_to_base64(filename: str) -> str:
    """从本地或 pip 包中读取 assets 图片，并返回 base64 编码的 data URL"""
    try:
        # 1. 本地开发时优先读取 ./assets/xxx.png
        local_path = os.path.join("assets", filename)
        if os.path.exists(local_path):
            with open(local_path, "rb") as f:
                data = f.read()
        else:
            # 2. 如果 pip 安装环境下读取包内文件
            with resources.files("housing_market_sim.assets").joinpath(filename).open("rb") as f:
                data = f.read()

        b64 = base64.b64encode(data).decode("utf-8")
        return f"data:image/png;base64,{b64}"
    except Exception as e:
        return ""  # 返回空字符串而不是直接报错，防止崩溃

# ✅ 图标变量预定义
home_b64 = key_b64 = visual_b64 = llm_b64 = ""

# ✅ 替换 translation 中的图标占位
def initialize_icons():
    global home_b64, key_b64, visual_b64, llm_b64
