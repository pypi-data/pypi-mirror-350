...
class Agent:
    ...
    def run(self, prompt: str, stream: bool = False, output_format: Optional[str] = None,
        ...
        # When using stream-json with --print mode, --verbose is required
...
class AsyncAgent:
    ...
    async def run(self, prompt: str, stream: bool = False, output_format: Optional[str] = None,
            additional_args: Optional[Dict[str, Any]] = None, auto_print: bool = True,
            continue_session: bool = False, session_id: Optional[str] = None) -> Union[str, asyncio.subprocess.Process, List[str]]:
        """
        Run Claude with the specified prompt.
        ...
        if stream:
            ...
            try:
                ...
                if return_code != 0:
                    ...
                    print(f"Error (code {return_code}): {stderr_str}", file=os.sys.stderr)
        ...
        try:
            ...
            if process.returncode != 0:
                error_msg = f"Error executing Claude (code {process.returncode}): {stderr.decode('utf-8')}"
                raise RuntimeError(error_msg)
            ...
            error_msg = f"Error executing Claude: {str(e)}"
            raise RuntimeError(error_msg)

    async def run_with_tools(self, prompt: str, tools: List[str], stream: bool = False,
                      auto_print: bool = True, continue_session: bool = False,
                      session_id: Optional[str] = None) -> Union[str, asyncio.subprocess.Process, List[str]]:
        """
        Run Claude with specific allowed tools.
