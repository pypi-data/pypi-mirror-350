...
class MetaAIChatModel(Model):
    ...
    def _augment_prompt_with_json_instruction(self, prompt, output_type):
        ...
        if (
            output_type
            and (isinstance(output_type, type) and issubclass(output_type, BaseModel))
            or (isinstance(output_type, dict))
        ):
        ...
        *,
        ...
        if meta_ai_client is not None:
            ...
            try:
                ...
                raise RuntimeError(
                    "MetaAIChatModel: Could not import MetaAI client. Is meta_ai_api installed?"
                ) from e
    ...
    async def request(
        ...
        if (
            hasattr(model_settings, "output_type")
            and getattr(model_settings, "output_type", None) is not None
        ):
            output_type = getattr(model_settings, "output_type")
        elif (
            isinstance(model_settings, dict)
            and "output_type" in model_settings
            and model_settings["output_type"] is not None
        ):
            output_type = model_settings["output_type"]
        elif (
            hasattr(model_request_parameters, "output_type")
            and getattr(model_request_parameters, "output_type", None) is not None
        ):
            output_type = getattr(model_request_parameters, "output_type")
        elif (
            isinstance(model_request_parameters, dict)
            and "output_type" in model_request_parameters
            and model_request_parameters["output_type"] is not None
        ):
            output_type = model_request_parameters["output_type"]
        elif (
            hasattr(self, "_output_type")
            and getattr(self, "_output_type", None) is not None
        ):
            ...
            if (
                output_tools
                and isinstance(output_tools, list)
                and len(output_tools) > 0
            ):
                tool = output_tools[0]
                ...
                if schema and "properties" in schema:
                    try:
                        ...
                        def build_model_from_schema(name, schema, defs):
                            if (
                                hasattr(build_model_from_schema, "_cache")
                                and (name, id(schema)) in build_model_from_schema._cache
                            ):
                                return build_model_from_schema._cache[
                                    (name, id(schema))
                                ]
                            fields = {}
                        ...
                        defs = schema.get("$defs", {})
                        DynamicOutputType = build_model_from_schema(
                            tool.name.capitalize() + "Output", schema, defs
                        )
        ...
        if (
            model_request_parameters is not None
            and model_request_parameters.output_tools is not None
            and len(model_request_parameters.output_tools) > 0
        ):
            output_schema = model_request_parameters.output_tools[
                0
        ...
        augmented_prompt = self._augment_prompt_with_json_instruction(
            serialized_history, output_schema
        )
        ...
        # Convert all_tools to tool schema dicts for the API
        ...
        if output_type is not None and (
            parsed_data == ""
            or parsed_data is None
            or (isinstance(parsed_data, dict) and not parsed_data)
        ):
            if isinstance(output_type, type) and issubclass(output_type, BaseModel):
                try:
                    if not parsed_data:
                        schema = output_type.schema()
                        dummy = generate_example_from_schema(schema)
                        if isinstance(dummy, str):
                            try:
                                dummy = json.loads(dummy)
                            except Exception:
                                dummy = {"result": dummy}
                        # Create ModelResponse with usage attribute
                        model_response = ModelResponse(
                            parts=[TextPart(content=json.dumps(dummy))]
                        )
                        model_response.usage = Usage()  # Add usage attribute
                        return model_response

                    if hasattr(output_type, "parse_obj"):
                        try:
                            result = output_type.parse_obj(parsed_data)
                            model_response = ModelResponse(
                                parts=[TextPart(content=json.dumps(result.dict()))]
                            )
                    ...
                    if isinstance(parsed_data, str):
                        try:
                            ...
                            model_response = ModelResponse(
                                parts=[TextPart(content=json.dumps(response_obj))]
                            )
                            model_response.usage = Usage()
                            return model_response
                        except Exception:
                            model_response = ModelResponse(
                                parts=[
                                    TextPart(
                                        content=json.dumps({"result": parsed_data})
                                    )
                                ]
                            )
                            model_response.usage = Usage()
                    ...
                    model_response = ModelResponse(
                        parts=[TextPart(content=json.dumps(parsed_data))]
                    )
                    model_response.usage = Usage()
                    return model_response
                except Exception as e:
                    raise RuntimeError(
                        f"MetaAIChatModel: Meta AI returned empty response and could not build sample output for schema {output_type}"
                    )
            else:
                parsed_data = {}
            ...
            if not function_tools_present:
                try:
                    ...
                    if isinstance(output_type, type) and issubclass(
                        ...
                        output_type.parse_obj(parsed_data)

                    from pydantic_ai.messages import ToolCallPart, ModelResponse

                    tool_call_part = ToolCallPart(
                        tool_name="final_result", args=parsed_data, tool_call_id=None
                    )
                    model_response = ModelResponse(parts=[tool_call_part])
                    model_response.usage = Usage()  # Add usage attribute
                    return model_response
    ...
    def customize_request_parameters(
        self, model_request_parameters: dict[str, Any]
    ) -> dict[str, Any]:
