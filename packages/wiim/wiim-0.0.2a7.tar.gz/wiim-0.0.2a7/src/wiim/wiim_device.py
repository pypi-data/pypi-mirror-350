...
class WiimDevice:
    ...
    async def async_init_services_and_subscribe(self) -> bool:
        ...
        try:
            ...
            if self.upnp_device.client: # Ensure client is available for subscription
                ...
                self.logger.error("Device %s: UPnP client not available for event subscription.", self.name)
                self._available = False
                return False
    ...
    async def _renew_subscriptions(self) -> None:
        ...
        if not self.upnp_device.client:
            self.logger.error("Device %s: UPnP client not available for renewing subscriptions.", self.name)
            self._available = False
    ...
    async def _http_request(self, command: WiimHttpCommand | str, params: str | None = None) -> dict[str, Any]:
        ...
        full_command = command.format(params) if params and isinstance(command, str) else command
        try:
            return await self._http_api.json_request(str(full_command))
        except WiimRequestException as err:
    ...
    async def _http_command_ok(self, command: WiimHttpCommand | str, params: str | None = None) -> None:
        ...
        full_command = command.format(params) if params and isinstance(command, str) else command
        try:
            await self._http_api.request(str(full_command))
        except WiimRequestException as err:
    ...
    async def async_update_http_status(self) -> None:
        ...
        try:
            ...
            self.volume = int(self._player_properties.get(PlayerAttribute.VOLUME, self.volume)) # Keep current if not in HTTP
            self.is_muted = self._player_properties.get(PlayerAttribute.MUTED, MuteMode.MUTED if self.is_muted else MuteMode.UNMUTED) == MuteMode.MUTED
            
            # Only update playing_status from HTTP if not actively playing/paused from UPnP (less disruptive)
            # Or if current status is unknown/stopped.
            # This avoids HTTP poll overwriting a fresh UPnP event.
            ...
            if http_playing_status_str:
                ...
                if self.playing_status in [PlayingStatus.STOPPED, PlayingStatus.LOADING] or \
                   self.playing_status is None or \
                   http_playing_status != self.playing_status: # Only update if different or currently stopped/loading
            ...
            if http_play_mode_str:
                self.play_mode = PlayingMode(http_play_mode_str)
            ...
            self.logger.warning("Device %s: Error parsing HTTP status data: %s. Data: %s", self.name, err, self._player_properties)
    ...
    async def _invoke_upnp_action(self, service_name: str, action_name: str, **kwargs) -> dict:
        ...
        if not service.has_action(action_name): # type: ignore
            raise WiimDeviceException(f"Device {self.name}: Service {service_name} has no action {action_name}")

        action = service.action(action_name) # type: ignore
        try:
    ...
    async def async_play(self, uri: str | None = None, metadata: str | None = None) -> None:
        ...
        if uri:
            await self._invoke_upnp_action(
                ...
                "SetAVTransportURI",
                CurrentURI=uri,
                CurrentURIMetaData=metadata or "<DIDL-Lite xmlns=\"urn:schemas-upnp-org:metadata-1-0/DIDL-Lite/\" xmlns:dc=\"http://purl.org/dc/elements/1.1/\" xmlns:upnp=\"urn:schemas-upnp-org:metadata-1-0/upnp/\"><item id=\"0\" parentID=\"-1\" restricted=\"1\"><dc:title>Unknown</dc:title><upnp:class>object.item.audioItem.musicTrack</upnp:class></item></DIDL-Lite>", # Minimal valid DIDL
            )
    ...
    async def async_set_play_mode(self, mode: PlayingMode) -> None:
        ...
        if http_mode_val and self._http_api:
            ...
            self.logger.warning("Device %s: No HTTP command mapping for play mode %s or HTTP API unavailable.", self.name, mode.value)
            raise WiimDeviceException(f"Cannot set play mode {mode.value} without HTTP mapping or API.")
    ...
    def _parse_queue_data(self, queue_data_str: str) -> list:
        ...
        self.logger.debug("Device %s: Parsing queue data: %s", self.name, queue_data_str[:100])
    ...
    def ip_address(self) -> str | None:
        ...
        if self.upnp_device and self.upnp_device.device_url: # Check if upnp_device exists
            return urlparse(self.upnp_device.device_url).hostname
        if self._http_api: # Fallback to host from HTTP API if UPnP URL not available
    ...
    def available(self) -> bool:
        ...
        # If upnp_device is a shell, its .available might be misleading.
        # Rely on our internal _available flag, which is set after successful init.
        return self._available and (self.upnp_device.available if hasattr(self.upnp_device, 'available') else True)
    ...
    async def disconnect(self) -> None:
        ...
        # Only attempt to unsubscribe if we have a real client and services
        if self.upnp_device and hasattr(self.upnp_device, 'client') and self.upnp_device.client and self.upnp_device.client.event_handler:
            try:
                if self.av_transport:
        ...
        self._available = False
        if self._event_callback: # Notify HA that device became unavailable
            await self._event_callback(self)
            
    def _format_time_for_sync(self) -> str: # Added method for button
