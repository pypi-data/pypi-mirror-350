...
class WiimDevice:
    ...
    def __init__(
        ...
        # The UpnpDevice object created by UpnpDevice.async_create_device has a .requester
        if hasattr(self.upnp_device, 'requester') and self.upnp_device.requester:
            # We pass None as callback_target because we use per-service callbacks
            # passed to self._event_handler.subscribe()
            ...
            # This is a critical issue if upnp_device is a MinimalUpnpShell without a proper requester.
            # The MinimalUpnpShell in __init__.py *does* set self.requester.
        ...
        try:
            ...
            if self._event_handler and self.av_transport and self.rendering_control:
                if not self._event_handler_started:
                    try:
                        await self._event_handler.async_start()
                        self._event_handler_started = True
                        self.logger.info("Device %s: UPnP event handler started.", self.name)
                    except Exception as e:
                        self.logger.error("Device %s: Failed to start UPnP event handler: %s", self.name, e)
                        # Eventing will not work, but HTTP might.
                        # self._available = False # Potentially mark as unavailable or degraded
                        # return False
                
                if self._event_handler_started:
                    if self.av_transport:
                        await self._event_handler.subscribe(self.av_transport, self._handle_av_transport_event)
                    if self.rendering_control:
                        await self._event_handler.subscribe(self.rendering_control, self._handle_rendering_control_event)
                    if self.play_queue_service:
                        await self._event_handler.subscribe(self.play_queue_service, self._handle_play_queue_event)
                    self.logger.info("Device %s: Subscribed to UPnP service events.", self.name)
                else:
                    self.logger.warning("Device %s: Event handler not started, cannot subscribe to service events.", self.name)
            elif not self._event_handler:
                 self.logger.warning("Device %s: No event handler, cannot subscribe to service events.", self.name)
            else: # Services missing
                 self.logger.warning("Device %s: Core services missing, cannot subscribe to events.", self.name)
    ...
    async def _renew_subscriptions(self) -> None:
        ...
        if not self._event_handler or not self._event_handler_started:
            ...
            # Potentially mark as degraded or try to restart event handler
            self._available = False # If subscriptions are critical and fail to renew
        ...
        try:
            ...
            if self.play_queue_service: # Ensure play_queue_service exists before trying to resubscribe
            ...
            self._available = False # Mark as unavailable if renewals fail
            if self._event_callback:
                await self._event_callback(self) # Notify HA
        except Exception as err: # Catch any other error during resubscribe
    ...
    async def disconnect(self) -> None:
        ...
        if self._event_handler and self._event_handler_started:
            try:
                ...
                if self.play_queue_service and self.play_queue_service.event_subscription_sid:
                    await self._event_handler.async_unsubscribe(self.play_queue_service)
                
                # Stop the event handler server
                await self._event_handler.async_stop()
                self._event_handler_started = False
                self.logger.info("Device %s: UPnP event handler stopped.", self.name)
            except UpnpError as err:
                self.logger.warning("Device %s: Error during UPnP unsubscribe/stop: %s", self.name, err)
            except Exception as err: 
                self.logger.error("Device %s: Unexpected error during UPnP event handler stop/unsubscribe: %s", self.name, err, exc_info=True)
        
        self._available = False
        # Notify HA that device became unavailable, if callback is set
        if self._event_callback: 
            try:
                await self._event_callback(self)
            except Exception as e: # Catch errors from callback itself
