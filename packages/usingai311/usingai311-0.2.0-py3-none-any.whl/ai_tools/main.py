...
def main():
    ...
    # ✅ 출력 경로 초기 설정
    output_dir = input("💾 출력 이미지를 저장할 폴더를 입력하세요 (Enter 시 입력 이미지와 같은 위치): ").strip()
    if output_dir:
    ...
    while True:
        ...
        if choice == '1':
            ...
            file_name = f"output_{base_name}_{style_prompt.replace(' ', '_')}.jpg"

            if output_dir:
                output_path = os.path.join(output_dir, file_name)
            else:
                output_path = os.path.join(os.path.dirname(input_image_path), file_name)
            ...
            result_path = drawing_ai.draw(input_image_path, user_description, style_prompt, output_path)
            if result_path:
