...
class UIExplorer:
    ...
    def _track_tool_usage(self, tool_name: str) -> Dict[str, Any]:
        ...
        # Detect potential repetitive usage patterns
        recent_calls = [entry for entry in self.tool_usage_history[-10:] if entry["tool_name"] == tool_name]
        repetitive_warning = None
        
        if len(recent_calls) >= 3:
            # Check if the same tool was called 3+ times in the last 10 calls
            repetitive_warning = f"WARNING: '{tool_name}' has been called {len(recent_calls)} times recently. Consider changing strategy if not making progress."
        
        if self.tool_usage_stats[tool_name] >= 5:
            # Check for excessive usage of a single tool
            if not repetitive_warning:
                repetitive_warning = f"NOTICE: '{tool_name}' has been called {self.tool_usage_stats[tool_name]} times this session. Ensure this is intentional."
...
async def main():
    ...
    async def call_tool(name: str, arguments: Dict[str, Any]) -> List[types.TextContent]:
        ...
        if name == "explore_ui":
            ...
            # Add usage stats and auto summary info to cursor position result
            ...
            # Don't log this action to avoid recursion
            result["tool_usage_stats"] = usage_stats
            return [types.TextContent(type="text", text=json.dumps(result, indent=2))]
        
        # For unknown tools, still track the attempt and return usage stats
        error_result = {
            "error": f"Unknown tool: {name}",
            "tool_usage_stats": usage_stats
        }
        ui_explorer._log_action(name, arguments, error_result)
        if auto_summary:
            error_result["auto_summary_created"] = auto_summary
