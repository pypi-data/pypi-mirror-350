...
def Typed(expected_type, cls=None):
    ...
    def __set__(self, instance, value):
        ...
        if not isinstance(value, expected_type):
            raise exceptions.TypeError(f"Field {cls.name}, expect type {expected_type.__name__}")
        super_set(self, instance, value)
...
class FieldRuler:
    ...
    def express_eval(self, field_info_list: List):
        ...
        for i in range(0, len(field_info_list), 2):
            ...
            second_value = field_info_list[i + 1] if i + 1 < len(field_info_list) else None
            if not second_value:
                continue
...
class SimpleRuleController(RuleControllerBase):
    ...
    def build(self):
        ...
        for name, field in self._fields:
            ...
            rule_operator = self.RULE_OP.get(validator_prefix.replace("-", "_"), DefaultRuler)
