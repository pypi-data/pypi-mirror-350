...
class from_meta(type):
    ...
    def __call__(cls, *args: Any, **kwds: Any) -> Any:
        ...
        if hasattr(obj, "rule_adapter"):
            """ """
            rule_cls = obj.rule_adapter if obj.rule_adapter is not None else SimpleRuleController
...
class Form(metaclass=from_meta):
    ...
    def __init__(self, *args, **kw) -> None:
        ...
        self._valid_attr = {}
        try:
            bound_values = self.__signature__.bind(*args, **ChainMap(order_dict(kwargs), self.__kw_default__))
    ...
    def __get_bases_sig_args(self) -> Type[set]:
        ...
        for __cls in __bases:
            ...
            __sig_args = __sig_args.union(set(__bind_args.args.__add__(__bind_args.kwonlyargs)))
    ...
    def __repr__(self) -> str:
        """ """
        f_str = "".join(f"{__name}={self.__dict__.get(__name)}," for __name in self.__signature__.parameters.keys())
    ...
    def raise_valid(self) -> NoReturn:
        ...
        for e in self.errors:
            if self.code is not None:
                raise exceptions.ValidationError(str(e), code=self.code)
