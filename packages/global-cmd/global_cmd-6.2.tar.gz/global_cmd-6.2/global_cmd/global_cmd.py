...
def call_bedrock_api(image_path, invoice_number, party_name):
    ...
    prompt = (
        ...
        "If the '" + party_name + "' is 'Platinum Industries Pvt Ltd' then the invoice number in the PDF will begin with 'Invoice No. PIPL.then the PDF will contain a line starting with Invoice No. . The invoice number always follows the exact pattern: PIPLdddd/YY-YY where: - “PIPL” is literal - “dddd” is exactly four digits (0–9) - “YY” are exactly two digits for each year segment For example: Invoice No. PIPL0317/23-24 Scan the document text, find that invoice number, and return only the substring matching PIPL followed by four digits, a slash, two digits, a dash, and two digits.'"
      ...
      """2. 'Invoice Dt' - Extract the invoice date and STRICTLY output it in DD.MM.YYYY format.
           Find dates near 'Invoice Dt:', 'BILL.Date:', etc.
           CRITICAL CONVERSIONS (MUST DO):
           - Month Names/Abbr. to Two-Digit Number:(e.g., 'Jan'/'January' -> "01", 'Jun'/'June' -> "06").
           - Day/Month to Two Digits: (e.g., '8' -> "08", '6' -> "06").
           - Year to Four Digits: (e.g., '23' -> "2023").
           EXAMPLES: '8-Jun-23' -> "08.06.2023"; '05/04/24' -> "05.04.2024"; '7.1.2023' -> "07.01.2023".
           If conversion to exact DD.MM.YYYY is impossible, the field is "missing"; follow rule for missing fields (empty JSON {}).
           """
...
def process_subfolder(subfolder_path, invoice_number, party_name):
    ...
    for pdf_file in pdf_files:
        ...
        try:
            ...
            #if the formatted_supplier contain PPL or PIP in thefirst 4 characters then make sure it is converted to PIPL
            if formatted_supplier[:4].upper() == 'PPL' or formatted_supplier[:4].upper() == 'PIP':
                formatted_supplier = 'PIPL' + formatted_supplier[4:]
