...
def get_net(vsl: bool = False) -> tuple[Network, dict[str, Any]]:
    ...
    if vsl:
        L1 = LinkWithVsl[cs.SX](
            4, *args, segments_with_vsl={2, 3}, alpha=0.1, name="L1"
        )
    else:
        L1 = Link(4, *args, a, name="L1")
    L2 = Link(2, *args, name="L2")
    net = (
        Network(name="A1")
...
class TestExamples(unittest.TestCase):
    ...
    def test_coordinated_control_example(self):
        engine = engines.use("casadi", sym_type=self.sym_type)
        net, pars = get_net(vsl=True)
        net.step(
            T=pars["T"],
            tau=pars["tau"],
            eta=pars["eta"],
        ...
        mpc.minimize(
            pars["T"] * cs.sum2(cs.sum1(rho * pars["L"] * pars["lanes"]) + cs.sum1(w))
            + 0.4
            * cs.sumsqr(cs.diff(cs.horzcat(v_ctrl_last, v_ctrl), 1, 1) / pars["v_free"])
            + 0.4 * cs.sumsqr(cs.diff(cs.horzcat(r_last, r)))
        )
        opts = {
            "expand": True,
        ...
        v = cs.DM([80, 80, 78, 72.5, 66, 62])
        w = cs.DM([0, 0])
        v_ctrl_last = v[: net.links_by_name["L1"].N][net.links_by_name["L1"].vsl]
        r_last = cs.DM.ones(r.size1(), 1)
        sol_prev = None
        ...
        for k in range(100):
            ...
            R.append(r_last)
        RHO, V, W, Q, Q_o, V_CTRL, R = (np.squeeze(o) for o in (RHO, V, W, Q, Q_o, V_CTRL, R))  # type: ignore
        tts = pars["T"] * sum(
            (rho * pars["L"] * pars["lanes"]).sum() + w.sum() for rho, w in zip(RHO, W)
        )

        RHO_, V_, W_, Q_, Q_o_, V_CTRL_, R_, tts_ = RESULTS["test_coord"]
        np.testing.assert_allclose(RHO, RHO_, rtol=1e-3, atol=1e-3)
        np.testing.assert_allclose(V, V_, rtol=1e-3, atol=1e-3)
        np.testing.assert_allclose(W, W_, rtol=1e-3, atol=1e-3)
        np.testing.assert_allclose(Q, Q_, rtol=1e-3, atol=1e-3)
        np.testing.assert_allclose(Q_o, Q_o_, rtol=1e-3, atol=1e-3)
        np.testing.assert_allclose(R, R_, rtol=1e-3, atol=1e-3)
        np.testing.assert_allclose(V_CTRL, V_CTRL_, rtol=1e-3, atol=1e-3)
        np.testing.assert_allclose(tts, tts_, rtol=1e-3, atol=1e-3)
