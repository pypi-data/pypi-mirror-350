...
import numpy as np
import pandas as pd
from category_encoders import CatBoostEncoder as _CatBoostEncoder
...
class CatBoostEncoder(Encoder[dtype.Categorical, dtype.Float64]):
    """
    Encoder for performing CatBoost encoding.
    Inherits the Encoder base class.
    """

    slug = CategoricalEncoder.CAT_BOOST_ENCODER
    columns: List[str]
    ...
    @property
    def encoder(self) -> _CatBoostEncoder:
    ...
    def fit(self, X: Series[dtype.Categorical], y: Optional[SeriesTarget] = None) -> None:
        self._encoder = _CatBoostEncoder(return_df=False)  # Instantiate the encoder
        self.encoder.fit(X, y)  # Fit the encoder on the data

        self.columns = [f"{self.name}_{col}_catboost" for col in range(1, len(self.encoder.get_feature_names()) + 1)]

    def transform(self, X: Series[dtype.Categorical]) -> DataFrame[dtype.Float64]:
        # @data: new object
        transformed = self.encoder.transform(X).astype(np.float64, copy=False)
