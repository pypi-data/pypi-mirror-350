...
from evoml_utils.convertors.detected_types import convert_unit_number_column
...
class UnitTransformer(Transformer):
    """
    The unit type data could be a number and unit  e.g. 1m, 2kg, 3cm, 4mm, 5km, 6ft, 7in, 8yd, 9mi.
    ...
    def __init__(
        ...
        if "unit" not in self.metadata:
            raise ValueError("The unit field must be supplied in the metadata when converting the unit type.")
        self.is_categorical = False
        self.scaler = None
        self.encoder = None
        self.impute_setting = DetectedType.unit_number
        self.block = Block.UNIT_NUMBER
    ...
    def _init_operations(self, data: pd.Series[Any]) -> None:
        ...
        self.encoder_slug = get_default_numeric_encoding(data.to_numpy(), self.name)
    ...
    def _convert(self, data: pd.Series[Any]) -> pd.Series[Any]:
        ...
        if self.encoder_slug == GenericOption.NONE:
            return data
        return convert_unit_number_column(data, self.metadata["unit"])
