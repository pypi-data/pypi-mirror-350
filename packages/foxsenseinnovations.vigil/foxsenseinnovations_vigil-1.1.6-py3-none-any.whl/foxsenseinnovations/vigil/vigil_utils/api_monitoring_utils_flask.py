...
from foxsenseinnovations.vigil.vigil_types.api_monitoring_types import ApiRequest, ApiResponse, ExcludeOptions, IncludeOptions,RequestDetails
...
def is_monitor_api(method: HttpMethod, path: str, exclude_options: Optional[ExcludeOptions] = None, include_options: Optional[IncludeOptions] = None) -> bool:
    try:
        """
...
def check_path_excluded(method: HttpMethod, path: str, exclude_options: Optional[ExcludeOptions] = None) -> bool:
...
def check_path_included(method: HttpMethod, path: str,include_options: Optional[IncludeOptions] = None) -> bool:
    ...
    if len(include_paths) > 0:
        ...
        for include_path in include_paths:
            params = extract_path_params(include_path, path)
