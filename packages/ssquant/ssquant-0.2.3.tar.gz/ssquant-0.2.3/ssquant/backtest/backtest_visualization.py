...
class BacktestVisualizer:
    ...
    def plot_results(self, multi_data_source, results):
        ...
        for i, ds in enumerate(multi_data_source.data_sources):
            ...
            elif 'bid1' in df.columns and 'ask1' in df.columns:
                prices = ((df['bid1'] + df['ask1']) / 2).values
            else:
    ...
    def _generate_price_chart(self, result, chart_path):
        ...
        try:
            ...
            if 'datetime' in klines.columns and 'open' in klines.columns and 'high' in klines.columns and 'low' in klines.columns and 'close' in klines.columns:
                ...
                ax.plot(dates[highest_idx], highs[highest_idx], 'r^', markersize=10)
                ax.text(dates[highest_idx], highs[highest_idx], f" 最高: {highs[highest_idx]:.2f}", verticalalignment='bottom')
                
                ax.plot(dates[lowest_idx], lows[lowest_idx], 'gv', markersize=10)
                ax.text(dates[lowest_idx], lows[lowest_idx], f" 最低: {lows[lowest_idx]:.2f}", verticalalignment='top')
