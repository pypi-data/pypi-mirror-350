...
class DentrixServiceRequests(DentrixServiceLogin):
    ...
    @dentrix_request_handling
    @retry(tries=3, delay=2, backoff=2)
    def _get_schedules(self, timestamp: int, location_id: int, is_week_view: bool = True) -> dict:
        """Gather schedules based on a timestamp and location ID, with optional week view."""
        url = DentrixUrls.GET_SCHEDULES(timestamp, location_id, str(is_week_view).lower())
