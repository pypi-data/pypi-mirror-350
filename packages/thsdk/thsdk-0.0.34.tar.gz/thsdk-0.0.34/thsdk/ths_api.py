...
class HQ:
    ...
    def _get_lib_path(self) -> str:
        ...
        if system == 'Linux':
            return os.path.join(base_dir, f"hq{self.ops.get('plan', '')}.so")
        elif system == 'Darwin':
            if arch == 'arm64':
    ...
    def _initialize_library(self) -> None:
        ...
        try:
            ...
            self._lib.HQ.argtypes = [c.c_char_p]
            self._lib.HQ.restype = c.c_char_p
            self._lib.Connect.restype = c.c_char_p
            self._lib.Disconnect.restype = c.c_char_p
            self._lib.QueryData.argtypes = [c.c_char_p, c.c_char_p]
            self._lib.QueryData.restype = c.c_char_p
            self._lib.About.restype = c.c_char_p
...
class Response:
    ...
    def __init__(self, json_str: str):
        ...
        self.code: int = data_dict.get("code", 0)
        self.message: str = data_dict.get("message", "")
        self.payload: Payload = Payload(data_dict.get("payload", {}))

    def __repr__(self) -> str:
        return f"Response(code={self.code}, message={self.message}, payload={self.payload})"
...
class THS:
    ...
    def connect(self, max_retries: int = 3) -> Response:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if not response:
                    raise ConnectionError("No data returned from connect.")
                rs = Response(response)
                if rs.is_success():
                    self._login = True
                    logger.info("Connected to THS server successfully")
                    return rs
                logger.warning(f"Connection attempt {attempt + 1} failed: {rs.message}")
    ...
    def disconnect(self) -> Response:
        ...
        return Response('{"code": 0, "message": "Already disconnected", "payload": {}}')
    ...
    def download(self, code: str, start: Optional[Any] = None, end: Optional[Any] = None,
        ...
        Args:
            code: 证券代码，格式为10位，以'USHA'或'USZA'开头等开头
            start: 开始时间，格式取决于周期
            end: 结束时间，格式取决于周期
            adjust: 复权类型
            period: 周期范围，默认为"max"
        ...
        """
        code = code.upper()
        if len(code) != 10 or not any(code.startswith(market) for market in MARKETS):
    ...
    def wencai_nlp(self, condition: str, domain: Optional[str] = "") -> Response:
        ...
        return self.query_data(condition, query_type)
