...
try:
    ...
    logging.info(f"Failed to import ezQgd module: {e}")
...
class QcisBackend(HardwareBackend):
    ...
    def __init__(
        ...
        """
        Args:
            is_prob (bool|str, optional):'0' return probability, '1' return sample,
            '2' return raw binary string. Defaults to False sample.
        """
        if logger is None:
        ...
        if lab_id:
            ...
            create_res = self._account.create_experiment(exp_name, remark=remark)
            if create_res == 0:
                print("新建实验合集失败")
            else:
    ...
    def sample(
        ...
        **kwargs,
        ...
        query_id = self._account.submit_job(  # 1
