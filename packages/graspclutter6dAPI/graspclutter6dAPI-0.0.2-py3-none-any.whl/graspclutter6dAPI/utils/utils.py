...
def generate_scene_model(dataset_root, scene_name, anno_idx, return_poses=False, align=False, camera='realsense'):
    ...
    **Output:**
    ...
    img_id = annId2ImgId(anno_idx, camera)

    if align:
        scene_camera_path = os.path.join(dataset_root, 'scenes', scene_name, 'scene_camera.json')
        with open(scene_camera_path) as f:
            scene_camera = json.load(f)
        cam_R_w2c = np.array(scene_camera[str(img_id)]['cam_R_w2c']).reshape((3,3))
        cam_t_w2c = np.array(scene_camera[str(img_id)]['cam_t_w2c']).reshape((3,1))
    ...
    scene_gt_path = os.path.join(dataset_root, 'scenes', scene_name, 'scene_gt.json')
    ...
    model_list = []
    for obj_idx, pose in zip(obj_list, pose_list):
...
def generate_scene_pointcloud(dataset_root, scene_name, anno_idx, align=False, camera='zivid'):
    '''
    Author: chenxi-wang
    ...
    **Input:**
    ...
    - scene_name: str, name of scene folder, e.g. 000001

    - anno_idx: int, frame index from 0-12
    ...
    **Output:**
    ...
    '''
    img_id = annId2ImgId(anno_idx, camera)
    colors = np.array(Image.open(os.path.join(dataset_root, 'scenes', scene_name, 'rgb', '%06d.png'%img_id)), dtype=np.float32) / 255.0
    depths = np.array(Image.open(os.path.join(dataset_root, 'scenes', scene_name, 'depth', '%06d.png'%img_id)))
    scene_camera_path = os.path.join(dataset_root, 'scenes', scene_name, 'scene_camera.json')
    with open(scene_camera_path) as f:
        scene_camera = json.load(f)
    intrinsics = np.array(scene_camera[str(img_id)]['cam_K']).reshape((3,3))
