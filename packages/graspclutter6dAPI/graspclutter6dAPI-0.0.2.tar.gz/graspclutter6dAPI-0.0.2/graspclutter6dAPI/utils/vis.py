...
def get_camera_parameters(camera='azure_kinect'):
    '''
    author: Minghao Gou
    ...
    **Output:**
    ...
    if camera == 'realsense-d415':
        param.intrinsic.set_intrinsics(1920, 1280, 1429.1852897390465, 1435.274462851161, 977.0425291412042, 543.5870281084453)
    elif camera == 'realsense-d435':
        param.intrinsic.set_intrinsics(1920, 1080, 1415.1207600904063, 1424.6133036516164, 969.0317075291746, 548.5843257919785)
    elif camera == 'azure_kinect':
        param.intrinsic.set_intrinsics(3840, 2160, 2111.7545934678865, 2110.121816063188, 1912.6446092068736, 1108.7622578230528)
    elif camera == 'zivid':
        param.intrinsic.set_intrinsics(1920, 1200, 2789.760446808816, 2792.540885096279, 951.9569376586062, 595.5559842401404)
    return param
...
def vis6D(dataset_root, scene_name, anno_idx, camera, save_folder='save_fig', show=False, per_obj=False):
    '''
    **Input:**
    ...
    '''
    model_list, obj_list, pose_list = generate_scene_model(dataset_root, scene_name, anno_idx, return_poses=True, align=False, camera=camera)
    point_cloud = generate_scene_pointcloud(dataset_root, scene_name, anno_idx, align=False, camera=camera)
