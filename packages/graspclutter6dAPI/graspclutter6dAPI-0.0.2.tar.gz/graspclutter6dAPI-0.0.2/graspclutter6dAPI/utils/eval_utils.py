...
def collision_detection(grasp_list, model_list, dexnet_models, poses, scene_points, outlier=0.05, empty_thresh=10, return_dexgrasps=False):
    ...
    **Output:**
    ...
    for i in range(num_models):
        # print('collision detection: {}/{}'.format(i+1, num_models))
...
def collision_detection_per_object(grasps, workspace, empty_thresh=10):
...
def collision_detection_per_object_optimized(grasps, workspace, empty_thresh=10):
    # optimized version using PyTorch
    import torch
    height = 0.02
    depth_base = 0.02
    finger_width = 0.01
    ...
    # transform scene to gripper frame
    target = (workspace.unsqueeze(0) - grasp_points.unsqueeze(1))
    target = torch.matmul(target, grasp_poses)
    
    # compute collision mask
    mask1 = ((target[:, :, 2] > -height / 2) & (target[:, :, 2] < height / 2))
    mask2 = ((target[:, :, 0] > -depth_base) & (target[:, :, 0] < grasp_depths.unsqueeze(1)))
    mask3 = (target[:, :, 1] > -(grasp_widths.unsqueeze(1) / 2 + finger_width))
    mask4 = (target[:, :, 1] < -grasp_widths.unsqueeze(1) / 2)
    mask5 = (target[:, :, 1] < (grasp_widths.unsqueeze(1) / 2 + finger_width))
    mask6 = (target[:, :, 1] > grasp_widths.unsqueeze(1) / 2)
    mask7 = ((target[:, :, 0] > -(depth_base + finger_width)) & (target[:, :, 0] < -depth_base))

    # use only grasps that has angle difference between 0 and 90 degrees (grasp's x-axis and camera's z-axis)
    # grasp_x_axis: (N, 3)
    grasp_x_axis = grasp_poses[:, :, 0]
    camera_z_axis = torch.tensor([0, 0, 1], dtype=torch.float32).to(grasp_x_axis.device)
    # Reshape camera_z_axis to (1, 3) to match the batch dimension of grasp_x_axis
    camera_z_axis = camera_z_axis.view(1, 3).expand_as(grasp_x_axis)

    # Calculate dot product for each pair using torch.einsum
    dot_prod = torch.einsum('ij,ij->i', [grasp_x_axis, camera_z_axis])

    # Calculate magnitudes and angle difference
    magnitute_prod = torch.norm(grasp_x_axis, dim=1) * torch.norm(camera_z_axis, dim=1)
    angle_diff = torch.acos(dot_prod / magnitute_prod)

    # Create mask for angle difference between 0 and 75 degrees
    mask8 = (angle_diff > 0) & (angle_diff < np.pi / 180 * 75)
    ...
    collision_mask = collision_mask | (~mask8)
...
def eval_grasp(grasp_group, models, dexnet_models, poses, config, table=None, voxel_size=0.008, TOP_K = 50):
    ...
    **Input:**
    ...
    if len(all_grasp_list) > 0:
        ...
        # Handle the case when no grasps are available
        min_score = 0.0  #
    grasp_list = []
    for i in range(num_models):
