...
import json # For potential manifest file
import re # Added for robust paragraph splitting
from typing import List, Optional, Dict, Any

# --- Configuration ---
# Each "long document" will be a subdirectory within LONG_DOCS_ROOT_DIR.
# Chapters will be .md files within their respective document subdirectory.
LONG_DOCS_ROOT_DIR_NAME = os.environ.get("LONG_DOCUMENT_ROOT_DIR", "long_documents_storage")
LONG_DOCS_ROOT_PATH = Path(LONG_DOCS_ROOT_DIR_NAME)
LONG_DOCS_ROOT_PATH.mkdir(parents=True, exist_ok=True) # Ensure the root directory exists
...
mcp_server = FastMCP(
    name="LongDocumentManagementTools",
    description="A server providing tools to interact with structured long Markdown documents (composed of chapters) via stdio."
)
...
class ChapterMetadata(BaseModel):
    """Metadata for a chapter within a long document."""
...
class LongDocumentInfo(BaseModel):
    """Represents metadata for a long document."""
...
class FullLongDocumentContent(BaseModel):
    """Content of an entire long document, comprising all its chapters in order."""
    document_name: str
...
def _get_document_path(document_name: str) -> Path:
    ...
    return LONG_DOCS_ROOT_PATH / document_name
...
# --- Tool Definitions (To be refactored and added) ---

# Placeholder for the old list_documents to show where new tools will go
# @mcp_server.tool()
# def list_documents() -> List[DocumentFile]:
#     """Lists all available Markdown files in the configured document directory. 
#     Returns a list of objects, each detailing a file's name, size, and last modified time.
#     Example usage: `list_documents()`"""
#     markdown_files = []
#     try:
#         for item in DOCS_DIR.iterdir(): # OLD DOCS_DIR
...
# The rest of the original tools will be refactored or replaced below.
# For example, read_document will become read_chapter_content, etc.

# --- Start of refactored/new tool definitions ---

# TODO: Implement the following tools:
# Read Tools:
# - list_long_documents() -> List[LongDocumentInfo]
# - list_chapters(document_name: str) -> List[ChapterMetadata] (ordered)
# - read_chapter_content(document_name: str, chapter_name: str) -> Optional[ChapterContent]
# - read_paragraph_content(document_name: str, chapter_name: str, paragraph_index: int) -> Optional[ParagraphDetail]
# - read_full_long_document(document_name: str) -> Optional[FullLongDocumentContent]

# Write Tools:
# - create_long_document(document_name: str) -> OperationStatus
# - delete_long_document(document_name: str) -> OperationStatus
# - create_chapter(document_name: str, chapter_name: str, chapter_index_or_after_chapter_name: Optional[Union[int, str]] = None, initial_content: str = "") -> OperationStatus
# - delete_chapter(document_name: str, chapter_name: str) -> OperationStatus
# - write_chapter_content(document_name: str, chapter_name: str, new_content: str) -> OperationStatus (replaces edit_document)
# - modify_paragraph_content(document_name: str, chapter_name: str, paragraph_index: int, new_paragraph_content: str, mode: str) -> OperationStatus
#   (mode: "replace", "insert_before", "insert_after", "delete") (replaces edit_paragraph)
# - append_paragraph_to_chapter(document_name: str, chapter_name: str, paragraph_content: str) -> OperationStatus
# - replace_text_in_chapter(document_name: str, chapter_name: str, text_to_find: str, replacement_text: str) -> OperationStatus
# - replace_text_in_document(document_name: str, text_to_find: str, replacement_text: str) -> OperationStatus (iterates chapters)

# Analyze Tools:
# - get_document_statistics(document_name: str) -> Optional[StatisticsReport]
# - get_chapter_statistics(document_name: str, chapter_name: str) -> Optional[StatisticsReport]

# Retrieval Tools (Semantic search might be out of scope for FastMCP direct implementation, but exact text find is possible)
# - find_text_in_chapter(document_name: str, chapter_name: str, query: str, case_sensitive: bool = False) -> List[ParagraphDetail]
# - find_text_in_document(document_name: str, query: str, case_sensitive: bool = False) -> List[ParagraphDetail] (iterates chapters)


# --- Example of a refactored tool (list_long_documents) ---
@mcp_server.tool()
def list_long_documents() -> List[LongDocumentInfo]:
    """
    Lists all available long documents.
    A long document is a directory containing chapter files (.md).
    Returns a list of objects, each detailing a document's name, chapter count, word counts, and last modified time.
    """
    long_docs_info = []
    if not LONG_DOCS_ROOT_PATH.exists() or not LONG_DOCS_ROOT_PATH.is_dir():
        return []

    for doc_dir in LONG_DOCS_ROOT_PATH.iterdir():
        if doc_dir.is_dir(): # Each subdirectory is a potential long document
...
def list_chapters(document_name: str) -> Optional[List[ChapterMetadata]]:
    """
    Lists all chapters for a given long document, ordered by filename.
...
def read_chapter_content(document_name: str, chapter_name: str) -> Optional[ChapterContent]:
    """
    Reads the content of a specific chapter within a long document.
...
def read_paragraph_content(document_name: str, chapter_name: str, paragraph_index_in_chapter: int) -> Optional[ParagraphDetail]:
    """
    Reads a specific paragraph from a chapter in a long document.
...
@mcp_server.tool()
def read_full_long_document(document_name: str) -> Optional[FullLongDocumentContent]:
    """
    Reads the entire content of a long document, including all its chapters in order.
    Requires `document_name`.
    ...
    if not ordered_chapter_files:
        ...
        return FullLongDocumentContent(
            document_name=document_name,
            chapters=[],
            total_word_count=0,
...
@mcp_server.tool()
def create_long_document(document_name: str) -> OperationStatus:
    """
    Creates a new long document (as a directory).
    Requires `document_name`.
...
@mcp_server.tool()
def delete_long_document(document_name: str) -> OperationStatus:
    """
    Deletes a long document and all its chapters.
    Requires `document_name`.
...
def create_chapter(document_name: str, chapter_name: str, initial_content: str = "") -> OperationStatus:
    """
    Creates a new chapter file within a long document.
...
def delete_chapter(document_name: str, chapter_name: str) -> OperationStatus:
    """
    Deletes a chapter file from a long document.
...
def replace_text_in_document(document_name: str, text_to_find: str, replacement_text: str) -> OperationStatus:
    """
    Replaces all occurrences of a string with another string throughout all chapters of a long document.
...
def get_document_statistics(document_name: str) -> Optional[StatisticsReport]:
    """
    Retrieves aggregated statistics for an entire long document (total word/paragraph/chapter count).
    Requires `document_name`.
    """
    # Option 1: Re-use list_long_documents and find the specific document.
    # This is good for consistency if list_long_documents is already computed/cached by agent.
    # However, it might be slightly less direct if we only need one document.
    all_docs_info = list_long_documents() # This computes stats for all docs
    target_doc_info = next((doc for doc in all_docs_info if doc.document_name == document_name), None)

    if not target_doc_info:
        # Check if the document directory exists even if it has no chapters or failed to be processed by list_long_documents
        doc_path = _get_document_path(document_name)
        if not doc_path.is_dir():
            print(f"Document '{document_name}' not found for statistics.")
            return None
        # If dir exists but not in target_doc_info (e.g. no chapters or processing error in list_long_documents)
        ...
        # If it has chapters but wasn't in list_long_documents output, it implies an issue. Recalculate:
        # This part is a bit redundant with read_full_long_document logic but more direct for stats
        total_word_count = 0
        total_paragraph_count = 0
        chapter_count = 0
        for chapter_file_path in ordered_chapter_files:
...
def find_text_in_document(
    ...
    """
    Finds paragraphs containing the exact query string across all chapters of a long document.
...
# The old tool definitions (list_documents, read_document, read_paragraph, edit_document, edit_paragraph, replace_text_globally)
# were placeholders or have been fully replaced by the new set of tools with document/chapter structure.
# They can now be safely removed.

# --- Main Server Execution ---
if __name__ == "__main__":
    # This print will show the path used by the subprocess
    print(f"doc_tool_server.py: Initializing with LONG_DOCS_ROOT_PATH = {LONG_DOCS_ROOT_PATH.resolve()}") 
    print(f"doc_tool_server.py: Environment LONG_DOCUMENT_ROOT_DIR = {os.environ.get('LONG_DOCUMENT_ROOT_DIR')}")
    print(f"Long Document tool server starting. Tools exposed by '{mcp_server.name}':")
    print(f"Serving tools for root directory: {LONG_DOCS_ROOT_PATH.resolve()}")
