...
## 📖 What is Long Document MCP?

Long Document MCP provides a structured way to manage large documents composed of multiple chapters. Think of it as a file system specifically designed for books, research papers, documentation, or any content that benefits from being split into manageable sections.
...
```
long_documents_storage/
├── my_novel/                    # A long document
...
```
.
├── long_document_mcp/           # 📦 Main MCP server package
...
2. **Set up environment variables:**
   ...
   LONG_DOCUMENT_ROOT_DIR=sample_doc/
...
- Connect to the Long Document MCP Server
...
```
User Query → Pydantic AI Agent → MCP Client → Long Document MCP Server → File System
                ↓
...
🤖 Agent: Successfully created the new long document named 'My Novel'.
...
git clone https://github.com/long-document-mcp/long-document-mcp.git
...
- **[Package Documentation](long_document_mcp/README.md)**: MCP server API reference and installation
- **[API Reference](long_document_mcp/doc_tool_server.py)**: Complete MCP tools documentation
