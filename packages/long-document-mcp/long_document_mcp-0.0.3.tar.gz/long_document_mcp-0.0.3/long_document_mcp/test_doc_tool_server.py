...
from .doc_tool_server import (
    ...
    OperationStatus, ChapterMetadata, LongDocumentInfo, ParagraphDetail,
    ChapterContent, FullLongDocumentContent, StatisticsReport,
    # Import Tools
    list_long_documents, create_long_document, delete_long_document,
    ...
    read_full_long_document, # create_long_document is already imported
    get_chapter_statistics, get_document_statistics,
    find_text_in_chapter, find_text_in_document,
    # Import the global mcp_server and LONG_DOCS_ROOT_PATH for manipulation if needed,
    # but it's better to override the path for tests.
    LONG_DOCS_ROOT_PATH as SERVER_DEFAULT_LONG_DOCS_ROOT_PATH 
)
...
def temp_docs_root(tmp_path: Path) -> Path:
    ...
    test_root = tmp_path / "test_long_documents_storage"
    test_root.mkdir()
    
    original_path = doc_tool_server.LONG_DOCS_ROOT_PATH
    doc_tool_server.LONG_DOCS_ROOT_PATH = test_root
    ...
    doc_tool_server.LONG_DOCS_ROOT_PATH = original_path
...
def test_create_long_document_success(temp_docs_root: Path):
    doc_name = "my_test_document"
    status = create_long_document(document_name=doc_name)
...
def test_create_long_document_duplicate(temp_docs_root: Path):
    doc_name = "my_duplicate_doc"
    create_long_document(document_name=doc_name) # Create first time
    status = create_long_document(document_name=doc_name) # Attempt duplicate
    _assert_operation_failure(status, "already exists")

def test_list_long_documents_empty(temp_docs_root: Path):
    docs_list = list_long_documents()
    assert isinstance(docs_list, list)
    assert len(docs_list) == 0

def test_list_long_documents_with_one_doc(temp_docs_root: Path):
    doc_name = "listed_document"
    create_long_document(document_name=doc_name)
...
def test_read_full_long_document_success(temp_docs_root: Path):
    ...
    full_doc_obj = read_full_long_document(document_name=doc_name)
    assert full_doc_obj is not None
    assert isinstance(full_doc_obj, FullLongDocumentContent)
...
def test_read_full_long_document_empty_doc(temp_docs_root: Path):
...
def test_read_full_long_document_non_existent(temp_docs_root: Path):
    full_doc_obj = read_full_long_document(document_name="no_doc_here_for_full_read")
...
def pytest_sessionfinish(session, exitstatus):
    ...
    try:
        ...
        if hasattr(doc_tool_server, 'LONG_DOCS_ROOT_PATH'):
            default_path = Path.cwd() / "long_documents_storage"
            doc_tool_server.LONG_DOCS_ROOT_PATH = default_path
