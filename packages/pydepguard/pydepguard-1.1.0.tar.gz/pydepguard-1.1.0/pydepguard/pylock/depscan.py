...
def scan_script_for_imports(filepath: Path) -> list[ImportReference]:
    ...
    with open(filepath, 'r', encoding='utf-8') as f:
        ...
        except SyntaxError as e: # <-- Branch not checked
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            ...
            if node.level and not node.module: # <-- Branch not checked
                module = '.' * node.level 
            ...
            if isinstance(node.func, ast.Name) and node.func.id == '__import__':
                if len(node.args) >= 1 and isinstance(node.args[0], ast.Constant) and isinstance(node.args[0].value, str): # <-- Branch not checked? Somewhere in there..
                    refs.append(ImportReference(
                ...
                if hasattr(node.func.value, 'id') and node.func.value.id == 'importlib': # <-- Branch not checked
