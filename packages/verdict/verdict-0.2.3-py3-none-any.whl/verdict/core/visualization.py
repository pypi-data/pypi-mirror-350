...
class StreamingLayoutManager:
    ...
    class StreamingLayout:
        ...
        def __init__(self, manager: "StreamingLayoutManager", title: str, unit: "Unit", order: int): # noqa: F821 # type: ignore[name-defined]
    ...
    def add(self, unit: "Unit") -> StreamingLayout: # noqa: F821 # type: ignore[name-defined]
        title = f"[{unit.char}]" + (f" [orange_red1]{unit.name}[/orange_red1]" if unit.name else "")
        with self.lock:
            ...
            while len(self.children) >= self.capacity:
                ...
                if not any(child.unit.completed for child in self.children):
                    return layout
                self.children.sort(key=lambda child: (-int(child.unit.completed), child.order))
...
class BranchManager:
    ...
    def __init__(self, lock: threading.Lock, branch: Optional[rich.tree.Tree], n_peers: int, should_pin_output: bool) -> None:
    ...
    def update(self, state: ExecutionState, unit: "Unit") -> None: # noqa: F821 # type: ignore[name-defined]
        ...
        with self.lock:
            ...
            text = f"[{unit.char}]" + (f" [orange_red1]{unit.name}[/orange_red1]" if unit.name else "")
            if state == ExecutionState.WAITING_FOR_DEPENDENCIES:
                if self.n_peers <= 1:
                    ...
                    table.add_column(justify="right", ratio=1, no_wrap=True, overflow="ellipsis") # result

                    groups = [rich.spinner.Spinner(BranchManager.spinner[state], text=text)]
                    ...
                    progress = rich.progress.Progress(
                        rich.progress.SpinnerColumn(spinner_name=BranchManager.spinner[ExecutionState.RUNNING]),
                    ...
                    self.branch = self.branch.add(rich.console.Group(
                        progress
                    ))
            elif state in [ExecutionState.WAITING_FOR_RESOURCES, ExecutionState.RUNNING]:
                if self.n_peers <= 1:
                    self.branch.label.columns[0]._cells[0].renderables[0] = rich.spinner.Spinner( # type: ignore
                        BranchManager.spinner[state],
                        text=self.branch.label.columns[0]._cells[0].renderables[0].text, # type: ignore
                    )
                # (pending_tasks): increment pending (just once!), use task.fields
            elif state == ExecutionState.COMPLETE:
                if self.n_peers <= 1:
                    self.branch.label.columns[0]._cells[0].renderables[0] = f"✔ {self.branch.label.columns[0]._cells[0].renderables[0].text}" # type: ignore
                    if isinstance(unit.output, Schema):
                        ...
                        if len(unit_output) == 1:
                            ...
                            output_str = ", ".join([f"{k}: {clean(v)}" for k, v in unit_output.items()])

                        self.branch.label.columns[1]._cells[0] = rich.text.Text(output_str, style="bright_black") # type: ignore
                else:
                ...
                if self.n_peers <= 1:
                    ...
                    column = next(filter(lambda c: isinstance(c, rich.progress.SpinnerColumn), self.branch.label.renderables[0].columns)) # type: ignore
