...
def init_logger(name: Optional[str]=None) -> None:
    ...
    if config.DEBUG or (log_level := os.getenv("LOG_LEVEL", False)):
        logger.add(sys.stderr, format=VERDICT_COMPACT_LOG_FORMAT, level=log_level or 'DEBUG')

    if name and not os.getenv("VERDICT_NO_LOG", False):
        logger.add(config.VERDICT_LOG_DIR / f"{name}_{{time}}.log", format=VERDICT_VERBOSE_LOG_FORMAT)
