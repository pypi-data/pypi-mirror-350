...
class ContractsBackend:
    """Class that manages the contracts system"""
    def __init__(self):
        self.enabled = True
...
class ContractDefinitionError(ContractException):
    ...
    def __init__(self, error, where=None):
        self.error = error
        self.where = where
        ContractException.__init__(self, error, where)
...
class ExternalScopedVariableNotFound(ContractDefinitionError):
    """ Thrown when a variable is not found in the external scope. """
