...
class OWComPair(OWWidget):
    name = _tr.m[39, "Com Pair"]
    description = _tr.m[40, "Learns a model that compares pairs of items"]
    icon = "icons/compair.svg"

    class Inputs:
        data = Input(_tr.m[41, "Items"], Table)
        model = Input(_tr.m[42, "Model"], LogisticRegressionClassifier)

    class Outputs:
        model = Output(_tr.m[43, "Model"], LogisticRegressionClassifier)

    class Error(OWWidget.Error):
        no_continuous_target = Msg(
            _tr.m[44, "If data has a target variable, it must be numeric."])
        invalid_data = Msg(
            _tr.m[45, "Data must contain numeric variables without missing values."])
        not_enough_data = Msg(
            _tr.m[46, "Not enough training data (at least 5 instances)."])
        equal_values = Msg(
            _tr.m[47, "Some instances have the same value of the target variable."])
        no_image_column = Msg(
            _tr.m[48, "Data must contain a string column with image URLs."])
    ...
    ButtonTexts = [_tr.m[49, "No Data"], _tr.m[50, "Make a Prediction"], _tr.m[51, "Show Next pair"], _tr.m[52, "No More Pairs"]]
    ButtonTextGuess = _tr.m[53, "Make a Guess"]
    ...
    def __init__(self):
        ...
        self.restart_button = QPushButton(_tr.m[54, "Start Again"], autoDefault=False)
        self.restart_button.clicked.connect(self.restart)
        layout.addWidget(self.restart_button, 4, 0, 1, 3, Qt.AlignLeft)

        hlayout = QHBoxLayout()
        self.next5_button = QPushButton(_tr.m[55, "Next Five"], autoDefault=False)
        self.next5_button.clicked.connect(self.next5)
        hlayout.addWidget(self.next5_button)

        self.finish_button = QPushButton(_tr.m[56, "Classify All"], autoDefault=False)
        ...
        layout.addWidget(
            gui.checkBox(
                None, self, "exclude_seen", _tr.m[57, "Exclude from model pairs with either car in this pair"]),
            5, 0, 1, 3, Qt.AlignCenter)

        layout.setRowMinimumHeight(6, 20)
        layout.addWidget(
            QLabel(_tr.m[58, "Learning Curve & Current Score"]),
    ...
    def _show_prediction(self):
        old_young = [_tr.m[59, "later"], _tr.m[60, "earlier"]]
        wrong_correct = [_tr.m[61, "wrong"], _tr.m[62, "correct"]]
        ...
        correct = pred == (i1 < i2)
        self.prediction_left.setText(_tr.e(_tr.c(63, f"Prediction: <b>{old_young[int(pred)]}</b>")))
