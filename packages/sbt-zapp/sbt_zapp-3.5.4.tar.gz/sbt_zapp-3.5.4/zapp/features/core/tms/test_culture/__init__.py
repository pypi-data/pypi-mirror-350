...
TEST_CULTURE_LABEL_NAME = env.str("TEST_CULTURE_DEFAULT_LABEL_NAME", default="label")

if TEST_CULTURE_INTEGRATION_ENABLED:
    if not TEST_CULTURE_SPACE:
        raise ValueError("Параметр TEST_CULTURE_SPACE не задан")

    if not TEST_CULTURE_LABEL_NAME:
        raise ValueError("Параметр TEST_CULTURE_LABEL_NAME не задан")

    if TEST_CULTURE_LABEL_NAME == "allure.link.tms":
        raise ValueError("Параметр TEST_CULTURE_LABEL_NAME задан некорректно")
