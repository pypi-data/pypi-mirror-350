...
class DatabaseManager:
    ...
    def verify_api_key(self, api_key: str) -> tuple[bool, Optional[str]]:
        ...
        try:
            # Don't print API key verification attempts in production
            ...
            result = self.supabase.table('api_keys') \
                .select('id, key, revoked_at, rate_limit, last_used') \
                .eq('key', api_key) \
                .is_('revoked_at', 'null') \
                .execute()
            ...
            if result.data and len(result.data) > 0:
                ...
                # last_used = result.data[0].get('last_used')
                # rate_limit = result.data[0].get('rate_limit', 100)  # Default to 100 requests per day
            ...
            logging.warning("No valid key found")
            return False, None
