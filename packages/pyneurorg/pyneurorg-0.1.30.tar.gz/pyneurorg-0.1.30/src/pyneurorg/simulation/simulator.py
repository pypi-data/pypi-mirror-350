...
class Simulator:
    ...
    def add_stimulus(self, electrode_id: int, stimulus_waveform: b2.TimedArray,
        ...
        The `target_current_var` (e.g., 'I_stimulus' or 'I_input') of the
        identified target neurons will be set to follow the `stimulus_waveform`.
        It's crucial that `target_current_var` is defined in the neuron model
        (e.g., `I_stimulus : amp`) and preferably initialized (e.g., to 0*amp)
        when the NeuronGroup is created.
        """
        if self.mea is None:
        ...
        if not hasattr(target_ng, target_current_var):
            # This checks if the attribute exists on the NeuronGroup object.
            # For variables defined in equations, this should generally be true.
            raise AttributeError(
                f"Target NeuronGroup '{target_group_name}' does not have "
                f"a settable variable/attribute named '{target_current_var}'. "
        ...
        # Directly assign the TimedArray to the indexed state variable of the NeuronGroup.
        # Brian2 is designed to handle this: when a TimedArray is assigned to a slice
        # of a state variable, that variable for those specific neurons will follow
        # the TimedArray's values over time.
        try:
            ...
            # This is where the "Quantities can only be created from numerical data" error occurs.
            # It suggests an issue with how Brian2 is trying to process this specific assignment.
            print(f"Error assigning TimedArray directly to subgroup for variable '{target_current_var}': {e}")
            print("This usually means Brian2 encountered an issue trying to set the sliced variable "
                  "with the TimedArray object itself, possibly expecting raw numerical data at some internal step.")
            print("Ensure that the variable is defined in the NeuronGroup's equations (e.g., 'I_input : amp') "
                  "and was initialized (e.g., to 0*amp upon NeuronGroup creation).")
            import traceback
            traceback.print_exc()
            raise
