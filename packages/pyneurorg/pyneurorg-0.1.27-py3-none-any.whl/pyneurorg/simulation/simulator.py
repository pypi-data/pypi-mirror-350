...
class Simulator:
    ...
    def add_stimulus(self, electrode_id: int, stimulus_waveform: b2.TimedArray,
        ...
        # --- MODIFIED APPROACH FOR NAMING AND NAMESPACE ---
        # Determine a suitable name for the TimedArray in the NeuronGroup's namespace.
        # Use the TimedArray's own name if it's good, otherwise generate a unique one.
        name_in_namespace = stimulus_waveform.name
        
        # Brian2 default names can be like '_timedarray_1', or None if not set by user.
        # Also check for collisions in the target NeuronGroup's namespace.
        is_generic_name = name_in_namespace is None or name_in_namespace.startswith(('_timedarray', 'timedarray')) # Common Brian2 default prefixes
        
        if is_generic_name or (name_in_namespace in target_ng.namespace and target_ng.namespace[name_in_namespace] is not stimulus_waveform):
            # If name is generic, or if it collides with a *different* object in the namespace, generate a unique one.
            name_in_namespace = f'pyneurorg_stim_ta_{self._stimulus_namespace_counter}'
            self._stimulus_namespace_counter += 1
        
        # Add the TimedArray object to the target NeuronGroup's namespace using the chosen name.
        # This makes it accessible in the 'code' string of run_on_event.
        target_ng.namespace[name_in_namespace] = stimulus_waveform
        
        # Define the code to be executed for the subset of neurons
        code_to_run = f"{target_current_var} = {name_in_namespace}(t)"
        # --- END OF MODIFIED APPROACH FOR NAMING AND NAMESPACE ---
        ...
        try:
            ...
            print(f"Stimulus from electrode {electrode_id} (TimedArray referenced as '{name_in_namespace}') "
            ...
            # Clean up the namespace if the run_on_event setup failed
            if name_in_namespace in target_ng.namespace and target_ng.namespace[name_in_namespace] is stimulus_waveform:
                del target_ng.namespace[name_in_namespace]
            raise
