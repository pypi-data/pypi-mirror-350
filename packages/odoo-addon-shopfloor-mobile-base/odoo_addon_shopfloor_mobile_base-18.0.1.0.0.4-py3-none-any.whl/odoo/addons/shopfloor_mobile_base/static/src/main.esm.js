...
import {GlobalMixin} from "./mixin.esm.js";
import {config_registry} from "./services/config_registry.esm.js";
import {process_registry} from "./services/process_registry.esm.js";
import {page_registry} from "./services/page_registry.esm.js";
import {color_registry} from "./services/color_registry.esm.js";
import {auth_handler_registry} from "./services/auth_handler_registry.esm.js";
import {Odoo, OdooMocked} from "./services/odoo.esm.js";
import event_hub from "./services/event_hub.esm.js";
import VueSuperMethod from "./lib/vue-super-call.min.js";
import {translation_registry} from "./services/translation_registry.esm.js";
...
Vue.use(window.Vue2StoragePlugin, {
    ...
    driver: _.result(shopfloor_app_info, "storage.driver", "session"),
    // 24 hours
    ttl: _.result(shopfloor_app_info, "storage.ttl", 60 * 60 * 24 * 1000),
});
...
const register_app_components = function (components) {
    _.forEach(components, function (process) {
...
new Vue({
    ...
    beforeMount: function () {
        if (this.current_language) {
            ...
            this.switch_language(this.current_language);
        }
    ...
    methods: {
        ...
        */
        trigger(event_name, data, no_state) {
            let emitted_event = event_name;
            if (this.global_state_key && !no_state) {
                emitted_event = this.global_state_key + ":" + event_name;
            }
            event_hub.$emit(emitted_event, data);
        },
    },
}).$mount("#app");
