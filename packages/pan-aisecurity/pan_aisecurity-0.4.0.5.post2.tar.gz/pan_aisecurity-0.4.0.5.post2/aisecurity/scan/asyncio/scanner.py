...
class Scanner(BaseLogger):
    ...
    async def sync_scan(
        ...
        scan_response = await self._scan_executor.sync_request(ai_profile=ai_profile, content=content, tr_id=tr_id, metadata=metadata)

        self.logger.info("event=%s " % (self.sync_scan.__name__,))
        self.logger.debug(
            "event=%s ai_profile=%r "
            % (
                self.sync_scan.__name__,
                ai_profile,
            )
        )
    ...
    async def async_scan(self, async_scan_objects: list[AsyncScanObject]) -> AsyncScanResponse:
        ...
        self.logger.info("event=%s " % (self.async_scan.__name__,))
    ...
    async def query_by_scan_ids(self, scan_ids: list[str]) -> list[ScanIdResult]:
        ...
        self.logger.info("event=%s " % (self.query_by_report_ids.__name__,))
        self.logger.debug(
            "event=%s scan_ids=%r "
            % (
                self.query_by_report_ids.__name__,
                scan_ids,
            )
        )
    ...
    async def close(self):
        if self._scan_executor is not None:
            await self._scan_executor.close()
        if self._async_scan_executor is not None:
            await self._async_scan_executor.close()
        if self._query_by_report_ids is not None:
            await self._query_by_report_ids.close()
        if self._query_scan_by_scan_ids is not None:
            await self._query_scan_by_scan_ids.close()
