...
from src.sensory_detector.yolo_server.video_analizer.frame_reader import AVError
        ...
        if content_type.startswith("video/"):
            ...
            try:
                ...
                frames = await asyncio.to_thread(
                    ...
                    str(tmp_path),
                    detector,
                    model_cache, # Передаем кэш
                    current_loop # Передаем loop
                )
    ...
    if path:
        ...
        if is_image:
             ...
             try:
                ...
                # !!! ПЕРЕДАЕМ model_cache И ТЕКУЩИЙ EVENT LOOP !!!
                current_loop = asyncio.get_running_loop()
                frames = await asyncio.to_thread(
                    process_video_sync,
                    final_path,
