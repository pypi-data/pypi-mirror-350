...
class TestVectorSynchronizer:
	...
	async def test_get_qdrant_state(self, vector_synchronizer: VectorSynchronizer) -> None:
		...
		# Replace the methods on the manager using patch
		with (
			patch.object(vector_synchronizer.qdrant_manager, "initialize", mock_initialize),
			patch.object(vector_synchronizer.qdrant_manager, "get_all_point_ids_with_filter", mock_get_ids),
			patch.object(vector_synchronizer.qdrant_manager, "get_payloads_by_ids", mock_get_payloads),
		):
	...
	async def test_process_and_upsert_batch(
		...
		async def mock_process_batch(chunk_batch: list[dict[str, Any]]) -> int:
			...
			for chunk in chunk_batch:
				...
				points.append(PointStruct(id=chunk_id, vector=[0.1, 0.2, 0.3], payload=payload))
		...
		with (
			patch.object(vector_synchronizer.qdrant_manager, "upsert_points", mock_upsert),
			patch.object(vector_synchronizer, "_process_and_upsert_batch", mock_process_batch),
		):
	...
	async def test_process_and_upsert_batch_empty(self, vector_synchronizer: VectorSynchronizer) -> None:
		...
		with patch.object(vector_synchronizer, "_process_and_upsert_batch", mock_process_empty):
	...
	async def test_sync_index(
		...
		mock_chunk_file = MagicMock(return_value=sample_chunks)
		...
		with (
			...
			patch.object(
				vector_synchronizer.repo_checksum_calculator,
				"read_latest_checksum_data_for_current_branch",
				mock_read_checksum,
			),
			patch.object(vector_synchronizer.repo_checksum_calculator, "calculate_repo_checksum", mock_calculate),
			patch.object(vector_synchronizer.chunker, "chunk_file", mock_chunk_file),
			patch.object(vector_synchronizer, "_get_qdrant_state", mock_get_state),
			patch.object(vector_synchronizer, "_compare_states", mock_compare),
			patch.object(vector_synchronizer, "_process_and_upsert_batch", mock_process),
		):
			# Call sync_index
