                ...
                if original_region.is_fixed_for(target, identifier):
                    violations.append(
                        self._create_violation(
                            ...
                            original_content="\n".join(
                                original_region.content.splitlines()[: self.context_lines * 2]
                            )
                            + (
                                "\n... (region continues)"
                                if len(original_region.content.splitlines())
                        ...
                        if original_region.is_fixed_for(target, identifier):
                            violations.append(
                                self._create_violation(
                                    ...
                                    original_content="\n".join(
                                        original_region.content.splitlines()[
                                            : self.context_lines * 2
                                        ]
                                    )
                                    + (
                                    ...
                                    modified_content="\n".join(
                                        modified_region.content.splitlines()[
                                            : self.context_lines * 2
                                        ]
                                    )
                                    + (
                                        "\n... (region continues)"
                                        if len(modified_region.content.splitlines())
                                        > self.context_lines * 2
                                        else ""
                                    ),
                                    original_hash=self.hash_calculator.calculate_hash(
    ...
    def _extract_change_context(
        ...
        for tag, i1, i2, j1, j2 in matcher.get_opcodes():
            if tag != "equal":
                ...
                for i in range(
                    max(0, i1 - context_lines), min(len(orig_lines), i2 + context_lines)
                ):
        ...
        if orig_indices and max(orig_indices) < len(orig_lines) - 1:
            orig_context_lines.append(
                f"... ({len(orig_lines) - max(orig_indices) - 1} lines omitted) ..."
            )
        ...
        if mod_indices and max(mod_indices) < len(mod_lines) - 1:
            mod_context_lines.append(
                f"... ({len(mod_lines) - max(mod_indices) - 1} lines omitted) ..."
            )
