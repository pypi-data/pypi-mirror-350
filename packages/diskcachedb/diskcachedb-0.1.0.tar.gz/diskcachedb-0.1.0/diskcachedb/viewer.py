...
app.secret_key = "discache-db"  # Change this in production

# Secret key for delete operations - this should be changed in production
DELETE_SECRET_KEY = "discache-db"  # Change this in production
...
def get_database_instance(cache_dir: str, db_name: str) -> Optional[DiskDB]:
    """Get database instance for the given cache directory and database name.
    ...
    """
    try:
        return DiskDB(db_name, base_directory=cache_dir)
    except Exception:
        return None
...
def index():
    ...
    if request.method == 'POST':
        ...
        if cache_dir and db_name:
            db_path = Path(cache_dir) / db_name
            if not db_path.exists():
                return render_template('error.html', message=f'Database {db_path} does not exist')
...
@app.route('/list_dbs', methods=['POST'])
def list_dbs():
    ...
    if cache_dir and os.path.isdir(cache_dir):
        for directory in os.listdir(cache_dir):
            db_path = cache_dir / directory / 'cache.db'
            if os.path.isfile(db_path):
                db_names.append(directory)

    return jsonify({'db_names': db_names})
...
def view_documents():
    ...
    if not cache_dir or not db_name:
        return redirect(url_for('index'))

    db = get_database_instance(cache_dir, db_name)
    if not db:
    ...
    try:
        documents = db.query(
            # Use descending order (-1) to get newest first
            sort={'last_updated_at': -1},
            limit=10
        )
    except Exception:
        # If last_updated_at doesn't exist, get documents without sorting
        documents = db.query(limit=10)
...
def delete_records():
    ...
    try:
        ...
        db = get_database_instance(cache_dir, db_name)
        if not db:
            raise DeleteError('Could not connect to database')

        # First get matching records count
        matching_records = db.query(conditions=conditions)
        ...
        try:
            ...
            for record in matching_records:
                # Get the key from the record - it's stored in _id
                key = record.get('_id')
                if key:
                    db.remove(key)
                    deleted_count += 1
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='Diskcache database viewer'
    )
    parser.add_argument(
        '-b', '--bind',
        type=str,
        default='0.0.0.0:5000',
        help='Bind address (default: 0.0.0.0:5000)',
        required=False
    )
    args = parser.parse_args()
    try:
        bind_address, port = args.bind.split(':')
    except ValueError:
        logger.error(f'Invalid bind address: {args.bind}. Must be in format <host>:<port>')
        sys.exit(1)
    app.run(host=bind_address, port=int(port))
