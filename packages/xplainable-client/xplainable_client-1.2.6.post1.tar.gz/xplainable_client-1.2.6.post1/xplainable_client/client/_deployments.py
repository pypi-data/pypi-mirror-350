...
class Deployments(Client_Cog):
    ...
    def deploy(
        ...
        url = f'{self.session.hostname}/client/deployments/create'
    ...
    def generate_deploy_key(
        self, team_id, deployment_id: str, description: str = "",
        ...
        url = f'{self.session.hostname}/client/deployments/create-deploy-key'
        response = self.session._session.post(url=url, json=force_json_compliant(payload))
        content = self.session.get_response_content(response)
