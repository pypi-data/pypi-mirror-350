...
def ANTARESS_settings(data_dic,mock_dic,gen_dic,theo_dic,plot_dic,glob_fit_dic,detrend_prof_dic):
    ...
    #    - this field controls the upload and formatting of the data
    ...
    #%%%%% Directory name    
    #    - by default ANTARESS stores all outputs under /Working_directory/Star_name/
    #    - this field defines the possible suffix to add to 'Star_name'
    gen_dic['dir_suff']=''
    ...
    #    - format : {inst : mod}
    #    - set 'mod' to :
    # + 'spline' : preferred option for blaze-derived profiles (default if available)
    #              use the calibration plot to check that the chosen smoothing factor captures well the measured profiles
    # + 'custom' : piecewise model made of a blue, a central, and a red polynomial
    #              preferred option for estimated profiles 
    gen_dic['gcal_mod']={}
    
    
    #%%%%% Smoothing factor
    #    - format : {inst : val}
    #    - relevant for spline model
    gen_dic['gcal_smooth']={}    
    
    
    #%%%%% Edge polynomials
    #    - relevant for custom model
    ...
    #      binw should be large enough to smooth out sharp variations in the model calibration profile
    #      binw should be fine enough to capture the variations of the calibration profiles at the edges of the orders
    #    - format: inst : value
    ...
    #    - format is : mod_prop = { inst : { vis : { par : { 'vary' : bool , 'value': float , 'min': float, 'max': float } } } }        
    #      leave empty the various fields to use default values
    #    - see details of fit settings in data_dic['DI']['mod_prop'] 
    #    - 'par' can be one of:
    # + 'Temperature__'+mol  (in K) : temperature of the Earth model layer
    # + 'Pressure_LOS__'+mol (in atm) : average pressure over the layers occupied by the species
    # + 'ISV_LOS__'+mol (in cm-2) : integrated species vapour along the LOS
    #      where mol is the molecule name from 'tell_species'
    gen_dic['tell_mod_prop']={}
        

    #%%%% Model and fit settings
    #    - for the 'autom' correction mode
    #    - the fit of the telluric model is performed by comparing measured and modelled CCFs over telluric lines
    #      by default the model is a telluric spectrum, but it can be multiplied by a reference stellar spectrum if provided
    
    #%%%%% Single / multi fit
    #    - for the 'autom' correction mode
    #    - runs the fit sequentially over each molecule ('single'), or over all of them together ('multi')
    gen_dic['tell_fit_mod']='single'

    
    #%%%%% Stellar spectrum
    #    - must be a 1D spectrum (flux in arbitrary units as a function of wavelength bin edges, in A in the star rest frame)
    #    - format : 
    # { inst : { 'orig' : str , 'path' : str , 'conv' : bool, 'shift' : str } }
    #    - choose the origin 'orig' of the spectrum and the corresponding format of the file to be uploaded from the absolute 'path' :
    # + 'ANTARESS1D' : the file should be an ANTARESS .npz file with standard 'flux' and 'edge_bins' tables, defining a 1D spectrum over the telluric-absorbed range, ideally normalized to a continuum unity
    # + 'ANTARESS2D' : the file should be an ANTARESS .npz file with standard 'flux' and 'edge_bins' tables, defined over the same spectral orders as the processed dataset, and blazed
    # + TBD
    #    - the field 'conv' indicates whether the provided spectrum is convolved or not by an instrumental response. 
    #      ideally it should be unconvolved for the model spectrum to reproduce more realistically the measured spectrum
    #    - the field 'shift' defines the RV used to shift the provided spectrum into the Earth rest frame for each exposure:
    # + 'kep': Keplerian model 
    #          preferred option, as pipeline RVs will be biased by the RM effect, but requires knowledge of the correct systemic rv and keplerian properties
    # + 'pip': pipeline RVs (if available)
    gen_dic['tell_Fstar_ref'] = {}
    

    #%%%%% CCF definition range
    ...
    #%%%%% CCF continuum
    #    - we allow for a polynomial continuum, with degree ideg from 1 to 4 (default: 0 for flat, constant continuum)
    ...
    if gen_dic['sequence']=='st_master_tseries':gen_dic['Fbal_clip'] = False
    ...
    if gen_dic['sequence']=='st_master_tseries':gen_dic['Fbal_mod']='spline'
    ...
    #      use of this module should be limited to the analyis of differential CCF
    #      for subsequent steps of the workflow it is preferred to process diffential echelle spectra for intrinsic or atmospheric extraction, and then convert those profiles into CCFs
