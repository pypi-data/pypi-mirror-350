...
def rescale_profiles(data_inst,inst,vis,data_dic,coord_dic,exp_dur_d,gen_dic,plot_dic,system_param,theo_dic,ar_dic={}):
    ...
    if (vis not in data_dic['DI']['transit_prop'][inst]):
        if (not cond_ar) and ((not cond_tr) or (data_vis['n_in_tr']==0)):cond_lc = False
        else:
...
def extract_diff_profiles(gen_dic,data_dic,inst,vis,data_prop,coord_dic):
    ...
    if (gen_dic['calc_diff_data']):
        ...
        if (inst in data_dic['Diff']['vis_in_bin']) and (len(data_dic['Diff']['vis_in_bin'][inst])>1):
            mode='multivis'
        ...
        if (isub==0) or resamp_cond_all:                
            ...
            for iexp_off in idx_to_bin_mast:
                ...
                if resamp_cond[iexp_off]:
                    ...
                    #Weight definition         
                    data_to_bin[iexp_off]['weight'] = weights_bin_prof(range(nord),scaled_data_paths_vis[vis_bin],inst,vis_bin,corr_Fbal,corr_FbalOrd,save_data_dir,nord,iexp_glob,'DI',vis_type,dim_exp,tell_exp,sing_gcal_exp,data_exp['cen_bins'],data_to_bin_gen[iexp_off]['dt'],flux_ref_exp,None,calc_cond,sdet_exp2 = sdet2_exp,EFsc2_all_in = EFsc2_exp)[0]
...
def EvE_outputs(gen_dic,data_dic):

    #Create EvE output directory
    EvE_outputs_dir = gen_dic['save_data_dir']+'EvE_outputs/'
    if (not path_exist(EvE_outputs_dir)):makedirs(EvE_outputs_dir) 
    if (not path_exist(EvE_outputs_dir+'Observed_data/')):makedirs(EvE_outputs_dir+'Observed_data/') 
    
    #Loop over processed instruments 
    for inst in data_dic['instrum_list']:

        #Creating relevant directories
        if (not path_exist(EvE_outputs_dir+'Observed_data/'+inst+'/')):makedirs(EvE_outputs_dir+'Observed_data/'+inst+'/')

        #Loop over processed visits
        for vis in data_dic[inst]['visit_list']:
          
            #Creating relevant directories
            if (not path_exist(EvE_outputs_dir+'Observed_data/'+inst+'/'+vis+'/')):makedirs(EvE_outputs_dir+'Observed_data/'+inst+'/'+vis+'/')

            #Copy master stellar spectrum into relevant directory
            shutil.copy(glob.glob(gen_dic['save_data_dir']+'/DIbin_data/'+inst+'_all_spec1D_*.npz')[0],EvE_outputs_dir+'Observed_data/'+inst+'/'+vis+'/Master.npz')
    
            #Copy 1D time-series  
            for iexp in range(data_dic[inst][vis]['n_in_visit']):
                shutil.copy(glob.glob(gen_dic['save_data_dir']+'/DI_data/1Dfrom2D/'+inst+'_'+vis+'_'+str(iexp)+'.npz')[0])
    
    #-----------------------------------------------------------
    
    #Default EvE system file
    code_dir = os_system.path.dirname(__file__).split('ANTARESS_launch')[0]
    default_file = code_dir+'ANTARESS_outputs/EVE_ref/EVE_systems.py'
    with open(default_file,'r') as file:lines_default = file.readlines()        



    #Default EvE settings file
    default_file = code_dir+'ANTARESS_outputs/EVE_ref/EVE_settings.py'
    with open(default_file,'r') as file:lines_default = file.readlines() 

    
    #Store a copy of EvE settings and systems in ANTARESS distrib, and modify it as in the launcher
