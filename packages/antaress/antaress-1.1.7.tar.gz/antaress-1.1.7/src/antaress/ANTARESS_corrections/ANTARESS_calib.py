...
def calc_gcal(gen_dic,data_dic,inst,plot_dic,coord_dic,data_prop):
    ...
    if gen_dic['calc_gcal']:
        ...
        for vis in data_dic[inst]['visit_list']:
            ...
            #    - calibration profiles are resampled in each exposure for fitting
            #    - measured blaze-derived profiles are also resampled because they are too heavy to store and use directly throughout the workflow, and 
            # because they may not be defined over the edges of the orders after shifting them
            else:
                data_all_temp = {}
                
                #Calibration model
                if inst in gen_dic['gcal_mod']:model_choice = gen_dic['gcal_mod'][inst]
                else:model_choice = None
                ...
                if (vis in data_inst['gcal_blaze_vis']):
                    ...
                    n_glob_groups = data_vis['n_in_visit']
                    if model_choice is None:model_choice = 'spline'
                    n_edge = gen_dic['gcal_n_edge']
                ...
                #Calibration estimated from sum(s[F]^2)/sum(F^2) summed over larger bins  
                    ...
                    n_edge=  None
                    if model_choice is None:model_choice = 'custom'
                ...
                #Scaling factor for calibration values and smoothing factor
                if model_choice=='spline':
                    scal_val = 1.
                    cal_smooth_inst = gen_dic['gcal_smooth'][inst]
                    p_start = None
                    nfree_gainfit = 10  #arbitrary for spline fitting
                    fixed_args = None                    
                else:
                    scal_val = 1e3 
                    cal_smooth_inst = None

                    #Initialize fit structure for custom model
                    p_start = Parameters()           
                    p_start.add_many(
                ...
                #Calibration measured from blazed spectra
                #    - blaze is retrieved exactly for each exposure, thus there is no need to group them over several exposures
                #    - no need to fit the profile if fully defined, as it will be directly used to define the order calibration profile
                if gcal_blaze_vis:              
                    common_args = (plot_save,data_dic[inst]['nord'],inst,gen_dic['gcal_thresh'][inst],gen_dic['gcal_edges'],gen_dic['gcal_nooutedge'],fixed_args,nfree_gainfit,p_start,data_vis['cal_data_paths'],gcal_blaze_vis,
                                   cal_smooth_inst,scal_val,data_com_vis['min_edge_ord'],data_com_vis['max_edge_ord'],n_edge,data_vis['proc_DI_data_paths'],data_vis['sing_gcal_DI_data_paths'],gen_dic['gcal_binw'][inst],data_inst[vis]['dim_exp'],data_inst['cal_weight'],gen_dic['cond_plot_gcal'])
                    if (gen_dic['gcal_nthreads']>1) and (gen_dic['gcal_nthreads']<=n_glob_groups):data_gain_all,gcal_exp_all[vis] = para_model_gain_meas(model_gain_meas,gen_dic['gcal_nthreads'],n_glob_groups,[iexp_glob_groups_vis[vis]],common_args,data_inst[vis]['dim_exp'])                           
                    else:data_gain_all,gcal_exp_all[vis] = model_gain_meas(iexp_glob_groups_vis[vis],*common_args) 
                    
                    #Exposure-specific profiles from blaze-derived profiles
                    #    - updating the path only if the original profile was completed with the derived model
                    #      if not, it means the profile was fully defined
                    if data_inst['cal_weight']:
                        for iexp in list(data_gain_all.keys()):
                            data_vis['sing_gcal_DI_data_paths'][iexp] = deepcopy(data_vis['cal_data_paths']+str(iexp))                     
                ...
                #Calibration estimated from sum(s[F]^2)/sum(F^2) summed over larger bins  
                    ...
                    for iexp_glob,iexp_in_group in enumerate(iexp_gain_groups):
                        for iexp in iexp_in_group:data_all_temp[iexp] = dataload_npz(data_vis['proc_DI_data_paths']+str(iexp))                              
                            ...
                            if np.sum(idx_def_group)>0:
                                ...
                                for ibin,(low_bin_loc,high_bin_loc) in enumerate(zip(bin_bd[0:-1],bin_bd[1:])):
                                    ...
                                    if len(bin_loc_dic)>0:
                                        bin_ord_dic['gcal'] = np.append( bin_ord_dic['gcal'] , bin_loc_dic['gcal']/scal_val)
                    ...
                    iexp_glob_groups_vis[vis] = range(n_glob_groups)
                    common_args = (plot_save,data_dic[inst]['nord'],gcal_val_all,inst,gen_dic['gcal_thresh'][inst],gen_dic['gcal_edges'],gen_dic['gcal_nooutedge'],fixed_args,nfree_gainfit,p_start,gcal_blaze_vis,cal_smooth_inst,scal_val,data_com_vis['min_edge_ord'],data_com_vis['max_edge_ord'],n_edge)
                    ...
                    #Exposure-specific profiles from calibration profile estimate
                    #    - to be used for weighing
                    #    - we use the model in all pixels, to avoid the spurious features in the estimated profile
                    #    - only calculated for weighing, as the model is directly recomputed on a common grid to define the scaling profile
                    if (plot_save or data_inst['cal_weight']):
                        for iexp_glob,iexp_in_group in zip(iexp_glob_groups_vis[vis],iexp_gain_groups):
                            if (data_inst['cal_weight']):cal_inputs_iexp_glob = data_gain_all[iexp_glob]['gcal_inputs']                        
                            for iexp in iexp_in_group:    
                                data_save_exp = {}
                                
                                #Exposure-specific profile
                                if (data_inst['cal_weight']):
                                    for iord in range(data_inst['nord']):
                                        if cal_smooth_inst is None:
                                            ...
                                            gcal_exp_all[vis][iexp][iord] = cal_inputs_iexp_glob[iord]['spline'](data_all_temp[iexp]['cen_bins'][iord])
                                    data_save_exp['gcal'] = gcal_exp_all[vis][iexp]
                                    
                                    #Defining path toward weight profile
                                    data_vis['sing_gcal_DI_data_paths'][iexp] = deepcopy(data_vis['cal_data_paths']+str(iexp))
                                                                          
                                #Save calibration model properties for each original exposure associated with current exposure group
                                if plot_save:data_save_exp.update(data_gain_all[iexp_glob])                           
                        
                                #Saving
                                datasave_npz(data_vis['cal_data_paths']+str(iexp),data_save_exp)
                                                            
                    data_all_temp.clear()

            #Storing best-fit results
            ...
            for iord in range(data_dic[inst]['nord']): 
                ...
                #Retrieve spline or custom function inputs 
                #    - defined in the rest frame of each input spectrum
                #    - undefined for fully-defined blaze profiles (no model is then required)
                ...
                for iexp_glob in iexp_glob_groups_vis[vis]:
                    ...
                    if (vis not in data_inst['gcal_blaze_vis']):
                        ...
                        if cal_smooth_inst is None:
                            ...
                            mean_gcal_ord_loc=cal_inputs_dic[vis][iord,iexp_glob]['spline'](cen_bins_ord/dopp_shift)
        ...
        datasave_npz(gen_dic['save_data_dir']+'Processed_data/Calibration/'+inst+'_mean_gcal',{'func' : mean_gcal_func})  
        ...
        for vis in data_inst['visit_list']:
            ...
            for iexp in range(data_vis['n_in_visit']):
                ...
                datasave_npz(data_vis['mean_gcal_DI_data_paths'][iexp] ,{'mean_gcal':gcal_exp}) 
...
def model_gain(iexp_glob_groups,iexp_gain_groups,plot_save,nord,gcal_val_all,inst,gcal_thresh,gcal_edges,gcal_nooutedge,fixed_args,nfree_gainfit,p_start,gcal_blaze_vis,cal_smooth_inst,scal_val,min_edge_ord,max_edge_ord,n_edge):
    r"""**Main fit function for estimated instrumental calibration.**
    ...
    """ 
    data_gain_all = {}
    for iexp_glob,iexp_in_group in zip(iexp_glob_groups,iexp_gain_groups):
        ...
        for iord in range(nord):
            data_gain = model_gain_ord(data_gain,iord,gcal_val_all[iexp_glob,iord],gcal_blaze_vis,gcal_thresh,scal_val,cal_smooth_inst,gcal_edges,
                                       fixed_args,p_start,nfree_gainfit,n_edge,min_edge_ord,max_edge_ord,inst,gcal_nooutedge,plot_save)
...
def model_gain_meas(iexp_glob_groups,plot_save,nord,inst,gcal_thresh,gcal_edges,gcal_nooutedge,fixed_args,nfree_gainfit,p_start,cal_data_paths,gcal_blaze_vis,cal_smooth_inst,
                    scal_val,min_edge_ord,max_edge_ord,n_edge,proc_DI_data_paths,sing_gcal_DI_data_paths,gcal_binw,dim_exp,cal_weight,cond_plot_gcal):
    r"""**Main fit function for blaze-derived instrumental calibration.**
    ...
    """ 
    data_gain_all = {}
    gcal_exp_vis = np.zeros([len(iexp_glob_groups)]+dim_exp,dtype=float)*np.nan
    for iexp_sub,iexp_glob in enumerate(iexp_glob_groups):
        data_gcal = dataload_npz(sing_gcal_DI_data_paths[iexp_glob])
        gcal_exp_vis[iexp_sub] = data_gcal['gcal']
        cond_undef_blaze_exp = np.isnan(gcal_exp_vis[iexp_sub])   
        idx_ord_undef = np_where1D(np.sum(cond_undef_blaze_exp,axis=1)>0)
        if len(idx_ord_undef)>0:
            data_exp = dataload_npz(proc_DI_data_paths+str(iexp_glob))
            ...
            #Processing orders with undefined pixels
            for iord in idx_ord_undef: 
                if np.sum(cond_undef_blaze_exp[iord])>0:
                    min_pix = np.nanmin(data_exp['edge_bins'][iord])
                    max_pix = np.nanmax(data_exp['edge_bins'][iord])
                    n_bins_init=int(np.ceil((max_pix-min_pix)/gcal_binw))
                    if n_bins_init==0:stop('ERROR : no bins in resampled calibration profile grid')
                    bin_siz=(max_pix-min_pix)/n_bins_init
                    edge_bin_new=min_pix+bin_siz*np.arange(n_bins_init+1,dtype=float)  
    
                    #Resampling calibration profile
                    gcal_temp = bind.resampling(edge_bin_new, data_exp['edge_bins'][iord],gcal_exp_vis[iexp_sub,iord]/scal_val, kind='cubic')      
                    cond_def_gcal = ~np.isnan(gcal_temp)
                    gcal_val_ord = {
                        'cen_bins':0.5*(edge_bin_new[0:-1]+edge_bin_new[1:])[cond_def_gcal],
                        'gcal' : gcal_temp[cond_def_gcal]}
              
                    #Fitting model
                    data_gain = model_gain_ord(data_gain,iord,gcal_val_ord,gcal_blaze_vis,gcal_thresh,scal_val,cal_smooth_inst,gcal_edges,
                                               fixed_args,p_start,nfree_gainfit,n_edge,min_edge_ord,max_edge_ord,inst,gcal_nooutedge,plot_save)
    
                    #Exposure-specific profiles from blaze-derived profile
                    #    - to be used for weighing
                    ...
                    if cal_smooth_inst is None:
                        gcal_exp_vis[iexp_sub,iord,cond_undef_blaze_exp[iord]] = cal_piecewise_func(data_gain['gcal_inputs'][iord]['par'],data_exp['cen_bins'][iord,cond_undef_blaze_exp[iord]],args=data_gain['gcal_inputs'][iord]['args'])[0] 
                    else:
                        gcal_exp_vis[iexp_sub,iord,cond_undef_blaze_exp[iord]] = data_gain['gcal_inputs'][iord]['spline'](data_exp['cen_bins'][iord,cond_undef_blaze_exp[iord]])
    
            ### End of orders    

            #Saving
            data_gain_all[iexp_glob]=data_gain 
            if (plot_save or cal_weight):
                data_save_exp = {}
            
                #Updating exposure-specific profile with model at undefined pixels
                if cal_weight:
                    data_save_exp['gcal'] = gcal_exp_vis[iexp_sub]
                    data_save_exp['sdet2'] = data_gcal['sdet2']
                
                #Saving calibration model properties for each original exposure associated with current exposure group        
                if plot_save:data_save_exp.update(data_gain)
                    
                #Saving
                #    - with a different path than the original 'sing_gcal_DI_data_paths' otherwise undefined bins get defined after a first path through the module, and are overwritten in the original file
                datasave_npz(cal_data_paths+str(iexp_glob),data_save_exp) 

    ### End of exposures           
          
    return data_gain_all,gcal_exp_vis


def model_gain_ord(data_gain,iord,gcal_val_ord,gcal_blaze_vis,gcal_thresh,scal_val,cal_smooth_inst,gcal_edges,fixed_args,p_start,nfree_gainfit,n_edge,min_edge_ord,max_edge_ord,inst,gcal_nooutedge,plot_save):
    r"""**Sub fit function for instrumental calibration.**
    ...
    fit_done = False
    if gcal_val_ord is not None:
        bin_ord_dic = gcal_val_ord
        ...
        if (not gcal_blaze_vis):
            ...
            cond_fit = (bin_ord_dic['gcal']>0.) & (bin_ord_dic['gcal']<gcal_thresh['global']/scal_val)
        ...
        if (np.sum(cond_fit)>nfree_gainfit):
            ...
            if (np.sum(cond_check)>nfree_gainfit):
                ...
                if (not gcal_blaze_vis):
                    ...
                    #Fit using spline model
                    if cal_smooth_inst is not None:
                        cal_func = UnivariateSpline(bin_ord_dic['cen_bins'][cond_check],bin_ord_dic['gcal'][cond_check],s=cal_smooth_inst,w=1./var_fit)
                        merit={'fit':cal_func(bin_ord_dic['cen_bins'][cond_check])}
                        chi2 = np.sum((bin_ord_dic['gcal'][cond_check] - merit['fit'])**2./var_fit)
                        merit['chi2r'] = chi2/( np.sum(cond_check) - len(cal_func.get_coeffs()) )
                    
                    #Fit using custom model
                    else:
                ...
                if (np.sum(cond_fit)>nfree_gainfit):
                    ...
                    if cal_smooth_inst is not None:
                        ...
                        #Defining phantom bins up to defined order edges to force spline to behave
                        #    - see ANTARESS_flux_balance.py for details
                        #    - this is quite conservative, as there should not be more than a few undefined calibration pixels on each side (since they should be defined where flux is defined)
                        cen_bins_fit = bin_ord_dic['cen_bins'][cond_fit]                               
                        dbin_phantom_blue = np.mean(cen_bins_fit[1:n_edge+1]-cen_bins_fit[0:n_edge])
                        dbin_phantom_red = np.mean(cen_bins_fit[-n_edge::]-cen_bins_fit[-n_edge-1:-1]) 
                        cen_mid = 0.5*(cen_bins_fit[0]+cen_bins_fit[-1])
                        val_fit = bin_ord_dic['gcal'][cond_fit]
                        coeffs_blue = poly.polyfit(cen_bins_fit[0:n_edge]-cen_mid,val_fit[0:n_edge],1)  
                        coeffs_red = poly.polyfit(cen_bins_fit[-n_edge::]-cen_mid,val_fit[-n_edge::],1) 
                        n_phantom_blue = max([10,int((cen_bins_fit[0]-min_edge_ord[iord])/dbin_phantom_blue)])
                        n_phantom_red = max([10,int((max_edge_ord[iord]-cen_bins_fit[-1])/dbin_phantom_red)])
                        cen_bins_phantom_blue = (cen_bins_fit[0] - dbin_phantom_blue - dbin_phantom_blue*np.arange(n_phantom_blue))[::-1] 
                        val_phantom_blue = poly.polyval(cen_bins_phantom_blue-cen_mid, coeffs_blue)
                        cen_bins_fit = np.concatenate((cen_bins_phantom_blue,cen_bins_fit))
                        val_fit = np.concatenate((val_phantom_blue,val_fit))
                        var_fit = np.concatenate((val_phantom_blue*merit['chi2r'] ,var_fit))
                        cen_bins_phantom_red = cen_bins_fit[-1] + dbin_phantom_red + dbin_phantom_red*np.arange(n_phantom_red) 
                        val_phantom_red = poly.polyval(cen_bins_phantom_red-cen_mid, coeffs_red) 
                        cen_bins_fit = np.concatenate((cen_bins_fit,cen_bins_phantom_red))
                        val_fit = np.concatenate((val_fit,val_phantom_red))
                        var_fit = np.concatenate((var_fit,val_phantom_red*merit['chi2r'] ))

                        #Fitting
                        cal_func = UnivariateSpline(cen_bins_fit,val_fit,s=cal_smooth_inst,w=1./var_fit)

                        #Storing best-fit
                        data_gain['gcal_inputs'][iord] = {'spline':cal_func}
                    ...
                    #Fit using custom model
                        ...
                        for par_loc in ['a1','a2','a3','a4','b3','b4','c0','c1','c2','c3','c4']:p_final[par_loc]*=scal_val
        ...
        if (not fit_done):
            data_gain['gcal_inputs'][iord]={'par':None,'args':{'constant':np.median(bin_ord_dic['gcal']*scal_val)}}

        #Save
        if plot_save:
            data_gain['wav_bin_all'][iord] = bin_ord_dic['cen_bins']
            data_gain['cond_fit_all'][iord] = cond_fit
            data_gain['gcal_bin_all'][iord] = bin_ord_dic['gcal']*scal_val
            if cal_smooth_inst is None:
...
def para_model_gain_meas(func_input,nthreads,n_elem,y_inputs,common_args,dim_exp): 
    r"""**Multithreading routine for model_gain_meas().**
    ...
    data_gain_all = {}
    gcal_exp_vis = np.zeros([0]+dim_exp)*np.nan
    for data_gain,gcal_exp in tuple(all_results[i] for i in range(nthreads)):
        data_gain_all.update(data_gain)
        gcal_exp_vis = np.append(gcal_exp_vis,gcal_exp,axis=0)
    y_output=(data_gain_all,gcal_exp_vis)
