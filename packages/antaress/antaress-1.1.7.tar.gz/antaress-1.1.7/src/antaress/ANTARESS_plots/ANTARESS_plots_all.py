...
def ANTARESS_plot_functions(system_param,plot_dic,data_dic,gen_dic,coord_dic,theo_dic,data_prop,glob_fit_dic,mock_dic,nbook_dic,custom_plot_settings):
    r"""**Generic plot initialization.**
    
    Initializes plots with default, user, and notebook settings. 
            ...
            for vis in np.intersect1d(list(data_dic[inst].keys()),plot_set_key['visits_to_plot'][inst]): 
                ...
                if ('spec' in data_dic['DI']['type'][inst]):
                    if (plot_dic['gcal_all']!='') or ((plot_dic['gcal_ord']!='') and plot_set_key['plot_meas_exp']):
                        gcal_cen_binned_all = np.zeros([nord_list,nexp_plot],dtype=object)
                        gcal_meas_binned_all =  np.zeros([nord_list,nexp_plot],dtype=object)
                ...
                for isub_iexp,iexp in enumerate(iexp2plot):
                    ...
                    #Exposure-specific profiles from blaze-derived profiles
                    #    - before they are completed at undefined pixels with model
                    if (gcal_blaze_all is not None):
                        data_sing_gcal= dataload_npz(gen_dic['save_data_dir']+'Processed_data/'+inst+'_'+vis+'_sing_gcal_'+str(iexp)) 
                        if (var_det_all is not None) and ('sdet2' not in data_sing_gcal):var_det_all = None
                    for isub_ord,iord in enumerate(order_list):
                        if ('spec' in data_dic['DI']['type'][inst]):
                            
                            #Binned profiles are not computed if blaze-derived profile is fully defined
                            if (gcal_cen_binned_all is not None):
                                if ('wav_bin_all' in data_load) and (type(data_load['wav_bin_all'][iord])!=int):
                                    ...
                                    gcal_cen_binned_all[isub_ord,isub_iexp] = [np.nan]
                                    gcal_meas_binned_all[isub_ord,isub_iexp] =[np.nan]
                                
                            if len(data_exp['cond_def'][iord]):
                                if (plot_dic['gcal_ord']!='') or (plot_dic['noises_ord']!=''):
                                    cen_bins_all[isub_ord,isub_iexp] = data_exp['cen_bins'][iord][data_exp['cond_def'][iord]]
                                
                                #Model was defined for current order
                                if (plot_dic['gcal_ord']!='') and ('gcal_inputs' in data_load) and (iord in data_load['gcal_inputs']):
                                    if ('spline' in data_load['gcal_inputs'][iord]):
                                        gcal_bins_all[isub_ord,isub_iexp] = data_load['gcal_inputs'][iord]['spline'](cen_bins_all[isub_ord,isub_iexp])
                                        wav_trans_all[:,isub_ord,isub_iexp] = None
                                    elif 'wav_trans_all' in data_load:
                        ...
                        if (plot_dic['gcal_all']!=''):
                            if (gcal_cen_binned_all is not None) and (np.sum(np.isnan(gcal_cen_binned_all[isub_ord,isub_iexp]))==0):
                                ...
                                gcal_cen_binned_mean[isub_ord,isub_iexp] = np.nan
                                gcal_meas_binned_mean[isub_ord,isub_iexp] = np.nan
                                cen_bins_mean[isub_ord,isub_iexp] = gen_dic[inst]['wav_ord_inst'][iord]
                                
                        #Mean calibration per order (from model, for normalization)
                        if ((plot_dic['gcal_all']!='') or plot_set_key['norm_exp']) and ('gcal_inputs' in data_load) and (iord in data_load['gcal_inputs']):
                            if 'spline' in data_load['gcal_inputs'][iord]:
                                gcal_bins_mean[isub_ord,isub_iexp] = np.mean(data_load['gcal_inputs'][iord]['spline'](data_load['wav_bin_all'][iord]))
                            elif 'wav_trans_all' in data_load:
                            ...
                            else:gcal_bins_mean[isub_ord,isub_iexp] = np.nan
                ...
                if (plot_dic['gcal_all']!=''):
                    ...
                    cond_gcal_cen_binned_mean_def = ~np.isnan(gcal_cen_binned_mean)
                    if np.sum(cond_gcal_cen_binned_mean_def)>0:                       
                        plt.ioff()        
                        ...
                        for isub_iexp in range(len(iexp2plot)):
                            cond_gcal_cen_binned_mean_exp_def = cond_gcal_cen_binned_mean_def[:,isub_iexp]
                            if np.sum(cond_gcal_cen_binned_mean_exp_def)>0:  
                                
                                #Mean calibration per order (from measurements)
                                if ('spec' in data_dic['DI']['type'][inst]):plt.plot(gcal_cen_binned_mean[cond_gcal_cen_binned_mean_exp_def,isub_iexp],gcal_meas_binned_mean[cond_gcal_cen_binned_mean_exp_def,isub_iexp],linestyle='',marker='o',markerfacecolor=col_visit[isub_iexp],markeredgecolor=col_visit[isub_iexp],markersize=plot_set_key['markersize'],rasterized=plot_set_key['rasterized'],color = col_visit[isub_iexp])
                                
                                #Mean calibration per order (from model)
                                plt.plot(cen_bins_mean[cond_gcal_cen_binned_mean_exp_def,isub_iexp],gcal_bins_mean[cond_gcal_cen_binned_mean_exp_def,isub_iexp]*sc_fact,linestyle='-',marker='.',markerfacecolor=col_visit[isub_iexp],markeredgecolor=col_visit[isub_iexp],markersize=plot_set_key['markersize'],rasterized=plot_set_key['rasterized'],color = col_visit[isub_iexp])            
                        ...
                        print('WARNING : no calibration profile stored for plotting')                   
                    ...
                    if (plot_dic['gcal_ord']!=''): 
                        for isub_ord,iord in enumerate(order_list):
                            ...
                            for isub_iexp in range(len(iexp2plot)):
                                ...
                                if (not np.isnan(norm_gcal[isub_ord,isub_iexp])):
                                    ...
                                    if plot_set_key['plot_meas_exp'] and (np.sum(np.isnan(gcal_meas_binned_all[isub_ord,isub_iexp]))==0): 
                            ...
                            for isub_iexp in range(len(iexp2plot)):
                                if wav_trans_all[0,isub_ord,isub_iexp] is not None:
    ...
    if ('tell_CCF' in plot_settings):
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_set_key['visits_to_plot'].keys())): 
            ...
            for vis in np.intersect1d(list(data_dic[inst].keys()),plot_set_key['visits_to_plot'][inst]): 
                print('     - Visit :',vis)
                if vis not in plot_set_key['color_dic'][inst]:plot_set_key['color_dic'][inst][vis]='red'                    #uncorrected
                if vis not in plot_set_key['color_dic_sec'][inst]:plot_set_key['color_dic_sec'][inst][vis]='dodgerblue'     #corrected
                ls_dic = {'master':'--','full':'-'}
                ...
                if plot_set_key['print_disp'] is not None:
                    for molec in plot_set_key['tell_species']:  
                        ...
                        for line_key in disp_dic[molec]:
                            ...
                            if True in cond_def:
                                if (inst in plot_set_key['pl_ref']):
                                    x_plot = coord_dic[inst][vis][plot_set_key['pl_ref'][inst][vis]]['cen_ph'][cond_def]
                                else:
                                    x_plot=coord_dic[inst][vis]['bjd'][cond_def]
                                disp_molec = disp_dic[molec][line_key][cond_def]
                                plt.plot(x_plot,disp_molec,color='dodgerblue',linestyle='',zorder=1,marker='o',markersize=1.5)
                                ...
                                #Boundaries
                                x_min=np.min([np.nanmin(x_plot),x_min])
                                x_max=np.max([np.nanmax(x_plot),x_max])                      
    ...
    if ('tell_prop' in plot_settings):
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_set_key['visits_to_plot'].keys())): 
            ...
            for vis in np.intersect1d(list(data_dic[inst].keys()),plot_set_key['visits_to_plot'][inst]): 
                ...
                data_vis = data_dic[inst][vis]
                if (inst in plot_set_key['pl_ref']):
                    x_plot = coord_dic[inst][vis][plot_set_key['pl_ref'][inst][vis]]['cen_ph']
                else:
                    x_plot=coord_dic[inst][vis]['bjd']
                    ...
                    if len(data_path)>0:
                        ...
                        for molec in tell_prop:
                            ...
                            for key in ['Temperature','ISV_LOS','Pressure_LOS']:
                                if p_best[key]['vary']:
                                    if plot_settings[key_plot]['plot_err']:tell_prop[molec][key][:,iexp] = [p_best[key]['val'],p_best[key]['err']]
                                    else:tell_prop[molec][key][:,iexp] = [p_best[key]['val'],0.]
                    ...
                    for key in ['Temperature','ISV_LOS','Pressure_LOS']:
                        ...
                        if True in cond_def:
                            ...
                            if plot_settings[key_plot]['plot_err']:
                                ...
                                plt.errorbar(x_plot[cond_def][sub_cond_def],tell_prop[molec][key][0][cond_def][sub_cond_def],yerr=tell_prop[molec][key][1][cond_def][sub_cond_def],linestyle='',color=plot_set_key['color_dic'][inst][vis],rasterized=plot_set_key['rasterized'],zorder=0,alpha=plot_set_key['alpha_err'])                       
                            else:sub_cond_def = np.repeat(True,np.sum(cond_def))
                            plt.plot(x_plot[cond_def],tell_prop[molec][key][0][cond_def],color=plot_set_key['color_dic'][inst][vis],linestyle='',zorder=1,marker='o',markersize=1.5)
                     
                            #Frame
                            if plot_set_key['x_range'] is not None:
                                x_range_loc=plot_set_key['x_range']
                            else:
                                xmin = x_plot[cond_def][0]
                                xmax = x_plot[cond_def][-1]
                                dx_range = xmax-xmin
    ...
    if ('glob_mast' in plot_settings):
        ...
        sub_plot_all_prof(plot_settings[key_plot],key_plot,plot_dic[key_plot],data_dic,gen_dic)   
    ...
    if ('trans_sp' in plot_settings):
        ...
        sub_plot_DI_trans(plot_settings[key_plot],key_plot,plot_dic[key_plot],data_dic,gen_dic,coord_dic,data_prop,plot_dic)               
    ...
    if ('all_DI_data' in plot_settings):  
        ...
        sub_plot_all_prof(plot_settings[key_plot],'DI',plot_dic['all_DI_data'],data_dic,gen_dic)                 
    ...
    for key_plot in ['DImask_spectra','Intrmask_spectra']:
        if key_plot in plot_settings:    
            ...
            for inst in np.intersect1d(data_dic['instrum_list'],list(plot_set_key['visits_to_plot'].keys())): 
                ...
                #    - this is the spectrum used in the mask definition
                ...
                if key_step=='cont':
                    ...
                    if plot_set_key['plot_raw']:
                        ...
                        #Continuum
                        cont_norm = dataload_npz(gen_dic['save_data_dir']+'Stellar_cont_DI/St_cont_'+inst+'_'+vis_det)['cont_func_dic'](data_mast['cen_bins'][0])
    ...
    for key_plot in ['DImask_ld','Intrmask_ld']:
        if key_plot in plot_settings:
            ...
            #%%%% Plot
            for dist_info in plot_settings[key_plot]['dist_info']:dist2D_stlines_CCFmasks(dist_info,plot_settings[key_plot],key_plot,plot_dic[key_plot],data_dic,gen_dic)  
    ...
    for key_plot in ['DImask_RVdev_fit','Intrmask_RVdev_fit']:
        if key_plot in plot_settings: 
            ...
            #%%%% Plot
            for dist_info in plot_settings[key_plot]['dist_info']:dist1D_stlines_CCFmasks(dist_info,plot_settings[key_plot],key_plot,plot_dic[key_plot],data_dic,gen_dic)
    ...
    for key_plot in ['DImask_tellcont','Intrmask_tellcont']:
        if key_plot in plot_settings:      
            ...
            for dist_info in plot_settings[key_plot]['dist_info']:
                dist1D_stlines_CCFmasks(dist_info,plot_settings[key_plot],key_plot,plot_dic[key_plot],data_dic,gen_dic)
                dist1D_stlines_CCFmasks(dist_info,plot_settings[key_plot],key_plot+'_final',plot_dic[key_plot],data_dic,gen_dic)
    ...
    for key_plot in ['DImask_RVdisp','Intrmask_RVdisp']:
        if key_plot in plot_settings:      
            ...
            #%%%% Plot
            for dist_info in plot_settings[key_plot]['dist_info']:
                dist2D_stlines_CCFmasks(dist_info,plot_settings[key_plot],key_plot,plot_dic[key_plot],data_dic,gen_dic)
                dist1D_stlines_CCFmasks(dist_info,plot_settings[key_plot],key_plot,plot_dic[key_plot],data_dic,gen_dic)
    ...
    if ('input_LC' in plot_settings):
        ...
        for iband in idx_bands_plot:
            ...
            for inst in np.intersect1d(data_dic['instrum_list'],list(plot_set_key['visits_to_plot'].keys())):                           
                ...
                y_max=-1e100
                if (inst not in plot_set_key['color_dic'] ):plot_set_key['color_dic'][inst]={}
                for ivis,vis in enumerate(np.intersect1d(plot_dic['visits_to_plot'][inst],plot_set_key['visits_to_plot'][inst])): 
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_set_key['visits_to_plot'].keys())):   
            ...
            for ivis,vis in enumerate(np.intersect1d(list(data_dic[inst].keys()),plot_set_key['visits_to_plot'][inst])): 
                ...
                for cont_ph in contact_phases[pl_ref]:
                    plt.plot([cont_ph,cont_ph],y_range_loc,color=plot_set_key['col_contacts'],linestyle=':',lw=1.)                
        ...
        for inst in np.intersect1d(data_dic['instrum_list'],list(plot_set_key['visits_to_plot'].keys())): 
            ...
            for vis in np.intersect1d(list(data_dic[inst].keys()),plot_set_key['visits_to_plot'][inst]):
                ...
                if plot_set_key['pc_rms']:
                    ...
                    for cont_ph in contact_phases[pl_ref]:plt.plot([cont_ph,cont_ph],y_range_loc,color=plot_set_key['col_contacts'],linestyle=':',lw=plot_set_key['lw_plot'],zorder=0)
    ...
    if ('all_intr_data' in plot_settings):
        ...
        sub_plot_all_prof(plot_settings[key_plot],'Intr',plot_dic[key_plot],data_dic,gen_dic)             
    ...
    if ('binned_DI_Intr' in plot_settings):
        ...
        #Plot
        sub_plot_DI_Intr_binprof(plot_settings,plot_dic[key_plot],data_dic,gen_dic)     
            ...
            for vis in np.intersect1d(list(data_dic[inst].keys())+['binned'],plot_set_key['visits_to_plot'][inst]): 
                ...
                if orig_vis in list(data_dic['Diff'][inst].keys()):
                    ...
                    if data_type=='Introrig':
                        ...
                        data_format = data_dic['Intr']['type'][inst]
                        data_bin = np.load(gen_dic['save_data_dir']+'Intrbin_data/'+inst+'_'+vis+'_'+data_format+'_'+plot_set_key['dim_plot']+'_add.npz',allow_pickle=True)['data'].item()
...
#            st_spin_norm=1.1  #1.33   #1.08   
        ...
        #Orbits
        #--------------------------------------------
        def plot_orb_func(ax_plot,npts_orbits_plot,pl_params_plot,pl_ref,col_orb_plot,alph_front,alph_back,zord_ref,dzord,lw):
            ...
            ax_plot.plot(x_orbit_plot[w_front][isort],y_orbit_plot[w_front][isort],zorder=zord_ref + dzord, color=col_orb_plot,lw=plot_set_key['lw_plot'],alpha=alph_front)
            ...
            ax_plot.plot(x_orbit_back,y_orbit_back,zorder=zord_ref - dzord, color=col_orb_plot,lw=plot_set_key['lw_plot'],alpha=alph_back)
            ...
            for ipl,pl_loc in enumerate(plot_set_key['pl_to_plot']):
                ...
                if (pl_loc in plot_set_key['lambdeg_err']) or (pl_loc in plot_set_key['aRs_err']) or (pl_loc in plot_set_key['ip_err']):
                    if (pl_loc in plot_set_key['lambdeg_err']):
                        ...
                        rand_draw = np.random.uniform(low=0.0, high=1.0, size=plot_set_key['norb'][ipl])    
                        ...
                        lambdarad_tab = np.repeat(system_param[pl_loc]['lambda_rad'],plot_set_key['norb'][ipl])
                    ...
                    if (pl_loc in plot_set_key['aRs_err']):
                        ...
                        aRs_tab = np.repeat(system_param[pl_loc]['aRs'],plot_set_key['norb'][ipl])
                        aRs_range=[0.,1e10]
                    if (pl_loc in plot_set_key['ip_err']):
                        ...
                        ip_tab = np.repeat(system_param[pl_loc]['inclin_rad'],plot_set_key['norb'][ipl])
                        ip_range=[-1e10,1e10]
    ...
    if ('all_atm_data' in plot_settings):
        ...
        #Plot        
        sub_plot_all_prof(plot_settings,'atm',plot_settings[key_plot],data_dic,gen_dic)          
...
'''    
def get_tell_lines(tell_HITRANS,gen_dic,inst):
    wave_tell_lines = {}
...
def sub_plot_prof_init(plot_mod,plot_options,inst,data_dic):
    ...
    if (plot_mod=='DI_prof_corr'):data_format='spec2D'
    else:data_format = data_dic[data_type_gen]['type'][inst] 
    ...
    return data_type_gen,data_mode,data_format,add_txt_path,txt_aligned


def get_data_path(plot_mod,data_type,inst,vis,data_dic,gen_dic):
...
def sub_plot_prof_dir(inst,vis,plot_options,data_mode,series,add_txt_path,plot_mod,txt_aligned,data_format,data_type_gen,data_dic,gen_dic,glob_fit_dic):
    inout_flag=None
    
    #Data at chosen steps
    data_path_dic = get_data_path(plot_mod,data_format,inst,vis,data_dic,gen_dic)
    ...
    if plot_options['plot_line_model'] or plot_options['plot_line_model_HR'] or (('_res' in plot_mod) & (not plot_options['cont_only'])):
        ...
        #Line profile from best-fit
            ...
            if (plot_options['fit_type']=='indiv'):
                if not os_system.path.exists(gen_dic['save_data_dir']+data_type_gen+data_mode+'_prop/'+add_txt_path+inst+'_'+vis+'.npz'):
                    print('WARNING: No existing fit results')
                    prof_fit_vis = None
                else:prof_fit_vis=dataload_npz(gen_dic['save_data_dir']+data_type_gen+data_mode+'_prop/'+add_txt_path+inst+'_'+vis)  
                ...
                if (data_type_gen in ['Intr','Atm']) and (inst in glob_fit_dic['IntrProf']['idx_in_fit']) and (vis in glob_fit_dic['IntrProf']['idx_in_fit'][inst]):
                    if not os_system.path.exists(gen_dic['save_data_dir']+'Joined_fits/IntrProf/IntrProf_fit_'+add_txt_path+inst+'_'+vis+'.npz'):
                        print('WARNING: No existing fit results')
                        prof_fit_vis = None
                    else:
    ...
    main_path_txt+='/'
    if ('spec' in data_format):main_path_txt+='Spec'
    elif ('CCF' in data_format):main_path_txt+='CCF'
    if (gen_dic['type'][inst]=='spec2D') and ('_1D' in plot_mod):main_path_txt+='1Dfrom2D'
    if ('spec' in gen_dic['type'][inst]) and (data_format=='CCF'):
    ...
    if '_pca' in plot_mod: 
        ...
        if (data_mode == 'bin'):
            ...
            data_bin = dataload_npz(gen_dic['save_data_dir']+data_type_gen+'bin_data/'+add_txt_path+inst+'_'+vis+'_'+data_format+'_'+plot_options['dim_plot']+'_add')
            ...
            if plot_mod=='DImast': 
                ...
                #Specific options
                if (plot_mod=='DI_prof_corr') and ('spec' in data_format):   
    ...
    #Measured data
        ...
        if (data_mode == 'bin'):
            data_path_all = [gen_dic['save_data_dir']+data_type_gen+'bin_data/'+add_txt_path+'/'+inst+'_'+vis+'_'+data_format+'_'+plot_options['dim_plot']+str(iexp) for iexp in iexp2plot]
...
'''
def sub_plot_prof(plot_options,plot_mod,plot_ext,data_dic,gen_dic,glob_fit_dic,data_prop,coord_dic):
    ...
    for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options['visits_to_plot'].keys())): 
        ...
        if ('spec' in gen_dic['type'][inst]): 
            ...
            wave_tell_lines = get_tell_lines(plot_options['plot_tell_HITRANS'],gen_dic,inst)

        #Generic init function
        data_type_gen,data_mode,data_format,add_txt_path,txt_aligned=sub_plot_prof_init(plot_mod,plot_options,inst,data_dic)
        ...
        if (plot_mod in ['DImast']):                                        
            ref_name='phase'
        elif (plot_mod in ['DIbin','DIbin_res','Intrbin','Intrbin_res','Atmbin','Atmbin_res','Intrbin','Atmbin']):
        ...
        for vis in np.intersect1d(list(data_dic[inst].keys())+['binned'],plot_options['visits_to_plot'][inst]): 
            ...
            #Stellar continuum
            if ('spec' in data_format) and (plot_options['st_cont'] is not None):
            ...
            for isub,(iexp,data_path_exp) in enumerate(zip(iexp2plot,data_path_all)): 
                ...
                if do_plot:
                    ...
                    for isub_ord,iord in enumerate(order_list):                    
                        ...
                        if np.sum(cond_def)>0:
                            ...
                            if plot_ord:  
                                ...
                                if ('spec' in data_format) and (plot_options['st_cont'] is not None) and (('DI' in plot_mod) or ('Intr' in plot_mod)):                      
                                    all_ax[key_frame].plot(x_loc,sc_fact*cont_func_dic(x_loc)/mean_flux ,color='black',linestyle='-',lw=plot_options['lw_plot']-0.5,zorder=11) 
                                ...
                                if (not plot_options['multi_ord']):
                                    ...
                                    if (not plot_options['multi_exp'] or isub==1):
                                        ...
                                        if ('_res' in plot_mod) and plot_options['plot_reflev']:
                                    ...
                                    if ((not plot_options['multi_exp']) or (isub==len(iexp2plot)-1)): 
                                        filename = end_plot_prof(pl_ref,inst,vis,all_figs[key_frame],all_ax[key_frame],x_range_loc[key_frame],y_range_loc[key_frame],hide_yticks,plot_options,iexp_or,plot_mod,title_name,ref_name,iord,plot_ext,x_title,y_title,path_loc,data_bin,coord_dic,data_format,data_dic,gen_dic)
                    ...
                    if plot_options['multi_ord'] and ((not plot_options['multi_exp']) or (isub==len(iexp2plot)-1)): 
                        filename = end_plot_prof(pl_ref,inst,vis,all_figs[key_frame],all_ax[key_frame],x_range_loc[key_frame],y_range_loc[key_frame],hide_yticks,plot_options,iexp_or,plot_mod,title_name,ref_name,None,plot_ext,x_title,y_title,path_loc,data_bin,coord_dic,data_format,data_dic,gen_dic)    
            ...
            if plot_options['multi_ord'] and plot_options['multi_exp']:
                filename = end_plot_prof(pl_ref,inst,vis,all_figs[key_frame],all_ax[key_frame],x_range_loc[key_frame],y_range_loc[key_frame],hide_yticks,plot_options,None,plot_mod,title_name,ref_name,None,plot_ext,x_title,y_title,path_loc,data_bin,coord_dic,data_format,data_dic,gen_dic)                
...
def pre_proc_DI_exp(plot_options,inst,vis,maink_list,iexp2plot,iexp_mast_list,data_inst,data_vis,nspec,data_path_dic,idx_sel_ord,nspec_com,cen_bins_com,edge_bins_com,nord_proc,dim_exp_proc,data_list,fixed_args_loc,data_dic,coord_dic,gen_dic,p_best,data_prop):
    ...
    for isub_exp,iexp in enumerate(iexp_proc_list):
        ...
        for maink in maink_list:
            ...
            #Exposure is resampled on common table otherwise
                ...
                if data_proc[maink][iexp]['tell'] is not None:tell_temp = np.zeros([nord_eff,nspec_com],dtype=float)*np.nan
                ...
                if data_proc[maink][iexp]['tell'] is not None:data_proc[maink][iexp]['tell'] = deepcopy(tell_temp)
...
def end_plot_prof(pl_ref,inst,vis,fig_frame,ax_frame,x_range_frame,y_range_frame,hide_yticks,plot_options,iexp_or,plot_mod,title_name,ref_name,iord,plot_ext,x_title,y_title,path_loc,data_bin,coord_dic,data_format,data_dic,gen_dic):
    if (not plot_options['multi_exp']):
        if (plot_mod in ['DIbin','DIbin_res','DImast']):                                        
            ref_val=data_bin['cen_bindim'][iexp_or]
        elif (plot_mod in ['Intrbin','Intrbin_res','Atmbin','Atmbin_res']):
            ref_val=data_bin['cen_bindim'][iexp_or]
        # elif (plot_mod in ['sp_Intr_1D','sp_Atm_1D']) and (plot_options['dim_plot']=='bin'):
    ...
    if plot_options['title']:
        title = title_name
        if 'spec' in data_format:title+=' spectrum'
        else:title+=' CCF'
    ...
    xmajor_int,xminor_int,xmajor_form = autom_tick_prop(dx_range)
    ymajor_int,yminor_int,ymajor_form = autom_tick_prop(dy_range)
    xminor_int_loc = plot_options['xminor_int'] if plot_options['xminor_int'] is not None else xminor_int
    xmajor_int_loc = plot_options['xmajor_int'] if plot_options['xmajor_int'] is not None else xmajor_int
    custom_axis(plt,fig = fig_frame,ax=ax_frame,position=plot_options['margins'],x_range=x_range_frame,y_range=y_range_frame,
                xmajor_int=xmajor_int_loc,xminor_int=xminor_int_loc,ymajor_int=ymajor_int,yminor_int=yminor_int,
                xmajor_form=xmajor_form,ymajor_form=ymajor_form,
    ...
    if not plot_options['multi_exp']:
        ...
        loc_str='_'   
        if vis=='binned':exp_dic = deepcopy(data_bin)
        else:exp_dic = deepcopy(gen_dic[inst][vis])
        if ('idx_exp2in' in exp_dic):
            idx_exp2in = exp_dic['idx_exp2in']
            idx_in = exp_dic['idx_in']
            idx_out = exp_dic['idx_out'] 
...
def calc_occ_plot(coord_dic,gen_dic,contact_phases,system_param,plot_dic,data_dic,data_bin,theo_dic_loc,inst,vis,genpar_instvis,param_loc,args,iband=0,par_list = ['rv','CB_RV','mu','xp_abs','r_proj','y_st','lat']):
    r"""**High-resolution model of planet-occulted regions.**
    
    Calculates coordinates of planets and occulted regions in a given visit.
...
def sub_dist_CCFmasks(dist_info,plot_info,plot_options,ax_loc,var,var_thresh,var_thresh_test,prop_type,y_title,range_loc,ax_name,mode='2D'):
    r"""**CCF mask: 1D histogram sub-routine.**
    
    Adds 1D distributions of line properties for mask generation to existing plot.
...
def dist1D_stlines_CCFmasks(dist_info,plot_options,key_plot,plot_ext,data_dic,gen_dic):
    r"""**CCF mask: 1D distribution.**
    
    Plots 1D distribution of line properties for mask generation.
...
def dist2D_stlines_CCFmasks(dist_info,plot_options,key_plot,plot_ext,data_dic,gen_dic):
    r"""**CCF mask: 2D distribution.**
    
    Plots 2D distribution of line properties for mask generation.
...
def doppler_track_plots(key_track,line_mask,rest_frame,col_loc,cond_track,cond_range,lw_mod,ls_loc,pl_list,theo_HR,line_range,iexp2plot,iexp_range,reverse_2D,data_format,x_range_loc,y_range_loc,coord_vis):
...
def sub_2D_map(plot_mod,save_res_map,plot_options,data_dic,gen_dic,glob_fit_dic,system_param,theo_dic,coord_dic,contact_phases,plot_dic):
    r"""**2D map.**
    ...
    for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options['visits_to_plot'].keys())):             
        ...
        for ivis,vis in enumerate(np.intersect1d(list(data_dic[inst].keys())+['binned'],plot_options['visits_to_plot'][inst])):                 
            ...
            if plot_options['theoRV_HR']:
                theo_HR_prop_plocc = calc_occ_plot(coord_dic,gen_dic,contact_phases,system_param,plot_dic,data_dic,data_bin,deepcopy(theo_dic),inst,vis,{},params,{})
            if plot_options['theoRV_HR_align']:
                system_param_align = deepcopy(system_param)
                system_param_align[pl_ref]['lambda_rad'] = 0.
                theo_HR_prop_plocc_align = calc_occ_plot(coord_dic,gen_dic,contact_phases,system_param_align,plot_dic,data_dic,data_bin,deepcopy(theo_dic),inst,vis,{},params,{})
                ...
                #Data
                    ...
                    if plot_mod in ['map_Diff_prof_clean_pl_est','map_Diff_prof_clean_ar_est','map_Diff_prof_unclean_ar_est','map_Diff_prof_unclean_pl_est',
                        ...
                        #Building residuals
                            ...
                            data_exp_est = dataload_npz(gen_dic['save_data_dir']+'Diff_estimates/'+plot_options['mode_loc_prof_est']+'/'+inst+'_'+vis+'_'+str(iexp)) 
            ...
            for iord in order_list:
                ...
                if cond_plot:
                    ...
                    if (inst in plot_options['v_range_all']) and (vis in plot_options['v_range_all'][inst]) and (plot_options['v_range_all'][inst][vis] is not None):
                        ...
                        if (data_format == 'CCF') and (plot_options['verbose']):print('      range for color table:',"{0:.5f}".format(v_range[0]),' ; ',"{0:.5f}".format(v_range[1]))
                    ...
                    if plot_options['plot_zermark']:
                        if plot_options['reverse_2D']:
                            ...
                            if (data_format == 'CCF') :plt.plot([0,0],y_range_loc,color='black',linestyle='--',lw=0.8,zorder=10)
                        else:
                            if ordi_name == 'phase':plt.plot([0,0],y_range_loc,color='black',linestyle='--',lw=0.8,zorder=10)
                            if (data_format == 'CCF') :plt.plot(x_range_loc,[0,0],color='black',linestyle='--',lw=0.8,zorder=10)                            
                        ...
                        if (rest_frame in ['star','surf']) and ((plot_mod in ['map_Diff_prof','map_Intr_prof_est','map_Intr_prof_res','map_Intrbin']) or ((plot_mod in ['map_Intr_prof']) and (not plot_options['aligned']))) and (plot_options['theoRV_HR'] or plot_options['theoRV_HR_align']): 
                            ...
                            if ('spec' in data_format):
                                if (len(plot_options['st_lines_wav'])>0):line_mask = plot_options['st_lines_wav']
                                    ...
                                    if ('CCF_mask_wav' not in gen_dic) or (inst not in gen_dic['CCF_mask_wav']):
                                        cond_plot_track = False
...
##################################################################################################
#%%% Sub-routines for profiles
##################################################################################################

'''
Sub-function to plot all profiles from a given visit together
'''
def sub_plot_all_prof(plot_options,plot_mod,plot_ext,data_dic,gen_dic):
    sc_fact=10**plot_options['sc_fact10']
...
'''
def sub_plot_DI_trans(plot_options,plot_mod,plot_ext,data_dic,gen_dic,coord_dic,data_prop,plot_dic):
    ...
    for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options['visits_to_plot'].keys())): 
        ...
        for vis in np.intersect1d(list(data_dic[inst].keys()),plot_options['visits_to_plot'][inst]): 
            ...
            # + transmission spectra are calculated in the star rest frame, after spectra are being shifted and resampled over the common grid in pre_proc_DI_exp
            ...
            cond_sel = np.zeros(data_com['dim_exp'],dtype=bool)
            ...
            data_proc,data_mod,data4mast = pre_proc_DI_exp(plot_options,inst,vis,maink_list,iexp2plot,iexp_mast_list,data_inst,data_vis,data_vis['nspec_ref'],data_path_dic,idx_sel_ord,nspec_com,cen_bins_com,edge_bins_com,nord_proc,dim_exp_proc,data_list,fixed_args_loc,data_dic,coord_dic,gen_dic,p_best,data_prop)  
            ...
            for isub_exp,iexp in enumerate(iexp2plot):
                ...
                plt.savefig(path_loc+'idx'+str(iexp)+'_multi_ord.'+plot_dic['trans_sp']) 
                plt.close()      
...
'''
def sub_plot_DI_Intr_binprof(plot_options,plot_ext,data_dic,gen_dic):
    sc_fact=10**plot_options['sc_fact10']
    cmap = plt.get_cmap('rainbow')
    ...
    for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options['visits_to_plot'].keys())):   
        ...
        for vis in np.intersect1d(list(data_dic[inst].keys())+['binned'],plot_options['visits_to_plot'][inst]): 
            ...
            #Exposures to plot
            data_DIbin = np.load(gen_dic['save_data_dir']+'DIbin_data/'+inst+'_'+vis+'_'+data_format+'_'+plot_options['dim_plot_DI']+'_add.npz',allow_pickle=True)['data'].item()
            iexp2plot_DI = np.arange(data_DIbin['n_exp'],dtype=int) 
            nexp_plot_DI=len(iexp2plot_DI) 

            data_Intrbin = np.load(gen_dic['save_data_dir']+'Intrbin_data/'+inst+'_'+vis+'_'+data_format+'_'+plot_options['dim_plot_intr']+'_add.npz',allow_pickle=True)['data'].item()                    
            ...
            if plot_options['data_type']=='CCF':
                ...
                for isub,iexp in enumerate(iexp2plot_DI):             
                    data_exp=np.load(gen_dic['save_data_dir']+'DIbin_data/'+inst+'_'+vis+'_'+data_format+'_'+plot_options['dim_plot_DI']+str(iexp)+'.npz' ,allow_pickle=True)['data'].item()    
                    var_loc=sc_fact*data_exp['flux'][0]
                ...
                for isub,iexp in enumerate(iexp2plot_intr):                
                    data_exp=np.load(gen_dic['save_data_dir']+'Intrbin_data/'+inst+'_'+vis+'_'+data_format+'_'+plot_options['dim_plot_intr']+str(iexp)+'.npz' ,allow_pickle=True)['data'].item()    
                    var_loc=sc_fact*data_exp['flux'][0]
                ...
                #Reference level
                x_range_loc=np.array(plot_options['x_range']) if plot_options['x_range'] is not None else np.array([data_exp['cen_bins'][0,0]-5.,data_exp['cen_bins'][0,-1]+5.]) 
                if plot_options['ref_level']:plt.plot(x_range_loc,[1.,1.],linestyle='-',lw=0.5,color='black')
...
def sub_plot_CCF_prop(prop_mode,plot_options,data_mode,gen_dic,data_dic,system_param,coord_dic,contact_phases,plot_dic,theo_dic,data_prop,glob_fit_dic): 
    ...
    if data_mode=='DI':    
        data_fit_prop=None  
        if prop_mode in ['rv_res','FWHM','ctrst']:
            if plot_options['DIProp_path'] is not None:
                if prop_mode=='rv_res':prop_mode_get = 'rv'
                else:prop_mode_get = prop_mode
                data_fit_prop = dataload_npz(plot_options['DIProp_path']+prop_mode_get+'/Fit_results')      
    elif data_mode=='Intr':    
        data_fit_prop=None  
        data_fit_prof=None
    ...
    for inst in np.intersect1d(data_dic['instrum_list'],list(plot_options['visits_to_plot'].keys())): 
        ...
        for vis in vis_list: 
            dic_vis = {}
            for key in ['x_all','st_x_all','end_x_all','val_all']:dic_vis[key] = np.empty(0,dtype=float)
            dic_vis['eval_all'] = np.empty([2,0],dtype=float)
            dic_vis['HDI_all'] = np.empty([2,0],dtype=float)    
            ...
            if (data_mode=='Intr') and (prop_mode in ['rv','rv_res']) and plot_options['theo_HR_nom'] : 
                ...
                #Residuals from surface RVs      
                    ...
                    if len(plot_options['contrib_theo_HR'])>0:
                        if 'CB' in plot_options['contrib_theo_HR']:
                            plt.plot(xvar_HR[wsort],sc_unit*theo_HR_prop_plocc[pl_ref]['CB_RV'][wsort],color='dodgerblue',linestyle='--',lw=plot_options['lw_plot'],zorder=-1)   
                        if 'DR' in plot_options['contrib_theo_HR']:
                            plt.plot(xvar_HR[wsort],sc_unit*(theo_HR_prop_plocc[pl_ref]['rv'][wsort] - rv_sb_theo_HR_nom - theo_HR_prop_plocc[pl_ref]['CB_RV'][wsort]) ,color='magenta',linestyle='--',lw=plot_options['lw_plot'],zorder=-1)   
                        if 'CB_DR' in plot_options['contrib_theo_HR']:
                            plt.plot(xvar_HR[wsort],sc_unit*(theo_HR_prop_plocc[pl_ref]['rv'][wsort] - rv_sb_theo_HR_nom),color='black',linestyle='--',lw=plot_options['lw_plot'],zorder=-1)   
            ...
            if cond_vis:                
                ...
                if data_mode=='DI':
                    ...
                    if (data_fit_prop is not None):
                        ...
                        #Retrieve modulation
                        if plot_options['norm_mod'] and (prop_mode in ['ctrst','FWHM']):c0_mod = data_fit_prop['p_final'][data_fit_prop['name_prop2input']['c__ord0__IS'+inst+'_VS'+vis]] 
                        
                        #Coordinates of the property model
                        coord_list = list(data_fit_prop['coord_mod'][inst][vis].keys())
                        ...
                        if plot_options['theo_obs_prop']:
                            if coord_plot_eff not in coord_list:stop('ERROR: plot and fit coordinates must match') 
                            prop_mod = sc_unit*data_fit_prop['prop_mod'][inst][vis]
                            plt.plot(data_fit_prop['coord_mod'][inst][vis][coord_plot_eff],prop_mod,color='green',linestyle='',lw=1,marker='s',markersize=plot_options['markersize'],zorder=-1)
                        ...
                        if plot_options['theo_HR_prop']:
                            ...
                            #Constant value was fitted (coordinate-independent)
                            if len(coord_list)==0:
                                mod_prop_HR = sc_unit*mod_DIProp(data_fit_prop['p_final'],fixed_args,inst,vis,1)
                                plt.axhline(mod_prop_HR[0],color='darkgrey', lw=1,linestyle='-',zorder=50)              
                            ...
                            #Single coordinate  
                                ...
                                if len(coord_list)==1:
                                    ...
                                    max_coord_obs = max(coord_obs)
                                    d_HR=(max_coord_obs-min_coord_obs)/n_HR
                                    coord_HR = min_coord_obs + d_HR*np.arange(n_HR)
                                    fixed_args['coord_fit'][inst][vis][coord_plot_eff] = coord_HR
    
                                #Multiple coordinates
                                #    - the model can still be calculated over the full observed coordinate time-series
                                else:    
                                    print('WARNING: HR model cannot be calculated with multiple coordinates')
                ...
                #Intrinsic properties
                    ...
                    if data_type=='Introrig':
                        ...
                        if plot_options['plot_data']:
                            ...
                            data_bin = np.load(gen_dic['save_data_dir']+'Intrbin_data/'+inst+'_'+orig_vis+'_'+data_format+'_'+plot_options['dim_plot']+'_add.npz',allow_pickle=True)['data'].item()
                            ...
                            if prop_mode in ['rv','rv_res']:
                                ...
                                if len(plot_options['contrib_theo_HR'])>0:
                                    if (prop_mode=='rv'):
                                        ...
                                        if 'CB' in plot_options['contrib_theo_HR']:
                                            plt.plot(xvar_HR_loc,sc_unit*theo_HR_prop_loc['CB_RV'][wsort],color='dodgerblue',linestyle='--',lw=plot_options['lw_plot'],zorder=-1)   
                                        if 'DR' in plot_options['contrib_theo_HR']: 
                                            plt.plot(xvar_HR_loc,sc_unit*(theo_HR_prop_loc['Rot_RV'][wsort] - rv_sb_theo_HR),color='magenta',linestyle='--',lw=plot_options['lw_plot'],zorder=-1)   
                                        if 'CB_DR' in plot_options['contrib_theo_HR']:
                                            plt.plot(xvar_HR_loc,sc_unit*(theo_HR_prop_loc['CB_RV'][wsort] + theo_HR_prop_loc['Rot_RV'][wsort] - rv_sb_theo_HR),color='black',linestyle='--',lw=plot_options['lw_plot'],zorder=-1)   
                    ...
                    if (data_fit_prop is not None):
                        ...
                        if plot_options['theo_obs_prop']:
                            coord_plot = plot_options['prop_'+data_mode+'_absc']
                            if ((prop_mode in ['rv','rv_res'] and coord_plot!='phase') or (data_fit_prop['coord_line']!=coord_plot)):stop('ERROR: plot and fit coordinates must match')
                ...
                elif plot_options['prop_'+data_mode+'_absc'] in ['snr','snrQ']: 
                    if plot_options['prop_'+data_mode+'_absc']=='snr':x_obs =np.mean(SNR_obs[:,plot_options['idx_SNR'][inst]],axis=1)
                    elif plot_options['prop_'+data_mode+'_absc']=='snrQ':x_obs =np.sqrt(np.sum(SNR_obs[:,plot_options['idx_SNR'][inst]]**2.,axis=1))
                    st_x_obs,end_x_obs = x_obs,x_obs                                          
                ...
                if plot_options['plot_data']:
                    ...
                    if prop_mode in ['rv','rv_res','rv_pip_res','FWHM','ctrst','amp','rv_l2c','amp_l2c','FWHM_l2c','RV_lobe','amp_lobe','FWHM_lobe','true_ctrst','cont','c1_pol','c2_pol','c3_pol','c4_pol','vsini',\
                        ...
                        for idx_loc in range(n_exp_vis):
                            if idx_loc in prof_fit_vis:
                                val_obs[idx_loc]=sc_unit*prof_fit_vis[idx_loc][prop_loc]
                                if err_prop_loc is not None:eval_obs[:,idx_loc]=[sc_unit*prof_fit_vis[idx_loc][err_prop_loc][0],sc_unit*prof_fit_vis[idx_loc][err_prop_loc][1]]
                        if (prop_mode in ['rv_res','rv_pip_res']):                               
                            if (data_mode=='DI'):
                                for idx_loc in range(n_exp_vis):rv_mod_obs[idx_loc] = sc_unit*prof_fit_vis[idx_loc]['RVmod']
                            ...
                            #Redefine residual from surface RVs using model
                                ...
                                for idx_loc in range(n_exp_vis):
                                    rv_mod_obs[idx_loc] = sc_unit*np.nanmean(y_theo_HR[ (x_theo_HR>=st_x_obs[idx_loc]) & (x_theo_HR<=end_x_obs[idx_loc])])
                                val_obs -= rv_mod_obs 
                    ...
                    elif prop_mode in ['snr','snrQ']: 
                        if prop_mode=='snr':
                            val_obs =np.mean(SNR_obs[:,plot_options['idx_SNR'][inst]],axis=1)
                        elif prop_mode=='snrQ':val_obs =np.sqrt(np.sum(SNR_obs[:,plot_options['idx_SNR'][inst]]**2.,axis=1))
                ...
                if data_mode=='DI':
                    ...
                    #Normalise values
                    if plot_options['norm_ref'] and plot_options['norm_mod']:stop('ERROR: "norm_ref" and "norm_mod" cannot be used together.')
                    if (plot_options['norm_ref'] or plot_options['norm_mod']) and ('rv' not in prop_mode):
                        if (plot_options['norm_ref']) and (not np.isnan(val_out)):val_norm = val_out
                        if (plot_options['norm_mod']) and (not np.isnan(c0_mod)):val_norm = c0_mod
                        val_obs/=val_norm
                        eval_obs/=np.abs(val_norm)   #in case of negative mean            
                    ...
                    for i_loc,iexp_eff in enumerate(idx_in_plot):
                        ...
                        if plot_options['plot_HDI']: 
                            ...
                            for HDI_sub in data_exp['HDI_interv'][ipar]:
                                ...
                                if prop_mode=='rv_res':yerr_sub = sc_unit*np.array(yerr_sub)-rv_mod_obs[i_loc]
                ...
                dic_vis['x_all']=np.append(dic_vis['x_all'],x_obs)
                dic_vis['st_x_all']=np.append(dic_vis['st_x_all'],st_x_obs)
                dic_vis['end_x_all']=np.append(dic_vis['end_x_all'],end_x_obs)
                dic_vis['val_all']=np.append(dic_vis['val_all'],val_obs)
                dic_vis['eval_all']=np.append(dic_vis['eval_all'],eval_obs,axis=1)
                dic_vis['HDI_all']=np.append(dic_vis['HDI_all'],HDIval_obs,axis=1)
            ...
            #-------------------------------------------------------
            #Plot data binned over current visit dataset for current instrument
            if (inst in plot_options['bin_val']) and (vis in plot_options['bin_val'][inst]) and (plot_options['plot_data']): 
                x_min,x_max,y_min,y_max=sub_func_bin(plot_options['bin_val'][inst][vis],dic_vis,x_min,x_max,y_min,y_max,plot_options) 
        ...
        if (plot_options['prop_'+data_mode+'_absc']=='time') and (len(gen_dic['Tcenter_visits'])==0): 
            ...
            for ipl,pl_loc in enumerate(np.unique(studied_pl)):
                ...
                Tcenter_loc = Tcenter_min
                while (Tcenter_loc - 0.5*system_param[pl_loc]['T14_num']) <x_max:
                    contact_times_loc = Tcenter_loc + contact_times_rel
                    for cont_t in contact_times_loc:plt.plot([cont_t,cont_t],[-1e6,1e6],color=plot_options['col_contacts'],linestyle=ls_pl,lw=plot_options['lw_plot'],zorder=0)                                                 
                    Tcenter_loc +=system_param[pl_loc]["period"]    

        #-------------------------------------------------------
        #Plot data binned over all visit datasets for current instrument
        if (inst in plot_options['bin_val']) and ('all' in plot_options['bin_val'][inst]) and (plot_options['plot_data']): 
            x_min,x_max,y_min,y_max=sub_func_bin(plot_options['bin_val'][inst]['all'],dic_inst,x_min,x_max,y_min,y_max,plot_options)   
    ...
    if plot_options['plot_reflev']:
       ...
       elif (data_mode=='DI') and (plot_options['norm_ref'] or plot_options['norm_mod']):val_ref = 1.
       else:val_ref = None
