...
def def_masks(vis_mode,gen_dic,data_type_gen,inst,vis,data_dic,plot_dic,system_param,data_prop):
    ...
    #Using master from single visit
        ...
        vis_det=vis   
        data_format = data_dic[inst][vis]['type']
    
    print('         Calculating data')
    if data_format!='spec1D':stop('Spectra must be 1D') 
    ...
    data_bin = dataload_npz(gen_dic['save_data_dir']+data_type_gen+'bin_data/'+gen_dic['add_txt_path'][data_type_gen]+inst+'_'+vis_det+'_'+data_format+'_'+prop_dic['dim_bin']+'_add')

    #Retrieve master spectrum
    if data_bin['n_exp']>1:stop('Bin data into a single master spectrum')
    data_mast = dataload_npz(gen_dic['save_data_dir']+data_type_gen+'bin_data/'+gen_dic['add_txt_path'][data_type_gen]+inst+'_'+vis_det+'_'+data_format+'_'+prop_dic['dim_bin']+str(0))
