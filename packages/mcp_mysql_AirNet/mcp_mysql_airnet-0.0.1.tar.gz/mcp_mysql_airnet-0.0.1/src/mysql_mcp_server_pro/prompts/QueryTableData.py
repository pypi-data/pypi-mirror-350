    ...
    async def run_prompt(self, arguments: Dict[str, Any]) -> GetPromptResult:
        ...
        - Examples:
          ...
          - 例子3：用户输入“查询订单表张三的订单金额”
            ...
            查询结果：返回张三的订单金额。  
          - 例子4：用户输入“查询EA2289航班的航路信息”
            解析结果：表描述为“航班”，查询条件为“航路信息”。
            调用工具1：根据“航班”描述获取表名，航班信息从报文获取，假设返回表名为“tab_recvtele”。
            调用工具2：根据“tab_recvtele”获取表结构，假设表结构包含字段“flight”、“teletype”、“inittext”，从tab_recvtele表中查询teletype为FPL的记录。
            生成SQL查询语句：`SELECT inittext FROM tab_recvtele WHERE flight = "EA2289"  AND teletype = 'FPL' ORDER BY savetime DESC LIMIT 1;`
            查询结果：返回inittext的相关数据。      
        """
