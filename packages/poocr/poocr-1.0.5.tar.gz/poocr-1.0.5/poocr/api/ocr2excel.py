    ...
    for vat_img in simple_progress(vat_img_files):
        ...
        if isinstance(api_res, list):
            ...
            for pdf_page, page_res in enumerate(api_res):
                res_df = VatInvoiceOCR2(page_res, vat_img, file_name, trans, pdf_page + 1)
                result_df.extend(copy.deepcopy(res_df))
        else:
            res_df = VatInvoiceOCR2(api_res, vat_img, file_name, trans)
            result_df.extend(copy.deepcopy(res_df))
    # 整理全部识别结果
    if len(result_df) > 0:
