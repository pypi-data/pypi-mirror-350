...
from .llm_server import run_server, get_base_url, get_llm_api_key
...
class AgentID(abc.ABC): 
    ...
    def run_llm_proxy(self):
        if get_llm_api_key() != "" and get_llm_api_key() is not None:
            log_info("本地服务已启动")
            return
        run_server()
    ...
    def add_message_handler(
        self, handler: typing.Callable[[dict], typing.Awaitable[None]],session_id:str="",router:str=""
    ):
        """消息监听器装饰器"""
    ...
    def create_session(self, name, subject, *, type='public'):
        ...
        session.set_on_session_message_ack(self.__on_session_message_ack)
        session.set_on_system_message(self.__on_system_message)
    ...
    def send_form_message(self,session_id: str,to_aid_list: [],result: [],ref_msg_id: str):
        try:
            ...
            self.db_manager.update_message(self.id,msg)
                ...
                for line in response.iter_lines():
                    ...
                    if not decoded_line.startswith("data:") and not decoded_line.startswith("event:"):
                        ...
                        if chunk is None:
                            log_error("保持连接-等待3")
                            continue
                        content_text = content_text+chunk.choices[0].delta.content
                        msg_block["content"] = content_text
                    else:
                        log_info("开始拉取流式数据...2："+decoded_line)
...
class AgentCP(_AgentCP):
    ...
    def __init__(self, agent_data_path, certificate_path:str = "", seed_password:str = "",debug = False,log_level:int=logging.INFO):
        super().__init__()       
