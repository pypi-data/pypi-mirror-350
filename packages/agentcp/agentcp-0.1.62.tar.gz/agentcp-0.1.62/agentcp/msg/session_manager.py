...
class SessionManager:
    ...
    def on_message(self, ws, message):
        ...
        try:
            ...
            if cmd == "create_session_ack":
            ...
            elif cmd == "session_message" and session is not None and session.on_message_receive is not None:
                import urllib.parse
                message_content = js["data"]["message"]
                js["data"]["message"] = urllib.parse.unquote(message_content)
                if session.on_message_receive is not None:
                    session.on_message_receive(js["data"])
                ...
                if session.on_invite_ack is not None:
                    session.on_invite_ack(js["data"])
                else:
                    log_error("on_message_recive is None")
            elif cmd == "session_message_ack" and session is not None and session.on_session_message_ack is not None:
                return session.on_session_message_ack(js["data"])
            ...
            elif cmd == "system_message" and session is not None and session.on_system_message is not None:
                session.on_system_message(js["data"])
        except Exception as e:
            import traceback
