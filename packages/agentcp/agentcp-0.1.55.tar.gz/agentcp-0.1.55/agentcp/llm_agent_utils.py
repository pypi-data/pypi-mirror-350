...
class LLMAgent:
    ...
    async def chat_create(self, open_ai_message_body):
        ...
        async def reply_message_handler(reply_msg):
            print(f'[{format_date()}]: 收到agent响应原始消息: {reply_msg}')
            #解析大模型返回结果
            msg_type, response = self.parse_message(reply_msg = reply_msg)
            print(f'[{format_date()}]: llm agent message parse result msg_type = {msg_type}, response = {response}')
        ...
        while len(self.result) == 0:
            # print(f'reply_result = {self.result[1]}')
            await asyncio.sleep(0.1)  # 每0.1s检查一次
            if time.time() - start_time > timeout:
                return fail_response('消息读取超时')
        # 流式响应
        if self.result_type == 'text/event-stream':
            return self.read_stream_new(content=self.result, aid=self.aid.id)
        return self.result

    def read_stream_new(self, content, aid):
        try:
            # 增加URL参数验证
            url = content+"&agent_id="+aid
            ...
            def generate():
                try:
                    ...
                    # 返回超时错误（SSE 格式）
                    error_msg = json.dumps({"error": "目标服务器响应超时"})  # 转换为合法 JSON
                    yield f"data: {error_msg}\n\n".encode('utf-8')  # 编码为字节流
                except requests.exceptions.RequestException as e:
                    # 返回其他请求错误（SSE 格式）
                    error_msg = json.dumps({"error": f"目标服务器请求失败: {str(e)}"})
                    yield f"data: {error_msg}\n\n".encode('utf-8')  # 编码为字节流
                except Exception as e:
                    # 捕获其他异常并返回错误（SSE 格式）
                    error_msg = json.dumps({"error": f"处理过程中发生错误: {str(e)}"})
                    yield f"data: {error_msg}\n\n".encode('utf-8')
            # 返回流式响应
            return Response(
                generate(),
            ...
            print(f'{format_date()}连接异常: {str(e)}')
