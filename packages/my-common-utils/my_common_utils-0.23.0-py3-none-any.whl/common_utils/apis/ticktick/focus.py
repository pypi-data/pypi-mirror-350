...
from common_utils.apis.helpers.time import get_timestamp_from_offset
...
class TickTickFocusTime(BaseModel):
    ...
    def _set_timezone_and_duration(self):
        ...
        tz = ZoneInfo(tz_name)
        self.startTime = self.startTime.replace(tzinfo=self.startTime.tzinfo or tz).astimezone(tz)
        self.endTime = self.endTime.replace(tzinfo=self.endTime.tzinfo or tz).astimezone(tz)
