...
@dataclass
class DriverAction:
    action: Literal["url", "sleep", "click", "send_keys", "get_text", "get_texts", "get_class", "get_classes", "get_href", "get_hrefs", "scroll_down"]
    ...
    def run_action(self, action: DriverAction, driver: webdriver.Chrome) -> str | list[str] | None:
        ...
        match action.action:
            ...
            case "send_keys":
                ...
                self.log_msg(f"Sent keys to {action.identifier}", level='debug')
            case "click":
            ...
            case "get_href":
                result = driver.find_element(By.CSS_SELECTOR, action.identifier).get_attribute(name='href')
                self.log_msg(f"Storing href: {result}", level='debug')
            case "get_hrefs":
                result = [element.get_attribute('href') for element in driver.find_elements(By.CSS_SELECTOR, action.identifier)]
                self.log_msg(f"Storing {len(result)} hrefs: {result}", level='debug')
            case "scroll_down":
                if action.identifier:
                    scroll_element = driver.find_element(By.CSS_SELECTOR, value=action.identifier)
                    driver.execute_script(f"arguments[0].scrollBy(0, {action.input});", scroll_element)
                else:
                    driver.execute_script(f"window.scrollBy(0, {action.input})")
            case _:
