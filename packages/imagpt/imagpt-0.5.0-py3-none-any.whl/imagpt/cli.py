...
def save_image(
    ...
    """Save image data to file with optional processing."""
    output_path.parent.mkdir(parents=True, exist_ok=True)

    # Apply image processing if any parameters are specified
    if rescale or any([crop_left, crop_right, crop_top, crop_bottom]):
        processed_data = process_image(
            image_data, rescale, crop_left, crop_top, crop_right, crop_bottom, output_format
        )
    else:
...
def generate(
    prompt: Annotated[
        Optional[str], typer.Argument(help="Direct prompt text for image generation")
    ] = None,
    dir: Annotated[
    ...
    rescale = rescale or config.default_rescale
    crop_left = crop_left if crop_left is not None else config.default_crop_left
    crop_right = crop_right if crop_right is not None else config.default_crop_right
    crop_top = crop_top if crop_top is not None else config.default_crop_top
    crop_bottom = crop_bottom if crop_bottom is not None else config.default_crop_bottom
    ...
    if dir:
        ...
        for prompt_file in prompt_files:
            try:
                ...
                if skip_existing and output_path.exists():
                    rprint(
                        f"⏭️  Skipping [dim]{prompt_file.name}[/dim] (image already exists)"
                    )
                    continue
        ...
        rprint(
            f"✅ Successfully generated [bold]{success_count}/{len(prompt_files)}[/bold] images"
        )
    ...
    # Handle direct prompt mode
        ...
        try:
            # Generate image
            image_data = generate_image(
                client, prompt, output_path.name, model, size, quality, style, format
            )
...
def version():
    ...
    rprint("🎨 [bold]imagpt[/bold] v0.5.0")
