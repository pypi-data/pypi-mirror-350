...
class MissingBlockError(Exception):
    ...
    def __str__(self):
        return f'Missing block "{self.block_name}" with index {self.index}'
...
class Block:
    ...
    def __init__(self, fp, parent=None):
        ...
        if self.Type[1:] != b"\x19\x00\x00\x00":
            raise ValueError(
                "Wrong block type ({Type}) found @{pos}".format(
                    pos=self.offset, Type=binascii.hexlify(self.Type[1:])
                )
            )
        if len(self.Type) < 5:
    ...
    def add_child(self, blk):
        ...
        if new_name:
            self.f.write(
                struct.pack(
                    "<I", struct.unpack("<I", self.value[:4])[0] - len(blk.name)
                )
            )
    ...
    def edit_child(self, old_block, new_block, debug=False):
        ...
        for i in range(self.head["N"]):
            ...
            if entry[6] == old_block.offset:
                ...
                break
        if not found:
            raise Exception(f"Child {old_block.name} not found in {self.name}")
        self.refresh()

    def create_dir(self, name, children=None, nums=100, assign=True, id=0):
        """
        ...
        if children is None:
            children = []
        size = 53 * nums
        value = struct.pack(f"<2IB6IQ{size - 41}x", size, nums, 0, 0, 0, 0, 0, 0, 0, 0)
    ...
    def edit_block(self, path, name, value, id=0, _type=0, force=False, debug=False):
        ...
        try:
            if do_debug(debug):
                print(f'Accessing block "{name}"[{id}]')
            ...
            if child.head["length1"] == len(value):
                ...
                blk = parent.create_block(
                    name, value, id=id, _type=_type, debug=dec_debug(debug)
                )
            ...
            return parent.add_child(
                parent.create_block(name, value, id=id, _type=_type)
            )
    ...
    def depth_first_search(
        self, callback=None, filter=lambda x: True, func=lambda x: x
    ):
        """
    ...
    def create_list(self, limit=None, debug=False):
        ...
        while True:
            ...
            if len(data) < 25:
                raise Exception(
                    "Children of {} @{} cannot be read. Data might be corrupted!".format(
                        self.name, offset
                    )
                )
            head = dict(
                ...
                if child.Type[0:1] == b"\x00":
                    ...
                    if len(child.value) % 2 == 0:
                        ...
                        if len(vS) > 20:
                            vS = vS[:20] + "..."
                        print(
                            '{name} ({id}) <{blen}> @{bidx}, value = {value} (hex) = "{vS}" (UTF-16)= {vL} ({Dtype}){other}'.format(
                                value=value, vL=vL, Dtype=Dtype, other=other, vS=vS, **l
                            )
                        )
                    else:
                        print(
                            "{name} ({id}) <{blen}> @{bidx}, value = {value} (hex) = {vL} ({Dtype}){other}".format(
                                value=value, vL=vL, Dtype=Dtype, other=other, **l
                            )
                        )
                else:
                    print(
                        "{name} ({id}) [{T}] <{blen}> @{bidx}".format(
                            T=child.Type[0], **l
                        )
                    )
                del child
    ...
    def get_bytes(self):
        ...
        """
        return struct.unpack(f"<{len(self.value)}B", self.value)
    ...
    def get_key_value(self, offset=16):
        ...
        int_value, float_value = struct.unpack(
            "<2xqd", self.value[offset + 4 + L : offset + 22 + L]
        )
        L2 = struct.unpack("<I", self.value[offset + 22 + L : offset + 26 + L])[0]
        SVal = self.value[offset + 26 + L : offset + 26 + L + L2].decode(
            "utf16", "ignore"
        )
                ...
                if digraph:
                    out.write(
                        '"{parent}-{name}" [label="{name}"]\n"{parent}" -> "{parent}-{name}"\n'.format(
                            parent=parent, name=l["name"].decode("utf8")
                        )
                    )
                else:
                    if ex is None:
    ...
    def rewrite(self, content, debug=False):
        ...
        if do_debug(debug):
            print(f'Rewriting block "{self.name}"')
        # set pointer at beginning of data
        self.f.seek(self.offset + 25 + self.head["name_length"])
        self.f.write(content)
        self.refresh()

    def modify_block_and_export(
        self, path, new_data, output, debug=False, prog=False, lazy=False
    ):
        assert not os.path.exists(
            output
        )  # Avoid to erase an existing file. Erase it outside the library if needed.
        out = open(output, "wb")
        ...
        while self.f.tell() < FILE_SIZE:
            ...
            try:
                current = Block(
                    self.f
                )  # Here we don't care about the parent argument. It is used only
                # for debug purpose anyway.
            ...
            curr_block_length = (
                current.head["length1"] + current.head["name_length"] + 25
            )
            debug_msg.append(
                f'Block Name: "{current.name}" / length: {curr_block_length}'
            )
            if current.offset == block_offset:  # Found the block to change
                ...
                out.write(
                    struct.pack(
                        "<5I",
                        block.head["name_length"],
                        block.head["ID"],
                        block.head["N"],
                        length_diff + block.head["length1"],
                        length_diff + block.head["length2"],
                    )
                )
                ...
                current.Type[0] in [1, 3]
            ):  # found a container, check for references to block after the modified block
                ...
                SubHeader = list(
                    struct.unpack("<2I5s5IQ", self.f.read(41))
                )  # read sub-block header
                if (
                    SubHeader[8] > block_offset
                ):  # Is the nextbloxk after the modified block? Yes => Adjust the offset position
                ...
                for _i in range(N):
                    ...
                    if (
                        bidx == block_offset
                    ):  # If the children block is the modified block, adjust length
                        blen = len(new_data)
                    elif (
                        bidx > block_offset
                    ):  # If the children is after the modifien block, adjust its offset
                        bidx += length_diff
                    out.write(
                        struct.pack("<B4I2Q", X, index, slen, id, Y, blen, bidx)
                    )  # write child info
                # Write the extra bytes used by iontof which seems to be useless as well as the childrens' name
                delta = curr_block_length - (
                    self.f.tell() - current.offset
                )  # number of bytes remaining till the end of the block
