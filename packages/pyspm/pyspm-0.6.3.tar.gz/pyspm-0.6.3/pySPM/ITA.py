...
class ITA(ITM):
    def __init__(self, filename, *args, **kargs):
        ...
        try:
            self.sx = self.root.goto(
                "filterdata/TofCorrection/ImageStack/Reduced Data/ImageStackScans/Image.XSize"
            ).get_ulong()
        ...
        try:
            ...
            self.Nimg = int(
                self.root.goto(
                    "filterdata/TofCorrection/ImageStack/Reduced Data"
                    "/ImageStackScans/Image.NumberOfImages"
                ).get_ulong()
            )
        except:
            self.Nimg = 0
    ...
    def get_channel_SN(self, channel):
        ...
        raise Exception(f'Channel name "{channel}" not found')
    ...
    def show_channels(self, ch):
        ...
        for z in ch:
            print(
                "\t{name} ({desc}), mass: {lower:.2f} - {upper:.2f}".format(
                    desc=z["desc"]["utf16"],
                    name=z["assign"]["utf16"],
                    lower=z["lmass"]["float"],
                    upper=z["umass"]["float"],
    ...
    def get_channel_by_mass(self, mass, full=False):
        ...
        for P in self.peaks:
            ...
            if (
                p["id"]["long"] > 1
                and p["lmass"]["float"] <= mass
                and mass <= p["umass"]["float"]
            ):
                if full:
                    return p
                return p["id"]["long"]
        raise ValueError(f"Mass {mass:.2f} Not Found")
    ...
    def get_shift_corrected_image_by_name(self, names, **kargs):
        ...
        return self.get_sum_image_by_name(
            names, shifts=[(-x, -y) for x, y in self.get_saved_shift()], **kargs
        )
    ...
    def get_sum_image_by_sn(self, SN, scans=None, prog=False, raw=False, **kargs):
        ...
        for s in scans:
            node = self.root.goto(
                f"filterdata/TofCorrection/ImageStack/Reduced Data/Images/{SN}/ScanData/EDROff/{s}"
            )
            dat = node.decompress()
            data = struct.unpack(f"<{len(dat) // 4}I", dat)
    ...
    def get_sum_image_by_name(
        self, names, scans=None, strict=False, prog=False, raw=False, **kargs
    ):
        """
    ...
    def get_shifts_by_mass(self, masses, centered=True, prog=False, Filter=None):
        ...
        for i in Y:
            ...
            Shift = np.real(
                np.fft.fftshift(np.fft.ifft2(np.conj(np.fft.fft2(S0)) * np.fft.fft2(S)))
            )
    ...
    def get_saved_shift(self):
        ...
        try:
            X = zlib.decompress(
                self.root.goto(
                    "filterdata/TofCorrection/ImageStack/Reduced Data"
                    "/ImageStackScans/ShiftCoordinates/ImageStack.ShiftCoordinates"
                ).value
            )
        except:
    ...
    def get_sum_image_by_mass(self, masses, scans=None, prog=False, raw=False, **kargs):
        ...
        if raw:
            return Z, channels
        channels_name = [
            ["{:.2f}u".format(m["cmass"]["float"]), m["assign"]["utf16"]][
                m["assign"]["utf16"] != ""
            ]
            for m in channels
        ]
    ...
    def get_added_image_by_SN(self, SN, raw=False):
        ...
        """
        node = self.root.goto(
            f"filterdata/TofCorrection/ImageStack/Reduced Data/Images/{SN}/SumImage/EDROff"
        )
        dat = node.decompress()
        data = struct.unpack(f"<{len(dat) // 4}I", dat)
    ...
    def get_added_image(self, channel, **kargs):
        ...
        V = np.array(
            struct.unpack("<" + str(self.sx * self.sy) + "I", D), dtype=np.float64
        ).reshape((self.sy, self.sx))
        return V
    ...
    @alias("getImage")
    def get_image(
        self, channel, scan, shifts=None, shift_mode="roll", const=0, **kargs
    ):
        """
    ...
    def get_opertion(self, OpID):
        ...
        """
        Nop = self.root.goto(
            "Presentation/Imaging Worksheet/Worksheet/OPERATIONS/OpCount"
        ).get_ulong()
        for i in range(Nop):
            blk = self.root.goto(
                f"Presentation/Imaging Worksheet/Worksheet/OPERATIONS/Operation[{i}]"
            )
        ...
        for i, it in enumerate(items):
            ...
            while OPTYPE != 3:
                ...
                if OPTYPE == 4:
                    ...
                    palette = np.array(
                        blk.goto_item("BMP-Palette").get_data("B")
                    ).reshape((256, 4))
                    ...
                    img = np.array(
                        blk.goto("Cache/IImage-Cache-Intensities").get_data("d")
                    ).reshape((dimy, dimx))
    ...
    def add_new_images(self, miblock, scans=None, added=None, prog=False, **kargs):
        ...
        self.root.edit_block(
            f"filterdata/TofCorrection/ImageStack/Reduced Data/ImageStackScansAdded/Image[{AN}]",
            "Image.MassIntervalSN",
            SN.encode("utf8"),
        )
        self.root.edit_block(
            f"filterdata/TofCorrection/ImageStack/Reduced Data/ImageStackScansAdded/Image[{AN}]",
        ...
        if scans is not None:
            ...
            for i in RS:
                ...
                data = zlib.compress(
                    struct.pack(f"<{sx * sy}I", *np.ravel(img)), level=lvl
                )
                self.root.edit_block(
                    f"filterdata/TofCorrection/ImageStack/Reduced Data/ImageStackScans/Image[{N}]",
                    "ImageArray.Long",
                    data,
                    id=i,
                    _type=128,
...
class ITA_collection(Collection):
    ...
    def __init__(
        ...
        Collection.__init__(
            self,
            sx=self.ita.fov,
            sy=self.ita.fov * self.ita.sy / self.ita.sx,
            unit="m",
            name=name,
        )
        self.msg = ""
        ...
        for channels in CHS:
            ...
            if isinstance(channels, list):
                ...
                raise TypeError(
                    f"Channels should be a list or a dictionnary. Got {type(channels)}"
                )
    ...
    def stitch_correction(self, channel, stitches, gauss=0, debug=False):
        ...
        for i in range(stitches[0]):
            for j in range(stitches[1]):
                S += self.channels[channel].pixels[
