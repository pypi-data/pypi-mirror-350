...
class PCA:
    ...
    def standardized(self, meanCentering=True):
        self.standX = pd.DataFrame(
            scale(self.data, with_mean=meanCentering),
    ...
    def pca_summary(self):
        if self.pca is None:
            self.run_pca()
        names = [
            "PC" + str(i) for i in range(1, len(self.pca.explained_variance_ratio_) + 1)
        ]
        a = list(np.std(self.pca.transform(self.standX), axis=0))
        b = list(self.pca.explained_variance_ratio_)
        c = [
            np.sum(self.pca.explained_variance_ratio_[:i])
    ...
    def loadings(self, id=None):
        ...
        if id is not None:
            return pd.DataFrame(
                self.pca.components_[id, None], columns=self.data.columns
            )
        return pd.DataFrame(
    ...
    def show_stand(self):
        return pd.DataFrame(
            [self.standX.apply(np.mean), self.standX.apply(np.std)],
            index=["Mean", "Std"],
        )
...
class ITA_PCA(PCA):
        ...
        **kargs,
        ...
        if loadings:
            ...
            gs = GridSpec(
                Ny,
                Nx,
                height_ratios=[width / Nx for i in range(Ny - 1)]
                + [width * N / len(L.columns)],
            )
    ...
    def get_pca_col(self, num=None, pn=False):
        ...
        for i in range(num):
            PC = self.get_pca(i)
            if pn:
                PCA_col.add(PC * (PC >= 0), f"PC{i + 1}+")
                PCA_col.add(-PC * (PC <= 0), f"PC{i + 1}-")
            else:
                PCA_col.add(PC, f"PC{i + 1}")
        return PCA_col
