...
class ITAX:
        ...
        for x in self.root.goto("CommonDataObjects/Attachments"):
            for y in x.getList():
                if y["name"] == "Video Snapshot":
                    ...
                    I = np.flipud(
                        np.array(
                            struct.unpack("<" + str(3 * sx * sy) + "B", data)
                        ).reshape((sy, sx, 3))
                    )
                    snapshots.append(I)
                    del blk
    ...
    def getSpectrum(self, sf=None, k0=None, time=False, **kargs):
        ...
        """
        slen = self.root.goto(
            "CommonDataObjects/DataViewCollection/*/sizeSpectrum"
        ).getLong()
        raw = self.root.goto(
        ...
        for x in self.root.goto("CommonDataObjects/MeasurementOptions/*/massintervals"):
            if x.name == "mi":
                ...
                if lab == name:
                    ...
                    break
        if SN is None:
            raise Exception(f'Profile "{name}" not found')
        path = f"CommonDataObjects/DataViewCollection/*/dataSource/simsDataCache/{SN}/profile"
