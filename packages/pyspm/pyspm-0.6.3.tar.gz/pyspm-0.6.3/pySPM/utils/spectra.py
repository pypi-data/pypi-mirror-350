...
from .misc import alias, dec_debug, do_debug
...
def show_peak(
    ...
    **kargs,
    ...
    if delta is None:
        ...
        upper_mass = m0 + delta
    if do_debug(debug):
        print(f"Lower mass: {lower_mass:.2f}\nUpper mass: {upper_mass:.2f}")
    ...
    if show_elts:
        if include_only is not None:
            ...
            E = [
                x
                for NM in range(int(round(lower_mass)), int(round(upper_mass)) + 1)
    ...
    if dofit or fakefit:
        ...
        if len(E) > 0:
            ...
            )  # which element is the closest to max_peak
            if dm0 is None:
                dm = mp - get_mass(E[i])
        p0 = [kargs.get("asym0", 1), dm] + [0, 0] * len(
            E
        )  # delta m is estimated from the deviation of the highest peak
        ...
        if dofit:
            try:
                ...
                popt, pcov = curve_fit(
                    ...
                    p0=p0,
                    bounds=(
                        [1 / kargs.get("asym_max", 10), -0.015]
                        + [0, 0] * ((len(p0) - 1) // 2),
                        [kargs.get("asym_max", 10), 0.015]
                        + [kargs.get("sig_max", 0.01), np.inf] * ((len(p0) - 1) // 2),
                    ),
                )
                fit_type = 0
            except:
                ...
                try:
                    ...
                    popt, pcov = curve_fit(
                        ...
                        p0=p0,
                        bounds=(
                            [1 / kargs.get("asym_max", 4), -0.015]
                            + [0, 0] * ((len(p0) - 1) // 2),
                            [kargs.get("asym_max", 4), 0.015]
                            + [kargs.get("sig_max", 0.01), np.inf]
                            * ((len(p0) - 1) // 2),
                        ),
                    )
                    fit_type = 1
        ...
        for i in range((len(popt) - 1) // 2):
            Y = LG(
                m[mask], m0s[i], popt[2 + 2 * i], popt[3 + 2 * i], lg=0, asym=popt[0]
            )
            Area = (
                popt[2 * i + 3]
                * popt[2 * i + 2]
    ...
    if ax is not None:
        if label is None:
            p = ax.plot(
                m[mask] - popt[1], D[mask], color=kargs.get("curve_color", None)
            )
        else:
            p = ax.plot(
                m[mask] - popt[1],
                D[mask],
                label=label,
                color=kargs.get("curve_color", None),
            )
        ...
        if dofit and ax is not None:
            ...
            if pretty:
                put_Xlabels(
                    ax,
                    m0s,
                    [f"{E}: {res[E]['Area']:.2f}" for E in res],
                    color=colors,
                    debug=dec_debug(debug),
                    **kargs,
                )
...
def plot_isotopes(
    ...
    **kargs,
    ...
    from . import LG, Molecule, get_abund, get_isotopes, get_mass
