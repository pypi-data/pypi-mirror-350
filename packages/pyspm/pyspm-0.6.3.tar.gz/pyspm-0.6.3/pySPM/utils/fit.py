...
def peak_fit(m, s, m0, delta=0.02):
    ...
    p0, _ = curve_fit(
        ...
        s[mask],
        p00,
        bounds=((-np.inf, 0, 0, 0, 0), (np.inf, np.inf, np.inf, 1, np.inf)),
    )
    return p0
...
def CDF_fit(x, y, p0, dic=False):
    ...
    n = (len(p0) - 1) // 3
    bounds = [[0] + [-np.inf, -np.inf, 0] * n, [np.inf] + [np.inf, np.inf, np.inf] * n]
...
def LG2Da(A, Rweight=None, sigma=None, dic=False, **kargs):
    ...
    p0_def = [Amplitude, 0, 10, 10, 10, 10, Center[1], Center[0], 0, 0, 0, 0, 0]
    bounds_def = [
