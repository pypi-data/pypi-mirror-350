...
from .math import clip01, positify, strictly_positify
...
def _rl(x, image, psf, type="default", extend=True, damping=0, ndamp=10):
    ...
    """
    I = strictly_positify(
        convolve(x, psf, type=type, extend=extend)
    )  # reconvoluted estimation.
    if damping != 0:
        ratio = _rl_damped(I, image, damping=damping, ndamp=ndamp)
    else:
        ratio = image / I
    return x * convolve(
        ratio, psf[::-1, ::-1], type=type, extend=extend
    )  # Correlation is the convolution of mirrored psf
...
def convolve(
    img, psf, type="default", extend=True, mode="same", extend_margin=100, **kargs
):
    """
