...
def smiley(
    ...
    smiley = (ratio > R) * ((ratio - thick) < R)
    smiley += (
        np.sqrt(
            (X - eye_sep * ratio * size / 2) ** 2
            + (Y + eye_height * ratio * size / 2) ** 2
        )
        < ratio * eye * size / 2
    )
    smiley += (
    ...
    smiley += (
        (ratio * mouth_rad > R)
        * ((ratio * mouth_rad - ratio * mouth_thick) < R)
        * (Y > 0)
    )
    return smiley * 1.0
...
class deprecated:
    ...
    def __call__(self, func):
        ...
        msg = "the function {} is DEPRECATED. Please use {} instead".format(
...
def getBAM(x, x0, N=10, least_one=False):
    ...
    for pos, width in lines:
        ...
        if least_one:
            P[np.argmin(abs(x - x0 - pos))] = 1  # At least 1 pixel set ?
    return P
