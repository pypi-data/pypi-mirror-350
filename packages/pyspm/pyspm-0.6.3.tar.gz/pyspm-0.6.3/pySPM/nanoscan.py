...
def get_curve(filename, channel="Normal Deflection", backward=False):
    ...
    RAW = root.findall(
        "spm:vector/spm:contents/spm:direction/spm:vector/"
        "spm:contents/spm:name[spm:v='{direction}']/../spm:channel/spm:vector/"
        "spm:contents/spm:name[spm:v='{channel}']/../spm:data/spm:v".format(
            direction=["forward", "backward"][backward], channel=channel
        ),
        namespace,
    )[0].text
    start = float(
        root.findall(
            "spm:vector/spm:contents/spm:axis/spm:vector/"
            "spm:contents/spm:start/spm:vector/spm:v",
            namespace,
        )[0].text
    )
    stop = float(
        root.findall(
            "spm:vector/spm:contents/spm:axis/spm:vector/"
            "spm:contents/spm:stop/spm:vector/spm:v",
            namespace,
        )[0].text
    )
    root.findall(
...
class Nanoscan:
    def __init__(self, filename=None):
        ...
        if self.root.tag == "{http://www.nanoscan.ch/SPM}scan":
            ...
            self.size = {"unit": x["unit"], "x": x["value"], "y": y["value"]}
            try:
                self.feedback = {
                    "channel": self.__grab(
                        f"{self.fbPath}/spm:z_feedback_channel/spm:v"
                    )
                }
                self.feedback["P"] = {
                    "value": float(
                        self.__grab(f"{self.fbPath}/spm:proportional_z_gain/spm:v")
                    ),
                    "unit": self.__grab(
                        f"{self.fbPath}/spm:proportional_z_gain_unit/spm:v"
                    ),
                }
                self.feedback["I"] = {
                    "value": float(
                        self.__grab(f"{self.fbPath}/spm:integral_z_time/spm:v")
                    ),
                    "unit": self.__grab(
                        f"{self.fbPath}/spm:integral_z_time_unit/spm:v"
                    ),
                }
            ...
            self.scan_speed = {
                z: {
                    "value": float(
                        self.__grab(
                            f"spm:vector//spm:direction/spm:vector/spm:contents/spm:name[spm:v='{z}']/../spm:point_interval/spm:v"
                        )
                    )
                    * self.pixel_size[0],
                    "unit": self.__grab(
                        f"spm:vector//spm:direction/spm:vector/spm:contents/spm:name[spm:v='{z}']/../spm:point_interval_unit/spm:v"
                    ),
                }
                for z in ["forward", "backward"]
            }
        else:
            raise TypeError(
    ...
    def list_channels(self):
        ...
        for d in ["forward", "backward"]:
            ...
            for z in self.root.findall(
                f"spm:vector//spm:direction/spm:vector/spm:contents/spm:name[spm:v='{d}']/../spm:channel//spm:contents/spm:name/spm:v",
                self.namespaces,
            ):
                print("  - " + z.text)
            print()
    ...
    def get_channel(self, channel="Topography", backward=False, corr=None):
        ...
        return SPM_image(
            image_array, channel=channel, _type=self.type, real=recorded_size, corr=corr
        )
