...
class Collection:
    ...
    def add(self, Img, name, force=False):
        ...
        if name in self.channels:
            if force:
                ...
                raise KeyError(
                    "The channel {} is already present in "
        ...
        if ax is None:
            if channels_number == 4 and ncols == 3:
                fig, ax = plt.subplots(
                    ...
                    figsize=(
                        width,
                        self[channels[0]].pixels.shape[0]
                        * width
    ...
    def overlay(
        ...
        **kargs,
        ...
        data = [
            SPM.normalize(self[ch].pixels, sig=sig, vmin=vmin, vmax=vmax)
            for ch in channel_names
        ]
        layers = [
            np.stack([data[i] * colors[i][j] for j in range(3)], axis=2)
            for i in range(len(channel_names))
        ]
        overlay = np.clip(np.sum(layers, axis=0), 0, 1)
        o = self.create_image(overlay, key="overlay")
        ch = [
            self.create_image(x, key=self[channel_names[i]].channel)
            ...
            for i in range(stitches[0]):
                for j in range(stitches[1]):
                    F[sy * i : sy * (i + 1), sx * j : sx * (j + 1)] = (
                        self.channels[x][sy * i : sy * (i + 1), sx * j : sx * (j + 1)]
                        / S
                    )
            result.add(F, x)
        return result
...
def overlay_triangle(
    ...
    centers = [
        (
            radius * proportion * np.cos(np.radians(120 * i)),
            radius * proportion * np.sin(np.radians(120 * i)),
        )
        for i in range(3)
    ]
    dist = [
        np.sqrt((X - centers[i][0]) ** 2 + (Y - centers[i][1]) ** 2) for i in range(3)
    ]

    for j in range(3):
        RGB[j] = sum(
            [
                colors[i][j] * np.maximum((distance - dist[i]) / distance, 0)
                for i in range(3)
            ]
        )
        ax.annotate(
            channel_names[j],
            (
                radius * np.cos(np.radians(120 * j)),
                radius * np.sin(np.radians(120 * j)),
            ),
            color=textcolor,
            fontsize=fontsize,
            va="center",
