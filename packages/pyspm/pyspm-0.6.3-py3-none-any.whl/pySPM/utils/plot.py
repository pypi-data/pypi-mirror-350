...
def Xdist(
    ...
    **kargs: additional arguments.
    ...
    ax.annotate(
        ...
        ({"center": 0.5 * (left + right), "left": right, "right": left}[ann["ha"]], y),
        offset,
...
def Ydist(
    ...
    **kargs: additional arguments.
    ...
    ax.annotate(
        ...
        (x, {"center": 0.5 * (down + up), "top": up, "bottom": down}[ann["va"]]),
        offset,
        textcoords="offset pixels",
        rotation=rotation,
        **ann,
...
def put_Xlabels(
    ...
    for i, (x, lab) in enumerate(P):
        ...
        labs.append(
            ax.text(pos[0], pos[1], lab, va="bottom", ha="left", rotation=90, color=col)
        )
        ax.axvline(
            x,
            color=col,
            linestyle=kargs.get("linestyle", "-"),
            alpha=kargs.get("line_alpha", 0.5),
        )
        ax.draw(renderer)  # make sure to draw the new object
        ...
        if save:
            if ov:
                ...
                ax.add_patch(coli[0])
                ax.add_patch(coli[1])
            fig.savefig(f"put_Xlabels_{i:03d}_000.png")
            if ov:
                coli[0].remove()
                coli[1].remove()
        last_ov = ov
        ...
        while ov:
            it += 1
            if debug:
                print(f'Label "{labs[-1].get_text()}" overlap with {ov!r}')
            if type(ov) is mpl.lines.Line2D:
                new_y = _get_yextend_of_curve(ax, r, ov)[1]
            else:
                rov = ov.get_window_extent(renderer).transformed(
                    ax.transData.inverted()
                )
                new_y = rov.ymax
            if ov == last_ov:
                y_offset += kargs.get("offset_step", 5)
                if y_offset > kargs.get("offset_step", 5) * kargs.get(
                    "max_iteration", 6
                ):
                    break
            else:
            ...
            new_xy = ax.transData.inverted().transform(
                new_xy_px
            )  # Offset the new_y of 5 pixels
            labs[-1].set_position(new_xy)
            ax.draw(renderer)
            ov = _overlap(ax, labs[-1], objs + labs[:-1], debug=debug)
            if save:
                ax.draw(renderer)
                fig.savefig(f"put_Xlabels_{i:03d}_{it:03d}.png")
                if ov:
                    r2 = ov.get_window_extent(
                        renderer
                    )  # get the Bbox of the last label
