...
import numpy as np
from PyQt5.QtCore import QDir, QFileInfo, QSettings, Qt
...
class SpectraViewer(QMainWindow):
    ...
    def plot_labels(self, colors=None):
        ...
        for i, (mi, Ei) in enumerate(P):
            dmi = (
                ...
                * np.sqrt(mi)
                * np.sqrt(
                    (self.dk0**2 / (self.sf**2)) + mi * (self.dsf**2 / (self.sf**2))
                )
            )
            col = colors[i % len(colors)]
            self.lab_lines.append(self.ax.axvline(mi, color=col, alpha=0.5))
            self.labels.append(
                self.ax.annotate(
                    Ei,
                    (mi, y),
            ...
            if dmi > 0:
                self.lab_lines.append(
                    self.ax.axvline(mi + dmi, color=col, alpha=0.5, ls=":")
                )
                self.lab_lines.append(
                    self.ax.axvline(mi - dmi, color=col, alpha=0.5, ls=":")
                )
    ...
    def yAxisScaleChanged(self):
        ...
        if self.ita is not None:
            SatLevel = (
                self.ita.size["pixels"]["x"]
                * self.ita.size["pixels"]["y"]
                * self.ita.Nscan
            )
        ...
        dm = (
            2
            * np.sqrt(m0)
            * np.sqrt((self.dk0**2 / (self.sf**2)) + m0 * (self.dsf**2 / (self.sf**2)))
        )
        self.ui.lab_m0.setText(f"m0 = {m0:.5f} ± {dm:.5f}")
    ...
    def DoMassCal(self):
        ...
        for i in range(len(self.MassCal)):
            ...
            self.ui.tableMassCal.setItem(
                i, 2, QTableWidgetItem("{:.0f}".format(self.MassCal[i]["time"]))
            )
