...
from .engine_config import Acceleration, AccelerationWithConfig, Architecture
...
class DeploymentConfigBuilder(object):
    ...
    def sidekick_gpus(
        ...
        key = DeploymentConfigBuilder._get_gpu_resource(Acceleration(accel))
    ...
    def sidekick_accel(
        ...
        config = self._guarantee_sidekick_stanza(model_version)
        old_accel = Acceleration(config.get("accel", Acceleration.default()))
    ...
    def _adjust_gpu_resources(
        component: Dict[str, Any], new_accel: Acceleration, old_accel: Acceleration
    ) -> None:
    ...
    @staticmethod
    def _get_gpu_resource(accel: Acceleration) -> str:
        match accel:
            case Acceleration.OpenVINO:
                return "gpu.intel.com/i915"
            case Acceleration.QAIC:
                return "qaic.qualcomm.com/device"
            case _:
    ...
    def _clear_gpu_resources(component: Dict[str, Any]):
        ...
        resources["limits"].pop("qaic.qualcomm.com/device", None)
        resources["requests"].pop("gpu.intel.com/i915", None)
        resources["requests"].pop("nvidia.com/gpu", None)
        resources["requests"].pop("qaic.qualcomm.com/device", None)
