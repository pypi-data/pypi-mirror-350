...
@responses.activate
def test_sidekick_options(test_client, now):
    barnacle_boy_data = {
        "data": {
            "model_by_pk": {
                "id": 1,
...
@responses.activate
def test_sidekick_deployment_label(test_client, now):
    barnacle_boy_data = {
        "data": {
            "model_by_pk": {
                "id": 1,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (Acceleration.QAIC, "qaic.qualcomm.com/device"),
...
def test_engine_acceleration(builder, new_accel, expected_gpu_resource):
    ...
    gpu_resources = ("nvidia.com/gpu", "qaic.qualcomm.com/device", "gpu.intel.com/i915")
...
def test_sidekick_acceleration(test_client, now, new_accel, expected_gpu_resource):
    model_data = {
        "data": {
            "model_by_pk": {
                "id": 1,
