...
class DisplayEmbryo:
    ...
    def _init_data(self):
        ...
        try:
            ...
            logging.warning("Column 'orig.ident' not found, using 'slices' instead.")
            self.all_layers = self.embryo.anndata.obs['slices'].unique().tolist()
        self.all_layers = sorted(self.all_layers)

        try:
            self.all_germ_layers = self.embryo.anndata.obs['germ_layer'].unique().tolist()
        except KeyError:
            print("Column 'germ_layer' not found,using 'heart_regions' instead.")
            self.all_germ_layers = self.embryo.anndata.obs['heart_regions'].unique().tolist()
    ...
    def _create_dock(self):
        ...
        print(f"[DEBUG] Dock可见性: {self.dock.dockWidget.isVisible()}")
        print(f"[DEBUG] 容器尺寸: {container.size().width()}x{container.size().height()}")
        print(f"[DEBUG] 标签页数量: {main_tab.count()}")
    ...
    def _on_dock_closed(self):
        ...
        self.dock_widget = None
        # 注册重建回调
        self.viewer.window._dock_widgets['动态控制台'].dockWidget.visibilityChanged.connect(
