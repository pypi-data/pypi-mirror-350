...
class NGAppManager(DesktopApp, metaclass=ScopedSingletonMeta):
    ...
    def set_text(self, field: UIAWrapper, text: str, max_retries: int = 4) -> None:
        """Set a text in a specific field and double check if the action was performed corretly, retry if not.

        Args:
            field (UIAWrapper): Field where the text shoud be set.
    ...
    def toggle_checkbox(self, checkbox: UIAWrapper, max_retries: int = 4) -> None:
        """Toggle a checkbox and double check if the action was performed corretly, retry if not.

        Args:
            checkbox (UIAWrapper): Checkbox element that should be toggled.
            max_retries (int, optional): Number of max retries attempt. Defaults to 4.
        """
        retries = 0
        while retries < max_retries:
    ...
    def untoggle_checkbox(self, checkbox: UIAWrapper, max_retries: int = 4) -> None:
        """Untoggle a checkbox and double check if the action was performed corretly, retry if not.

        Args:
            checkbox (UIAWrapper): Checkbox element that should be untoggled.
            max_retries (int, optional): Number of max retries attempt. Defaults to 4.
        """
        retries = 0
        while retries < max_retries:
