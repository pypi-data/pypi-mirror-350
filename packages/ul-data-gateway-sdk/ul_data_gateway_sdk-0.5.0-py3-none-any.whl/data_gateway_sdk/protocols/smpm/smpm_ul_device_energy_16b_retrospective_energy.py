...
class SmpmUlDeviceEnergy16BRetrospectiveEnergyData(Packet):
    ...
    def to_integration_data(self, received_at: datetime, device_tz: tzinfo, **kwargs: Any) -> List[IntegrationV0MessageData]:
        ...
        if self.packet_type_id_enum in {400, 401, 402, 403, 404, 405, 406, 407, 408}:
            ...
            dt_value_current = first_day_of_month(days_ago_calculation(received_at, device_tz, time(0), period_ago - delta)) if self.period_ago != 0 else received_at
            dt_value_previous_1_delta = first_day_of_month(days_ago_calculation(received_at, device_tz, time(0), period_ago))
            dt_value_previous_2_delta = first_day_of_month(days_ago_calculation(received_at, device_tz, time(0), period_ago + delta))
            dt_value_previous_3_delta = first_day_of_month(days_ago_calculation(received_at, device_tz, time(0), period_ago + delta * 2))

        integration_message_data = []
        # adding value_current with checking validity
        if self.is_valid and self.value_current != 0:
