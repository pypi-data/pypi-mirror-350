...
def log_writer():
    ...
    Log file format:
    USER_COMPUTER;sequence_number;user_name;as
    """
    _log_sequence = 0
...
def workerpool():
    """
    Handle HTTP requests for the '/api/workerpool' endpoint supporting GET, POST, and PATCH methods.
    ...
    - PATCH: Calls patch_workerpool(request) to partially update workerpool data.
...
def patch_workerpool(request):
    """
    Handles PATCH requests to the worker pool endpoint.
    
    Authenticates the incoming request and returns an unauthorized error if authentication fails.
    If authentication succeeds, responds with a 400 Bad Request status.
    
    Args:
        request (flask.Request): The incoming HTTP request object.
    
    Returns:
        flask.Response: A JSON response with an error message and 401 status if unauthorized,
                        otherwise an empty response with a 400 status.
    """
    if not authenticate(request):
        return jsonify({'error': 'unauthorized'}), 401
