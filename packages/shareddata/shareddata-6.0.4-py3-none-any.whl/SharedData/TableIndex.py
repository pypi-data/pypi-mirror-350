...
class TableIndex:        
    ...
    def malloc(self):
        """
        Allocates memory and initializes indices based on the primary key string.
        
        This method retrieves the shared memory name from the associated table and performs
        memory allocation for the primary key. Depending on the contents of the primary key string,
        it conditionally allocates additional indices:
        - If 'date' is present in the primary key string, allocates a date index.
        - If 'symbol' is present and there is more than one primary key column, allocates a symbol index.
        - If either 'portfolio' or 'tag' is present, allocates a portfolio index.
        """
    ...
    def malloc_symbolidx(self, shm_name):
        """
        Allocate and initialize symbol index structures in shared memory or memory-mapped file based on the table type.
        
        For table type 2, allocate shared memory segments for symbol indices with appropriate sizes. If the shared memory segment does not exist, create it and mark the index as not created.
        
        For table type 1, manage a memory-mapped file for symbol indices. If the file does not exist or its size is incorrect, create or reset the file and mark the index as not created.
        
        Initialize two numpy arrays, `symbollastidx` and `symbolprevidx`, which represent hash table and list indices respectively. If the index is newly created or reset, initialize all entries to -1.
        
        Parameters:
            shm_name (str): Base name for the shared memory or file used for symbol indices.
        
        Side Effects:
            - Creates or resets shared memory or memory-mapped files.
    ...
    def create_index(self,start=0,update=False):
        ...
        if self.table.records.count > 0:
            ...
            if 'date' in self.pkeystr:                
                vidx = (arr['date']>=self.table.mindate) & (arr['date']<self.table.maxdate)
                if not np.all(vidx):
                    Logger.log.warning('Records outside date range found in %s!' % self.table.relpath)
                    lenvalid = np.sum(vidx)
                    arr[:lenvalid] = arr[vidx]
    ...
    def update_index(self, start):                
        """
        Updates the index starting from the given position.
        
        Parameters:
        start (int): The starting position from which to update the index.
        
        This method calls the create_index method with the update flag set to True,
        indicating that the index should be updated rather than created anew.
        """
    ...
    def sort_index(self, shnumpy, start=0):
        ...
        """
        Sorts the entries in `shnumpy` starting from the given index `start` based on the primary key columns in reverse order.
        
        This method acquires a lock on `self.table` to ensure thread safety during sorting. It uses `np.lexsort` to obtain the sorted indices and checks if the current order differs from the sorted order. If differences are found, it updates `shnumpy` by reordering the entries, updates the minimum change ID (`minchgid`), and refreshes the index accordingly. The method also updates header flags to indicate the sorting and indexing status.
        
        Parameters:
            shnumpy (structured numpy array): The data array to be sorted.
            start (int, optional): The starting index from which to sort. Defaults to 0.
    ...
    def create_file(self,fpath,size):
        ...
        This function opens a file in binary write mode, seeks to the byte position size-1,
    ...
    def flush(self):        
        """
        Flushes memory-mapped arrays associated with the primary key and updates their file modification times.
        
        If the primary key size is greater than one, this method flushes the memory-mapped arrays (`pkey`, `datelastidx`, `dateprevidx`, `symbollastidx`, `symbolprevidx`, `portlastidx`, `portprevidx`) depending on the presence of specific substrings ('date', 'symbol', 'portfolio') in the primary key string (`pkeystr`). After flushing, it updates the file modification times of the corresponding files to match the modification time stored in the table header.
        
        This ensures that all changes to the memory-mapped arrays are written to disk and that the file timestamps are consistent with the table's metadata.
        """
        if self.pkey.size>1:
    ...
    def get_pkeycolumns(database):    
        """
        Retrieve the primary key columns for a given database.
        
        Parameters:
            database (str): The name of the database to retrieve primary key columns for.
        
        Returns:
            list: A list of primary key column names associated with the specified database.
        
        Raises:
            Exception: If the specified database is not implemented in DATABASE_PKEYS.
        """
