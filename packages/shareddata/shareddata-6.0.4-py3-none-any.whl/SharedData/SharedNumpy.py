...
class _LocIndexer:
    ...
    def __getitem__(self, index):
        ...
        if isinstance(index, tuple):
            ...
            # Fast path: tbl.loc[:, ['stock1','stock2']]
            if (
                len(index) == 2 and
                isinstance(row_index, (type(None), slice)) and
                isinstance(col_index, (list, np.ndarray, pd.Index)) and
                len(pkeycolumns) >= 2 and
                pkeycolumns[1] in {'symbol', 'tag', 'portfolio'}
            ):
        ...
        # Generalized: handle string keys (symbol, tag, portfolio, etc)
            ...
            if pkeycolumns:
                ...
                if primary_string_keys:
                    ...
                    if method is not None:
                        # If date is the first pkey and not supplied, filter from mindate onward for the secondary key
                        if pkeycolumns[0] == "date":
                            mindate = self.data.table.mindate
                            locs = self.data.get_date_loc_gte(mindate)
                            key_locs = method(index)
                            # Intersect to keep date ordering
                            ...
                            # Just use the secondary method (reverse for backwards link)
                            locs = method(index)
                            return self.data[locs] if len(locs) else np.array([], dtype=self.data.dtype)

        row_index, col_index = self._parse_index(index)

        # --- 1. Fast Multi-key/tuple/slice access with boolean masks and column-list support ---
        pkeycolumns = getattr(self.data.table.index, "pkeycolumns", [])
        from SharedData.Database import STRING_FIELDS

        if isinstance(index, tuple) and (1 <= len(index) <= len(pkeycolumns) + 1):
            ...
            for val, pkey in zip(row_keys, pkeycolumns):
                ...
                if pkey == "date":
                    if isinstance(val, slice):
                        ...
                        if stop is not None:
                            stop = pd.Timestamp(stop) if isinstance(stop, str) else stop
                            stop = np.datetime64(stop, 'ns')
                        else:
                            stop = None
                        if start is not None and stop is not None:
                            mask &= (records['date'] >= start) & (records['date'] <= stop)
                        elif start is not None:
                            mask &= (records['date'] >= start)
                        elif stop is not None:
                            mask &= (records['date'] <= stop)
                    elif val is not None:
                        dval = pd.Timestamp(val) if isinstance(val, str) else val
                        dval = np.datetime64(dval, 'ns')
                        mask &= (records['date'] == dval)
                # -- STRING LIST/ARRAY logic (new/modified for any string field) --
                elif pkey in STRING_FIELDS:
                    if isinstance(val, slice) or val is None:
                        continue
                    # If val is a list/array/pd.Index: match ANY (np.isin)
                    if isinstance(val, (list, np.ndarray, pd.Index)):
                        # Use bytes if dtype matches (for symbol columns)
                        arr_val = np.asarray(val)
                        if records[pkey].dtype.kind in {"S", "O"}:  # fixed-width or object string fields
                            arr_val = np.array([v.encode('utf-8') if isinstance(v, str) else v for v in arr_val])
                        mask &= np.isin(records[pkey], arr_val)
                    # Scalar string match
                    elif isinstance(val, (str, bytes)):
                        target = val.encode('utf-8') if isinstance(val, str) else val
                        mask &= (records[pkey] == target)
                    else:
                        continue
                # -- Other (unchanged) --
                else:
        ...
        # 5. Timestamp row index (date lookup)
