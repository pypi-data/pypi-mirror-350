...
class NoConfigApp(Application):
    def __init__(self):
        super().__init__("no_usage", prog="gumicorn_test")
...
def test_load_enviroment_variables_config(monkeypatch):
    monkeypatch.setenv("GUMICORN_CMD_ARGS", "--workers=4")
    with AltArgs():
        app = NoConfigApp()
    assert app.cfg.workers == 4

def test_config_file_environment_variable(monkeypatch):
    monkeypatch.setenv("GUMICORN_CMD_ARGS", "--config=" + alt_cfg_file())
    with AltArgs():
        app = NoConfigApp()
...
def test_invalid_enviroment_variables_config(monkeypatch, capsys):
    monkeypatch.setenv("GUMICORN_CMD_ARGS", "--foo=bar")
    with AltArgs():
        with pytest.raises(SystemExit):
            NoConfigApp()
        _, err = capsys.readouterr()
