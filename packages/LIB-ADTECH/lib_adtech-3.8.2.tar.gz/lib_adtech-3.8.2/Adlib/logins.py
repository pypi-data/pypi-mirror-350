...
from Adlib.utils import chatIdMapping, aguardarTempo, tokenBotLogin, chatIdUsuarios
from Adlib.funcoes import setupDriver, esperarElemento, mensagemTelegram, aguardarAlert, clickarElemento, \
                          enviarCaptcha, saveCaptchaImage, solveReCaptcha, coletarEmailEspecifico, getCredenciais
from selenium.webdriver import Chrome
from selenium.webdriver.common.keys import Keys


chatId = chatIdUsuarios
            ...
            try:
                ...
                match loginReturn:
                    case LoginReturn.LOGIN_COM_SUCESSO:
                        mensagemTelegram(tokenBotLogin, chatId, f"Login com sucesso! {formatEnumName(enumBanco)} {formatEnumName(enumProcesso)} ✅")
                    case LoginReturn.RESETAR_SENHA:
                        mensagemTelegram(tokenBotLogin, chatId, f"Resetar a senha! {formatEnumName(enumBanco)} {formatEnumName(enumProcesso)} ⚠️")
                    case LoginReturn.CREDENCIAIS_INVALIDAS:
                        mensagemTelegram(tokenBotLogin, chatId, f"Credenciais inválidas! {formatEnumName(enumBanco)} {formatEnumName(enumProcesso)} ❌")
                    case LoginReturn.USUARIO_INATIVO:
                        mensagemTelegram(tokenBotLogin, chatId, f"Usuário inativo! {formatEnumName(enumBanco)} {formatEnumName(enumProcesso)} ❌")
                    case LoginReturn.ATUALIZAR_DADOS:
                        mensagemTelegram(tokenBotLogin, chatId, f"Atualizar dados cadastrais! {formatEnumName(enumBanco)} {formatEnumName(enumProcesso)} ❌")
                    case LoginReturn.ERRO_AO_LOGAR:
                        mensagemTelegram(tokenBotLogin, chatId, f"Erro ao fazer login! {formatEnumName(enumBanco)} {formatEnumName(enumProcesso)} ❌")
        ...
        while True:
            ...
            if loginReturn != LoginReturn.CAPTCHA_INCORRETO:
                ...
                if loginReturn == LoginReturn.ACESSO_SIMULTANEO:
                    mensagemTelegram(tokenBotLogin, chatId, f"Acesso simultâneo! {formatEnumName(enumBanco)} {formatEnumName(enumProcesso)} ⚠️")
                    mensagemTelegram(tokenBotLogin, chatId, f"Aguarde 30 minutos...")
...
if __name__=="__main__":
    ...
    user, senha = getCredenciais(470)#"SE07547063543A", "G@O1987Ts"#getCredenciais(409)
    
    loginFacta(driver, user, senha)
