...
def get_embs(
    ...
    if emb_mode == "cls":
        ...
        assert filtered_input_data["input_ids"][0][0].item() == cls_token_id, "First token is not <cls>"
    ...
    for i in range(0, total_batch_length, forward_batch_size):
        ...
        minibatch = {
            k: v[i:max_range] for k, v in filtered_input_data.items()
        }

        max_len = int(max(minibatch["lengths"]).item())
        original_lens = minibatch["lengths"].to("cuda") if isinstance(minibatch["lengths"], torch.Tensor) else torch.tensor(minibatch["lengths"], device="cuda")

        input_data_minibatch = minibatch["input_ids"][:, :max_len]
        ...
        with torch.no_grad():
            outputs = model(
                ...
                attention_mask=pu.gen_attention_mask({
                    "input_ids": input_data_minibatch,
                    'length': original_lens.tolist()}),
            )
        ...
        if emb_mode == "cell":
            ...
            if summary_stat is None:
                embs_list.append(embs_i.cpu())
            else:
                for h in range(minibatch["input_ids"].shape[0]):
                    length_h = minibatch["lengths"][h].item()
                    input_ids_h = minibatch["input_ids"][h][:length_h]
                    embs_h = embs_i[h, :length_h, :]

                    for idx, token_id in enumerate(input_ids_h):
                        accumulate_tdigests(
                            embs_tdigests_dict[int(token_id.item())], embs_h[idx], emb_dims
                        )
