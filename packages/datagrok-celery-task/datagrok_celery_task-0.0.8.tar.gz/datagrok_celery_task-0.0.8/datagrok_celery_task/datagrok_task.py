...
class DatagrokTask(celery.Task):
    ...
    def before_start(self, task_id, args, kwargs):
        ...
        if not hasattr(self, '_revoked_handler'):
            self._init_revoke_handler()
    ...
    def _init_revoke_handler(self):
        def on_task_cancel(request, terminated, signum, expired, **kwargs):
            task_id = getattr(request, 'id', None)
            if task_id is None or self._call is None:
                return

            if task_id != self._call.id:
                return

            if self._call.status in (FuncCallStatus.COMPLETED, FuncCallStatus.ERROR):
                return
            
            try:
                self._call.status = FuncCallStatus.CANCELED
                amqp_publisher = AmqpFanoutPublisher.get_instance()
                amqp_publisher.publish(self._call.to_json(), task_id, DatagrokFanoutType.CALL)
            except Exception as e:
                self._logger.error("Couldn't cancel call: %s", str(e), exc_info=True, extra={"task_id": self._call.id})

        self._revoked_handler = on_task_cancel
        task_revoked.connect(self._revoked_handler, weak=False)
