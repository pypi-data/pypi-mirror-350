...
class DatagrokTask(celery.Task):
    ...
    def before_start(self, task_id, args, kwargs):
        ...
        if not self._terminate_registered:
            self._logger.debug("Registered SIGTERM handler")
    ...
    def _get_param_grok_pipe(self, param: FuncCallParam):
        ...
        while True:
            ...
            if isinstance(message, str):
                ...
                if total_bytes_received == expected_size:
                    start_time = time_lib.monotonic()
                    self._pipe.recv() # receive fin message
                    fin = time_lib.monotonic()
                    self._logger.info("Received fin message", param.name, (fin - start_time) * 1000, extra={"task_id": self._call.id})
