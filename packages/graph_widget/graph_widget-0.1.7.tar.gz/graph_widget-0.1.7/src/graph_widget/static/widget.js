...
function render({ model, el }) {
    ...
    const create_plot = (width, height, data) => {
    ...
    plot = create_plot(width, height, data);
    ...
    model.on("change:colour_feature", () => {
        ...
        if ( feature_type == "numeric" ) {
            if ( colour_scale_type == "diverging" ) {
                colour_scale = scaleLinear().domain([min(feature_values), 0, max(feature_values)]).range(["#0000FFBF","#EEEEEE33","#FF0000BF"])
            } else {
        ...
        plot.nodeCanvasObject((node, ctx) => {
            const radius = node_size * node_scale;
    ...
    model.on("change:colour_scale_type", () => {
        ...
        if ( feature_type == "numeric" ) {
            if ( colour_scale_type == "diverging" ) {
                colour_scale = scaleLinear().domain([min(feature_values), 0, max(feature_values)]).range(["#0000FF80","#EEEEEE80","#FF000080"])
            } else {
                colour_scale = scaleLinear().domain(extent(feature_values)).range(["#FFFF0080", "#0000FF80"])
            }
        }
