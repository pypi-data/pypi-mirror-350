...
if TYPE_CHECKING:
    # 型チェック時のみ使用
    from storekiss.export_import import LiteStoreExporter, LiteStoreImporter  # noqa: F401
...
class QueryBuilder:
    ...
    def count(self) -> int:
        ...
        # _filters_listが存在する場合は、それを使用
        filters_list = getattr(self, "_filters_list", None)
        
        try:
            if filters_list is not None:
                return self.store.count(filters_list)
            else:
    ...
    def get(self):
        ...
        try:
            ...
            return DocumentSnapshot(
                data, id=self.id, collection=self.collection, store=self.store
            )
        except NotFoundError:
            return None
    ...
    def reference(self):
        ...
        if self._reference is None:
            ...
            if self._collection is None:
                raise ValueError(
                    f"ドキュメントのコレクション名が設定されていません: id={self._id}"
                )

            if self._store is None:
                raise ValueError(
                    f"ドキュメントのストアオブジェクトが設定されていません: id={self._id}, collection={self._collection}"
                )
...
class LiteStore:
    ...
    def _ensure_table_exists(self, collection_name: str) -> None:
        ...
        Args:
            collection_name: 確認または作成するテーブル名（クォートなし）

        Raises:
            DatabaseError: テーブルまたはインデックスの作成に失敗した場合
        """
        # テーブル名を適切にクォート
        quoted_name = quote_table_name(collection_name)
        logger.debug(f"テーブルの存在確認: {quoted_name}")
        ...
        try:
            with self._get_connection() as conn:
                ...
                try:
                    # テーブルが存在するか確認（クォートされた名前で確認）
                    cursor.execute(
                        "SELECT name FROM sqlite_master WHERE type='table' AND name=?",
                        (collection_name,)  # クォートなしの名前で検索
                    )
                    table_exists = cursor.fetchone() is not None

                    if not table_exists:
                        logger.info(f"テーブル {quoted_name} が存在しないため作成します")
                        
                        # テーブルを作成（クォートされた名前で作成）
                        cursor.execute(
                            f"""
                        ...
                        logger.info(f"テーブル {quoted_name} の作成に成功しました")
                        
                        # 作成日時と更新日時のインデックスを作成
                        for col in ["created_at", "updated_at"]:
                            index_name = f"idx_{collection_name}_{col}"
                            cursor.execute(
                                f"CREATE INDEX {quote_table_name(index_name)} "
                                f"ON {quoted_name}({col})"
                            )
                            logger.info(f"インデックス {index_name} を作成しました")
                        
                        # コミット
                        conn.commit()
                        logger.info(f"テーブル {quoted_name} の初期化が完了しました")
                    else:
                        logger.debug(f"テーブル {quoted_name} は既に存在します")
                        
                        # 必要なインデックスが存在するか確認
                        required_indexes = [
                            f"idx_{collection_name}_created_at",
                            f"idx_{collection_name}_updated_at"
                        ]
                        ...
                        for index_name in required_indexes:
                            cursor.execute(
                                ...
                                (index_name,)  # クォートなしの名前で検索
                            )
                            if not cursor.fetchone():
                                # カラム名を正しく取得（例: idx_col_created_at から created_at を抽出）
                                column_name = index_name.replace(f"idx_{collection_name}_", "")
                                cursor.execute(
                                    f"CREATE INDEX {quote_table_name(index_name)} "
                                    f"ON {quoted_name}({column_name})"
                                )
                                logger.info(f"インデックス {index_name} を作成しました")
            ...
            error_msg = f"テーブル {quoted_name} の作成中にエラーが発生しました: {str(e)}"
    ...
    def _initialize_db(self) -> None:
        ...
        try:
            with self._get_connection() as conn:
                ...
                # デフォルトコレクションのテーブルを作成
                self._ensure_table_exists("items")
            ...
            logger.error("データベースの初期化中にエラーが発生しました: %s", str(e))
            raise
    ...
    def update(
        ...
        """
        logger.debug(
            f"updateメソッドが呼び出されました: id={id}, merge={merge}, collection={self.default_collection}"
        )
        try:
            ...
            with self._get_connection() as conn:
                cursor = conn.cursor()
                logger.debug(
                    f"SQLクエリ実行: SELECT data FROM {self.default_collection} WHERE id = {id}"
                )
                cursor.execute(
            ...
            if "no such table" in str(e):
                logger.info(
                    f"テーブル {self.default_collection} が存在しないため作成します"
                )
                self._ensure_table_exists(self.default_collection)
                # テーブル作成後に再試行
        ...
        if merge:
            ...
            for key, value in data.items():
                ...
                if str(value) == "DELETE_FIELD":
                    if "." in key:
                        logger.warning(
                            f"ネストされたフィールドの削除はサポートされていません: {key}"
                        )
        ...
        try:
            with self._get_connection() as conn:
                cursor = conn.cursor()
                logger.debug(
                    f"SQLクエリ実行: UPDATE {self.default_collection} SET data = ?, updated_at = CURRENT_TIMESTAMP WHERE id = {id}"
                )
                cursor.execute(
    ...
    def count(
        ...
        """
        ストア内のアイテム数をカウントします。オプションでフィルターを適用できます。

        Args:
            filters: カウント前に適用するオプションのフィルター。以下のいずれか:
                - フィールドと値の辞書（暗黙的に '==' 演算子を使用）
                - (フィールド, 演算子, 値) のタプルのリスト

        Returns:
            int: フィルターに一致するアイテムの数。テーブルが存在しない場合は0。

        Raises:
            DatabaseError: データベースでエラーが発生した場合。
        """
        try:
                    ...
                    if isinstance(filters, dict):
                        ...
                        for key, value in filters.items():
                            ...
                            if value is None:
                                ...
                                params.extend([json_path, bool_value])
                            else:
                                where_clauses.append("json_extract(data, ?) = ?")
                                params.extend([json_path, value])
                    
                    elif isinstance(filters, list):
                        # リスト形式のフィルター処理 (field, op, value) のタプルのリスト
                        for field, op, value in filters:
                            json_path = f"$.{field}"
            ...
            raise DatabaseError(f"Error counting items: {str(e)}")
    ...
    def get_collection(self, name: str) -> "Collection":
        ...
        Args:
            name: Name of the collection (unquoted)

        Returns:
            A Collection object for the specified collection.
        """
        # テーブルが存在することを確認し、必要に応じて作成
        self._ensure_table_exists(name)
        
        # コレクションにはクォートされていない名前を使用
    ...
    def rebuild_indexes(
        ...
        with self._get_connection() as conn:
            ...
            if collection_name:
                # コレクション名をクォートして使用
                quoted_collection_name = quote_table_name(collection_name)
                collections = [quoted_collection_name]
                # 結果の辞書のキーには元の名前を使用
                collection_name_map = {quoted_collection_name: collection_name}
            else:
                cursor.execute(
                ...
                # 結果の辞書のキーには元の名前を使用
                collection_name_map = {name: name for name in collections}

            for quoted_coll_name in collections:
                # 結果の辞書のキーには元の名前を使用
                original_name = collection_name_map.get(quoted_coll_name, quoted_coll_name)
                created_indexes = []


                try:
                    # インデックスの一覧を取得（クォートされた名前で検索）
                    cursor.execute(
                        "SELECT name FROM sqlite_master WHERE type='index' AND tbl_name=? AND name NOT LIKE '%autoindex%'",
                        (original_name,),  # クォートなしの名前で検索
                    )
                    ...
                    for idx_name in indexes:
                        try:
                            cursor.execute(f"DROP INDEX IF EXISTS {quote_table_name(idx_name)}")
                            logging.info(f"インデックス {idx_name} を削除しました")
                        except sqlite3.Error as e:
                            logging.warning(
                                f"インデックス {idx_name} の削除中にエラーが発生しました: {e}"
                            )

                    # データを取得（クォートされた名前でテーブルを参照）
                    cursor.execute(f"SELECT data FROM {quoted_coll_name}")
                    rows = cursor.fetchall()

                    if not rows:
                        logging.info(
                            f"コレクション {original_name} にドキュメントがありません"
                        )
                    ...
                    for row in rows:
                        try:
                            ...
                            logging.warning(
                                f"コレクション {original_name} の無効なJSONデータをスキップします"
                            )

                    # 各フィールドにインデックスを作成
                    for field in all_fields:
                        # インデックス名からクォートを削除して作成
                        safe_coll_name = quoted_coll_name.replace('"', '')
                        index_name = f"idx_{safe_coll_name}_{field}"
                        try:
                            # フィールド名も適切にクォート
                            quoted_field = field.replace('"', '')
                            cursor.execute(
                                f"CREATE INDEX IF NOT EXISTS {quote_table_name(index_name)} "
                                f"ON {quoted_coll_name} (json_extract(data, '$.{quoted_field}'))"
                            )
                            conn.commit()
                            created_indexes.append(field)
                            logging.info(
                                f"フィールド {field} のインデックス {index_name} を作成しました"
                            )
                        except sqlite3.Error as e:
                            logging.error(
                                f"フィールド {field} のインデックス作成中にエラーが発生しました: {e}"
                            )

                    # 結果には元の名前を使用
                    result[original_name] = created_indexes

                except sqlite3.Error as e:
                    logging.error(
                        f"コレクション {original_name} のインデックス再構築中にエラーが発生しました: {e}"
                    )
                    raise DatabaseError(
                        f"Failed to rebuild indexes for collection {original_name}: {str(e)}"
                    ) from e
...
def _add_export_import_to_litestore():
    ...
    """
    try:
        # 実行時にのみインポート、ローカルスコープで使用
        from storekiss.export_import import LiteStoreExporter as _Exporter
        from storekiss.export_import import LiteStoreImporter as _Importer
