...
class BBoxCryptoFromQujingWithStandby(BBoxCryptoTemplate):
    def __init__(self, host_ports: dict):
        self.cryptos = []
        for host in host_ports:
            port_config = host_ports[host]["port_config"]
            port_invoke = host_ports[host]["port_invoke"]
...
class BBoxRequest:
    ...
    def request(crypto, typeint, method: str, url: str, **kwargs):
        ...
        if method == "POST":
            ...
            request_kwargs["params"] = kwargs.get("params")
        elif method == "GET":
            pass
        else:
