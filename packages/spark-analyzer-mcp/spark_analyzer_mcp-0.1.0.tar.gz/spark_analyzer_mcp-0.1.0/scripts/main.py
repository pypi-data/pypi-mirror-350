...
from analyzers.spark_log_parser import SparkLogParser, SparkLogData
...
def _find_spark_log_file(extract_dir: Path) -> Optional[Path]:
    ...
    if secondary_candidates:
        ...
        return secondary_candidates[0] # Take the first .log, .txt or extensionless file
...
async def analyze_spark_log_zip(zip_file_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir_name:
            ...
            logger.info(f"Analyzing parsed log data for App ID: {parsed_log_data.app_id}")
            analysis_result: SparkAnalysisResult = await analyzer.analyze_log(parsed_log_data)
            logger.info(f"Log analysis complete for App ID: {parsed_log_data.app_id}")

            return analysis_result.model_dump(mode='json') # Ensure Pydantic models are JSON serializable
        ...
        logger.exception(f"An unexpected error occurred during Spark log analysis for {zip_file_path}")
        return {
