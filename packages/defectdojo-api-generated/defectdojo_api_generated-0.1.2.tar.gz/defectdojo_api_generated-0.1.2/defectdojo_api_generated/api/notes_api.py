...
class NotesApi:
    ...
    def _notes_list_serialize(
        ...
        if var_date is not None:
            if isinstance(var_date, datetime):
                _query_params.append(('date', var_date.strftime(self.api_client.configuration.datetime_format)))
            else:
                _query_params.append(('date', var_date))

        if edit_time is not None:
            if isinstance(edit_time, datetime):
                _query_params.append(('edit_time', edit_time.strftime(self.api_client.configuration.datetime_format)))
            else:
                _query_params.append(('edit_time', edit_time))

        if edited is not None:
            _query_params.append(('edited', edited))

        if editor is not None:
            _query_params.append(('editor', editor))

        if entry is not None:
            _query_params.append(('entry', entry))
    ...
    def notes_partial_update(
        ...
        :type id: int
        :param patched_note_request:
        :type patched_note_request: PatchedNoteRequest
        :param _request_timeout: timeout setting for this request. If one
                                 number provided, it will be total request
                                 timeout. It can also be a pair (tuple) of
                                 (connection, read) timeouts.
    ...
    def _notes_partial_update_serialize(
        ...
        # process the form parameters
        # process the body parameter
        if patched_note_request is not None:
            _body_params = patched_note_request
