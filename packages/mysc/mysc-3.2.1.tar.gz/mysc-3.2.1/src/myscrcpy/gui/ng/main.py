...
def device_page(device_serial: str):
    ...
    def handle_key(e: KeyEventArguments):
        ...
        if not keyboard_switch.value:
            ...
            if e.action.keydown:
                try:
                    if e.indicator_badge.name in [str(_) for _ in range(10)]:
                        ngc.dc.adb_dev.keyevent(ADBKeyCode[f"KB_{e.indicator_badge.name}"].value)

                    elif e.indicator_badge.name in string.ascii_letters:
                        ngc.dc.adb_dev.send_keys(e.indicator_badge.name)

                    elif e.indicator_badge.name.upper().startswith('ARROW'):
                        ngc.dc.adb_dev.keyevent(ADBKeyCode['KB_' + e.indicator_badge.name.upper()[5:]])

                    else:
                        # Functions Keys
                        try:
                            ngc.dc.adb_dev.keyevent(KeyMapper.uk2adb(ng2uk(e.indicator_badge.code)))
                        except:
                            try:
                                ngc.dc.adb_dev.keyevent(ADBKeyCode['KB_' + e.indicator_badge.code.upper()])
                            except:
                                ngc.dc.adb_dev.send_keys(e.indicator_badge.name)
                                logger.warning(f"key is {e.indicator_badge.name}, Make Your OWN Keymapper~")
            ...
            if e.action.repeat:
                return
            if e.action.keydown:
                ngc.key_watcher.key_pressed(ng2uk(e.indicator_badge.code))
            elif e.action.keyup:
                ngc.key_watcher.key_release(ng2uk(e.indicator_badge.code))
