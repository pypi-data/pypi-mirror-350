               ...
               int optional)
{
    ...
    while (PyDict_Next(valid_dict, &pos, &name, (PyObject**)&vd)) {
        ...
        if (!item) {
            ...
            if (ValidationError_CreateMissing(name, val, ctx->model, err) < 0) {
                return -1;
            }
            continue;
        }
        ...
        if (!tmp) {
            if (ValidationError_Create(name, vd, item, ctx->model, err) < 0) {
                return -1;
            }
            continue;
        }

        int r = Dict_SetItem_String(dict, name, tmp);
        Py_DECREF(tmp);
        if (r < 0) {
            return -1;
        }
           ...
           int include)
{
    ...
    while (PyDict_Next(annot, &pos, &name, &hint)) {
        ...
        if (PyDict_SetItemStringDecrefVal(dict, name, (PyObject*)vd) < 0) {
            return -1;
        }
    }
