                           ...
                           PyObject* dict)
{
    ...
    while (PyDict_Next(val, &pos, &key, &item)) {
        ...
        if (!tmp_key) {
            if (ValidationError_IndexCreate(
...
{
    if (!TypeAdapter_CollectionCheckArgs(type_args, (PyTypeObject*)cls, 2)) {
        return NULL;
    }
    ...
    TypeAdapter* res = _TypeAdapter_NewCollection(cls, args, converter_dict);
    Py_DECREF(args);
    return res;
}
