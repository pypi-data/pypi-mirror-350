...
{
    char* buf;
    Py_ssize_t length = get_buffer(obj, &buf, raise_err);
    if (length == -1) {
        return NULL;
    }
...
converter_time(UNUSED TypeAdapter* self, ValidateContext* ctx, PyObject* val)
{
    ...
    if (PyLong_Check(val) && !PyBool_Check(val)) {
        return timestamp_to_time(PyLong_AsLongLong(val), 0.0);
    }
    ...
    if (PyFloat_Check(val)) {
        ...
        return timestamp_to_time(l, d - (double)l);
    }
    return date_time_parse_time(val, 0);
}
...
converter_date(UNUSED TypeAdapter* self, ValidateContext* ctx, PyObject* val)
{
    ...
    if (PyLong_Check(val) && !PyBool_Check(val)) {
        return timestamp_to_date(PyLong_AsLongLong(val));
    }
    ...
    if (PyFloat_Check(val)) {
        ...
        return timestamp_to_date(l);
    }

    return date_time_parse_date(val, 0);
}
                   ...
                   PyObject* val)
{
    ...
    if (PyLong_Check(val) && !PyBool_Check(val)) {
        return timestamp_to_datetime(PyLong_AsLongLong(val), 0.0);
    }
    ...
    if (PyFloat_Check(val)) {
        ...
        return timestamp_to_datetime(l, d - (double)l);
    }

    return date_time_parse_date_time(val, 0);
}
