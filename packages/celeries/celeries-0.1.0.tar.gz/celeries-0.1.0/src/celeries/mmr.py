...
class MMR:
  ...
  def solve_fp(self, v0, delta, **kwargs):
    ...
    The root-finding is performed using :func:`scipy.optimize.root`
    ...
    delta : float
      Value of delta.
    **kwargs :
      Optional arguments to pass to :func:`scipy.optimize.root`.
    ...
    """
    return root(self.eval_dv, v0, jac=self.eval_grad_dv, args=(delta,), **kwargs)
  ...
  def ell2v(self, ell):
    ...
    sig_phi = self.M[:-2] @ np.concatenate((-w, la0))
    x = np.sqrt(2 * D) * np.cos(sig_phi[: self.npla])
    y = np.sqrt(2 * D) * np.sin(sig_phi[: self.npla])
    dL = self.iM.T[self.npla : -2, self.npla :] @ (La - self.La0)
    v = np.concatenate((y, sig_phi[self.npla :] % (2 * np.pi), x, dL))
    return (v, delta, Gamma)

  def v2ell(self, v, delta, Gamma=1):
    r"""
    ...
    La = self.La0 + self.M.T[self.npla :, self.npla : -1] @ np.concatenate((dL, [eps]))
    a = (La * Gamma / self.beta) ** 2 / self.mu
    e = np.sqrt(1 - (1 - D / La) ** 2)

    sig = np.arctan2(y, x)
    sigphi = np.concatenate((sig, phi, [0, 0]))
  ...
  def random_v0(self, delta, nsample=1):
    r"""
    ...
    mr3_2 = self.mass_ratio**3 / 2
    d_3 = delta / 3
    a = mr3_2 - d_3**3
    b2 = mr3_2 * (mr3_2 - 2 * d_3**3)
    b = np.exp(1j * np.angle(b2) / 2) * np.sqrt(np.abs(b2))
    eps = ((a + b) ** (1 / 3) + (a - b) ** (1 / 3) - d_3).real

    ax = np.sqrt(10 * (delta + eps) / (2 * self.npla))
    aL = 10 * abs(eps)
