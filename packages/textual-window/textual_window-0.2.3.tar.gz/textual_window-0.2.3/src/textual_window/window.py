...
class TopBar(Horizontal):
    ...
    def compose(self) -> ComposeResult:
        ...
        if self.options:
            yield HamburgerButton(BUTTON_SYMBOLS["hamburger"], window=self.window, options=self.options)
        if self.window.show_maximize_button:
            self.maximize_button = MaximizeButton(BUTTON_SYMBOLS["maximize"], window=self.window)
            yield self.maximize_button
        yield CloseButton(BUTTON_SYMBOLS["close"], window=self.window)
...
class BottomBar(Horizontal):
    ...
    def compose(self) -> ComposeResult:
        ...
        if self.window.allow_resize:
            yield Resizer(BUTTON_SYMBOLS["resizer"], window=self.window)
