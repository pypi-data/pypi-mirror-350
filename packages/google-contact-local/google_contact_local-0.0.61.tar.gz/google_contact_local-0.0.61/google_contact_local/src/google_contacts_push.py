...
class GoogleContactsPush(GoogleContacts):
    ...
    def initialize_service(self, email: str):
        ...
        if self.creds.token_state == 3:
            message_internal = "Stored credentials are not valid."
            self.logger.error(
            ...
            if is_refresh_token_valid:
                ...
                self.logger.error(
                    f"{message_internal}. Refresh token is not valid; emaoil: {email}, is_refresh_token_valid: {is_refresh_token_valid}, expiry: {expiry_dt}",  # noqa
                    object={"token_info": token_info},
                )
                raise Exception(f"{message_internal}. Refresh token is not valid; emaoil: {email}, is_refresh_token_valid: {is_refresh_token_valid}, expiry: {expiry_dt}")  # noqa
