...
from polaris.utils.database.db_utils import read_and_close, commit_and_close, get_trigger_sql, drop_trigger
...
class GeoConsistency(WorkerThread):
    ...
    def update_active_network_association(self, for_locations=True, for_parking=True):
        ...
        with commit_and_close(self._network_path, commit=True, spatial=True) as conn:
            ...
            trig_manager = TriggerManager(trigger_names, conn)
            ...
            conn.execute(f'DELETE FROM Geo_Consistency_Controller WHERE table_name="{tab[0]}" and field_changed="geo"')
            conn.execute(f'DELETE FROM Geo_Consistency_Controller WHERE table_name="{tab[1]}" and field_changed="geo"')
            conn.execute(f'DELETE FROM Geo_Consistency_Controller WHERE table_name="{tab[2]}" and field_changed="geo"')

            trig_manager.recreate_triggers(conn)
        for message in self.messages:
            logging.warning(message)
        self.messages.clear()
        self.finish()
    ...
    def __update_county_association(self, conn: sqlite3.Connection):
        ...
        trig_manager = TriggerManager(["polaris_location_on_county_change"], conn)
    ...
    def __update_areatype_association(self, conn: sqlite3.Connection):
        ...
        trig_manager = TriggerManager(["polaris_network_changes_on_link_area_type"], conn)
    ...
    def __update_location_association(self, conn: sqlite3.Connection):
        ...
        trig_manager = TriggerManager(["polaris_ev_charging_stations_on_location_change"], conn)
...
class TriggerManager:
    def __init__(self, trigger_list: List[str], conn: sqlite3.Connection):
        self.sqls = []
        for trigger_name in trigger_list:
            self.sqls.append(get_trigger_sql(conn, trigger_name))
            drop_trigger(conn, trigger_name)
        conn.commit()

    def recreate_triggers(self, conn: sqlite3.Connection):
        _ = [conn.execute(sql) for sql in self.sqls]
