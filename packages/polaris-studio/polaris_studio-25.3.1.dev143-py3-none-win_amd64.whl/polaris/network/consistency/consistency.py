...
class Consistency(WorkerThread):
    ...
    def __init__(self, network_file: PathLike):
        ...
        self.proc_calls = {
            "update_link_association": {
                ...
                "run": False,
                "args": {"for_locations": False, "for_parking": False, "for_micromobility_docks": False},
            },
            "update_active_network_association": {
                "procedure": self.gc.update_active_network_association,
                "run": False,
                "args": {"for_locations": False, "for_parking": False},
            },
    ...
    def set_change_to_locations(self, fields):
        if any(x in fields for x in ["walk_link", "bike_link", "geo"]):
            ...
            self.proc_calls["update_active_network_association"]["args"]["for_locations"] = True
        if any(x in fields for x in ["link", "geo"]):
            self.proc_calls["update_link_association"]["run"] = True
            self.proc_calls["update_link_association"]["args"]["for_locations"] = True
    ...
    def set_change_to_micromobility_docks(self):
        self.proc_calls["update_link_association"]["run"] = True
        self.proc_calls["update_link_association"]["args"]["for_micromobility_docks"] = True  # type: ignore

    def set_change_to_active_networks(self):
        self.proc_calls["update_active_network_association"]["run"] = True
        self.proc_calls["update_active_network_association"]["args"]["for_locations"] = True  # type: ignore
        self.proc_calls["update_active_network_association"]["args"]["for_parking"] = True  # type: ignore

    def set_change_to_links(self, fields):
        self.proc_calls["update_active_network_association"]["run"] = True
        self.proc_calls["update_active_network_association"]["args"]["for_locations"] = True
        self.proc_calls["update_active_network_association"]["args"]["for_parking"] = True

        self.proc_calls["update_link_association"]["run"] = True
        self.proc_calls["update_link_association"]["args"]["for_locations"] = True
        self.proc_calls["update_link_association"]["args"]["for_parking"] = True
        self.proc_calls["update_link_association"]["args"]["for_micromobility_docks"] = True
    ...
    def __process_queue(self):
        for job in self.proc_calls.values():
            if job["run"]:
                if "args" in job:
