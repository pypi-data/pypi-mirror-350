...
# class UsageMetadata:
#     def __init__(self, model, candidates_token_count, prompt_token_count, total_token_count):
#         self.model = model
...
#
#     def __str__(self):
#         return f"UsageMetadata(model = {self.model}, candidates_token_count={self.candidates_token_count}, " \
#                f"prompt_token_count={self.prompt_token_count}, total_token_count={self.total_token_count})"
#
#     @staticmethod
#     def instance(model, meta_data: types.GenerateContentResponseUsageMetadata):
#         usage_meta_data = UsageMetadata(model=model,
#                                         candidates_token_count=meta_data.candidates_token_count,
#                                         prompt_token_count=meta_data.prompt_token_count,
#                                         total_token_count=meta_data.total_token_count)
#         return usage_meta_data
...
def google_genai_output_images_and_text(new_message: GenaiConversationMessage,
    ...
    if chunk and chunk.usage_metadata:
        try:
            print(f"model={model} , Usage Metadata:{chunk.usage_metadata}")
            # usage_metadata = UsageMetadata.instance(model, chunk.usage_metadata)
