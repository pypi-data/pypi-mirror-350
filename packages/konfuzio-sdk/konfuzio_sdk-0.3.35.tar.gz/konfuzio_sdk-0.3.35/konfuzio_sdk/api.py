...
def konfuzio_session(token: str = None, timeout: Optional[int] = None, num_retries: Optional[int] = None, host: str = None):
    ...
    retry_strategy = Retry(
        ...
        backoff_factor=2,
        allowed_methods=["HEAD", "GET", "OPTIONS", "PUT", "DELETE", "PATCH"],
        raise_on_status=False,
        respect_retry_after_header=True,
        # Only supported in latest URLlib version
        # connect_exceptions=(ChunkedEncodingError, ConnectionError, Timeout)
    )
...
def restore_snapshot(snapshot_id: int, session=None) -> int:
    ...
    if r.status_code == 201:
        logger.info(f'[SUCCESS] Restoring snapshot {snapshot_id} in progress...')
        new_snapshot_url = get_restored_snapshot_url(restored_snapshot_id=r.json()['id'])
        status = None
        while status != 'done':
            logger.info(f'Restoring snapshot {status}...')
            new_snapshot_status = session.get(url=new_snapshot_url)
            status = new_snapshot_status.json()['status']
            time.sleep(5)
        duration = time.time() - start_time
        logger.info(f'[DONE] Snapshot {snapshot_id} restored in {duration:.2f} seconds.')
