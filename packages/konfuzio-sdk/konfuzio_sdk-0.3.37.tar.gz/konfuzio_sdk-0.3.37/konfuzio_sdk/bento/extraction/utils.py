...
from konfuzio_sdk.data import Annotation, AnnotationSet, Document, Page, Project, Span, Bbox
            ...
            for annotation in annotation_set.annotations(use_correct=False, ignore_below_threshold=True):
                spans_list_of_dicts = [
                    schema.AnnotationSet.Annotation.Span(
                        ...
                        offset_string_original=span.offset_string_original,
                    )
        ...
        converted = schema(
            ...
            bboxes={
                k: {
                    ...
                    'bottom': v.bottom,
                    'text': document.text[k],
                    'line_index': doc_bbox[str(k)].get('line_index') if 'line_index' in doc_bbox[str(k)] else doc_bbox[str(k)]['line_number'] - 1,
                }
                for k, v in document.bboxes.items()
            },
            pages=pages,
        ...
        for annotation_set in response.annotation_sets:
            ...
            for annotation in annotation_set.annotations:
                ...
                for span in annotation.span:
                    ...
                    if span.x0 is not None and span.x1 is not None and span.y0 is not None and span.y1 is not None:
                        page = document.pages()[span.page_index]
                        sdk_span._bbox = Bbox(x0=span.x0, y0=span.y0, x1=span.x1, y1=span.y1, page=page)
                        sdk_span._page = page
