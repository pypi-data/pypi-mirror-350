...
class Molecule(base.Molecule):
    ...
    def __init__(
        ...
        if request_free is not None:
            ...
            self.set_requested_dofs(request_free)

        if request_fixed is not None:
            request_fixed = [(i - 1, j - 1) for i, j in request_fixed]
            self.set_fixed_dofs(request_fixed)

        if require_best_sequence:
            self.require_best_sequence()

        if sdf is not None:
            assert isinstance(
                sdf, str), "sdf must be an str object (name of your SDF)"
            self.sdf_constructor(sdf)
            ...
            self.graph_constructor(graph_data)
...
class ConfsearchInput(BaseModel):
    ...
    postopt_settings: Any = Field([], validate_default=True)
    geometry_validation: Any = {}
