...
class P2PNode:
    ...
    def _process_message(self, message_data, address):
        ...
        if message_type == 'verify_id':
            ...
            print(f"Nuevo usuario conectado: {peer_name} ({peer_birth_date}) desde {address[0]}:{message_data.get('port')}")
    ...
    def list_connected_nodes(self):
        ...
        for node_id, (host, port, name, birth_date) in self.peers.items():
            print(f"- {name} (Año de nacimiento: {birth_date})")
        ...
        try:
            ...
            if command.startswith("add "):
                try:
                    ...
                    contact_name = parts[1].split(">")[0].strip()
                    contact_birth_year = parts[2].rstrip(">").strip()
                    ...
                    contact_id = node.user_manager._generate_user_id(contact_name, contact_birth_year)
