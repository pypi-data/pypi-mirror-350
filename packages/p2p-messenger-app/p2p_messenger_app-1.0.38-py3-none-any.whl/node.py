...
def close_port_5000():
    """Cierra cualquier conexión existente en el puerto 5000"""
        ...
        if system == "Darwin":  # macOS
            ...
            if result.stdout:
                ...
                if len(lines) > 1:  # La primera línea es el encabezado
                    ...
                    print(f"Proceso {pid} usando el puerto 5000 ha sido cerrado")
                    return True
        elif system == "Linux":
            # Encontrar el proceso usando el puerto 5000
            result = subprocess.run(['fuser', '-k', '5000/tcp'], capture_output=True)
            if result.returncode == 0:
                print("Proceso usando el puerto 5000 ha sido cerrado")
                return True
        elif system == "Windows":
            # Encontrar el proceso usando el puerto 5000
                ...
                try:
                    for conn in proc.connections():
                        if conn.laddr.port == 5000:
                            proc.kill()
                            print(f"Proceso {proc.pid} usando el puerto 5000 ha sido cerrado")
                            return True
                except (psutil.NoSuchProcess, psutil.AccessDenied):
                    pass
    except Exception as e:
        print(f"Error al cerrar el puerto 5000: {e}")
    return False
...
class ChatServer:
    ...
    def _get_public_ip(self):
        ...
        try:
            ...
            for interface in socket.getaddrinfo(socket.gethostname(), None):
                if interface[0] == socket.AF_INET:  # Solo IPv4
                    ip = interface[4][0]
            ...
            if interfaces:
                ...
                # Intentar conectar a cada interfaz
            ...
            # Si nada funciona, intentar obtener IP local
    ...
    def broadcast_message(self, message, username):
        ...
        if self.room:
            ...
            for client_socket in self.room.users:
                try:
                    client_socket.send(json.dumps(message_data).encode() + b'\n')
                except:
                    disconnected_users.append(client_socket)
            
            # Limpiar usuarios desconectados
            for client_socket in disconnected_users:
                if client_socket in self.room.users:
                    username = self.room.users[client_socket]
                    del self.room.users[client_socket]
    ...
    def start(self):
        ...
        try:
            ...
            # Intentar vincular a todas las interfaces
            try:
                self.server_socket.bind((self.host, self.port))
            except Exception as e:
                print(f"\nError al vincular a {self.host}: {e}")
            ...
            try:
                ...
                test_socket.close()
                print("\nVerificación de conectividad local: OK")
            except:
                print("\nAdvertencia: No se pudo verificar la conectividad local")
        ...
        try:
            ...
            if option == "1":
                ...
                try:
                    ...
                    client_socket.send(json.dumps(join_message).encode() + b'\n')
                    ...
                    while True:
                        try:
                            ...
                            buffer += data.decode()
                            if '\n' in buffer:
                                response_str, buffer = buffer.split('\n', 1)
                                try:
                                    ...
                                    print(f"\nError al decodificar respuesta: {e}")
                                    server.stop()
                                    break
                        except Exception as e:
                            print(f"\nError al recibir respuesta: {e}")
                            server.stop()
                            break
                            ...
                            try:
                                ...
                                while '\n' in buffer:
                                    ...
                                    try:
                                        ...
                                        print(f"\nError al decodificar mensaje: {e}")
                                        continue
                            except Exception as e:
                                if server.running:
