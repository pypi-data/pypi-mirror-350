        ...
        try:
            ...
            if option == "1":
                ...
                try:
                    ...
                    client_socket.send(json.dumps(list_message).encode())
                    
                    # Esperar respuesta
                    response = client_socket.recv(4096)
                    if response:
                        response_data = json.loads(response.decode())
                        if response_data.get('type') == 'rooms_list':
                            rooms = response_data.get('rooms', [])
                            if rooms:
                                print("\nSalas activas:")
                                for room in rooms:
                                    print(f"\nCódigo: {room['room_id']}")
                                    print(f"Creador: {room['creator']}")
                                    print(f"Usuarios: {room['users']}")
                                    print(f"Creada: {room['created_at']}")
                            else:
