...
async def llm_chat_async(messages: list, max_tokens=None, model_tag="google/gemini-2.0-flash-lite-001"):
    if PROVIDER == "openrouter":
        return openrouter_chat_async(messages, model_tag, max_tokens)
    elif PROVIDER == "ollama":
        return ollama_chat_async(messages, model_tag, max_tokens)
    elif PROVIDER == "vllm":
        return vllm_chat_async(messages, model_tag, max_tokens)
    elif PROVIDER == "openai":
        return openai_chat_async(messages, model_tag, max_tokens)
    else:
        raise Exception(f"Unknown LLM provider: {PROVIDER}")
...
async def llm_prompt_async(message: str, max_tokens=None, model_tag="google/gemini-2.0-flash-lite-001"):
    if PROVIDER == "openrouter":
        return openrouter_prompt_async(message, model_tag, max_tokens)
    elif PROVIDER == "ollama":
        return ollama_prompt_async(message, model_tag, max_tokens)
    elif PROVIDER == "vllm":
        return vllm_prompt_async(message, model_tag, max_tokens)
    elif PROVIDER == "openai":
        return openai_prompt_async(message, model_tag, max_tokens)
    else:
        raise Exception(f"Unknown LLM provider: {PROVIDER}")
