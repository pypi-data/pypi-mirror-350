    ...
    for i in range(len(boxes['text'])):
        if boxes['text'][i] != "":
            block = {
                ...
                "par_num": boxes['par_num'][i]
            }
            blocks.append(block)

    # 693,570 até 1040,744
    def filter_by_bounds(data, x_min, y_min, x_max, y_max):
        return [
            item for item in data
            if x_min <= item['left'] and
            y_min <= item['top'] and
            item['left'] + item['width'] <= x_max and
            item['top'] + item['height'] <= y_max
        ]
    x_min, y_min = 693, 570
    x_max, y_max = 1040, 744
    filtered = filter_by_bounds(blocks, x_min, y_min, x_max, y_max)
    text = " ".join([item['text'] for item in filtered])
...
def boost_contrast(pil_image, boost_percent=100):
    img_np = np.array(pil_image.convert('L'))
    img_np = img_np * boost_percent / 100
