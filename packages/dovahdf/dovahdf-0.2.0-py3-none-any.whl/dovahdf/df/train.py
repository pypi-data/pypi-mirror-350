...
def run_df_train(
    ...
    *,
    ...
    init_logger(file=os.path.join(base_dir, "train.log"), level="INFO", model=base_dir)
    logger.info(f"Using pretrained model: {pretrained}")

    # 4. config  (load default -> merge existing)
    config_file = os.path.join(base_dir, "config.ini")
    default_cfg = os.path.join(os.path.dirname(base_dir), "my_model", "config.ini")
    config._loaded = False                      # type: ignore[attr-defined]
    config.load(default_cfg)
    if os.path.isfile(config_file):
        config.parser.read(config_file)
        logger.info(f"Merged existing config: {config_file}")
    else:
        logger.info("No config.ini found; will create a fresh one.")
    for sec in ("train", "optim"):
        if not config.parser.has_section(sec):
            config.parser.add_section(sec)

    # 5. overrides
    if pretrained:
        config.overwrite("train", "model", pretrained)
    if epochs is not None:
        config.overwrite("train", "max_epochs", epochs)
    if batch_size is not None:
        config.overwrite("train", "batch_size", batch_size)
    if lr is not None:
        config.overwrite("optim", "lr", lr)
    cfg_epochs = config("MAX_EPOCHS", 10, int, section="train")
    config.overwrite("train", "max_epochs", cfg_epochs)

    # write full config
    with open(config_file, "w", encoding="utf-8") as fp:
        for sec in config.parser.sections():
            fp.write(f"[{sec}]\n")
            for k, v in config.parser.items(sec):
                fp.write(f"{k} = {v}\n")
            fp.write("\n")
    logger.info(f"Wrote full config to {config_file}")
    ...
    mask_only     = config("DF_ONLY", False, bool, section="train")
    train_df_only = config("DF_ONLY", False, bool, section="train")
    jit           = config("JIT", False, bool, section="train")

    model, start_epoch = load_model(
        ckpt_dir, state, jit=jit,
        mask_only=mask_only, train_df_only=train_df_only
    )

    # 9. dataloader
    bs   = config("BATCH_SIZE", 1, int, section="train")
    bs_e = config("BATCH_SIZE_EVAL", 0, int, section="train")
    bs_eval = bs if bs_e <= 0 else bs_e
    dataloader = DataLoader(
        ds_dir=data_dir, ds_config=cfg_path, sr=p.sr,
        batch_size=bs, batch_size_eval=bs_eval,
        num_workers=config("NUM_WORKERS", 4, int, section="train"),
        pin_memory=(get_device().type == "cuda"),
        max_len_s=config("MAX_SAMPLE_LEN_S", 5.0, float, section="train"),
        fft_size=p.fft_size, hop_size=p.hop_size,
        nb_erb=p.nb_erb, nb_spec=p.nb_df,
        norm_alpha=get_norm_alpha(log=False),
        p_reverb=config("p_reverb", 0.2, float, section="distortion"),
        p_bw_ext=config("p_bandwidth_ext", 0.0, float, section="distortion"),
        p_clipping=config("p_clipping", 0.0, float, section="distortion"),
        ...
        prefetch=config("NUM_PREFETCH_BATCHES", 32, int, section="train"),
        overfit=config("OVERFIT", False, bool, section="train"),
        seed=seed, min_nb_erb_freqs=p.min_nb_freqs,
        log_timings=log_timings,
        global_sampling_factor=config("GLOBAL_DS_SAMPLING_F", 1.0, float, section="train"),
    ...
    # 10. optimiser & schedulers
    opt = load_opt(ckpt_dir, model, mask_only, train_df_only)
    lrs = setup_lrs(len(dataloader))
    wds = setup_wds(len(dataloader))
    if jit:
        model = torch.jit.script(model)
    ...
    max_epochs = config("MAX_EPOCHS", 10, int, section="train")
    patience   = config("EARLY_STOPPING_PATIENCE", 5, int, section="train")
    val_rule   = config("VALIDATION_CRITERIA_RULE", "min", section="train")\
                   .replace("less", "min").replace("more", "max")
    ...
    for epoch_idx in range(start_epoch, max_epochs):
        # ---- train ----
        tr_loss = run_epoch(model, epoch_idx, dataloader, "train", opt,
                            losses, summary_dir, lrs, wds)
        log_metrics(f"[{epoch_idx}] [train]", {"loss": tr_loss})
        write_cp(model, "model", ckpt_dir, epoch_idx + 1)
        write_cp(opt,   "opt",   ckpt_dir, epoch_idx + 1)
        losses.reset_summaries()

        # ---- valid ----
        val_loss = run_epoch(model, epoch_idx, dataloader, "valid", opt,
                             losses, summary_dir)
        write_cp(model, "model", ckpt_dir, epoch_idx + 1,
                 metric=val_loss, cmp=val_rule)
        log_metrics(f"[{epoch_idx}] [valid]", {"loss": val_loss})

        if not check_patience(ckpt_dir, patience, val_loss, val_rule, raise_=False):
            break
        if globals().get("should_stop", False):
            logger.info("External stop signal received; terminating.")
            break

    logger.info("Finished training")

    # ──────────────────── Test-set evaluation ──────────────────────────
    if config("EVAL_TEST_SET", True, bool, section="train"):
        logger.info("Loading best checkpoint for final test-set evaluation …")
        model_best, best_ep = load_model(
            ckpt_dir, state, jit=jit,
            epoch="best", mask_only=mask_only, train_df_only=train_df_only
        )

    test_loss = run_epoch(
        model_best, best_ep, dataloader, "test",
        opt, losses, summary_dir
    )

    metrics = {"loss": test_loss}
    for n, vals in losses.get_summaries():
        metrics[n] = torch.mean(torch.stack(vals)).item()

    log_metrics(f"[{best_ep}] [test]", metrics)
    losses.reset_summaries()
