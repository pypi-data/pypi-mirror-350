...
from rbom.rbom_parser import RBOMParser, MetaData
...
# load rbom
python src/cli.py load "src/rbom/example.rbom.yaml"

# set attribute
```bash
python src/cli.py set-attribute --name=actor --value=prof
```

# sign and save
```bash
python src/cli.py sign "keys/private_rbom_key.pem" --public-key-path="keys/public_rbom_key.pem" --repository="spockops/rbom" --commit-sha="214124412214" --github-token="ghyp21214412412" 
