...
@mcp.tool(name="记录收入和支出", description="记录收入和支出，更新账户余额")
def record_transaction(income: int = Field(description="收入", default=0), expense: int = Field(description="支出", default=0)) -> Dict:
    """记录今天的收入和支出，更新账户余额"""
    data = load_data()
