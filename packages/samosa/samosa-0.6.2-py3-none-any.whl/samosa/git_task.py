...
def switch_branch(root, name):
    ...
    _main = get_config_key(repo.config.snapshot(), "init.defaultBranch") or "main"
    _upstream_main = f"upstream/{_main}"

    if name == "main" and _main != "main":
        sys.stderr.write(f"The default branch is {_main}.")
        exit(1)
    elif name == _main:
        fetch_all(root)
        shell("checkout", "--track", _upstream_main, "-B", _main)
    elif local_branch := repo.branches.get(name):
        repo.checkout(local_branch)
    else:
        fetch_all(root)
        shell("checkout", "--track", _upstream_main, "-b", name)


def diff_repository(root):
    repo: Repository = Repository(root, flags=0)
    _main = get_config_key(repo.config.snapshot(), "init.defaultBranch") or "main"
    _upstream_main = f"upstream/{_main}"
    shell("--no-pager", "diff", _upstream_main)
...
def fix_and_validate_root(root, quiet_if_valid=False):
    ...
    elif origin_address is None and upstream_address is None:
        errors.append("Origin and upstream remotes not defined!")
    elif origin_address is None and upstream_address:
        add_remote(root, "origin", upstream_address)
    elif upstream_address is None and origin_address:
        add_remote(root, "upstream", origin_address)
    else:
        pass  # all remotes are defined
    ...
    # PRIMARY BRANCH IS SET (OPTIONAL)
    _main = get_config_key(config, "init.defaultBranch") or "main"
    _upstream_main = f"upstream/{_main}"

    # LOCAL MAIN BRANCH EXISTS

    local_main_branch = repo.lookup_branch(_main, GIT_BRANCH_LOCAL)
    if local_main_branch is None:
        local_main_name = "missing"
        errors.append(f"No local branch named {_main}!")
    else:
        local_main_name = _main

    # UPSTREAM MAIN BRANCH EXISTS

    upstream_main_branch = repo.lookup_branch(_upstream_main, GIT_BRANCH_REMOTE)
    if upstream_main_branch is None:
        errors.append(f"No upstream branch named {_main}!")
    ...
    if local_main_branch and local_main_branch.upstream:
        ...
        if local_main_upstream != _upstream_main:
            errors.append(
    ...
    tag_sort = get_config_key(config, "tag.sort")
    if tag_sort is None or tag_sort != "-version:refname":
        errors.append("Fix tag sort with: git config --local tag.sort -version:refname")
