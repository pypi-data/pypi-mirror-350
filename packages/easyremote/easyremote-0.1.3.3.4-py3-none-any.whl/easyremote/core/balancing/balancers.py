    ...
    def select_node(self, 
        ...
        try:
            ...
            try:
                ...
                # We're in an async context, run the coroutine synchronously
                task = asyncio.create_task(self.route_request(function_name, request_context))
                return loop.run_until_complete(task)
            except RuntimeError:
                # No running loop, create a new one
                return asyncio.run(self.route_request(function_name, request_context))
