...
class DistributedComputingGateway(service_pb2_grpc.RemoteServiceServicer, ModernLogger):
    ...
    async def CallWithLoadBalancing(self, request, context):
        ...
        try:
            self.debug(f"Processing load balanced call {call_id} for function '{function_name}'")
            
            # Deserialize arguments
            try:
                args = self._serializer.deserialize(request.args) if request.args else ()
                kwargs = self._serializer.deserialize(request.kwargs) if request.kwargs else {}
            except Exception as e:
            ...
            # Use load balancer to execute function
            try:
                start_time = time.time()
                result = self.execute_function_with_load_balancing(
                    function_name, 
                    {"strategy": strategy}, 
                    *args, 
                    **kwargs
                ...
                self.debug(f"Successfully completed load balanced call {call_id} "
                          f"for function '{function_name}' ({execution_time_ms:.2f}ms)")
                ...
                self.error(f"Load balanced execution failed for call {call_id}: {e}", exc_info=True)
            ...
            self.error(f"Critical error in CallWithLoadBalancing for call {call_id}: {e}", exc_info=True)
    ...
    async def CallDirect(self, request, context):
        ...
        try:
            ...
            try:
                result = self.execute_function(node_id, function_name, *args, **kwargs)
