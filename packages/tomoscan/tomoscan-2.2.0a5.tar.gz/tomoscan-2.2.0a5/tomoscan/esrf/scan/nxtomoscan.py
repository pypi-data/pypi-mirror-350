        ...
        if self.nexus_version is None or self.nexus_version >= 1.5:
            path = "/".join(
                (
                    ...
                    self.nexus_path.nx_sample_paths.X_PIXEL_SIZE,
                )
            )
        else:
            # before nxtomo version 1.5 the 'sample' pixel size was stored in instrument/detector/pixel_size for simplicity
            path = "/".join(
                (
                    self.nexus_path.INSTRUMENT_PATH,
                    self.nexus_path.nx_instrument_paths.DETECTOR_PATH,
    ...
    def sample_y_pixel_size(self) -> float | None:
        ...
        if self.nexus_version is None or self.nexus_version >= 1.5:
            path = "/".join(
                (
                    self.nexus_path.SAMPLE_PATH,
                    self.nexus_path.nx_sample_paths.Y_PIXEL_SIZE,
                )
            )
        else:
            # before nxtomo version 1.5 the 'sample' pixel size was stored in instrument/detector/pixel_size for simplicity
    ...
    def frames(self) -> tuple | None:
        ...
        if self._frames is None:
            ...
            if sequence_number is None:
                ...
                sequence_number = numpy.linspace(
...
class TomoFrame:
    ...
    def __init__(
        ...
        if not isinstance(index, numpy.uint32):
            raise TypeError(
                f"index is expected to be an instance of {numpy.uint32}. Got {type(index)}"
            )
        self._index = index
