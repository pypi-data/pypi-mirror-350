...
class H2oModel:
    ...
    @classmethod
    def from_db(cls, db, model_id, category, version, foreign=True):
    ...
    @staticmethod
    def load_models_from_db(db, name=None, category=None, version=None, foreign=True, limit=6):
        registry = MLRegistry(db, foreign=foreign)  # Use MLRegistry to access the database
        return registry.get_models_ranked(name, category, version)
    ...
    def predict(self, data: pd.DataFrame, join_index='order_id'):
            ...
            p_data, features, target = self.format_class(data.copy(), self)
...
class MLRegistry(Db):
    ...
    def get_models_ranked(self, model_name=None, model_version=None, model_category=None, order='auc desc', limit=6, min_auc=0.8, max_mean_sq=.2):
        whr = ""
        if model_name:
        ...
        qry = f"""
            select distinct a.model_id from (select model_id from {self.schema}.{self.table} {whr}) as a
