...
class H2oModel:
    def __init__(self, model: dict, columns: pd.DataFrame, training_params:pd.DataFrame, performance:dict):
    ...
    @classmethod
    def from_db(cls, db, model_id, category, version):
        """Loads an H2oModel from the database."""
        registry = MLRegistry(db)  # Use MLRegistry to access the database
    ...
    def predict(self, data: pd.DataFrame):
            ...
            h2o_data = h2o.H2OFrame(data[self.features])
...
class MLRegistry(Db):
    ...
    def get_model(self, model_id, category, version):
        model = dict(self.engine.execute(f" select * from {self.schema}.{self.table} where model_id='{model_id}' and model_category='{category}' and model_version='{version}'").fetchone())
        ...
        return H2oModel(model, columns, performance, training_data)
    ...
    def set_model(self,
        ...
        try:
            with open(f"./{model_id}", "rb") as f:  # Open in binary read mode
                binary_model = f.read()
