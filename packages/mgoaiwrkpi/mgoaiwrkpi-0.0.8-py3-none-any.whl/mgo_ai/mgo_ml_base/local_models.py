...
class H2oModel:
    def __init__(self, model: dict, columns, training_params, performance):
        ...
        self.training_params = {v[0]: int(v[1]) for v in training_params.drop(columns='model_id').to_dict('split')['data']}
    ...
    def predict(self, data: pd.DataFrame):
            ...
            data, features, target = self.format_class(data, self)
            # Convert Pandas DataFrame to H2OFrame
            h2o_data = h2o.H2OFrame(data[features])
...
class MLRegistry(Db):
    ...
    def get_model(self, model_id, category, version):
        model = self.engine.execute(f" select * from {self.schema}.{self.table} where model_id='{model_id}' and model_category='{category}' and model_version='{version}'").fetchone()
