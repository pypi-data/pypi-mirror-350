...
if not sys.platform.startswith("win"):
    raise Exception("Cannot run Memory/_platform/_win32.py under non Windows OS")

class MemBlock:
    """A Memory Block class used to define protected sections of RAM (Virtual RAM)"""
    def __init__(self, size:int, access:str, name:str="BLOCK", seek:int=0):
        """Init func
...
class Memory:
    ...
    def write_ram(self, data:bytes, addr:int=None) -> bool:
        """Writes data to a address. If addr is None, writes data at current addr."""
        if addr:
            self.current_addr = addr

        if self._CFBlock(addr, len(data)) == False:
            return False
