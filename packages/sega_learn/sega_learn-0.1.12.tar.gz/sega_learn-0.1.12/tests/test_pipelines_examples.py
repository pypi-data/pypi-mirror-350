...
class TestExamplesPipelines(BaseTest):
...
def load_tests(loader, tests, pattern):
    ...
    example_files = glob.glob(
        os.path.join(os.path.dirname(__file__), "..\\examples\\pipelines\\*.py")
    )
    ...
    for example_file in example_files:
        ...
        if test_name in test_exeptions:
            ...
            setattr(TestExamplesPipelines, test_name, test_func)

    # Load the dynamically generated test cases.
    return loader.loadTestsFromTestCase(TestExamplesPipelines)
