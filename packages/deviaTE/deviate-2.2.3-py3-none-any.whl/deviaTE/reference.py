...
class InputFile:
    ...
    def analyse_coverage(self, force_map: bool = False) -> None:
        ...
        # if multiple mappings for a read: check if they are valid
        self.paf_dict = deviaTE.paf.Paf.validate_recs(paf_dict)
        # identify the internal deletions
        self.int_dels_raw = deviaTE.analyse.get_internal_deletions(paf_dict)
        self.int_dels = deviaTE.analyse.collapse_internal_deletions(int_dels=self.int_dels_raw)
        ...
        if self.conf.args.single_copy_genes:
            self.scg_norm_fac = single_gene_norm_fac(
                scgs=self.conf.args.single_copy_genes,
...
class Reference:
    ...
    def __init__(self,
        ...
        self.annotations = []
        self.int_dels = np.array(int_dels)
        if incr:
    ...
    def _normalize_intdel(self, norm_factor: np.float64 | float) -> None:
        """
        Normalise the internal deletions by given factor
        :param norm_factor: Can be either from rpm or scg
        :return:
        """
        # the 2nd col are the counts of the deletions
        self.int_dels = self.int_dels.astype(float)
        self.int_dels[:, 2] /= norm_factor
    ...
    def normalize_rpm(self, nreads: int) -> None:
        ...
        if not rpm_fac:
            ...
            self._normalize_intdel(norm_factor=rpm_fac)
    ...
    def normalize_scg(self, scg_factor: np.float64) -> None:
        ...
        if not scg_factor:
            ...
            self._normalize_intdel(norm_factor=scg_factor)
    ...
    def write_results(self, infile: Path) -> str:
        ...
        """
        # format the internal deletions
        intdels = self.format_intdels()
    ...
    def format_intdels(self) -> NDArray:
        """
        Format the internal deletions for output
        :return: array of formatted internal deletions
        """
        intdels = np.full(fill_value='0', shape=self.length, dtype=np.dtypes.StrDType)
        for d in self.int_dels:
            if intdels[int(d[0])] == '0':
                intdels[int(d[0])] = f'{d[0]}:{d[1]}:{d[2]}'
            else:
                intdels[int(d[0])] += f',{d[0]}:{d[1]}:{d[2]}'
        return intdels
