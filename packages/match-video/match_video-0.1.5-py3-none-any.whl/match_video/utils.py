...
def write_anchors(
    ...
    # add a dummy anchor as ffmpeg misses the clock assigned to the first anchor
    dummy_anchor = Anchor(-1, 0, 0)
    anchors_with_dummy = [dummy_anchor] + anchors

    for anchor in anchors_with_dummy:
...
def read_anchors(video_path: str) -> List[Anchor]:
    ...
    anchors_with_dummy = [get_anchor(chapter) for chapter in result["chapters"]]
    anchors = [anchor for anchor in anchors_with_dummy if anchor.period != -1]
