...
def build_scalar_reliability(
    x: Layer, n_scorers: int, temperature: float = 1.0
) -> Layer:
    """Build scalar reliability branch (one value per scorer per image)

    Args:
        x: Input layer
        n_scorers: Number of scorers/annotators
        temperature: Temperature parameter for softmax scaling (default: 1.0)
    """
    ...
    x = Dense(n_scorers, name="scalar_reliability_logits")(x)
    x = x / temperature
    x = tf.nn.softmax(x, name="scalar_reliability")
    return x
...
def unet_tgce_scalar(
    ...
    rel_output = build_scalar_reliability(x, n_scorers, temperature)
