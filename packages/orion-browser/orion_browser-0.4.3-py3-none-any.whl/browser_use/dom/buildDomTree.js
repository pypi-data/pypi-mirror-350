      ...
      if (!element || element.nodeType !== Node.ELEMENT_NODE) {
        ...
        if (steps.length > 0 && element.nodeType === Node.ELEMENT_NODE) {
            const step = steps.find(step => {
                ...
                const stepNodeClassName = eventInfo.className || eventInfo.target?.attributes?.class || ''
                ...
                const isMatchClassName = stepNodeClassName && element.className && stepNodeClassName?.includes?.(element.className);
                ...
                // }
                const isMatchDataAttribute = stepDataAttributeKeys.every(v => elementDataAttributeKeys.includes(v))
                if (isMatchURL && isMatchTagName && (isMatchId || isMatchClassName || isMatchDataAttribute) && element.style.display !== 'none') {
                    console.log('recording', step, element)
                    return true;
                }
                return false;
            });
            if (step) {
                ...
                return {
                    type: step.event,
                    desc: desc ? `${step.event} Element: ${desc}` : undefined,
                };
            }
        }
