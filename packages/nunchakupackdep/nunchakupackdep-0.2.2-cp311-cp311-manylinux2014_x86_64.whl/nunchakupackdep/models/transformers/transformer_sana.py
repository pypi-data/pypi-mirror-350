...
from .utils import NunchakupackdepModelLoaderMixin
...
class NunchakupackdepSanaTransformerBlocks(nn.Module):
    def __init__(self, m: QuantizedSanaModel, dtype: torch.dtype, device: str | torch.device):
        super(NunchakupackdepSanaTransformerBlocks, self).__init__()
        self.m = m
        self.dtype = dtype
        self.device = device
    ...
    def forward(
        ...
        nunchakupackdep_encoder_hidden_states = encoder_hidden_states[mask > -9000]
        ...
        return (
            self.m.forward(
                ...
                nunchakupackdep_encoder_hidden_states.to(self.dtype).to(self.device),
...
class NunchakupackdepSanaTransformer2DModel(SanaTransformer2DModel, NunchakupackdepModelLoaderMixin):
    @classmethod
    ...
    def inject_quantized_module(self, m: QuantizedSanaModel, device: str | torch.device = "cuda"):
        self.transformer_blocks = torch.nn.ModuleList([NunchakupackdepSanaTransformerBlocks(m, self.dtype, device)])
        return self
...
def inject_quantized_module(
    ...
    net.transformer_blocks = torch.nn.ModuleList([NunchakupackdepSanaTransformerBlocks(m, net.dtype, device)])
    return net
