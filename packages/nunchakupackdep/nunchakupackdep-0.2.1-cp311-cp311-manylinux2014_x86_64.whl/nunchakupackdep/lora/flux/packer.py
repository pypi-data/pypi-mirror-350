...
class NunchakupackdepWeightPacker(MmaWeightPackerBase):
    ...
    def pack_weight(self, weight: torch.Tensor) -> torch.Tensor:
        ...
        # currently, Nunchakupackdep did not check the boundry of unrolled `k` dimension
        ...
        weight = weight.reshape(
            ...
            self.n_pack_size,  # always 2 in nunchakupackdep
            self.num_n_lanes,  # constant 8
            self.reg_n,  # constant 1
            k_tiles,
            self.num_k_packs,  # 1
            self.k_pack_size,  # always 2 in nunchakupackdep
    ...
    def pack_scale(self, scale: torch.Tensor, group_size: int) -> torch.Tensor:
        ...
        assert scale.dtype in (torch.float16, torch.bfloat16), "currently nunchakupackdep only supports fp16 and bf16."
        n = scale.shape[0]
        # nunchakupackdep load scales all in one access
