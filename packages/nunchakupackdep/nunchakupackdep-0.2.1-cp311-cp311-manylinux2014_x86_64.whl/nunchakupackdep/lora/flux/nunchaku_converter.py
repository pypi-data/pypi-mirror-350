# convert the diffusers lora to nunchakupackdep format
"""Convert LoRA weights to Nunchakupackdep format."""
import logging
import os
...
from .packer import NunchakupackdepWeightPacker
from .utils import is_nunchakupackdep_format, pad
...
def convert_to_nunchakupackdep_transformer_block_lowrank_dict(  # noqa: C901
...
def convert_to_nunchakupackdep_flux_single_transformer_block_lowrank_dict(
    orig_state_dict: dict[str, torch.Tensor],
    extra_lora_dict: dict[str, torch.Tensor],
    converted_block_name: str,
    candidate_block_name: str,
    ...
    return convert_to_nunchakupackdep_transformer_block_lowrank_dict(
...
def convert_to_nunchakupackdep_flux_transformer_block_lowrank_dict(
    orig_state_dict: dict[str, torch.Tensor],
    extra_lora_dict: dict[str, torch.Tensor],
    converted_block_name: str,
    candidate_block_name: str,
...
def convert_to_nunchakupackdep_flux_lowrank_dict(
    ...
    converted: dict[str, torch.Tensor] = {}
    for block_name in tqdm(block_names, dynamic_ncols=True, desc="Converting LoRAs to nunchakupackdep format"):
        if block_name.startswith("transformer_blocks"):
            convert_fn = convert_to_nunchakupackdep_flux_transformer_block_lowrank_dict
        else:
            convert_fn = convert_to_nunchakupackdep_flux_single_transformer_block_lowrank_dict
        update_state_dict(
            converted,
            convert_fn(
                orig_state_dict=orig_state_dict,
...
def to_nunchakupackdep(
    ...
    if isinstance(input_lora, str):
        ...
        tensors = input_lora
    if is_nunchakupackdep_format(tensors):
        logger.debug("Already in nunchakupackdep format, no conversion needed.")
        converted = tensors
    else:
        ...
        converted = convert_to_nunchakupackdep_flux_lowrank_dict(
...
def fuse_vectors(
    ...
    packer = NunchakupackdepWeightPacker(bits=4)
