...
from .utils import NunchakupackdepModelLoaderMixin, pad_tensor
from ..._C import QuantizedFluxModel, utils as cutils
from ...lora.flux.nunchakupackdep_converter import fuse_vectors, to_nunchakupackdep
from ...lora.flux.utils import is_nunchakupackdep_format
...
class NunchakupackdepFluxTransformerBlocks(nn.Module):
    def __init__(self, m: QuantizedFluxModel, device: str | torch.device):
        super(NunchakupackdepFluxTransformerBlocks, self).__init__()
        self.m = m
...
class NunchakupackdepFluxTransformer2dModel(FluxTransformer2DModel, NunchakupackdepModelLoaderMixin):
    @register_to_config
    def __init__(
        self,
        patch_size: int = 1,
        ...
        super(NunchakupackdepFluxTransformer2dModel, self).__init__(
    ...
    def inject_quantized_module(self, m: QuantizedFluxModel, device: str | torch.device = "cuda"):
        ...
        self.transformer_blocks = nn.ModuleList([NunchakupackdepFluxTransformerBlocks(m, device)])
    ...
    def set_attention_impl(self, impl: str):
        ...
        assert isinstance(block, NunchakupackdepFluxTransformerBlocks)
    ...
    def update_lora_params(self, path_or_state_dict: str | dict[str, torch.Tensor]):
        ...
        if not is_nunchakupackdep_format(state_dict):
            state_dict = to_nunchakupackdep(state_dict, base_sd=self._quantized_part_sd)
    ...
    def forward(
        ...
        nunchakupackdep_block = self.transformer_blocks[0]
        encoder_hidden_states, hidden_states = nunchakupackdep_block(
