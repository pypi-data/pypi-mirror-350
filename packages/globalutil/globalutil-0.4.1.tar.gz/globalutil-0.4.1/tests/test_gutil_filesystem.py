...
class TestGUtil(unittest.TestCase):
    ...
    def test_inspect(self):
        ...
        # Test generate_copy_structure to ensure main directory files are included
        temp_folder = os.path.join(self.test_dir, "temp_copy")
        copy_structure = Inspect.generate_copy_structure(self.test_dir, temp_folder)
        
        # Flatten the structure to get all source paths
        all_source_paths = []
        for source_list, _ in copy_structure:
            all_source_paths.extend(source_list)
        
        # Check that the main directory is included (should contain file1.txt and file2.txt)
        self.assertIn(self.test_dir, all_source_paths)
        # Check that subdirectories are also included
        self.assertIn(os.path.join(self.test_dir, "subdir"), all_source_paths)
