...
class DetRec(nn.Module):
    ...
    def live_test(self, conf_thresh=0.5, frame_skip=0, recognition_conf_func=None):
        ...
        saved_faces = []  # Store (embedding, label)
        add_face_mode = False
        name_buffer = ""
        face_id_counter = 0
...
def FaceDetectionWeights():
    return torch.load("FaceDetectionWeights.pth")

def FaceRecognitionWeights():
    return torch.load("FaceRecognitionWeights.pth")
