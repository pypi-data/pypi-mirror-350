...
class UVInstaller:
    ...
    def _run_command_safe(self, cmd, check=True, capture_output=True):
        ...
        """
        try:
            if self.debug or not capture_output:
                # 调试模式或不需要捕获输出时，直接执行
            ...
            if hasattr(e, 'stdout') and e.stdout:
                try:
                    ...
                    try:
                        stdout_text = e.stdout.decode('gbk') if isinstance(e.stdout, bytes) else str(e.stdout)
                    except (UnicodeDecodeError, AttributeError):
            ...
            if hasattr(e, 'stderr') and e.stderr:
                try:
                    ...
                    try:
                        stderr_text = e.stderr.decode('gbk') if isinstance(e.stderr, bytes) else str(e.stderr)
                    except (UnicodeDecodeError, AttributeError):
                        stderr_text = str(e.stderr)
            
            # 重新创建异常，包含解码后的输出
            new_exception = subprocess.CalledProcessError(e.returncode, e.cmd)
            new_exception.stdout = stdout_text
            new_exception.stderr = stderr_text
            raise new_exception
    ...
    def _ensure_uv_available(self):
        ...
        # 检查uv命令是否可用
        try:
            result = self._run_command_safe(["uv", "--version"])
        ...
        for attempt in range(max_retries):
            try:
                ...
                # 安装UV
                install_result = self._run_command_safe(
                    ["pip", "install", "-i", self.MIRROR_URL, "uv"]
                )
                print("UV安装成功")
                
                # 验证安装
                verify_result = self._run_command_safe(["uv", "--version"])
    ...
    def _create_uv_env(self):
        ...
        for attempt in range(max_retries):
            try:
                ...
                result = self._run_command_safe(cmd, capture_output=(not self.debug))
