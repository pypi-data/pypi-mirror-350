...
fh.close()
print("{0} files found in {1}".format(len(infilenames), args.filename_list))
...
if args.copy:
    ...
    if not os.path.lexists(datapath):
        print('Creating {0}'.format(datapath))
    ...
    for n in infilenames:
        fh = fits.open(n)
        fw = fits.open(n.replace('.fits', '.weight.fits'))
        hd = fh[0].header
        for iext in args.extensions:
            # copy the image
            fhout = fits.HDUList([fh[0], fh[iext]])
...
if args.mosaic is not None:
    ...
    for n in rnames:
        ...
        bgvals.append(fits.getval(n, 'BACKMEAN'))
    ...
    fh = fits.open(mosaic_file)
    mosaicim = fh[0].data
    mosaicim_wght = fits.getdata(mosaic_weightfile)
    ...
    exptot = sum([fits.getval(datapath + n, 'EXPTIME') for
