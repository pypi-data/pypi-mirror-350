...
try:
    ...
    print("Import Error! Please install the required packages.")
    raise e
...
try:
    ...
    if openai_version >= pkg_resources.parse_version("1.0.0"):
        ...
        _RETRY_ERRORS: Tuple = (  # type: ignore[no-redef]
            openai.APIError,  # pylint: disable=no-member
            openai.RateLimitError,  # pylint: disable=no-member
            openai.APIConnectionError,  # pylint: disable=no-member
            openai.Timeout,  # pylint: disable=no-member
        )

except ImportError as e:
    print(
        "In order to use qa, please install the 'qa_generation' extra of azure-ai-generative"
    )
    raise e

_TEMPLATES_DIR = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "prompt_templates/en"
)

logger = logging.getLogger(__name__)

_DEFAULT_AOAI_VERSION = "2024-12-01-preview"
_MAX_RETRIES = 7
...
async def _completion_with_retries_async(*args, **kwargs):
    ...
    while True:
        try:
            ...
            secs = 2**n
            logger.warning(
                "Retrying after %ss. API call failed due to %s: %s",
                secs,
                _re.__class__.__name__,
                _re,
            )
...
class QADataGenerator:
    ...
    _PARSING_ERR_UNEQUAL_Q_AFTER_MOD = (
        "Parsing error: Unequal question count after modification"
    )
    ...
    def __init__(self, model_config: Dict):
        ...
        self._chat_completion_params = dict(
            ...
            api_type=(
                ...
                else os.getenv("OPENAI_API_TYPE", "azure")
            ),
            api_version=(
    ...
    def _get_messages_for_qa_type(
        self, qa_type: QAType, text: str, num_questions: int
    ) -> List:
    ...
    def _modify_conversation_questions(self, questions) -> Tuple[List[str], Dict]:
        ...
        assert len(modified_questions) == len(
            questions
        ), self._PARSING_ERR_UNEQUAL_Q_AFTER_MOD
    ...
    def export_to_file(
        ...
        if output_format == OutputStructure.PROMPTFLOW:
            ...
            if qa_type == QAType.CONVERSATION and not (
                ...
                raise Exception(
                    "Field mapping for Promptflow output with Conversation must contain following keys: "
                    + keys
                )
            ...
            for qs_and_as in results:
                ...
                for question, answer in qs_and_as:
                    ...
                    data_dict[answer_key].append(
                        answer
                    )  # Consider generated answer as the ground truth
        else:
            for qs_and_as in results:
                chat_history = []
                for question, answer in qs_and_as:
    ...
    @distributed_trace
    def generate(
        self, text: str, qa_type: QAType, num_questions: Optional[int] = None
    ) -> Dict:
        ...
        content, token_usage = _completion_with_retries(
            messages=self._get_messages_for_qa_type(
                qa_type, text, validated_num_questions
            ),
            **self._chat_completion_params,
        )
    ...
    async def _modify_conversation_questions_async(
        self, questions
    ) -> Tuple[List[str], Dict]:
    ...
    async def generate_async(
        ...
        assert len(questions) == len(answers), self._PARSING_ERR_UNEQUAL_QA
        if qa_type == QAType.CONVERSATION:
            questions, token_usage2 = await self._modify_conversation_questions_async(
                questions
            )
            token_usage = self._merge_token_usage(token_usage, token_usage2)
        return {
...
class CustomQADataGenerator(QADataGenerator):
    """
    Create a custom QA data generator by providing the path to the templates directory.
    """
...
def convert_to_oai_format(
    qa_pair,
    system_prompt_msg="You're an AI assistant that guides a user to the location of your CS center",
):
    """
...
def generate_qas(
    ...
    qa_generator: Union[QADataGenerator, CustomQADataGenerator],
    qa_type: QAType = QAType.LONG_ANSWER,
    num_questions: int = 3,
    concurrency: int = 3,
) -> List[List[Dict[str, str]]]:
    """
    Generates question-answer pairs for a list of input texts using Azure GenAI QA generator.

    Args:
        input_texts (List[str]): List of texts to generate QAs for.
        qa_generator (Union[QADataGenerator, CustomQADataGenerator]): QA generator instance.
        qa_type (QAType): Type of QA to generate (e.g., LONG_ANSWER, SHORT_ANSWER). Default is LONG_ANSWER.
        num_questions (int): Number of questions to generate per text. Default is 3.
        concurrency (int): Number of concurrent calls to the generator. Default is 3.

    Returns:
        List[List[Dict[str, str]]]: A list of lists containing question-answer pairs for each input text.
    """
    ...
    async def generate_async(text: str) -> Dict:
        async with sem:
            result = await qa_generator.generate_async(
                text=text,
                qa_type=qa_type,
                num_questions=num_questions,
            )
            # print(f"Task completed for input: {text}")
            return result

    async def main():
        tasks = [generate_async(text) for text in input_texts]
    ...
    # Run the event loop in the current context
    return (
        asyncio.run(main())
        if not asyncio.get_event_loop().is_running()
        else asyncio.get_event_loop().create_task(main())
    )  # type: ignore
