...
class LLMService:
    ...
    def _ensure_github_client(self):
        """Ensure GitHub Copilot API key is available, prompt for key if needed."""
        github_key = config.get_github_api_key()
        if not github_key:
            github_key = config.prompt_for_api_key("github")
            config.set_github_api_key(github_key)
        self.github_api_key = github_key
    ...
    def _generate_with_github_models(
        ...
        """Generate code using GitHub Models endpoint with OpenAI client."""
        import os
        from openai import OpenAI
        import gocodeo_cli.utils.config as config
    ...
    def generate_code(
        ...
        - إذا كان النموذج يبدأ بـ 'openai/', 'github-model', 'gpt-4.1', 'gpt-3.5' سيتم استخدام GitHub Models endpoint (https://models.github.ai/inference)
        - إذا كان النموذج يبدأ بـ 'github-copilot', 'github/', 'copilot' سيتم استخدام GitHub Copilot API
        - باقي النماذج: OpenAI, Anthropic, Gemini
        """
        import re
        ...
        try:
            ...
            if (
                ...
                model.startswith("github-model") or
                re.match(r"gpt-(3|4)", model)
            ):
                response = self._generate_with_github_models(prompt, model, system_prompt)
            # GitHub Copilot endpoint
            elif (
                model.startswith("github-copilot") or
                model.startswith("github/") or
                model.startswith("copilot")
            ):
                response = self._generate_with_github_copilot(prompt, model, system_prompt)
            # OpenAI
    ...
    def _generate_with_github_copilot(
        self,
        prompt: str,
        model: str,
        system_prompt: Optional[str] = None,
    ) -> str:
        """Generate code using GitHub Copilot API (experimental)."""
        import requests
        ...
        data = {
            "model": model if model else "gpt-4-copilot",
            "messages": [
        ...
        if response.status_code != 200:
            raise ValueError(f"GitHub Copilot API error: {response.status_code} {response.text}")
        result = response.json()
        # GitHub Copilot API returns completions in a similar structure
