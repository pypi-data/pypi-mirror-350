...
class DocFormerEditor:
    def __init__(self, root):
        ...
        self.processed_markers = {
            'images': {},  # 格式: {line_number: {'start': start, 'end': end, 'path': path}}
            'files': {}    # 格式同上
        }
    ...
    def insert_image(self, line_number, start, end, img_path):
        try:
            img = Image.open(img_path)
            img.thumbnail((300, 300))
            photo = ImageTk.PhotoImage(img)
            
            img_frame = tk.Frame(self.text_editor, bd=1, relief=tk.SUNKEN)
            img_label = tk.Label(img_frame, image=photo)
            img_label.image = photo
            img_label.pack(padx=2, pady=2)
            
            pos = f"{line_number}.{start}"
            self.text_editor.window_create(pos, window=img_frame)
            self.text_editor.delete(f"{line_number}.{start}", f"{line_number}.{end+1}")
            return True
            
        except Exception as e:
            print(f"图片加载错误: {e}")
            error_text = f"[图片加载失败: {os.path.basename(img_path)}]"
            self.text_editor.insert(f"{line_number}.{start}", error_text)
            self.text_editor.delete(f"{line_number}.{start}", f"{line_number}.{end+1}")
            return False

    def insert_file(self, line_number, start, end, file_path):
        if not os.path.exists(file_path):
            error_text = f"[文件不存在: {file_path}]"
            self.text_editor.insert(f"{line_number}.{start}", error_text)
            self.text_editor.delete(f"{line_number}.{start}", f"{line_number}.{end+1}")
            return False
            
        file_name = os.path.basename(file_path)
        file_frame = tk.Frame(self.text_editor)
        ...
        file_link = tk.Label(file_frame, text=f"{icon} {file_name}",
                           fg='blue', cursor='hand2')
        file_link.pack()
        
        if hasattr(self, 'open_file'):
            file_link.bind('<Button-1>', lambda e, p=file_path: self.open_file(p))
        else:
            def open_file(path):
                try:
            ...
            file_link.bind('<Button-1>', lambda e, p=file_path: open_file(p))
        
        pos = f"{line_number}.{start}"
        self.text_editor.window_create(pos, window=file_frame)
        self.text_editor.delete(f"{line_number}.{start}", f"{line_number}.{end+1}")
        return True

    def clear_marker(self, marker_type, line_num):
        """清除指定类型的标记和对应的显示框"""
        if line_num in self.processed_markers[marker_type]:
            # 移除显示框
            marker_info = self.processed_markers[marker_type][line_num]
            if 'display_id' in marker_info:
                self.text_editor.window_delete(marker_info['display_id'])
            # 从已处理标记中移除
            del self.processed_markers[marker_type][line_num]
    ...
    def on_backspace(self, event):
        ...
        # 检查是否在图片标记处
        for marker_type in ['images', 'files']:
            if line_num in self.processed_markers[marker_type]:
                marker_info = self.processed_markers[marker_type][line_num]
                cursor_col = int(cursor_pos.split('.')[1])
                if marker_info['start'] <= cursor_col <= marker_info['end']:
                    # 在标记处按Backspace，清除标记和显示框
                    self.clear_marker(marker_type, line_num)
                    return "break"  # 阻止默认Backspace行为
        
        return None  # 允许默认Backspace行为
    ...
    def update_font_sizes(self):
        ...
        # 清除已删除或修改的标记
        for marker_type in ['images', 'files']:
            for line_num, marker_info in list(self.processed_markers[marker_type].items()):
                line_content = self.text_editor.get(f"{line_num}.0", f"{line_num}.end")
                if not (marker_info['start'] < len(line_content) and 
                       line_content[marker_info['start']:marker_info['end']+1] == marker_info['marker']):
                    # 标记已被修改或删除，清除对应的显示框
                    self.clear_marker(marker_type, line_num)
        ...
        # 配置蓝色编号标签
        self.text_editor.tag_config('blue_number', foreground='blue')
        ...
        for line in lines:
            ...
            if re.match(r'^\d+\.\s', line):
                self.text_editor.tag_add('blue_number', line_start, f"{line_number}.{len(line.split()[0])}")
            
                            # 处理图片引用 [img:path]
                if '[img:' in line and ']' in line:
                    start = line.index('[img:')
                    end = line.index(']', start)
                    img_path = line[start+5:end]
                    marker = line[start:end+1]
                    
                    # 检查是否已经处理过这个标记
                    if line_number in self.processed_markers['images']:
                        marker_info = self.processed_markers['images'][line_number]
                        if (marker_info['start'] == start and 
                            marker_info['end'] == end and
                    ...
                    try:
                        ...
                        img_label.pack(padx=5, pady=5)
                                                ...
                                                # 插入图片到文本
                        pos = f"{line_number}.{start}"
                        print(f"在位置 {pos} 插入图片")  # 调试信息
                        try:
                            # 确保标记位置正确
                            line_content = self.text_editor.get(f"{line_number}.0", f"{line_number}.end")
                            if start >= len(line_content):
                                print(f"标记位置超出范围: {start} >= {len(line_content)}")
                                continue
                            
                            # 创建并插入图片显示框
                            display_id = self.text_editor.window_create(f"{line_number}.{start}", window=img_frame)
                            print(f"图片显示框创建成功，ID: {display_id}")
                        
                            # 强制刷新显示
                            self.text_editor.update_idletasks()
                            self.text_editor.see(f"{line_number}.{start}")
                        except Exception as e:
                            print(f"图片显示框创建失败: {e}")
                            error_text = f"[图片显示失败: {os.path.basename(img_path)}]"
                            self.text_editor.insert(f"{line_number}.{start}", error_text)
                            self.text_editor.tag_add('error', f"{line_number}.{start}", f"{line_number}.{start+len(error_text)}")
                            continue
                        
                        # 记录已处理的标记
                        self.processed_markers['images'][line_number] = {
                            'start': start,
                            'end': end,
                            'marker': marker,
                            'path': img_path,
                    ...
                    marker = line[start:end+1]
                                ...
                                # 检查是否已经处理过这个标记
                    if line_number in self.processed_markers['files']:
                        marker_info = self.processed_markers['files'][line_number]
                        if (marker_info['start'] == start and 
                            marker_info['end'] == end and
                            marker_info['marker'] == marker):
                            continue
                    
                    print(f"尝试加载文件: {file_path}")  # 调试信息
                    ...
                    file_name = os.path.basename(file_path)
                                ...
                                # 创建可点击的文件显示框架
                    file_frame = tk.Frame(self.text_editor, bd=1, relief=tk.RAISED)
                    ...
                    if hasattr(self, 'open_file'):
                        ...
                        file_link.bind('<Button-1>', lambda e, path=file_path: open_file(path))
                    
                    # 插入文件组件到文本
                    pos = f"{line_number}.{start}"
                    print(f"在位置 {pos} 插入文件")  # 调试信息
                    try:
                        # 确保标记位置正确
                        line_content = self.text_editor.get(f"{line_number}.0", f"{line_number}.end")
                        if start >= len(line_content):
                        ...
                        # 创建并插入文件显示框
                        display_id = self.text_editor.window_create(f"{line_number}.{start}", window=file_frame)
                        print(f"文件显示框创建成功，ID: {display_id}")
                        ...
                        print(f"文件显示框创建失败: {e}")
                        error_text = f"[文件显示失败: {os.path.basename(file_path)}]"
                        self.text_editor.insert(f"{line_number}.{start}", error_text)
                        self.text_editor.tag_add('error', f"{line_number}.{start}", f"{line_number}.{start+len(error_text)}")
                        continue
                    
                    # 记录已处理的标记
                    self.processed_markers['files'][line_number] = {
                        'start': start,
                        'end': end,
                        'marker': marker,
                        'path': file_path,
