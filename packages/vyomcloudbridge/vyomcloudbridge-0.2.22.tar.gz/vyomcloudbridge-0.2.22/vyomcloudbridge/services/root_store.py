...
class RootStore(ServiceAbstract):
    """
    ...
    def __init__(self):
        ...
        self.connection = None  # rabbit mq connection
        self.channel = None
    ...
    def get_data(self, key: str) -> Optional[Any]:
        ...
        try:
            ...
            method_frame, _, body = self.channel.basic_get(queue=key, auto_ack=False)
    ...
    def delete_data(self, key: str):
        ...
        try:
            ...
            while True:
                method_frame, _, _ = self.channel.basic_get(queue=key, auto_ack=True)
                if not method_frame:
                    break
...
def _ensure_queue(channel, key: str):
    """
    Ensure that a queue exists for the given key.
...
def get_data(key: str, rabbitmq_url: str = DEFAULT_RABBITMQ_URL) -> Optional[Any]:
    ...
    try:
        ...
        # Setup connection
        connection, channel = _setup_connection(rabbitmq_url)

        # Ensure queue exists
        _ensure_queue(channel, key)

        # Get data from queue
        method_frame, _, body = channel.basic_get(queue=key, auto_ack=False)
...
def delete_queue_data(channel, key: str):
    """
