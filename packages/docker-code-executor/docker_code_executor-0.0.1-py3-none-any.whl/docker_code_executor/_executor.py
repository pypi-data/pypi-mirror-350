# File based from: https://github.com/microsoft/autogen/blob/main/python/packages/autogen-ext/src/autogen_ext/code_executors/docker/_docker_code_executor.py
...
from ._func_with_reqs import FunctionWithRequirements, FunctionWithRequirementsStr, build_python_functions_file
from ._types import CodeBlock, CommandLineCodeResult
from ._utils import get_file_name_from_content, lang_to_cmd, silence_pip

A = ParamSpec("A")


async def _wait_for_ready(container: Any, timeout: int = 60, stop_time: float = 0.1) -> None:  # noqa: ASYNC109
    elapsed_time = 0.0
...
$functions"""
    ...
    def __init__(
        ...
        *,
        ...
        if container_name is None:
            self.container_name = f"ks-code-exec-{uuid.uuid4()}"
        else:
    ...
    async def _setup_functions(self, cancellation_token: CancellationToken) -> None:
        ...
        if len(required_packages) > 0:
            ...
            result: CommandLineCodeResult = await self._execute_code_dont_check_setup(
        ...
        exec_result: CommandLineCodeResult = await self._execute_code_dont_check_setup(
    ...
    async def start(self) -> None:
        ...
        assert self._container is not None, "Container creation failed, _container is None."
