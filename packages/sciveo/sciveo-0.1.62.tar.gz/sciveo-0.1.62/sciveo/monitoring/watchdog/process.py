...
class ProcessWatchDogDaemon(BaseWatchDogDaemon):
  ...
  def get_threads(self, process):
    ...
    for t in process.threads():
      ...
      if t.id not in self.info_threads:
        self.info_threads[t.id] = {
          "id": t.id,
        ...
        if cpu_time != self.info_threads[t.id]["last_cpu"]:
          self.info_threads[t.id]["last_active"] = current_time
        self.info_threads[t.id]["last_cpu"] = cpu_time

  def check_threads(self, pid):
    current_time = time.time()
    for tid, thr in self.info_threads.items():
