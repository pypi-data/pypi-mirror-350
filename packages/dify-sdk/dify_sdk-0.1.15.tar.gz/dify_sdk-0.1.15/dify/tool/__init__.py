...
from .schemas import ToolProvider, Tool
...
class DifyTool:
    ...
    async def list_api_tools(self) -> List[Dict[str, Any]]:
        ...
        Returns:
            List[Dict[str, Any]]: API工具列表

        Raises:
            httpx.HTTPStatusError: 当API请求失败时抛出
        """
        # 发送GET请求获取API工具列表
        response_data = await self.admin_client.get("/workspaces/current/tools/api")

        # 直接返回原始数据
        return response_data

    async def list_builtin_tools(self) -> List[Dict[str, Any]]:
        """获取内置工具列表
        ...
        """
        # 发送GET请求获取内置工具列表
        response_data = await self.admin_client.get("/workspaces/current/tools/builtin")

        # 直接返回原始数据
        return response_data

    async def list_workflow_tools(self) -> List[Dict[str, Any]]:
        """获取工作流工具列表

        Returns:
            List[Dict[str, Any]]: 工作流工具列表

        Raises:
            httpx.HTTPStatusError: 当API请求失败时抛出
        """
        # 发送GET请求获取工作流工具列表
        response_data = await self.admin_client.get("/workspaces/current/tools/workflow")
