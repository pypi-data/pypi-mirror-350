        ...
        for ep in eps:
            ...
            try:
                ...
                if not issubclass(plugin_class, FastPluggyBaseModule):
                    logger.warning(f"🚫 Skipped entry '{ep.name}': Not a FastPluggyBaseModule")
                    continue
                ...
                plugin_state = PluginState(
                    ...
                    path=plugin_path,
                    enabled=self.plugin_states.get(plugin_instance.module_name, True),
                    initialized=True,
                )
                plugin_state.process()
                self.modules[plugin_instance.module_name] = plugin_state
                logger.success(f"✅ EntryPoint plugin loaded: {plugin_instance.module_name} from {ep.value}")

            except Exception as e:
                logger.exception(f"❌ Failed to load plugin from entry point '{ep.name}': {e}")
                plugin_state = PluginState(
                    module_type="plugin-pkg",
                    plugin=FastPluggyBaseModule(module_name=ep.name),
                    enabled=False,
                    error=[str(e)],
                ...
                plugin_state.plugin.module_menu_icon = "fas fa-exclamation-triangle"
                plugin_state.process()
                self.modules[ep.name] = plugin_state
