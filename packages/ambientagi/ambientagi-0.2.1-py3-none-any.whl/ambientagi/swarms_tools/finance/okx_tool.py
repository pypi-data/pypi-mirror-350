...
class OKXAPI:
    ...
    def fetch_coin_data(
        ...
        logger.info(f"Fetching coin data for: {coin_symbols or 'all available coins'}")

        try:
            response = requests.get(endpoint, params=params, timeout=10)
            response.raise_for_status()
        except requests.RequestException as e:
            logger.error(f"Failed to fetch coin data from OKX API: {e}")
            raise
        ...
        if data.get("code") != "0":
            logger.error(f"Error from OKX API: {data.get('msg', 'Unknown error')}")
            raise ValueError(f"OKX API error: {data.get('msg', 'Unknown error')}")

        filtered_data = OKXAPI._filter_data(data["data"], coin_symbols)
    ...
    def _filter_data(
        ...
        filtered_data = {
            coin["instId"]: coin for coin in data if coin["instId"] in coin_symbols
        }
        if not filtered_data:
            logger.warning(f"No data found for specified coin symbols: {coin_symbols}")
        return filtered_data
