...
def tool_chainer(tools: List[Callable[[], Any]], parallel: bool = True) -> str:
    """
    ...
    if parallel:
        ...
        with ThreadPoolExecutor() as executor:
            future_to_tool = {executor.submit(tool): tool for tool in tools}

            for future in as_completed(future_to_tool):
                tool = future_to_tool[future]
                try:
                    result = future.result()
                    logger.info(f"Tool {tool.__name__} executed successfully")
                    results.append(
                        {
                            "tool": tool.__name__,
                            "status": "success",
