...
def json_str_to_pydantic_model(json_str: str, model: BaseModel) -> BaseModel:
...
def pydantic_model_to_json_str(model: BaseModel, indent: int, *args, **kwargs) -> str:
    """
...
def dict_to_pydantic_model(dictionary: dict, model: BaseModel) -> BaseModel:
...
def tool_schema_to_str(tool_schema: BaseModel = None, *args, **kwargs) -> str:
    """Convert a tool schema to a string"""
    out = base_model_to_openai_function(tool_schema)
    return str(out)


def tool_schemas_to_str(tool_schemas: List[BaseModel] = None, *args, **kwargs) -> str:
...
def list_str_to_pydantic_model(list_str: List[str], model: BaseModel) -> BaseModel:
