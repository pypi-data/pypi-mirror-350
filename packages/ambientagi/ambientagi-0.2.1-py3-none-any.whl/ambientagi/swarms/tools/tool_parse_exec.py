...
def parse_and_execute_json(
    ...
    if parse_md:
        try:
            code_blocks = re.findall(r"```(?:json)?\s*([\s\S]*?)```", json_string)
            if code_blocks and len(code_blocks) > 1:
                ...
                def process_json_block(json_block: str) -> dict:
                    try:
                        ...
                        for function_data in function_list:
                            ...
                            for attempt in range(max_retries):
                                try:
                                    ...
                                    block_results[function_name] = str(result)
                                    logger.info(f"Result for {function_name}: {result}")
                                    break
                        ...
                        logger.error(f"Failed to process JSON block: {e}")
                        return {"error": f"Failed to process block: {str(e)}"}

                combined_results = {}
                for idx, block in enumerate(code_blocks, start=1):
                    combined_results[f"block_{idx}"] = process_json_block(block)
                return json.dumps({"results": combined_results}, indent=4)
            elif code_blocks:
    ...
    try:
        ...
        if not json_string:
            raise ValueError("JSON string is empty after stripping whitespace")
