...
class SwarmConfig(BaseModel):
    ...
    def validate_swarm_type(cls, v):
        ...
        if v not in valid_types:
            raise ValueError(f"Swarm type must be one of: {valid_types}")
        return v
...
class YAMLConfig(BaseModel):
    ...
    model_config = {"extra": "forbid"}  # Prevent additional fields not in the model


def load_yaml_safely(yaml_file: str = None, yaml_string: str = None) -> Dict:
    ...
    try:
        if yaml_string:
            ...
            raise ValueError("Either yaml_file or yaml_string must be provided")
...
def create_agent_with_retry(agent_config: Dict, model: LiteLLM) -> Agent:
    ...
    try:
        ...
        for idx, agent_config in enumerate(config["agents"], 1):
            ...
            if "model_name" in agent_config:
                ...
                model_instance = LiteLLM(model_name=agent_config["model_name"])
            else:
                model_name = "gpt-4"
                logger.info(f"No model specified, using default: {model_name}")
                model_instance = LiteLLM(model_name=model_name)

            agent = create_agent_with_retry(agent_config, model_instance)
        ...
        if "swarm_architecture" in config:
            ...
            try:
                ...
                swarm_config = SwarmConfig(**config["swarm_architecture"])
                ...
                logger.info(f"SwarmRouter '{swarm_config.name}' created successfully.")
            except Exception as e:
        ...
        if return_type in ("run_swarm", "swarm"):
            ...
            try:
                ...
                return swarm_router.run(config["swarm_architecture"]["task"])
            except Exception as e:
