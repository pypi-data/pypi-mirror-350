...
def extract_code_blocks_with_language(markdown_text: str):
    ...
    for language, content in matches:
        ...
        code_blocks.append({"language": language, "content": content.strip()})

    return code_blocks


def extract_code_from_markdown(markdown_text: str, language: str = None):
    """
    ...
    if language:
        code_blocks = [
            block["content"] for block in code_blocks if block["language"] == language
        ]
    else:
        code_blocks = [block["content"] for block in code_blocks]  # Include all blocks
