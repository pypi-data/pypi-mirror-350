...
def showcase_available_agents(
        ...
        for idx, agent in enumerate(agents):
            if isinstance(agent, Agent):
                ...
                description = getattr(
                    agent,
                    "description",
                    getattr(agent, "system_prompt", "Unknown description"),
                )
                desc = truncate(description, 50)
                output.append(f"| {idx + 1} | {agent_name} | {desc} |")
            else:
                output.append(f"| {idx + 1} | {agent} | Unknown description |")
        return "\n".join(output)
    ...
    if description:
        output.append(f"  <description>{truncate(description)}</description>")
    for idx, agent in enumerate(agents):
        ...
        if isinstance(agent, Agent):
            ...
            output.append("    <description>Unknown description</description>")
        output.append("  </agent>")
    output.append("</agents>")
