...
class SwarmRunMetadata(BaseModel):
    run_id: str = Field(default_factory=lambda: f"spreadsheet_swarm_run_{uuid_hex}")
    name: str
    description: str
    agents: List[str]
...
class SpreadSheetSwarm(BaseSwarm):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.save_file_path = f"spreadsheet_swarm_run_id_{uuid_hex}.csv"
        # --------------- NEW CHANGE END ---------------
    ...
    async def _load_from_csv(self):
        ...
        try:
            ...
            logger.info(f"Loading agent configurations from {csv_path}")
            ...
            async with aiofiles.open(csv_path, mode="r") as file:
                ...
                for row in csv_reader:
                    ...
                    new_agent = Agent(
                        ...
                        # output_type="str",
                        stopping_token=(
                            row["stopping_token"] if "stopping_token" in row else None
                        ),
                    )
            ...
            self.metadata.agents = [agent.name for agent in self.agents]
        ...
        for agent in self.agents:
            ...
            if config:
                for _ in range(self.max_loops):
                    tasks.append(
                        asyncio.to_thread(self._run_agent_task, agent, config.task)
                    )
    ...
    def _run_agent_task(self, agent, task, *args, **kwargs):
        ...
        try:
            ...
            logger.error(f"Error running task for {agent.agent_name}: {e}")
