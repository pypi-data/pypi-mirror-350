...
class MetadataSchema(BaseModel):
    swarm_id: Optional[str] = Field(..., description="Unique ID for the run")
    name: Optional[str] = Field("RoundRobinSwarm", description="Name of the swarm")
    task: Optional[str] = Field(..., description="Task or query given to all agents")
    ...
    max_loops: Optional[int] = Field(1, description="Maximum number of loops to run")
...
class RoundRobinSwarm(BaseSwarm):
    ...
    def _execute_agent(self, agent: Agent, task: str, *args, **kwargs) -> str:
        """Execute a single agent with retries and error handling"""
        try:
            logger.info(f"Running Agent {agent.agent_name} on task: {task}")
            result = agent.run(task, *args, **kwargs)
            self.output_schema.agent_outputs.append(agent.agent_output)
            return result
        except Exception as e:
            logger.error(f"Error executing agent {agent.agent_name}: {str(e)}")
            raise
    ...
    def run(self, task: str, *args, **kwargs):
        ...
        try:
            ...
            for loop in range(self.max_loops):
                logger.debug(f"Starting loop {loop + 1}/{self.max_loops}")
                ...
                if self.callback:
                    logger.debug(f"Executing callback for loop {loop + 1}")
                    try:
