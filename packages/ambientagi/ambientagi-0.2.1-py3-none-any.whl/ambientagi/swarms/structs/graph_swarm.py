...
class SwarmMemory:
    ...
    def store_execution(self, task: str, result: SwarmOutput):
        ...
        try:
            ...
            metadata = {
                ...
                "agent_sequence": json.dumps([name for name in result.outputs.keys()]),
            ...
            logger.error(f"Failed to store execution in memory: {str(e)}")
    ...
    def get_similar_executions(self, task: str, limit: int = 5):
        ...
        try:
                ...
                executions.append(
                    {
                        ...
                        "agent_sequence": json.loads(metadata["agent_sequence"]),
            ...
            logger.error(f"Failed to retrieve similar executions: {str(e)}")
            return []
    ...
    def get_optimal_sequence(self, task: str) -> Optional[List[str]]:
        ...
        successful_execs = [ex for ex in similar_executions if ex["success"]]
...
class GraphSwarm:
        ...
        try:
            ...
            for item in agents:
                ...
                if not isinstance(agent, Agent):
                    raise ValueError(f"Expected Agent object, got {type(agent)}")
    ...
    def _validate_graph(self):
        ...
        if not nx.is_directed_acyclic_graph(self.graph):
            ...
            raise ValueError(f"Agent dependency graph contains cycles: {cycles}")
    ...
    async def _execute_agent(
        ...
        try:
            ...
            full_prompt = self._build_agent_prompt(agent_name, task, context)
            ...
            return AgentOutput(
                ...
                metadata={
                    "task": task,
                    "context": context,
                    "position_in_workflow": list(nx.topological_sort(self.graph)).index(
                        agent_name
                    ),
                },
            )
    ...
    async def execute(self, task: str) -> SwarmOutput:
        ...
        try:
            ...
            similar_executions = self.memory.get_similar_executions(task, limit=3)
            optimal_sequence = self.memory.get_optimal_sequence(task)

            # Get base execution order
            base_execution_order = list(nx.topological_sort(self.graph))

            # Determine final execution order
            if optimal_sequence and all(
                agent in base_execution_order for agent in optimal_sequence
            ):
                logger.info(f"Using optimal sequence from memory: {optimal_sequence}")
            ...
            if similar_executions:
                ...
                if best_execution["success"]:
                    historical_context = {
                        ...
                        "execution_time": best_execution["execution_time"],
                ...
                try:
                    ...
                    agent_context = {
                        "dependencies": {
                            ...
                            for dep in self.graph.predecessors(agent_name)
                            if dep in outputs
                        },
                    ...
                    output = await self._execute_agent(agent_name, task, agent_context)
            ...
            result = SwarmOutput(
                ...
                metadata={
                    "task": task,
                    "used_optimal_sequence": optimal_sequence is not None,
                    "similar_executions_found": len(similar_executions),
    ...
    def _extract_success_patterns(self, similar_executions: List[Dict]) -> Dict:
        ...
        if successful_execs:
            patterns = {
                "common_sequences": self._find_common_sequences(successful_execs),
                "avg_execution_time": sum(
                    ex["execution_time"] for ex in successful_execs
                )
                / len(successful_execs),
                "successful_strategies": self._extract_strategies(successful_execs),
            }
    ...
    def _attempt_recovery(
        ...
        for execution in similar_executions:
            if execution["success"] and failed_agent in execution["outputs"]:
                historical_output = execution["outputs"][failed_agent]

                return AgentOutput(
                    agent_name=failed_agent,
                    output=historical_output["output"],
                    execution_time=historical_output["execution_time"],
                    metadata={
    ...
    async def _store_execution_async(self, task: str, result: SwarmOutput):
        """Asynchronously store execution in memory."""
        try:
            await asyncio.to_thread(self.memory.store_execution, task, result)
        except Exception as e:
            logger.error(f"Failed to store execution in memory: {str(e)}")
