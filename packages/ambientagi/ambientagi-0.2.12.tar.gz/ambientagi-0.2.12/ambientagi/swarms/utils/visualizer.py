...
class SwarmVisualizationRich:
    ...
    def _create_unified_info_panel(self) -> Panel:
        ...
        description_text = Text(self.swarm_metadata.description or "", style="italic")
        description_text.wrap(self.console, width=60, overflow="fold")

        metadata_table.add_row("Swarm Name", self.swarm_metadata.name or "N/A")
        metadata_table.add_row("Description", description_text)
        metadata_table.add_row("Version", self.swarm_metadata.version or "N/A")
        metadata_table.add_row("Total Agents", str(total_agents))
        metadata_table.add_row("Author", self.swarm_metadata.author or "N/A")
    ...
    def _create_outputs_panel(self) -> Panel:
        ...
        for msg in all_messages:
            ...
            message_text.append(f"{msg['agent']}: ", style="bold cyan")
            ...
            while True:
                ...
                def process_agent_streams(agent: Agent):
                    while not agent.output_stream.empty():
                        ...
                        asyncio.create_task(self.stream_output(agent, new_output))
                    for child in getattr(agent, "children", []):
