...
from swarms.telemetry.main import capture_system_data, log_agent_data
...
class Prompt(BaseModel):
    ...
    name: str = Field(default="prompt", description="Name of your prompt")
    ...
    def initialize_history(cls, v, values):
        ...
        """
        if not v:
            return [values["content"]]  # Store initial version in history
        return v
    ...
    def edit_prompt(self, new_content: str) -> None:
        ...
        if new_content == self.content:
            ...
            raise ValueError("New content must be different from the current content.")
    ...
    def save_to_storage(self) -> None:
        ...
        raise NotImplementedError("Persistent storage integration is required.")

    def load_from_storage(self, prompt_id: str = uuid.uuid4().hex) -> None:
        """
    ...
    def _autosave(self) -> None:
        ...
        if not workspace_dir:
            logger.error("WORKSPACE_DIR environment variable is not set.")
            return

        autosave_path = os.path.join(workspace_dir, self.autosave_folder)
        if not os.path.exists(autosave_path):
            os.makedirs(autosave_path)

        file_path = os.path.join(autosave_path, f"prompt-id-{self.id}.json")
