...
class SwarmRegistry(BaseModel):
    ...
    def query(self, swarm_name: str) -> Callable:
        ...
        for swarm in self.swarm_pool:
            if swarm.__name__ == swarm_name:
                name = swarm.__name__
                description = swarm.__doc__.strip().split("\n")[0] or swarm.description
    ...
    def query_multiple_of_swarms(self, swarm_names: List[str]) -> List[Callable]:
        """
    ...
    def run_multiple_of_swarms(self, swarm_names: List[str], *args, **kwargs):
        """
        ...
        """
        return [self.run(swarm_name, *args, **kwargs) for swarm_name in swarm_names]
