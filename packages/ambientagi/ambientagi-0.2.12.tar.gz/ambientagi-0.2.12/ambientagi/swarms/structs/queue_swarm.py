...
class TaskQueueSwarm(BaseSwarm):
    ...
    def reliability_checks(self):
        ...
        if not self.agents:
            raise ValueError("You must provide a non-empty list of Agent instances.")
    ...
    def _process_task(self, agent: Agent):
        ...
        while True:
            try:
                ...
                break
            try:
                logger.info(f"Agent {agent.agent_name} is running task: {task}")
                ...
                logger.info(f"Agent {agent.agent_name} completed task: {task}")
    ...
    def run(self):
        ...
        threads = [
            threading.Thread(target=self._process_task, args=(agent,), daemon=True)
    ...
    def save_json_to_file(self):
        ...
        file_path = os.path.join(self.workspace_dir, self.save_file_path)
