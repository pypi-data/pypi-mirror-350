...
class AgentOutputSchema(BaseModel):
    run_id: Optional[str] = Field(..., description="Unique ID for the run")
    agent_name: Optional[str] = Field(..., description="Name of the agent")
    task: Optional[str] = Field(..., description="Task or query given to the agent")
    output: Optional[str] = Field(..., description="Output generated by the agent")
    start_time: Optional[datetime] = Field(..., description="Start time of the task")
    end_time: Optional[datetime] = Field(..., description="End time of the task")
...
class ConcurrentWorkflow(BaseSwarm):
    ...
    def reliability_check(self):
        try:
            ...
            if not self.agents:
                ...
                raise ValueError("The list of agents must not be empty.")
            ...
            logger.error(f"An unexpected error occurred during reliability checks: {e}")
            raise
    ...
    def transform_metadata_schema_to_str(self, schema: MetadataSchema):
        """
    ...
    def _run(
        ...
        def run_agent(agent: Agent, task: str, img: str = None) -> AgentOutputSchema:
        ...
        with ThreadPoolExecutor(max_workers=os.cpu_count()) as executor:
            agent_outputs = list(
                executor.map(lambda agent: run_agent(agent, task), self.agents)
            )
    ...
    def run_batched(self, tasks: List[str]) -> List[Union[Dict[str, Any], str]]:
        """
