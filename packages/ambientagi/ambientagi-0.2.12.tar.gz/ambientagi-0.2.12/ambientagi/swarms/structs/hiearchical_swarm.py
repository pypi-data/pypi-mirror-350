...
class HierarchicalSwarm(BaseSwarm):
    ...
    def check_agents(self):
        ...
        logger.info("Reliability checks have passed. Swarm is ready to execute.")
    ...
    def run(
        ...
        for loop_idx in range(self.max_loops):
            ...
            for order in orders_list:
                ...
                self.agent_outputs[self.current_loop][order.agent_name] = agent_output
    ...
    def list_all_agents(self) -> str:
        ...
        formatter.print_panel(all_agents, title="All Agents Available in the Swarm")
    ...
    def run_agent(self, agent_name: str, task: str, img: str = None) -> str:
        """
        ...
        try:
            ...
            if not agent:
                ...
                self.conversation.add(role="System", content=f"Error: {error_msg}")
                return error_msg
    ...
    def parse_orders(self, swarm_spec: swarmspec) -> List[Any]:
        ...
        try:
            ...
            error_msg = f"Error parsing and executing orders: {str(e)}"
            logger.error(error_msg)
            self.conversation.add(role="System", content=f"Error: {error_msg}")
            return [error_msg]
    ...
    def parse_swarm_spec(self, swarm_spec: swarmspec) -> List[HierarchicalOrder]:
        ...
        try:
            ...
            logger.error("Invalid swarmspec format: missing 'orders' attribute")
            return []
        except Exception as e:
            logger.error(f"Error parsing swarmspec: {str(e)}")
            return []

    def provide_feedback(self, agent_outputs: Dict[str, str]) -> Dict[str, str]:
        """
    ...
    def check_director_agent_output(self, output: any) -> dict:
        if isinstance(output, dict):
            ...
            try:
                ...
                logger.error(f"Failed to parse output string as JSON: {str(e)}")
                return {}
        else:
            # Log an error if the output is neither a dict nor a string
            logger.error("Output is neither a dictionary nor a string.")
            return {}
    ...
    def concurrent_run(
        ...
        with ThreadPoolExecutor(max_workers=len(tasks)) as executor:
            ...
            return list(executor.map(lambda args: self.run(*args), task_functions))
