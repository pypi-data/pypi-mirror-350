...
class MixtureOfAgentsOutput(BaseModel):
    id: str = Field(..., description="The ID of the mixture of agents.")
...
class MixtureOfAgents:
    ...
    async def _run_agent_async(
        ...
        if prev_responses:
            system_prompt_with_responses = self._get_final_system_prompt(
        ...
        self.output_schema.normal_agent_outputs.append(agent.agent_output)
    ...
    async def _run_async(self, task: str) -> None:
        ...
        results: List[str] = await asyncio.gather(
            *[self._run_agent_async(agent, task) for agent in self.agents]
        )
        ...
        for _ in range(1, self.layers - 1):
            results = await asyncio.gather(
                *[
                    self._run_agent_async(agent, task, prev_responses=results)
    ...
    def run(self, task: str) -> None:
        ...
        try:
            ...
            for _ in range(self.max_loops):
                ...
                prev_context = self.output_schema.aggregator_agent_summary
    ...
    def run_concurrently(self, tasks: List[str]) -> List[str]:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            futures = [executor.submit(self.run, task) for task in tasks]
            return [
