...
class MultiAgentRouter:
    ...
    def route_task(self, task: str) -> dict:
        ...
        try:
            ...
            self.conversation.add(role="assistant", content=boss_response_str)
            ...
            if self.execute_task:
                ...
                self.conversation.add(role=selected_agent.name, content=agent_response)
                execution_time = (datetime.now() - execution_start).total_seconds()
            else:
                logger.info("Task execution skipped (execute_task=False)")
            ...
            result = {
                ...
                "task": {
                    "original": task,
                    "modified": (final_task if boss_response.modified_task else None),
                },
                "boss_decision": {
                ...
                "execution": {
                    ...
                    "response": (agent_response if self.execute_task else None),
                    "execution_time": (execution_time if self.execute_task else None),
                },
                "total_time": total_time,
            }

            logger.info(f"Successfully routed task to {selected_agent.name}")
            return result
    ...
    def concurrent_batch_run(self, tasks: List[str] = []):
        ...
        with ThreadPoolExecutor() as executor:
            futures = [executor.submit(self.route_task, task) for task in tasks]
