...
class AgentRouter:
    ...
    def add_agent(self, agent: Agent) -> None:
        ...
        try:
            ...
            logger.info(f"Added agent {agent.name} to the vector database.")
    ...
    def add_agents(self, agents: List[Union[Agent, Callable, Any]]) -> None:
        """
    ...
    def update_agent_history(self, agent_name: str) -> None:
        ...
        if agent:
            ...
            updated_text = (
                f"{agent.name} {agent.description} {agent.system_prompt} {history_text}"
            )
            ...
            logger.info(f"Updated agent {agent_name} with interaction history.")
        else:
            logger.warning(f"Agent {agent_name} not found in the database.")

    @retry(
        stop=stop_after_attempt(3),
        wait=wait_exponential(multiplier=1, min=4, max=10),
    )
    def find_best_agent(self, task: str, *args, **kwargs) -> Optional[Agent]:
        """
        ...
        try:
            ...
            if results["ids"]:
                ...
                best_agent = next(
                    (a for a in self.agents if a.name == best_match_name),
                    None,
                )
                if best_agent:
                    logger.info(f"Found best matching agent: {best_match_name}")
                    return best_agent
                else:
                    logger.warning(
