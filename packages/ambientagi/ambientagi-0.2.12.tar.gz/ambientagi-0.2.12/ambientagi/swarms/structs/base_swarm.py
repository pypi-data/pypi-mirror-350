...
class BaseSwarm(ABC):
    ...
    def __init__(
        ...
        metadata_filename: Optional[str] = "multiagent_structure_metadata.json",
        ...
        **kwargs,
        ...
        if speaker_selection_func is not None:
            if not callable(speaker_selection_func):
                raise TypeError("Speaker selection function must be callable.")
            self.speaker_selection_func = speaker_selection_func
        ...
        self.agents_dict = {agent.agent_name: agent for agent in self.agents}
    ...
    async def arun(self, task: Optional[str] = None, *args, **kwargs):
        ...
        result = await loop.run_in_executor(None, self.run, task, *args, **kwargs)
        return result
    ...
    async def aloop(
        ...
        **kwargs,
        ...
        loop = asyncio.get_event_loop()
        result = await loop.run_in_executor(None, self.loop, task, *args, **kwargs)
        return result
    ...
    def run_batch_async(self, tasks: List[str], *args, **kwargs):
        ...
        """
        loop = asyncio.get_event_loop()
        result = loop.run_until_complete(self.abatch_run(tasks, *args, **kwargs))
        return result
    ...
    def task_assignment_by_id(self, task: str, agent_id: str, *args, **kwargs):
        """
    ...
    def task_assignment_by_name(self, task: str, agent_name: str, *args, **kwargs):
        """
        Assign a task to an agent
        """
        # Assign task to agent by their agent id
    ...
    def concurrent_run(self, task: str) -> List[str]:
        ...
        with ThreadPoolExecutor() as executor:
            future_to_llm = {
                executor.submit(agent, task): agent for agent in self.agents
            }
            responses = []
            for future in as_completed(future_to_llm):
                try:
                    ...
                    print(
                        f"{future_to_llm[future]} generated an" f" exception: {error}"
                    )
    ...
    def swarm_initialization(self, *args, **kwargs):
        ...
        logger.info(f"Agent names: {[agent.name for agent in self.agents]}")
        ...
        if self.director is None:
            logger.info("No director found. Please add a director to the swarm.")
            return None

        logger.info(f"Initialization complete for the hierarchical swarm: {self.name}")
    ...
    def export_and_autosave(self):
        ...
        return logger.info(f"Metadata saved to {self.metadata_filename}")
