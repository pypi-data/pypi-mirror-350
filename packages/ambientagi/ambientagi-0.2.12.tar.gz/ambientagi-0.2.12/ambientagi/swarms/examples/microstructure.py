...
class SignalCSVWriter:
    ...
    def get_filename(self, signal_type: str, symbol: str) -> str:
        ...
        return f"{self.output_dir}/{signal_type}_{symbol}_{date_str}.csv"
    ...
    def write_arbitrage_signal(self, signal: MarketSignal):
        if "best_opportunity" not in signal.data or not signal.data["best_opportunity"]:
            return
...
class OrderBookAgent(BaseMarketAgent):
    ...
    def calculate_order_book_metrics(self, order_book: Dict) -> Dict[str, float]:
        ...
        # Calculate VWAP
        bid_vwap = (
            np.sum(bids[: self.vwap_window, 0] * bids[: self.vwap_window, 1])
            / bid_volume
            if bid_volume > 0
            else 0
        )
        ask_vwap = (
            np.sum(asks[: self.vwap_window, 0] * asks[: self.vwap_window, 1])
            / ask_volume
            if ask_volume > 0
            else 0
        )
        ...
        bid_slope = np.polyfit(range(len(bids[:10])), bids[:10, 0], 1)[0]
        ask_slope = np.polyfit(range(len(asks[:10])), asks[:10, 0], 1)[0]
    ...
    def detect_large_orders(
        ...
        hist_volumes = [
            book["bid_volume"] + book["ask_volume"] for book in historical_books
        ]
        volume_mean = np.mean(hist_volumes)
        volume_std = np.std(hist_volumes)

        current_volume = metrics["bid_volume"] + metrics["ask_volume"]
        z_score = (current_volume - volume_mean) / (volume_std if volume_std > 0 else 1)
    ...
    def analyze_order_book(self, symbol: str) -> MarketSignal:
        ...
        try:
            ...
            return MarketSignal(
                ...
                data={
                    "metrics": metrics,
                    "large_order_detected": self.detect_large_orders(metrics),
                    "symbol": symbol,
                    "llm_analysis": llm_analysis,  # Add LLM insights
                },
                confidence=min(
                    abs(metrics["depth_imbalance"]) * 0.7
                    + (1.0 if self.detect_large_orders(metrics) else 0.0) * 0.3,
                    1.0,
                ),
                metadata={
                    "update_latency": (
                    ...
                    "buffer_size": len(self.order_book_buffer.get_latest()),
                },
            )
        except Exception as e:
...
class TickDataAgent(BaseMarketAgent):
    ...
    def calculate_tick_metrics(self, ticks: List[Dict]) -> Dict[str, float]:
        ...
        metrics["vwap"] = (df["price"] * df["volume"]).sum() / df["volume"].sum()
        ...
        metrics["trade_intensity"] = len(df) / time_diff if time_diff > 0 else 0
    ...
    def calculate_kyle_lambda(self, df: pd.DataFrame) -> float:
        ...
        try:
            ...
            if len(price_changes) > 1 and len(volume_changes) > 1:
                slope, _, _, _, _ = stats.linregress(volume_changes, price_changes)
                return abs(slope)
        except Exception as e:
    ...
    def calculate_tick_metrics(self, ticks: List[Dict]) -> Dict[str, float]:
        try:
            ...
            logger.info(f"Raw tick data structure: {ticks[0] if ticks else 'No ticks'}")
            ...
            for trade in ticks:
                formatted_trade = {
                    ...
                    "amount": float(
                        trade.get(
                            "amount",
                            trade.get("size", trade.get("quantity", 0)),
                        )
                    ),
            ...
            metrics = {}
            metrics["vwap"] = (
                (df["price"] * df["amount"]).sum() / df["amount"].sum()
                if not df.empty
                else 0
            )
            metrics["price_momentum"] = df["price"].diff().mean() if len(df) > 1 else 0
    ...
    def analyze_ticks(self, symbol: str) -> MarketSignal:
        ...
        try:
            ...
            return MarketSignal(
                ...
                confidence=min(metrics["trade_intensity"] / 100, 1.0) * 0.4
            ...
            logger.error(f"Error in tick analysis: {str(e)}", exc_info=True)
            return None
...
class LatencyArbitrageAgent(BaseMarketAgent):
    def __init__(self, api_key: str):
        ...
        """
        super().__init__("LatencyArbitrageAgent", system_prompt, api_key)
        exchange_manager = ExchangeManager()
        ...
        self.price_buffer = {
            ex: MarketDataBuffer(max_size=100) for ex in self.exchanges
        }
    ...
    def calculate_arbitrage_metrics(self, prices: Dict[str, Dict]) -> Dict:
        opportunities = []
        ...
        return {
            ...
            "best_opportunity": (
                max(opportunities, key=lambda x: x["return"]) if opportunities else None
            ),
        }
        ...
        try:
            ...
            for name, exchange in self.exchanges.items():
                try:
                    ...
                    logger.warning(f"Error fetching {name} price: {e}")
            ...
            timing_factor = 1.0 - min(
                abs(timestamps[opp["sell_venue"]] - timestamps[opp["buy_venue"]])
                / 1000,
                1.0,
            )
            spread_factor = min(opp["return"] * 5, 1.0)  # Scale return to confidence
                ...
                data={
                    ...
                    "timing": {
                        "time_difference_ms": abs(
                            timestamps[opp["sell_venue"]] - timestamps[opp["buy_venue"]]
                        ),
...
class SwarmCoordinator:
    ...
    def aggregate_signals(self, signals: List[MarketSignal]) -> Dict[str, Any]:
        ...
        for signal in signals:
            ...
            if signal.signal_type == "order_book_analysis":
                ...
                aggregated["market_state"].update(
                    {
                        "order_book_imbalance": metrics.get("depth_imbalance"),
                ...
                aggregated["market_state"].update(
                    {
                        "price_momentum": metrics.get("price_momentum"),
                        "trade_intensity": metrics.get("trade_intensity"),
    ...
    def start(self, symbols: List[str], interval: float = 1.0):
            ...
            while self.running:
                try:
                    ...
                    if current_signals:
                        ...
                        aggregated = self.aggregate_signals(current_signals)
                        logger.info(f"Aggregated market view: {aggregated}")
        ...
        collection_thread = threading.Thread(target=signal_collection_loop, daemon=True)
    ...
    for signal in signals:
        if signal.confidence > 0.8:
            if signal.signal_type == "arbitrage_opportunity":
                opp = signal.data.get("best_opportunity")
                if opp and opp["return"] > 0.001:  # 0.1% return threshold
...
logger.info("Starting market microstructure analysis with LLM integration...")
logger.info(f"Monitoring symbols: {symbols}")
logger.info(f"CSV files will be written to: {os.path.abspath('market_data')}")
