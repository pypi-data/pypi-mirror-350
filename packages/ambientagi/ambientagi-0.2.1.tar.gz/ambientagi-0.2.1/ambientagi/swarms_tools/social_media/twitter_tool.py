...
class TwitterTool:
    ...
    def _fetch_mentions(self) -> List[Dict[str, Any]]:
        try:
            ...
            time.sleep(int(e.response.headers.get("x-rate-limit-reset", 60)))
            return []
        except tweepy.TweepyException as e:
    ...
    def reply_to_mentions_with_agent(self, agent: Any) -> None:
        ...
        while True:
            try:
                ...
                reset_time = int(headers.get("x-rate-limit-reset", 60))
...
def reply_to_mentions_with_agent(agent: Any, interval: int = 10) -> None:
    """
