...
class MajorityVoting:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.conversation = Conversation(time_enabled=True, *args, **kwargs)
    ...
    def run(self, task: str, *args, **kwargs) -> List[Any]:
        ...
        """
        results = run_agents_concurrently(self.agents, task, max_workers=os.cpu_count())
        ...
        if self.consensus_agent is not None:
            ...
            self.conversation.add(self.consensus_agent.agent_name, majority_vote)
        else:
            # fetch the last agent
            majority_vote = self.agents[-1].run(prompt)

            self.conversation.add(self.agents[-1].agent_name, majority_vote)
    ...
    async def run_async(self, tasks: List[str], *args, **kwargs) -> List[Any]:
        """
