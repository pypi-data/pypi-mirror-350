    ...
    def validate_flow(self):
        ...
        for task in tasks:
            ...
            for swarm_name in swarm_names:
                if swarm_name not in self.swarms and swarm_name != "H":
                    raise ValueError(f"swarm '{swarm_name}' is not registered.")
        ...
        if len(set(swarms_in_flow)) != len(swarms_in_flow):
            raise ValueError("Duplicate swarm names in the flow are not allowed.")
        ...
        **kwargs,
        ...
        try:
            ...
            if isinstance(custom_tasks, dict) and custom_tasks:
                c_swarm_name, c_task = next(iter(custom_tasks.items()))
            ...
            while loop_count < self.max_loops:
                for task in tasks:
                    ...
                    if len(swarm_names) > 1:
                        # Parallel processing
                        logger.info(f"Running swarms in parallel: {swarm_names}")
                        results = []
                        for swarm_name in swarm_names:
                            if swarm_name == "H":
                                ...
                                if (
                                    ...
                                    current_task = self.custom_human_in_the_loop(
                                        current_task
                                    )
                                else:
                                    current_task = input("Enter your response: ")
                            else:
                                ...
                                if isinstance(result, bool):
                                    ...
                                    result = str(result)  # Convert boolean to string
                        ...
                        logger.info(f"Running swarms sequentially: {swarm_names}")
                        ...
                        if swarm_name == "H":
                            ...
                            if self.human_in_the_loop and self.custom_human_in_the_loop:
                                ...
                                current_task = input("Enter the next task: ")
                        else:
                            swarm = self.swarms[swarm_name]
                            result = swarm.run(current_task, img, *args, **kwargs)
                            result = any_to_str(result)
                            ...
                            if isinstance(result, bool):
                                ...
                                result = str(result)  # Convert boolean to string
                            current_task = (
                                result if result is not None else current_task
                            )
                loop_count += 1
...
def swarm_arrange(
    ...
    **kwargs,
    ...
    try:
        ...
        logger.error(f"An error occurred during swarm arrangement execution: {e}")
        return str(e)
