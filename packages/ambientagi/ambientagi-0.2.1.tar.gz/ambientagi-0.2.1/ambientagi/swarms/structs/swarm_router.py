...
class SwarmRouter:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.load_agents_from_csv:
            self.agents = AgentLoader(csv_path=self.csv_file_path).load_agents()
        ...
        try:
            ...
            for agent in self.agents:
                if hasattr(agent, "auto_generate_prompt"):
                    agent.auto_generate_prompt = self.auto_generate_prompts
                    activated_count += 1
                    logger.debug(
                        f"Activated APE for agent: {agent.name if hasattr(agent, 'name') else 'unnamed'}"
                    )

            logger.info(f"Successfully activated APE for {activated_count} agents")
            self._log(
                "info",
    ...
    def _create_swarm(self, task: str = None, *args, **kwargs) -> Union[
        AgentRearrange,
        MixtureOfAgents,
        SpreadSheetSwarm,
        SequentialWorkflow,
    ...
    def async_run(self, task: str, *args, **kwargs) -> Any:
        ...
        async def run_async():
            try:
                result = await asyncio.to_thread(self.run, task, *args, **kwargs)
                return result
            except Exception as e:
                self._log(
                    "error",
    ...
    def concurrent_batch_run(self, tasks: List[str], *args, **kwargs) -> List[Any]:
        """
        ...
        with ThreadPoolExecutor() as executor:
            ...
            futures = [
                executor.submit(self.run, task, *args, **kwargs) for task in tasks
            ]
...
def swarm_router(
    ...
    **kwargs,
    ...
    """
    try:
        logger.info(f"Creating SwarmRouter with name: {name}, swarm_type: {swarm_type}")

        if not agents:
            logger.warning("No agents provided, router may have limited functionality")
        ...
        logger.info(f"Task execution completed successfully: {result}")
        return result

    except ValueError as e:
        logger.error(f"Invalid arguments provided to swarm_router: {str(e)}")
        raise
    except Exception as e:
