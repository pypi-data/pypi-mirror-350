...
class SequentialWorkflow:
    ...
    def run_batched(self, tasks: List[str]) -> List[str]:
        ...
        try:
            ...
            logger.error(f"An error occurred while executing the batch of tasks: {e}")
            raise
    ...
    async def run_concurrent(self, tasks: List[str]) -> List[str]:
        ...
        try:
            with ThreadPoolExecutor() as executor:
                results = [
                    executor.submit(self.agent_rearrange.run, task) for task in tasks
                ]
                return [result.result() for result in as_completed(results)]
        except Exception as e:
            logger.error(
