    ...
    def _load_from_disk(self) -> None:
        if self._persistence_path and os.path.exists(self._persistence_path):
            with open(self._persistence_path, "r") as f:
                data = json.load(f)
                self._memory = {k: SharedMemoryItem(**v) for k, v in data.items()}
...
class AsyncWorkflow(BaseWorkflow):
    ...
    async def _execute_agent_task(self, agent: Agent, task: str) -> AgentOutput:
        ...
        try:
            ...
            end_time = datetime.utcnow()
            self.logger.info(f"Agent {agent.agent_name} completed task {task_id}")
        ...
        try:
            ...
            if self.autosave:
                ...
                await self._save_results(
                    (self.results[0].start_time if self.results else end_time),
                    end_time,
                )
