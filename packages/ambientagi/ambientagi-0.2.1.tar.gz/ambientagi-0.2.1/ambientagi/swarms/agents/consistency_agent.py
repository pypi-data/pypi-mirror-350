...
class SelfConsistencyAgent(Agent):
    ...
    def run(self, task: str, answer: str = None, *args, **kwargs) -> str:
        """
        ...
        if self.eval:
            if answer is not None:
                correct = self.check_responses_for_answer(responses, answer)

                if not correct:
                    logger.info("The answer is not correct. Please try again.")
                    return None
        ...
        self.conversation.add(role="Majority Voting Agent", content=final_answer)
    ...
    def check_responses_for_answer(self, responses: List[str], answer: str) -> bool:
        """
