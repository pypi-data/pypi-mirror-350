...
class CogVLMMultiModal(BaseMultiModalModel):
    ...
    def generate_stream_cogvlm(
        ...
        query, history, image_list = self.process_history_and_images(messages)
        ...
        inputs = {
            ...
            "token_type_ids": (
                input_by_model["token_type_ids"].unsqueeze(0).to(self.device)
            ),
            "attention_mask": (
                input_by_model["attention_mask"].unsqueeze(0).to(self.device)
            ),
            "images": [
