...
class CoinMarketCapAPI:
    ...
    API_KEY = os.getenv("COINMARKETCAP_API_KEY")  # Replace with your actual API key
    ...
    def fetch_coin_data(
        ...
        filtered_data = CoinMarketCapAPI._filter_data(data["data"], coin_names)
    ...
    def _filter_data(
        ...
        filtered_data = {
            coin["name"]: coin
            for coin in data
            if coin["name"].lower() in {name.lower() for name in coin_names}
        }
        if not filtered_data:
            logger.warning(f"No data found for specified coin names: {coin_names}")
        return filtered_data
