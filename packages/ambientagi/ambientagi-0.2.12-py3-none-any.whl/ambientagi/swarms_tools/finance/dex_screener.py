...
class DexScreenerAPI:
    ...
    def get_latest_token_profiles(self) -> Dict[str, Any]:
        ...
        """
        try:
            response = self.client.get(f"{BASE_URL}/token-profiles/latest/v1")
            response.raise_for_status()
            logger.debug("Successfully fetched latest token profiles")
            return response.json()
        except httpx.HTTPStatusError as e:
            if e.response.status_code == 429:
                raise RateLimitExceeded("Token profiles rate limit exceeded")
    ...
    def get_latest_token_boosts(self) -> Dict[str, Any]:
        ...
        """
        try:
            response = self.client.get(f"{BASE_URL}/token-boosts/latest/v1")
            response.raise_for_status()
            logger.debug("Successfully fetched latest token boosts")
            return response.json()
        except httpx.HTTPStatusError as e:
            if e.response.status_code == 429:
                raise RateLimitExceeded("Token boosts rate limit exceeded")
    ...
    def get_pair(self, chain_id: str, pair_id: str) -> TokenPairInfo:
        ...
        try:
            ...
            if not data.get("pairs"):
                logger.warning(f"No pair data found for {chain_id}/{pair_id}")
                return None

            pair = data["pairs"][0]
            logger.debug(f"Successfully fetched pair data for {chain_id}/{pair_id}")
            ...
            return TokenPairInfo(
                ...
                liquidity=(
                    Liquidity(**pair["liquidity"]) if pair.get("liquidity") else None
                ),
                fdv=pair.get("fdv"),
    ...
    def search_pairs(self, query: str) -> List[TokenPairInfo]:
        ...
        try:
            ...
            logger.debug(f"Successfully searched pairs with query: {query}")
            return pairs
        except httpx.HTTPStatusError as e:
            if e.response.status_code == 429:
    ...
    def get_token_pairs(
        ...
        try:
            response = self.client.get(f"{BASE_URL}/tokens/v1/{chain_id}/{addresses}")
            response.raise_for_status()
            pairs = []
            ...
            logger.debug(f"Successfully fetched token pairs for {chain_id}/{addresses}")
            return pairs
        except httpx.HTTPStatusError as e:
            if e.response.status_code == 429:
                raise RateLimitExceeded("Token pairs rate limit exceeded")
