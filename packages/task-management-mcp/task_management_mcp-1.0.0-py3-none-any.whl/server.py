...
class TaskManagementMCP:
    ...
    def __init__(self):
        ...
        self.base_url = os.getenv("TASK_API_BASE_URL", "http://localhost:8000")
    ...
    async def _handle_tool_call(self, name: str, arguments: dict[str, Any]) -> list[TextContent]:
        ...
        if not self.api_key:
            return [TextContent(
                type="text",
                text="Error: No API key configured. Please set TASK_API_KEY environment variable or download the MCP Client app from the Apple App Store to generate one."
            )]
