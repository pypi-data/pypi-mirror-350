...
class UploadApi:
    ...
    def upload(self, local_file, remote_file) -> bool:
        ...
        remote_file_path = process_path(remote_file)

        uploadid = self._precreate(filesize, remote_file_path, md5str)

        if self.debug:
            pprint(f'预处理上传：{uploadid}')

        try:
            for index, item in enumerate(tempfiles):
               chunk_res = self._upload_chunk(uploadid, remote_file_path, item, index)
               if self.debug:
                   pprint(f'上传分片:{chunk_res}')

            create_res = self._create(uploadid, remote_file_path, filesize, md5str)
            if self.debug:
                pprint(f'合并分片:{create_res}')
