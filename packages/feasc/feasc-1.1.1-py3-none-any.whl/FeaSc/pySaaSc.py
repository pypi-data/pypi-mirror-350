...
def _load_background_geneset(
	...
	if isinstance(background_geneset, dict):
		...
		if not os.path.exists(background_geneset):
			raise FileNotFoundError(f"Cannot find file: {background_geneset}")
...
def _ridge_regression(
	...
	try:
		...
		raise ValueError("X.T @ X 不可逆，无法进行回归")
	# β = (X^T X + λI)^(-1) X^T Y
...
def compute_response(adata: sc.AnnData,
	...
	if celltype is None:
		...
		if obs_celltype not in adata.obs.columns:
			raise KeyError("Column 'celltype' not found in adata.obs")
		if celltype not in adata.obs[obs_celltype].unique():
			raise ValueError(f"Celltype '{celltype}' not exist in adata.obs['celltype']")
...
def plot_response_signaling(res_list):
	...
	for ax, (name, df) in zip(axes, res_list.items()):
		x = df['TGFB1']
		y = df['Tcell_activation']
		corr = np.corrcoef(x, y)[0, 1]
		title = f"{name} (cor: {corr:.2f})"
		
		# 散点图
		sns.scatterplot(x='TGFB1', y='Tcell_activation', data=df, alpha=0.7, color='blue', ax=ax)
		
		# 回归线
		sns.regplot(x='TGFB1', y='Tcell_activation', data=df, scatter=False, color='red', ci=True, ax=ax)
		
		# 设置标题和样式
		ax.set_title(title, fontsize=12, ha='center')
		ax.set_xlabel("TGFB1", fontsize=10)
		ax.set_ylabel("Tcell_activation", fontsize=10)
...
# 绘制 Wnt 信号通路活性中位数柱状图
def plot_wnt_activity(signaling, adata):
	"""
	...
	wnt_data = pd.DataFrame({
			'celltype': celltypes,
			'wnt':      signaling['Wnt']
	})
	
	# 计算每个 celltype 的中位数
	grouped = wnt_data.groupby('celltype', observed=False)['wnt'].median().reset_index()
	grouped = grouped.sort_values(by='wnt', ascending=False)
	
	# 添加颜色列
	grouped['color'] = grouped['wnt'].apply(lambda x: "Positive" if x > 0 else "Negative")
	...
	plt.ylabel("Wnt Pathway Activity (Median)")
	plt.title("Wnt Activity per Cell Type")
...
def main():
	...
	# 计算基因频率
	gs = read_gmt('../data/gmt/CD8_Tcells_activation.gmt')
	...
	Signaling = compute_signaling(adata, model_file="../data/SPEED2_signaling.tsv", celltype=None,
	                              cytokine=None, lambd=10000, num_permutations=0, test_method="two-sided")
	plot_wnt_activity(Signaling, adata)
