...
async def get_component(request: web.Request) -> web.Response:
    ...
    async with mongo.MongoContext(request) as mongo_:
        ...
        _add_preflight_actions(request, obj)
...
async def get_component_by_type(request: web.Request) -> web.Response:
    ...
    if result is None:
        async with mongo.MongoContext(request) as mongo_:
            ...
            _add_preflight_actions(request, component)
...
async def get_all_components(request: web.Request) -> web.Response:
    ...
    """
    sub = request.headers.get(SUB, NONE_USER)
    sort = mongoservicelib.extract_sort_dict(request, 'display_name')
    if not request.query:
        cache_key = (sub, MONGODB_COMPONENT_COLLECTION, None,
                     mongoservicelib.sort_dict_to_cache_key(sort or {}))
...
def _add_preflight_actions(request, obj):
    wstl_factory = cast(RuntimeWeSTLDocumentBuilder, request.app[appproperty.HEA_WSTL_BUILDER_FACTORY])
    for resource in obj.resources:
        if resource.delete_preflight:
            name = f'heaserver-registry-component-resource-delete-preflight-{resource.resource_type_name}'
            wstl_factory.add_design_time_action(WeSTLactionType(
                    name=name,
                    description=f'Preflight check for deleting {resource.resource_type_name} objects.',
                    type='safe',
                    action='read',
                    target='item read cj',
                    prompt='Preflight check for deletes'
                ))
            wstl_factory.add_run_time_action(name = name,
                                                 path = resource.delete_preflight,
                                                 rel = 'delete-preflight')
    for i, link_preflight in enumerate(obj.link_preflights):
        name = f'heaserver-registry-component-link-preflight-{link_preflight.desktop_object_type_name}-{i}'
        wstl_factory.add_design_time_action(WeSTLactionType(
                name=name,
                description=f'Preflight check for linking {link_preflight.desktop_object_type_name} objects.',
                type='safe',
                action='read',
                target='item read cj',
                prompt='Preflight check for links'
            ))
        wstl_factory.add_run_time_action(name = name,
                                             path = link_preflight.path,
                                             rel = f'link-preflight {" ".join(link_preflight.rels)}')
