...
"""
    ...
    def generate_drift_images_interstack(self,parameters,embryo_name,begin):
        ...
        if not os.path.isdir(co_stack_folder):
            print("Weird, drift inter stack didn't generate CO-STACK folder ")
            return
        image_name = embryo_name+"_fuse_t{:03d}".format(begin)+".mha"
        ...
        for drif_exp in drif_experiments:
            ...
            if found_iteration > -1:
                ...
                if os.path.isdir(score_path):
                    ...
                    self.upload_on_omero(omero_config_file,embryo_name,"DRIFT_"+str(drif_exp)+"ITER"+str(found_iteration)+"-CO-SCORE",score_path)
                if os.path.isdir(transformations_path):
                    print("Uploading transformations")
                    self.upload_on_omero(omero_config_file,embryo_name,"DRIFT_"+str(drif_exp)+"ITER"+str(found_iteration)+"-CO-TRSFS",transformations_path)
