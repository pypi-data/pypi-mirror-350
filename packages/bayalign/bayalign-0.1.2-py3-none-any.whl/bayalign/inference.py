...
class Sampler:
    ...
    def __init__(self, distribution, initial_state, seed=None, **kwargs):
        ...
        initial_state: numpy or jax.numpy ndarray
    ...
    def sample(self, n_samples, burnin=0):
        ...
        while len(samples) < (n_samples + burnin):
            samples.append(jnp.array(next(self)))
...
class MetropolisHastings(Sampler, AdaptiveStepsize):
    ...
    def accept_reject(self, state):
        ...
        return jnp.log(random.uniform(subkey)) < prob
    ...
    def __next__(self):
        ...
        # In JAX, we need to handle state updates differently due to immutability
        self.state = jnp.where(accepted, state, self.state)

        return jnp.copy(self.state)
...
class SphericalHMC(MetropolisHastings):
    ...
    def __init__(
        ...
        """
        state = jnp.hstack([initial_state, jnp.zeros_like(initial_state)])
    ...
    def sample_momenta(self):
        ...
        x, v = jnp.reshape(self.state, (2, -1))
        v = project(random.normal(subkey, shape=(len(x),)), x)
        self.state = jnp.hstack([x, v])

    def hamiltonian(self, state):
        """
        Evaluates Hamiltonian, i.e. kinetic energy minus log probability.
        """
        x, v = jnp.reshape(state, (2, -1))
    ...
    def propose(self):
        ...
        x, v = jnp.reshape(jnp.copy(self.state), (2, -1))

        v += 0.5 * eps * project(gradient(x), x)

        # Define single leapfrog step
        def leapfrog_step(t, inputs):
            x, v = inputs
            norm = jnp.linalg.norm(v)

            y = jnp.copy(x)
            x_new = y * jnp.cos(eps * norm) + (v / norm) * jnp.sin(eps * norm)
            v_new = v * jnp.cos(eps * norm) - (y * norm) * jnp.sin(eps * norm)

            # Full step for all but last iteration
            is_last = t == self.n_steps - 1
            v_new = v_new + jnp.where(
                is_last,
                0.5 * eps * project(gradient(x_new), x_new),
                eps * project(gradient(x_new), x_new),
            )

            return x_new, v_new

        # Use scan for the leapfrog integration loop
        x, v = jax.lax.fori_loop(
    ...
    def sample(self, n_samples, burnin=0, return_momenta=False):
        ...
        positions, momenta = jnp.swapaxes(
            jnp.reshape(samples, (n_samples, 2, -1)), 1, 0
        )
...
class RejectionSphericalSliceSampler(Sampler):
    ...
    def __next__(self):
        ...
        # Sample subsphere and compute threshold
        self.key, subkey1, subkey2 = random.split(self.key, 3)
        ...
        # Due to JAX's functional nature and lack of while loops in a pure context,
        # we implement the rejection sampling using a NumPy loop but JAX operations
        interval = (0, 2 * jnp.pi)

        while True:
            self.key, subkey = random.split(self.key)
            theta = random.uniform(subkey, minval=interval[0], maxval=interval[1])
            y = jnp.cos(theta) * x + jnp.sin(theta) * u
...
class ShrinkageSphericalSliceSampler(RejectionSphericalSliceSampler):
    ...
    def __next__(self):
        ...
        # Initial range
        theta_init = random.uniform(subkey3, minval=0, maxval=2 * jnp.pi)
        interval = (theta_init - 2 * jnp.pi, theta_init)

        # Due to JAX's functional nature, we implement the shrinkage slice
        # sampling using a NumPy loop but JAX operations
        while True:
            self.key, subkey = random.split(self.key)
            theta = random.uniform(subkey, minval=interval[0], maxval=interval[1])
            y = jnp.cos(theta) * x + jnp.sin(theta) * u
