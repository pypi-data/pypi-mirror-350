...
def initialize_dbs(settings):
    ...
    """
    global _MAIN_SETTINGS, _MAIN_SITEURL, _MAIN_LANG, _SUBSITE_QUEUE  # noqa: PLW0603
...
class GeneratorInspector:
    """Inspector of generator instances."""

    generators_info = {  # noqa: RUF012
        ArticlesGenerator: {
...
def interlink_translated_content(generator):
    ...
    Applies to generators that may contain translated content.
    """
    inspector = GeneratorInspector(generator)
...
def interlink_static_files(generator):
    ...
    for staticfile in _MAIN_STATIC_FILES:
        if staticfile.get_relative_source_path() not in static_content:
            new_staticfile = copy(staticfile)  # prevent override in main site
            new_staticfile.override_url = posixpath.join(relpath, new_staticfile.url)
            try:
                generator.add_source_path(new_staticfile, static=True)
            except TypeError:
                generator.add_source_path(new_staticfile)


def save_main_static_files(static_generator):
    """Save the static files generated for the main site."""
    global _MAIN_STATIC_FILES  # noqa: PLW0603
...
def update_generators():
    ...
    """
    for generator in _GENERATOR_DB:
...
def create_next_subsite(pelican_obj):
    ...
    """
    global _MAIN_SETTINGS  # noqa: PLW0603
...
def register():
    ...
    for sig_name in _SIGNAL_HANDLERS_DB:
        if not hasattr(signals, sig_name):
            _LOGGER.error(
                f"The i18n_subsites plugin requires the {sig_name} "
