...
from moapy.steel_pre import SteelMaterial, SteelLength, SteelMomentModificationFactorLTB, SteelSectionAISC16US, SteelOptions
from moapy.enum_pre import en_H_AISC16_US, enum_to_list, enSteelMaterial_ASTM
from moapy.dgnengine.base import call_func, load_dll, read_file_as_binary

@auto_schema(
    title="AISC-LRFD16(US) Beam Design",
...
def report_aisc16us_beam_column(matl: SteelMaterial = SteelMaterial.create_default(code="ASTM09(S)", enum_list=enum_to_list(enSteelMaterial_ASTM)),
                                sect: SteelSectionAISC16US = SteelSectionAISC16US.create_default(name="W40X183", enum_list=enum_to_list(en_H_AISC16_US)),
                                load: SectionForce = SectionForce.create_default(enUnitSystem.US),
                                opt: SteelOptions = SteelOptions.create_default(enUnitSystem.US)) -> ResultBytes:
    dll = load_dll()
    eff_len = opt.eff_len
    factor = opt.factor
    length = opt.length
...
def calc_aisc16us_beam_column(matl: SteelMaterial = SteelMaterial.create_default(code="ASTM09(S)", enum_list=enum_to_list(enSteelMaterial_ASTM)),
                              sect: SteelSectionAISC16US = SteelSectionAISC16US.create_default(name="W40X183", enum_list=enum_to_list(en_H_AISC16_US)),
                              load: SectionForce = SectionForce.create_default(enUnitSystem.US),
                              opt: SteelOptions = SteelOptions.create_default(enUnitSystem.US)) -> dict:
    dll = load_dll()
    eff_len = opt.eff_len
    factor = opt.factor
    length = opt.length
...
if __name__ == "__main__":
    res = report_aisc16us_beam_column()
    print(res)