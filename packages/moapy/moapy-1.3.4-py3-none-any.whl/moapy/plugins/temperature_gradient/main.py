...
def girder_type(
    ...
    if type_of_girder == "Concrete":
        return 1
    elif type_of_girder == "Steel":
        return 2
...
def stress_calculation(
    ...
    # =============================================================================
    # Read units data
    res_unit = MidasAPI.db_read("UNIT")
    ...
    x_g, y_g, x_outer_h, y_outer_h, x_outer_c, y_outer_c = dc.create_chart_data(sec_dimension["height"], inf_point, inf_temp_h, inf_temp_c, self_eq_stress)
...
def assign_load(
        heating_assign:str,
        cooling_assign:str,
        select_stld_key_heat:int == 0,
        select_stld_key_cool:int == 0,
        result_data:str
):
    # Input from Javascript
    ...
    select_elem = MidasAPI.view_select_get().get("ELEM_LIST")
    ...
    # Get Load Data
    res_btmp = MidasAPI.db_read("BTMP")
    res_stld = MidasAPI.db_read("STLD")
    res_elem = MidasAPI.db_read("ELEM")
    ...
    if heating_assign and cooling_assign:
        MidasAPI.db_update("BTMP", btmp_data_heat)
        MidasAPI.db_update("BTMP", btmp_data_cool)
