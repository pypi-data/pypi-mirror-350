...
from moapy.steel_pre import SteelSectionAISC05SI, SteelLength
...
from moapy.enum_pre import enUnitSystem, en_H_AISC05_SI, enum_to_list

@auto_schema(
    title="AA-LRFD05(SI) Aluminum Beam Design",
    description=(
        "This functionality performs the design and verification of aluminum beam members "
        "in accordance with the AA-LRFD05(SI) standard. The design process incorporates key "
    ...
    std_type="AA.ADM",
    design_code="AA-LRFD05(SI)"
)
def report_aa_lrfd05m_beam(matl: AluMaterial = AluMaterial(), sect: SteelSectionAISC05SI = SteelSectionAISC05SI.create_default(name="W920X201", enum_list=enum_to_list(en_H_AISC05_SI)),
                           load: SectionForce = SectionForce.create_default(enUnitSystem.SI), option: AluminumOptions = AluminumOptions.create_default(enUnitSystem.SI)) -> ResultBytes:
    dll = load_dll()
    ...
    file_path = call_func(dll, 'Report_AA_LRFD05M_Beam', json_data_list)
