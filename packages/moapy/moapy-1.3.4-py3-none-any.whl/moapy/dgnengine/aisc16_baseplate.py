...
from moapy.steel_pre import (
    AnchorBolt_US, BasePlateSection, BasePlateMaterials, BasePlateLayoutUS, BasePlateStrengthReductionFactor, SteelSection,
    BasePlateStrengthReductionFactor
)
from moapy.enum_pre import enum_to_list, en_H_AISC16_US, enUnitSystem, enAnchorBoltName, enSteelMaterial_ASTM, enBoltMaterialASTM
...
@auto_schema(
    ...
    std_type="AISC LRFD",
    design_code="AISC-LRFD16(US)"
)
def report_aisc16_baseplate(matl: BasePlateMaterials = BasePlateMaterials.create_default(unit_system=enUnitSystem.US, bolt_name=enBoltMaterialASTM.A193_B7, bolt_enum_list=enum_to_list(enBoltMaterialASTM), code="ASTM", enum_list=enum_to_list(enSteelMaterial_ASTM)),
                            sect: BasePlateSection = BasePlateSection.create_default(unit_system=enUnitSystem.US, name="W10X39", enum_list=enum_to_list(en_H_AISC16_US)),
                            force: SectionForce = SectionForce.create_default(enUnitSystem.US),
                            layout: BasePlateLayoutUS = BasePlateLayoutUS.create_default(unit_system=enUnitSystem.US, name=enAnchorBoltName._1_2, enum_list=enum_to_list(enAnchorBoltName)),
...
def calc_aisc16_baseplate(matl: BasePlateMaterials = BasePlateMaterials.create_default(unit_system=enUnitSystem.US, bolt_name=enBoltMaterialASTM.A193_B7, bolt_enum_list=enum_to_list(enBoltMaterialASTM), code="ASTM", enum_list=enum_to_list(enSteelMaterial_ASTM)),
                          sect: BasePlateSection = BasePlateSection.create_default(unit_system=enUnitSystem.US, name="W10X39", enum_list=enum_to_list(en_H_AISC16_US)),
                          force: SectionForce = SectionForce.create_default(enUnitSystem.US),
                          layout: BasePlateLayoutUS = BasePlateLayoutUS.create_default(unit_system=enUnitSystem.US, name=enAnchorBoltName._1_2, enum_list=enum_to_list(enAnchorBoltName)),
                          opt: BasePlateStrengthReductionFactor = BasePlateStrengthReductionFactor()) -> dict:
    dll = load_dll()
    json_data_list = [matl.model_dump_json(), sect.model_dump_json(), force.model_dump_json(), layout.model_dump_json(), opt.model_dump_json(), ctypes.c_void_p(0), ctypes.c_void_p(0)]
