...
from cython.cimports.bv.codec.context import CodecContext
from cython.cimports.bv.codec.hwaccel import HWAccel, HWConfig
...
from cython.cimports.bv.video.format import VideoFormat, get_pix_fmt, get_video_format
from cython.cimports.bv.video.frame import VideoFrame, alloc_video_frame
from cython.cimports.bv.video.reformatter import VideoReformatter
from cython.cimports.libc.stdint import int64_t


def _get_hw_format(ctx, pix_fmts):
    ...
    private_data = cython.declare(
        cython.pointer[AVCodecPrivateData],
        cython.cast(cython.pointer[AVCodecPrivateData], ctx.opaque),
    )
    i: cython.int = 0
    while pix_fmts[i] != -1:
...
class VideoCodecContext(CodecContext):
    ...
    @cython.cfunc
    def _prepare_frames_for_encode(self, input: Frame):
        if not input:
            return [None]
    ...
    def _transfer_hwframe(self, frame: Frame):
        ...
        if frame.ptr.format != self.hwaccel_ctx.config.ptr.pix_fmt:
            # If we get a software frame, that means we are in software fallback mode,
            # and don't actually need to transfer.
            return frame
