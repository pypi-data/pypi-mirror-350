...
from cython.cimports.bv.utils import (
    avdict_to_dict,
...
class Stream:
    ...
    ::
        ...
        >>> fh = bv.open(video_path)
        >>> stream = fh.streams.video[0]
        >>> stream
        <bv.VideoStream #0 h264, yuv420p 1280x720 at 0x...>
    ...
    def __repr__(self):
        ...
        return (
            f"<bv.{self.__class__.__name__} #{self.index} {self.type or '<notype>'}/"
...
class DataStream(Stream):
    def __repr__(self):
        return (
            f"<bv.{self.__class__.__name__} #{self.index} data/"
