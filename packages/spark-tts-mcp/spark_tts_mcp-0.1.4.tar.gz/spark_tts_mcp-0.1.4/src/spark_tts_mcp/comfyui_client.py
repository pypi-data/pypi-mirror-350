...
def modify_spark_tts_workflow(
    ...
    if reference_audio_name:
        ...
        if load_audio_node_id and "inputs" in modified_workflow[load_audio_node_id]:
            if "audio" in modified_workflow[load_audio_node_id]["inputs"]:
                modified_workflow[load_audio_node_id]["inputs"]["audio"] = "钟离_prompt.wav"
                logger.info(f"Set LoadAudio node {load_audio_node_id} audio to default: 钟离_prompt.wav. Current value: {modified_workflow[load_audio_node_id]['inputs']['audio']}")
            else:
                logger.warning(f"LoadAudio node {load_audio_node_id} does not have 'audio' input.")
        else:
            logger.debug("No LoadAudio node found to set default audio input.")
...
async def queue_prompt_async(prompt_workflow: Dict[str, Any], client_id: str) -> str:
    ...
    async with httpx.AsyncClient(timeout=300.0) as client:
        try:
            ...
            logger.debug(f"Full prompt_workflow sent to ComfyUI: {json.dumps(prompt_workflow, indent=2)}")
