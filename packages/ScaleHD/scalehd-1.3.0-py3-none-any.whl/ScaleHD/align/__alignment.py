...
def purge_alignment_map(alignment_outdir, alignment_outfile):
	...
	premapped_pcnt = [x for x in (prepurge_readcount[0].decode().split('\n')) if '%' in x]
	try:
	...
	postpurge_readcount = subprocess.Popen(['samtools', 'flagstat', purged_assembly],
							 stdout=subprocess.PIPE, stderr=subprocess.PIPE).communicate()
	postmapped_pcnt = [x for x in (postpurge_readcount[0].decode().split('\n')) if '%' in x]
	try:
...
class SeqAlign:
	...
	def execute_alignment(self, reference_index, target_fqfile, feedback_string, io_index, typical_flag):
		...
		report_file.write(bwa_error.decode())
		report_file.close()
		...
		if self.enshrine_flag:
			...
			with open(flagstat_path, 'w') as outfi:
				outfi.write(flagstat_output[0].decode())
				outfi.close()
			mapped_pcnt = [x for x in (flagstat_output[0].decode().split('\n')) if '%' in x]
		...
		## Otherwise -e wasn't present (default), and we purge all non-uniquely mapped reads
			...
			with open(flagstat_path, 'w') as outfi:
				...
				outfi.write(flagstat_output[0][0].decode())
				outfi.write('\nAfter purging:\n')
				outfi.write(flagstat_output[1][0].decode())
				outfi.close()
...
class ReferenceIndex:
	...
	def index_reference(self):
		...
		build_stderr = build_rawoutput[1].decode()
