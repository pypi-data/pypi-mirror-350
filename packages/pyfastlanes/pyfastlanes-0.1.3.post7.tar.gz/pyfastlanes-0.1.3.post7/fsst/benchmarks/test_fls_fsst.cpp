...
public:
	...
	uint64_t compressCorpus(
		...
		az_printer::yellow_cout << "-- fsst_encoded_size: " << compressedLen << std::endl;
		///--------------
		fastlanes::fsst12_helper::fsst12_compress(encoder,
		                                          data.size(),
		                                          rowLens.data(),
		                                          rowPtrs.data(),
		                                          compressionBuffer.size(),
		                                          compressionBuffer.data(),
		                                          compressedRowLens.data());
		unsigned long fls_fsst_encoded_size = (compressedRowLens[data.size()]);
		az_printer::yellow_cout << "-- fls_encoded_size: " << fls_fsst_encoded_size << std::endl;
...
public:
	...
	uint64_t compressCorpus(
		...
		vector<uint32_t>         rowLens, compressedRowLens;
		vector<unsigned char*>   rowPtrs, compressedRowPtrs;
		vector<fastlanes::ofs_t> encoded_offsets;
		rowLens.reserve(data.size());
		compressedRowLens.resize(data.size());
		rowPtrs.reserve(data.size());
		encoded_offsets.reserve(data.size() + 1);
		compressedRowPtrs.resize(data.size() + 1);
		uint32_t totalLen = 0;
		for (auto& d : data) {
			totalLen += d.size();
		...
		fsst_compress(encoder,
		              ...
		              compressedRowPtrs.data());
		auto          stopTime = std::chrono::steady_clock::now();
		unsigned long fsst_encoded_size =
		    compressedRowPtrs[data.size() - 1] + compressedRowLens[data.size() - 1] - compressionBuffer.data();
		az_printer::yellow_cout << "-- fsst_encoded_size: " << fsst_encoded_size << std::endl;
		///--------------
		fastlanes::fsst_helper::fsst_compress(encoder,
		                                      data.size(),
		                                      rowLens.data(),
		                                      rowPtrs.data(),
		                                      compressionBuffer.size(),
		                                      compressionBuffer.data(),
		                                      encoded_offsets.data());
		unsigned long fls_fsst_encoded_size = (encoded_offsets[data.size()]);
		az_printer::yellow_cout << "-- fls_encoded_size: " << fls_fsst_encoded_size << std::endl;

		EXPECT_EQ(fls_fsst_encoded_size, fsst_encoded_size);
		///

		compressedData.resize(fsst_encoded_size + 8192);
		memcpy(compressedData.data(), compressionBuffer.data(), fsst_encoded_size);
		offsets.reserve(data.size());
		compressedRowPtrs[data.size()] = compressionBuffer.data() + fsst_encoded_size;
		for (unsigned index = 0, limit = data.size(); index != limit; ++index)
			offsets.push_back(compressedRowPtrs[index + 1] - compressionBuffer.data());
		bareSize        = compressedData.size();
		uint64_t result = bareSize + (offsets.size() * sizeof(unsigned));
...
class TestFLSFSST : public ::testing::Test {};

TEST_F(TestFLSFSST, fsst_test_data) {
...
TEST_F(TestFLSFSST, fsst_dbtext) {
...
TEST_F(TestFLSFSST, fsst12_test_data) {
