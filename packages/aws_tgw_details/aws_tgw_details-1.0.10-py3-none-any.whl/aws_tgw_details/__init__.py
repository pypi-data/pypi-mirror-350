...
logger = logging.getLogger(f"{__name__}:{get_current_time()}")
...
def exception_handler(type, value, tb):
    ...
    print(error_message)
    logger.error("Exception: {0}".format(str(error_message)))
...
def get_tgw_rts(
    ...
    for page in response_iterator:
        for tgw_rt in page["TransitGatewayRouteTables"]:
            ...
            tgw_rt_routes = get_tgw_routes(client, tgw_rt_id)
            tgw_rts.append(
                {
                    "tgw_rt_id": tgw_rt_id,
...
def get_tgw_vpn_connections(
    ...
    if tgw_vpn_connection_id == "ALL":
        ...
        if log_level != "NONE":
            logger.error(f"Invalid VPN connection ID: {tgw_vpn_connection_id}...")
        else:
            pass
...
def login_to_aws(
    ...
    caller_message = f"Signed-in as {caller_id['Arn']}."
    if log_level != "NONE" and log_level < 30:
        logger.info(caller_message)
    ...
    if len(sys.argv) == 1:
        ...
        if (
            ...
            if tgw_only:
                ...
                if log_level != "NONE" and log_level < 30:
                    logger.info(f"Transit Gateway details written to {output_file}.")
