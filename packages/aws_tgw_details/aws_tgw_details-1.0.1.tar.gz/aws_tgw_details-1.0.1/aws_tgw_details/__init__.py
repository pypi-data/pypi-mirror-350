...
def get_tgws(
    ...
    for page in response_iterator:
        for tgw in page["TransitGateways"]:
            ...
            for tag in tgw["Tags"]:
                if tag["Key"] == "Name":
                    tgw_name = tgw["Tags"]["Value"]
                    break
            tgw_owner_id = tgw["OwnerId"]
            tgw_amazon_asn = tgw["Options"]["AmazonSideAsn"]
            tgw_dns_support = tgw["Options"]["DnsSupport"]
            tgw_vpn_ecmp_support = tgw["Options"]["VpnEcmpSupport"]
            tgws.append(
                {
                    "tgw_id": tgw_id,
                    "tgw_name": tgw_name,
    ...
    for page in response_iterator:
        for attachment in page["TransitGatewayAttachments"]:
            ...
            for tag in attachment["Tags"]:
                if tag["Key"] == "Name":
                    tgw_attachment_name = attachment["Tags"]["Value"]
                    break
            tgw_attachment_resource_type = attachment["ResourceType"]
            tgw_attachment_resource_id = attachment["ResourceId"]
            tgw_attachment_assoc_rt_id = attachment["Association"]["TransitGatewayRouteTableId"]
...
def get_tgw_vpn_connections(
    ...
    if tgw_vpn_connection_id:
        ...
        for connection in response["VpnConnections"]:
            ...
            tgw_vpn_connection_name = ""
            for tag in connection["Tags"]:
                if tag["Key"] == "Name":
                    tgw_vpn_connection_name = connection["Tags"]["Value"]
                    break
            tgw_vpn_connection_cgw_id = connection["CustomerGatewayId"]
            tgw_vpn_connection_cgw_name = get_cgws(
                client,
                tgw_vpn_connection_cgw_id
            )[0]["cgw_name"]
            tgw_vpn_connection_accel = connection["Options"]["EnableAcceleration"]
            tgw_vpn_connection_routing = connection["Routes"]["Source"]
            tgw_vpn_connection_cgw_ip = get_cgws(
                client,
                tgw_vpn_connection_cgw_id
            )[0]["cgw_ip_address"]
            tgw_vpn_connection_inside_cidr_0 = connection["Options"]["TunnelOptions"][0]["TunnelInsideCidr"]
            tgw_vpn_connection_outside_ip_0 = connection["Options"]["TunnelOptions"][0]["OutsideIpAddress"]
            tgw_vpn_connection_inside_cidr_1 = connection["Options"]["TunnelOptions"][1]["TunnelInsideCidr"]
            tgw_vpn_connection_outside_ip_1 = connection["Options"]["TunnelOptions"][1]["OutsideIpAddress"]
...
def get_cgws(
    ...
    for cgw in response["CustomerGateways"]:
        ...
        for tag in cgw["Tags"]:
            if tag["Key"] == "Name":
                cgw_name = cgw["Tags"]["Value"]
                break
        cgw_device_name = cgw["DeviceName"]
        cgw_ip_address = cgw["IpAddress"]
        cgw_bgp_asn = cgw["BgpAsn"]
        cgw_type = cgw["Type"]
        cgws.append(
            {
                "cgw_id": cgw_id,
...
def main():
    ...
    if len(sys.argv) == 1:
        print(f"\n{sys.argv[0]} v{__version__} - Get AWS Transit Gateway details.")
        print(
            "\n"
    ...
    if (
        ...
        for tgw in tgws:
            tgw_id = tgw["tgw_id"]
            tgw_rts = get_tgw_rts(client, tgw_id)
            pp(tgw_rts, indent=2)
            tgw_attachments = get_tgw_attachments(client, tgw_id)
            pp(tgw_attachments, indent=2)
            tgw_vpn_connections = get_tgw_vpn_connections(client, tgw_id)
            pp(tgw_vpn_connections, indent=2)
