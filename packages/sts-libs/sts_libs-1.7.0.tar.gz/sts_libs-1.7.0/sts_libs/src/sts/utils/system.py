...
class VersionInfo(NamedTuple):
    """A named tuple representing a software version with major, minor, and micro components."""
...
class SystemInfo:
    ...
    def version(self) -> VersionInfo:
        ...
        """
        if self.release:
            # Handle versions with different formats
            parts = self.release.split('.')

            # Map parts to major, minor, micro
            if len(parts) == 1:
                return VersionInfo(major=parts[0])
            if len(parts) == 2:
                return VersionInfo(major=parts[0], minor=parts[1])
            if len(parts) >= 3:
                return VersionInfo(major=parts[0], minor=parts[1], micro=parts[2])
