...
class TestLsscsi:
    ...
    def test_brief_format(self) -> None:
        ...
        # Should have SCSI_DEVICES entries in brief format
        line_count = len([line for line in result.stdout.splitlines() if line.strip()])
        assert line_count == SCSI_DEVICES, f'Expected {SCSI_DEVICES} entries in brief format, got {line_count}'
    ...
    def test_protection_options(self, option: str) -> None:
