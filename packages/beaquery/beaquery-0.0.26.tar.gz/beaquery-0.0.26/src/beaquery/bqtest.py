...
class TestBQQ():
    ...
    """ TestBQQ
    use beahierarchy.json produced by beaqueryq to
    exhaustively test the BEA API
    ...
    def d2ns(self, qsd):
        ...
        compatible with argparse.parse_args
        """
    ...
    def dsp2ns(self, js, ds, dsp, p):
        """dsp2ns(js, ds, dsp, p)
        js - bea hierarchy javascript
        ds - dataset name
        dsp = parameters for the dataset
        p - parameter value array
        construct argument to d2ns
        """
        qsd={}
        qsd['DatasetName'] = ds
        qsd['format'] = 'CSV'
    ...
    # these form cartesian products of n parameters
    def cp2parameters(self, js, ds, dsp):
        """ cp2parameters(js, ds, dsp)
        js - beahierarchy javascript
        ds - dataset name
        dsp - representation of the parameters of the dataset
        """
    ...
    def cpNparameters(self, js, ds, dsp):
        """ cpNparameters(js, ds, dsp)
        js - beahierarchy javascript
        ds - dataset name
        dsp - representation of the parameters of the dataset
        call proper function for generating cartesian product of
        parameters based on their number
        """
        n = len(dsp['Parameters'])

        # try to control combinatorial explosion
        # takes an insane amount of time
        # for 12 parameters - 479,001,600 calls
        for i in range(n):
    ...
    def collectdatasetparams(self, js, ds):
        """ collectdatasetparams(js, ds)
        js - beahierarchy javascript
        ds - dataset name
        collect and return a representation of the dataset parameters
        for testing
        """
        dsp = {}
        dsp['DatasetName'] = ds
    ...
    def prmbqq(self, args):
        """ prmbqq(args)
        args - arguments to the function that constructs url parameters
        returns parameters for appending to the BEA API url
        """
        ds = args.DatasetName
        if ds == 'NIPA':
    ...
    def callbqq(self, args):
        """ callbqq(args)
        args - arguments to the beaqueryq function
    ...
    def testdatasets(self, jsf):
        """ testdatasets
        using the BEA dataset hierarchy generated from the BEA API
        exhaustively test beaqueryq.py against the BEA site
        ...
        with open(jsf) as jfp:
            ...
            for i in range(1, len(dsa)):
                ...
                if cp == None:
                    print('testdatasets: %s no cartesian product' % (ds))
                    return
                cpa = list(cp)
                for p in cpa:
                    # print(p)
                    args = self.dsp2ns(js, ds, dsp, p)
                    #try:
