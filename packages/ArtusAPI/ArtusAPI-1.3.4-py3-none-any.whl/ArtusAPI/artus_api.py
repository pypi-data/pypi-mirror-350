...
def main_flash():
    try:
        ...
        "master_64_left": "https://gist.githubusercontent.com/SDRyanLee/ad8bbe17164c74e6417d1e2d4d0843d2/raw/master_64_left.txt",
        "master_plus_64_left": "https://gist.githubusercontent.com/SDRyanLee/ad8bbe17164c74e6417d1e2d4d0843d2/raw/master_plus_64_left.txt"
        }
        ...
        # add required arguments
        parser.add_argument('-p','--port',required=True,help="required com port (COMx) or (/dev/ttyUSBx)")
        parser.add_argument('-r','--robot',required=True,default='artus_lite',help="Robot type",choices=['lite','lite_plus'])
        parser.add_argument('-s','--side',required=True,default='right',choices=['left','right'],help='specify hand side, left or right')
        # parser.add_argument('-b','--baudrate',default=921600,help='specify baudrate',choices=[921600,115200])
        args = parser.parse_args()
        ...
        if type_flash == 'actuator':
            ...
            if args.robot == 'lite':
                if args.side == 'right':
                    ...
                    print('Flashing master lite right')
                elif args.side == 'left':
                    file_location = bin_paths['master_64_left']
                    print('Flashing master lite left')
            elif args.robot == 'lite_plus':
                if args.side == 'right':
                    file_location = bin_paths['master_plus_64_right']
                    print('Flashing master lite plus right')
                elif args.side == 'left':
                    file_location = bin_paths['master_plus_64_left']
                    print('Flashing master lite plus left')
