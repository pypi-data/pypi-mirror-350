...
from purgo_ingest.cloning import check_repo_exists, clone_repo
from purgo_ingest.schemas import CloneConfig
from purgo_ingest.utils.exceptions import AsyncTimeoutError
...
async def test_clone_with_commit() -> None:
    ...
    with patch("purgo_ingest.cloning.check_repo_exists", return_value=True) as mock_check:
        with patch("purgo_ingest.cloning.run_command", new_callable=AsyncMock) as mock_exec:
...
async def test_clone_nonexistent_repository() -> None:
    ...
    with patch("purgo_ingest.cloning.check_repo_exists", return_value=False) as mock_check:
...
async def test_clone_with_custom_branch() -> None:
    ...
    with patch("purgo_ingest.cloning.check_repo_exists", return_value=True):
        with patch("purgo_ingest.cloning.run_command", new_callable=AsyncMock) as mock_exec:
            await clone_repo(clone_config)
...
async def test_git_command_failure() -> None:
    ...
    with patch("purgo_ingest.cloning.check_repo_exists", return_value=True):
        with patch("purgo_ingest.cloning.run_command", side_effect=RuntimeError("Git command failed")):
...
async def test_clone_specific_branch(tmp_path):
    ...
    """
    repo_url = "https://github.com/cyclotruc/purgo_ingest.git"
    branch_name = "main"
    local_path = tmp_path / "purgo_ingest"
