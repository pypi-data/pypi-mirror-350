...
class BasicItems:
    ...
    @staticmethod
    def edit_event_tickets(save_file: core.SaveFile):
        names_o = core.core_data.get_gatya_item_names(save_file)
        items = core.core_data.get_gatya_item_buy(save_file)
        items_1 = items.get_by_gatya_ticket_id(95)  # lucky ticket
        print()
        if items_1 is None:
            return
        print(len(items_1))
        items_2 = items.get_by_gatya_ticket_id(96)  # lucky ticket g
        if items_2 is None:
            return
        print(len(items_2))
        items_3 = items.get_by_gatya_ticket_id(97)  # misc tickets
        if items_3 is None:
            return
        print(len(items_3))
        ...
        items_4 = items.get_by_category(1)
        if items_4 is None:
            return
        items_5 = items.get_by_category(8)
        if items_5 is None:
            return
        items_6 = items.get_by_category(10)
        if items_6 is None:
            return

        for i, item in enumerate(items_4):
            print(names_o.get_name(item.id), item.id, i, item.index)
        for i, item in enumerate(items_5):
            print(names_o.get_name(item.id), item.id, i, item.index)

        for i, item in enumerate(items_6):
            print(names_o.get_name(item.id), item.id, i, item.index)
        ...
        # id of event_capsules_1 = index of group 1
        # id of event_capsules_counter = index of group_1 + 2 (as)
        # id of lucky_tickets = index of group 8

        print(len(save_file.event_capsules))
        print(len(save_file.event_capsules_counter))
        print(len(save_file.lucky_tickets))

        for i in range(0, len(save_file.event_capsules)):
            save_file.event_capsules[i] = i + 2

        save_file.event_capsules_counter[-3] = 5

        save_file.event_capsules[-1] = 110
        save_file.event_capsules[-2] = 120
        save_file.event_capsules[-3] = 130

        server_handler = ServerHandler(save_file)

        gatya_data = server_handler.get_event_data("gatya.tsv")
