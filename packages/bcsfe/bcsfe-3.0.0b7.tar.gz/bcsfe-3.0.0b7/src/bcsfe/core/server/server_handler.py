...
class ServerHandler:
    ...
    def get_event_data(self, filename: str) -> str | None:
        token = self.get_auth_token()
        if token is None:
            return None
        url = f"https://nyanko-events.ponosgames.com/battlecats{self.save_file.cc.get_patching_code()}_production/{filename}?jwt={token}"
        print(url)
