...
def full_pyfile_or_archive_path(spark: SparkSession, target_file_name: str) -> str:
    """
    Resolve the full HDFS path of a file out of the Spark Session's PyFiles
    object and archives object.
    """
    spark_conf = spark.sparkContext.getConf()
    py_files = spark_conf.get("spark.yarn.dist.pyFiles")
    archives = spark_conf.get("spark.yarn.dist.archives")

    source = ", ".join(x for x in [py_files, archives] if x)
    file_names = source.split(",")

    log.info(
        f"Searching py_files and archives for {target_file_name}: '{source}'",
    ...
    for file_name in file_names:
        print(file_name)
        if file_name.split("/")[-1] == target_file_name:
            print("found")
            return file_name

    raise PublicValueError(
        f"py_files or archives do not contain {target_file_name}: {source}"
    )
...
def run_spark_net_from_known_assembly(
    ...
    """
    fully_resolved_zip_file_name = full_pyfile_or_archive_path(spark, zip_file_name)
