...
def component_spec_yaml_exists_and_is_parsable(component_spec_path):
    ...
    if use_component_sdk:
        try:
            definition = load_component(source=component_spec_path)
...
def component_uses_private_acr_componentsdk(component_spec_path, definition, acr_url):
    ...
    try:
        if hasattr(definition.environment, "docker"):
            base_image_url = definition.environment.docker["image"]
        else:
            base_image_url = definition.environment.image
...
def component_uses_private_python_feed(component_spec_path, feed_url):
    ...
    if use_component_sdk:
        ...
        try:
            if hasattr(definition.environment, "conda") and (
                hasattr(definition.environment.conda, "conda_dependencies")
                or "conda_dependencies_file" in definition.environment.conda
            ):
                if hasattr(definition.environment.conda, "conda_dependencies"):
                    conda_deps_yaml = (
                        definition.environment.conda.conda_dependencies._to_dict()
                    )
                else:
                    conda_deps_yaml = definition.environment.conda["conda_dependencies"]
            else:
                conda_file = None
                if hasattr(definition.environment, "conda"):
                    conda_file = definition.environment.conda["conda_dependencies_file"]
                else:
                    conda_file = definition.environment.conda_file
                conda_deps_abspath = os.path.join(
                    os.path.dirname(component_spec_path), conda_file
                )
                assert os.path.isfile(
                    conda_deps_abspath
                ), "Component {} specified a conda_dependencies_file {} that cannot be found (abspath: {})".format(
                    component_spec_path, conda_file, conda_deps_abspath
                )
                conda_deps_yaml = yaml.safe_load(open(conda_deps_abspath, "r"))
        except KeyError:
            ...
            assert (
                ...
                component_spec_path, conda_deps_abspath, e
            )
    else:
...
def find_run_py_in_command(definition, use_component_sdk):
    ...
    if use_component_sdk:
        definition_type = definition.type
        if definition_type == "HDInsightComponent":
            run_py_command = definition.hdinsight["file"]
            definition_command = definition.hdinsight["args"]
        elif definition_type == "DistributedComponent":
            # run_py_command not provided, we need to find it
            definition_command = definition.launcher["additional_arguments"]
        elif definition_type in ["ParallelComponent", "parallel"]:
            run_py_command = definition.entry
            definition_command = definition.args
        elif definition_type in ["CommandComponent", "command"]:
            # run_py_command not provided, we need to find it
            definition_command = definition.command
        elif definition_type == "spark":
            run_py_command = definition.entry.file
            definition_command = definition.args
        ...
        if (
            ...
            and definition.type != "ScopeComponent"
            and definition.type != "DataTransferComponent"
        ):
...
def _generate_fake_input_arg_componentsdk(arg_spec):
    ...
    if "string" in arg_spec.type.lower():
        return "0"
    if "integer" in arg_spec.type.lower():
        return arg_spec.min or arg_spec.max or "0"
    if "boolean" in arg_spec.type.lower():
        return False
    if "float" in arg_spec.type.lower() or "number" in arg_spec.type.lower():
        return arg_spec.min or arg_spec.max or "0.32"
    if "enum" in arg_spec.type.lower():
    ...
    if isinstance(arg, list):  # optional argument or root list
        ...
        if io_key.startswith("inputs."):
            ...
            if input_key in component_spec.inputs:
                ...
                hasattr(component_spec, "parameters")
                and input_key in component_spec.parameters
            ):
                output_script_arguments.append(
                    str(
                        _generate_fake_input_arg_componentsdk(
...
def _generate_fake_input_arg_modulesdk(arg_spec):
    ...
    if arg_spec["type"] == "AzureMLDataset":
        return "/mnt/fakeinputdatasetpath"
    if arg_spec["type"] in ["AnyDirectory", "uri_folder"]:
        return "/mnt/fakeinputdirectorypath"
    if arg_spec["type"] in ["AnyFile", "uri_file"]:
        return "/mnt/fakeinputfilepath/file.txt"
    if "default" in arg_spec:
        return arg_spec["default"]
    if arg_spec["type"].lower() == "string":
        return "0"
    if arg_spec["type"].lower() == "integer":
        return "0"
    if arg_spec["type"].lower() == "boolean":
        return False
    if arg_spec["type"].lower() in ["float", "number"]:
        return "0.32"
    if arg_spec["type"].lower() == "enum":
