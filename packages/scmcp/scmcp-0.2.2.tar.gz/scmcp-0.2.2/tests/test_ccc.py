...
async def test_ccc_communicate():
    ...
    async with Client(sc_mcp) as client:
        ...
        result = await client.call_tool("li_ccc_communicate", {
            "request": {
                "method": "cellphonedb",
                "groupby": "bulk_labels"
            }
...
async def test_rank_aggregate():
    ...
    async with Client(sc_mcp) as client:
        # First read the data
        result = await client.call_tool("sc_io_read", {"request": {"filename": test_dir}, "adinfo":{ "sampleid": "pbmc68k", "adtype": "exp"}})
        assert "AnnData" in result[0].text

        # Run rank aggregation
        result = await client.call_tool("li_ccc_rank_aggregate", {
            "request": {
