...
'''Nawalny nickname for New AWesome Access Library to Nm in pYthon    autotest '''
...
def usage(arg0):
    fmt = '''Usage: {:s} [--help] [--debug] --conf=<file>] --cmd=<cmd>
  cmd among {:s} 
...
def mygetopt(cmd,largs):
    ...
    return sacmd,scmd,bdebug,sconffn
...
if __name__ == '__main__':
    sacmd,scmd,bdebug,sconffilename = mygetopt(sys.argv[0],sys.argv[1:])
    ologger.debug("%s %s %s %s",sacmd,scmd,bdebug,sconffilename )
...
#    dparam_default = {'endUserId':"Nanwalnytest",'sendTime': snawalnytodaytime()}
    ...
    print(lret)
    if lret[0] : 
        print("Technical Acces to nmb2b is valid")
    else:
        print("Technical Acces to nmb2b is INVALID.stop.")
        sys.exit(0)
    
    lret = onmaccess.test_std()        
    print(lret[1])
    if lret[0] : 
        print("Soap Acces to nmb2b is valid")
    else:
        print("Soap Acces to nmb2b is INVALID")
        sys.exit(1)       
    try:
