...
# Function to get resource summarization prompt with substituted values
def get_resource_summarization_prompt(
    ...
    return create_resource_summarization_prompt(
        resource_yaml, resource_kind, resource_name, edit_context
    )


# Prompt for converting edited summary back to patch
SUMMARY_TO_PATCH_PROMPT: PromptFragments = create_planning_prompt(
    command="patch",
    description=(
        ...
        "Always use strategic merge patches unless JSON patch is specifically needed."
    ),
    examples=Examples(
        [
            (
            ...
            (
                ...
                "memory limit added as 512Mi",
                {
                    ...
                    "explanation": (
                        "Diff shows image version update and memory limit addition. "
                        "Using strategic merge patch to update both container image "
                        "and resource limits."
                    ),
                },
            ),
...
def plan_edit_scope(
    ...
    return PromptFragments((system_frags, UserFragments(user_frags)))
...
def get_patch_generation_prompt(
    ...
    """
    system_fragments, user_fragments_base = SUMMARY_TO_PATCH_PROMPT
