...
run-dev:
	uv tool run . agora-mcp

# When a new version is not found, you can refresh the package with:  uvx --refresh agora-mcp@0.1.1
run:
	uvx --refresh agora-mcp
...
# 4.a Test the release locally using the release wheel. I am not sure why the command `agora-mcp` can't be ommited here.
run-release: build
	uv tool run --with dist/agora_mcp-0.1.0-py3-none-any.whl agora-mcp
...
run-release-client: build
	uv --directory /Users/pengren/go/github.com/Fewsats/agora-mcp run agora-mcp
