...
from qubitron.study import ResultDict


def _create_test_circuits(qubits: list[qubitron.Qid], n_circuits: int) -> list[qubitron.Circuit]:
    """Helper function to generate circuits for testing."""
    if len(qubits) < 2:
        raise ValueError(
            "Need at least two qubits to generate two-qubit circuits."
        )  # pragma: no cover
    two_qubit_gates = [qubitron.ISWAP**0.5, qubitron.CNOT**0.5]
    input_circuits = []
    ...
    for i in range(n_circuits):
        ...
        for circuit in circuits:
            circuit.append(qubitron.measure(*qubits, key="m"))
...
def test_shuffled_circuits_with_readout_benchmarking_errors_no_noise():
    ...
    measurements, readout_calibration_results = (
        qubitron.contrib.shuffle_circuits.run_shuffled_with_readout_benchmarking(
            input_circuits,
            sampler,
            circuit_repetitions,
            rng,
    ...
    for qlist, readout_calibration_result in readout_calibration_results.items():
        ...
        assert all(isinstance(q, qubitron.Qid) for q in qlist)
...
def test_shuffled_circuits_with_readout_benchmarking_errors_with_noise_and_lists_input_qubits():
    ...
    qubits_2 = qubitron.LineQubit.range(4)
...
def test_non_circuit_input():
    """Test that the input circuits are not of type qubitron.Circuit."""
    q = qubitron.LineQubit(0)
    with pytest.raises(ValueError, match="Input circuits must be of type qubitron.Circuit."):
        qubitron.contrib.shuffle_circuits.run_shuffled_with_readout_benchmarking(
            [q],
            qubitron.ZerosSampler(),
            circuit_repetitions=10,
...
def test_no_measurements():
    ...
    q = qubitron.LineQubit(0)
    circuit = qubitron.Circuit(qubitron.H(q))
