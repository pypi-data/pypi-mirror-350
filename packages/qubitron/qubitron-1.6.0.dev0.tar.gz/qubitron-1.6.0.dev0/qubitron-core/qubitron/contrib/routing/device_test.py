...
def test_get_linear_device_graph(n_qubits):
    ...
    assert sorted(graph) == qubitron.LineQubit.range(n_qubits)
    assert len(graph.edges()) == n_qubits - 1
    assert all(abs(a.x - b.x) == 1 for a, b in graph.edges())


def test_nx_qubit_layout():
    grid_qubit_graph = ccr.gridqubits_to_graph_device(qubitron.GridQubit.rect(5, 5))
...
def test_nx_qubit_layout_2():
    g = nx.from_edgelist(
        [(qubitron.LineQubit(0), qubitron.LineQubit(1)), (qubitron.LineQubit(1), qubitron.LineQubit(2))]
    )
...
def test_nx_qubit_layout_3():
    g = nx.from_edgelist(
        [(qubitron.NamedQubit('a'), qubitron.NamedQubit('b')), (qubitron.NamedQubit('b'), qubitron.NamedQubit('c'))]
    )
    node_to_i = {qubitron.NamedQubit('a'): 0, qubitron.NamedQubit('b'): 1, qubitron.NamedQubit('c'): 2}
