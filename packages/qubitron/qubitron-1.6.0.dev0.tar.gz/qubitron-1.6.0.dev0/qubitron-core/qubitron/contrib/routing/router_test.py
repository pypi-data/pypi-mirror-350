...
def test_route_circuit(n_moments, algo, circuit_seed, routing_seed):
    circuit = qubitron.testing.random_circuit(10, n_moments, 0.5, random_state=circuit_seed)
...
def test_route_circuit_reproducible_with_seed(algo, seed):
    circuit = qubitron.testing.random_circuit(8, 20, 0.5, random_state=seed)
    device_graph = ccr.get_grid_device_graph(4, 3)
    ...
    eq = qubitron.testing.equals_tester.EqualsTester()
...
def test_route_circuit_reproducible_between_runs(algo):
    seed = 23
    circuit = qubitron.testing.random_circuit(6, 5, 0.5, random_state=seed)
...
def test_route_circuit_via_unitaries(n_moments, algo, seed, make_bad):
    circuit = qubitron.testing.random_circuit(4, n_moments, 0.5, random_state=seed)
    ...
    if make_bad:
        swap_network.circuit += [qubitron.CNOT(*physical_qubits[:2])]
...
def test_router_bad_args():
    ...
    circuit = qubitron.Circuit(qubitron.CCZ(*qubitron.LineQubit.range(3)))
    with pytest.raises(ValueError):
        ccr.route_circuit(circuit, device_graph, algo_name=algo_name)

    circuit = qubitron.Circuit(qubitron.CZ(qubitron.LineQubit(i), qubitron.LineQubit(i + 1)) for i in range(5))
    with pytest.raises(ValueError):
        ccr.route_circuit(circuit, device_graph, algo_name=algo_name)
