...
from qubitron.contrib.acquaintance.gates import acquaint
from qubitron.contrib.acquaintance.permutation import PermutationGate, SwapPermutationGate
...
class BipartiteGraphType(enum.Enum):
    ...
    def __repr__(self) -> str:
        return 'qubitron.contrib.acquaintance.bipartite.BipartiteGraphType.' + self.name
...
class BipartiteSwapNetworkGate(PermutationGate):
    ...
    def __init__(
        self, subgraph: str | BipartiteGraphType, part_size: int, swap_gate: qubitron.Gate = ops.SWAP
    ) -> None:
    ...
    def decompose_complete(self, qubits: Sequence[qubitron.Qid]) -> Iterator[qubitron.OP_TREE]:
    ...
    def decompose_matching(self, qubits: Sequence[qubitron.Qid]) -> Iterator[qubitron.OP_TREE]:
        swap_gate = SwapPermutationGate(self.swap_gate)
    ...
    def __repr__(self) -> str:
        ...
        args_str = ', '.join(args)
        return f'qubitron.contrib.acquaintance.bipartite.BipartiteSwapNetworkGate({args_str})'
