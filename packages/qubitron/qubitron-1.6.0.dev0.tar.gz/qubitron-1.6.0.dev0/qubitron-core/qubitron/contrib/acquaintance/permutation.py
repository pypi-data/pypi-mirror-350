...
from qubitron import circuits, ops, protocols, transformers, value
...
LogicalIndexSequence = Sequence[int] | Sequence['qubitron.Qid']
...
class PermutationGate(ops.Gate, metaclass=abc.ABCMeta):
    ...
    def __init__(self, num_qubits: int, swap_gate: qubitron.Gate = ops.SWAP) -> None:
    ...
    def update_mapping(
        self, mapping: dict[ops.Qid, LogicalIndex], keys: Sequence[qubitron.Qid]
    ) -> None:
    ...
    ) -> str | Iterable[str] | qubitron.CircuitDiagramInfo:
...
def display_mapping(circuit: qubitron.Circuit, initial_mapping: LogicalMapping) -> None:
...
class SwapPermutationGate(PermutationGate):
    ...
    def __init__(self, swap_gate: qubitron.Gate = ops.SWAP):
...
class LinearPermutationGate(PermutationGate):
    ...
    def __init__(
        self, num_qubits: int, permutation: dict[int, int], swap_gate: qubitron.Gate = ops.SWAP
    ) -> None:
    ...
    def __repr__(self) -> str:
        return (
            'qubitron.contrib.acquaintance.LinearPermutationGate('
...
def update_mapping(mapping: dict[ops.Qid, LogicalIndex], operations: qubitron.OP_TREE) -> None:
...
def get_logical_operations(
    operations: qubitron.OP_TREE, initial_mapping: dict[ops.Qid, ops.Qid]
) -> Iterable[qubitron.Operation]:
    ...
    """
    mapping = initial_mapping.copy()
    for op in cast(Iterable['qubitron.Operation'], ops.flatten_op_tree(operations)):
        if isinstance(op, ops.GateOperation) and isinstance(op.gate, PermutationGate):
            op.gate.update_mapping(mapping, op.qubits)
        else:
            for q in op.qubits:
...
class DecomposePermutationGates:
    ...
    def optimize_circuit(self, circuit: qubitron.Circuit) -> None:
        circuit._moments = [*transformers.expand_composite(circuit, no_decomp=self.no_decomp)]

    def __call__(self, circuit: qubitron.Circuit) -> None:
...
def return_to_initial_mapping(circuit: qubitron.Circuit, swap_gate: qubitron.Gate = ops.SWAP) -> None:
    qubits = sorted(circuit.all_qubits())
    n_qubits = len(qubits)
...
def uses_consistent_swap_gate(circuit: qubitron.Circuit, swap_gate: qubitron.Gate) -> bool:
