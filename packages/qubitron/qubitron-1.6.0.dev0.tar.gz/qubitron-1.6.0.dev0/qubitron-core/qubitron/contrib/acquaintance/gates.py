...
from qubitron.contrib.acquaintance.permutation import (
    LinearPermutationGate,
    PermutationGate,
    SwapPermutationGate,
)
from qubitron.contrib.acquaintance.shift import CircularShiftGate
...
def operations_to_part_lens(
    qubit_order: Sequence[qubitron.Qid], op_tree: qubitron.OP_TREE
) -> tuple[int, ...]:
    ...
    singletons: list[tuple[qubitron.Qid, ...]] = [(q,) for q in set(qubit_order).difference(*op_parts)]
...
class AcquaintanceOpportunityGate(ops.Gate, ops.InterchangeableQubitsGate):
    ...
    def __repr__(self) -> str:
        return (
            'qubitron.contrib.acquaintance.AcquaintanceOpportunityGate('
            f'num_qubits={self.num_qubits()!r})'
        )

    def _circuit_diagram_info_(self, args: qubitron.CircuitDiagramInfoArgs) -> Iterable[str]:
...
def acquaint(*qubits) -> qubitron.Operation:
...
Layers = NamedTuple(
    'Layers',
    [
        ('prior_interstitial', list['qubitron.Operation']),
        ('pre', list['qubitron.Operation']),
        ('intra', list['qubitron.Operation']),
        ('post', list['qubitron.Operation']),
        ('posterior_interstitial', list['qubitron.Operation']),
    ],
)


def new_layers(**kwargs: list[qubitron.Operation]) -> Layers:
    return Layers._make(kwargs.get(field, []) for field in Layers._fields)


def acquaint_insides(
    swap_gate: qubitron.Gate,
    acquaintance_gate: qubitron.Operation,
...
def acquaint_and_shift(
    parts: tuple[list[qubitron.Qid], list[qubitron.Qid]],
    layers: Layers,
...
class SwapNetworkGate(PermutationGate):
    ...
    def __init__(
        ...
        swap_gate: qubitron.Gate = ops.SWAP,
    ) -> None:
    ...
    @staticmethod
    def from_operations(
        qubit_order: Sequence[qubitron.Qid],
    ...
    def __repr__(self) -> str:
        return (
            'qubitron.contrib.acquaintance.SwapNetworkGate('
