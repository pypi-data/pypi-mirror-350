...
def test_complete_acquaintance_strategy() -> None:
    qubits = [qubitron.NamedQubit(s) for s in alphabet]
...
h: ───×(7,0)───
    ...
    quadratic_strategy = qubitron.expand_composite(quadratic_strategy, no_decomp=is_shift_or_lin_perm)
...
def test_rectification() -> None:
    ...
    strategy = qubitron.Circuit(operations)
