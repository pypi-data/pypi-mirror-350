...
def fixup_text(text: str):
    if '\n' in text:
        # https://github.com/amyssnippet/Qubitron/issues/4499
        # TODO: Visualize Custom MatrixGate
        return '?'
    # https://github.com/amyssnippet/Qubitron/issues/2905
    text = text.replace('[<virtual>]', '')
    text = text.replace('[qubitron.VirtualTag()]', '')
    text = text.replace('&', '&amp;')
...
def _fit_horizontal(
    tdd: qubitron.TextDiagramDrawer, ref_boxwidth: float, col_padding: float
...
def _fit_vertical(
    tdd: qubitron.TextDiagramDrawer, ref_boxheight: float, row_padding: float
...
def tdd_to_svg(
    tdd: qubitron.TextDiagramDrawer,
...
def _validate_circuit(circuit: qubitron.Circuit):
    if len(circuit) == 0:
        raise ValueError("Can't draw SVG diagram for empty circuits")


class SVGCircuit:
    """A wrapper around qubitron.Circuit to enable rich display in a Jupyter
    notebook.
    ...
    def __init__(self, circuit: qubitron.Circuit):
...
def circuit_to_svg(circuit: qubitron.Circuit) -> str:
