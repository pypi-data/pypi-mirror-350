...
from qubitron import linalg, ops, protocols, transformers
...
def _matrix_to_clifford_op(
    mat: np.ndarray, qubit: qubitron.Qid, *, atol: float
...
def _matrix_to_pauli_string_phasors(
    mat: np.ndarray, qubit: qubitron.Qid, *, keep_clifford: bool, atol: float
) -> ops.OP_TREE:
    rotations = transformers.single_qubit_matrix_to_pauli_rotations(mat, atol)
...
class CliffordTargetGateset(transformers.TwoQubitCompilationTargetGateset):
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            single_qubit_target: Specifies the decomposition strategy for single qubit gates.
                SINGLE_QUBIT_CLIFFORDS: Decompose all single qubit gates to
                    `qubitron.SingleQubitCliffordGate`.
                PAULI_STRING_PHASORS_AND_CLIFFORDS: Accept both `qubitron.SingleQubitCliffordGate` and
                    `qubitron.PauliStringPhasorGate`; but decompose unknown gates into
        ...
        gates: list[qubitron.Gate | type[qubitron.Gate]] = [ops.CZ, ops.MeasurementGate]
    ...
    def _decompose_single_qubit_operation(
        self, op: qubitron.Operation, _
    ) -> NotImplementedType | qubitron.OP_TREE:
    ...
    def postprocess_transformers(self) -> list[qubitron.TRANSFORMER]:
        ...
        def rewriter(o: qubitron.CircuitOperation):
