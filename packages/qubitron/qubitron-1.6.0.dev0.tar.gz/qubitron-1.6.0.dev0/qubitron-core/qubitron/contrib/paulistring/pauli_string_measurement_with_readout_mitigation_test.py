...
import qubitron
from qubitron.contrib.paulistring import measure_pauli_strings
from qubitron.experiments import SingleQubitReadoutCalibrationResult
from qubitron.experiments.single_qubit_readout_calibration_test import NoisySingleQubitReadoutSampler


def _create_ghz(number_of_qubits: int, qubits: Sequence[qubitron.Qid]) -> qubitron.Circuit:
    ghz_circuit = qubitron.Circuit(
        qubitron.H(qubits[0]),
        *[qubitron.CNOT(qubits[i - 1], qubits[i]) for i in range(1, number_of_qubits)],
    )
    return ghz_circuit


def _generate_random_pauli_string(
    qubits: Sequence[qubitron.Qid], enable_coeff: bool = False, allow_pauli_i: bool = True
):
    pauli_ops = [qubitron.I, qubitron.X, qubitron.Y, qubitron.Z]

    if not allow_pauli_i:
        pauli_ops = [qubitron.X, qubitron.Y, qubitron.Z]
    ...
    if enable_coeff:
        ...
        return coefficient * qubitron.PauliString(operators)
    return qubitron.PauliString(operators)


def _generate_qwc_paulis(
    input_pauli: qubitron.PauliString, num_output: int, exclude_input_pauli: bool = False
) -> list[qubitron.PauliString]:
    ...
    for qubit in qubits:
        pauli_op = input_pauli.get(qubit, qubitron.I)

        allowed_pauli_op = []
        if pauli_op == qubitron.I:
            allowed_pauli_op = [qubitron.I, qubitron.X, qubitron.Y, qubitron.Z]  # pragma: no cover
        elif pauli_op == qubitron.X:
            allowed_pauli_op = [qubitron.I, qubitron.X]
        elif pauli_op == qubitron.Y:
            allowed_pauli_op = [qubitron.I, qubitron.Y]
        elif pauli_op == qubitron.Z:
            allowed_pauli_op = [qubitron.I, qubitron.Z]

        allowed_paulis_per_qubit.append(allowed_pauli_op)

    qwc_paulis: list[qubitron.PauliString] = []
    ...
    for pauli_combination in itertools.product(*allowed_paulis_per_qubit):
        ...
        qwc_pauli: qubitron.PauliString = qubitron.PauliString(pauli_dict)
        if exclude_input_pauli and qwc_pauli == input_pauli:
            continue  # pragma: no cover
        if all(q == qubitron.I for q in qwc_pauli):
            continue
...
def _ideal_expectation_based_on_pauli_string(
    pauli_string: qubitron.PauliString, final_state_vector: np.ndarray
) -> float:
...
def test_pauli_string_measurement_errors_no_noise() -> None:
    ...
    circuit = qubitron.FrozenCircuit(_create_ghz(5, qubits))
    sampler = qubitron.Simulator()

    circuits_to_pauli: dict[qubitron.FrozenCircuit, list[qubitron.PauliString]] = {}
    ...
    for circuit_with_pauli_expectations in circuits_with_pauli_expectations:
        assert isinstance(circuit_with_pauli_expectations.circuit, qubitron.FrozenCircuit)
...
def test_group_pauli_string_measurement_errors_no_noise_with_coefficient() -> None:
    ...
    circuits_to_pauli: dict[qubitron.FrozenCircuit, list[list[qubitron.PauliString]]] = {}
    ...
    circuits_to_pauli[circuit].append([qubitron.PauliString({q: qubitron.X for q in qubits})])
...
def test_pauli_string_measurement_errors_with_noise() -> None:
    ...
    circuit = qubitron.FrozenCircuit(_create_ghz(7, qubits))
...
def test_many_circuits_input_measurement_with_noise() -> None:
    ...
    qubits_1 = qubitron.LineQubit.range(3)
    qubits_2 = [
    ...
    qubits_3 = qubitron.LineQubit.range(8)

    circuit_1 = qubitron.FrozenCircuit(_create_ghz(3, qubits_1))
    circuit_2 = qubitron.FrozenCircuit(_create_ghz(5, qubits_2))
    circuit_3 = qubitron.FrozenCircuit(_create_ghz(8, qubits_3))
...
def test_coefficient_not_real_number() -> None:
    ...
    with pytest.raises(
        ...
        measure_pauli_strings(
            circuits_to_pauli, qubitron.Simulator(), 1000, 1000, 1000, np.random.default_rng()
        )
...
def test_invalid_input_circuit_type() -> None:
    ...
    qubits_to_pauli: dict[tuple, list[qubitron.PauliString]] = {}
    qubits_to_pauli[tuple(qubits)] = [qubitron.PauliString({q: qubitron.X for q in qubits})]
    with pytest.raises(
...
def test_invalid_input_pauli_string_type() -> None:
    ...
    qubits_1 = qubitron.LineQubit.range(5)
    qubits_2 = [
        qubitron.GridQubit(0, 1),
        qubitron.GridQubit(1, 1),
        qubitron.GridQubit(1, 0),
    ...
    circuit_1 = qubitron.FrozenCircuit(_create_ghz(5, qubits_1))
    circuit_2 = qubitron.FrozenCircuit(_create_ghz(5, qubits_2))

    circuits_to_pauli: dict[qubitron.FrozenCircuit, qubitron.FrozenCircuit] = {}
    ...
    with pytest.raises(
        TypeError,
        match="All elements in the Pauli string lists must be qubitron.PauliString "
        "instances, got <class 'qubitron.circuits.frozen_circuit.FrozenCircuit'>.",
    ):
        measure_pauli_strings(
...
def test_all_pauli_strings_are_pauli_i() -> None:
    ...
    circuits_to_pauli: dict[qubitron.FrozenCircuit, list[qubitron.PauliString]] = {}
    circuits_to_pauli[circuit_1] = [
        qubitron.PauliString({q: qubitron.I for q in qubits_1}),
        qubitron.PauliString({q: qubitron.X for q in qubits_1}),
    ]
    circuits_to_pauli[circuit_2] = [qubitron.PauliString({q: qubitron.X for q in qubits_2})]
...
def test_zero_pauli_repetitions() -> None:
    ...
    circuits_to_pauli: dict[qubitron.FrozenCircuit, list[qubitron.PauliString]] = {}
    circuits_to_pauli[circuit] = [qubitron.PauliString({q: qubitron.X for q in qubits})]
    with pytest.raises(ValueError, match="Must provide non-zero pauli_repetitions."):
        measure_pauli_strings(
            circuits_to_pauli, qubitron.Simulator(), 0, 1000, 1000, np.random.default_rng()
        )
...
def test_negative_num_random_bitstrings() -> None:
    ...
    with pytest.raises(ValueError, match="Must provide zero or more num_random_bitstrings."):
        measure_pauli_strings(
            circuits_to_pauli, qubitron.Simulator(), 1000, 1000, -1, np.random.default_rng()
        )
...
def test_zero_readout_repetitions() -> None:
    ...
    with pytest.raises(
        ...
        measure_pauli_strings(
            circuits_to_pauli, qubitron.Simulator(), 1000, 0, 1000, np.random.default_rng()
        )
...
def test_rng_type_mismatch() -> None:
    ...
    with pytest.raises(ValueError, match="Must provide a numpy random generator or a seed"):
        measure_pauli_strings(
            circuits_to_pauli, qubitron.Simulator(), 1000, 1000, 1000, "test"  # type: ignore[arg-type]
        )
...
def test_pauli_type_mismatch() -> None:
    ...
    circuits_to_pauli: dict[qubitron.FrozenCircuit, int] = {}
...
def test_group_paulis_are_not_qwc() -> None:
    ...
    pauli_str1: qubitron.PauliString = qubitron.PauliString({qubits[0]: qubitron.X, qubits[1]: qubitron.Y})
    pauli_str2: qubitron.PauliString = qubitron.PauliString({qubits[0]: qubitron.Y})
...
def test_group_paulis_type_mismatch() -> None:
    ...
    with pytest.raises(
        ...
        "but found <class 'qubitron.ops.pauli_string.PauliString'>.",
    ):
        measure_pauli_strings(
            circuits_to_pauli, qubitron.Simulator(), 1000, 1000, 1000, np.random.default_rng()
        )
