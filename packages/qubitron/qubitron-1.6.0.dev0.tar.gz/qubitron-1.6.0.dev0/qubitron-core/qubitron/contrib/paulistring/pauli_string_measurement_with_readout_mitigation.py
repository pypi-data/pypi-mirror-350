...
from qubitron import circuits, ops, work
from qubitron.contrib.shuffle_circuits import run_shuffled_with_readout_benchmarking
...
def _validate_group_paulis_qwc(
    ...
    Args:
        pauli_strings: A list of qubitron.PauliString objects.
...
def _validate_single_pauli_string(pauli_str: ops.PauliString):
    if not isinstance(pauli_str, ops.PauliString):
        raise TypeError(
            f"All elements in the Pauli string lists must be qubitron.PauliString "
...
def _pauli_strings_to_basis_change_ops(
    ...
    for qubit in qid_list:
        for pauli_str in pauli_strings:
            ...
            if pauli_op == ops.X:
                operations.append(ops.ry(-np.pi / 2)(qubit))  # =qubitron.H
                break
            elif pauli_op == ops.Y:
