...
from qubitron import Circuit, CircuitOperation, CX, FrozenCircuit, NamedQubit, ops, value
from qubitron.circuits.qasm_output import QasmUGate
from qubitron.contrib.qasm_import._lexer import QasmLexer
from qubitron.contrib.qasm_import.exception import QasmException
...
def _generate_op_qubits(args: list[list[ops.Qid]], lineno: int) -> list[list[ops.Qid]]:
    """Generates the Qubitron qubits for an operation from the OpenQASM qregs.
    ...
    op_qubits_gen = functools.reduce(
        cast(Callable[[list['qubitron.Qid'], list['qubitron.Qid']], list['qubitron.Qid']], np.broadcast), args
    )
...
class QasmGateStatement:
    ...
    to a list of `qubitron.GateOperation`s.

    Has the responsibility to validate the arguments
    and parameters of the call and to generate a list of corresponding
    `qubitron.GateOperation`s in the `on` method.
    """

    def __init__(
        self,
        qasm_gate: str,
        qubitron_gate: ops.Gate | Callable[[list[float]], ops.Gate],
        num_params: int,
        num_args: int,
    ):
        ...
        Args:
            ...
            qubitron_gate: The gate class on the qubitron side.
        ...
        self.qubitron_gate = qubitron_gate
    ...
    def on(
        ...
        final_gate: ops.Gate = (
            self.qubitron_gate if isinstance(self.qubitron_gate, ops.Gate) else self.qubitron_gate(params)
        )
...
class QasmParser:
    ...
    basic_gates: dict[str, QasmGateStatement] = {
        'CX': QasmGateStatement(qasm_gate='CX', qubitron_gate=CX, num_params=0, num_args=2),
        'U': QasmGateStatement(
            qasm_gate='U',
            num_params=3,
            num_args=1,
            # QasmUGate expects half turns
            qubitron_gate=(lambda params: QasmUGate(*[p / np.pi for p in params])),
        ),
    }

    qelib_gates = {
        'ccx': QasmGateStatement(qasm_gate='ccx', num_params=0, num_args=3, qubitron_gate=ops.CCX),
        'ch': QasmGateStatement(
            qasm_gate='ch', qubitron_gate=ops.ControlledGate(ops.H), num_params=0, num_args=2
        ),
        'crx': QasmGateStatement(
            qasm_gate='crx',
            num_params=1,
            num_args=2,
            qubitron_gate=(lambda params: ops.ControlledGate(ops.rx(params[0]))),
        ),
        'cry': QasmGateStatement(
            qasm_gate='cry',
            num_params=1,
            num_args=2,
            qubitron_gate=(lambda params: ops.ControlledGate(ops.ry(params[0]))),
        ),
        'crz': QasmGateStatement(
            qasm_gate='crz',
            num_params=1,
            num_args=2,
            qubitron_gate=(lambda params: ops.ControlledGate(ops.rz(params[0]))),
        ),
        'cswap': QasmGateStatement(
            qasm_gate='cswap', num_params=0, num_args=3, qubitron_gate=ops.CSWAP
        ),
        'cu1': QasmGateStatement(
            qasm_gate='cu1',
            num_params=1,
            num_args=2,
            qubitron_gate=(lambda params: ops.ControlledGate(QasmUGate(0, 0, params[0] / np.pi))),
        ),
        'cu3': QasmGateStatement(
            qasm_gate='cu3',
            num_params=3,
            num_args=2,
            qubitron_gate=(lambda params: ops.ControlledGate(QasmUGate(*[p / np.pi for p in params]))),
        ),
        'cx': QasmGateStatement(qasm_gate='cx', qubitron_gate=CX, num_params=0, num_args=2),
        'cy': QasmGateStatement(
            qasm_gate='cy', qubitron_gate=ops.ControlledGate(ops.Y), num_params=0, num_args=2
        ),
        'cz': QasmGateStatement(qasm_gate='cz', qubitron_gate=ops.CZ, num_params=0, num_args=2),
        'h': QasmGateStatement(qasm_gate='h', num_params=0, num_args=1, qubitron_gate=ops.H),
        'id': QasmGateStatement(
            qasm_gate='id', qubitron_gate=ops.IdentityGate(1), num_params=0, num_args=1
        ),
        'iswap': QasmGateStatement(
            qasm_gate='iswap', qubitron_gate=ops.ISwapPowGate(), num_params=0, num_args=2
        ),
        'r': QasmGateStatement(
            qasm_gate='r',
            num_params=2,
        ...
        'rx': QasmGateStatement(
            qasm_gate='rx', qubitron_gate=(lambda params: ops.rx(params[0])), num_params=1, num_args=1
        ),
        'ry': QasmGateStatement(
            qasm_gate='ry', qubitron_gate=(lambda params: ops.ry(params[0])), num_params=1, num_args=1
        ),
        'ryy': QasmGateStatement(
            qasm_gate='ryy',
            num_params=1,
            num_args=2,
            qubitron_gate=(lambda params: ops.YYPowGate(exponent=params[0] / np.pi)),
        ),
        'rz': QasmGateStatement(
            qasm_gate='rz', qubitron_gate=(lambda params: ops.rz(params[0])), num_params=1, num_args=1
        ),
        'rxx': QasmGateStatement(
            qasm_gate='rxx',
            num_params=1,
            num_args=2,
            qubitron_gate=(lambda params: ops.XXPowGate(exponent=params[0] / np.pi)),
        ),
        'rzz': QasmGateStatement(
            qasm_gate='rzz',
            num_params=1,
            num_args=2,
            qubitron_gate=(lambda params: ops.ZZPowGate(exponent=params[0] / np.pi)),
        ),
        's': QasmGateStatement(qasm_gate='s', num_params=0, num_args=1, qubitron_gate=ops.S),
        'sdg': QasmGateStatement(qasm_gate='sdg', num_params=0, num_args=1, qubitron_gate=ops.S**-1),
        'swap': QasmGateStatement(qasm_gate='swap', qubitron_gate=ops.SWAP, num_params=0, num_args=2),
        'sx': QasmGateStatement(
            qasm_gate='sx', num_params=0, num_args=1, qubitron_gate=ops.XPowGate(exponent=0.5)
        ),
        'sxdg': QasmGateStatement(
            qasm_gate='sxdg', num_params=0, num_args=1, qubitron_gate=ops.XPowGate(exponent=-0.5)
        ),
        't': QasmGateStatement(qasm_gate='t', num_params=0, num_args=1, qubitron_gate=ops.T),
        'tdg': QasmGateStatement(qasm_gate='tdg', num_params=0, num_args=1, qubitron_gate=ops.T**-1),
        'u1': QasmGateStatement(
            qasm_gate='u1',
            qubitron_gate=(lambda params: QasmUGate(0, 0, params[0] / np.pi)),
            num_params=1,
            num_args=1,
        ),
        'u2': QasmGateStatement(
            qasm_gate='u2',
            qubitron_gate=(lambda params: QasmUGate(0.5, params[0] / np.pi, params[1] / np.pi)),
            num_params=2,
            num_args=1,
        ),
        ...
        'x': QasmGateStatement(qasm_gate='x', num_params=0, num_args=1, qubitron_gate=ops.X),
        'y': QasmGateStatement(qasm_gate='y', num_params=0, num_args=1, qubitron_gate=ops.Y),
        'z': QasmGateStatement(qasm_gate='z', num_params=0, num_args=1, qubitron_gate=ops.Z),
    }
    ...
    def p_format(self, p):
        ...
        if p[1] not in ["2.0", "3.0"]:
            raise QasmException(
                ...
                "only 2.0 and 3.0 are supported currently by Qubitron"
            )
