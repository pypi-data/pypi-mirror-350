...
@lru_cache()
def _qpos_y(qubits: qubitron.Qid | tuple[qubitron.Qid, ...], all_qubits: tuple[qubitron.Qid, ...]) -> float:
...
def _add_to_positions(
    ...
    mi: int,
    qubits: qubitron.Qid | tuple[qubitron.Qid],
    *,
...
def circuit_to_density_matrix_tensors(
    circuit: qubitron.Circuit, qubits: Sequence[qubitron.Qid] | None = None
) -> tuple[list[qtn.Tensor], dict[qubitron.Qid, int], dict[tuple[str, str], tuple[float, float]]]:
    ...
    Args:
        circuit: The circuit containing operations that support the
            qubitron.unitary() or qubitron.kraus() protocols.
    ...
    qubit_frontier: dict[qubitron.Qid, int] = {q: 0 for q in qubits}
    kraus_frontier = 0
    positions: dict[tuple[str, str], tuple[float, float]] = {}
    ...
    for mi, moment in enumerate(circuit.moments):
        for op in moment.operations:
            ...
            if qubitron.has_unitary(op):
                U = qubitron.unitary(op).reshape((2,) * 2 * len(op.qubits)).astype(np.complex128)
                tensors.append(
                    qtn.Tensor(
                        data=U,
            ...
            elif qubitron.has_kraus(op):
                K = np.asarray(qubitron.kraus(op), dtype=np.complex128)
...
def tensor_density_matrix(
    circuit: qubitron.Circuit, qubits: list[qubitron.Qid] | None = None
) -> np.ndarray:
