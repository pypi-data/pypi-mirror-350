...
def _get_circuit(width, height, rs, p=2):
    ...
    qubits = [qubitron.GridQubit(*n) for n in graph]
    circuit = qubitron.Circuit(
        qubitron.H.on_each(qubits),
        [
            (ccq.get_grid_moments(graph), qubitron.Moment([qubitron.rx(0.456).on_each(qubits)]))
            for _ in range(p)
        ],
    )
    ...
    for width in [2, 3]:
        for height in [1, 3]:
            for p in [1, 2]:
                ...
                operator = qubitron.PauliString(
                    {q: qubitron.Z for q in rs.choice(qubits, size=2, replace=False)}
                )
...
def test_circuit_to_tensors(simplify):
    ...
    operator = qubitron.PauliString({q: qubitron.Z for q in rs.choice(qubits, size=2, replace=False)})
