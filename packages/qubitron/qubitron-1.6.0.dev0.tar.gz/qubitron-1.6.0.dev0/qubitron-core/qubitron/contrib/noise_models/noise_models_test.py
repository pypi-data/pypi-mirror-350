# Copyright 2019 The Qubitron Developers
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     https://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

from __future__ import annotations

import qubitron
import qubitron.contrib.noise_models as ccn
from qubitron import ops
from qubitron.testing import assert_equivalent_op_tree


def test_depol_noise() -> None:
    noise_model = ccn.DepolarizingNoiseModel(depol_prob=0.005)
    qubits = qubitron.LineQubit.range(2)
    moment = qubitron.Moment([qubitron.X(qubits[0]), qubitron.Y(qubits[1])])
    noisy_mom = noise_model.noisy_moment(moment, system_qubits=qubits)
    assert len(noisy_mom) == 2
    assert noisy_mom[0] == moment
    for g in noisy_mom[1]:
        assert isinstance(g.gate, qubitron.DepolarizingChannel)


def test_depol_noise_prepend() -> None:
    noise_model = ccn.DepolarizingNoiseModel(depol_prob=0.005, prepend=True)
    qubits = qubitron.LineQubit.range(2)
    moment = qubitron.Moment([qubitron.X(qubits[0]), qubitron.Y(qubits[1])])
    noisy_mom = noise_model.noisy_moment(moment, system_qubits=qubits)
    assert len(noisy_mom) == 2
    assert noisy_mom[1] == moment
    for g in noisy_mom[0]:
        assert isinstance(g.gate, qubitron.DepolarizingChannel)


# Composes depolarization noise with readout noise.
def test_readout_noise_after_moment() -> None:
    program = qubitron.Circuit()
    qubits = qubitron.LineQubit.range(3)
    program.append(
        [qubitron.H(qubits[0]), qubitron.CNOT(qubits[0], qubits[1]), qubitron.CNOT(qubits[1], qubits[2])]
    )
    program.append(
        [
            qubitron.measure(qubits[0], key='q0'),
            qubitron.measure(qubits[1], key='q1'),
            qubitron.measure(qubits[2], key='q2'),
        ],
        strategy=qubitron.InsertStrategy.NEW_THEN_INLINE,
    )

    # Use noise model to generate circuit
    depol_noise = ccn.DepolarizingNoiseModel(depol_prob=0.01)
    readout_noise = ccn.ReadoutNoiseModel(bitflip_prob=0.05)
    noisy_circuit = qubitron.Circuit(depol_noise.noisy_moments(program, qubits))
    noisy_circuit = qubitron.Circuit(readout_noise.noisy_moments(noisy_circuit, qubits))

    # Insert channels explicitly
    true_noisy_program = qubitron.Circuit()
    true_noisy_program.append([qubitron.H(qubits[0])])
    true_noisy_program.append(
        [qubitron.DepolarizingChannel(0.01).on(q).with_tags(ops.VirtualTag()) for q in qubits],
        strategy=qubitron.InsertStrategy.NEW_THEN_INLINE,
    )
    true_noisy_program.append([qubitron.CNOT(qubits[0], qubits[1])])
    true_noisy_program.append(
        [qubitron.DepolarizingChannel(0.01).on(q).with_tags(ops.VirtualTag()) for q in qubits],
        strategy=qubitron.InsertStrategy.NEW_THEN_INLINE,
    )
    true_noisy_program.append([qubitron.CNOT(qubits[1], qubits[2])])
    true_noisy_program.append(
        [qubitron.DepolarizingChannel(0.01).on(q).with_tags(ops.VirtualTag()) for q in qubits],
        strategy=qubitron.InsertStrategy.NEW_THEN_INLINE,
    )
    true_noisy_program.append(
        [qubitron.BitFlipChannel(0.05).on(q).with_tags(ops.VirtualTag()) for q in qubits]
    )
    true_noisy_program.append(
        [
            qubitron.measure(qubits[0], key='q0'),
            qubitron.measure(qubits[1], key='q1'),
            qubitron.measure(qubits[2], key='q2'),
        ]
    )
    assert_equivalent_op_tree(true_noisy_program, noisy_circuit)


def test_readout_noise_no_prepend() -> None:
    noise_model = ccn.ReadoutNoiseModel(bitflip_prob=0.005, prepend=False)
    qubits = qubitron.LineQubit.range(2)
    moment = qubitron.Moment([qubitron.measure(*qubits, key="meas")])
    noisy_mom = noise_model.noisy_moment(moment, system_qubits=qubits)
    assert len(noisy_mom) == 2
    assert noisy_mom[0] == moment
    for g in noisy_mom[1]:
        assert isinstance(g.gate, qubitron.BitFlipChannel)


# Composes depolarization, damping, and readout noise (in that order).
def test_decay_noise_after_moment() -> None:
    program = qubitron.Circuit()
    qubits = qubitron.LineQubit.range(3)
    program.append(
        [qubitron.H(qubits[0]), qubitron.CNOT(qubits[0], qubits[1]), qubitron.CNOT(qubits[1], qubits[2])]
    )
    program.append(
        [
            qubitron.measure(qubits[0], key='q0'),
            qubitron.measure(qubits[1], key='q1'),
            qubitron.measure(qubits[2], key='q2'),
        ],
        strategy=qubitron.InsertStrategy.NEW_THEN_INLINE,
    )

    # Use noise model to generate circuit
    depol_noise = ccn.DepolarizingNoiseModel(depol_prob=0.01)
    readout_noise = ccn.ReadoutNoiseModel(bitflip_prob=0.05)
    damping_noise = ccn.DampedReadoutNoiseModel(decay_prob=0.02)
    noisy_circuit = qubitron.Circuit(depol_noise.noisy_moments(program, qubits))
    noisy_circuit = qubitron.Circuit(damping_noise.noisy_moments(noisy_circuit, qubits))
    noisy_circuit = qubitron.Circuit(readout_noise.noisy_moments(noisy_circuit, qubits))

    # Insert channels explicitly
    true_noisy_program = qubitron.Circuit()
    true_noisy_program.append([qubitron.H(qubits[0])])
    true_noisy_program.append(
        [qubitron.DepolarizingChannel(0.01).on(q).with_tags(ops.VirtualTag()) for q in qubits],
        strategy=qubitron.InsertStrategy.NEW_THEN_INLINE,
    )
    true_noisy_program.append([qubitron.CNOT(qubits[0], qubits[1])])
    true_noisy_program.append(
        [qubitron.DepolarizingChannel(0.01).on(q).with_tags(ops.VirtualTag()) for q in qubits],
        strategy=qubitron.InsertStrategy.NEW_THEN_INLINE,
    )
    true_noisy_program.append([qubitron.CNOT(qubits[1], qubits[2])])
    true_noisy_program.append(
        [qubitron.DepolarizingChannel(0.01).on(q).with_tags(ops.VirtualTag()) for q in qubits],
        strategy=qubitron.InsertStrategy.NEW_THEN_INLINE,
    )
    true_noisy_program.append(
        [qubitron.AmplitudeDampingChannel(0.02).on(q).with_tags(ops.VirtualTag()) for q in qubits]
    )
    true_noisy_program.append(
        [qubitron.BitFlipChannel(0.05).on(q).with_tags(ops.VirtualTag()) for q in qubits]
    )
    true_noisy_program.append(
        [
            qubitron.measure(qubits[0], key='q0'),
            qubitron.measure(qubits[1], key='q1'),
            qubitron.measure(qubits[2], key='q2'),
        ]
    )
    assert_equivalent_op_tree(true_noisy_program, noisy_circuit)


def test_damped_readout_noise_no_prepend() -> None:
    noise_model = ccn.DampedReadoutNoiseModel(decay_prob=0.005, prepend=False)
    qubits = qubitron.LineQubit.range(2)
    moment = qubitron.Moment([qubitron.measure(*qubits, key="meas")])
    noisy_mom = noise_model.noisy_moment(moment, system_qubits=qubits)
    assert len(noisy_mom) == 2
    assert noisy_mom[0] == moment
    for g in noisy_mom[1]:
        assert isinstance(g.gate, qubitron.AmplitudeDampingChannel)


# Test the aggregate noise models.
def test_aggregate_readout_noise_after_moment() -> None:
    program = qubitron.Circuit()
    qubits = qubitron.LineQubit.range(3)
    program.append(
        [qubitron.H(qubits[0]), qubitron.CNOT(qubits[0], qubits[1]), qubitron.CNOT(qubits[1], qubits[2])]
    )
    program.append(
        [
            qubitron.measure(qubits[0], key='q0'),
            qubitron.measure(qubits[1], key='q1'),
            qubitron.measure(qubits[2], key='q2'),
        ],
        strategy=qubitron.InsertStrategy.NEW_THEN_INLINE,
    )

    # Use noise model to generate circuit
    noise_model = ccn.DepolarizingWithReadoutNoiseModel(depol_prob=0.01, bitflip_prob=0.05)
    noisy_circuit = qubitron.Circuit(noise_model.noisy_moments(program, qubits))

    # Insert channels explicitly
    true_noisy_program = qubitron.Circuit()
    true_noisy_program.append([qubitron.H(qubits[0])])
    true_noisy_program.append(
        [qubitron.DepolarizingChannel(0.01).on(q) for q in qubits],
        strategy=qubitron.InsertStrategy.NEW_THEN_INLINE,
    )
    true_noisy_program.append([qubitron.CNOT(qubits[0], qubits[1])])
    true_noisy_program.append(
        [qubitron.DepolarizingChannel(0.01).on(q) for q in qubits],
        strategy=qubitron.InsertStrategy.NEW_THEN_INLINE,
    )
    true_noisy_program.append([qubitron.CNOT(qubits[1], qubits[2])])
    true_noisy_program.append(
        [qubitron.DepolarizingChannel(0.01).on(q) for q in qubits],
        strategy=qubitron.InsertStrategy.NEW_THEN_INLINE,
    )
    true_noisy_program.append([qubitron.BitFlipChannel(0.05).on(q) for q in qubits])
    true_noisy_program.append(
        [
            qubitron.measure(qubits[0], key='q0'),
            qubitron.measure(qubits[1], key='q1'),
            qubitron.measure(qubits[2], key='q2'),
        ]
    )
    assert_equivalent_op_tree(true_noisy_program, noisy_circuit)


def test_aggregate_decay_noise_after_moment() -> None:
    program = qubitron.Circuit()
    qubits = qubitron.LineQubit.range(3)
    program.append(
        [qubitron.H(qubits[0]), qubitron.CNOT(qubits[0], qubits[1]), qubitron.CNOT(qubits[1], qubits[2])]
    )
    program.append(
        [
            qubitron.measure(qubits[0], key='q0'),
            qubitron.measure(qubits[1], key='q1'),
            qubitron.measure(qubits[2], key='q2'),
        ],
        strategy=qubitron.InsertStrategy.NEW_THEN_INLINE,
    )

    # Use noise model to generate circuit
    noise_model = ccn.DepolarizingWithDampedReadoutNoiseModel(
        depol_prob=0.01, decay_prob=0.02, bitflip_prob=0.05
    )
    noisy_circuit = qubitron.Circuit(noise_model.noisy_moments(program, qubits))

    # Insert channels explicitly
    true_noisy_program = qubitron.Circuit()
    true_noisy_program.append([qubitron.H(qubits[0])])
    true_noisy_program.append(
        [qubitron.DepolarizingChannel(0.01).on(q) for q in qubits],
        strategy=qubitron.InsertStrategy.NEW_THEN_INLINE,
    )
    true_noisy_program.append([qubitron.CNOT(qubits[0], qubits[1])])
    true_noisy_program.append(
        [qubitron.DepolarizingChannel(0.01).on(q) for q in qubits],
        strategy=qubitron.InsertStrategy.NEW_THEN_INLINE,
    )
    true_noisy_program.append([qubitron.CNOT(qubits[1], qubits[2])])
    true_noisy_program.append(
        [qubitron.DepolarizingChannel(0.01).on(q) for q in qubits],
        strategy=qubitron.InsertStrategy.NEW_THEN_INLINE,
    )
    true_noisy_program.append([qubitron.AmplitudeDampingChannel(0.02).on(q) for q in qubits])
    true_noisy_program.append([qubitron.BitFlipChannel(0.05).on(q) for q in qubits])
    true_noisy_program.append(
        [
            qubitron.measure(qubits[0], key='q0'),
            qubitron.measure(qubits[1], key='q1'),
            qubitron.measure(qubits[2], key='q2'),
        ]
    )
    assert_equivalent_op_tree(true_noisy_program, noisy_circuit)
