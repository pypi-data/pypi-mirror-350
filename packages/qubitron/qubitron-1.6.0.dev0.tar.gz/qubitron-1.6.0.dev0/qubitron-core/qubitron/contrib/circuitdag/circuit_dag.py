...
class Unique(Generic[T]):
    ...
    of the circuit are wrapped by `qubitron.Unique(X(q0))` so they are distinct
    nodes in the graph.
    """
    ...
    def __repr__(self) -> str:
        return f'qubitron.contrib.Unique({id(self)}, {self.val!r})'
...
def _disjoint_qubits(op1: qubitron.Operation, op2: qubitron.Operation) -> bool:
...
class CircuitDag(networkx.DiGraph):
    ...
    def __init__(
        self,
        can_reorder: Callable[[qubitron.Operation, qubitron.Operation], bool] = _disjoint_qubits,
    ...
    @staticmethod
    def make_node(op: qubitron.Operation) -> Unique:
        return Unique(op)
    ...
    def append(self, op: qubitron.Operation) -> None:
    ...
    def ordered_nodes(self) -> Iterator[Unique[qubitron.Operation]]:
        if not self.nodes():
            return
        g = self.copy()

        def get_root_node(some_node: Unique[qubitron.Operation]) -> Unique[qubitron.Operation]:
            pred = g.pred
            while pred[some_node]:
                some_node = next(iter(pred[some_node]))
            return some_node

        def get_first_node() -> Unique[qubitron.Operation]:
            return get_root_node(next(iter(g.nodes())))

        def get_next_node(succ: networkx.classes.coreviews.AtlasView) -> Unique[qubitron.Operation]:
            if succ:
    ...
    def to_circuit(self) -> qubitron.Circuit:
        return qubitron.Circuit(self.all_operations(), strategy=qubitron.InsertStrategy.EARLIEST)

    def findall_nodes_until_blocked(
        self, is_blocker: Callable[[qubitron.Operation], bool]
    ) -> Iterator[Unique[qubitron.Operation]]:
