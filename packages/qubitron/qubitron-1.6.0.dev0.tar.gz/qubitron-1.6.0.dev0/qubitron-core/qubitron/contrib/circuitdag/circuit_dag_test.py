# Copyright 2018 The Qubitron Developers
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     https://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

from __future__ import annotations

import itertools
import random

import networkx
import pytest

import qubitron


class FakeDevice(qubitron.Device):
    pass


def test_wrapper_eq():
    q0, q1 = qubitron.LineQubit.range(2)
    eq = qubitron.testing.EqualsTester()
    eq.add_equality_group(qubitron.contrib.CircuitDag.make_node(qubitron.X(q0)))
    eq.add_equality_group(qubitron.contrib.CircuitDag.make_node(qubitron.X(q0)))
    eq.add_equality_group(qubitron.contrib.CircuitDag.make_node(qubitron.Y(q0)))
    eq.add_equality_group(qubitron.contrib.CircuitDag.make_node(qubitron.X(q1)))


def test_wrapper_cmp():
    u0 = qubitron.contrib.Unique(0)
    u1 = qubitron.contrib.Unique(1)
    # The ordering of Unique instances is unpredictable
    u0, u1 = (u1, u0) if u1 < u0 else (u0, u1)
    assert u0 == u0
    assert u0 != u1
    assert u0 < u1
    assert u1 > u0
    assert u0 <= u0
    assert u0 <= u1
    assert u0 >= u0
    assert u1 >= u0


def test_wrapper_cmp_failure():
    with pytest.raises(TypeError):
        _ = object() < qubitron.contrib.Unique(1)
    with pytest.raises(TypeError):
        _ = qubitron.contrib.Unique(1) < object()


def test_wrapper_repr():
    q0 = qubitron.LineQubit(0)

    node = qubitron.contrib.CircuitDag.make_node(qubitron.X(q0))
    expected = f'qubitron.contrib.Unique({id(node)}, qubitron.X(qubitron.LineQubit(0)))'
    assert repr(node) == expected


def test_init():
    dag = qubitron.contrib.CircuitDag()
    assert networkx.dag.is_directed_acyclic_graph(dag)
    assert list(dag.nodes()) == []
    assert list(dag.edges()) == []


def test_append():
    q0 = qubitron.LineQubit(0)
    dag = qubitron.contrib.CircuitDag()
    dag.append(qubitron.X(q0))
    dag.append(qubitron.Y(q0))
    assert networkx.dag.is_directed_acyclic_graph(dag)
    assert len(dag.nodes()) == 2
    assert [(n1.val, n2.val) for n1, n2 in dag.edges()] == [(qubitron.X(q0), qubitron.Y(q0))]


def test_two_identical_ops():
    q0 = qubitron.LineQubit(0)
    dag = qubitron.contrib.CircuitDag()
    dag.append(qubitron.X(q0))
    dag.append(qubitron.Y(q0))
    dag.append(qubitron.X(q0))
    assert networkx.dag.is_directed_acyclic_graph(dag)
    assert len(dag.nodes()) == 3
    assert set((n1.val, n2.val) for n1, n2 in dag.edges()) == {
        (qubitron.X(q0), qubitron.Y(q0)),
        (qubitron.X(q0), qubitron.X(q0)),
        (qubitron.Y(q0), qubitron.X(q0)),
    }


def test_from_ops():
    q0 = qubitron.LineQubit(0)
    dag = qubitron.contrib.CircuitDag.from_ops(qubitron.X(q0), qubitron.Y(q0))
    assert networkx.dag.is_directed_acyclic_graph(dag)
    assert len(dag.nodes()) == 2
    assert [(n1.val, n2.val) for n1, n2 in dag.edges()] == [(qubitron.X(q0), qubitron.Y(q0))]


def test_from_circuit():
    q0 = qubitron.LineQubit(0)
    circuit = qubitron.Circuit(qubitron.X(q0), qubitron.Y(q0))
    dag = qubitron.contrib.CircuitDag.from_circuit(circuit)
    assert networkx.dag.is_directed_acyclic_graph(dag)
    assert len(dag.nodes()) == 2
    assert [(n1.val, n2.val) for n1, n2 in dag.edges()] == [(qubitron.X(q0), qubitron.Y(q0))]
    assert sorted(circuit.all_qubits()) == sorted(dag.all_qubits())


def test_to_empty_circuit():
    circuit = qubitron.Circuit()
    dag = qubitron.contrib.CircuitDag.from_circuit(circuit)
    assert networkx.dag.is_directed_acyclic_graph(dag)
    assert circuit == dag.to_circuit()


def test_to_circuit():
    q0 = qubitron.LineQubit(0)
    circuit = qubitron.Circuit(qubitron.X(q0), qubitron.Y(q0))
    dag = qubitron.contrib.CircuitDag.from_circuit(circuit)

    assert networkx.dag.is_directed_acyclic_graph(dag)
    # Only one possible output circuit for this simple case
    assert circuit == dag.to_circuit()

    qubitron.testing.assert_allclose_up_to_global_phase(
        circuit.unitary(), dag.to_circuit().unitary(), atol=1e-7
    )


def test_equality():
    q0, q1 = qubitron.LineQubit.range(2)
    circuit1 = qubitron.Circuit(
        qubitron.X(q0), qubitron.Y(q0), qubitron.Z(q1), qubitron.CZ(q0, q1), qubitron.X(q1), qubitron.Y(q1), qubitron.Z(q0)
    )
    circuit2 = qubitron.Circuit(
        qubitron.Z(q1), qubitron.X(q0), qubitron.Y(q0), qubitron.CZ(q0, q1), qubitron.Z(q0), qubitron.X(q1), qubitron.Y(q1)
    )
    circuit3 = qubitron.Circuit(
        qubitron.X(q0),
        qubitron.Y(q0),
        qubitron.Z(q1),
        qubitron.CZ(q0, q1),
        qubitron.X(q1),
        qubitron.Y(q1),
        qubitron.Z(q0) ** 0.5,
    )
    circuit4 = qubitron.Circuit(
        qubitron.X(q0), qubitron.Y(q0), qubitron.Z(q1), qubitron.CZ(q0, q1), qubitron.X(q1), qubitron.Y(q1)
    )

    eq = qubitron.testing.EqualsTester()
    eq.make_equality_group(
        lambda: qubitron.contrib.CircuitDag.from_circuit(circuit1),
        lambda: qubitron.contrib.CircuitDag.from_circuit(circuit2),
    )
    eq.add_equality_group(qubitron.contrib.CircuitDag.from_circuit(circuit3))
    eq.add_equality_group(qubitron.contrib.CircuitDag.from_circuit(circuit4))


def test_larger_circuit():
    q0, q1, q2, q3 = [
        qubitron.GridQubit(0, 5),
        qubitron.GridQubit(1, 5),
        qubitron.GridQubit(2, 5),
        qubitron.GridQubit(3, 5),
    ]
    # This circuit does not have CZ gates on adjacent qubits because the order
    # dag.to_circuit() would append them is non-deterministic.
    circuit = qubitron.Circuit(
        qubitron.X(q0),
        qubitron.CZ(q1, q2),
        qubitron.CZ(q0, q1),
        qubitron.Y(q0),
        qubitron.Z(q0),
        qubitron.CZ(q1, q2),
        qubitron.X(q0),
        qubitron.Y(q0),
        qubitron.CZ(q0, q1),
        qubitron.T(q3),
        strategy=qubitron.InsertStrategy.EARLIEST,
    )

    dag = qubitron.contrib.CircuitDag.from_circuit(circuit)

    assert networkx.dag.is_directed_acyclic_graph(dag)
    # Operation order within a moment is non-deterministic
    # but text diagrams still look the same.
    desired = """
(0, 5): ───X───@───Y───Z───X───Y───@───
               │                   │
(1, 5): ───@───@───@───────────────@───
           │       │
(2, 5): ───@───────@───────────────────

(3, 5): ───T───────────────────────────
"""
    qubitron.testing.assert_has_diagram(circuit, desired)
    qubitron.testing.assert_has_diagram(dag.to_circuit(), desired)

    qubitron.testing.assert_allclose_up_to_global_phase(
        circuit.unitary(), dag.to_circuit().unitary(), atol=1e-7
    )


@pytest.mark.parametrize('circuit', [qubitron.testing.random_circuit(10, 10, 0.5) for _ in range(3)])
def test_is_maximalist(circuit):
    dag = qubitron.contrib.CircuitDag.from_circuit(circuit)
    transitive_closure = networkx.dag.transitive_closure(dag)
    assert qubitron.contrib.CircuitDag(incoming_graph_data=transitive_closure) == dag
    assert not any(dag.has_edge(b, a) for a, b in itertools.combinations(dag.ordered_nodes(), 2))


def _get_circuits_and_is_blockers():
    qubits = qubitron.LineQubit.range(10)
    circuits = [qubitron.testing.random_circuit(qubits, 10, 0.5) for _ in range(1)]
    edges = [
        set(qubit_pair) for qubit_pair in itertools.combinations(qubits, 2) if random.random() > 0.5
    ]
    not_on_edge = lambda op: len(op.qubits) > 1 and set(op.qubits) not in edges
    is_blockers = [lambda op: False, not_on_edge]
    return itertools.product(circuits, is_blockers)


@pytest.mark.parametrize('circuit, is_blocker', _get_circuits_and_is_blockers())
def test_findall_nodes_until_blocked(circuit, is_blocker):
    dag = qubitron.contrib.CircuitDag.from_circuit(circuit)
    all_nodes = list(dag.ordered_nodes())
    found_nodes = list(dag.findall_nodes_until_blocked(is_blocker))
    assert not any(dag.has_edge(b, a) for a, b in itertools.combinations(found_nodes, 2))

    blocking_nodes = set(node for node in all_nodes if is_blocker(node.val))
    blocked_nodes = blocking_nodes.union(*(dag.succ[node] for node in blocking_nodes))
    expected_nodes = set(all_nodes) - blocked_nodes
    assert sorted(found_nodes) == sorted(expected_nodes)
