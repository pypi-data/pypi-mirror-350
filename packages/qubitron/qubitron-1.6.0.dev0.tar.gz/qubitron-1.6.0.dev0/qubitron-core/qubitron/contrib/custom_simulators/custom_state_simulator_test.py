...
import qubitron
from qubitron.contrib.custom_simulators.custom_state_simulator import CustomStateSimulator


class ComputationalBasisState(qubitron.qis.QuantumStateRepresentation):
    ...
    def measure(self, axes: Sequence[int], seed: qubitron.RANDOM_STATE_OR_SEED_LIKE = None):
        return [self.basis[i] for i in axes]


class ComputationalBasisSimState(qubitron.SimulationState[ComputationalBasisState]):
    def __init__(self, initial_state, qubits, classical_data):
        state = ComputationalBasisState(
            qubitron.big_endian_int_to_bits(initial_state, bit_count=len(qubits))
        )
        super().__init__(state=state, qubits=qubits, classical_data=classical_data)

    def _act_on_fallback_(self, action, qubits: Sequence[qubitron.Qid], allow_decompose: bool = True):
        gate = action.gate if isinstance(action, qubitron.Operation) else action
        if isinstance(gate, qubitron.XPowGate):
...
def create_test_circuit():
    ...
    return qubitron.Circuit(
        ...
        qubitron.CircuitOperation(
            qubitron.FrozenCircuit(x(q1), qubitron.measure(q1, key='b')),
            repeat_until=qubitron.SympyCondition(sympy.Eq(sympy.Symbol('b'), 2)),
...
def test_built_in_states():
    ...
    sim = CustomStateSimulator(qubitron.StateVectorSimulationState)
    circuit = create_test_circuit()
    r = sim.simulate(circuit)
    assert r.measurements == {'a': np.array([1]), 'b': np.array([2])}
    assert np.allclose(
...
def test_product_state_mode_built_in_state():
    sim = CustomStateSimulator(qubitron.StateVectorSimulationState, split_untangled_states=True)
    circuit = create_test_circuit()
    r = sim.simulate(circuit)
    assert r.measurements == {'a': np.array([1]), 'b': np.array([2])}

    # Ensure the state is in product-state mode, and it's got three states (q0, q1, phase)
    assert isinstance(r._final_simulator_state, qubitron.SimulationProductState)
...
def test_parameterized_repetitions():
    q = qubitron.LineQid(0, dimension=5)
    x = qubitron.XPowGate(dimension=5)
    circuit = qubitron.Circuit(
        qubitron.CircuitOperation(
            qubitron.FrozenCircuit(x(q), qubitron.measure(q, key='a')),
...
class ComputationalBasisProductState(qubitron.qis.QuantumStateRepresentation):
    def __init__(self, initial_state: list[int]):
        self.basis = initial_state
...
class ComputationalBasisSimProductState(qubitron.SimulationState[ComputationalBasisProductState]):
    def __init__(self, initial_state, qubits, classical_data):
