...
class EmptyQuantumState(qubitron.QuantumStateRepresentation):
...
class EmptySimulationState(qubitron.SimulationState):
...
q0, q1, q2 = qs3 = qubitron.LineQubit.range(3)
qs2 = qubitron.LineQubit.range(2)


def create_container(
    qubits: Sequence[qubitron.Qid], split_untangled_states=True
) -> qubitron.SimulationProductState[EmptySimulationState]:
    state_map: dict[qubitron.Qid | None, EmptySimulationState] = {}
    log = qubitron.ClassicalDataDictionaryStore()
    ...
    return qubitron.SimulationProductState(
...
def test_subcircuit_entanglement_causes_join() -> None:
    state = create_container(qs2)
    assert len(set(state.values())) == 3
    state.apply_operation(qubitron.CircuitOperation(qubitron.FrozenCircuit(qubitron.CNOT(q0, q1))))
    assert len(set(state.values())) == 2
    assert state[q0] is state[q1]
...
def test_subcircuit_entanglement_causes_join_in_subset() -> None:
    ...
    assert len(set(state.values())) == 3
    assert state[q0] is state[q1]
    state.apply_operation(qubitron.CircuitOperation(qubitron.FrozenCircuit(qubitron.CNOT(q0, q2))))
    assert len(set(state.values())) == 2
...
def test_identity_does_not_join() -> None:
    state = create_container(qs2)
    assert len(set(state.values())) == 3
    state.apply_operation(qubitron.IdentityGate(2)(q0, q1))
    assert len(set(state.values())) == 3
...
def test_identity_fallback_does_not_join() -> None:
    state = create_container(qs2)
    assert len(set(state.values())) == 3
    state._act_on_fallback_(qubitron.I, (q0, q1))
    assert len(set(state.values())) == 3
    assert state[q0] is not state[q1]
    assert state[q0] is not state[None]
...
def test_subcircuit_identity_does_not_join() -> None:
    state = create_container(qs2)
    assert len(set(state.values())) == 3
    state.apply_operation(qubitron.CircuitOperation(qubitron.FrozenCircuit(qubitron.IdentityGate(2)(q0, q1))))
    assert len(set(state.values())) == 3
    assert state[q0] is not state[q1]
...
def test_subcircuit_measurement_causes_split() -> None:
    ...
    state.apply_operation(qubitron.CNOT(q0, q1))
    assert len(set(state.values())) == 2
    state.apply_operation(qubitron.CircuitOperation(qubitron.FrozenCircuit(qubitron.measure(q0))))
    assert len(set(state.values())) == 3
    assert state[q0] is not state[q1]
...
def test_subcircuit_measurement_causes_split_in_subset() -> None:
    state = create_container(qs3)
    state.apply_operation(qubitron.CNOT(q0, q1))
    state.apply_operation(qubitron.CNOT(q0, q2))
    assert len(set(state.values())) == 2
    state.apply_operation(qubitron.CircuitOperation(qubitron.FrozenCircuit(qubitron.measure(q0))))
    assert len(set(state.values())) == 3
    assert state[q0] is not state[q1]
    state.apply_operation(qubitron.CircuitOperation(qubitron.FrozenCircuit(qubitron.measure(q1))))
    assert len(set(state.values())) == 4
    assert state[q0] is not state[q1]
...
def test_reset_causes_split() -> None:
    ...
    state.apply_operation(qubitron.CNOT(q0, q1))
    assert len(set(state.values())) == 2
    state.apply_operation(qubitron.reset(q0))
    assert len(set(state.values())) == 3
    assert state[q0] is not state[q1]
    assert state[q0] is not state[None]
...
def test_measurement_of_all_qubits_causes_split() -> None:
    ...
    state.apply_operation(qubitron.CNOT(q0, q1))
    assert len(set(state.values())) == 2
    state.apply_operation(qubitron.measure(q0, q1))
    assert len(set(state.values())) == 3
    assert state[q0] is not state[q1]
    assert state[q0] is not state[None]
...
def test_swap_does_not_merge() -> None:
    ...
    old_q0 = state[q0]
    old_q1 = state[q1]
    state.apply_operation(qubitron.SWAP(q0, q1))
    assert len(set(state.values())) == 3
...
def test_half_swap_does_merge() -> None:
    state = create_container(qs2)
    state.apply_operation(qubitron.SWAP(q0, q1) ** 0.5)
    assert len(set(state.values())) == 2
    assert state[q0] is state[q1]
...
def test_swap_after_entangle_reorders() -> None:
    state = create_container(qs2)
    state.apply_operation(qubitron.CX(q0, q1))
    assert len(set(state.values())) == 2
...
def test_act_on_gate_does_not_join() -> None:
    state = create_container(qs2)
    assert len(set(state.values())) == 3
    qubitron.act_on(qubitron.X, state, [q0])
    assert len(set(state.values())) == 3
    assert state[q0] is not state[q1]
    assert state[q0] is not state[None]
