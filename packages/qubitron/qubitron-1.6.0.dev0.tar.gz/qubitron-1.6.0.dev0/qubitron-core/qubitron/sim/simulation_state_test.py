...
import qubitron
from qubitron.sim import simulation_state
from qubitron.testing import PhaseUsingCleanAncilla, PhaseUsingDirtyAncilla


class ExampleQuantumState(qubitron.QuantumStateRepresentation):
...
class ExampleSimulationState(qubitron.SimulationState):
    def __init__(self, qubits=qubitron.LineQubit.range(2)):
        super().__init__(state=ExampleQuantumState(), qubits=qubits)

    def _act_on_fallback_(
        self, action: Any, qubits: Sequence[qubitron.Qid], allow_decompose: bool = True
    ) -> bool:
        return True
...
class DelegatingAncillaZ(qubitron.Gate):
...
def test_measurements() -> None:
    ...
    args.measure([qubitron.LineQubit(0)], "test", [False], {})
...
def test_decompose() -> None:
    ...
    assert simulation_state.strat_act_on_from_apply_decompose(
        Composite(), args, [qubitron.LineQubit(0)]
    )


def test_decompose_for_gate_allocating_qubits_raises() -> None:
    class Composite(qubitron.testing.SingleQubitGate):
        def _decompose_(self, qubits):
            anc = qubitron.NamedQubit("anc")
            yield qubitron.CNOT(*qubits, anc)

    args = ExampleSimulationState()

    with pytest.raises(TypeError, match="add_qubits but not remove_qubits"):
        simulation_state.strat_act_on_from_apply_decompose(Composite(), args, [qubitron.LineQubit(0)])


def test_mapping() -> None:
    args = ExampleSimulationState()
    assert list(iter(args)) == qubitron.LineQubit.range(2)
    r1 = args[qubitron.LineQubit(0)]
    assert args is r1
    with pytest.raises(IndexError):
        _ = args[qubitron.LineQubit(2)]


def test_swap_bad_dimensions() -> None:
    q0 = qubitron.LineQubit(0)
    q1 = qubitron.LineQid(1, 3)
    args = ExampleSimulationState()
...
def test_field_getters() -> None:
    ...
    assert args.qubit_map == {q: i for i, q in enumerate(qubitron.LineQubit.range(2))}
...
def test_delegating_gate_unitary(exp) -> None:
    ...
    test_circuit = qubitron.Circuit()
    test_circuit.append(qubitron.H(q))
    test_circuit.append(DelegatingAncillaZ(exp).on(q))

    control_circuit = qubitron.Circuit(qubitron.H(q))
    control_circuit.append(qubitron.ZPowGate(exponent=exp).on(q))
...
def test_phase_using_dirty_ancilla(num_ancilla: int) -> None:
    q = qubitron.LineQubit(0)
    anc = qubitron.NamedQubit.range(num_ancilla, prefix='anc')

    u = qubitron.MatrixGate(qubitron.testing.random_unitary(2 ** (num_ancilla + 1)))
    test_circuit = qubitron.Circuit(
        u.on(q, *anc), PhaseUsingDirtyAncilla(ancilla_bitsize=num_ancilla).on(q)
    )
    control_circuit = qubitron.Circuit(u.on(q, *anc), qubitron.Z(q))
    assert_test_circuit_for_dm_simulator(test_circuit, control_circuit)
    assert_test_circuit_for_sv_simulator(test_circuit, control_circuit)
...
def test_phase_using_clean_ancilla(num_ancilla: int, theta: float) -> None:
    q = qubitron.LineQubit(0)
    u = qubitron.MatrixGate(qubitron.testing.random_unitary(2))
    test_circuit = qubitron.Circuit(
        u.on(q), PhaseUsingCleanAncilla(theta=theta, ancilla_bitsize=num_ancilla).on(q)
    )
    control_circuit = qubitron.Circuit(u.on(q), qubitron.ZPowGate(exponent=theta).on(q))
    assert_test_circuit_for_dm_simulator(test_circuit, control_circuit)
    assert_test_circuit_for_sv_simulator(test_circuit, control_circuit)


def assert_test_circuit_for_dm_simulator(test_circuit, control_circuit) -> None:
    # Density Matrix Simulator: For unitary gates, this fallbacks to `qubitron.apply_channel`
    # which recursively calls to `qubitron.apply_unitary(decompose=True)`.
    for split_untangled_states in [True, False]:
        sim = qubitron.DensityMatrixSimulator(split_untangled_states=split_untangled_states)
...
def assert_test_circuit_for_sv_simulator(test_circuit, control_circuit) -> None:
    ...
    for split_untangled_states in [True, False]:
        sim = qubitron.Simulator(split_untangled_states=split_untangled_states)
