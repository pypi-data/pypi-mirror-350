...
def test_sample_density_matrix_big_endian() -> None:
    results = []
    for x in range(8):
        matrix = qubitron.to_valid_density_matrix(x, 3)
        sample = qubitron.sample_density_matrix(matrix, [2, 1, 0])
    ...
    for index in range(3):
        for x in range(8):
            ...
            np.testing.assert_equal(
                qubitron.sample_density_matrix(matrix, [index]), [[bool(1 & (x >> (2 - index)))]]
            )
...
def test_sample_density_matrix_partial_indices_oder() -> None:
    for x in range(8):
        ...
        np.testing.assert_equal(qubitron.sample_density_matrix(matrix, [2, 1]), expected)
...
def test_sample_density_matrix_partial_indices_all_orders() -> None:
    for perm in itertools.permutations([0, 1, 2]):
        for x in range(8):
            ...
            np.testing.assert_equal(qubitron.sample_density_matrix(matrix, perm), expected)
...
def test_sample_density_matrix() -> None:
    ...
    state[0] = 1 / np.sqrt(2)
    state[2] = 1 / np.sqrt(2)
    matrix = qubitron.to_valid_density_matrix(state, num_qubits=3)
    for _ in range(10):
        sample = qubitron.sample_density_matrix(matrix, [2, 1, 0])
    ...
    for _ in range(10):
        np.testing.assert_equal(qubitron.sample_density_matrix(matrix, [2]), [[False]])
        np.testing.assert_equal(qubitron.sample_density_matrix(matrix, [0]), [[False]])


def test_sample_density_matrix_seed() -> None:
    density_matrix = 0.5 * np.eye(2)

    samples = qubitron.sample_density_matrix(density_matrix, [0], repetitions=10, seed=1234)
    assert np.array_equal(
        samples,
        [[False], [True], [False], [True], [True], [False], [False], [True], [True], [True]],
    )

    samples = qubitron.sample_density_matrix(
...
def test_sample_empty_density_matrix() -> None:
    ...
    np.testing.assert_almost_equal(qubitron.sample_density_matrix(matrix, []), [[]])


def test_sample_density_matrix_no_repetitions() -> None:
    matrix = qubitron.to_valid_density_matrix(0, 3)
    np.testing.assert_almost_equal(
        qubitron.sample_density_matrix(matrix, [1], repetitions=0), np.zeros(shape=(0, 1))
    )
    np.testing.assert_almost_equal(
        qubitron.sample_density_matrix(matrix, [0, 1], repetitions=0), np.zeros(shape=(0, 2))
    )
...
def test_sample_density_matrix_repetitions() -> None:
    for perm in itertools.permutations([0, 1, 2]):
        for x in range(8):
            ...
            result = qubitron.sample_density_matrix(matrix, perm, repetitions=3)
            np.testing.assert_equal(result, expected)
...
def test_sample_density_matrix_negative_repetitions() -> None:
    ...
    with pytest.raises(ValueError, match='-1'):
        qubitron.sample_density_matrix(matrix, [1], repetitions=-1)


def test_sample_density_matrix_not_square() -> None:
    with pytest.raises(ValueError, match='not square'):
        qubitron.sample_density_matrix(np.array([1, 0, 0]), [1])


def test_sample_density_matrix_not_power_of_two() -> None:
    with pytest.raises(ValueError, match='power of two'):
        qubitron.sample_density_matrix(np.ones((3, 3)) / 3, [1])
    with pytest.raises(ValueError, match='power of two'):
        qubitron.sample_density_matrix(np.ones((2, 3, 2, 3)) / 6, [1])


def test_sample_density_matrix_higher_powers_of_two() -> None:
    with pytest.raises(ValueError, match='powers of two'):
        qubitron.sample_density_matrix(np.ones((2, 4, 2, 4)) / 8, [1])


def test_sample_density_matrix_out_of_range() -> None:
    matrix = qubitron.to_valid_density_matrix(0, 3)
    with pytest.raises(IndexError, match='-2'):
        qubitron.sample_density_matrix(matrix, [-2])
    with pytest.raises(IndexError, match='3'):
        qubitron.sample_density_matrix(matrix, [3])


def test_sample_density_matrix_no_indices() -> None:
    matrix = qubitron.to_valid_density_matrix(0, 3)
    bits = qubitron.sample_density_matrix(matrix, [])
    np.testing.assert_almost_equal(bits, np.zeros(shape=(1, 0)))


def test_sample_density_matrix_validate_qid_shape() -> None:
    matrix = qubitron.to_valid_density_matrix(0, 3)
    qubitron.sample_density_matrix(matrix, [], qid_shape=(2, 2, 2))
    with pytest.raises(ValueError, match='Matrix size does not match qid shape'):
        qubitron.sample_density_matrix(matrix, [], qid_shape=(2, 2, 1))
    matrix2 = qubitron.to_valid_density_matrix(0, qid_shape=(1, 2, 3))
    qubitron.sample_density_matrix(matrix2, [], qid_shape=(1, 2, 3))
    with pytest.raises(ValueError, match='Matrix size does not match qid shape'):
        qubitron.sample_density_matrix(matrix2, [], qid_shape=(2, 2, 2))
...
def test_measure_density_matrix_computational_basis() -> None:
    ...
    for x in range(8):
        matrix = qubitron.to_valid_density_matrix(x, 3)
        bits, out_matrix = qubitron.measure_density_matrix(matrix, [2, 1, 0])
        results.append(bits)
...
def test_measure_density_matrix_computational_basis_reversed() -> None:
    ...
    for x in range(8):
        matrix = qubitron.to_valid_density_matrix(x, 3)
        bits, out_matrix = qubitron.measure_density_matrix(matrix, [0, 1, 2])
        results.append(bits)
        np.testing.assert_almost_equal(out_matrix, matrix)
...
def test_measure_density_matrix_computational_basis_reshaped() -> None:
    results = []
    for x in range(8):
        matrix = np.reshape(qubitron.to_valid_density_matrix(x, 3), (2,) * 6)
        bits, out_matrix = qubitron.measure_density_matrix(matrix, [2, 1, 0])
        results.append(bits)
        np.testing.assert_almost_equal(out_matrix, matrix)
    expected = [list(reversed(x)) for x in list(itertools.product([False, True], repeat=3))]
...
def test_measure_density_matrix_partial_indices() -> None:
    for index in range(3):
        for x in range(8):
            matrix = qubitron.to_valid_density_matrix(x, 3)
            bits, out_matrix = qubitron.measure_density_matrix(matrix, [index])
            np.testing.assert_almost_equal(out_matrix, matrix)
...
def test_measure_density_matrix_partial_indices_all_orders() -> None:
    for perm in itertools.permutations([0, 1, 2]):
        for x in range(8):
            matrix = qubitron.to_valid_density_matrix(x, 3)
            bits, out_matrix = qubitron.measure_density_matrix(matrix, perm)
...
def matrix_000_plus_010():
    ...
    return qubitron.to_valid_density_matrix(state, num_qubits=3)
...
def test_measure_density_matrix_collapse() -> None:
    ...
    # Partial sample is correct.
    for _ in range(10):
        bits, out_matrix = qubitron.measure_density_matrix(matrix, [2])
        np.testing.assert_almost_equal(out_matrix, matrix)
        assert bits == [False]

        bits, out_matrix = qubitron.measure_density_matrix(matrix, [0])
        np.testing.assert_almost_equal(out_matrix, matrix)
        assert bits == [False]
...
def test_measure_density_matrix_seed() -> None:
    ...
    bits, out_matrix1 = qubitron.measure_density_matrix(matrix, range(n), seed=1234)
    assert bits == [False, False, True, True, False]

    bits, out_matrix2 = qubitron.measure_density_matrix(
...
def test_measure_density_matrix_out_is_matrix() -> None:
    matrix = matrix_000_plus_010()
    bits, out_matrix = qubitron.measure_density_matrix(matrix, [2, 1, 0], out=matrix)
...
def test_measure_state_out_is_not_matrix() -> None:
    ...
    _, out_matrix = qubitron.measure_density_matrix(matrix, [2, 1, 0], out=out)
...
def test_measure_density_matrix_not_square() -> None:
    with pytest.raises(ValueError, match='not square'):
        qubitron.measure_density_matrix(np.array([1, 0, 0]), [1])
    with pytest.raises(ValueError, match='not square'):
        qubitron.measure_density_matrix(
...
def test_measure_density_matrix_not_power_of_two() -> None:
    with pytest.raises(ValueError, match='power of two'):
        qubitron.measure_density_matrix(np.ones((3, 3)) / 3, [1])
    with pytest.raises(ValueError, match='power of two'):
        qubitron.measure_density_matrix(np.ones((2, 3, 2, 3)) / 6, [1])


def test_measure_density_matrix_higher_powers_of_two() -> None:
    with pytest.raises(ValueError, match='powers of two'):
        qubitron.measure_density_matrix(np.ones((2, 4, 2, 4)) / 8, [1])
...
def test_measure_density_matrix_out_of_range() -> None:
    matrix = qubitron.to_valid_density_matrix(0, 3)
    with pytest.raises(IndexError, match='-2'):
        qubitron.measure_density_matrix(matrix, [-2])
    with pytest.raises(IndexError, match='3'):
        qubitron.measure_density_matrix(matrix, [3])


def test_measure_state_no_indices() -> None:
    matrix = qubitron.to_valid_density_matrix(0, 3)
    bits, out_matrix = qubitron.measure_density_matrix(matrix, [])
    assert [] == bits
    np.testing.assert_almost_equal(out_matrix, matrix)
...
def test_measure_state_no_indices_out_is_matrix() -> None:
    matrix = qubitron.to_valid_density_matrix(0, 3)
    bits, out_matrix = qubitron.measure_density_matrix(matrix, [], out=matrix)
    assert [] == bits
    np.testing.assert_almost_equal(out_matrix, matrix)
    assert out_matrix is matrix
...
def test_measure_state_no_indices_out_is_not_matrix() -> None:
    matrix = qubitron.to_valid_density_matrix(0, 3)
    out = np.zeros_like(matrix)
    bits, out_matrix = qubitron.measure_density_matrix(matrix, [], out=out)
    assert [] == bits
    np.testing.assert_almost_equal(out_matrix, matrix)
    assert out is out_matrix
    assert out is not matrix
...
def test_to_valid_density_matrix_on_simulator_output(seed, dtype, split) -> None:
    circuit = qubitron.testing.random_circuit(qubits=5, n_moments=20, op_density=0.9, random_state=seed)
    simulator = qubitron.DensityMatrixSimulator(split_untangled_states=split, dtype=dtype)
    result = simulator.simulate(circuit)
    _ = qubitron.to_valid_density_matrix(result.final_density_matrix, num_qubits=5, atol=1e-6)


def test_factor_validation() -> None:
    args = qubitron.DensityMatrixSimulator()._create_simulation_state(0, qubits=qubitron.LineQubit.range(2))
    args.apply_operation(qubitron.H(qubitron.LineQubit(0)))
    t = args.create_merged_state().target_tensor
    qubitron.linalg.transformations.factor_density_matrix(t, [0])
    qubitron.linalg.transformations.factor_density_matrix(t, [1])
