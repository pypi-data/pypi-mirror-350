...
from qubitron.value import big_endian_int_to_digits, random_state
...
class StabilizerStateChForm(qis.StabilizerState):
    ...
    def copy(self, deep_copy_buffers: bool = True) -> qubitron.StabilizerStateChForm:
    ...
    def __str__(self) -> str:
        ...
        return qubitron.dirac_notation(self.to_state_vector())
    ...
    def inner_product_of_state_and_x(self, x: int) -> complex:
        ...
        if type(x) == int:
            y = qubitron.big_endian_int_to_bits(x, bit_count=self.n)
    ...
    def kron(self, other: qubitron.StabilizerStateChForm) -> qubitron.StabilizerStateChForm:
        n = self.n + other.n
        copy = StabilizerStateChForm(n)
    ...
    def reindex(self, axes: Sequence[int]) -> qubitron.StabilizerStateChForm:
        copy = StabilizerStateChForm(self.n)
