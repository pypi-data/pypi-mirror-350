...
from qubitron.protocols import act_on
from qubitron.sim import clifford, simulator_base


class CliffordSimulator(
    simulator_base.SimulatorBase[
        'qubitron.CliffordSimulatorStepResult',
    ...
    def __init__(
        self, seed: qubitron.RANDOM_STATE_OR_SEED_LIKE = None, split_untangled_states: bool = False
    ):
    ...
    @staticmethod
    def is_supported_operation(op: qubitron.Operation) -> bool:
    ...
    def _create_partial_simulation_state(
        self,
        initial_state: int | qubitron.StabilizerChFormSimulationState,
    ...
    def _create_step_result(
        self, sim_state: qubitron.SimulationStateBase[clifford.StabilizerChFormSimulationState]
    ):
    ...
    def _create_simulator_trial_result(
        ...
        final_simulator_state: qubitron.SimulationStateBase[qubitron.StabilizerChFormSimulationState],
    ):
...
class CliffordTrialResult(
    ...
    @property
    def final_state(self) -> qubitron.CliffordState:
    ...
    def _repr_pretty_(self, p: Any, cycle: bool):
        ...
        p.text("qubitron.CliffordTrialResult(...)" if cycle else self.__str__())


class CliffordSimulatorStepResult(
    simulator_base.StepResultBase['qubitron.StabilizerChFormSimulationState']
):
    ...
    def _repr_pretty_(self, p, cycle):
        """iPython (Jupyter) pretty print."""
        p.text("qubitron.CliffordSimulatorStateResult(...)" if cycle else self.__str__())
...
class CliffordState:
    ...
    def __init__(self, qubit_map, initial_state: int | qubitron.StabilizerStateChForm = 0):
    ...
    def copy(self) -> qubitron.CliffordState:
    ...
    def apply_unitary(self, op: qubitron.Operation):
    ...
    def apply_measurement(
        ...
        if not isinstance(op.gate, qubitron.MeasurementGate):
            raise TypeError(
                f'apply_measurement only supports qubitron.MeasurementGate operations. '
