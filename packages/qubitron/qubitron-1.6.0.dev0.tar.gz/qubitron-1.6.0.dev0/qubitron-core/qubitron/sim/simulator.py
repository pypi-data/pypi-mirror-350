...
Simulator types include:
    ...
    SimulatesIntermediateState: allows for access to the state of the simulation
        as the simulation iterates through the moments of a qubitron.
"""
...
from qubitron import circuits, ops, protocols, study, value, work
from qubitron.sim.simulation_state_base import SimulationStateBase
...
class SimulatesSamples(work.Sampler, metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def _run(
        self, circuit: qubitron.AbstractCircuit, param_resolver: qubitron.ParamResolver, repetitions: int
    ) -> dict[str, np.ndarray]:
...
class SimulatesIntermediateState(
    ...
    @abc.abstractmethod
    def _base_iterator(
        self, circuit: qubitron.AbstractCircuit, qubits: tuple[qubitron.Qid, ...], initial_state: Any
    ) -> Iterator[TStepResult]:
...
class StepResult(Generic[TSimulatorState], metaclass=abc.ABCMeta):
    ...
    def sample_measurement_ops(
        self,
        measurement_ops: list[qubitron.GateOperation],
        repetitions: int = 1,
        seed: qubitron.RANDOM_STATE_OR_SEED_LIKE = None,
        *,
        ...
        `qubitron.GateOperation` instances whose gates are `qubitron.MeasurementGate`
        ...
        measured_qubits = []
        seen_qubits: set[qubitron.Qid] = set()
        for op in measurement_ops:
            for q in op.qubits:
    ...
    def _confuse_results(
        ...
        Compare with _confuse_result in qubitron-core/qubitron/sim/simulation_state.py.
        """
        prng = value.parse_random_state(seed)
        for rep in bits:
...
class SimulationTrialResult(Generic[TSimulatorState]):
    ...
    Unlike `qubitron.Result`, a SimulationTrialResult contains the final
    ...
    @property
    def params(self) -> qubitron.ParamResolver:
        return self._params
    ...
    def __repr__(self) -> str:
        return (
            f'qubitron.SimulationTrialResult(params={self.params!r}, '
    ...
    @property
    def qubit_map(self) -> Mapping[qubitron.Qid, int]:
...
def _qubit_map_to_shape(qubit_map: Mapping[qubitron.Qid, int]) -> tuple[int, ...]:
...
def split_into_matching_protocol_then_general(
    circuit: qubitron.AbstractCircuit, predicate: Callable[[qubitron.Operation], bool]
) -> tuple[qubitron.AbstractCircuit, qubitron.AbstractCircuit]:
    ...
    blocked_keys: set[qubitron.MeasurementKey] = set()
