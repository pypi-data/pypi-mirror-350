...
TState = TypeVar('TState', bound='qubitron.QuantumStateRepresentation')
...
class SimulationState(SimulationStateBase, Generic[TState], metaclass=abc.ABCMeta):
    ...
    def __init__(
        ...
        *,
        ...
        qubits: Sequence[qubitron.Qid] | None = None,
        classical_data: qubitron.ClassicalDataStore | None = None,
    ):
    ...
    def get_axes(self, qubits: Sequence[qubitron.Qid]) -> list[int]:
        return [self.qubit_map[q] for q in qubits]

    def _perform_measurement(self, qubits: Sequence[qubitron.Qid]) -> list[int]:
    ...
    def _confuse_result(
        ...
        Compare with _confuse_results in qubitron-core/qubitron/sim/simulator.py.
        """
        confused = list(bits)
    ...
    def add_qubits(self: Self, qubits: Sequence[qubitron.Qid]) -> Self:
        ...
        Returns:
            ...
            Self: A `qubitron.SimulationState` with qubits added or `self` if there are no qubits to
                add.
        """
        if not qubits:
            return self
    ...
    def remove_qubits(self: Self, qubits: Sequence[qubitron.Qid]) -> Self:
        ...
        Returns:
            NotImplemented: If the subclass does not implement this method.
            Self: A `qubitron.SimulationState` with qubits removed or `self` if there are no qubits to
                remove.
        """
        if not qubits:
            return self
    ...
    def factor(
        self, qubits: Sequence[qubitron.Qid], *, validate=True, atol=1e-07, inplace=False
    ) -> tuple[Self, Self]:
    ...
    def transpose_to_qubit_order(self, qubits: Sequence[qubitron.Qid], *, inplace=False) -> Self:
    ...
    def swap(self, q1: qubitron.Qid, q2: qubitron.Qid, *, inplace=False):
        """Swaps two qubits.
    ...
    def rename(self, q1: qubitron.Qid, q2: qubitron.Qid, *, inplace=False):
        """Renames `q1` to `q2`.
    ...
    def __getitem__(self, item: qubitron.Qid | None) -> Self:
    ...
    def __iter__(self) -> Iterator[qubitron.Qid | None]:
...
def strat_act_on_from_apply_decompose(
    val: Any, args: qubitron.SimulationState, qubits: Sequence[qubitron.Qid]
) -> bool:
    ...
    if decomposed is None:
        return NotImplemented
    all_ancilla: set[qubitron.Qid] = set()
