...
from qubitron import circuits, ops, protocols
from qubitron.circuits.circuit import CIRCUIT_TYPE
...
def map_moments(
    ...
    *,
    ...
    Args:
        ...
        map_func: Mapping function from (qubitron.Moment, moment_index) to a sequence of moments.
...
def _map_operations_impl(
    ...
    *,
    ...
    is to
        ...
        2) Use the `get_earliest_accommodating_moment_index` infrastructure built for `qubitron.Circuit`
    ...
    Args:
        circuit: Input circuit to apply the transformations on. The input circuit is not mutated.
        map_func: Mapping function from (qubitron.Operation, moment_index) to a qubitron.OP_TREE. If the
            ...
            wrapping the operations in a circuit operation and then calling `qubitron.unroll_circuit_op`
    ...
    def apply_map_func(op: qubitron.Operation, idx: int) -> list[qubitron.Operation]:
        ...
        mapped_ops_qubits: set[qubitron.Qid] = set()
    ...
    new_moments: list[list[qubitron.Operation]] = []
    for idx, moment in enumerate(circuit):
        curr_moments: list[list[qubitron.Operation]] = [[]] if wrap_in_circuit_op else []
    ...
    *,
    ...
    Args:
        ...
        map_func: Mapping function from (qubitron.Operation, moment_index) to a qubitron.OP_TREE. If the
            ...
            `qubitron.CircuitOperation(qubitron.FrozenCircuit(op_tree)).with_tags(MAPPED_CIRCUIT_OP_TAG)`
            to preserve moment structure. Utility methods like `qubitron.unroll_circuit_op` can
...
def map_operations_and_unroll(
    ...
    *,
    ...
    """Applies local transformations via `qubitron.map_operations` & unrolls intermediate circuit ops.

    See `qubitron.map_operations` and `qubitron.unroll_circuit_op` for more details.

    Args:
        circuit: Input circuit to apply the transformations on. The input circuit is not mutated.
        map_func: Mapping function from (qubitron.Operation, moment_index) to a qubitron.OP_TREE.
        deep: If true, `map_func` will be recursively applied to circuits wrapped inside
            any circuit operations contained within `circuit`.
        raise_if_add_qubits: Set to True by default. If True, raises ValueError if
...
class _MergedCircuit:
    """An optimized internal representation of a circuit, tailored for `qubitron.merge_operations`
    ...
    qubit_indexes: dict[qubitron.Qid, list[int]] = dataclasses.field(
        default_factory=lambda: defaultdict(lambda: [-1])
    )
    mkey_indexes: dict[qubitron.MeasurementKey, list[int]] = dataclasses.field(
        default_factory=lambda: defaultdict(lambda: [-1])
    )
    ckey_indexes: dict[qubitron.MeasurementKey, list[int]] = dataclasses.field(
        default_factory=lambda: defaultdict(lambda: [-1])
    )
    ops_by_index: list[dict[qubitron.Operation, int]] = dataclasses.field(default_factory=list)

    def append_empty_moment(self) -> None:
        self.ops_by_index.append({})

    def add_op_to_moment(self, moment_index: int, op: qubitron.Operation) -> None:
    ...
    def remove_op_from_moment(self, moment_index: int, op: qubitron.Operation) -> None:
    ...
    def get_mergeable_ops(
        self, op: qubitron.Operation, op_qs: set[qubitron.Qid]
    ) -> tuple[int, list[qubitron.Operation]]:
    ...
    def get_qubitron_circuit(self) -> qubitron.Circuit:
...
def merge_operations(
    ...
    *,
    ...
    for moment_idx, current_moment in enumerate(cast(list['qubitron.Moment'], circuit)):
        ...
        for op in sorted(current_moment.operations, key=lambda op: op.qubits):
            ...
            while left_ops and op_qs:
                ...
                left_idx, left_ops = merged_circuit.get_mergeable_ops(op, op_qs)
            merged_circuit.add_op_to_moment(moment_idx, op)
    ret_circuit = merged_circuit.get_qubitron_circuit()
    if deep:
...
def merge_operations_to_circuit_op(
    circuit: CIRCUIT_TYPE,
    can_merge: Callable[[Sequence[qubitron.Operation], Sequence[qubitron.Operation]], bool],
    *,
    tags_to_ignore: Sequence[Hashable] = (),
    ...
    Uses `qubitron.merge_operations` to identify connected components of operations. Moment structure
    ...
    using `qubitron.merge_operations` directly with a custom `merge_func`.
    ...
    def merge_func(op1: qubitron.Operation, op2: qubitron.Operation) -> qubitron.Operation | None:
        def get_ops(op: qubitron.Operation):
            op_untagged = op.untagged
            return (
...
def merge_k_qubit_unitaries_to_circuit_op(
    ...
    *,
    ...
    Uses `qubitron.merge_operations_to_circuit_op` to identify and merge connected components of
    ...
    def can_merge(ops1: Sequence[qubitron.Operation], ops2: Sequence[qubitron.Operation]) -> bool:
        return all(
...
def unroll_circuit_op(
    ...
    *,
    ...
    """Unrolls (tagged) `qubitron.CircuitOperation`s while preserving the moment structure.
    ...
    def map_func(m: circuits.Moment, _: int):
        to_zip: list[qubitron.AbstractCircuit] = []
        for op in m:
            op_untagged = op.untagged
            if isinstance(op_untagged, circuits.CircuitOperation):
                if deep:
...
def unroll_circuit_op_greedy_earliest(
    ...
    *,
    ...
    """Unrolls (tagged) `qubitron.CircuitOperation`s by inserting operations using EARLIEST strategy.

    Each matching `qubitron.CircuitOperation` is replaced by inserting underlying operations using the
    `qubitron.InsertStrategy.EARLIEST` strategy. The greedy approach attempts to minimize circuit depth
...
def unroll_circuit_op_greedy_frontier(
    ...
    *,
    ...
    """Unrolls (tagged) `qubitron.CircuitOperation`s by inserting operations inline at qubit frontier.
    ...
    """
    unrolled_circuit = circuit.unfreeze(copy=True)
    frontier: dict[qubitron.Qid, int] = defaultdict(lambda: 0)
    idx = 0
...
def toggle_tags(circuit: CIRCUIT_TYPE, tags: Sequence[Hashable], *, deep: bool = False):
    ...
    def map_func(op: qubitron.Operation, _) -> qubitron.Operation:
        return (
            op
