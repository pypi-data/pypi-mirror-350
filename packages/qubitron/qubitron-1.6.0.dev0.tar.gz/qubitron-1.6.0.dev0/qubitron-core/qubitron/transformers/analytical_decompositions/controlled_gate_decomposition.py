...
from qubitron.linalg import is_special_unitary, is_unitary, map_eigenvalues
from qubitron.protocols import unitary
...
def _decompose_single_ctrl(
    matrix: np.ndarray, control: qubitron.Qid, target: qubitron.Qid
...
def _ccnot_congruent(c0: qubitron.Qid, c1: qubitron.Qid, target: qubitron.Qid) -> list[qubitron.Operation]:
...
def decompose_multi_controlled_x(
    controls: list[qubitron.Qid], target: qubitron.Qid, free_qubits: list[qubitron.Qid]
) -> list[qubitron.Operation]:
...
def _decompose_su(
    matrix: np.ndarray, controls: list[qubitron.Qid], target: qubitron.Qid
) -> list[qubitron.Operation]:
...
def _decompose_recursive(
    matrix: np.ndarray,
    power: float,
    controls: list[qubitron.Qid],
    target: qubitron.Qid,
    free_qubits: list[qubitron.Qid],
) -> list[qubitron.Operation]:
