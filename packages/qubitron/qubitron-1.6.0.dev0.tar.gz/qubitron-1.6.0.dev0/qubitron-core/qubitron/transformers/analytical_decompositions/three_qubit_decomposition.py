...
import qubitron
from qubitron import ops, transformers as opt
...
def three_qubit_matrix_to_operations(
    ...
    if not qubitron.is_unitary(u, atol=atol):
    ...
    try:
        ...
        raise ImportError(
            "qubitron.three_qubit_unitary_to_operations requires "
    ...
    if len(cs_ops) > 0 and cs_ops[-1] == qubitron.CZ(q2, q0):
        # optimization A.1 - merging the last CZ from the end of CS into UD
        # cz = qubitron.Circuit([cs_ops[-1]]).unitary()
    ...
    return list(qubitron.Circuit(vdh_ops + cs_ops + ud_ops).all_operations())
...
def _cs_to_ops(q0: ops.Qid, q1: ops.Qid, q2: ops.Qid, theta: np.ndarray) -> list[ops.Operation]:
    ...
    # CSD decomposition, but qubitron.ry takes full angles.
    angles = _multiplexed_angles(theta * 2)
    rys = [qubitron.ry(angle).on(q0) for angle in angles]
    ops = [
        rys[0],
        qubitron.CZ(q1, q0),
        rys[1],
...
def _two_qubit_multiplexor_to_ops(
    ...
    """
    u1u2 = u1 @ u2.conj().T
    eigvals, v = qubitron.unitary_eig(u1u2)
...
def _optimize_multiplexed_angles_circuit(operations: Sequence[ops.Operation]):
    ...
    circuit = qubitron.transformers.drop_negligible_operations(circuit)
...
def _middle_multiplexor_to_ops(q0: ops.Qid, q1: ops.Qid, q2: ops.Qid, eigvals: np.ndarray):
    ...
    rzs = [qubitron.rz(angle).on(q0) for angle in angles]
    ops = [
        rzs[0],
        qubitron.CNOT(q1, q0),
        rzs[1],
