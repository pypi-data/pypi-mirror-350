...
def random_state(seed: float):
    return qubitron.testing.random_superposition(4, random_state=seed)
...
def states_with_phases(st: np.ndarray):
    ...
    for _ in range(3):
        ...
        qubitron.to_valid_state_vector(curr_st, num_qubits=2)
        for i in range(4):
...
def test_prepare_two_qubit_state_using_cz(state):
    state = qubitron.to_valid_state_vector(state, num_qubits=2)
    q = qubitron.LineQubit.range(2)
    circuit = qubitron.Circuit(qubitron.prepare_two_qubit_state_using_cz(*q, state))
    ops_cz = [*circuit.findall_operations(lambda op: op.gate == qubitron.CZ)]
    ops_2q = [*circuit.findall_operations(lambda op: qubitron.num_qubits(op) > 1)]
    assert ops_cz == ops_2q
    assert len(ops_cz) <= 1
    assert qubitron.allclose_up_to_global_phase(
...
def test_prepare_two_qubit_state_using_iswap(state, use_iswap_inv):
    ...
    circuit = qubitron.Circuit(
        qubitron.prepare_two_qubit_state_using_iswap(*q, state, use_iswap_inv=use_iswap_inv)
    )
    iswap_gate = qubitron.ISWAP_INV if use_iswap_inv else qubitron.ISWAP
...
def test_prepare_two_qubit_state_using_sqrt_iswap(state, use_sqrt_iswap_inv):
    ...
    q = qubitron.LineQubit.range(2)
    circuit = qubitron.Circuit(
        qubitron.prepare_two_qubit_state_using_sqrt_iswap(
