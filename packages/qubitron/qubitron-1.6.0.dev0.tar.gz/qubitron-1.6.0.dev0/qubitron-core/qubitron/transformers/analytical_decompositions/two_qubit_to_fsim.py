...
from qubitron import circuits, devices, linalg, ops, protocols
...
def decompose_two_qubit_interaction_into_four_fsim_gates(
    interaction: qubitron.SupportsUnitary | np.ndarray,
    *,
    fsim_gate: qubitron.FSimGate | qubitron.ISwapPowGate,
    ...
    Args:
        interaction: The two qubit operation to synthesize. This can either be
            a qubitron object (such as a gate, operation, or circuit) or a raw numpy
        ...
        qubits: The qubits that the resulting operations should apply the
            ...
            qubits of the given interaction (if it is a `qubitron.Operation`) or
            else to `qubitron.LineQubit.range(2)`.
...
def _decompose_two_qubit_interaction_into_two_b_gates(
    interaction: qubitron.SupportsUnitary | np.ndarray | qubitron.KakDecomposition,
    *,
    qubits: Sequence[qubitron.Qid],
) -> list[qubitron.Operation]:
    kak = linalg.kak_decomposition(interaction)
...
def _decompose_b_gate_into_two_fsims(
    *, fsim_gate: qubitron.FSimGate, qubits: Sequence[qubitron.Qid]
) -> list[qubitron.Operation]:
...
def _decompose_interaction_into_two_b_gates_ignoring_single_qubit_ops(
    qubits: Sequence[qubitron.Qid], kak_interaction_coefficients: Iterable[float]
) -> list[qubitron.Operation]:
...
def _fix_single_qubit_gates_around_kak_interaction(
    *, desired: qubitron.KakDecomposition, operations: list[qubitron.Operation], qubits: Sequence[qubitron.Qid]
) -> Iterator[qubitron.Operation]:
