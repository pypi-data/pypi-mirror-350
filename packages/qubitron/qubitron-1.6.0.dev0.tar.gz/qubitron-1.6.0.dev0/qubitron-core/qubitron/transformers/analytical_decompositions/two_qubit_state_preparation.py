...
from qubitron import circuits, ops, qis
...
def prepare_two_qubit_state_using_sqrt_iswap(
    q0: qubitron.Qid, q1: qubitron.Qid, state: qubitron.STATE_VECTOR_LIKE, *, use_sqrt_iswap_inv: bool = True
    ...
    Args:
        ...
        use_sqrt_iswap_inv: If True, uses `qubitron.SQRT_ISWAP_INV` instead of `qubitron.SQRT_ISWAP`.
...
def prepare_two_qubit_state_using_cz(
    q0: qubitron.Qid, q1: qubitron.Qid, state: qubitron.STATE_VECTOR_LIKE
) -> list[qubitron.Operation]:
...
def prepare_two_qubit_state_using_iswap(
    q0: qubitron.Qid, q1: qubitron.Qid, state: qubitron.STATE_VECTOR_LIKE, use_iswap_inv: bool = False
) -> list[qubitron.Operation]:
    """Prepares the given 2q state from |00> using at-most 1 ISWAP gate + single qubit rotations.
    ...
    Args:
        ...
        q1: The other qubit being operated on.
        state: 4x1 matrix representing two qubit state vector, ordered as 00, 01, 10, 11.
        use_iswap_inv: If True, uses `qubitron.ISWAP_INV` instead of `qubitron.ISWAP`.
