...
from qubitron import circuits, ops, protocols, transformers
from qubitron.transformers import merge_k_qubit_gates, merge_single_qubit_gates
...
def create_transformer_with_kwargs(transformer: qubitron.TRANSFORMER, **kwargs) -> qubitron.TRANSFORMER:
    """Method to capture additional keyword arguments to transformers while preserving mypy type.

    Returns a `qubitron.TRANSFORMER` which, when called with a circuit and transformer context, is
    ...
    expects `qubitron.TRANSFORMER`. For example:

    >>> def run_transformers(transformers: list[qubitron.TRANSFORMER]):
    ...     circuit = qubitron.Circuit(qubitron.X(qubitron.q(0)))
    ...     context = qubitron.TransformerContext()
    ...     for transformer in transformers:
    ...         transformer(circuit, context=context)
    ...
    >>> transformers: list[qubitron.TRANSFORMER] = []
    >>> transformers.append(
    ...     qubitron.create_transformer_with_kwargs(
    ...         qubitron.expand_composite, no_decomp=lambda op: qubitron.num_qubits(op) <= 2
    ...     )
    ... )
    >>> transformers.append(qubitron.create_transformer_with_kwargs(qubitron.merge_k_qubit_unitaries, k=2))
    >>> run_transformers(transformers)


    Args:
         transformer: A `qubitron.TRANSFORMER` for which additional kwargs should be captured.
         **kwargs: The keyword arguments which should be captured and passed to `transformer`.

    Returns:
        A `qubitron.TRANSFORMER` method `transformer_with_kwargs`, s.t. executing
...
class CompilationTargetGateset(ops.Gateset, metaclass=abc.ABCMeta):
    ...
    An instance of this type can be passed to transformers like `qubitron.optimize_for_target_gateset`,
    which can transform any given circuit to contain gates accepted by this gateset.
    """

    def __init__(
        self,
        *gates: type[qubitron.Gate] | qubitron.Gate | qubitron.GateFamily,
        name: str | None = None,
    ...
    @abc.abstractmethod
    def decompose_to_target_gateset(self, op: qubitron.Operation, moment_idx: int) -> DecomposeResult:
        """Method to rewrite the given operation using gates from this gateset.

        Args:
            op: `qubitron.Operation` to be rewritten using gates from this gateset.
            moment_idx: Moment index where the given operation `op` occurs in a circuit.

        Returns:
            - An equivalent `qubitron.OP_TREE` implementing `op` using gates from this gateset.
            - `None` or `NotImplemented` if does not know how to decompose `op`.
        """

    def _validate_operation(self, op: qubitron.Operation) -> bool:
    ...
    @property
    def postprocess_transformers(self) -> list[qubitron.TRANSFORMER]:
        """List of transformers which should be run after decomposing individual operations."""
        processors: list[qubitron.TRANSFORMER] = [
...
class TwoQubitCompilationTargetGateset(CompilationTargetGateset):
    ...
    implements the logic to
        ...
        1. Apply `self.preprocess_transformers` to the input circuit, which by default will:
            a) Expand composite gates acting on > 2 qubits using `qubitron.expand_composite`.
            b) Merge connected components of 1 & 2 qubit unitaries into tagged
                `qubitron.CircuitOperation` using `qubitron.merge_k_qubit_unitaries`.
        ...
        3. Apply `self.postprocess_transformers` to the transformed circuit, which by default will:
            a) Apply `qubitron.merge_single_qubit_moments_to_phxz` to preserve moment structure (eg:
               alternating layers of single/two qubit gates).
            b) Apply `qubitron.drop_negligible_operations` and `qubitron.drop_empty_moments` to minimize
               circuit depth.
    ...
    def decompose_to_target_gateset(self, op: qubitron.Operation, moment_idx: int) -> DecomposeResult:
        ...
        if switch_to_new:
            return new_optree
        mapped_old_optree: list[qubitron.OP_TREE] = []
    ...
    def _decompose_single_qubit_operation(
        self, op: qubitron.Operation, moment_idx: int
    ) -> DecomposeResult:
        """Decomposes (connected component of) 1-qubit operations using gates from this gateset.

        By default, rewrites every operation using a single `qubitron.PhasedXZGate`.

        Args:
            op: A single-qubit operation (can be a tagged `qubitron.CircuitOperation` wrapping
                a connected component of single qubit unitaries).
            moment_idx: Index of the moment in which operation `op` occurs.

        Returns:
            A `qubitron.OP_TREE` implementing `op` using gates from this gateset OR
    ...
    def _decompose_two_qubit_operation(
        ...
        Args:
            op: A two-qubit operation (can be a tagged `qubitron.CircuitOperation` wrapping
