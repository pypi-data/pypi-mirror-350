...
import qubitron
from qubitron.ops import SingleQubitCliffordGate
...
class RandomizedMeasurements:
    ...
    def __call__(
        ...
        *,
        ...
        return qubitron.Circuit.from_moments(
            *circuit.moments, pre_measurement_moment, qubitron.M(*subsystem_qubits, key="m")
        )

    def random_single_qubit_unitary_moment(
        self, unitary_ensemble: str, qubits: Sequence[Any], rng: np.random.Generator
    ) -> qubitron.Moment:
        """Outputs the qubitron moment associated with the pre-measurement rotations.
        ...
        Returns:
            The qubitron moment associated with the pre-measurement rotations
        ...
        op_list: list[qubitron.Operation] = []

        for idx, unitary in enumerate(unitaries):
            op_list.append(unitary.on(qubits[idx]))

        return qubitron.Moment.from_ops(*op_list)


def _pauli_basis_rotation(rng: np.random.Generator) -> qubitron.Gate:
    ...
    if basis_idx == 0:
        gate: qubitron.Gate = qubitron.Ry(rads=-np.pi / 2)
    elif basis_idx == 1:
        gate = qubitron.Rx(rads=np.pi / 2)
    else:
        gate = qubitron.I
    return gate


def _single_qubit_clifford(rng: np.random.Generator) -> qubitron.Gate:
...
def _single_qubit_cue(rng: np.random.Generator) -> qubitron.Gate:
