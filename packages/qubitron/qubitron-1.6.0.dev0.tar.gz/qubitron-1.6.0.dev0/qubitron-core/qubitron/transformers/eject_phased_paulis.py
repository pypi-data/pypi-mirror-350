...
from qubitron import circuits, ops, protocols, value
...
def eject_phased_paulis(
    ...
    *,
    ...
    then be removed by the `qubitron.eject_z` transformation).

    `qubitron.PhasedXZGate` with `z_exponent=0` (i.e. equivalent to PhasedXPow) or with `x_exponent=0`
    ...
    `qubitron.eject_z(qubitron.eject_phased_paulis(qubitron.eject_z(circuit)))`.
    ...
    def map_func(op: qubitron.Operation, _: int) -> qubitron.OP_TREE:
...
def _single_cross_over_cz(op: ops.Operation, qubit_with_w: qubitron.Qid) -> qubitron.OP_TREE:
