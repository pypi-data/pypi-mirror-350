...
from qubitron.transformers.routing import line_initial_mapper, mapping_manager
...
def _disjoint_nc2_combinations(
    ...
    For example:
        ...
        >>> q = [*range(5)]
        >>> disjoint_swaps = qubitron.transformers.routing.route_circuit_cqc._disjoint_nc2_combinations(
...
class RouteCQC:
    ...
    2. Places the logical qubits in the input circuit onto some input device graph by using an
        initial mapper (`qubitron.LineInitialMapper` by default).
    ...
    For example:
        ...
        >>> import qubitron_google as cg
        >>> circuit = qubitron.testing.random_circuit(5, 10, 0.6)
        >>> device = cg.Sycamore
        >>> router = qubitron.RouteCQC(device.metadata.nx_graph)
        >>> rcirc, initial_map, swap_map = router.route_circuit(circuit)
        >>> fcirc = qubitron.optimize_for_target_gateset(rcirc, gateset = cg.SycamoreTargetGateset())
    ...
    def __call__(
        ...
        *,
        ...
        initial_mapper: qubitron.AbstractInitialMapper | None = None,
        ...
        Args:
            ...
            tag_inserted_swaps: whether or not a `qubitron.RoutingSwapTag` should be attached to
            ...
            initial_mapper: an initial mapping strategy (placement) of logical qubits in the
                ...
                instance of `qubitron.LineInitialMapper`.
    ...
    def route_circuit(
        ...
        *,
        ...
        initial_mapper: qubitron.AbstractInitialMapper | None = None,
        context: qubitron.TransformerContext | None = None,
    ) -> tuple[qubitron.AbstractCircuit, dict[qubitron.Qid, qubitron.Qid], dict[qubitron.Qid, qubitron.Qid]]:
        """Transforms the given circuit to make it executable on the device.
        ...
        `circuit` contains `qubitron.CircuitOperation`s and `context.deep` is True then they are first
        unrolled before proceeding. If `context.deep` is False or `context` is None then any
        `qubitron.CircuitOperation` that acts on more than 2-qubits will also raise an error.
    ...
    def _get_one_and_two_qubit_ops_as_timesteps(
        cls, circuit: qubitron.AbstractCircuit
    ) -> tuple[list[list[qubitron.Operation]], list[list[qubitron.Operation]]]:
        ...
        single_qubit_ops: list[list[qubitron.Operation]] = []
    ...
    def _route(
        ...
        two_qubit_ops: list[list[qubitron.Operation]],
        single_qubit_ops: list[list[qubitron.Operation]],
        lookahead_radius: int,
        tag_inserted_swaps: bool = False,
    ) -> list[list[qubitron.Operation]]:
        ...
        routed_ops: list[list[qubitron.Operation]] = []

        def process_executable_two_qubit_ops(timestep: int) -> int:
            unexecutable_ops: list[qubitron.Operation] = []
    ...
    def __repr__(self) -> str:
        return f'qubitron.RouteCQC(nx.Graph({dict(self.device_graph.adjacency())}))'
