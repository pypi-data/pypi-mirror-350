...
def construct_small_circuit():
    return qubitron.Circuit(
        [
            qubitron.Moment(qubitron.CNOT(qubitron.NamedQubit('1'), qubitron.NamedQubit('3'))),
            qubitron.Moment(qubitron.CNOT(qubitron.NamedQubit('2'), qubitron.NamedQubit('3'))),
            qubitron.Moment(
                qubitron.CNOT(qubitron.NamedQubit('4'), qubitron.NamedQubit('3')), qubitron.X(qubitron.NamedQubit('5'))
            ),
        ]
    )
...
def construct_step_circuit(k: int):
    q = qubitron.LineQubit.range(k)
    return qubitron.Circuit([qubitron.CNOT(q[i], q[i + 1]) for i in range(k - 1)])
...
def construct_valid_circuit():
    return qubitron.Circuit(
        [
            qubitron.Moment(
                qubitron.CNOT(qubitron.NamedQubit('3'), qubitron.NamedQubit('9')),
                qubitron.CNOT(qubitron.NamedQubit('8'), qubitron.NamedQubit('12')),
            ),
            qubitron.Moment(
                qubitron.CNOT(qubitron.NamedQubit('10'), qubitron.NamedQubit('11')),
                qubitron.CNOT(qubitron.NamedQubit('8'), qubitron.NamedQubit('12')),
                qubitron.CNOT(qubitron.NamedQubit('14'), qubitron.NamedQubit('6')),
                qubitron.CNOT(qubitron.NamedQubit('5'), qubitron.NamedQubit('4')),
            ),
            qubitron.Moment(
                qubitron.CNOT(qubitron.NamedQubit('8'), qubitron.NamedQubit('2')),
                qubitron.CNOT(qubitron.NamedQubit('3'), qubitron.NamedQubit('9')),
                qubitron.CNOT(qubitron.NamedQubit('6'), qubitron.NamedQubit('0')),
                qubitron.CNOT(qubitron.NamedQubit('14'), qubitron.NamedQubit('10')),
            ),
            qubitron.Moment(
                qubitron.CNOT(qubitron.NamedQubit('14'), qubitron.NamedQubit('6')),
                qubitron.CNOT(qubitron.NamedQubit('1'), qubitron.NamedQubit('4')),
            ),
            qubitron.Moment(
                qubitron.CNOT(qubitron.NamedQubit('8'), qubitron.NamedQubit('12')),
                qubitron.CNOT(qubitron.NamedQubit('14'), qubitron.NamedQubit('10')),
...
def test_valid_circuit() -> None:
    ...
    mapper = qubitron.LineInitialMapper(device_graph)
...
def test_small_circuit_on_grid_device() -> None:
    ...
    device_graph = qubitron.testing.construct_grid_device(7, 7).metadata.nx_graph
    mapper = qubitron.LineInitialMapper(device_graph)
    mapping = mapper.initial_mapping(circuit)

    assert mapper.center == qubitron.GridQubit(3, 3)

    expected_circuit = qubitron.Circuit(
        [
            qubitron.Moment(qubitron.CNOT(qubitron.GridQubit(1, 3), qubitron.GridQubit(2, 3))),
            qubitron.Moment(qubitron.CNOT(qubitron.GridQubit(3, 3), qubitron.GridQubit(2, 3))),
            qubitron.Moment(
                qubitron.CNOT(qubitron.GridQubit(2, 2), qubitron.GridQubit(2, 3)), qubitron.X(qubitron.GridQubit(3, 2))
            ),
        ]
    )
    qubitron.testing.assert_same_circuits(circuit.transform_qubits(mapping), expected_circuit)


def test_small_circuit_on_ring_device() -> None:
    circuit = construct_small_circuit()
    device_graph = qubitron.testing.construct_ring_device(10, directed=True).metadata.nx_graph

    mapper = qubitron.LineInitialMapper(device_graph)
    mapping = mapper.initial_mapping(circuit)
    assert mapper.center == qubitron.LineQubit(0)

    expected_circuit = qubitron.Circuit(
        [
            qubitron.Moment(qubitron.CNOT(qubitron.LineQubit(2), qubitron.LineQubit(1))),
            qubitron.Moment(qubitron.CNOT(qubitron.LineQubit(0), qubitron.LineQubit(1))),
            qubitron.Moment(qubitron.CNOT(qubitron.LineQubit(3), qubitron.LineQubit(1)), qubitron.X(qubitron.LineQubit(4))),
        ]
    )
    qubitron.testing.assert_same_circuits(circuit.transform_qubits(mapping), expected_circuit)


glob_device_graph = qubitron.testing.construct_grid_device(7, 7).metadata.nx_graph
glob_mapper = qubitron.LineInitialMapper(glob_device_graph)
...
def test_repr() -> None:
    device_graph = qubitron.testing.construct_grid_device(7, 7).metadata.nx_graph
    mapper = qubitron.LineInitialMapper(device_graph)
    qubitron.testing.assert_equivalent_repr(mapper, setup_code='import qubitron\nimport networkx as nx')
