...
from qubitron.transformers.routing import initial_mapper
...
class LineInitialMapper(initial_mapper.AbstractInitialMapper):
    ...
    def _make_circuit_graph(
        self, circuit: qubitron.AbstractCircuit
    ) -> tuple[list[deque[qubitron.Qid]], dict[qubitron.Qid, qubitron.Qid]]:
        ...
        """
        circuit_graph: list[deque[qubitron.Qid]] = [deque([q]) for q in sorted(circuit.all_qubits())]
        component_id: dict[qubitron.Qid, int] = {q[0]: i for i, q in enumerate(circuit_graph)}
        partners: dict[qubitron.Qid, qubitron.Qid] = {}

        def degree_lt_two(q: qubitron.Qid):
    ...
    def initial_mapping(self, circuit: qubitron.AbstractCircuit) -> dict[qubitron.Qid, qubitron.Qid]:
        ...
        Returns:
            ...
            device (values).
        """
        mapped_physicals: set[qubitron.Qid] = set()
        qubit_map: dict[qubitron.Qid, qubitron.Qid] = {}
        circuit_graph, partners = self._make_circuit_graph(circuit)

        def next_physical(
            current_physical: qubitron.Qid, partner: qubitron.Qid, isolated: bool = False
        ) -> qubitron.Qid:
    ...
    def _closest_unmapped_qubit(
        self, source: qubitron.Qid, mapped_physicals: set[qubitron.Qid]
    ) -> qubitron.Qid:
    ...
    def __repr__(self):
        ...
        return f'qubitron.LineInitialMapper(nx.{graph_type}({dict(self.device_graph.adjacency())}))'
