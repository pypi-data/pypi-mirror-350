...
from qubitron._compat import proper_eq, proper_repr
from qubitron.transformers.heuristic_decompositions.gate_tabulation_math_utils import (
...
class TwoQubitGateTabulation:
    ...
    def compile_two_qubit_gate(self, unitary: np.ndarray) -> TwoQubitGateTabulationResult:
        ...
        kak_vec = qubitron.kak_vector(unitary, check_preconditions=False)
    ...
    def __repr__(self) -> str:
        ...
        return (
            f'qubitron.TwoQubitGateTabulation({proper_repr(self.base_gate)}, '
...
def _outer_locals_for_unitary(
    ...
    """
    target_decomp = qubitron.kak_decomposition(target)
    base_decomp = qubitron.kak_decomposition(base)
...
def _tabulate_kak_vectors(
    *,
    ...
    kak_vectors = qubitron.kak_vector(prods, check_preconditions=False)
...
def two_qubit_gate_product_tabulation(
    ...
    *,
    ...
    kak_vecs = [qubitron.kak_vector(base_gate, check_preconditions=False)]
    ...
    for ind in missing_vec_inds:
        ...
        kaks = qubitron.kak_vector(products, check_preconditions=False)
        ...
        if min_dist < tabulation_cutoff:
            ...
            kak_vecs.append(qubitron.kak_vector(base_gate @ actual, check_preconditions=False))
