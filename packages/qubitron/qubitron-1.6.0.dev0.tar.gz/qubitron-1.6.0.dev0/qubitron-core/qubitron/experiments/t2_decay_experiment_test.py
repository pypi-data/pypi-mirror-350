...
import qubitron
import qubitron.experiments.t2_decay_experiment as t2
...
def test_init_t2_decay_result():
    ...
    result = qubitron.experiments.T2DecayResult(x_data, y_data)
    assert result
    ...
    with pytest.raises(ValueError):
        qubitron.experiments.T2DecayResult(bad_data, y_data)
    with pytest.raises(ValueError):
        qubitron.experiments.T2DecayResult(x_data, bad_data)


@pytest.mark.usefixtures('closefigures')
def test_plot_does_not_raise_error():
    class _TimeDependentDecay(qubitron.NoiseModel):
        def noisy_moment(self, moment, system_qubits):
            duration = max(
                (
                    op.gate.duration
                    for op in moment.operations
                    if isinstance(op.gate, qubitron.WaitGate)
                ),
                default=qubitron.Duration(nanos=1),
            )
            yield qubitron.amplitude_damp(1 - 0.99 ** duration.total_nanos()).on_each(system_qubits)
            yield moment

    results = qubitron.experiments.t2_decay(
        sampler=qubitron.DensityMatrixSimulator(noise=_TimeDependentDecay()),
        qubit=qubitron.GridQubit(0, 0),
        num_points=3,
        repetitions=10,
        max_delay=qubitron.Duration(nanos=500),
    )
...
def test_result_eq():
    ...
    eq.make_equality_group(lambda: qubitron.experiments.T2DecayResult(example_data, example_data))

    eq.add_equality_group(qubitron.experiments.T2DecayResult(other_data, example_data))
    eq.add_equality_group(qubitron.experiments.T2DecayResult(example_data, other_data))


def test_sudden_decay_results():
    class _SuddenDecay(qubitron.NoiseModel):
        def noisy_moment(self, moment, system_qubits):
            duration = max(
                (
                    op.gate.duration
                ...
                default=qubitron.Duration(),
            )
            if duration > qubitron.Duration(nanos=500):
                yield qubitron.amplitude_damp(1).on_each(system_qubits)
            yield moment

    results = qubitron.experiments.t2_decay(
        sampler=qubitron.DensityMatrixSimulator(noise=_SuddenDecay()),
        qubit=qubitron.GridQubit(0, 0),
        num_points=4,
        repetitions=500,
        min_delay=qubitron.Duration(nanos=100),
        max_delay=qubitron.Duration(micros=1),
    )
...
def test_spin_echo_cancels_out_constant_rate_phase(experiment_type):
    class _TimeDependentPhase(qubitron.NoiseModel):
        def noisy_moment(self, moment, system_qubits):
            duration = max(
                (
                    op.gate.duration
            ...
            yield (qubitron.Y**phase).on_each(system_qubits)
            yield moment

    pulses = [1] if experiment_type == t2.ExperimentType.CPMG else None
    results = qubitron.experiments.t2_decay(
        sampler=qubitron.DensityMatrixSimulator(noise=_TimeDependentPhase()),
        qubit=qubitron.GridQubit(0, 0),
        num_points=10,
        repetitions=100,
        min_delay=qubitron.Duration(nanos=100),
...
def test_all_off_results(experiment_type):
    pulses = [1] if experiment_type == t2.ExperimentType.CPMG else None
    results = t2.t2_decay(
        sampler=qubitron.DensityMatrixSimulator(noise=qubitron.amplitude_damp(1)),
        qubit=qubitron.GridQubit(0, 0),
        num_points=4,
        repetitions=10,
        min_delay=qubitron.Duration(nanos=100),
    ...
    assert results == qubitron.experiments.T2DecayResult(
...
def test_custom_delay_sweep(experiment_type):
    ...
    results = t2.t2_decay(
        ...
        experiment_type=experiment_type,
        num_pulses=pulses,
        delay_sweep=qubitron.Points('delay_ns', [1.0, 10.0, 100.0, 1000.0, 10000.0]),
    )
    assert results == qubitron.experiments.T2DecayResult(
        x_basis_data=pd.DataFrame(
            columns=['delay_ns', 0, 1],
...
def test_bad_args():
    with pytest.raises(ValueError, match='repetitions <= 0'):
        _ = qubitron.experiments.t2_decay(
            sampler=qubitron.Simulator(),
            qubit=qubitron.GridQubit(0, 0),
            num_points=4,
            repetitions=0,
    ...
    with pytest.raises(ValueError, match='max_delay < min_delay'):
        _ = qubitron.experiments.t2_decay(
            ...
            num_points=4,
            repetitions=10,
            min_delay=qubitron.Duration(micros=1),
            max_delay=qubitron.Duration(micros=0),
        )
    ...
    with pytest.raises(ValueError, match='sympy expressions'):
        _ = qubitron.experiments.t2_decay(
            ...
            num_points=4,
            repetitions=10,
            max_delay=qubitron.Duration(micros=sympy.Symbol('t')),
            min_delay=qubitron.Duration(micros=sympy.Symbol('t')),
        )
    ...
    with pytest.raises(ValueError, match='delay_ns'):
        _ = qubitron.experiments.t2_decay(
            ...
            num_points=4,
            repetitions=10,
            max_delay=qubitron.Duration(micros=10),
            min_delay=qubitron.Duration(micros=1),
            delay_sweep=qubitron.Linspace(sympy.Symbol('t'), start=10, stop=2000, length=10),
        )
    sweep1 = qubitron.Linspace(sympy.Symbol('delay_ns'), start=10, stop=100, length=10)
    sweep2 = qubitron.Linspace(sympy.Symbol('t'), start=20, stop=200, length=10)
    product = qubitron.Product(sweep1, sweep2)
    with pytest.raises(ValueError, match='delay_ns'):
        _ = qubitron.experiments.t2_decay(
            sampler=qubitron.Simulator(),
            qubit=qubitron.GridQubit(0, 0),
...
def test_cpmg_circuit():
    ...
    expected = qubitron.Circuit(
        qubitron.Y(q) ** 0.5,
        qubitron.wait(q, nanos=t),
        qubitron.X(q),
        qubitron.wait(q, nanos=2 * t * sympy.Symbol('pulse_0')),
        qubitron.X(q) ** sympy.Symbol('pulse_0'),
        qubitron.wait(q, nanos=2 * t * sympy.Symbol('pulse_1')),
        qubitron.X(q) ** sympy.Symbol('pulse_1'),
        qubitron.wait(q, nanos=t),
    )
...
def test_cpmg_sweep():
    ...
    expected = qubitron.Zip(
        qubitron.Points('pulse_0', [1, 1, 1]),
        qubitron.Points('pulse_1', [0, 1, 1]),
        qubitron.Points('pulse_2', [0, 1, 1]),
        qubitron.Points('pulse_3', [0, 0, 1]),
        qubitron.Points('pulse_4', [0, 0, 1]),
    )
