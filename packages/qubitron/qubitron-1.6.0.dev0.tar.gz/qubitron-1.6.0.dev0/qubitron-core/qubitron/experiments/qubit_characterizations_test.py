...
import qubitron
import qubitron.experiments.qubit_characterizations as ceqc
from qubitron import circuits, GridQubit, ops, sim
...
def test_single_qubit_cliffords():
    ...
    def is_pauli(u):
        return any(qubitron.equal_up_to_global_phase(u, p) for p in PAULIS)
    ...
    def check_distinct(unitaries):
        ...
        for i in range(n):
            for j in range(i + 1, n):
                ...
                assert not qubitron.allclose_up_to_global_phase(Ui, Uj), f'{i}, {j}'
    ...
    for Uxy in xy_unitaries:
        assert any(qubitron.allclose_up_to_global_phase(Uxy, Uxz) for Uxz in xz_unitaries)
    ...
    for gates in cliffords.c1_in_xz:
        num_i = len([gate for gate in gates if gate == qubitron.ops.SingleQubitCliffordGate.I])
        num_x = len(
            [
                gate
                for gate in gates
                if gate
                in (
                    qubitron.ops.SingleQubitCliffordGate.X,
                    qubitron.ops.SingleQubitCliffordGate.X_sqrt,
                    qubitron.ops.SingleQubitCliffordGate.X_nsqrt,
                )
            ]
        )
        num_z = len(
            [
                ...
                if gate
                in (
                    qubitron.ops.SingleQubitCliffordGate.Z,
                    qubitron.ops.SingleQubitCliffordGate.Z_sqrt,
                    qubitron.ops.SingleQubitCliffordGate.Z_nsqrt,
                )
            ]
        )
...
def test_single_qubit_cliffords_gateset():
    ...
    clifford_group = qubitron.experiments.qubit_characterizations._single_qubit_cliffords()
    c = qubitron.experiments.qubit_characterizations._create_parallel_rb_circuit(
        qubits, 5, clifford_group.c1_in_xy
    )
    device = qubitron.testing.ValidatingTestDevice(
        qubits=qubits, allowed_gates=(qubitron.ops.PhasedXZGate, qubitron.MeasurementGate)
    )
