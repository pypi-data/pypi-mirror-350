...
from qubitron import ops, value, vis
from qubitron._compat import cached_method
from qubitron.experiments import random_quantum_circuit_generation as rqcg
from qubitron.experiments.qubit_characterizations import (
    parallel_single_qubit_randomized_benchmarking,
    ParallelRandomizedBenchmarkingResult,
)
from qubitron.experiments.xeb_fitting import (
    benchmark_2q_xeb_fidelities,
    exponential_decay,
    fit_exponential_decays,
)
from qubitron.experiments.xeb_sampling import sample_2q_xeb_circuits
from qubitron.qis import noise_utils
...
def _grid_qubits_for_sampler(sampler: qubitron.Sampler) -> Sequence[qubitron.GridQubit] | None:
...
def qubits_and_pairs(
    ...
    qubits: Sequence[qubitron.GridQubit] | None = None,
    pairs: Sequence[tuple[qubitron.GridQubit, qubitron.GridQubit]] | None = None,
) -> tuple[Sequence[qubitron.GridQubit], Sequence[tuple[qubitron.GridQubit, qubitron.GridQubit]]]:
...
class TwoQubitXEBResult:
    ...
    def _qubit_pair_map(self) -> dict[tuple[qubitron.GridQubit, qubitron.GridQubit], int]:
    ...
    @functools.cached_property
    def all_qubit_pairs(self) -> tuple[tuple[qubitron.GridQubit, qubitron.GridQubit], ...]:
    ...
    def plot_heatmap(self, ax: plt.Axes | None = None, **plot_kwargs) -> plt.Axes:
        ...
        heatmap_data: dict[tuple[qubitron.GridQubit, ...], float] = {
    ...
    def plot_fitted_exponential(
        self, q0: qubitron.GridQubit, q1: qubitron.GridQubit, ax: plt.Axes | None = None, **plot_kwargs
    ) -> plt.Axes:
    ...
    def xeb_fidelity(self, q0: qubitron.GridQubit, q1: qubitron.GridQubit) -> float:
    ...
    def xeb_error(self, q0: qubitron.GridQubit, q1: qubitron.GridQubit) -> float:
        """Return the XEB error of a qubit pair."""
        return 1 - self.xeb_fidelity(q0, q1)

    def all_errors(self) -> dict[tuple[qubitron.GridQubit, qubitron.GridQubit], float]:
    ...
    @cached_method
    def pauli_error(self) -> dict[tuple[qubitron.GridQubit, qubitron.GridQubit], float]:
...
class InferredXEBResult:
    ...
    @property
    def all_qubit_pairs(self) -> Sequence[tuple[qubitron.GridQubit, qubitron.GridQubit]]:
        return self.xeb_result.all_qubit_pairs

    @cached_method
    def single_qubit_pauli_error(self) -> Mapping[qubitron.Qid, float]:
        """Return the single-qubit Pauli error for all qubits (RB results)."""
        return self.rb_result.pauli_error()

    @cached_method
    def two_qubit_pauli_error(self) -> Mapping[tuple[qubitron.GridQubit, qubitron.GridQubit], float]:
        """Return the two-qubit Pauli error for all pairs."""
        return MappingProxyType(self.xeb_result.pauli_error())

    @cached_method
    def inferred_pauli_error(self) -> Mapping[tuple[qubitron.GridQubit, qubitron.GridQubit], float]:
        """Return the inferred Pauli error for all pairs."""
        single_q_paulis = self.rb_result.pauli_error()
        xeb = self.xeb_result.pauli_error()

        def _pauli_error(q0: qubitron.GridQubit, q1: qubitron.GridQubit) -> float:
            q0, q1 = sorted([q0, q1])
    ...
    @cached_method
    def inferred_decay_constant(self) -> Mapping[tuple[qubitron.GridQubit, qubitron.GridQubit], float]:
    ...
    @cached_method
    def inferred_xeb_error(self) -> Mapping[tuple[qubitron.GridQubit, qubitron.GridQubit], float]:
    ...
    ) -> Mapping[tuple[qubitron.GridQubit, qubitron.GridQubit], float]:
    ...
    def plot_heatmap(
        ...
        if not isinstance(ax, plt.Axes):
            fig, ax = plt.subplots(1, 1, figsize=(8, 8))
        heatmap_data = cast(
            Mapping[tuple['qubitron.GridQubit', ...], float], self._target_errors(target_error)
        )
...
def parallel_xeb_workflow(
    sampler: qubitron.Sampler,
    qubits: Sequence[qubitron.GridQubit] | None = None,
    entangling_gate: qubitron.Gate = ops.CZ,
    n_repetitions: int = 10**4,
    n_combinations: int = 10,
    n_circuits: int = 20,
    cycle_depths: Sequence[int] = (5, 25, 50, 100, 200, 300),
    random_state: qubitron.RANDOM_STATE_OR_SEED_LIKE = None,
    ax: plt.Axes | None = None,
    pairs: Sequence[tuple[qubitron.GridQubit, qubitron.GridQubit]] | None = None,
    ...
    tags: Sequence[Any] = (),
    **plot_kwargs,
) -> tuple[pd.DataFrame, Sequence[qubitron.Circuit], pd.DataFrame]:
