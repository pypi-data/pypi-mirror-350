...
from qubitron import devices, ops, protocols, value
from qubitron.circuits import Circuit, Moment
from qubitron.experiments.random_quantum_circuit_generation import CircuitLibraryCombination
...
class _Sample2qXEBTask:
    ...
    layer_i: int
    combination_i: int
    prepared_circuit: qubitron.AbstractCircuit
...
def _verify_and_get_two_qubits_from_circuits(circuits: Sequence[qubitron.Circuit]):
    """Make sure each of the provided circuits uses the same two qubits and return them."""
    all_qubits_set: set[qubitron.Qid] = set()
...
def _verify_two_line_qubits_from_circuits(circuits: Sequence[qubitron.Circuit]):
...
class _ZippedCircuit:
    ...
    wide_circuit: qubitron.Circuit
    pairs: list[tuple[qubitron.Qid, qubitron.Qid]]
    combination: list[int]
    layer_i: int
    combination_i: int
...
) -> tuple[list[CircuitLibraryCombination], list[qubitron.Circuit]]:
...
def _zip_circuits(
    circuits: Sequence[qubitron.Circuit], combinations_by_layer: list[CircuitLibraryCombination]
) -> list[_ZippedCircuit]:
...
def sample_2q_xeb_circuits(
    ...
    *,
    ...
    shuffle: qubitron.RANDOM_STATE_OR_SEED_LIKE | None = None,
    dataset_directory: str | None = None,
):
    """Sample two-qubit XEB circuits given a sampler.

    Args:
        sampler: A Qubitron sampler for executing circuits.
