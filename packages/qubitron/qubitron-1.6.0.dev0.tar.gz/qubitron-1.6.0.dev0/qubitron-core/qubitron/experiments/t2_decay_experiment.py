...
from qubitron import circuits, ops, study, value
...
def t2_decay(
    ...
    *,
    ...
    num_points: int,
    max_delay: qubitron.DURATION_LIKE,
    min_delay: qubitron.DURATION_LIKE = None,
    repetitions: int = 1000,
    delay_sweep: study.Sweep | None = None,
    num_pulses: list[int] | None = None,
) -> qubitron.experiments.T2DecayResult | list[qubitron.experiments.T2DecayResult]:
    ...
    prepared with a square root Y gate (`qubitron.Y ** 0.5`) and then waits for
    ...
    by performing either a `qubitron.Y ** -0.5` or `qubitron.X ** 0.5`.  The square of
    ...
    will also be prepared with a square root Y gate (`qubitron.Y ** 0.5`).
    However, during the mid-point of the delay time being measured, a pi-pulse
    (`qubitron.X`) gate will be applied to cancel out inhomogeneous dephasing.
...
def _cpmg_circuit(qubit: qubitron.Qid, delay_var: sympy.Symbol, max_pulses: int) -> qubitron.Circuit:
...
class T2DecayResult:
    ...
    def __repr__(self):
        return (
            f'qubitron.experiments.T2DecayResult('
