...
import qubitron.vis.heatmap as qubitron_heatmap
import qubitron.vis.histogram as qubitron_histogram
from qubitron import circuits, ops, protocols
from qubitron.devices import grid_qubit
...
class ParallelRandomizedBenchmarkingResult:
    ...
    results_dictionary: Mapping[qubitron.Qid, RandomizedBenchMarkResult]

    def plot_single_qubit(
        self, qubit: qubitron.Qid, ax: plt.Axes | None = None, **plot_kwargs: Any
    ) -> plt.Axes:
    ...
    def pauli_error(self) -> Mapping[qubitron.Qid, float]:
    ...
    def plot_heatmap(
        ...
        **plot_kwargs: Any,
        ...
        """Plot a heatmap of the Pauli errors. If qubits are not qubitron.GridQubits, throws an error.
        ...
        Args:
            ...
            **plot_kwargs: Arguments to be passed to 'qubitron.Heatmap.plot()'.
        Returns:
            The plt.Axes containing the plot.
        """
        ...
        for qubit in pauli_errors:
            assert type(qubit) == grid_qubit.GridQubit, "qubits must be qubitron.GridQubits"
            pauli_errors_with_grid_qubit_keys[qubit] = pauli_errors[qubit]  # just for typecheck

        if ax is None:
            _, ax = plt.subplots(dpi=200, facecolor='white')

        ax, _ = qubitron_heatmap.Heatmap(pauli_errors_with_grid_qubit_keys).plot(
    ...
    def plot_integrated_histogram(
        ...
        **kwargs,
        ...
        """Plot the Pauli errors using qubitron.integrated_histogram().
        ...
        ax = qubitron_histogram.integrated_histogram(
...
def parallel_single_qubit_randomized_benchmarking(
    ...
    *,
    ...
    # create circuits
    circuits_all: list[qubitron.AbstractCircuit] = []
    ...
    idx = 0
    for num_cliffords in num_clifford_range:
        excited_probs: dict[qubitron.Qid, list[float]] = {q: [] for q in qubits}
        for _ in range(num_circuits):
...
def two_qubit_randomized_benchmarking(
    sampler: qubitron.Sampler,
    first_qubit: qubitron.Qid,
    second_qubit: qubitron.Qid,
    *,
...
def single_qubit_state_tomography(
    sampler: qubitron.Sampler, qubit: qubitron.Qid, circuit: qubitron.AbstractCircuit, repetitions: int = 1000
) -> TomographyResult:
...
def _create_parallel_rb_circuit(
    qubits: Sequence[qubitron.Qid], num_cliffords: int, c1: list
) -> qubitron.Circuit:
...
def _two_qubit_clifford_matrices(q_0: qubitron.Qid, q_1: qubitron.Qid, cliffords: Cliffords) -> np.ndarray:
    mats = []
...
def _random_single_q_clifford(
    qubit: qubitron.Qid, num_cfds: int, cfds: Sequence[Sequence[qubitron.ops.SingleQubitCliffordGate]]
...
def _random_two_q_clifford(
    q_0: qubitron.Qid, q_1: qubitron.Qid, num_cfds: int, cfd_matrices: np.ndarray, cliffords: Cliffords
) -> qubitron.Circuit:
    clifford_group_size = 11520
...
def _two_qubit_clifford(
    q_0: qubitron.Qid, q_1: qubitron.Qid, idx: int, cliffords: Cliffords
) -> Iterator[qubitron.OP_TREE]:
...
def _two_qubit_clifford_starters(
    q_0: qubitron.Qid, q_1: qubitron.Qid, idx_0: int, idx_1: int, cliffords: Cliffords
) -> Iterator[qubitron.OP_TREE]:
...
def _two_qubit_clifford_mixers(
    q_0: qubitron.Qid, q_1: qubitron.Qid, idx_2: int, cliffords: Cliffords
) -> Iterator[qubitron.OP_TREE]:
...
def _single_qubit_gates(gate_seq: Sequence[qubitron.Gate], qubit: qubitron.Qid) -> Iterator[qubitron.OP_TREE]:
