...
import qubitron
from qubitron.experiments.xeb_fitting import XEBPhasedFSimCharacterizationOptions
from qubitron.experiments.z_phase_calibration import (
    calibrate_z_phases,
...
class _TestSimulator(qubitron.Simulator):
    """A simulator that replaces a specific gate by another."""

    def __init__(self, gate: qubitron.Gate, replacement: qubitron.Gate, **kwargs):
        super().__init__(**kwargs)
        self.gate = gate
        self.replacement = replacement

    def _core_iterator(
        self, circuit: qubitron.AbstractCircuit, sim_state, all_measurements_are_terminal: bool = False
    ):
        new_circuit = qubitron.Circuit(
            [
...
def test_calibrate_z_phases(angles, error, characterization_flags):
    ...
    original_gate = qubitron.PhasedFSimGate(**{k: v for k, v in zip(_ANGLES, angles)})
    actual_gate = qubitron.PhasedFSimGate(**{k: v + e for k, v, e in zip(_ANGLES, angles, error)})
    ...
    qubits = qubitron.q(0, 0), qubitron.q(0, 1)
    ...
    initial_unitary = qubitron.unitary(original_gate)
    final_unitary = qubitron.unitary(calibrated_gate)
    target_unitary = qubitron.unitary(actual_gate)
...
def test_plot_z_phase_calibration_result():
    df = pd.DataFrame()
    qs = qubitron.q(0, 0), qubitron.q(0, 1), qubitron.q(0, 2)
    df.index = [qs[:2], qs[-2:]]
...
def test_transform_circuit(angles):
    ...
    old_zs = angles[2:6]
    new_zs = angles[6:]
    gate = qubitron.PhasedFSimGate.from_fsim_rz(theta, phi, old_zs[:2], old_zs[2:])
    fsim = qubitron.PhasedFSimGate.from_fsim_rz(theta, phi, new_zs[:2], new_zs[2:])
    c = qubitron.Circuit(gate(qubitron.q(0), qubitron.q(1)))
    replacement_map = {(qubitron.q(1), qubitron.q(0)): fsim}
    ...
    circuit_with_replacement_gate = qubitron.Circuit(
        op if op.gate != gate else fsim(*op.qubits) for op in new_circuit.all_operations()
    )
    np.testing.assert_allclose(qubitron.unitary(circuit_with_replacement_gate), qubitron.unitary(c))


def test_transform_circuit_invalid_gate_raises():
    with pytest.raises(ValueError, match="is not equivalent to a PhasedFSimGate"):
        _ = CalibrationTransformer(qubitron.XX, {})


def test_transform_circuit_uncalibrated_gates_pass():
    c = qubitron.Circuit(qubitron.CZ(qubitron.q(0), qubitron.q(1)), qubitron.measure(qubitron.q(0)))
    assert c == CalibrationTransformer(qubitron.CZ, {})(c)
