...
from qubitron import circuits, ops, study
...
class SingleQubitReadoutCalibrationResult:
    ...
    zero_state_errors: dict[qubitron.Qid, float]
    one_state_errors: dict[qubitron.Qid, float]
    repetitions: int
    timestamp: float
    ...
    def plot_heatmap(
        ...
        **plot_kwargs: Any,
        ...
        """Plot a heatmap of the readout errors. If qubits are not qubitron.GridQubits, throws an error.
        ...
        Raises:
            ...
            TypeError: qubits are not qubitron.GridQubits
        """
        ...
        for ax, title, data in zip(
            ...
            for qubit in data:
                if type(qubit) != grid_qubit.GridQubit:
                    raise TypeError(f'{qubit} must be of type qubitron.GridQubit')  # pragma: no cover
                data_with_grid_qubit_keys[qubit] = data[qubit]  # just for typecheck
            _, _ = qubitron_heatmap.Heatmap(data_with_grid_qubit_keys).plot(
    ...
    def plot_integrated_histogram(
        ...
        **kwargs,
        ...
        """Plot the readout errors using qubitron.integrated_histogram().
    ...
    def __repr__(self) -> str:
        return (
            'qubitron.experiments.SingleQubitReadoutCalibrationResult('
...
def estimate_single_qubit_readout_errors(
    sampler: qubitron.Sampler, *, qubits: Iterable[qubitron.Qid], repetitions: int = 1000
...
def estimate_parallel_single_qubit_readout_errors(
    ...
    *,
    ...
    Args:
        sampler: The `qubitron.Sampler` used to run the circuits.
        qubits: The qubits being tested.
