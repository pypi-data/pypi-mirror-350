...
def test_sample_2q_xeb_circuits():
    ...
    q1 = qubitron.NamedQubit('b')
    circuits = [
        rqcg.random_rotations_between_two_qubit_circuit(
            q0, q1, depth=20, two_qubit_op_factory=lambda a, b, _: qubitron.SQRT_ISWAP(a, b)
        )
        for _ in range(2)
    ]
...
def test_sample_2q_xeb_circuits_error():
    ...
    circuits = [qubitron.testing.random_circuit(qubits, n_moments=5, op_density=0.8, random_state=52)]
    cycle_depths = np.arange(3, 50, 9)
    with pytest.raises(ValueError):  # three qubit circuits
        _ = sample_2q_xeb_circuits(
            sampler=qubitron.Simulator(), circuits=circuits, cycle_depths=cycle_depths
        )


def test_sample_2q_xeb_circuits_no_progress(capsys):
    qubits = qubitron.LineQubit.range(2)
    circuits = [qubitron.testing.random_circuit(qubits, n_moments=7, op_density=0.8, random_state=52)]
    cycle_depths = np.arange(3, 4)
    _ = sample_2q_xeb_circuits(
        sampler=qubitron.Simulator(), circuits=circuits, cycle_depths=cycle_depths, progress_bar=None
    )
...
def test_sample_2q_parallel_xeb_circuits(tmpdir):
    circuits = rqcg.generate_library_of_2q_circuits(
        n_library_circuits=5, two_qubit_gate=qubitron.ISWAP**0.5, max_cycle_depth=10
    )
    cycle_depths = [5, 10]
    graph = _gridqubits_to_graph_device(qubitron.GridQubit.rect(3, 2))
    ...
    chunks = [record for fn in glob.glob(f'{tmpdir}/my_dataset/*') for record in qubitron.read_json(fn)]
