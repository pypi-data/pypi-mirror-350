...
def sample_noisy_bitstrings(
    circuit: qubitron.Circuit, qubit_order: Sequence[qubitron.Qid], depolarization: float, repetitions: int
) -> np.ndarray:
    ...
    circuit_with_measurements = qubitron.Circuit(circuit, qubitron.measure(*qubit_order, key='m'))
    r = qubitron.sample(circuit_with_measurements, repetitions=n_coherent)
    coherent_samples = r.data['m'].to_numpy()
    return np.concatenate((coherent_samples, incoherent_samples))


def make_random_quantum_circuit(qubits: Sequence[qubitron.Qid], depth: int) -> qubitron.Circuit:
    SQ_GATES = [qubitron.X**0.5, qubitron.Y**0.5, qubitron.T]
    circuit = qubitron.Circuit()
    cz_start = 0
    for q in qubits:
        circuit.append(qubitron.H(q))
...
@pytest.mark.parametrize(
    ...
    itertools.product(
        (0.0, 0.2, 0.7, 1.0),
        (
            qubitron.hog_score_xeb_fidelity_from_probabilities,
            qubitron.linear_xeb_fidelity_from_probabilities,
            qubitron.log_xeb_fidelity_from_probabilities,
        ),
    ),
)
def test_xeb_fidelity(depolarization, estimator):
    ...
    for _ in range(10):
        ...
        f = qubitron.xeb_fidelity(circuit, bitstrings, qubits, estimator=estimator)
        amplitudes = qubitron.final_state_vector(circuit)
        f2 = qubitron.xeb_fidelity(
...
def test_linear_and_log_xeb_fidelity():
    ...
    for _ in range(10):
        ...
        f_log = qubitron.log_xeb_fidelity(circuit, bitstrings, qubits)
        f_lin = qubitron.linear_xeb_fidelity(circuit, bitstrings, qubits)
...
def test_xeb_fidelity_invalid_qubits():
    ...
    with pytest.raises(ValueError):
        qubitron.xeb_fidelity(circuit, bitstrings, (q0, q2))
...
def test_xeb_fidelity_invalid_bitstrings():
    ...
    with pytest.raises(ValueError):
        qubitron.xeb_fidelity(circuit, bitstrings, (q0, q1))
...
def test_xeb_fidelity_tuple_input():
    ...
    circuit = qubitron.Circuit(qubitron.H(q0), qubitron.CNOT(q0, q1))
    bitstrings = [0, 1, 2]
    f1 = qubitron.xeb_fidelity(circuit, bitstrings, (q0, q1))
    f2 = qubitron.xeb_fidelity(circuit, tuple(bitstrings), (q0, q1))
    assert f1 == f2
