...
from qubitron.interop.quirk.cells.cell import CellMaker, ExplicitOperationsCell
...
def _measurement(identifier: str, basis_change: qubitron.Gate | None = None) -> CellMaker:
    return CellMaker(
        ...
        maker=lambda args: ExplicitOperationsCell(
            ...
            basis_change=cast(
                Iterable['qubitron.Operation'], [basis_change.on(*args.qubits)] if basis_change else ()
            ),
        ),
    )
