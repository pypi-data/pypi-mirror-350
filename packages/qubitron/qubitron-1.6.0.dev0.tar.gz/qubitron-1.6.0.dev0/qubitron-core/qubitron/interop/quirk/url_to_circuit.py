...
from qubitron import circuits, devices, ops, protocols
from qubitron.interop.quirk.cells import (
    Cell,
    CellMaker,
    CellMakerArgs,
    CompositeCell,
...
from qubitron.interop.quirk.cells.parse import parse_matrix
...
def quirk_url_to_circuit(
    ...
    *,
    ...
    extra_cell_makers: dict[str, qubitron.Gate] | Iterable[qubitron.interop.quirk.cells.CellMaker] = (),
    max_operation_count: int = 10**6,
) -> qubitron.Circuit:
    """Parses a Qubitron circuit out of a Quirk URL.
    ...
    Args:
        ...
        qubits: Qubits to use in the circuit. The length of the list must be
            ...
            This argument defaults to `qubitron.LineQubit.range(16)` when not
            specified.
        extra_cell_makers: Non-standard Quirk cells to accept. This can be
            ...
            as either a list of `qubitron.interop.quirk.cells.CellMaker` instances,
    ...
    >>> print(qubitron.quirk_url_to_circuit(
    ...
    >>> print(qubitron.quirk_url_to_circuit(
    ...     'http://algassert.com/quirk#circuit={"cols":[["H"],["•","X"]]}',
    ...     qubits=[qubitron.NamedQubit('Alice'), qubitron.NamedQubit('Bob')]
    ... ))
    Alice: ───H───@───
                  │
    Bob: ─────────X───
    ...
    ...             identifier='iswap',
    ...             size=2,
    ...             maker=lambda args: qubitron.ISWAP(*args.qubits))
    ...     ]))
    0: ───iSwap───
          │
    1: ───iSwap───
...
def quirk_json_to_circuit(
    ...
    *,
    ...
    """Constructs a Qubitron circuit from Quirk's JSON format.
    ...
    >>> print(qubitron.quirk_json_to_circuit(
    ...     {"cols":[["H"], ["•", "X"]]}
    ... ))
    0: ───H───@───
              │
    ...
    if qubits is not None:
        ...
        def map_qubit(qubit: qubitron.Qid) -> qubitron.Qid:
...
def _init_ops(data: dict[str, Any]) -> qubitron.OP_TREE:
    if 'init' not in data:
        return []
    init = data['init']
