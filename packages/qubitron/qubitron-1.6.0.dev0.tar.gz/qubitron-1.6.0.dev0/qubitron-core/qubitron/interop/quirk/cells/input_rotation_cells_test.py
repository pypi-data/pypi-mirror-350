...
def test_validation() -> None:
    with pytest.raises(ValueError, match='sign'):
        _ = qubitron.interop.quirk.QuirkInputRotationOperation(
            identifier='xxx',
            register=qubitron.LineQubit.range(4),
            base_operation=qubitron.X.on(qubitron.LineQubit(5)),
            exponent_sign=2,
        )
...
def test_input_rotation_with_qubits() -> None:
    ...
    x, y, z, t, w = qubitron.LineQubit.range(10, 15)
    op = qubitron.interop.quirk.QuirkInputRotationOperation(
        identifier='test',
        register=[a, b, c],
        base_operation=qubitron.X(d).controlled_by(e),
        exponent_sign=-1,
    )
    assert op.qubits == (e, d, a, b, c)
    assert op.with_qubits(x, y, z, t, w) == (
        qubitron.interop.quirk.QuirkInputRotationOperation(
            identifier='test',
            register=[z, t, w],
            base_operation=qubitron.X(y).controlled_by(x),
            exponent_sign=-1,
        )
    )
...
def test_input_rotation_cell_with_qubits() -> None:
    a, b, c, d, e = qubitron.LineQubit.range(5)
    x, y, z, t, w = qubitron.LineQubit.range(10, 15)
    cell = qubitron.interop.quirk.cells.input_rotation_cells.InputRotationCell(
        identifier='test',
        register=[a, b, c],
        base_operation=qubitron.X(d).controlled_by(e),
        exponent_sign=-1,
    ...
    assert cell.with_line_qubits_mapped_to([x, y, z, t, w]) == (
        qubitron.interop.quirk.cells.input_rotation_cells.InputRotationCell(
            identifier='test',
            register=[x, y, z],
            base_operation=qubitron.X(t).controlled_by(w),
            exponent_sign=-1,
        )
    )
...
def test_input_rotation_cell_with_qubits_before_register_specified() -> None:
    d, e = qubitron.LineQubit.range(3, 5)
    x, y, z, t, w = qubitron.LineQubit.range(10, 15)
    cell = qubitron.interop.quirk.cells.input_rotation_cells.InputRotationCell(
        identifier='test',
        register=None,
