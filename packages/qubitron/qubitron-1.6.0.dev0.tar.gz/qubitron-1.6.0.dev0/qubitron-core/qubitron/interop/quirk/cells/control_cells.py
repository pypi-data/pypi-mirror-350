...
class ControlCell(Cell):
    ...
    def __init__(self, qubit: qubitron.Qid, basis_change: Iterable[qubitron.Operation]):
        self.qubit = qubit
    ...
    def __repr__(self) -> str:
        return (
            f'qubitron.interop.quirk.cells.control_cells.ControlCell('
            f'\n    {self.qubit!r},'
...
class ParityControlCell(Cell):
    ...
    def __init__(self, qubits: Iterable[qubitron.Qid], basis_change: Iterable[qubitron.Operation]):
    ...
    def __repr__(self) -> str:
        return (
            f'qubitron.interop.quirk.cells.control_cells.ParityControlCell('
            f'\n    {self.qubits!r},'
            f'\n    {self._basis_change!r})'
        )
    ...
    def basis_change(self) -> Iterator[qubitron.OP_TREE]:
...
def _reg_control(identifier: str, *, basis_change: qubitron.Gate | None) -> CellMaker:
    return CellMaker(
...
def _reg_parity_control(identifier: str, *, basis_change: qubitron.Gate | None = None) -> CellMaker:
    return CellMaker(
        identifier=identifier,
        size=1,
...
def _basis_else_empty(
    basis_change: qubitron.Gate | None, qureg: qubitron.Qid | Iterable[qubitron.Qid]
