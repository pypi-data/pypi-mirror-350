...
from qubitron import ops, protocols, study, value
from qubitron.work.observable_measurement import (
    CheckpointFileOptions,
    measure_observables,
    RepetitionsStoppingCriteria,
)
from qubitron.work.observable_settings import _hashable_param
...
class Sampler(metaclass=value.ABCMetaImplementAnyOneOf):
    ...
    def run(
        ...
        Returns:
            `qubitron.Result` that contains all the measurements for a run.
        """
    ...
    async def run_async(
        ...
        Provides measurement outcomes as a `qubitron.Result` object.  This
    ...
    def sample(
        self, program: qubitron.AbstractCircuit, *, repetitions: int = 1, params: qubitron.Sweepable = None
    ) -> pd.DataFrame:
        """Samples the given Circuit, producing a pandas data frame.

        This interface will operate in a similar way to the `run` method
        except that it returns a pandas data frame rather than a `qubitron.Result`
        object.
        ...
        Args:
            ...
            params: Maps symbols to one or more values. This argument can be
                a dictionary, a list of dictionaries, a `qubitron.Sweep`, a list of
                `qubitron.Sweep`, etc. The program will be sampled `repetition`
        ...
        Returns:
            ...
            See `qubitron.big_endian_int_to_bits` and similar functions for how
        ...
        Examples:
            ...
            >>> circuit = qubitron.Circuit(qubitron.X(a),
            ...                        qubitron.CNOT(a, b),
            ...                        qubitron.measure(a, b, c, key='out'))
            >>> print(sampler.sample(circuit, repetitions=4))
               out
            0    6
            1    6
            ...
            >>> circuit = qubitron.Circuit(qubitron.X(a)**sympy.Symbol('t'),
            ...                        qubitron.measure(a, key='out'))
    ...
    async def run_batch_async(
        self,
        programs: Sequence[qubitron.AbstractCircuit],
        params_list: Sequence[qubitron.Sweepable] | None = None,
        repetitions: int | Sequence[int] = 1,
    ) -> Sequence[Sequence[qubitron.Result]]:
    ...
    def _normalize_batch_args(
        ...
        params_list: Sequence[qubitron.Sweepable] | None = None,
        repetitions: int | Sequence[int] = 1,
    ) -> tuple[Sequence[qubitron.Sweepable], Sequence[int]]:
    ...
    def sample_expectation_values(
        ...
        *,
        num_samples: int,
        params: qubitron.Sweepable = None,
        permit_terminal_measurements: bool = False,
    ) -> Sequence[Sequence[float]]:
        """Calculates estimated expectation values from samples of a circuit.

        Please see also `qubitron.work.observable_measurement.measure_observables`
        ...
        pauli_sums: list[qubitron.PauliSum] = (
        ...
        flat_pstrings: list[qubitron.PauliString] = []
        pstring_to_psum_i: dict[qubitron.PauliString, int] = {}
        ...
        flat_params: list[qubitron.ParamMappingType] = [
