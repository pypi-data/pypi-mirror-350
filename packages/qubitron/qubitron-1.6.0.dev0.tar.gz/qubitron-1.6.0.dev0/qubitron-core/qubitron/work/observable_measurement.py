...
from qubitron.work.observable_grouping import group_settings_greedy, GROUPER_T
from qubitron.work.observable_measurement_data import (
    BitstringAccumulator,
    flatten_grouped_results,
    ObservableMeasuredResult,
)
from qubitron.work.observable_settings import _MeasurementSpec, InitObsSetting, observables_to_settings

if TYPE_CHECKING:
    import qubitron
    from qubitron.value.product_state import _NamedOneQubitState
...
def _with_parameterized_layers(
    circuit: qubitron.AbstractCircuit, qubits: Sequence[qubitron.Qid], needs_init_layer: bool
) -> qubitron.Circuit:
...
_OBS_TO_PARAM_VAL: dict[tuple[qubitron.Pauli, bool], tuple[float, float]] = {
    (ops.X, False): (0, -1 / 2),
    (ops.X, True): (0, +1 / 2),
    (ops.Y, False): (1 / 2, 0),
    (ops.Y, True): (-1 / 2, 0),
...
def _pad_setting(
    ...
    pad_obs_with: qubitron.Gate = ops.Z,
) -> InitObsSetting:
...
class CheckpointFileOptions:
    ...
    def maybe_to_json(self, obj: Any):
        """Call `qubitron.to_json with `value` according to the configuration options in this class.
...
def measure_grouped_settings(
    ...
    *,
    ...
    circuit_sweep: qubitron.Sweepable = None,
...
def _get_all_qubits(
    circuit: qubitron.AbstractCircuit, observables: Iterable[qubitron.PauliString]
) -> list[qubitron.Qid]:
...
def measure_observables(
    circuit: qubitron.AbstractCircuit,
    observables: Iterable[qubitron.PauliString],
    sampler: qubitron.Simulator | qubitron.Sampler,
    stopping_criteria: StoppingCriteria,
    *,
    readout_symmetrization: bool = False,
    circuit_sweep: qubitron.Sweepable | None = None,
    ...
    Args:
        ...
        checkpoint: Options to set up optional checkpointing of intermediate data for each
            ...
            more. Load in these results with `qubitron.read_json`.
