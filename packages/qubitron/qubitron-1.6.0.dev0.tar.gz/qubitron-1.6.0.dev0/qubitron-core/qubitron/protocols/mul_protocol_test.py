...
def test_equivalent_to_builtin_mul():
    ...
    for a in test_vals:
        for b in test_vals:
            ...
            c = qubitron.mul(a, b, default=None)
            if c is None:
...
def test_symbol_special_case():
    x = sympy.Symbol('x')
    assert qubitron.mul(x, 1.0) is x
    assert qubitron.mul(1.0, x) is x
    assert str(qubitron.mul(-1.0, x)) == '-x'
    assert str(qubitron.mul(x, -1.0)) == '-x'
