...
from qubitron import devices, ops
from qubitron._doc import doc_private
from qubitron.protocols import qid_shape_protocol
...
DecomposeResult = Union[None, NotImplementedType, 'qubitron.OP_TREE']
...
class OpDecomposerWithContext(Protocol):
    def __call__(
        self, __op: qubitron.Operation, *, context: qubitron.DecompositionContext | None = None
    ) -> DecomposeResult: ...


OpDecomposer = Callable[['qubitron.Operation'], DecomposeResult] | OpDecomposerWithContext
...
class DecompositionContext:
    ...
    Args:
        qubit_manager: A `qubitron.QubitManager` instance to allocate clean / dirty ancilla qubits as
            part of the decompose protocol.
    """

    qubit_manager: qubitron.QubitManager
...
class SupportsDecompose(Protocol):
    ...
    2-qubit gates included by default in Qubitron. If a custom decomposition is not
    specified, Qubitron will decompose all operations to XPow/YPow/ZPow/CZPow/Measurement
    + Global phase gateset. However, the default decomposition in Qubitron should be a last resort
    fallback and it is recommended for consumers of decomposition to either not depend
    upon a specific target gateset, or give an `intercepting_decomposer` to `qubitron.decompose`
    that attempts to target a specific gate set.

    For example, `qubitron.TOFFOLI` has a `_decompose_` method that returns a pair
    of Hadamard gates surrounding a `qubitron.CCZ`. Although `qubitron.CCZ` is not a
    ...
    decomposing `qubitron.TOFFOLI` terminates in 1-qubit and 2-qubit operations, and
    so almost all decomposition-aware code will be able to handle `qubitron.TOFFOLI`
    instances.

    Callers are responsible for iteratively decomposing until they are given
    operations that they understand. The `qubitron.decompose` method is a simple way
    ...
    in infinite loops when calling `qubitron.decompose`.
    ...
    2-qubit operations. When this happens, `qubitron.decompose` will raise
...
class SupportsDecomposeWithQubits(Protocol):
    ...
    def _decompose_(self, qubits: tuple[qubitron.Qid, ...]) -> DecomposeResult:
        pass

    def _decompose_with_context_(
        self, qubits: tuple[qubitron.Qid, ...], *, context: DecompositionContext | None = None
    ) -> DecomposeResult:
        pass
...
class _DecomposeArgs:
    ...
    keep: Callable[[qubitron.Operation], bool] | None
    on_stuck_raise: None | Exception | Callable[[qubitron.Operation], Exception | None]
    preserve_structure: bool


def _decompose_dfs(item: Any, args: _DecomposeArgs) -> Iterator[qubitron.Operation]:
    from qubitron.circuits import CircuitOperation, FrozenCircuit
...
def decompose(
    ...
    *,
    ...
    keep: Callable[[qubitron.Operation], bool] | None = None,
    on_stuck_raise: (
        None | Exception | Callable[[qubitron.Operation], Exception | None]
    ) = _value_error_describing_bad_operation,
    preserve_structure: bool = False,
    context: DecompositionContext | None = None,
) -> list[qubitron.Operation]:
    """Recursively decomposes a value into `qubitron.Operation`s meeting a criteria.
    ...
    Args:
        ...
        intercepting_decomposer: An optional method that is called before the
            ...
            if `val` isn't a `qubitron.Operation`.
    ...
    Raises:
        TypeError:
            `val` isn't a `qubitron.Operation` and can't be decomposed even once.
...
@overload
def decompose_once(val: Any, **kwargs) -> list[qubitron.Operation]:
    pass
...
) -> TDefault | list[qubitron.Operation]:
    pass
...
def decompose_once(
    ...
    **kwargs,
    ...
    until some criteria is met (which is what `qubitron.decompose` does).
...
def _try_decompose_into_operations_and_qubits(
    val: Any,
) -> tuple[list[qubitron.Operation] | None, Sequence[qubitron.Qid], tuple[int, ...]]:
    ...
    if isinstance(val, ops.Gate):
        ...
        qubits: Sequence[qubitron.Qid] = devices.LineQid.for_qid_shape(qid_shape)
    ...
    if result is not None:
        qubit_set = set()
        qid_shape_dict: dict[qubitron.Qid, int] = defaultdict(lambda: 1)
        for op in result:
