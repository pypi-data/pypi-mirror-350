...
from qubitron import linalg, qis
...
class SupportsConsistentApplyUnitary(Protocol):
    ...
    def _apply_unitary_(self, args: ApplyUnitaryArgs) -> np.ndarray | None | NotImplementedType:
        ...
        Args:
            args: A `qubitron.ApplyUnitaryArgs` object with the `args.target_tensor`
    ...
    *,
    ...
    C. Try to use `unitary_value._decompose_()` (if `allow_decompose`).
        ...
        Case b) Method returns an OP_TREE.
            Delegate to `qubitron.apply_unitaries`.
    ...
    Args:
        ...
        args: A mutable `qubitron.ApplyUnitaryArgs` object describing the target
...
def apply_unitaries(
    ...
    Uses `qubitron.apply_unitary` on each of the unitary values, to apply them to
    ...
    for op in unitary_values:
        ...
        if result is None:
            if default is RaiseTypeErrorIfNotProvided:
                raise TypeError(
                    "qubitron.apply_unitaries failed. "
