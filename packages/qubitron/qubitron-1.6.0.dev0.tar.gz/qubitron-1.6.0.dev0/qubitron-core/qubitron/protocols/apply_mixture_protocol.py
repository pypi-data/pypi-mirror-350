...
from qubitron.protocols.apply_unitary_protocol import apply_unitary, ApplyUnitaryArgs
...
class SupportsApplyMixture(Protocol):
    ...
    def _apply_mixture_(self, args: ApplyMixtureArgs) -> np.ndarray | None | NotImplementedType:
        ...
        Args:
            args: A `qubitron.ApplyMixtureArgs` object with the `args.target_tensor`
...
def apply_mixture(
    ...
    *,
    ...
    B. Construct an ApplyUnitaryArgs object `uargs` from `args` and then
        try to use `qubitron.apply_unitary(val, uargs, None)`.
    ...
    C. Try to use `val._mixture_()`.
        ...
        3. If '_mixture_' returns a list of tuples, loop over the list and
            ...
            attempt to use `qubitron.apply_unitary(op, uargs, None)`. If this
    ...
    Args:
        ...
        args: A mutable `qubitron.ApplyMixtureArgs` object describing the target
    ...
    # Possibly use `qubitron.apply_unitary`. (STEP B)
