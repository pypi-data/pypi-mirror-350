...
from qubitron._doc import doc_private
...
class JsonResolver(Protocol):
    ...
    def __call__(self, qubitron_type: str) -> ObjectFactory | None: ...
...
def _lazy_resolver(dict_factory: Callable[[], dict[str, ObjectFactory]]) -> JsonResolver:
    ...
    def json_resolver(qubitron_type: str) -> ObjectFactory | None:
        return dict_factory().get(qubitron_type, None)
...
For more information about qubitron_type resolution during deserialization
please read the docstring for `qubitron.read_json`.

3rd party packages which extend Qubitron's JSON serialization API should
...
prepended to this list:
    ...
    MY_DEFAULT_RESOLVERS = [_resolve_my_classes] \
                           + qubitron.protocols.json.DEFAULT_RESOLVERS

    def my_read_json(file_or_fn, resolvers=None):
        if resolvers is None:
            resolvers = MY_DEFAULT_RESOLVERS
        return qubitron.read_json(file_or_fn, resolvers=resolvers)
"""
...
def _register_resolver(dict_factory: Callable[[], dict[str, ObjectFactory]]) -> None:
    ...
    Qubitron modules are the ones referred in qubitron/__init__.py. If a Qubitron module
    ...
    qubitron/__init__.py or qubitron/google/__init__.py.

    As Qubitron modules are imported by qubitron/__init__.py, they are different from
    ...
    depend on where in qubitron/__init__.py the module is imported first, as some
...
class SupportsJSON(Protocol):
    ...
    `_from_json_dict_`, that will be called. Otherwise, the `qubitron_type` key
...
class HasJSONNamespace(Protocol):
    """An object which prepends a namespace to its JSON qubitron_type.

    Classes which implement this method have the following qubitron_type format:

        f"{obj._json_namespace_()}.{obj.__class__.__name__}

    Classes outside of Qubitron or its submodules MUST implement this method to be
...
def obj_to_dict_helper(obj: Any, attribute_names: Iterable[str]) -> dict[str, Any]:
    ...
    returned dictionary has an additional key "qubitron_type" whose value
...
def dataclass_json_dict(obj: Any) -> dict[str, Any]:
    ...
    @qubitron.json_serializable_dataclass) or combining additional decorators with @dataclass.
...
def _json_dict_with_qubitron_type(obj: Any):
    base_dict = obj._json_dict_()
    if 'qubitron_type' in base_dict:
        raise ValueError(
            f"Found 'qubitron_type': '{base_dict['qubitron_type']}' in user-specified _json_dict_. "
            "'qubitron_type' is now automatically generated from the class's name and its "
            "_json_namespace_ method as `qubitron_type: '[<namespace>.]<class_name>'`."
            "\n\n"
            "Starting in v0.15, custom 'qubitron_type' values will trigger an error. "
            "To fix this, remove 'qubitron_type' from the class _json_dict_ method and "
            "define _json_namespace_ for the class."
            "\n\n"
            "For backwards compatibility, third-party classes whose old 'qubitron_type' value "
            ...
            "DEFAULT_RESOLVER docstring in qubitron-core/qubitron/protocols/json_serialization.py."
        )
    return {'qubitron_type': json_qubitron_type(type(obj)), **base_dict}


class QubitronEncoder(json.JSONEncoder):
    """Extend json.JSONEncoder to support Qubitron objects.
    ...
    protocol, this encoder deals with common, basic types:
     ...
     - Preliminary support for Sympy objects. Currently only sympy.Symbol.
       See https://github.com/amyssnippet/Qubitron/issues/2014
    """
    ...
    def default(self, o):
        ...
        if hasattr(o, '_json_dict_'):
            json_dict = _json_dict_with_qubitron_type(o)
            if isinstance(o, SerializableByKey):
                if ref := self._memo.get(o):
                    return ref
                key = len(self._memo)
                ref = {"qubitron_type": "REF", "key": key}
                self._memo[o] = ref
                return {"qubitron_type": "VAL", "key": key, "val": json_dict}
            return json_dict

        # Sympy object? (Must come before general number checks.)
        # TODO: More support for sympy
        # Github issue: https://github.com/amyssnippet/Qubitron/issues/2014

        if isinstance(o, sympy.Symbol):
            return {'qubitron_type': 'sympy.Symbol', 'name': o.name}
        ...
        if isinstance(
            ...
            return {'qubitron_type': f'sympy.{o.__class__.__name__}', 'args': o.args}

        if isinstance(o, sympy.Integer):
            return {'qubitron_type': 'sympy.Integer', 'i': o.p}

        if isinstance(o, sympy.Float):
            return {'qubitron_type': 'sympy.Float', 'approx': float(o)}

        if isinstance(o, sympy.Rational):
            return {'qubitron_type': 'sympy.Rational', 'p': o.p, 'q': o.q}
        ...
        if isinstance(o, sympy.NumberSymbol):
            ...
            if o is sympy.pi:
                return {'qubitron_type': 'sympy.pi'}
            if o is sympy.E:
                return {'qubitron_type': 'sympy.E'}
            if o is sympy.EulerGamma:
                return {'qubitron_type': 'sympy.EulerGamma'}
        ...
        if isinstance(o, numbers.Complex):
            return {'qubitron_type': 'complex', 'real': o.real, 'imag': o.imag}
        ...
        if isinstance(o, pd.MultiIndex):
            return {'qubitron_type': 'pandas.MultiIndex', 'tuples': list(o), 'names': list(o.names)}
        if isinstance(o, pd.Index):
            return {'qubitron_type': 'pandas.Index', 'data': list(o), 'name': o.name}
        if isinstance(o, pd.DataFrame):
            cols = [o[col].tolist() for col in o.columns]
            rows = list(zip(*cols))
            return {
                'qubitron_type': 'pandas.DataFrame',
                'data': rows,
                'columns': o.columns,
                'index': o.index,
            }
        ...
        if isinstance(o, datetime.datetime):
            return {'qubitron_type': 'datetime.datetime', 'timestamp': o.timestamp()}
...
class ObjectHook:
    ...
    def __call__(self, d):
        qubitron_type = d.get('qubitron_type')
        if qubitron_type is None:
            return d

        if qubitron_type == 'VAL':
            obj = d['val']
            self.memo[d['key']] = obj
            return obj

        if qubitron_type == 'REF':
            return self.memo[d['key']]

        # Deserialize from legacy "contextual serialization" format
        if qubitron_type in self.LEGACY_CONTEXT_TYPES:
            if qubitron_type == '_SerializedKey':
                return self.context_map[d['key']]
            if qubitron_type == '_SerializedContext':
                self.context_map[d['key']] = d['obj']
                return None
            if qubitron_type == '_ContextualSerialization':
                return d['object_dag'][-1]

        cls = factory_from_json(qubitron_type, resolvers=self.resolvers)
...
def json_namespace(type_obj: type) -> str:
    ...
    Qubitron type will not include a namespace in its qubitron_type. Non-Qubitron types
    must provide a namespace for serialization in Qubitron.

    Args:
        type_obj: Type to retrieve the namespace from.

    Returns:
        The namespace to prepend `type_obj` with in its JSON qubitron_type.

    Raises:
        ValueError: if `type_obj` is not a Qubitron type and does not explicitly
            define its namespace with _json_namespace_.
    """
    if hasattr(type_obj, '_json_namespace_'):
        return type_obj._json_namespace_()
    if type_obj.__module__.startswith('qubitron'):
        return ''
    raise ValueError(f'{type_obj} is not a Qubitron type, and does not define _json_namespace_.')


def json_qubitron_type(type_obj: type) -> str:
    ...
    `qubitron_type_from_json`, it can be used to provide type-object serialization
...
def factory_from_json(
    ...
    DEFAULT_RESOLVERS is updated dynamically as qubitron submodules are imported.
    ...
    for resolver in resolvers:
        qubitron_type = resolver(type_str)
        if qubitron_type is not None:
            return qubitron_type
    raise ValueError(f"Could not resolve type '{type_str}' during deserialization")


def qubitron_type_from_json(type_str: str, resolvers: Sequence[JsonResolver] | None = None) -> type:
    ...
    `json_qubitron_type`, it can be used to provide type-object deserialization
    for classes that need it.
    ...
    Returns:
        The type object T for which json_qubitron_type(T) matches `type_str`.

    Raises:
        ValueError: if type_str does not have a match in `resolvers`, or if the
            match found is a factory method instead of a type.
    """
    qubitron_type = factory_from_json(type_str, resolvers)
    if isinstance(qubitron_type, type):
        return qubitron_type
...
@overload
def to_json(
    obj: Any, file_or_fn: IO | pathlib.Path | str, *, indent=2, separators=None, cls=QubitronEncoder
) -> None:
    pass
...
@overload
def to_json(
    obj: Any, file_or_fn: None = None, *, indent=2, separators=None, cls=QubitronEncoder
) -> str:
    pass
...
def to_json(
    ...
    *,
    ...
    cls: type[json.JSONEncoder] = QubitronEncoder,
) -> str | None:
    """Write a JSON file containing a representation of obj.

    The object may be a qubitron object or have data members that are qubitron
    ...
    Args:
        ...
        cls: Passed to json.dump; the default value of QubitronEncoder
            enables the serialization of Qubitron objects which implement
...
def read_json(
    ...
    *,
    ...
    """Read a JSON file that optionally contains qubitron objects.
    ...
    Args:
        ...
        resolvers: A list of functions that are called in order to turn
            the serialized `qubitron_type` string into a constructable class.
            By default, top-level qubitron objects that implement the SupportsJSON
            ...
            to indicate that it cannot resolve the given qubitron_type and that
...
def read_json_gzip(
    ...
    *,
    ...
    """Read a gzipped JSON file that optionally contains qubitron objects.
