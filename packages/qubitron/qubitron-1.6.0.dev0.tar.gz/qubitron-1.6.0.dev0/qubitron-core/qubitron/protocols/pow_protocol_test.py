...
def test_powerless(val):
    assert qubitron.pow(val, 5, None) is None
    assert qubitron.pow(val, 2, NotImplemented) is NotImplemented

    # Don't assume X**1 == X if X doesn't define __pow__.
    assert qubitron.pow(val, 1, None) is None


def test_pow_error():
    with pytest.raises(TypeError, match="returned NotImplemented"):
        _ = qubitron.pow(ReturnsNotImplemented(), 3)
    with pytest.raises(TypeError, match="no __pow__ method"):
        _ = qubitron.pow(NoMethod(), 3)


@pytest.mark.parametrize('val,exponent,out', ((ReturnsExponent(), 2, 2), (1, 2, 1), (2, 3, 8)))
def test_pow_with_result(val, exponent, out):
    assert qubitron.pow(val, exponent) == qubitron.pow(val, exponent, default=None) == val**exponent == out
