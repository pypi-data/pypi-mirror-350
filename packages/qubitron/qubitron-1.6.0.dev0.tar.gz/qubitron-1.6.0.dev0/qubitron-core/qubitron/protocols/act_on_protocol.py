...
class SupportsActOn(Protocol):
    ...
    @doc_private
    def _act_on_(self, sim_state: qubitron.SimulationStateBase) -> NotImplementedType | bool:
        ...
        checks if `isinstance(sim_state, qubitron.StateVectorSimulationState)` and,
        if so, apply their unitary effect to the state vector.

        The global `qubitron.act_on` method looks for whether or not the given
...
def act_on(
    action: Any,
    sim_state: qubitron.SimulationStateBase,
    ...
    *,
    ...
    For example, the action may be a `qubitron.Operation` and the state argument may
