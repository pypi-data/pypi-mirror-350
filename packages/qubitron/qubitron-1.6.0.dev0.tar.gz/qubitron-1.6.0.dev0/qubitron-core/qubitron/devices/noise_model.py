...
from qubitron import ops, protocols, value
from qubitron._doc import document
from qubitron._import import LazyLoader

moment_module = LazyLoader("moment_module", globals(), "qubitron.circuits.moment")
...
class NoiseModel(metaclass=value.ABCMetaImplementAnyOneOf):
    ...
    @classmethod
    def from_noise_model_like(cls, noise: qubitron.NOISE_MODEL_LIKE) -> qubitron.NoiseModel:
        """Transforms an object into a noise model if unambiguously possible.

        Args:
            noise: `None`, a `qubitron.NoiseModel`, or a single qubit operation.

        Returns:
            `qubitron.NO_NOISE` when given `None`,
            `qubitron.ConstantQubitNoiseModel(gate)` when given a single qubit
            gate, or the given value if it is already a `qubitron.NoiseModel`.

        Raises:
            ValueError: If noise is a `qubitron.Gate` that acts on more than one
                qubit.
            TypeError: The input is not a ``qubitron.NOISE_MODE_LIKE``.
        """
        if noise is None:
            return NO_NOISE
        ...
        if isinstance(noise, ops.Gate):
            if noise.num_qubits() != 1:
                raise ValueError(
                    ...
                    'be wrapped with `qubitron.ConstantQubitNoiseModel`) or '
                    'an instance of `qubitron.NoiseModel`.'
                )
            return ConstantQubitNoiseModel(noise)

        raise TypeError(
            'Expected a NOISE_MODEL_LIKE (None, a qubitron.NoiseModel, '
            f'or a single qubit gate). Got {noise!r}'
        )

    def is_virtual_moment(self, moment: qubitron.Moment) -> bool:
        ...
        Args:
            moment: ``qubitron.Moment`` to check for non-virtual operations.
    ...
    def _noisy_moments_impl_moment(
        self, moments: Iterable[qubitron.Moment], system_qubits: Sequence[qubitron.Qid]
    ) -> Sequence[qubitron.OP_TREE]:
        result = []
    ...
    def _noisy_moment_impl_moments(
        self, moment: qubitron.Moment, system_qubits: Sequence[qubitron.Qid]
    ) -> qubitron.OP_TREE:
    ...
    def noisy_moment(self, moment: qubitron.Moment, system_qubits: Sequence[qubitron.Qid]) -> qubitron.OP_TREE:
    ...
    def _noisy_operation_impl_moments(self, operation: qubitron.Operation) -> qubitron.OP_TREE:
        return self.noisy_moments([moment_module.Moment([operation])], operation.qubits)

    def _noisy_operation_impl_moment(self, operation: qubitron.Operation) -> qubitron.OP_TREE:
    ...
    def noisy_operation(self, operation: qubitron.Operation) -> qubitron.OP_TREE:
...
class _NoNoiseModel(NoiseModel):
    ...
    def noisy_moments(self, moments: Iterable[qubitron.Moment], system_qubits: Sequence[qubitron.Qid]):
        return list(moments)

    def noisy_moment(self, moment: qubitron.Moment, system_qubits: Sequence[qubitron.Qid]) -> qubitron.Moment:
        return moment

    def noisy_operation(self, operation: qubitron.Operation) -> qubitron.Operation:
        return operation
...
class ConstantQubitNoiseModel(NoiseModel):
    ...
    def __init__(self, qubit_noise_gate: qubitron.Gate, prepend: bool = False):
    ...
    def __repr__(self) -> str:
        return f'qubitron.ConstantQubitNoiseModel({self.qubit_noise_gate!r})'
...
class GateSubstitutionNoiseModel(NoiseModel):
    def __init__(self, substitution_func: Callable[[qubitron.Operation], qubitron.Operation]):
...
NO_NOISE: qubitron.NoiseModel = _NoNoiseModel()
document(
    NO_NOISE,
...
NOISE_MODEL_LIKE = Union[None, 'qubitron.NoiseModel', 'qubitron.Gate']
document(
    NOISE_MODEL_LIKE,
    """A `qubitron.NoiseModel` or a value that can be trivially converted into one.

    `None` is a `NOISE_MODEL_LIKE`. It will be replaced by the `qubitron.NO_NOISE`
    noise model.

    A single qubit gate is a `NOISE_MODEL_LIKE`. It will be wrapped inside of a
    `qubitron.ConstantQubitNoiseModel`.
    """,
)


def validate_all_measurements(moment: qubitron.Moment) -> bool:
