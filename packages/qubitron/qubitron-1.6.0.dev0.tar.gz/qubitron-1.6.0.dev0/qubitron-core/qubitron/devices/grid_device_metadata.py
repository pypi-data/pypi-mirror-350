...
class GridDeviceMetadata(device.DeviceMetadata):
    ...
    def __init__(
        self,
        qubit_pairs: Iterable[tuple[qubitron.GridQubit, qubitron.GridQubit]],
        gateset: qubitron.Gateset,
        gate_durations: Mapping[qubitron.GateFamily, qubitron.Duration] | None = None,
        all_qubits: Iterable[qubitron.GridQubit] | None = None,
        compilation_target_gatesets: Iterable[qubitron.CompilationTargetGateset] = (),
    ):
        ...
        Args:
            qubit_pairs: Iterable of pairs of `qubitron.GridQubit`s representing
                bi-directional couplings.
            gateset: `qubitron.Gateset` indicating gates supported
                everywhere on the device.
            gate_durations: Optional dictionary of `qubitron.GateFamily`
                instances mapping to `qubitron.Duration` instances for
            ...
            compilation_target_gatesets: A collection of valid
                `qubitron.CompilationTargetGateset`s which can be used to
    ...
    @property
    def qubit_set(self) -> frozenset[qubitron.GridQubit]:
        """Returns the set of grid qubits on the device.

        Returns:
            Frozenset of qubits on device.
        """
        return cast(frozenset['qubitron.GridQubit'], super().qubit_set)

    @property
    def qubit_pairs(self) -> frozenset[frozenset[qubitron.GridQubit]]:
    ...
    @property
    def isolated_qubits(self) -> frozenset[qubitron.GridQubit]:
        """Returns the set of all isolated qubits on the device (if applicable)."""
        return self._isolated_qubits

    @property
    def gateset(self) -> qubitron.Gateset:
        """Returns the `qubitron.Gateset` of supported gates on this device."""
        return self._gateset

    @property
    def compilation_target_gatesets(self) -> tuple[qubitron.CompilationTargetGateset, ...]:
        """Returns a sequence of valid `qubitron.CompilationTargetGateset`s for this device."""
        return self._compilation_target_gatesets

    @property
    def gate_durations(self) -> Mapping[qubitron.GateFamily, qubitron.Duration] | None:
        ...
        >>> gateset = qubitron.Gateset(qubitron.ZPowGate)
        >>> durations = {qubitron.GateFamily(qubitron.ZPowGate): qubitron.Duration(nanos=1)}
        >>> grid_device_metadata = qubitron.GridDeviceMetadata((), gateset, durations)
        >>>
        >>> my_gate = qubitron.Z
    ...
    def __repr__(self) -> str:
        ...
        return (
            f'qubitron.GridDeviceMetadata({repr(qubit_pair_tuples)},'
