...
from qubitron import ops, protocols
...
class _BaseGridQid(ops.Qid):
    ...
    def is_adjacent(self, other: qubitron.Qid) -> bool:
...
class GridQid(_BaseGridQid):
    ...
    >>> qubitron.GridQid(2, 3, dimension=2) + (3, 1)
    qubitron.GridQid(5, 4, dimension=2)
    >>> qubitron.GridQid(2, 3, dimension=2) - (1, 2)
    qubitron.GridQid(1, 1, dimension=2)
    >>> qubitron.GridQid(2, 3, dimension=2) + np.array([3, 1], dtype=int)
    qubitron.GridQid(5, 4, dimension=2)
    """
    ...
    _cache = weakref.WeakValueDictionary[tuple[int, int, int], 'qubitron.GridQid']()

    def __new__(cls, row: int, col: int, *, dimension: int) -> qubitron.GridQid:
    ...
    def from_diagram(diagram: str, dimension: int) -> list[GridQid]:
        ...
        Args:
            ...
            dimension: The dimension of the qubits in the `qubitron.GridQid`s used
                in this construction.

        Returns:
            A list of `qubitron.GridQid`s corresponding to qids in the provided diagram
    ...
    def __repr__(self) -> str:
        return f"qubitron.GridQid({self._row}, {self._col}, dimension={self._dimension})"
...
class GridQubit(_BaseGridQid):
    ...
    >>> qubitron.GridQubit(2, 3) + (3, 1)
    qubitron.GridQubit(5, 4)
    >>> qubitron.GridQubit(2, 3) - (1, 2)
    qubitron.GridQubit(1, 1)
    >>> qubitron.GridQubit(2, 3,) + np.array([3, 1], dtype=int)
    qubitron.GridQubit(5, 4)
    """
    ...
    # Holds weak references so instances can still be garbage collected.
    _cache = weakref.WeakValueDictionary[tuple[int, int], 'qubitron.GridQubit']()

    def __new__(cls, row: int, col: int) -> qubitron.GridQubit:
    ...
    def __repr__(self) -> str:
        return f"qubitron.GridQubit({self._row}, {self._col})"
