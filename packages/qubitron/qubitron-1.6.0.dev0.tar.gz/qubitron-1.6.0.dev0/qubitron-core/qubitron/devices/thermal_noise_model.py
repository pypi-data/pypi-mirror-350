...
from qubitron import devices, ops, protocols, qis
...
def _as_rate_dict(
    rate_or_dict: float | dict[qubitron.Qid, float] | None, qubits: set[qubitron.Qid]
) -> dict[qubitron.Qid, float]:
...
def _validate_rates(qubits: set[qubitron.Qid], rates: dict[qubitron.Qid, np.ndarray]) -> None:
...
class ThermalNoiseModel(devices.NoiseModel):
    ...
    def __init__(
        self,
        qubits: set[qubitron.Qid],
        gate_durations_ns: dict[type, float],
        heat_rate_GHz: float | dict[qubitron.Qid, float] | None = None,
        cool_rate_GHz: float | dict[qubitron.Qid, float] | None = None,
        dephase_rate_GHz: float | dict[qubitron.Qid, float] | None = None,
        ...
        self.rate_matrix_GHz: dict[qubitron.Qid, np.ndarray] = rate_dict
    ...
    def noisy_moment(self, moment: qubitron.Moment, system_qubits: Sequence[qubitron.Qid]) -> qubitron.OP_TREE:
        ...
        noise_ops: list[qubitron.Operation] = []
