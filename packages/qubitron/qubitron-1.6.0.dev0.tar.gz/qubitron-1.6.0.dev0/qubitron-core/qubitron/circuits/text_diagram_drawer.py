...
from qubitron.circuits._block_diagram_drawer import BlockDiagramDrawer
from qubitron.circuits._box_drawing_character_data import (
    ASCII_BOX_CHARS,
    BOLD_BOX_CHARS,
    BoxDrawCharacterSet,
    DOUBLED_BOX_CHARS,
...
class TextDiagramDrawer:
    ...
    def transpose(self) -> qubitron.TextDiagramDrawer:
    ...
    def shift(self, dx: int = 0, dy: int = 0) -> qubitron.TextDiagramDrawer:
        self._transform_coordinates(lambda x, y: (x + dx, y + dy))
        return self

    def shifted(self, dx: int = 0, dy: int = 0) -> qubitron.TextDiagramDrawer:
        return self.copy().shift(dx, dy)

    def superimpose(self, other: qubitron.TextDiagramDrawer) -> qubitron.TextDiagramDrawer:
    ...
    def superimposed(self, other: qubitron.TextDiagramDrawer) -> qubitron.TextDiagramDrawer:
        return self.copy().superimpose(other)

    @classmethod
    def vstack(
        cls,
        diagrams: Sequence[qubitron.TextDiagramDrawer],
