...
from qubitron.study.resolver import ParamResolver, ParamResolverOrSimilarType
from qubitron.study.sweeps import dict_to_product_sweep, ListSweep, Points, Sweep, UnitSweep, Zip
    ...
    if isinstance(sweepable, dict):
        if any(isinstance(val, Sequence) for val in sweepable.values()):
            warnings.warn(
                ...
                'of sweeps is deprecated and will be removed in qubitron 0.10. '
                'Instead, expand the sweep explicitly using '
                '`qubitron.dict_to_product_sweep`.',
                DeprecationWarning,
                stacklevel=2,
            )
...
def to_sweep(
    ...
    """Converts the argument into a ``qubitron.Sweep``.

    Args:
        sweep_or_resolver_list: The object to try to turn into a
            ``qubitron.Sweep`` . A ``qubitron.Sweep``, a single ``qubitron.ParamResolver``,
            or a list of ``qubitron.ParamResolver`` s.
