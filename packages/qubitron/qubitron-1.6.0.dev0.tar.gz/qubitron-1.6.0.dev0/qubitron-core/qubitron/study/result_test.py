...
from qubitron.study.result import _pack_digits


def test_result_init():
    assert qubitron.ResultDict(params=qubitron.ParamResolver({}), measurements=None).repetitions == 0
    assert qubitron.ResultDict(params=qubitron.ParamResolver({}), measurements={}).repetitions == 0


def test_default_repetitions():
    class MyResult(qubitron.Result):
...
def test_repr():
    v = qubitron.ResultDict(
        params=qubitron.ParamResolver({'a': 2}), measurements={'xy': np.array([[1, 0], [0, 1]])}
    )
    qubitron.testing.assert_equivalent_repr(v)

    v = qubitron.ResultDict(
        params=qubitron.ParamResolver({'a': 2}),
...
def test_empty_measurements():
    assert qubitron.ResultDict(params=None).repetitions == 0
    assert qubitron.ResultDict(params=None, measurements={}).repetitions == 0
    assert qubitron.ResultDict(params=None, records={}).repetitions == 0
...
def test_str():
    ...
    result = qubitron.ResultDict(records={'c': np.array([[[True], [True]]])})
    assert str(result) == 'c=1\nc=1'

    result = qubitron.ResultDict(records={'c': np.array([[[True, False], [False, True]]])})
...
def test_result_equality():
    ...
    et.add_equality_group(
        qubitron.ResultDict(params=qubitron.ParamResolver({}), measurements={'a': np.array([[0]] * 5)}),
        qubitron.ResultDict(params=qubitron.ParamResolver({}), records={'a': np.array([[[0]]] * 5)}),
    )
    et.add_equality_group(
        qubitron.ResultDict(params=qubitron.ParamResolver({}), measurements={'a': np.array([[0]] * 6)})
    )
    et.add_equality_group(
        qubitron.ResultDict(params=qubitron.ParamResolver({}), measurements={'a': np.array([[1]] * 5)})
    )
    et.add_equality_group(
        qubitron.ResultDict(params=qubitron.ParamResolver({}), records={'a': np.array([[[0], [1]]] * 5)})
    )


def test_result_addition_valid():
    a = qubitron.ResultDict(
        params=qubitron.ParamResolver({'ax': 1}),
        measurements={
            'q0': np.array([[0, 1], [1, 0], [0, 1]], dtype=bool),
            'q1': np.array([[0], [0], [1]], dtype=bool),
        },
    )
    b = qubitron.ResultDict(
        params=qubitron.ParamResolver({'ax': 1}),
...
def test_result_addition_invalid():
    ...
    b = qubitron.ResultDict(
        params=qubitron.ParamResolver({'bad': 1}),
        measurements={
            'q0': np.array([[0, 1], [1, 0], [0, 1]], dtype=bool),
            'q1': np.array([[0], [0], [1]], dtype=bool),
        },
    )
    c = qubitron.ResultDict(
        params=qubitron.ParamResolver({'ax': 1}),
        measurements={
...
def test_qubit_keys_for_histogram():
    ...
    circuit = qubitron.Circuit(qubitron.measure(a, b), qubitron.X(c), qubitron.measure(c))
    results = qubitron.Simulator().run(program=circuit, repetitions=100)
...
def test_json_bit_packing_and_dtype(use_records: bool) -> None:
    ...
    params = qubitron.ParamResolver({})
    if use_records:
        bits_result = qubitron.ResultDict(params=params, records={'m': bits})
        digits_result = qubitron.ResultDict(params=params, records={'m': digits})
    else:
        bits_result = qubitron.ResultDict(params=params, measurements={'m': bits})
        digits_result = qubitron.ResultDict(params=params, measurements={'m': digits})

    bits_json = qubitron.to_json(bits_result)
    digits_json = qubitron.to_json(digits_result)

    loaded_bits_result = qubitron.read_json(json_text=bits_json)
    loaded_digits_result = qubitron.read_json(json_text=digits_json)
...
def test_json_unpack_compat():
    ...
    old_json = """
        {
            "qubitron_type": "ResultDict",
            "params": {
                "qubitron_type": "ParamResolver",
                "param_dict": []
            },
            "measurements": {
                "m": {
    ...
    """
    result = qubitron.read_json(json_text=old_json)
    assert result == qubitron.ResultDict(
        params=qubitron.ParamResolver({}),
        measurements={
            'm': np.array(
                [
