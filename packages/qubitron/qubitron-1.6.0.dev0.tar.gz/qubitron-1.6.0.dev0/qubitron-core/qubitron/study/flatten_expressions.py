...
from qubitron.study import resolver, sweepable, sweeps
...
def flatten(val: Any) -> tuple[Any, ExpressionMap]:
    ...
    `qubitron.ExpressionMap({sympy.Symbol('x')+1: sympy.Symbol('<x + 1>')})`.  This
    ...
    ...     qubitron.X(qubit) ** (a/4),
    ...     qubitron.Y(qubit) ** (1-a/2),
    ... )
    >>> print(circuit)
    0: ───X^(a/4)───Y^(1 - a/2)───

    >>> sweep = qubitron.Linspace(a, start=0, stop=3, length=4)
    >>> print(qubitron.ListSweep(sweep))
    Sweep:
    {'a': 0.0}
    {'a': 1.0}
    {'a': 2.0}
    ...
    0: ───X^(<a/4>)───Y^(<1 - a/2>)───
    >>> expr_map
    qubitron.ExpressionMap({a/4: <a/4>, 1 - a/2: <1 - a/2>})
    ...
    ...     print(c_flat, '=>', end=' ')
    ...     print(qubitron.resolve_parameters(c_flat, params))
    0: ───X^(<a/4>)───Y^(<1 - a/2>)─── => 0: ───X^0───Y───
    0: ───X^(<a/4>)───Y^(<1 - a/2>)─── => 0: ───X^0.25───Y^0.5───
    0: ───X^(<a/4>)───Y^(<1 - a/2>)─── => 0: ───X^0.5───Y^0───
    0: ───X^(<a/4>)───Y^(<1 - a/2>)─── => 0: ───X^0.75───Y^-0.5───
...
class _ParamFlattener(resolver.ParamResolver):
    ...
    when it is used to resolve parameters with `qubitron.resolve_parameters` or
    ...
    def value_of(
        self, value: qubitron.TParamKey | qubitron.TParamValComplex, recursive: bool = False
    ) -> qubitron.TParamValComplex:
...
class ExpressionMap(dict):
    ...
    This is returned by `qubitron.flatten`.  See `ExpressionMap.transform_sweep` and
    ...
    def transform_sweep(self, sweep: sweeps.Sweep | list[resolver.ParamResolver]) -> sweeps.Sweep:
        ...
        See `qubitron.flatten` for an example.
    ...
    def __repr__(self) -> str:
        ...
        return f'qubitron.ExpressionMap({super_repr})'


def _ensure_not_str(
    param: sympy.Expr | qubitron.TParamValComplex | str,
) -> sympy.Expr | qubitron.TParamValComplex:
