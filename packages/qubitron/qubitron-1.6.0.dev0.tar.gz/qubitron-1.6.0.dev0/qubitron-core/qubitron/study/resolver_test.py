...
def test_value_of_strings() -> None:
    assert qubitron.ParamResolver().value_of('x') == sympy.Symbol('x')


def test_value_of_calculations() -> None:
    assert not bool(qubitron.ParamResolver())

    r = qubitron.ParamResolver({'a': 0.5, 'b': 0.1, 'c': 1 + 1j})
    assert bool(r)
...
def test_resolve_integer_division() -> None:
    r = qubitron.ParamResolver({'a': 1, 'b': 2})
...
def test_resolve_symbol_division() -> None:
    B = sympy.Symbol('B')
    r = qubitron.ParamResolver({'a': 1, 'b': B})
    resolved = r.value_of(sympy.Symbol('a') / sympy.Symbol('b'))
    assert resolved == sympy.core.power.Pow(B, -1)


def test_param_dict() -> None:
    r = qubitron.ParamResolver({'a': 0.5, 'b': 0.1})
    r2 = qubitron.ParamResolver(r)
    assert r2 is r
...
def test_formulas_in_param_dict() -> None:
    ...
    with pytest.raises(TypeError, match='formula'):
        _ = qubitron.ParamResolver({a: b + 1, b: 2, b + c: 101, 'd': 2 * e})
...
def test_recursive_evaluation() -> None:
    ...
    d = sympy.Symbol('d')
    e = sympy.Symbol('e')
    r = qubitron.ParamResolver({a: a, b: e + 2, c: b + d, d: a + 3, e: 0})
...
def test_resolution_of_unknown_formulas() -> None:
    ...
    r = qubitron.ParamResolver({a: b - 2})
...
def test_unbound_recursion_halted() -> None:
    ...
    r = qubitron.ParamResolver({a: b, b: a})
    ...
    r = qubitron.ParamResolver({a: a})
    assert r.value_of(a) == a

    r = qubitron.ParamResolver({a: a + 1})
    ...
    r = qubitron.ParamResolver({a: b, b: c, c: b})
    with pytest.raises(RecursionError):
        _ = r.value_of(a)

    r = qubitron.ParamResolver({a: b + c, b: 1, c: a})
    with pytest.raises(RecursionError):
        _ = r.value_of(a)
...
def test_resolve_unknown_type() -> None:
    a = sympy.Symbol('a')
    b = sympy.Symbol('b')
    r = qubitron.ParamResolver({a: b})
    assert r.value_of(qubitron.X) == qubitron.X
...
def test_custom_resolved_value() -> None:
    ...
    a = sympy.Symbol('a')
    b = sympy.Symbol('c')
    r = qubitron.ParamResolver({a: foo, b: baz})
...
def test_custom_value_not_implemented() -> None:
    ...
    for cls in [BarImplicit, BarExplicit]:
        b = sympy.Symbol('b')
        bar = cls()
        r = qubitron.ParamResolver({b: bar})
        assert r.value_of(b) == b


def test_compose() -> None:
    """Tests that qubitron.resolve_parameters on a ParamResolver composes."""
    a = sympy.Symbol('a')
    b = sympy.Symbol('b')
    c = sympy.Symbol('c')
    d = sympy.Symbol('d')
    r1 = qubitron.ParamResolver({a: b})
    r2 = qubitron.ParamResolver({b: c + d})
    r3 = qubitron.ParamResolver({c: 12})

    r12 = qubitron.resolve_parameters(r1, r2)
    assert r12.value_of('a') == c + d

    r23 = qubitron.resolve_parameters(r2, r3)
    assert sympy.Eq(r23.value_of('b'), 12 + d)

    r123 = qubitron.resolve_parameters(r12, r3)
    assert sympy.Eq(r123.value_of('a'), 12 + d)

    r13 = qubitron.resolve_parameters(r1, r3)
...
def test_equals() -> None:
    ...
    et.add_equality_group(
        qubitron.ParamResolver(),
        qubitron.ParamResolver(None),
        qubitron.ParamResolver({}),
        qubitron.ParamResolver(qubitron.ParamResolver({})),
    )
    et.make_equality_group(lambda: qubitron.ParamResolver({'a': 0.0}))
    et.add_equality_group(qubitron.ParamResolver({'a': 0.1}))
    et.add_equality_group(qubitron.ParamResolver({'a': 0.0, 'b': 0.1}))
    et.add_equality_group(qubitron.ParamResolver({'a': 0.3, 'b': 0.1}))
    et.add_equality_group(qubitron.ParamResolver({'b': 0.1}))
    et.add_equality_group(qubitron.ParamResolver({'c': 0.1}))


def test_repr() -> None:
    qubitron.testing.assert_equivalent_repr(qubitron.ParamResolver())
    qubitron.testing.assert_equivalent_repr(qubitron.ParamResolver({'a': 2.0}))
    qubitron.testing.assert_equivalent_repr(qubitron.ParamResolver({'a': sympy.Symbol('a')}))
    qubitron.testing.assert_equivalent_repr(
        qubitron.ParamResolver({sympy.Symbol('a'): sympy.Symbol('b') + 1})
    )
