...
from qubitron.ops import control_values as cv, controlled_gate, raw_types


@value.value_equality(approximate=True)
class GlobalPhaseGate(raw_types.Gate):
    def __init__(self, coefficient: qubitron.TParamValComplex, atol: float = 1e-8) -> None:
    ...
    def __pow__(self, power) -> qubitron.GlobalPhaseGate:
    ...
    def _apply_unitary_(self, args: qubitron.ApplyUnitaryArgs) -> np.ndarray | NotImplementedType:
        if not self._has_unitary_():
            return NotImplemented
        assert not qubitron.is_parameterized(self)
    ...
    def __repr__(self) -> str:
        return f'qubitron.GlobalPhaseGate({proper_repr(self.coefficient)})'

    def _op_repr_(self, qubits: Sequence[qubitron.Qid]) -> str:
        return f'qubitron.global_phase_operation({proper_repr(self.coefficient)})'
    ...
    def controlled(
        ...
        if (
            ...
            return qubitron.ZPowGate(exponent=exponent).controlled(
...
def global_phase_operation(
    coefficient: qubitron.TParamValComplex, atol: float = 1e-8
