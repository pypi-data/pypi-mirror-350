...
from qubitron import _import, linalg, protocols
from qubitron._compat import proper_repr
from qubitron.ops import global_phase_op, identity, phased_x_z_gate, raw_types

if TYPE_CHECKING:
    import qubitron

single_qubit_decompositions = _import.LazyLoader(
    'single_qubit_decompositions', globals(), 'qubitron.transformers.analytical_decompositions'
)
two_qubit_to_cz = _import.LazyLoader(
    'two_qubit_to_cz', globals(), 'qubitron.transformers.analytical_decompositions'
)
three_qubit_decomposition = _import.LazyLoader(
    'three_qubit_decomposition', globals(), 'qubitron.transformers.analytical_decompositions'
)
...
class MatrixGate(raw_types.Gate):
    ...
    For example `qubitron.MatrixGate(np.array([[0, 1j], [1, 0]]))` has the unitary matrix:
        ...
        *,
        ...
        Args:
            ...
            unitary_check_rtol: The relative tolerance for checking whether the supplied matrix
                is unitary. See `qubitron.is_unitary`.
    ...
    def _decompose_(self, qubits: tuple[qubitron.Qid, ...]) -> qubitron.OP_TREE:
        from qubitron.circuits import Circuit

        decomposed: list[qubitron.Operation] = NotImplemented
    ...
    def __repr__(self) -> str:
        if all(e == 2 for e in self._qid_shape):
            return f'qubitron.MatrixGate({proper_repr(self._matrix)})'
        return f'qubitron.MatrixGate({proper_repr(self._matrix)}, qid_shape={self._qid_shape})'
