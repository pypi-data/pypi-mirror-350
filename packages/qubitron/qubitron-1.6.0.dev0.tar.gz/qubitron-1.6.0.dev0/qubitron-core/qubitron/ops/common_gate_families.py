...
"""Common Gate Families used in qubitron-core"""
...
from qubitron.ops import eigen_gate, gateset, parallel_gate, raw_types
...
class AnyUnitaryGateFamily(gateset.GateFamily):
    ...
    def __repr__(self) -> str:
        return f'qubitron.AnyUnitaryGateFamily(num_qubits = {self._num_qubits})'
...
class AnyIntegerPowerGateFamily(gateset.GateFamily):
    """GateFamily which accepts instances of a given `qubitron.EigenGate`, raised to integer power."""

    def __init__(self, gate: type[eigen_gate.EigenGate]) -> None:
        """Init AnyIntegerPowerGateFamily

        Args:
            gate: A subclass of `qubitron.EigenGate` s.t. an instance `g` of `gate` will be
                accepted if `g.exponent` is an integer.

        Raises:
            ValueError: If `gate` is not a subclass of `qubitron.EigenGate`.
        """
        if not (isinstance(gate, type) and issubclass(gate, eigen_gate.EigenGate)):
            raise ValueError(f'{gate} must be a subclass of `qubitron.EigenGate`.')
        super().__init__(
            gate,
    ...
    def __repr__(self) -> str:
        return f'qubitron.AnyIntegerPowerGateFamily({self._gate_str()})'
...
class ParallelGateFamily(gateset.GateFamily):
    """GateFamily which accepts instances of `qubitron.ParallelGate` and its sub_gate.

    ParallelGateFamily is useful for description and validation of scenarios where multiple
    copies of a unitary gate can act in parallel. `qubitron.ParallelGate` is used to express
    ...
    *    Gate Types whose instances can be applied in parallel.
    ...
    To verify containment of a given `qubitron.Gate` instance `g`, the gate family verfies that:

    *    `qubitron.num_qubits(g)` <= `max_parallel_allowed` if `max_parallel_allowed` is not None.
    *    `g` or `g.sub_gate` (if `g` is an instance of `qubitron.ParallelGate`) is an accepted gate
        ...
        *,
        ...
        Args:
            gate: The gate which can act in parallel. It can be a python `type` inheriting from
                `qubitron.Gate` or a non-parameterized instance of a `qubitron.Gate`. If an instance of
                `qubitron.ParallelGate` is passed, then the corresponding `gate.sub_gate` is used.
    ...
    def _default_description(self) -> str:
        ...
        return (
            f'Accepts\n'
            f'1. `qubitron.Gate` instances `g` s.t. `{check_type.format(self._gate_str())}` OR\n'
            f'2. `qubitron.ParallelGate` instance `g` s.t. `g.sub_gate` satisfies 1. and '
            f'`qubitron.num_qubits(g) <= {self._max_parallel_str()}` OR\n'
            f'3. `qubitron.Operation` instance `op` s.t. `op.gate` satisfies 1. or 2.'
        )
