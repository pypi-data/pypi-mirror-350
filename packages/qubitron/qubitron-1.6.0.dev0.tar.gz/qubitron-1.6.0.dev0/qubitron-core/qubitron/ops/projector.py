...
class ProjectorString:
    """Mapping of `qubitron.Qid` to measurement values (with a coefficient) representing a projector."""

    def __init__(self, projector_dict: dict[qubitron.Qid, int], coefficient: complex = 1):
        """Constructor for ProjectorString

        Args:
            projector_dict: A python dictionary mapping from qubitron.Qid to integers. A key value pair
    ...
    @property
    def projector_dict(self) -> dict[qubitron.Qid, int]:
    ...
    def matrix(self, projector_qids: Iterable[qubitron.Qid] | None = None) -> csr_matrix:
    ...
    def _get_idx_to_keep(self, qid_map: Mapping[qubitron.Qid, int]):
    ...
    def expectation_from_state_vector(
        self, state_vector: np.ndarray, qid_map: Mapping[qubitron.Qid, int]
    ) -> complex:
    ...
    def expectation_from_density_matrix(
        self, state: np.ndarray, qid_map: Mapping[qubitron.Qid, int]
    ) -> complex:
    ...
    def __repr__(self) -> str:
        return (
            f"qubitron.ProjectorString(projector_dict={self._projector_dict},"
