...
from qubitron import _import, protocols, value
...
controlled_gate_decomposition = _import.LazyLoader(
    'controlled_gate_decomposition', globals(), 'qubitron.transformers.analytical_decompositions'
)
common_gates = _import.LazyLoader('common_gates', globals(), 'qubitron.ops')
line_qubit = _import.LazyLoader('line_qubit', globals(), 'qubitron.devices')
...
class ControlledGate(raw_types.Gate):
    ...
    def __init__(
        ...
        if num_controls != protocols.num_qubits(control_values):
            raise ValueError('qubitron.num_qubits(control_values) != num_controls')
    ...
    @property
    def sub_gate(self) -> qubitron.Gate:
    ...
    def _decompose_(self, qubits: tuple[qubitron.Qid, ...]) -> None | NotImplementedType | qubitron.OP_TREE:
        return self._decompose_with_context_(qubits)

    def _decompose_with_context_(
        self, qubits: tuple[qubitron.Qid, ...], context: qubitron.DecompositionContext | None = None
    ) -> None | NotImplementedType | qubitron.OP_TREE:
        ...
        if (
            ...
            invert_ops: list[qubitron.Operation] = []
    ...
    def on(self, *qubits: qubitron.Qid) -> cop.ControlledOperation:
        if len(qubits) == 0:
    ...
    def _resolve_parameters_(self, resolver: qubitron.ParamResolver, recursive: bool) -> ControlledGate:
    ...
    def __repr__(self) -> str:
        if self.control_qid_shape == (2,) and self.control_values.is_trivial:
            return f'qubitron.ControlledGate(sub_gate={self.sub_gate!r})'

        if self.control_values.is_trivial and set(self.control_qid_shape) == {2}:
            return (
                f'qubitron.ControlledGate(sub_gate={self.sub_gate!r}, '
...
def _validate_sub_object(sub_object: qubitron.Gate | qubitron.Operation):
