...
from qubitron.linalg import transformations
from qubitron.ops import common_gates, identity, pauli_gates, raw_types
...
class AsymmetricDepolarizingChannel(raw_types.Gate):
    ...
    Note: prior to Qubitron v0.14, this class contained `num_qubits` as a property.
    This violates the contract of `qubitron.Gate` so it was removed.  One can
    ...
    def __repr__(self) -> str:
        return 'qubitron.asymmetric_depolarize(' + f"error_probabilities={self._error_probabilities})"
...
class DepolarizingChannel(raw_types.Gate):
    ...
    def __repr__(self) -> str:
        if self._n_qubits == 1:
            return f"qubitron.depolarize(p={self._p})"
        return f"qubitron.depolarize(p={self._p},n_qubits={self._n_qubits})"
...
class GeneralizedAmplitudeDampingChannel(raw_types.Gate):
    ...
    def __repr__(self) -> str:
        return f'qubitron.generalized_amplitude_damp(p={self._p!r},gamma={self._gamma!r})'
...
class AmplitudeDampingChannel(raw_types.Gate):
    ...
    def __repr__(self) -> str:
        return f'qubitron.amplitude_damp(gamma={self._gamma!r})'
...
class ResetChannel(raw_types.Gate):
    ...
    def __repr__(self) -> str:
        if self._dimension == 2:
            return 'qubitron.ResetChannel()'
        else:
            return f'qubitron.ResetChannel(dimension={self._dimension!r})'
...
def reset(qubit: qubitron.Qid) -> raw_types.Operation:
    """Returns a `qubitron.ResetChannel` on the given qubit.

    This can also be used with the alias `qubitron.R`.
    """
...
def reset_each(*qubits: qubitron.Qid) -> list[raw_types.Operation]:
    """Returns a list of `qubitron.ResetChannel` instances on the given qubits."""
...
class PhaseDampingChannel(raw_types.Gate):
    ...
    def __repr__(self) -> str:
        return f'qubitron.phase_damp(gamma={self._gamma!r})'
...
class PhaseFlipChannel(raw_types.Gate):
    ...
    def __repr__(self) -> str:
        return f'qubitron.phase_flip(p={self._p!r})'
...
def _phase_flip_Z() -> common_gates.ZPowGate:
    """Returns a qubitron.Z which corresponds to a guaranteed phase flip."""
...
class BitFlipChannel(raw_types.Gate):
    ...
    def __repr__(self) -> str:
        return f'qubitron.bit_flip(p={self._p!r})'
