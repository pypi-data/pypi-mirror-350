...
from qubitron.ops import gate_features, raw_types


def _canonicalize(value: qubitron.TParamVal) -> qubitron.TParamVal:
...
def _zero_mod_pi(param: qubitron.TParamVal) -> bool:
    """Returns True iff param, assumed to be in [-pi, pi), is 0 (mod pi)."""
    return param in (0.0, -np.pi, -sympy.pi)


def _half_pi_mod_pi(param: qubitron.TParamVal) -> bool:
...
class FSimGate(gate_features.InterchangeableQubitsGate, raw_types.Gate):
    ...
    def __init__(self, theta: qubitron.TParamVal, phi: qubitron.TParamVal) -> None:
        """Inits FSimGate.
    ...
    @property
    def theta(self) -> qubitron.TParamVal:
        return self._theta

    @property
    def phi(self) -> qubitron.TParamVal:
        return self._phi
    ...
    def _is_parameterized_(self) -> bool:
        return qubitron.is_parameterized(self.theta) or qubitron.is_parameterized(self.phi)

    def _parameter_names_(self) -> AbstractSet[str]:
        return qubitron.parameter_names(self.theta) | qubitron.parameter_names(self.phi)
    ...
    def _resolve_parameters_(self, resolver: qubitron.ParamResolver, recursive: bool) -> qubitron.FSimGate:
        return FSimGate(
            protocols.resolve_parameters(self.theta, resolver, recursive),
            protocols.resolve_parameters(self.phi, resolver, recursive),
        )

    def _apply_unitary_(self, args: qubitron.ApplyUnitaryArgs) -> np.ndarray | None:
        if qubitron.is_parameterized(self):
            return None
        if self.theta != 0:
            inner_matrix = protocols.unitary(qubitron.rx(2 * self.theta))
            oi = args.subspace_index(0b01)
            io = args.subspace_index(0b10)
            out = qubitron.apply_matrix_to_slices(
    ...
    def _decompose_(self, qubits) -> Iterator[qubitron.OP_TREE]:
        a, b = qubits
        xx = qubitron.XXPowGate(exponent=self.theta / np.pi, global_shift=-0.5)
        yy = qubitron.YYPowGate(exponent=self.theta / np.pi, global_shift=-0.5)
        yield xx(a, b)
        yield yy(a, b)
        yield qubitron.CZ(a, b) ** (-self.phi / np.pi)
    ...
    def __pow__(self, power) -> FSimGate:
        return FSimGate(qubitron.mul(self.theta, power), qubitron.mul(self.phi, power))

    def __repr__(self) -> str:
        t = proper_repr(self.theta)
        p = proper_repr(self.phi)
        return f'qubitron.FSimGate(theta={t}, phi={p})'
...
class PhasedFSimGate(gate_features.InterchangeableQubitsGate, raw_types.Gate):
    ...
    Indeed, for any angle δ
        ...
        qubitron.PhasedFSimGate.from_fsim_rz(θ, φ, (α0, α1), (β0, β1))

    and

        qubitron.PhasedFSimGate.from_fsim_rz(θ, φ,
                                         (α0 + δ, α1 + δ),
                                         (β0 - δ, β1 - δ))
    ...
    def __init__(
        self,
        theta: qubitron.TParamVal,
        zeta: qubitron.TParamVal = 0.0,
        chi: qubitron.TParamVal = 0.0,
        gamma: qubitron.TParamVal = 0.0,
        phi: qubitron.TParamVal = 0.0,
    ) -> None:
    ...
    @property
    def zeta(self) -> qubitron.TParamVal:
        return self._zeta

    @property
    def chi(self) -> qubitron.TParamVal:
        return self._chi

    @property
    def gamma(self) -> qubitron.TParamVal:
        return self._gamma
    ...
    def from_fsim_rz(
        theta: qubitron.TParamVal,
        phi: qubitron.TParamVal,
        rz_angles_before: tuple[qubitron.TParamVal, qubitron.TParamVal],
        rz_angles_after: tuple[qubitron.TParamVal, qubitron.TParamVal],
    ) -> PhasedFSimGate:
    ...
    @property
    def rz_angles_before(self) -> tuple[qubitron.TParamVal, qubitron.TParamVal]:
    ...
    @property
    def rz_angles_after(self) -> tuple[qubitron.TParamVal, qubitron.TParamVal]:
    ...
    def _is_parameterized_(self) -> bool:
        return (
            qubitron.is_parameterized(self.theta)
            or qubitron.is_parameterized(self.zeta)
            or qubitron.is_parameterized(self.chi)
            or qubitron.is_parameterized(self.gamma)
            or qubitron.is_parameterized(self.phi)
        )
    ...
    def _apply_unitary_(self, args: qubitron.ApplyUnitaryArgs) -> np.ndarray | None:
        ...
        if self.theta != 0 or self.zeta != 0 or self.chi != 0:
            rx = protocols.unitary(qubitron.rx(2 * self.theta))
            rz1 = protocols.unitary(qubitron.rz(-self.zeta + self.chi))
            rz2 = protocols.unitary(qubitron.rz(-self.zeta - self.chi))
    ...
    def _decompose_(self, qubits) -> Iterator[qubitron.OP_TREE]:
        ...
        def to_exponent(angle_rads: qubitron.TParamVal) -> qubitron.TParamVal:
        ...
        yield qubitron.Z(q0) ** to_exponent(before[0])
        yield qubitron.Z(q1) ** to_exponent(before[1])
        yield FSimGate(self.theta, self.phi).on(q0, q1)
        yield qubitron.Z(q0) ** to_exponent(after[0])
        yield qubitron.Z(q1) ** to_exponent(after[1])
    ...
    def __repr__(self) -> str:
        ...
        return (
            f'qubitron.PhasedFSimGate(theta={theta}, zeta={zeta}, chi={chi}, '
