...
import qubitron
from qubitron import _compat, value
...
class QuantumFourierTransformGate(raw_types.Gate):
    ...
    def __repr__(self) -> str:
        return (
            'qubitron.QuantumFourierTransformGate('
...
class PhaseGradientGate(raw_types.Gate):
    ...
    The gate `qubitron.PhaseGradientGate(n, t)` has the unitary
    $$
    ...
    def _decompose_(self, qubits):
        for i, q in enumerate(qubits):
            yield qubitron.Z(q) ** (self.exponent / 2**i)
    ...
    def __pow__(self, power):
        new_exponent = qubitron.mul(self.exponent, power, NotImplemented)
    ...
    def _is_parameterized_(self) -> bool:
        return qubitron.is_parameterized(self.exponent)

    def _parameter_names_(self) -> AbstractSet[str]:
        return qubitron.parameter_names(self.exponent)

    def _resolve_parameters_(
        self, resolver: qubitron.ParamResolver, recursive: bool
    ) -> PhaseGradientGate:
        new_exponent = qubitron.resolve_parameters(self.exponent, resolver, recursive)
        return PhaseGradientGate(num_qubits=self._num_qubits, exponent=new_exponent)
...
def qft(*qubits: qubitron.Qid, without_reverse: bool = False, inverse: bool = False) -> qubitron.Operation:
    """The quantum Fourier transform.

    Transforms a qubit register from the computational basis to the frequency
    basis.

    The inverse quantum Fourier transform is `qubitron.qft(*qubits)**-1` or
    equivalently `qubitron.inverse(qubitron.qft(*qubits))`.
    ...
    Args:
        *qubits: The qubits to apply the qft to.
        ...
        inverse: If set, the inverse qft is performed instead of the qft.
            Equivalent to calling `qubitron.inverse` on the result, or raising it
            to the -1.

    Returns:
        A `qubitron.Operation` applying the qft to the given qubits.
    """
    result = QuantumFourierTransformGate(len(qubits), without_reverse=without_reverse).on(*qubits)
    if inverse:
        result = qubitron.inverse(result)
    return result
