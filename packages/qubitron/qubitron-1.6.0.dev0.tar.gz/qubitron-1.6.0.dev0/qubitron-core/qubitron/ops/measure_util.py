...
from qubitron.ops import pauli_string, raw_types
from qubitron.ops.measurement_gate import MeasurementGate
from qubitron.ops.pauli_measurement_gate import PauliMeasurementGate
...
def measure_single_paulistring(
    pauli_observable: pauli_string.PauliString, key: str | qubitron.MeasurementKey | None = None
) -> raw_types.Operation:
    """Returns a single PauliMeasurementGate which measures the pauli observable

    Args:
        pauli_observable: The `qubitron.PauliString` observable to measure.
        key: Optional `str` or `qubitron.MeasurementKey` that gate should use.
    ...
    if not isinstance(pauli_observable, pauli_string.PauliString):
        raise ValueError(
            f'Pauli observable {pauli_observable} should be an instance of qubitron.PauliString.'
        )
...
def measure_paulistring_terms(
    ...
    Args:
        pauli_basis: The `qubitron.PauliString` basis in which each qubit should
            be measured.
    ...
    Raises:
        ValueError: if `pauli_basis` is not an instance of `qubitron.PauliString`.
    """
    if not isinstance(pauli_basis, pauli_string.PauliString):
        raise ValueError(
            f'Pauli observable {pauli_basis} should be an instance of qubitron.PauliString.'
        )
...
def measure(
    *target: raw_types.Qid,
    key: str | qubitron.MeasurementKey | None = None,
...
def measure(
    *target,
    ...
    used with the alias `qubitron.M`.
