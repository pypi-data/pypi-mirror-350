...
import qubitron
import qubitron.ops.boolean_hamiltonian as bh
...
def test_circuit(boolean_str):
    ...
    var_names = qubitron.parameter_names(boolean_expr)
    qubits = [qubitron.NamedQubit(name) for name in var_names]
    ...
    circuit.append(qubitron.H.on_each(*qubits))

    hamiltonian_gate = qubitron.BooleanHamiltonianGate(
    ...
    phi = qubitron.Simulator().simulate(circuit, qubit_order=qubits, initial_state=0).state_vector()
...
def test_gate_with_custom_names():
    ...
    gate = qubitron.BooleanHamiltonianGate(['a', 'b'], ['a'], 0.1)
    assert qubitron.decompose(gate.on(q0, q1)) == [qubitron.Rz(rads=-0.05).on(q0)]
    assert qubitron.decompose_once_with_qubits(gate, (q0, q1)) == [qubitron.Rz(rads=-0.05).on(q0)]
    assert qubitron.decompose(gate.on(q2, q3)) == [qubitron.Rz(rads=-0.05).on(q2)]
    assert qubitron.decompose_once_with_qubits(gate, (q2, q3)) == [qubitron.Rz(rads=-0.05).on(q2)]

    with pytest.raises(ValueError, match='Wrong number of qubits'):
        gate.on(q2)
    with pytest.raises(ValueError, match='Wrong shape of qids'):
        gate.on(q0, qubitron.LineQid(1, 3))


def test_gate_consistent():
    gate = qubitron.BooleanHamiltonianGate(['a', 'b'], ['a'], 0.1)
    op = gate.on(*qubitron.LineQubit.range(2))
...
def test_simplify_cnots_triplets(
    ...
    qubits = {qubit_id: qubitron.NamedQubit(f"{qubit_id}") for qubit_id in qubit_ids}

    target, control = (0, 1) if input_flip_control_and_target else (1, 0)

    circuit_input = qubitron.Circuit()
    for input_cnot in input_cnots:
        circuit_input.append(qubitron.CNOT(qubits[input_cnot[target]], qubits[input_cnot[control]]))
    circuit_actual = qubitron.Circuit()
    for actual_cnot in actual_output_cnots:
        circuit_actual.append(qubitron.CNOT(qubits[actual_cnot[target]], qubits[actual_cnot[control]]))

    np.testing.assert_allclose(qubitron.unitary(circuit_input), qubitron.unitary(circuit_actual), atol=1e-6)
