...
def test_projector_matrix():
    ...
    zero_projector = qubitron.ProjectorString({q0: 0})
    one_projector = qubitron.ProjectorString({q0: 1})
    coeff_projector = qubitron.ProjectorString({q0: 0}, 1.23 + 4.56j)
...
def test_projector_repr():
    ...
    assert (
        repr(qubitron.ProjectorString({q0: 0}))
        == "qubitron.ProjectorString(projector_dict={qubitron.NamedQubit('q0'): 0},coefficient=(1+0j))"
    )
...
def test_projector_matrix_missing_qid():
    ...
    proj = qubitron.ProjectorString({q0: 0})
    proj_with_coefficient = qubitron.ProjectorString({q0: 0}, 1.23 + 4.56j)
...
def test_equality():
    ...
    obj1a = qubitron.ProjectorString({q0: 0})
    obj1b = qubitron.ProjectorString({q0: 0})
    obj2 = qubitron.ProjectorString({q0: 1})
    obj3 = qubitron.ProjectorString({q0: 1}, coefficient=0.20160913)
...
def test_get_values():
    q0 = qubitron.NamedQubit('q0')
    d = qubitron.ProjectorString({q0: 0}, 1.23 + 4.56j)
...
def test_expectation_from_state_vector_basis_states_single_qubits():
    q0 = qubitron.NamedQubit('q0')
    d = qubitron.ProjectorString({q0: 0})
...
def test_expectation_from_state_vector_basis_states_three_qubits():
    ...
    d_1qbit = qubitron.ProjectorString({q1: 1})
    d_2qbits = qubitron.ProjectorString({q0: 0, q1: 1})

    state_vector = qubitron.testing.random_superposition(8)
...
def test_expectation_from_density_matrix_three_qubits():
    ...
    state = qubitron.testing.random_density_matrix(8)
...
def test_consistency_state_vector_and_density_matrix():
    ...
    for proj_qubit in q0, q1, q2:
        for proj_idx in [0, 1]:
            d = qubitron.ProjectorString({proj_qubit: proj_idx})
...
def test_expectation_higher_dims():
    qubit = qubitron.NamedQid('q0', dimension=2)
    qutrit = qubitron.NamedQid('q1', dimension=3)

    with pytest.raises(ValueError, match="Only qubits are supported"):
        qubitron.ProjectorString({qutrit: 0})

    d = qubitron.ProjectorString({qubit: 0})
    with pytest.raises(ValueError, match="Only qubits are supported"):
...
def test_expectation_with_coefficient():
    q0 = qubitron.NamedQubit('q0')
    d = qubitron.ProjectorString({q0: 0}, coefficient=(0.6 + 0.4j))
