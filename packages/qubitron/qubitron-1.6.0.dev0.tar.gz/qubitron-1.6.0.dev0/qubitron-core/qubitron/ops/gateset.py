...
"""Functionality for grouping and validating Qubitron gates."""
...
from qubitron.ops import global_phase_op, op_tree, raw_types
...
def _gate_str(
    gate: raw_types.Gate | type[raw_types.Gate] | qubitron.GateFamily, gettr: Callable[[Any], str] = str
) -> str:
...
class GateFamily:
    ...
    - Non-parameterized instances of `qubitron.Gate` (Instance Family).
    - Python types inheriting from `qubitron.Gate` (Type Family).

    By default, the containment checks depend on the initialization type:

    - Instance Family: Containment check is done via `qubitron.equal_up_to_global_phase`.
    ...
    - Instance Family:
        ...
        >>> gate_family = qubitron.GateFamily(qubitron.X)
        >>> assert qubitron.X in gate_family
        >>> assert qubitron.Rx(rads=np.pi) in gate_family
        >>> assert qubitron.X ** sympy.Symbol("theta") not in gate_family

    - Type Family:

        >>> gate_family = qubitron.GateFamily(qubitron.XPowGate)
        >>> assert qubitron.X in gate_family
        >>> assert qubitron.Rx(rads=np.pi) in gate_family
        >>> assert qubitron.X ** sympy.Symbol("theta") in gate_family

    As seen in the examples above, GateFamily supports containment checks for instances of both
    `qubitron.Operation` and `qubitron.Gate`. By default, a `qubitron.Operation` instance `op` is accepted if
    the underlying `op.gate` is accepted.

    Further constraints can be added on containment checks for `qubitron.Operation` objects by setting
    ...
    If a `qubitron.Operation` contains tags from both `tags_to_accept` and `tags_to_ignore`, it is
    ...
    For example:
        ...
        >>> gate_family = qubitron.GateFamily(qubitron.ZPowGate, tags_to_accept=['accepted_tag'])
        >>> assert qubitron.Z(q).with_tags('accepted_tag') in gate_family
        >>> assert qubitron.Z(q).with_tags('other_tag') not in gate_family
        >>> assert qubitron.Z(q) not in gate_family
        >>> assert qubitron.Z not in gate_family
        ...
        >>> gate_family = qubitron.GateFamily(qubitron.ZPowGate, tags_to_ignore=['ignored_tag'])
        >>> assert qubitron.Z(q).with_tags('ignored_tag') not in gate_family
        >>> assert qubitron.Z(q).with_tags('other_tag') in gate_family
        >>> assert qubitron.Z(q) in gate_family
        >>> assert qubitron.Z in gate_family

    In order to create gate families with constraints on parameters of a gate
    type, users should derive from the `qubitron.GateFamily` class and override the
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            gate: A python `type` inheriting from `qubitron.Gate` for type based membership checks, or
                a non-parameterized instance of a `qubitron.Gate` for equality based membership checks.
            ...
            ignore_global_phase: If True, value equality is checked via
                `qubitron.equal_up_to_global_phase`.
            tags_to_accept: If non-empty, only `qubitron.Operations` containing at least one tag in this
                sequence can be accepted.
            tags_to_ignore: Any `qubitron.Operation` containing at least one tag in this sequence is
        ...
        Raises:
            ValueError: if `gate` is not a `qubitron.Gate` instance or subclass.
            ValueError: if `gate` is a parameterized instance of `qubitron.Gate`.
        ...
        if not (
            ...
            raise ValueError(f'Gate {gate} must be an instance or subclass of `qubitron.Gate`.')
        if isinstance(gate, raw_types.Gate) and protocols.is_parameterized(gate):
            raise ValueError(f'Gate {gate} must be a non-parameterized instance of `qubitron.Gate`.')
    ...
    def _gate_json(self) -> raw_types.Gate | str:
        return self.gate if not isinstance(self.gate, type) else protocols.json_qubitron_type(self.gate)
    ...
    def _default_description(self) -> str:
        ...
        return (
            f'Accepts `qubitron.Gate` instances `g` s.t. `{check_type.format(self._gate_str())}`'
            + tags_to_accept_str
            + tags_to_ignore_str
        )
    ...
    def _predicate(self, gate: raw_types.Gate) -> bool:
        """Checks whether `qubitron.Gate` instance `gate` belongs to this GateFamily.

        The default predicate depends on the gate family initialization type:

        - Instance Family: `qubitron.equal_up_to_global_phase(gate, self.gate)`
            if self._ignore_global_phase else `gate == self.gate`.
        - Type Family: `isinstance(gate, self.gate)`.

        Args:
            gate: `qubitron.Gate` instance which should be checked for containment.
        """
    ...
    def _from_json_dict_(
        ...
        **kwargs,
        ...
        if isinstance(gate, str):
            gate = protocols.qubitron_type_from_json(gate)
        return cls(
            gate,
            name=name,
...
class Gateset:
    """Gatesets represent a collection of `qubitron.GateFamily` objects.

    Gatesets are useful for

    - Describing the set of allowed gates in a human-readable format.
    - Validating a given gate / `qubitron.OP_TREE` against the set of allowed gates.

    Gatesets rely on the underlying `qubitron.GateFamily` for both description and
    validation purposes.
    """
    ...
    def __init__(
        ...
        *gates: type[raw_types.Gate] | raw_types.Gate | GateFamily,
        ...
        - `qubitron.Gate` subclass
        - `qubitron.Gate` instance
        - `qubitron.GateFamily` instance

        `qubitron.Gate` subclasses and instances are converted to the default
        `qubitron.GateFamily(gate=g)` instance and thus a default name and
        description is populated.

        Args:
            *gates: A list of `qubitron.Gate` subclasses / `qubitron.Gate` instances /
                `qubitron.GateFamily` instances to initialize the Gateset.
            name: (Optional) Name for the Gateset. Useful for description.
            unroll_circuit_op: If True, `qubitron.CircuitOperation` is recursively
                validated by validating the underlying `qubitron.Circuit`.
        """
        self._name = name
    ...
    def with_params(
        ...
        Args:
            ...
            unroll_circuit_op: If True, new Gateset will recursively validate
                `qubitron.CircuitOperation` by validating the underlying `qubitron.Circuit`.
    ...
    def __contains__(self, item: raw_types.Gate | raw_types.Operation) -> bool:
        ...
        - For Gates or Operations that have an underlying gate (i.e. op.gate is not None):
            - Forwards the containment check to the underlying `qubitron.GateFamily` objects.
            - Examples of such operations include `qubitron.GateOperation`s and their controlled
                and tagged variants (i.e. instances of `qubitron.TaggedOperation`,
                `qubitron.ControlledOperation` where `op.gate` is not None) etc.
        - For Operations that do not have an underlying gate:
            - Forwards the containment check to `self._validate_operation(item)`.
            - Examples of such operations include `qubitron.CircuitOperation`s and their controlled
                and tagged variants (i.e. instances of `qubitron.TaggedOperation`,
                `qubitron.ControlledOperation` where `op.gate` is None) etc.

        The complexity of the method in terms of the number of `gates`, n, is

        - O(1) when any default `qubitron.GateFamily` instance accepts the given item, except
        ...
        Args:
            item: The `qubitron.Gate` or `qubitron.Operation` instance to check containment for.
        """
    ...
    def validate(self, circuit_or_optree: qubitron.AbstractCircuit | op_tree.OP_TREE) -> bool:
        """Validates gates forming `circuit_or_optree` should be contained in Gateset.

        Args:
            circuit_or_optree: The `qubitron.Circuit` or `qubitron.OP_TREE` to validate.
        """
        # To avoid circular import.
        from qubitron.circuits import circuit
    ...
    def _validate_operation(self, op: raw_types.Operation) -> bool:
        """Validates whether the given `qubitron.Operation` is contained in this Gateset.
        ...
        - For all other types of operations (eg: `qubitron.CircuitOperation`,
            etc):
        ...
        have an underlying `qubitron.Gate`.

        Args:
            op: The `qubitron.Operation` instance to check containment for.
        """

        # To avoid circular import.
        from qubitron.circuits import circuit_operation
