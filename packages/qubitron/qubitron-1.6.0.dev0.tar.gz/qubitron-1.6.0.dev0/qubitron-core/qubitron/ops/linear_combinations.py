...
from qubitron import linalg, protocols, qis, value
from qubitron._doc import document
from qubitron.linalg import operator_spaces
from qubitron.ops import identity, pauli_gates, pauli_string, raw_types
from qubitron.ops.pauli_string import _validate_qubit_mapping, PauliString
from qubitron.ops.projector import ProjectorString
from qubitron.value.linear_dict import _format_terms
...
class LinearCombinationOfGates(value.LinearDict[raw_types.Gate]):
    ...
    use overloaded arithmetic operators. For example,
        ...
        qubitron.LinearCombinationOfGates({qubitron.X: 2, qubitron.Z: -2})

    is equivalent to

        2 * qubitron.X - 2 * qubitron.Z
    """

    def __init__(self, terms: Mapping[raw_types.Gate, qubitron.TParamValComplex]) -> None:
    ...
    def _is_compatible(self, gate: qubitron.Gate) -> bool:
...
class LinearCombinationOfOperations(value.LinearDict[raw_types.Operation]):
    ...
    def __init__(self, terms: Mapping[raw_types.Operation, qubitron.TParamValComplex]) -> None:
        """Initializes linear combination from a collection of terms.
    ...
    def _is_compatible(self, operation: qubitron.Operation) -> bool:
    ...
    def _pauli_expansion_(self) -> value.LinearDict[str]:
        ...
        def extend_term(
            pauli_names: str, qubits: tuple[qubitron.Qid, ...], all_qubits: tuple[qubitron.Qid, ...]
        ) -> str:
...
def _pauli_string_from_unit(
    unit: UnitPauliStringT, coefficient: int | float | qubitron.TParamValComplex = 1
):
...
class PauliSum:
    ...
    Since `qubitron.PauliString`s store their own coefficients, this class
    does not implement the `qubitron.LinearDict` interface. Instead, you can
    ...
    >>> a, b = qubitron.GridQubit.rect(1, 2)
    >>> psum = qubitron.PauliSum.from_pauli_strings([
    ...     qubitron.PauliString(-1, qubitron.X(a), qubitron.Y(b)),
    ...     qubitron.PauliString(2, qubitron.Z(a), qubitron.Z(b)),
    ...     qubitron.PauliString(0.5, qubitron.Y(a), qubitron.Y(b))
    ... ])
    >>> print(psum)
    ...
    >>> a, b = qubitron.GridQubit.rect(1, 2)
    >>> psum = qubitron.X(a) * qubitron.X(b) + 3.0 * qubitron.Y(a)
    >>> print(psum)
    ...
    def __init__(self, linear_dict: value.LinearDict[UnitPauliStringT] | None = None):
        ...
        Args:
            linear_dict: Set of  (`qubitron.Qid`, `qubitron.Pauli`) tuples to construct the sum
                from.

        Raises:
            ValueError: If structure of `linear_dict` contains tuples other than the
                form (`qubitron.Qid`, `qubitron.Pauli`).
        """
    ...
    def wrap(val: PauliSumLike) -> PauliSum:
        """Convert a `qubitron.PauliSumLike` object to a PauliSum

        Attempts to convert an existing int, float, complex, `qubitron.PauliString`,
        `qubitron.PauliSum` or `qubitron.SingleQubitPauliStringGateOperation` into
        a `qubitron.PauliSum` object. For example:


        >>> my_psum = qubitron.PauliSum.wrap(2.345)
        >>> my_psum
        qubitron.PauliSum(qubitron.LinearDict({frozenset(): (2.345+0j)}))


        Args:
            `qubitron.PauliSumLike` to convert to PauliSum.
    ...
    def from_pauli_strings(cls, terms: PauliString | list[PauliString]) -> PauliSum:
        """Returns a PauliSum by combining `qubitron.PauliString` terms.

        Args:
            terms: `qubitron.PauliString` or List of `qubitron.PauliString`s to use inside
                of this PauliSum object.
        Returns:
            PauliSum object representing the addition of all the `qubitron.PauliString`
                terms in `terms`.
        """
    ...
    def from_boolean_expression(
        cls, boolean_expr: sympy.Expr, qubit_map: dict[str, qubitron.Qid]
    ) -> PauliSum:
    ...
    def with_qubits(self, *new_qubits: qubitron.Qid) -> PauliSum:
        """Return a new PauliSum on `new_qubits`.

        Args:
            *new_qubits: `qubitron.Qid` objects to replace existing
    ...
    def expectation_from_state_vector(
        ...
        *,
        ...
        # Github issue: https://github.com/amyssnippet/Qubitron/issues/2041
    ...
    def __repr__(self) -> str:
        ...
        return f'qubitron.{class_name}({self._linear_dict!r})'
