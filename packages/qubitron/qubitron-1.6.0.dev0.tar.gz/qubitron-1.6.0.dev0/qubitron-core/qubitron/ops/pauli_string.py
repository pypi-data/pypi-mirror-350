...
from qubitron import _compat, linalg, protocols, qis, value
from qubitron._compat import deprecated
from qubitron._doc import document
from qubitron._import import LazyLoader
from qubitron.ops import (
    clifford_gate,
    common_gates,
    dense_pauli_string,
    gate_operation,
...
linear_combinations = LazyLoader("linear_combinations", globals(), "qubitron.ops.linear_combinations")
...
# A value that can be unambiguously converted into a `qubitron.PauliString`.

PAULI_STRING_LIKE = Union[
    complex,
    'qubitron.OP_TREE',
    Mapping[TKey, 'qubitron.PAULI_GATE_LIKE'],
    Iterable,  # of PAULI_STRING_LIKE, but mypy doesn't do recursive types yet.
]
document(
    PAULI_STRING_LIKE,
    """A `qubitron.PauliString` or a value that can easily be converted into one.
    ...
    Each Pauli operation can be specified as a qubitron object (e.g. `qubitron.X`) or as
...
PAULI_GATE_LIKE = Union['qubitron.Pauli', 'qubitron.IdentityGate', str, int]
document(
    PAULI_GATE_LIKE,
    ...
    1. Qubitron gates: `qubitron.I`, `qubitron.X`, `qubitron.Y`, `qubitron.Z`.
...
class PauliString(raw_types.Operation, Generic[TKey]):
    ...
    `qubitron.PauliString` represents a multi-qubit pauli operator, i.e.
    ...
    - X(0) * Y(1) * Z(2): Represents a pauli string which is a tensor product of
                          `qubitron.X(q0)`, `qubitron.Y(q1)` and `qubitron.Z(q2)`.
    ...
    >>> print(qubitron.X(a) * qubitron.Y(b)) # Tensor product of Pauli's acting on different qubits.
    X(q(0))*Y(q(1))
    >>> print(qubitron.X(a) * qubitron.Y(a)) # Composition is reduced to an equivalent PauliString.
    1j*Z(q(0))
    >>> print(qubitron.X(a) * qubitron.I(b)) # Identity operations are dropped by default.
    X(q(0))
    >>> print(qubitron.PauliString()) # String representation of an "empty" PaulString is "I".
    I

    `qubitron.PauliString` is often used to represent:
    - Pauli operators: Can be inserted into circuits as multi qubit operations.
    - Pauli observables: Can be measured using either `qubitron.measure_single_paulistring`/
                        `qubitron.measure_paulistring_terms`; or using the observable
                        measurement framework in `qubitron.measure_observables`.

    PauliStrings can be constructed via various different ways, some examples are
    given as follows:

    >>> a, b, c = qubitron.LineQubit.range(3)
    >>> print(qubitron.PauliString([qubitron.X(a), qubitron.X(a)]))
    I
    >>> print(qubitron.PauliString(-1, qubitron.X(a), qubitron.Y(b), qubitron.Z(c)))
    -X(q(0))*Y(q(1))*Z(q(2))
    >>> print(-1 * qubitron.X(a) * qubitron.Y(b) * qubitron.Z(c))
    -X(q(0))*Y(q(1))*Z(q(2))
    >>> print(qubitron.PauliString({a: qubitron.X}, [-2, 3, qubitron.Y(a)]))
    -6j*Z(q(0))
    >>> print(qubitron.PauliString({a: qubitron.I, b: qubitron.X}))
    X(q(1))
    >>> print(qubitron.PauliString({a: qubitron.Y}, qubit_pauli_map={a: qubitron.X}))
    1j*Z(q(0))

    Note that `qubitron.PauliString`s are immutable objects. If you need a mutable version
    of pauli strings, see `qubitron.MutablePauliString`.
    """

    def __init__(
        self,
        *contents: qubitron.PAULI_STRING_LIKE,
        qubit_pauli_map: dict[TKey, qubitron.Pauli] | None = None,
        ...
        """
        if _compat.__qubitron_debug__.get() and qubit_pauli_map is not None:
            for v in qubit_pauli_map.values():
                if not isinstance(v, pauli_gates.Pauli):
                    raise TypeError(f'{v} is not a Pauli')

        self._qubit_pauli_map: dict[TKey, qubitron.Pauli] = qubit_pauli_map or {}
        self._coefficient: qubitron.TParamValComplex | sympy.Expr = (
    ...
    def equal_up_to_coefficient(self, other: qubitron.PauliString) -> bool:
    ...
    def get(self, key: Any, default: TDefault | None = None) -> pauli_gates.Pauli | TDefault | None:
        """Returns the `qubitron.Pauli` operation acting on qubit `key` or `default` if none exists."""
        return self._qubit_pauli_map.get(key, default)

    @overload
    def __mul__(self, other: qubitron.PauliString[TKeyOther]) -> qubitron.PauliString[TKey | TKeyOther]:
        pass

    @overload
    def __mul__(
        self, other: Mapping[TKeyOther, qubitron.PAULI_GATE_LIKE]
    ) -> qubitron.PauliString[TKey | TKeyOther]:
        pass

    @overload
    def __mul__(self, other: Iterable[qubitron.PAULI_STRING_LIKE]) -> qubitron.PauliString[TKey | qubitron.Qid]:
        pass

    @overload
    def __mul__(self, other: qubitron.Operation) -> qubitron.PauliString[TKey | qubitron.Qid]:
        pass

    @overload
    def __mul__(self, other: complex) -> qubitron.PauliString[TKey]:
        pass
    ...
    @property
    def gate(self) -> qubitron.DensePauliString:
        """Returns a `qubitron.DensePauliString`"""
        order: list[pauli_gates.Pauli | None] = [None, pauli_gates.X, pauli_gates.Y, pauli_gates.Z]
        from qubitron.ops.dense_pauli_string import DensePauliString
    ...
    def _circuit_diagram_info_(self, args: qubitron.CircuitDiagramInfoArgs) -> list[str]:
    ...
    def with_qubits(self, *new_qubits: qubitron.Qid) -> PauliString:
    ...
    def with_coefficient(self, new_coefficient: qubitron.TParamValComplex) -> PauliString:
    ...
    def values(self) -> ValuesView[pauli_gates.Pauli]:
        """Ordered sequence of `qubitron.Pauli` gates acting on `self.keys()`."""
        return self._qubit_pauli_map.values()

    def items(self) -> ItemsView[TKey, pauli_gates.Pauli]:
        """Returns (qubitron.Qid, qubitron.Pauli) pairs representing 1-qubit operations of pauli string."""
        return self._qubit_pauli_map.items()

    def frozen(self) -> qubitron.PauliString:
        """Returns a `qubitron.PauliString` with the same contents."""
        return self

    def mutable_copy(self) -> qubitron.MutablePauliString:
        """Returns a new `qubitron.MutablePauliString` with the same contents."""
    ...
    def __repr__(self) -> str:
        ...
        if not ordered_qubits:
            factors.append('qubitron.PauliString()')
    ...
    def expectation_from_state_vector(
        ...
        *,
        ...
        $|0\rangle |+\rangle$ and `q0, q1 = qubitron.LineQubit.range(2)` then:

            qubitron.X(q0).expectation(state, qubit_map={q0: 0, q1: 1}) = 0
            qubitron.X(q0).expectation(state, qubit_map={q0: 1, q1: 0}) = 1
    ...
    def zip_items(
        self, other: qubitron.PauliString[TKey]
        ...
        For every qubit that has a `qubitron.Pauli` operation acting on it in both `self` and `other`,
        the method yields a tuple corresponding to `(qubit, (pauli_in_self, pauli_in_other))`.

        Args:
            other: The other `qubitron.PauliString` to zip pauli operations with.

        Returns:
            A sequence of `(qubit, (pauli_in_self, pauli_in_other))` tuples for every `qubit`
            that has a `qubitron.Pauli` operation acting on it in both `self` and `other.
        """
    ...
    def zip_paulis(
        self, other: qubitron.PauliString
    ...
    def __pow__(self, power):
        ...
        if isinstance(power, (int, float)):
            ...
            from qubitron.ops import pauli_string_phasor
    ...
    def map_qubits(self, qubit_map: dict[TKey, TKeyNew]) -> qubitron.PauliString[TKeyNew]:
    ...
    def dense(self, qubits: Sequence[TKey]) -> qubitron.DensePauliString:
        """Returns a `qubitron.DensePauliString` version of this Pauli string.
        ...
        Returns:
            A `qubitron.DensePauliString` instance `D` such that `D.on(*qubits)`
            equals the receiving `qubitron.PauliString` instance `P`.
    ...
    def conjugated_by(self, clifford: qubitron.OP_TREE) -> PauliString:
        ...
        Examples:
            >>> a, b = qubitron.LineQubit.range(2)
            >>> print(qubitron.X(a).conjugated_by(qubitron.CZ(a, b)))
            X(q(0))*Z(q(1))
            >>> print(qubitron.X(a).conjugated_by(qubitron.S(a)))
            -Y(q(0))
            >>> print(qubitron.X(a).conjugated_by([qubitron.H(a), qubitron.CNOT(a, b)]))
            Z(q(0))*X(q(1))
        ...
        for op in all_ops[::-1]:
            ...
            gate_in_clifford: qubitron.CliffordGate
    ...
    def after(self, ops: qubitron.OP_TREE) -> qubitron.PauliString:
    ...
    def before(self, ops: qubitron.OP_TREE) -> qubitron.PauliString:
    ...
    def pass_operations_over(
        self, ops: Iterable[qubitron.Operation], after_to_before: bool = False
...
class SingleQubitPauliStringGateOperation(  # type: ignore
    ...
    Satisfies the contract of both `qubitron.GateOperation` and `qubitron.PauliString`. Relies
    implicitly on the fact that PauliString({q: X}) compares as equal to
    GateOperation(X, [q]).
    """

    def __init__(self, pauli: pauli_gates.Pauli, qubit: qubitron.Qid):
    ...
    def with_qubits(self, *new_qubits: qubitron.Qid) -> SingleQubitPauliStringGateOperation:
        if len(new_qubits) != 1:
    ...
    @classmethod
    def _from_json_dict_(cls, pauli: pauli_gates.Pauli, qubit: qubitron.Qid, **kwargs):  # type: ignore
...
class MutablePauliString(Generic[TKey]):
    """Mutable version of `qubitron.PauliString`, used mainly for efficiently mutating pauli strings.

    `qubitron.MutablePauliString` is a mutable version of `qubitron.PauliString`, which is often
    useful for mutating pauli strings efficiently instead of always creating a copy. Note
    that, unlike `qubitron.PauliString`, `MutablePauliString` is not a `qubitron.Operation`.

    It exists mainly to help mutate pauli strings efficiently and then convert back to a
    frozen `qubitron.PauliString` representation, which can then be used as operators or
    observables.
    """
    ...
    def __init__(
        ...
        *contents: qubitron.PAULI_STRING_LIKE,
        ...
        """
        self.coefficient: sympy.Expr | qubitron.TParamValComplex = (
            coefficient if isinstance(coefficient, sympy.Expr) else complex(coefficient)
        )
    ...
    def values(self) -> Iterator[qubitron.Pauli]:
        """Ordered sequence of `qubitron.Pauli` gates acting on `self.keys()`."""
    ...
    def frozen(self) -> qubitron.PauliString:
        ...
        For example, this is useful because `qubitron.PauliString` is an operation
        whereas `qubitron.MutablePauliString` is not.
        """
        return PauliString(
            coefficient=self.coefficient,
    ...
    def items(self) -> Iterator[tuple[TKey, qubitron.Pauli]]:
        """Returns (qubitron.Qid, qubitron.Pauli) pairs representing 1-qubit operations of pauli string."""
    ...
    def __getitem__(self, item: Any) -> qubitron.Pauli:
        return _INT_TO_PAULI[self.pauli_int_dict[item] - 1]

    def __setitem__(self, key: TKey, value: qubitron.PAULI_GATE_LIKE):
    ...
    # pylint: disable=function-redefined
    @overload
    def get(self, key: TKey, default: None = None) -> qubitron.Pauli | None:
        pass

    @overload
    def get(self, key: TKey, default: TDefault) -> qubitron.Pauli | TDefault:
        pass

    def get(self, key: TKey, default=None) -> qubitron.Pauli | TDefault | None:
        """Returns the `qubitron.Pauli` operation acting on qubit `key` or `default` if none exists."""
        result = self.pauli_int_dict.get(key, None)
        return default if result is None else _INT_TO_PAULI[result - 1]

    # pylint: enable=function-redefined
    def inplace_before(self, ops: qubitron.OP_TREE) -> qubitron.MutablePauliString:
    ...
    def inplace_after(self, ops: qubitron.OP_TREE) -> qubitron.MutablePauliString:
    ...
    def _imul_helper(self, other: qubitron.PAULI_STRING_LIKE, sign: int):
    ...
    def _imul_helper_checkpoint(self, other: qubitron.PAULI_STRING_LIKE, sign: int):
    ...
    def inplace_left_multiply_by(self, other: qubitron.PAULI_STRING_LIKE) -> qubitron.MutablePauliString:
        """Left-multiplies a pauli string into this pauli string.

        Args:
            other: A pauli string or `qubitron.PAULI_STRING_LIKE` to left-multiply
                into `self`.
        ...
        Raises:
            TypeError: `other` was not a `qubitron.PAULI_STRING_LIKE`. `self`
                was not mutated.
        """
        if self._imul_helper_checkpoint(other, -1) is NotImplemented:
            raise TypeError(f"{other!r} is not qubitron.PAULI_STRING_LIKE.")
        return self
    ...
    def inplace_right_multiply_by(self, other: qubitron.PAULI_STRING_LIKE) -> qubitron.MutablePauliString:
        """Right-multiplies a pauli string into this pauli string.

        Args:
            other: A pauli string or `qubitron.PAULI_STRING_LIKE` to right-multiply
                into `self`.
    ...
    def __neg__(self) -> qubitron.MutablePauliString:
        result = self.mutable_copy()
        result.coefficient *= -1
        return result

    def __pos__(self) -> qubitron.MutablePauliString:
        return self.mutable_copy()

    def transform_qubits(
        self, func: Callable[[TKey], TKeyNew], *, inplace: bool = False
    ) -> qubitron.MutablePauliString[TKeyNew]:
        ...
        Args:
            ...
            inplace: If false (the default), creates a new mutable pauli string
                ...
                `qubitron.PauliString.transform_qubits` in situations where the
        ...
        result = cast('qubitron.MutablePauliString[TKeyNew]', self)
        result.pauli_int_dict = new_dict
        return result

    def __imul__(self, other: qubitron.PAULI_STRING_LIKE) -> qubitron.MutablePauliString:
        """Left-multiplies a pauli string into this pauli string.
        ...
        Returns:
            ...
            If `other` is not a `qubitron.PAULI_STRING_LIKE`, `self` is not mutated
            and `NotImplemented` is returned.

        """
        return self._imul_helper_checkpoint(other, +1)

    def __mul__(self, other: qubitron.PAULI_STRING_LIKE) -> qubitron.PauliString:
    ...
    def __rmul__(self, other: qubitron.PAULI_STRING_LIKE) -> qubitron.PauliString:
        """Multiplies two pauli-string-likes together.
...
def _decompose_into_cliffords(op: qubitron.Operation) -> list[qubitron.Operation]:
...
PAULI_GATE_LIKE_TO_INDEX_MAP: dict[qubitron.PAULI_GATE_LIKE, int] = {
    _i: 0,
    _x: 1,
    _y: 2,
    _z: 3,
...
_INT_TO_PAULI_OR_IDENTITY: list[qubitron.Pauli | qubitron.IdentityGate] = [_i, _x, _y, _z]
_INT_TO_PAULI: list[qubitron.Pauli] = [_x, _y, _z]


PAULI_GATE_LIKE_TO_GATE_MAP: dict[qubitron.PAULI_GATE_LIKE, qubitron.Pauli | qubitron.IdentityGate] = {
...
def _pauli_like_to_pauli_int(key: Any, pauli_gate_like: PAULI_GATE_LIKE):
    ...
    if pauli_int is None:
        raise TypeError(
            ...
            f'qubitron.PAULI_GATE_LIKE value. '
