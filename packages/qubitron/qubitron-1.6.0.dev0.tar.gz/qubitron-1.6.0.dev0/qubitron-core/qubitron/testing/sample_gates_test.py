...
import qubitron
from qubitron.testing import sample_gates
...
def test_phase_using_clean_ancilla(theta: float) -> None:
    ...
    qubit_order = qubitron.QubitOrder.explicit([q], fallback=qubitron.QubitOrder.DEFAULT)
    decomposed_unitary = qubitron.Circuit(qubitron.decompose_once(g.on(q))).unitary(qubit_order=qubit_order)
...
def test_phase_using_dirty_ancilla(target_bitsize, phase_state, ancilla_bitsize) -> None:
    ...
    q = qubitron.LineQubit.range(target_bitsize)
    qubit_order = qubitron.QubitOrder.explicit(q, fallback=qubitron.QubitOrder.DEFAULT)
    decomposed_circuit = qubitron.Circuit(qubitron.decompose_once(g.on(*q)))
