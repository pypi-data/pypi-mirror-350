...
document(
    DEFAULT_GATE_DOMAIN,
    """The default gate domain for `qubitron.testing.random_circuit`.
...
def random_circuit(
    qubits: Sequence[qubitron.Qid] | int,
    n_moments: int,
    op_density: float,
    ...
    Args:
        qubits: If a sequence of qubits, then these are the qubits that
            ...
            `qubitron.NamedQubits` with names given by the integers in
            `range(qubits)`.
...
def random_two_qubit_circuit_with_czs(
    num_czs: int = 3,
    q0: qubitron.Qid | None = None,
    q1: qubitron.Qid | None = None,
    random_state: qubitron.RANDOM_STATE_OR_SEED_LIKE = None,
) -> circuits.Circuit:
