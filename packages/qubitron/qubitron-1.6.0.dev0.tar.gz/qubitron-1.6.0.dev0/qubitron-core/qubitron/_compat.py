...
__qubitron_debug__ = contextvars.ContextVar('__qubitron_debug__', default=__debug__)
document(
    __qubitron_debug__,
    "A qubitron specific flag which can be used to conditionally turn off all validations across Qubitron "
...
def with_debug(value: bool) -> Iterator[None]:
    """Sets the value of global constant `qubitron.__qubitron_debug__` within the context.

    If `__qubitron_debug__` is set to False, all validations in Qubitron are disabled to optimize
    performance. Users should use the `qubitron.with_debug` context manager instead of manually
    mutating the value of `__qubitron_debug__` flag. On exit, the context manager resets the
    value of `__qubitron_debug__` flag to what it was before entering the context manager.
    """
    token = __qubitron_debug__.set(value)
    try:
        yield
    finally:
        __qubitron_debug__.reset(token)
...
def dataclass_repr(value: Any, namespace: str = 'qubitron') -> str:
    """Create a Qubitron-style repr for a dataclass.
    ...
    if _called_from_test() and not deprecation_allowed:
        for filename, line_number, function_name, text in reversed(traceback.extract_stack()):
            if (
                ...
                and not filename.endswith(os.path.join("qubitron", "_compat.py"))
                and "_test.py" in filename
            ):
                break
        raise ValueError(
            f"During testing using Qubitron deprecated functionality is not allowed: {msg}"
            ...
            f"In case the usage of deprecated qubitron is intentional, use "
            f"`with qubitron.testing.assert_deprecated(...):` around this line:\n"
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def decorated_func(*args, **kwargs) -> Any:
            ...
            _warn_or_error(
                ...
                f'It will be removed in qubitron {deadline}.\n'
                f'{fix}\n'
            )
...
def deprecated_submodule(
    *, new_module_name: str, old_parent: str, old_child: str, deadline: str, create_attribute: bool
    ...
    For `new_module_name` (e.g. qubitron_google) creates an alias (e.g qubitron.google) in Python's module
    cache. It also recursively checks for the already imported submodules (e.g. qubitron_google.api) and
    creates the alias for them too (e.g. qubitron.google.api). With this method it is possible to create
    an alias that really looks like a module, e.g you can do things like
    `from qubitron.google import api` - which would be otherwise impossible.
    ...
    Args:
        ...
        deadline: The version of Qubitron where the module will be removed.
    ...
    if create_attribute:
        try:
            ...
            msg = (
                ...
                f"\n 2.) when developing Qubitron, you don't have your PYTHONPATH "
