...
from qubitron import circuits, devices, ops, protocols, study, value
from qubitron._doc import document
from qubitron.sim import density_matrix_simulator, sparse_simulator
from qubitron.sim.clifford import clifford_simulator
from qubitron.transformers import measurement_transformers
...
def _is_clifford_circuit(program: qubitron.Circuit) -> bool:
    return all(
...
def sample(
    ...
    *,
    ...
    param_resolver: qubitron.ParamResolver | None = None,
    repetitions: int = 1,
    ...
    Returns:
        A `qubitron.Result` object containing the requested measurement samples.
    """
...
def _to_circuit(program: qubitron.CIRCUIT_LIKE) -> qubitron.Circuit:
    ...
    *,
    ...
    if not protocols.has_unitary(protocols.resolve_parameters(circuit_like, param_resolver)):
        raise ValueError(
            ...
            "Maybe you wanted `qubitron.final_density_matrix`?\n"
            "\n"
...
def final_density_matrix(
    ...
    *,
    ...
    Note that, unlike `qubitron.final_state_vector`, terminal measurements
