...
TSimulationState = TypeVar('TSimulationState', bound='qubitron.SimulationState')
...
class SimulationStateBase(Generic[TSimulationState], metaclass=abc.ABCMeta):
    ...
    def __init__(
        self, *, qubits: Sequence[qubitron.Qid], classical_data: qubitron.ClassicalDataStore | None = None
    ):
    ...
    def _set_qubits(self, qubits: Sequence[qubitron.Qid]):
        self._qubits = tuple(qubits)
        self._qubit_map = {q: i for i, q in enumerate(self.qubits)}

    @property
    def classical_data(self) -> qubitron.ClassicalDataStoreReader:
    ...
    def apply_operation(self, op: qubitron.Operation):
