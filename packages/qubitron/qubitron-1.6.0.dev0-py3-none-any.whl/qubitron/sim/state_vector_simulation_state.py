...
from qubitron import linalg, protocols, qis, sim
from qubitron._compat import proper_repr
from qubitron.linalg import transformations
from qubitron.sim.simulation_state import SimulationState, strat_act_on_from_apply_decompose
...
class _BufferedStateVector(qis.QuantumStateRepresentation):
    ...
    def create(
        cls,
        *,
        initial_state: np.ndarray | qubitron.STATE_VECTOR_LIKE = 0,
    ...
    def _swap_target_tensor_for(self, new_target_tensor: np.ndarray):
        ...
        `args` is this `qubitron.StateVectorSimulationState` instance.
...
class StateVectorSimulationState(SimulationState[_BufferedStateVector]):
    ...
    def add_qubits(self, qubits: Sequence[qubitron.Qid]):
    ...
    def remove_qubits(self, qubits: Sequence[qubitron.Qid]):
    ...
    def _act_on_fallback_(
        ...
        strats: list[Callable[[Any, Any, Sequence[qubitron.Qid]], bool]] = [
    ...
    def __repr__(self) -> str:
        return (
            'qubitron.StateVectorSimulationState('
...
def _strat_act_on_state_vector_from_apply_unitary(
    action: Any, args: qubitron.StateVectorSimulationState, qubits: Sequence[qubitron.Qid]
) -> bool:
