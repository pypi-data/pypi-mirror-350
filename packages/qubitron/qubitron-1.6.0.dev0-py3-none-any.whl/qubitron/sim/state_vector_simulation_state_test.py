...
def test_default_parameter() -> None:
    dtype = np.complex64
    tensor = qubitron.one_hot(shape=(2, 2, 2), dtype=np.complex64)
    qubits = qubitron.LineQubit.range(3)
    args = qubitron.StateVectorSimulationState(qubits=qubits, initial_state=tensor, dtype=dtype)
    qid_shape = qubitron.protocols.qid_shape(qubits)
...
def test_infer_target_tensor() -> None:
    ...
    args = qubitron.StateVectorSimulationState(
        qubits=qubitron.LineQubit.range(2), initial_state=0, dtype=dtype
    )
    np.testing.assert_almost_equal(
        args.target_tensor,
        np.array([[1.0 + 0.0j, 0.0 + 0.0j], [0.0 + 0.0j, 0.0 + 0.0j]], dtype=dtype),
...
def test_shallow_copy_buffers() -> None:
    args = qubitron.StateVectorSimulationState(qubits=qubitron.LineQubit.range(1), initial_state=0)
...
def test_decomposed_fallback() -> None:
    ...
    args = qubitron.StateVectorSimulationState(
        ...
        initial_state=qubitron.one_hot(shape=(2, 2, 2), dtype=np.complex64),
        dtype=np.complex64,
    )

    qubitron.act_on(Composite(), args, [qubitron.LineQubit(1)])
    np.testing.assert_allclose(
        args.target_tensor, qubitron.one_hot(index=(0, 1, 0), shape=(2, 2, 2), dtype=np.complex64)
    )
...
def test_act_using_probabilistic_single_qubit_channel() -> None:
    class ProbabilisticSorX(qubitron.Gate):
        def num_qubits(self) -> int:
            return 1

        def _kraus_(self):
            return [qubitron.unitary(qubitron.S) * np.sqrt(1 / 3), qubitron.unitary(qubitron.X) * np.sqrt(2 / 3)]

    initial_state = qubitron.testing.random_superposition(dim=16).reshape((2,) * 4)
    mock_prng = mock.Mock()
    ...
    qubitron.act_on(ProbabilisticSorX(), args, [qubitron.LineQubit(2)])
    np.testing.assert_allclose(
        args.target_tensor.reshape(16),
        qubitron.final_state_vector(
            qubitron.X(qubitron.LineQubit(2)) ** -1,
            initial_state=initial_state,
            qubit_order=qubitron.LineQubit.range(4),
        ),
        atol=1e-8,
    )
    ...
    np.testing.assert_allclose(
        args.target_tensor.reshape(16),
        qubitron.final_state_vector(
            qubitron.S(qubitron.LineQubit(2)),
            initial_state=initial_state,
            qubit_order=qubitron.LineQubit.range(4),
        ),
        atol=1e-8,
...
def test_act_using_adaptive_two_qubit_channel() -> None:
    class Decay11(qubitron.Gate):
        def num_qubits(self) -> int:
            return 2

        def _kraus_(self):
            bottom_right = qubitron.one_hot(index=(3, 3), shape=(4, 4), dtype=np.complex64)
            top_right = qubitron.one_hot(index=(0, 3), shape=(4, 4), dtype=np.complex64)
            return [
    ...
    def get_result(state: np.ndarray, sample: float):
        ...
        qubitron.act_on(Decay11(), args, [qubitron.LineQubit(1), qubitron.LineQubit(3)])
    ...
    all_zeroes = qubitron.one_hot(index=(0, 0, 0, 0), shape=(2,) * 4, dtype=np.complex128)
    all_ones = qubitron.one_hot(index=(1, 1, 1, 1), shape=(2,) * 4, dtype=np.complex128)
    decayed_all_ones = qubitron.one_hot(index=(1, 0, 1, 0), shape=(2,) * 4, dtype=np.complex128)
    ...
    for _ in range(10):
        assert_not_affected(
            qubitron.testing.random_superposition(dim=16).reshape((2,) * 4), sample=3 / 4 - 1e-8
        )

    # Acts like identity on superpositions of first three states.
    for _ in range(10):
        mock_prng.random.return_value = 3 / 4 + 1e-6
        projected_state = qubitron.testing.random_superposition(dim=16).reshape((2,) * 4)
        projected_state[qubitron.slice_for_qubits_equal_to([1, 3], 3)] = 0
...
def test_probability_comes_up_short_results_in_fallback() -> None:
    class Short(qubitron.Gate):
        def num_qubits(self) -> int:
            return 1

        def _kraus_(self):
            return [qubitron.unitary(qubitron.X) * np.sqrt(0.999), np.eye(2) * 0]
    ...
    qubitron.act_on(Short(), args, qubitron.LineQubit.range(1))
...
def test_random_channel_has_random_behavior() -> None:
    ...
    s = qubitron.Simulator().sample(
        qubitron.Circuit(qubitron.X(q), qubitron.amplitude_damp(0.4).on(q), qubitron.measure(q, key='out')),
        repetitions=100,
    )
...
def test_measured_mixture() -> None:
    # This behaves like an X-basis measurement.
    mm = qubitron.MixedUnitaryChannel(
        mixture=((0.5, np.array([[1, 0], [0, 1]])), (0.5, np.array([[0, 1], [1, 0]]))), key='flip'
    )
    q0 = qubitron.LineQubit(0)
    circuit = qubitron.Circuit(mm.on(q0), qubitron.measure(q0, key='m'))
    sim = qubitron.Simulator(seed=0)
    results = sim.run(circuit, repetitions=100)
...
def test_qid_shape_error() -> None:
    with pytest.raises(ValueError, match="qid_shape must be provided"):
        qubitron.sim.state_vector_simulation_state._BufferedStateVector.create(initial_state=0)
