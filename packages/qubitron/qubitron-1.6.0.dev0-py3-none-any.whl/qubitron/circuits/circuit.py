...
import qubitron._version
from qubitron import _compat, devices, ops, protocols, qis
from qubitron._doc import document
from qubitron.circuits._bucket_priority_queue import BucketPriorityQueue
from qubitron.circuits.circuit_operation import CircuitOperation
from qubitron.circuits.insert_strategy import InsertStrategy
from qubitron.circuits.moment import Moment
from qubitron.circuits.qasm_output import QasmOutput
from qubitron.circuits.text_diagram_drawer import TextDiagramDrawer
from qubitron.protocols import circuit_diagram_info_protocol

if TYPE_CHECKING:
    import qubitron


_TGate = TypeVar('_TGate', bound='qubitron.Gate')
_MOMENT_OR_OP = Union['qubitron.Moment', 'qubitron.Operation']
...
document(
    ...
    and returns the same type, that is, if passed a `qubitron.Circuit` it will return
    `qubitron.Circuit`, and similarly if passed `qubitron.FrozenCircuit` it will return
    `qubitron.FrozenCircuit`. This is particularly useful for things like the
    transformer API, since it can preserve more type information than if we typed
    the function as taking and returning `qubitron.AbstractCircuit`.
    """,
)
...
class Alignment(enum.Enum):
    ...
    def __repr__(self) -> str:
        return f'qubitron.Alignment.{self.name}'
...
class AbstractCircuit(abc.ABC):
    ...
    *   get_independent_qubit_sets
    ...
    @classmethod
    def from_moments(cls: type[CIRCUIT_TYPE], *moments: qubitron.OP_TREE | None) -> CIRCUIT_TYPE:
        ...
        Args:
            *moments: Op trees for each moment, which can be one of the following:
                ...
                - Other OP_TREE: will be passed to `qubitron.Moment` to create a new moment
    ...
    @staticmethod
    def _make_moments(moments: Iterable[qubitron.OP_TREE | None]) -> Iterator[qubitron.Moment]:
        for m in moments:
    ...
    @classmethod
    @abc.abstractmethod
    def _from_moments(cls: type[CIRCUIT_TYPE], moments: Iterable[qubitron.Moment]) -> CIRCUIT_TYPE:
    ...
    @property
    @abc.abstractmethod
    def moments(self) -> Sequence[qubitron.Moment]:
        pass

    @abc.abstractmethod
    def freeze(self) -> qubitron.FrozenCircuit:
    ...
    @abc.abstractmethod
    def unfreeze(self, copy: bool = True) -> qubitron.Circuit:
    ...
    def _approx_eq_(self, other: Any, atol: float) -> bool:
        """See `qubitron.protocols.SupportsApproximateEquality`."""
        if not isinstance(other, AbstractCircuit):
            return NotImplemented
        return other is self or qubitron.protocols.approx_eq(
    ...
    def __iter__(self) -> Iterator[qubitron.Moment]:
        return iter(self.moments)

    def _decompose_(self) -> qubitron.OP_TREE:
        """See `qubitron.SupportsDecompose`."""
        return self.all_operations()

    # pylint: disable=function-redefined
    @overload
    def __getitem__(self, key: int) -> qubitron.Moment:
        pass

    @overload
    def __getitem__(self, key: tuple[int, qubitron.Qid]) -> qubitron.Operation:
        pass

    @overload
    def __getitem__(self, key: tuple[int, Iterable[qubitron.Qid]]) -> qubitron.Moment:
        pass
    ...
    @overload
    def __getitem__(self, key: tuple[slice, qubitron.Qid]) -> Self:
        pass

    @overload
    def __getitem__(self, key: tuple[slice, Iterable[qubitron.Qid]]) -> Self:
        pass
    ...
    def __repr__(self) -> str:
        ...
        return f'qubitron.{cls_name}({self._repr_args()})'
    ...
    def _first_moment_operating_on(
        self, qubits: Iterable[qubitron.Qid], indices: Iterable[int]
    ) -> int | None:
    ...
    def next_moments_operating_on(
        self, qubits: Iterable[qubitron.Qid], start_moment_index: int = 0
    ) -> dict[qubitron.Qid, int]:
    ...
    def reachable_frontier_from(
        self,
        start_frontier: dict[qubitron.Qid, int],
        *,
        is_blocker: Callable[[qubitron.Operation], bool] = lambda op: False,
    ) -> dict[qubitron.Qid, int]:
        ...
        ```
        {
            qubitron.LineQubit(0): 6,
            qubitron.LineQubit(1): 2,
            qubitron.LineQubit(2): 2
        }
        ```
        ...
        ```
        {
            qubitron.LineQubit(0): 11,
            qubitron.LineQubit(1): 9,
            qubitron.LineQubit(2): 6,
        }
        ```
        ...
        ```
        is_blocker=lambda: op == qubitron.CZ(qubitron.LineQubit(1),
        ...
        ```
        {
            qubitron.LineQubit(0): 0,
            qubitron.LineQubit(1): 0,
            qubitron.LineQubit(2): 0,
            qubitron.LineQubit(3): 0,
        }
        ```
        ...
        {
            qubitron.LineQubit(0): 11,
            qubitron.LineQubit(1): 3,
            qubitron.LineQubit(2): 3,
            qubitron.LineQubit(3): 5,
        }
        ```
        ...
        """
        active: set[qubitron.Qid] = set()
        end_frontier = {}
        queue = BucketPriorityQueue[ops.Operation](drop_duplicate_entries=True)

        def enqueue_next(qubit: qubitron.Qid, moment: int) -> None:
    ...
    def findall_operations_between(
        self,
        start_frontier: dict[qubitron.Qid, int],
        end_frontier: dict[qubitron.Qid, int],
        omit_crossing_operations: bool = False,
    ) -> list[tuple[int, qubitron.Operation]]:
    ...
    def findall_operations_until_blocked(
        ...
        blocked_qubits: set[qubitron.Qid] = set()
    ...
    def operation_at(self, qubit: qubitron.Qid, moment_index: int) -> qubitron.Operation | None:
    ...
    def findall_operations(
        self, predicate: Callable[[qubitron.Operation], bool]
    ) -> Iterable[tuple[int, qubitron.Operation]]:
    ...
    ) -> Iterable[tuple[int, qubitron.GateOperation, _TGate]]:
    ...
    def has_measurements(self):
        ...
        Returns: True if `qubitron.is_measurement(self)` is True otherwise False.
        """
    ...
    def are_all_matches_terminal(self, predicate: Callable[[qubitron.Operation], bool]) -> bool:
        ...
        """
        from qubitron.circuits import CircuitOperation
    ...
    def are_any_matches_terminal(self, predicate: Callable[[qubitron.Operation], bool]) -> bool:
    ...
    def _has_op_at(self, moment_index: int, qubits: Iterable[qubitron.Qid]) -> bool:
        return 0 <= moment_index < len(self.moments) and self.moments[moment_index].operates_on(
            qubits
        )

    def all_qubits(self) -> frozenset[qubitron.Qid]:
        """Returns the qubits acted upon by Operations in this circuit.

        Returns: frozenset of `qubitron.Qid` objects acted on by all operations
            in this circuit.
        """
        return frozenset(q for m in self.moments for q in m.qubits)

    def all_operations(self) -> Iterator[qubitron.Operation]:
        """Returns an iterator over the operations in the circuit.

        Returns: Iterator over `qubitron.Operation` elements found in this circuit.
        """
        return (op for moment in self for op in moment.operations)

    def map_operations(self, func: Callable[[qubitron.Operation], qubitron.OP_TREE]) -> Self:
        ...
        def map_moment(moment: qubitron.Moment) -> qubitron.Circuit:
    ...
    def qid_shape(
        self, qubit_order: qubitron.QubitOrderOrList = ops.QubitOrder.DEFAULT
    ) -> tuple[int, ...]:
    ...
    def all_measurement_key_objs(self) -> frozenset[qubitron.MeasurementKey]:
        return frozenset(
            key for op in self.all_operations() for key in protocols.measurement_key_objs(op)
        )

    def _measurement_key_objs_(self) -> frozenset[qubitron.MeasurementKey]:
        """Returns the set of all measurement keys in this circuit.

        Returns: frozenset of `qubitron.MeasurementKey` objects that are
            in this circuit.
        """
    ...
    def _with_rescoped_keys_(
        self, path: tuple[str, ...], bindable_keys: frozenset[qubitron.MeasurementKey]
    ):
        moments = []
    ...
    def unitary(
        self,
        qubit_order: qubitron.QubitOrderOrList = ops.QubitOrder.DEFAULT,
        qubits_that_should_be_present: Iterable[qubitron.Qid] = (),
        ignore_terminal_measurements: bool = True,
        dtype: type[np.complexfloating] = np.complex128,
    ) -> np.ndarray:
        """Converts the circuit into a unitary matrix, if possible.

        Returns the same result as `qubitron.unitary`, but provides more options.
    ...
    def final_state_vector(
        self,
        *,
        initial_state: qubitron.STATE_VECTOR_LIKE = 0,
        qubit_order: qubitron.QubitOrderOrList = ops.QubitOrder.DEFAULT,
        ignore_terminal_measurements: bool = False,
        dtype: type[np.complexfloating] = np.complex128,
        param_resolver: qubitron.ParamResolverOrSimilarType = None,
        seed: qubitron.RANDOM_STATE_OR_SEED_LIKE = None,
    ) -> np.ndarray:
        """Returns the state vector resulting from acting operations on a state.

        This is equivalent to calling qubitron.final_state_vector with the same
        ...
        """
        from qubitron.sim.mux import final_state_vector
    ...
    def to_text_diagram_drawer(
        ...
        *,
        use_unicode_characters: bool = True,
        qubit_namer: Callable[[qubitron.Qid], str] | None = None,
        transpose: bool = False,
        include_tags: bool = True,
        ...
        get_circuit_diagram_info: (
            Callable[[qubitron.Operation, qubitron.CircuitDiagramInfoArgs], qubitron.CircuitDiagramInfo] | None
        ) = None,
    ) -> qubitron.TextDiagramDrawer:
        ...
        if any(isinstance(op.gate, qubitron.GlobalPhaseGate) for op in self.all_operations()):
    ...
    def _resolve_parameters_(self, resolver: qubitron.ParamResolver, recursive: bool) -> Self:
        changed = False
        resolved_moments: list[qubitron.Moment] = []
        for moment in self:
    ...
    def _qasm_(self, args: qubitron.QasmArgs | None = None) -> str:
        if args is None:
    ...
    def _to_qasm_output(
        ...
        Args:
            header: A multi-line string that is placed in a comment at the top
                of the QASM. Defaults to a qubitron version specifier.
            precision: Number of digits to use when representing numbers.
        ...
        """
        if header is None:
            header = f'Generated from Qubitron v{qubitron._version.__version__}'
        qubits = ops.QubitOrder.as_qubit_order(qubit_order).order_for(self.all_qubits())
        return QasmOutput(
    ...
    def zip(
        *circuits: qubitron.AbstractCircuit, align: qubitron.Alignment | str = Alignment.LEFT
        ...
        Args:
            *circuits: The circuits to merge together.
            align: The alignment for the zip, see `qubitron.Alignment`.
        ...
        <BLANKLINE>
        3: ───────S───────
        >>> print(qubitron.Circuit.zip(circuit3, circuit2, circuit1))
        0: ───H───@───────
                  │
        1: ───────X───────
        <BLANKLINE>
        ...
        result = qubitron.Circuit()
        for k in range(n):
            try:
                if align == Alignment.LEFT:
                    moment = qubitron.Moment(c[k] for c in circuits if k < len(c))
                else:
                    moment = qubitron.Moment(c[len(c) - n + k] for c in circuits if len(c) - n + k >= 0)
    ...
    def concat_ragged(
        *circuits: qubitron.AbstractCircuit, align: qubitron.Alignment | str = Alignment.LEFT
        ...
        >>> A = qubitron.Circuit(qubitron.H(a))
        >>> B = qubitron.Circuit(qubitron.H(b))
        ...
        buffer: MutableSequence[qubitron.Moment] = [qubitron.Moment()] * (pad_len * 2 + n_acc)
        ...
        return qubitron.Circuit(buffer[offset : offset + n_acc])

    def get_independent_qubit_sets(self) -> list[set[qubitron.Qid]]:
        ...
        >>> circuit.append(qubitron.Moment(qubitron.H(q2)))
        >>> circuit.append(qubitron.Moment(qubitron.CZ(q0,q1)))
        >>> circuit.append(qubitron.H(q0))
        >>> print(circuit)
        0: ───────@───H───
                  │
        1: ───────@───────
        ...
        [[qubitron.LineQubit(0), qubitron.LineQubit(1)], [qubitron.LineQubit(2)]]
    ...
    def _control_keys_(self) -> frozenset[qubitron.MeasurementKey]:
...
def _overlap_collision_time(
    c1: Sequence[qubitron.Moment], c2: Sequence[qubitron.Moment], align: qubitron.Alignment
) -> int:
    ...
    seen_times: dict[qubitron.Qid, int] = {}
...
def _concat_ragged_helper(
    c1_offset: int,
    n1: int,
    buf: MutableSequence[qubitron.Moment],
    c2: Sequence[qubitron.Moment],
    align: qubitron.Alignment,
) -> tuple[int, int]:
    n2 = len(c2)
...
class Circuit(AbstractCircuit):
    ...
    *   `circuit.factorize()` returns a sequence of Circuits which represent
    ...
    def __init__(
        self, *contents: qubitron.OP_TREE, strategy: qubitron.InsertStrategy = InsertStrategy.EARLIEST
    ) -> None:
        ...
        Args:
            contents: The initial list of moments and operations defining the
                ...
                or generally anything meeting the `qubitron.OP_TREE` contract.
        ...
        self._moments: list[qubitron.Moment] = []
        ...
        self._all_qubits: frozenset[qubitron.Qid] | None = None
        self._frozen: qubitron.FrozenCircuit | None = None
    ...
    @classmethod
    def _from_moments(cls, moments: Iterable[qubitron.Moment]) -> Circuit:
        new_circuit = Circuit()
        new_circuit._moments[:] = moments
        new_circuit._placement_cache = None
        return new_circuit

    def _load_contents_with_earliest_strategy(self, contents: qubitron.OP_TREE):
        ...
        op_lists_by_index: dict[int, list[qubitron.Operation]] = defaultdict(list)
        moments_by_index: dict[int, qubitron.Moment] = {}
    ...
    def __copy__(self) -> qubitron.Circuit:
        return self.copy()
    ...
    def freeze(self) -> qubitron.FrozenCircuit:
        ...
        """
        from qubitron.circuits.frozen_circuit import FrozenCircuit
    ...
    # pylint: disable=function-redefined
    @overload
    def __setitem__(self, key: int, value: qubitron.Moment):
        pass

    @overload
    def __setitem__(self, key: slice, value: Iterable[qubitron.Moment]):
        pass
    ...
    def __pow__(self, exponent: int) -> qubitron.Circuit:
        ...
        for moment in self[::-1]:
            inv_moment = qubitron.inverse(moment, default=NotImplemented)
            if inv_moment is NotImplemented:
                return NotImplemented
            inv_moments.append(inv_moment)

        return qubitron.Circuit(inv_moments)
    ...
    def transform_qubits(
        self, qubit_map: dict[qubitron.Qid, qubitron.Qid] | Callable[[qubitron.Qid], qubitron.Qid]
    ) -> qubitron.Circuit:
        ...
        >>> grid_qubits = qubitron.GridQubit.square(2)
        >>> line_qubits = qubitron.LineQubit.range(4)
        >>> circuit = qubitron.Circuit([qubitron.H(q) for q in line_qubits])
        >>> circuit.transform_qubits(lambda q : grid_qubits[q.x])
        qubitron.Circuit([
            qubitron.Moment(
                qubitron.H(qubitron.GridQubit(0, 0)),
                qubitron.H(qubitron.GridQubit(0, 1)),
                qubitron.H(qubitron.GridQubit(1, 0)),
                qubitron.H(qubitron.GridQubit(1, 1)),
            ),
        ])
    ...
    def earliest_available_moment(
        self, op: qubitron.Operation, *, end_moment_index: int | None = None
    ) -> int:
        ...
        while k > 0:
            ...
            if self._can_add_op_at(k, op):
                ...
                # `qubitron.AbstractDevice`.
                last_available = k
        return last_available

    def _can_add_op_at(self, moment_index: int, operation: qubitron.Operation) -> bool:
    ...
    def insert(
        self,
        index: int,
        moment_or_operation_tree: qubitron.OP_TREE,
        strategy: qubitron.InsertStrategy = InsertStrategy.EARLIEST,
    ) -> int:
        ...
        for batch in batches:
            ...
            if (
                ...
                and not all(
                    ...
                    for op in cast(list[qubitron.Operation], batch)
                )
            ):
    ...
    def insert_into_range(self, operations: qubitron.OP_TREE, start: int, end: int) -> int:
    ...
    def _push_frontier(
        self,
        early_frontier: dict[qubitron.Qid, int],
        late_frontier: dict[qubitron.Qid, int],
        update_qubits: Iterable[qubitron.Qid] | None = None,
    ) -> tuple[int, int]:
    ...
    def _insert_operations(
        self, operations: Sequence[qubitron.Operation], insertion_indices: Sequence[int]
    ) -> None:
        ...
        self._mutated()
        moment_to_ops: dict[int, list[qubitron.Operation]] = defaultdict(list)
    ...
    def insert_at_frontier(
        self, operations: qubitron.OP_TREE, start: int, frontier: dict[qubitron.Qid, int] | None = None
    ) -> dict[qubitron.Qid, int]:
    ...
    def batch_remove(self, removals: Iterable[tuple[int, qubitron.Operation]]) -> None:
    ...
    def batch_replace(
        self, replacements: Iterable[tuple[int, qubitron.Operation, qubitron.Operation]]
    ) -> None:
    ...
    def batch_insert_into(self, insert_intos: Iterable[tuple[int, qubitron.OP_TREE]]) -> None:
    ...
    def batch_insert(self, insertions: Iterable[tuple[int, qubitron.OP_TREE]]) -> None:
        ...
        All insertions are done with the strategy `qubitron.InsertStrategy.EARLIEST`.
    ...
    def clear_operations_touching(self, qubits: Iterable[qubitron.Qid], moment_indices: Iterable[int]):
    ...
    def with_noise(self, noise: qubitron.NOISE_MODEL_LIKE) -> qubitron.Circuit:
...
def _pick_inserted_ops_moment_indices(
    operations: Sequence[qubitron.Operation],
    start: int = 0,
    frontier: dict[qubitron.Qid, int] | None = None,
) -> tuple[Sequence[int], dict[qubitron.Qid, int]]:
...
def _get_moment_annotations(moment: qubitron.Moment) -> Iterator[qubitron.Operation]:
    for op in moment.operations:
        if op.qubits:
            continue
        op = op.untagged
...
def _draw_moment_annotations(
    *,
    ...
    label_map: dict[qubitron.LabelEntity, int],
    out_diagram: qubitron.TextDiagramDrawer,
    precision: int | None,
    get_circuit_diagram_info: Callable[
        [qubitron.Operation, qubitron.CircuitDiagramInfoArgs], qubitron.CircuitDiagramInfo
    ],
    include_tags: bool,
...
def _get_global_phase_and_tags_for_op(op: qubitron.Operation) -> tuple[complex | None, list[Any]]:
    if isinstance(op.gate, ops.GlobalPhaseGate):
...
def _decompose_measurement_inversions(op: qubitron.Operation) -> qubitron.OP_TREE:
...
def _group_into_moment_compatible(inputs: Sequence[_MOMENT_OR_OP]) -> Iterator[list[_MOMENT_OR_OP]]:
    ...
    batch_qubits: set[qubitron.Qid] = set()
    for mop in inputs:
        is_moment = isinstance(mop, qubitron.Moment)
...
def get_earliest_accommodating_moment_index(
    ...
    qubit_indices: dict[qubitron.Qid, int],
    mkey_indices: dict[qubitron.MeasurementKey, int],
    ckey_indices: dict[qubitron.MeasurementKey, int],
...
class _PlacementCache:
    ...
    def __init__(self) -> None:
        ...
        self._qubit_indices: dict[qubitron.Qid, int] = {}
        self._mkey_indices: dict[qubitron.MeasurementKey, int] = {}
        self._ckey_indices: dict[qubitron.MeasurementKey, int] = {}
