...
from qubitron import _compat, protocols
from qubitron.circuits import AbstractCircuit, Alignment, Circuit
...
class FrozenCircuit(AbstractCircuit, protocols.SerializableByKey):
    ...
    @classmethod
    def _from_moments(cls, moments: Iterable[qubitron.Moment]) -> FrozenCircuit:
    ...
    @cached_property
    def _all_operations(self) -> tuple[qubitron.Operation, ...]:
    ...
    def __add__(self, other) -> qubitron.FrozenCircuit:
        return (self.unfreeze() + other).freeze()

    def __radd__(self, other) -> qubitron.FrozenCircuit:
    ...
    def __mul__(self, other) -> qubitron.FrozenCircuit:
        return (self.unfreeze() * other).freeze()

    def __rmul__(self, other) -> qubitron.FrozenCircuit:
        return (other * self.unfreeze()).freeze()

    def __pow__(self, other) -> qubitron.FrozenCircuit:
        try:
            return (self.unfreeze() ** other).freeze()
        except:
    ...
    def to_op(self) -> qubitron.CircuitOperation:
