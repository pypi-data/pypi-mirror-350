...
from qubitron import _compat, ops, protocols, qis
from qubitron._import import LazyLoader
from qubitron.ops import op_tree, raw_types
...
circuit = LazyLoader("circuit", globals(), "qubitron.circuits.circuit")
op_tree = LazyLoader("op_tree", globals(), "qubitron.ops.op_tree")
text_diagram_drawer = LazyLoader(
    "text_diagram_drawer", globals(), "qubitron.circuits.text_diagram_drawer"
)


def _default_breakdown(qid: qubitron.Qid) -> tuple[Any, Any]:
...
class Moment:
    ...
    *   `moment[qubits]` returns another Moment which consists only of those
    ...
    def __init__(self, *contents: qubitron.OP_TREE, _flatten_contents: bool = True) -> None:
        ...
        self._operations = (
            ...
            else cast(tuple['qubitron.Operation'], contents)
        )
        self._sorted_operations: tuple[qubitron.Operation, ...] | None = None

        # An internal dictionary to support efficient operation access by qubit.
        self._qubit_to_op: dict[qubitron.Qid, qubitron.Operation] = {}
        ...
        self._measurement_key_objs: frozenset[qubitron.MeasurementKey] | None = None
        self._control_keys: frozenset[qubitron.MeasurementKey] | None = None

    @classmethod
    def from_ops(cls, *ops: qubitron.Operation) -> qubitron.Moment:
    ...
    @property
    def operations(self) -> tuple[qubitron.Operation, ...]:
        return self._operations

    @cached_property
    def qubits(self) -> frozenset[qubitron.Qid]:
        return frozenset(self._qubit_to_op)

    def operates_on_single_qubit(self, qubit: qubitron.Qid) -> bool:
    ...
    def operates_on(self, qubits: Iterable[qubitron.Qid]) -> bool:
    ...
    def operation_at(self, qubit: raw_types.Qid) -> qubitron.Operation | None:
    ...
    def with_operation(self, operation: qubitron.Operation) -> qubitron.Moment:
    ...
    def with_operations(self, *contents: qubitron.OP_TREE) -> qubitron.Moment:
    ...
    def without_operations_touching(self, qubits: Iterable[qubitron.Qid]) -> qubitron.Moment:
    ...
    def _resolve_parameters_(self, resolver: qubitron.ParamResolver, recursive: bool) -> qubitron.Moment:
        changed = False
        resolved_ops: list[qubitron.Operation] = []
        for op in self:
    ...
    def _sorted_operations_(self) -> tuple[qubitron.Operation, ...]:
    ...
    def __iter__(self) -> Iterator[qubitron.Operation]:
    ...
    def expand_to(self, qubits: Iterable[qubitron.Qid]) -> qubitron.Moment:
        """Returns self expanded to given superset of qubits by making identities explicit.

        Args:
            qubits: Iterable of `qubitron.Qid`s to expand this moment to.

        Returns:
            A new `qubitron.Moment` with identity operations on the new qubits
    ...
    def _kraus_(self) -> Sequence[np.ndarray]:
        ...
        def row_subscripts(qs: Sequence[qubitron.Qid]) -> str:
            return ''.join(qubit_to_row_subscript[q] for q in qs)

        def col_subscripts(qs: Sequence[qubitron.Qid]) -> str:
            return ''.join(qubit_to_col_subscript[q] for q in qs)

        def kraus_tensors(op: qubitron.Operation) -> Sequence[np.ndarray]:
    ...
    def __add__(self, other: qubitron.OP_TREE) -> qubitron.Moment:
    ...
    def __sub__(self, other: qubitron.OP_TREE) -> qubitron.Moment:
    ...
    @overload
    def __getitem__(self, key: raw_types.Qid) -> qubitron.Operation:
        pass

    @overload
    def __getitem__(self, key: Iterable[raw_types.Qid]) -> qubitron.Moment:
        pass
    ...
    def to_text_diagram(
        self: qubitron.Moment,
        *,
        xy_breakdown_func: Callable[[qubitron.Qid], tuple[Any, Any]] = _default_breakdown,
        extra_qubits: Iterable[qubitron.Qid] = (),
        ...
        Args:
            xy_breakdown_func: A function to split qubits/qudits into x and y
                ...
                `qubitron.GridQubit(row, col)` into the tuple `(col, row)` and
                `qubitron.LineQubit(x)` into `(x, 0)`.
