...
"""Defines the API for circuit transformers in Qubitron."""
...
class LogLevel(enum.Enum):
    """Different logging resolution options for `qubitron.TransformerLogger`.
...
class _LoggerNode:
    """Stores logging data of a single transformer stage in `qubitron.TransformerLogger`.
    ...
    initial_circuit: qubitron.AbstractCircuit
    final_circuit: qubitron.AbstractCircuit
...
class TransformerLogger:
    ...
    def register_initial(self, circuit: qubitron.AbstractCircuit, transformer_name: str) -> None:
    ...
    def register_final(self, circuit: qubitron.AbstractCircuit, transformer_name: str) -> None:
...
class TransformerContext:
    ...
    Args:
        logger: `qubitron.TransformerLogger` instance, which is a stateful logger used for logging
        ...
        tags_to_ignore: Tuple of tags which should be ignored while applying transformations on a
                ...
                `qubitron.VirtualTag` etc.) is a valid tag.
...
class TRANSFORMER(Protocol):
    """Protocol class defining the Transformer API for circuit transformers in Qubitron.

    Any callable that satisfies the `qubitron.TRANSFORMER` contract, i.e. takes a `qubitron.AbstractCircuit`
    and `qubitron.TransformerContext` and returns a transformed `qubitron.AbstractCircuit`, is a valid
    transformer in Qubitron.
    ...
    ...     *,
    ...     context: 'Optional[qubitron.TransformerContext]' = None,
    ...     atol: float = 1e-8,
    ... ) -> qubitron.Circuit:
    ...     ...
    ...
    def __call__(
        self, circuit: qubitron.AbstractCircuit, *, context: TransformerContext | None = None
    ) -> qubitron.AbstractCircuit: ...
...
def transformer(cls_or_func: Any = None, *, add_deep_support: bool = False) -> Any:
    ...
    A transformer is a callable that takes as inputs a `qubitron.AbstractCircuit` and
    `qubitron.TransformerContext`, and returns another `qubitron.AbstractCircuit` without
    modifying the input circuit. A transformer could be a function, for example:

    >>> @qubitron.transformer
    ... def convert_to_cz(
    ...    circuit: qubitron.AbstractCircuit, *, context: 'Optional[qubitron.TransformerContext]' = None
    ... ) -> qubitron.Circuit:
    ...    ...
    ...
    ...     context: 'Optional[qubitron.TransformerContext]' = None,
    ...     atol: float = 1e-8,
    ...     sqrt_iswap_gate: qubitron.ISwapPowGate = qubitron.SQRT_ISWAP_INV,
    ...
    Args:
        ...
        add_deep_support: If True, the decorator adds the logic to first apply the
            decorated transformer on subcircuits wrapped inside `qubitron.CircuitOperation`s
    ...
    if isinstance(cls_or_func, type):
        ...
        def method_with_logging(
            self, circuit: qubitron.AbstractCircuit, **kwargs
        ) -> qubitron.AbstractCircuit:
        ...
        def func_with_logging(circuit: qubitron.AbstractCircuit, **kwargs) -> qubitron.AbstractCircuit:
            return _transform_and_log(
                add_deep_support,
                func,
                func.__name__,
