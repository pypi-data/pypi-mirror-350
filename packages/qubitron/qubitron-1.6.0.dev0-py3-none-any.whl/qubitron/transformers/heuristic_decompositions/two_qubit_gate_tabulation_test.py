...
from qubitron.testing import assert_equivalent_repr, random_special_unitary
from qubitron.transformers.heuristic_decompositions.gate_tabulation_math_utils import (
    unitary_entanglement_fidelity,
)
from qubitron.transformers.heuristic_decompositions.two_qubit_gate_tabulation import (
...
sycamore_tabulation = two_qubit_gate_product_tabulation(
    qubitron.unitary(qubitron.FSimGate(np.pi / 2, np.pi / 6)), 0.2, random_state=_rng
)

sqrt_iswap_tabulation = two_qubit_gate_product_tabulation(
    qubitron.unitary(qubitron.FSimGate(np.pi / 4, np.pi / 24)), 0.1, random_state=_rng
)
...
def test_sycamore_gate_tabulation(seed) -> None:
    base_gate = qubitron.unitary(qubitron.FSimGate(np.pi / 2, np.pi / 6))
