...
def _is_temp(q: qubitron.Qid) -> bool:
    return isinstance(q, (ops.CleanQubit, ops.BorrowableQubit))


def map_clean_and_borrowable_qubits(
    circuit: qubitron.AbstractCircuit, *, qm: qubitron.QubitManager | None = None
) -> qubitron.Circuit:
    ...
    within the same moment.
        4. Since this is not implemented using the qubitron transformers infrastructure, we currently
    do not support recursive mapping within sub-circuits and that is left as a future TODO.

    Args:
        circuit: Input `qubitron.Circuit` containing temporarily allocated
            `CleanQubit`/`BorrowableQubit`s.
        qm: An instance of `qubitron.QubitManager` specifying the strategy to use for allocating /
            / deallocating new ancilla qubits to replace the temporary qubits.

    Returns:
        An updated `qubitron.Circuit` with all `CleanQubit`/`BorrowableQubit` mapped to either existing
    ...
    allocated_map: dict[qubitron.Qid, qubitron.Qid] = {}
    to_free: set[qubitron.Qid] = set()
    last_op_idx = -1

    def map_func(op: qubitron.Operation, idx: int) -> qubitron.OP_TREE:
