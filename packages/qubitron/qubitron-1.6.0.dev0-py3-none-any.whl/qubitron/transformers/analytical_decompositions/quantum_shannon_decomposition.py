...
from qubitron.linalg import decompositions, predicates
from qubitron.protocols import unitary_protocol
...
def quantum_shannon_decomposition(
    qubits: list[qubitron.Qid], u: np.ndarray, atol: float = 1e-8
    ...
    if not predicates.is_unitary(u, atol=atol):  # Check that u is unitary
        raise ValueError(
            "Expected input matrix u to be unitary, \
                but it fails qubitron.is_unitary check"
        )
...
def _single_qubit_decomposition(qubit: qubitron.Qid, u: np.ndarray) -> Iterable[qubitron.Operation]:
...
def _msb_demuxer(
    demux_qubits: list[qubitron.Qid], u1: np.ndarray, u2: np.ndarray
) -> Iterable[qubitron.Operation]:
...
def _multiplexed_cossin(
    cossin_qubits: list[qubitron.Qid], angles: list[float], rot_func: Callable = ops.ry
) -> Iterable[qubitron.Operation]:
