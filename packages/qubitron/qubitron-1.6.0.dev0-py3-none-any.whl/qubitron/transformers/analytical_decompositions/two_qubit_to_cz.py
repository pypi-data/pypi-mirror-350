...
from qubitron.linalg import predicates
from qubitron.linalg.decompositions import extract_right_diag, num_cnots_required
from qubitron.transformers.analytical_decompositions import single_qubit_decompositions
from qubitron.transformers.eject_phased_paulis import eject_phased_paulis
from qubitron.transformers.eject_z import eject_z
from qubitron.transformers.merge_single_qubit_gates import merge_single_qubit_gates_to_phased_x_and_z
...
def _remove_partial_czs_or_fail(
    operations: Iterable[qubitron.Operation], atol: float
...
def two_qubit_matrix_to_diagonal_and_cz_operations(
    ...
    atol: float = 1e-8,
    clean_operations: bool = True,
) -> tuple[np.ndarray, list[qubitron.Operation]]:
...
def _xx_interaction_via_full_czs(q0: qubitron.Qid, q1: qubitron.Qid, x: float):
    a = x * -2 / np.pi
    yield ops.H(q1)
    yield ops.CZ(q0, q1)
    yield ops.X(q0) ** a
...
def _xx_yy_interaction_via_full_czs(q0: qubitron.Qid, q1: qubitron.Qid, x: float, y: float):
    a = x * -2 / np.pi
    b = y * -2 / np.pi
    yield ops.X(q0) ** 0.5
    yield ops.H(q1)
...
def _xx_yy_zz_interaction_via_full_czs(q0: qubitron.Qid, q1: qubitron.Qid, x: float, y: float, z: float):
    a = x * -2 / np.pi + 0.5
    b = y * -2 / np.pi + 0.5
    c = z * -2 / np.pi + 0.5
    yield ops.X(q0) ** 0.5
...
def _parity_interaction(
    q0: qubitron.Qid, q1: qubitron.Qid, rads: float, atol: float, gate: ops.Gate | None = None
):
...
def _do_single_on(u: np.ndarray, q: qubitron.Qid, atol: float = 1e-8):
