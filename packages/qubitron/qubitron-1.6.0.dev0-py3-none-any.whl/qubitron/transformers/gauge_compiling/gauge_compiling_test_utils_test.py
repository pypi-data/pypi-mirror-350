...
import qubitron
from qubitron.transformers import ConstantGauge, GaugeSelector, GaugeTransformer
from qubitron.transformers.gauge_compiling.gauge_compiling_test_utils import GaugeTester
...
class ExampleGate(qubitron.testing.TwoQubitGate):
    unitary = qubitron.unitary(qubitron.CZ**0.123)

    def _unitary_(self) -> np.ndarray:
        return self.unitary


class ExampleSweepGate(qubitron.testing.TwoQubitGate):
    unitary = qubitron.unitary(qubitron.CZ)
...
_BAD_TRANSFORMER = GaugeTransformer(
    ...
    gauge_selector=GaugeSelector(
        gauges=[ConstantGauge(two_qubit_gate=_EXAMPLE_TARGET, pre_q0=qubitron.X)]
    ),
)
