...
from qubitron.transformers.synchronize_terminal_measurements import find_terminal_measurements
...
class _MeasurementQid(ops.Qid):
    ...
    def __init__(self, key: str | qubitron.MeasurementKey, qid: qubitron.Qid, index: int = 0):
...
def defer_measurements(
    ...
    Args:
        ...
        context: `qubitron.TransformerContext` storing common configurable options
            for transformers.
    Returns:
    ...
    Raises:
        NotImplementedError: When attempting to defer a measurement with a
            confusion map. (https://github.com/amyssnippet/Qubitron/issues/5482)
    """
    ...
    measurement_qubits: dict[qubitron.MeasurementKey, list[tuple[qubitron.Qid, ...]]] = defaultdict(list)

    def defer(op: qubitron.Operation, _) -> qubitron.OP_TREE:
...
def _all_possible_datastore_states(
    keys: Iterable[tuple[qubitron.MeasurementKey, int]],
    measurement_qubits: dict[qubitron.MeasurementKey, list[tuple[qubitron.Qid, ...]]],
) -> Iterable[qubitron.ClassicalDataStoreReader]:
...
def dephase_measurements(
    ...
    *,
    context: qubitron.TransformerContext | None = transformer_api.TransformerContext(deep=True),
) -> qubitron.Circuit:
    ...
    Raises:
        ValueError: If the circuit contains classical controls. In this case,
            ...
            `qubitron.defer_measurements` first. Since deferral adds ancilla qubits
            to the circuit, this is not done automatically, to prevent
            surprises.
    """

    def dephase(op: qubitron.Operation, _) -> qubitron.OP_TREE:
        gate = op.gate
        if isinstance(gate, ops.MeasurementGate):
            key = value.MeasurementKey.parse_serialized(gate.key)
            ...
            raise ValueError('Use qubitron.defer_measurements first to remove classical controls.')
        return op
...
def drop_terminal_measurements(
    ...
    *,
    ...
    def flip_inversion(op: qubitron.Operation, _) -> qubitron.OP_TREE:
...
def _mod_add(source: qubitron.Qid, target: qubitron.Qid) -> qubitron.Operation:
