...
def construct_device_graph_and_mapping():
    device_graph = nx.Graph(
        [
            (qubitron.NamedQubit("a"), qubitron.NamedQubit("b")),
            (qubitron.NamedQubit("b"), qubitron.NamedQubit("c")),
            (qubitron.NamedQubit("c"), qubitron.NamedQubit("d")),
            (qubitron.NamedQubit("a"), qubitron.NamedQubit("e")),
            (qubitron.NamedQubit("e"), qubitron.NamedQubit("d")),
        ]
    )
    q = qubitron.LineQubit.range(5)
    initial_mapping = {
        q[1]: qubitron.NamedQubit("a"),
        q[3]: qubitron.NamedQubit("b"),
        q[2]: qubitron.NamedQubit("c"),
        q[4]: qubitron.NamedQubit("d"),
    }
...
def test_induced_subgraph():
    ...
    mm = qubitron.MappingManager(device_graph, initial_mapping)
...
def test_mapped_op():
    ...
    assert mm.mapped_op(qubitron.CNOT(q[1], q[3])).qubits == (
        qubitron.NamedQubit("a"),
        qubitron.NamedQubit("b"),
    )
    # does not fail if qubits non-adjacent
    assert mm.mapped_op(qubitron.CNOT(q[3], q[4])).qubits == (
        qubitron.NamedQubit("b"),
        qubitron.NamedQubit("d"),
    )

    # correctly changes mapped qubits when swapped
    mm.apply_swap(q_int[2], q_int[3])
    assert mm.mapped_op(qubitron.CNOT(q[1], q[2])).qubits == (
        qubitron.NamedQubit("a"),
        qubitron.NamedQubit("b"),
    )
...
def test_distance_on_device_and_is_adjacent():
    ...
    # 'dist_on_device' does not use qubitron.NamedQubit("e") to find shorter shortest path
