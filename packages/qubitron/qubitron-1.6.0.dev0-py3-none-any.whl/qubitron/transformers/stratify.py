...
from qubitron import _import, circuits, ops, protocols
from qubitron.transformers import transformer_api

drop_empty_moments = _import.LazyLoader('drop_empty_moments', globals(), 'qubitron.transformers')
...
Classifier = Callable[['qubitron.Operation'], bool]

# Any of the possible operation categories that we can stratify on.
Category = Union[
    'qubitron.Gate', 'qubitron.Operation', type['qubitron.Gate'], type['qubitron.Operation'], Classifier
]
    ...
    *,
    ...
    Args:
        ...
        categories: A list of classifiers picking out certain operations. There are several ways
            to specify a classifier. You can pass in a gate instance (e.g. `qubitron.X`),
            a gate type (e.g. `qubitron.XPowGate`), an operation instance
            (e.g. `qubitron.X(qubitron.LineQubit(0))`), an operation type (e.g.`qubitron.CircuitOperation`),
...
def _stratify_circuit(
    ...
    *,
    ...
    qubit_time_index: dict[qubitron.Qid, int] = {}
    measurement_time_index: dict[qubitron.MeasurementKey, int] = {}
    control_time_index: dict[qubitron.MeasurementKey, int] = {}
...
def _category_to_classifier(category) -> Classifier:
    ...
    if isinstance(category, ops.Operation):
        ...
        raise TypeError(
            ...
            f'Expected a qubitron.Gate, qubitron.Operation, '
            f'Type[qubitron.Gate], Type[qubitron.Operation], '
            f'or Callable[[qubitron.Operation], bool].'
        )


def _mock_classifier(op: qubitron.Operation) -> bool:
    """Mock classifier, used to "complete" a collection of classifiers and make it exhaustive."""
    return False  # pragma: no cover


def _get_op_class(op: qubitron.Operation, classifiers: Sequence[Classifier]) -> int:
