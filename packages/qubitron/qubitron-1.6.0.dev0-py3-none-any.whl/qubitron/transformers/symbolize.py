...
def symbolize_single_qubit_gates_by_indexed_tags(
    ...
    *,
    ...
    Example:
        >>> from qubitron import transformers
        >>> q0, q1 = qubitron.LineQubit.range(2)
        >>> c = qubitron.Circuit(
        ...         qubitron.X(q0).with_tags("phxz_0"),
        ...         qubitron.CZ(q0,q1),
        ...         qubitron.Y(q0).with_tags("phxz_1"),
        ...         qubitron.X(q0))
        >>> print(c)
        0: ───X[phxz_0]───@───Y[phxz_1]───X───
                          │
        1: ───────────────@───────────────────
        >>> new_circuit = qubitron.symbolize_single_qubit_gates_by_indexed_tags(
    ...
    def _map_func(op: qubitron.Operation, _):
