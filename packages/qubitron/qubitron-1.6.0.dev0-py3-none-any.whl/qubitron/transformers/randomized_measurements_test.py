...
import qubitron
import qubitron.transformers.randomized_measurements as rand_meas


def test_randomized_measurements_appends_two_moments_on_returned_circuit() -> None:
    # Create a 4-qubit circuit
    q0, q1, q2, q3 = qubitron.LineQubit.range(4)
    circuit_pre = qubitron.Circuit(
        [qubitron.H(q0), qubitron.CNOT(q0, q1), qubitron.CNOT(q1, q2), qubitron.CNOT(q2, q3)]
    )
...
def test_append_randomized_measurements_leaves_qubits_not_in_specified_subsystem_unchanged() -> (
    ...
    # Create a 4-qubit circuit
    q0, q1, q2, q3 = qubitron.LineQubit.range(4)
    circuit = qubitron.Circuit([qubitron.H(q0), qubitron.CNOT(q0, q1), qubitron.CNOT(q1, q2), qubitron.CNOT(q2, q3)])
