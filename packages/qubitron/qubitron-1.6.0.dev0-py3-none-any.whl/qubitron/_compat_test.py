...
def test_with_debug():
    assert qubitron.__qubitron_debug__.get()
    with qubitron.with_debug(False):
        assert not qubitron.__qubitron_debug__.get()
        with qubitron.with_debug(True):
            assert qubitron.__qubitron_debug__.get()
        assert not qubitron.__qubitron_debug__.get()
    assert qubitron.__qubitron_debug__.get()
...
def test_dataclass_repr_simple() -> None:
    ...
    assert repr(TestClass1(5, 'hi')) == "qubitron.TestClass1(x=5, y='hi')"
...
def test_dataclass_repr_numpy() -> None:
    ...
    class TestClass2:
        ...
        def __repr__(self):
            return dataclass_repr(self, namespace='qubitron.testing')

    tc = TestClass2(np.ones(3))
    assert (
        repr(tc)
        == "qubitron.testing.TestClass2(x=np.array([1.0, 1.0, 1.0], dtype=np.dtype('float64')))"
    )
...
def test_deprecated_with_name():
    ...
    with qubitron.testing.assert_deprecated(
        '_compat_test.py:',
        'test_func was used',
        'will be removed in qubitron v1.2',
        'Roll some dice.',
        deadline='v1.2',
    ):
        assert f(1, 2) == 3
...
def test_deprecated():
    ...
    with pytest.raises(
        ValueError, match='During testing using Qubitron deprecated functionality is not allowed'
    ):
        old_func(1, 2)
...
def test_deprecated_parameter():
    ...
    with qubitron.testing.assert_logs(count=0):
        assert f(1) == 1
...
def test_wrap_module():
    ...
    with qubitron.testing.assert_deprecated(
        ...
        'will be removed in qubitron v0.6',
        'use bar instead',
        deadline='v0.6',
    ):
        _ = wrapped.foo
...
def _test_deprecate_attributes_assert_attributes_in_sys_modules():
    ...
    import qubitron.testing._compat_test_data.module_a as module_a0

    module_a1 = deprecate_attributes(
        'qubitron.testing._compat_test_data.module_a',
        {'MODULE_A_ATTRIBUTE': ('v0.6', 'use plain string instead')},
    )

    assert module_a1 is not module_a0
    assert module_a1 is qubitron.testing._compat_test_data.module_a
    assert module_a1 is sys.modules['qubitron.testing._compat_test_data.module_a']
...
def _from_parent_import_deprecated():
    from qubitron.testing._compat_test_data import fake_a

    assert fake_a.MODULE_A_ATTRIBUTE == 'module_a'


def _import_deprecated_assert_sub():
    import qubitron.testing._compat_test_data.fake_a  # type: ignore

    assert qubitron.testing._compat_test_data.fake_a.module_b.MODULE_B_ATTRIBUTE == 'module_b'


def _from_deprecated_import_sub():
    from qubitron.testing._compat_test_data.fake_a import module_b
...
def _import_deprecated_first_new_second():
    ...
    assert 'qubitron.testing._compat_test_data.fake_a' in sys.modules
    assert 'qubitron.testing._compat_test_data.module_a' in sys.modules

    assert (
        sys.modules['qubitron.testing._compat_test_data.fake_a']
        == sys.modules['qubitron.testing._compat_test_data.module_a']
    )

    from qubitron.testing._compat_test_data.module_a import module_b
...
def _import_new_first_deprecated_second():
    ...
    assert 'qubitron.testing._compat_test_data.fake_a' not in sys.modules
    assert 'qubitron.testing._compat_test_data.module_a' in sys.modules
...
def _from_deprecated_import_sub_of_sub():
    ...
    from qubitron.testing._compat_test_data.module_a.module_b import module_c

    assert module_c.MODULE_C_ATTRIBUTE == 'module_c'
    from qubitron.testing._compat_test_data.fake_a.module_b import module_c  # type: ignore
...
def _import_multiple_deprecated():
    ...
    assert module_c.MODULE_C_ATTRIBUTE == 'module_c'
    from qubitron.testing._compat_test_data.fake_a.module_b import module_c

    assert module_c.MODULE_C_ATTRIBUTE == 'module_c'
    from qubitron.testing._compat_test_data.fake_b import module_c  # type: ignore
...
def _deprecate_grandchild_assert_attributes_in_sys_modules():
    ...
    import qubitron.testing._compat_test_data.module_a.fake_ab  # type: ignore

    assert (
        qubitron.testing._compat_test_data.module_a.fake_ab
        is sys.modules['qubitron.testing._compat_test_data.module_a.fake_ab']
    )
    assert (
        qubitron.testing._compat_test_data.module_a
        is sys.modules['qubitron.testing._compat_test_data.module_a']
    )
    assert qubitron.testing._compat_test_data is sys.modules['qubitron.testing._compat_test_data']


def _new_module_in_different_parent():
    from qubitron.testing._compat_test_data.fake_ops import raw_types  # type: ignore

    assert raw_types.Qid == qubitron.Qid
...
def _find_spec_deprecated_multiple_times():
    ...
    assert importlib.util.find_spec('qubitron.testing._compat_test_data.fake_a')
...
def _import_parent_use_constant_from_deprecated_module_attribute():
    ...
    # sets up the DeprecationFinders
    import qubitron.testing._compat_test_data

    # the parent should have fake_a set on it as an attribute - just like
    # a regular module import (e.g. qubitron.ops)
    # should have a DUPE_CONSTANT as its imported from the dupe submodule
    assert qubitron.testing._compat_test_data.fake_a.DUPE_CONSTANT is False

    assert 'module_a for module deprecation tests' in qubitron.testing._compat_test_data.fake_a.__doc__
    assert 'Test module for deprecation testing' in qubitron.testing._compat_test_data.__doc__


def _import_deprecated_sub_use_constant():
    """to ensure that submodule initializations set attributes correctly"""
    # sets up the DeprecationFinders
    import qubitron.testing._compat_test_data.fake_a.dupe  # type: ignore

    # should have a DUPE_CONSTANT as its defined on it, set to False
    assert qubitron.testing._compat_test_data.fake_a.dupe.DUPE_CONSTANT is False


def _import_deprecated_same_name_in_earlier_subtree():
    from qubitron.testing._compat_test_data.fake_a.sub.subsub.dupe import DUPE_CONSTANT  # type: ignore
...
def _import_top_level_deprecated():
    ...
    from qubitron.testing._compat_test_data.fake_freezegun import api  # type: ignore
...
def _repeated_import_path():
    ...
    from qubitron.testing._compat_test_data.repeated_child.qubitron.testing._compat_test_data.repeated_child import (  # type: ignore
        child,
    )
...
def _type_repr_in_deprecated_module():
    ...
    import qubitron.testing._compat_test_data.fake_a as mod_a

    expected_repr = "<class 'qubitron.testing._compat_test_data.module_a.types.SampleType'>"
    assert repr(mod_a.SampleType) == expected_repr


old_parent = 'qubitron.testing._compat_test_data'
...
# see qubitron_compat_test_data/__init__.py for the setup code
...
def run_in_subprocess(test_func, *args):
    ...
    # See https://github.com/amyssnippet/Qubitron/issues/6373
...
def _test_deprecated_module_inner(outdated_method, deprecation_messages):
    ...
    with qubitron.testing.assert_logs(
...
def test_same_name_submodule_earlier_in_subtree():
    ...
    We have two packages with a bool `DUPE_CONSTANT` attribute each:
       1. qubitron.testing._compat_test_data.module_a.sub.dupe.DUPE_CONSTANT=True # the right one
       2. qubitron.testing._compat_test_data.module_a.dupe.DUPE_CONSTANT=False # the wrong one

    If the new module's (in this case qubitron.testing._compat_test_data.module_a) path has precedence
    during module spec resolution, dupe number 2 is going to get resolved.

    You might wonder where this comes up in qubitron. There was a bug where the lookup path was not in
    the right order. The motivating example is qubitron.ops.calibration vs the
    qubitron.ops.engine.calibration packages. The wrong resolution resulted in false circular
    imports!
    """
...
def _test_same_name_submodule_earlier_in_subtree_inner():
    from qubitron.testing._compat_test_data.module_a.sub.subsub.dupe import DUPE_CONSTANT
...
def _test_metadata_distributions_after_deprecated_submodule():
    ...
    # See https://github.com/amyssnippet/Qubitron/issues/4729
    deprecated_submodule(
        new_module_name='qubitron.neutral_atoms',
        old_parent='qubitron',
...
def _test_parent_spec_after_deprecated_submodule():
    ...
    assert qubitron.testing._compat_test_data.__spec__.name == 'qubitron.testing._compat_test_data'
...
def _test_type_repr_in_new_module_inner():
    # initialize the DeprecatedModuleFinders
    # pylint: disable=unused-import
    import qubitron.testing._compat_test_data.module_a as mod_a
...
def _test_broken_module_1_inner():
    with pytest.raises(
        ...
        # pylint: disable=unused-import
        import qubitron.testing._compat_test_data.broken_ref as br  # type: ignore


def _test_broken_module_2_inner():
    warnings.simplefilter('always')
    with qubitron.testing.assert_deprecated(deadline="v0.20", count=None):
        with pytest.raises(
            DeprecatedModuleImportError,
            match="missing_module cannot be imported. The typical reasons",
        ):
            # note that this passes
            from qubitron.testing._compat_test_data import broken_ref
...
def _test_broken_module_3_inner():
    ...
    with qubitron.testing.assert_deprecated(deadline="v0.20", count=None):
        with pytest.raises(
            ...
            qubitron.testing._compat_test_data.broken_ref.something()
...
def _test_import_deprecated_with_no_attribute_inner():
    ...
    assert not hasattr(qubitron.testing._compat_test_data, 'fake_b')
...
def _dir_is_still_valid_inner():
    ...
    import qubitron.testing._compat_test_data as mod
...
def test_block_overlapping_deprecation():
    ...
    with qubitron.testing.assert_deprecated('f', deadline='v1000.0', count=1):
        f(5)
