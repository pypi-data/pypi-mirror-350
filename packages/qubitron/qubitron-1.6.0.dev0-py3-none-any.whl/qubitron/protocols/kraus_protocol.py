...
from qubitron.protocols.decompose_protocol import _try_decompose_into_operations_and_qubits
from qubitron.protocols.mixture_protocol import has_mixture
from qubitron.protocols.unitary_protocol import unitary
...
class SupportsKraus(Protocol):
    ...
    def _kraus_(self) -> Sequence[np.ndarray] | NotImplementedType:
        ...
        This method is used by the global `qubitron.channel` method. If this method
        ...
        (resulting in a TypeError or default result when calling `qubitron.channel`
    ...
    def _has_kraus_(self) -> bool:
        ...
        This method is used by the global `qubitron.has_channel` method.  If this
        method is not present, or returns NotImplemented, it will fallback
        to similarly checking `qubitron.has_mixture` or `qubitron.has_unitary`. If none
        of these are present or return NotImplemented, then `qubitron.has_channel`
        will fall back to checking whether `qubitron.channel` has a non-default
        value. Otherwise `qubitron.has_channel` returns False.
...
def kraus(
    ...
    if channel_getter is not None:
        warnings.warn(  # pragma: no cover
            '_channel_ is deprecated and will be removed in qubitron 0.13, rename to _kraus_',
            DeprecationWarning,
        )
