...
def test_objects_with_no_inverse(val) -> None:
    with pytest.raises(TypeError, match="isn't invertible"):
        _ = qubitron.inverse(val)
    assert qubitron.inverse(val, None) is None
    assert qubitron.inverse(val, NotImplemented) is NotImplemented
    assert qubitron.inverse(val, 5) == 5
...
def test_objects_with_inverse(val, inv) -> None:
    assert qubitron.inverse(val) == inv
    assert qubitron.inverse(val, 0) == inv
