...
def _measurement_key_objs_from_magic_methods(
    val: Any,
) -> frozenset[qubitron.MeasurementKey] | NotImplementedType | None:
...
def measurement_key_objs(val: Any) -> frozenset[qubitron.MeasurementKey]:
...
def is_measurement(val: Any) -> bool:
    ...
    or `qubitron.measurement_keys` returns a non-empty result for them.
...
def with_rescoped_keys(
    val: Any, path: tuple[str, ...], bindable_keys: frozenset[qubitron.MeasurementKey] | None = None
):
