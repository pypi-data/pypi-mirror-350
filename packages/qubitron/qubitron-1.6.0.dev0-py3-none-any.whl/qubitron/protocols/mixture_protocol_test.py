...
class ReturnsValidTuple(qubitron.SupportsMixture):
    def _mixture_(self):
        return ((0.4, a), (0.6, b))
...
def test_objects_with_no_mixture(val):
    with pytest.raises(TypeError, match="mixture"):
        _ = qubitron.mixture(val)
    assert qubitron.mixture(val, None) is None
    assert qubitron.mixture(val, NotImplemented) is NotImplemented
    default = ((0.4, a), (0.6, b))
    assert qubitron.mixture(val, default) == default


def test_has_mixture():
    assert qubitron.has_mixture(ReturnsValidTuple())
    assert not qubitron.has_mixture(ReturnsNotImplemented())
    assert qubitron.has_mixture(ReturnsMixtureButNoHasMixture())
    assert qubitron.has_mixture(ReturnsUnitary())
    assert not qubitron.has_mixture(ReturnsNotImplementedUnitary())


def test_valid_mixture():
    qubitron.validate_mixture(ReturnsValidTuple())
