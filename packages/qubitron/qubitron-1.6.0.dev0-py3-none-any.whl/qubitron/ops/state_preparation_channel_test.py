# Copyright 2021 The Qubitron Developers
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
#
#     https://www.apache.org/licenses/LICENSE-2.0
#
# Unless required by applicable law or agreed to in writing, software
# distributed under the License is distributed on an "AS IS" BASIS,
# WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
# See the License for the specific language governing permissions and
# limitations under the License.

from __future__ import annotations

import numpy as np
import pytest

import qubitron


@pytest.mark.parametrize(
    'state',
    np.array(
        [
            [1, 0, 0, 0],
            [1, 0, 0, 1],
            [3, 5, 2, 7],
            [0.7823, 0.12323, 0.4312, 0.12321],
            [23, 43, 12, 19],
            [1j, 0, 0, 0],
            [1j, 0, 0, 1j],
            [1j, -1j, -1j, 1j],
            [1 + 1j, 0, 0, 0],
            [1 + 1j, 0, 1 + 1j, 0],
            [3 + 1j, 5 + 8j, 21, 0.85j],
        ]
    ),
)
def test_state_prep_channel_kraus(state) -> None:
    qubits = qubitron.LineQubit.range(2)
    gate = qubitron.StatePreparationChannel(state)(qubits[0], qubits[1])
    qubitron.testing.assert_consistent_channel(gate)
    assert not qubitron.has_mixture(gate)
    state = state / np.linalg.norm(state)
    np.testing.assert_almost_equal(
        qubitron.kraus(gate),
        (
            np.array([state, np.zeros(4), np.zeros(4), np.zeros(4)]).T,
            np.array([np.zeros(4), state, np.zeros(4), np.zeros(4)]).T,
            np.array([np.zeros(4), np.zeros(4), state, np.zeros(4)]).T,
            np.array([np.zeros(4), np.zeros(4), np.zeros(4), state]).T,
        ),
    )


def test_state_prep_channel_kraus_small() -> None:
    gate = qubitron.StatePreparationChannel(np.array([0.0, 1.0]))(qubitron.LineQubit(0))
    np.testing.assert_almost_equal(
        qubitron.kraus(gate), (np.array([[0.0, 0.0], [1.0, 0.0]]), np.array([[0.0, 0.0], [0.0, 1.0]]))
    )
    assert qubitron.has_kraus(gate)
    assert not qubitron.has_mixture(gate)

    gate = qubitron.StatePreparationChannel(np.array([1.0, 0.0]))(qubitron.LineQubit(0))
    np.testing.assert_almost_equal(
        qubitron.kraus(gate), (np.array([[1.0, 0.0], [0.0, 0.0]]), np.array([[0.0, 1.0], [0.0, 0.0]]))
    )
    assert qubitron.has_kraus(gate)
    assert not qubitron.has_mixture(gate)


def test_state_prep_gate_printing() -> None:
    circuit = qubitron.Circuit()
    qubits = qubitron.LineQubit.range(2)
    gate = qubitron.StatePreparationChannel(np.array([1, 0, 0, 1]) / np.sqrt(2))
    circuit.append(qubitron.H(qubits[0]))
    circuit.append(qubitron.CNOT(qubits[0], qubits[1]))
    circuit.append(gate(qubits[0], qubits[1]))
    qubitron.testing.assert_has_diagram(
        circuit,
        """
0: ───H───@───StatePreparation[1]───
          │   │
1: ───────X───StatePreparation[2]───
""",
    )


@pytest.mark.parametrize('name', ['Prep', 'S'])
def test_state_prep_gate_printing_with_name(name) -> None:
    circuit = qubitron.Circuit()
    qubits = qubitron.LineQubit.range(2)
    gate = qubitron.StatePreparationChannel(np.array([1, 0, 0, 1]) / np.sqrt(2), name=name)
    circuit.append(qubitron.H(qubits[0]))
    circuit.append(qubitron.CNOT(qubits[0], qubits[1]))
    circuit.append(gate(qubits[0], qubits[1]))
    qubitron.testing.assert_has_diagram(
        circuit,
        f"""
0: ───H───@───{name}[1]───
          │   │
1: ───────X───{name}[2]───
""",
    )


def test_gate_params() -> None:
    state = np.array([1, 0, 0, 0], dtype=np.complex64)
    gate = qubitron.StatePreparationChannel(state)
    assert gate.num_qubits() == 2
    assert not gate._has_unitary_()
    assert gate._has_kraus_()
    assert str(gate) == 'StatePreparationChannel([1.+0.j 0.+0.j 0.+0.j 0.+0.j])'
    qubitron.testing.assert_equivalent_repr(gate)


def test_gate_error_handling() -> None:
    with pytest.raises(ValueError, match='`target_state` must be a 1d numpy array.'):
        qubitron.StatePreparationChannel(np.eye(2))
    with pytest.raises(ValueError, match='Matrix width \\(5\\) is not a power of 2'):
        qubitron.StatePreparationChannel(np.ones(shape=5))


def test_equality_of_gates() -> None:
    state = np.array([1, 0, 0, 0], dtype=np.complex64)
    gate_1 = qubitron.StatePreparationChannel(state)
    gate_2 = qubitron.StatePreparationChannel(state)
    assert gate_1 == gate_2, "Equal state not leading to same gate"
    assert not gate_1 == state, "Incompatible objects shouldn't be equal"
    state = np.array([0, 1, 0, 0], dtype=np.complex64)
    gate_3 = qubitron.StatePreparationChannel(state, name='gate_a')
    gate_4 = qubitron.StatePreparationChannel(state, name='gate_b')
    assert gate_3 == gate_4, "Equal state with different names not leading to same gate"
    assert gate_1 != gate_3, "Different states shouldn't lead to same gate"


def test_approx_equality_of_gates() -> None:
    state = np.array([1, 0, 0, 0], dtype=np.complex64)
    gate_1 = qubitron.StatePreparationChannel(state)
    gate_2 = qubitron.StatePreparationChannel(state)
    assert qubitron.approx_eq(gate_1, gate_2), "Equal state not leading to same gate"
    assert not qubitron.approx_eq(gate_1, state), "Different object types cannot be approx equal"
    perturbed_state = np.array([1 - 1e-9, 1e-10, 0, 0], dtype=np.complex64)
    gate_3 = qubitron.StatePreparationChannel(perturbed_state)
    assert qubitron.approx_eq(gate_3, gate_1), "Almost equal states should lead to the same gate"
    different_state = np.array([1 - 1e-5, 1e-4, 0, 0], dtype=np.complex64)
    gate_4 = qubitron.StatePreparationChannel(different_state)
    assert not qubitron.approx_eq(gate_4, gate_1), "Different states should not lead to the same gate"
    assert qubitron.approx_eq(
        gate_4, gate_1, atol=1e-3
    ), "Gates with difference in states under the tolerance aren't equal"
    assert not qubitron.approx_eq(
        gate_4, gate_1, atol=1e-6
    ), "Gates with difference in states over the tolerance are equal"
