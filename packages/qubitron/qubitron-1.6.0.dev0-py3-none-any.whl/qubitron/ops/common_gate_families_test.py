...
import qubitron
from qubitron.ops.gateset_test import CustomX, CustomXPowGate


class UnitaryGate(qubitron.Gate):
...
def test_any_unitary_gate_family():
    with pytest.raises(ValueError, match='must be a positive integer'):
        _ = qubitron.AnyUnitaryGateFamily(0)
    eq = qubitron.testing.EqualsTester()
    eq.add_equality_group(qubitron.AnyUnitaryGateFamily())
    for num_qubits in range(1, 6, 2):
        q = qubitron.LineQubit.range(num_qubits)
        gate = UnitaryGate(num_qubits)
        eq.add_equality_group(qubitron.AnyUnitaryGateFamily(num_qubits))
        for init_num_qubits in [None, num_qubits]:
            gate_family = qubitron.AnyUnitaryGateFamily(init_num_qubits)
            qubitron.testing.assert_equivalent_repr(gate_family)
    ...
    assert qubitron.testing.SingleQubitGate() not in qubitron.AnyUnitaryGateFamily()


def test_any_integer_power_gate_family():
    with pytest.raises(ValueError, match='subclass of `qubitron.EigenGate`'):
        qubitron.AnyIntegerPowerGateFamily(gate=qubitron.testing.SingleQubitGate)
    with pytest.raises(ValueError, match='subclass of `qubitron.EigenGate`'):
        qubitron.AnyIntegerPowerGateFamily(gate=CustomXPowGate())
    eq = qubitron.testing.EqualsTester()
    gate_family = qubitron.AnyIntegerPowerGateFamily(CustomXPowGate)
    eq.add_equality_group(gate_family)
    eq.add_equality_group(qubitron.AnyIntegerPowerGateFamily(qubitron.EigenGate))
    qubitron.testing.assert_equivalent_repr(gate_family)
...
@pytest.mark.parametrize('gate', [CustomX, qubitron.ParallelGate(CustomX, 2), CustomXPowGate])
...
def test_parallel_gate_family(gate, name, description, max_parallel_allowed):
    gate_family = qubitron.ParallelGateFamily(
        gate, name=name, description=description, max_parallel_allowed=max_parallel_allowed
    )
    qubitron.testing.assert_equivalent_repr(gate_family)
    for gate_to_test in [CustomX, qubitron.ParallelGate(CustomX, 2)]:
        assert gate_to_test in gate_family
        assert gate_to_test(*qubitron.LineQubit.range(qubitron.num_qubits(gate_to_test))) in gate_family

    if isinstance(gate, qubitron.ParallelGate) and not max_parallel_allowed:
        assert gate_family._max_parallel_allowed == qubitron.num_qubits(gate)
        assert qubitron.ParallelGate(CustomX, 4) not in gate_family
    else:
        assert gate_family._max_parallel_allowed == max_parallel_allowed
        assert (qubitron.ParallelGate(CustomX, 4) in gate_family) == (max_parallel_allowed is None)
...
def test_parallel_gate_family_eq():
    ...
    for name, description in [(None, None), ("Custom Name", "Custom Description")]:
        eq.add_equality_group(
            qubitron.ParallelGateFamily(
                CustomX, max_parallel_allowed=2, name=name, description=description
            ),
            qubitron.ParallelGateFamily(
                qubitron.ParallelGate(CustomX, 2), name=name, description=description
            ),
        )
        eq.add_equality_group(
            qubitron.ParallelGateFamily(
        ...
        eq.add_equality_group(
            ...
            qubitron.ParallelGateFamily(
                qubitron.ParallelGate(CustomX, 10),
