# pylint: disable=wrong-or-nonexistent-copyright-notice

from __future__ import annotations

import numpy as np
import pytest

import qubitron


def test_matrix_mixture_from_mixture() -> None:
    q0 = qubitron.LineQubit(0)
    dp = qubitron.depolarize(0.1)
    mm = qubitron.MixedUnitaryChannel.from_mixture(dp, key='dp')
    assert qubitron.measurement_key_name(mm) == 'dp'
    qubitron.testing.assert_consistent_channel(mm)
    qubitron.testing.assert_consistent_mixture(mm)

    circuit = qubitron.Circuit(mm.on(q0))
    sim = qubitron.Simulator(seed=0)

    results = sim.simulate(circuit)
    assert 'dp' in results.measurements
    # The depolarizing channel is composed of four unitaries.
    assert results.measurements['dp'] in range(4)


def test_matrix_mixture_equality() -> None:
    dp_pt1 = qubitron.depolarize(0.1)
    dp_pt2 = qubitron.depolarize(0.2)
    mm_a1 = qubitron.MixedUnitaryChannel.from_mixture(dp_pt1, key='a')
    mm_a2 = qubitron.MixedUnitaryChannel.from_mixture(dp_pt2, key='a')
    mm_b1 = qubitron.MixedUnitaryChannel.from_mixture(dp_pt1, key='b')

    # Even if their effect is the same, MixedUnitaryChannels are not treated
    # as equal to other channels defined in Qubitron.
    assert mm_a1 != dp_pt1
    assert mm_a1 != mm_a2
    assert mm_a1 != mm_b1
    assert mm_a2 != mm_b1

    mix = [(0.5, np.array([[1, 0], [0, 1]])), (0.5, np.array([[0, 1], [1, 0]]))]
    half_flip = qubitron.MixedUnitaryChannel(mix)
    mix_inv = list(reversed(mix))
    half_flip_inv = qubitron.MixedUnitaryChannel(mix_inv)
    # Even though these have the same effect on the circuit, their measurement
    # behavior differs, so they are considered non-equal.
    assert half_flip != half_flip_inv


def test_matrix_mixture_remap_keys() -> None:
    dp = qubitron.depolarize(0.1)
    mm = qubitron.MixedUnitaryChannel.from_mixture(dp)
    with pytest.raises(TypeError):
        _ = qubitron.measurement_key_name(mm)
    assert qubitron.with_measurement_key_mapping(mm, {'a': 'b'}) is NotImplemented

    mm_x = qubitron.MixedUnitaryChannel.from_mixture(dp, key='x')
    assert qubitron.with_measurement_key_mapping(mm_x, {'a': 'b'}) is mm_x
    assert qubitron.measurement_key_name(qubitron.with_key_path(mm_x, ('path',))) == 'path:x'
    assert qubitron.measurement_key_name(qubitron.with_key_path_prefix(mm_x, ('path',))) == 'path:x'

    mm_a = qubitron.MixedUnitaryChannel.from_mixture(dp, key='a')
    mm_b = qubitron.MixedUnitaryChannel.from_mixture(dp, key='b')
    assert mm_a != mm_b
    assert qubitron.with_measurement_key_mapping(mm_a, {'a': 'b'}) == mm_b


def test_matrix_mixture_from_unitaries() -> None:
    q0 = qubitron.LineQubit(0)
    mix = [(0.5, np.array([[1, 0], [0, 1]])), (0.5, np.array([[0, 1], [1, 0]]))]
    half_flip = qubitron.MixedUnitaryChannel(mix, key='flip')
    assert qubitron.measurement_key_name(half_flip) == 'flip'

    circuit = qubitron.Circuit(half_flip.on(q0), qubitron.measure(q0, key='m'))
    sim = qubitron.Simulator(seed=0)

    results = sim.simulate(circuit)
    assert 'flip' in results.measurements
    assert results.measurements['flip'] == results.measurements['m']


def test_matrix_mixture_str() -> None:
    mix = [(0.5, np.array([[1, 0], [0, 1]])), (0.5, np.array([[0, 1], [1, 0]]))]
    half_flip = qubitron.MixedUnitaryChannel(mix)
    assert (
        str(half_flip)
        == """MixedUnitaryChannel([(0.5, array([[1, 0],
       [0, 1]])), (0.5, array([[0, 1],
       [1, 0]]))])"""
    )
    half_flip_keyed = qubitron.MixedUnitaryChannel(mix, key='flip')
    assert (
        str(half_flip_keyed)
        == """MixedUnitaryChannel([(0.5, array([[1, 0],
       [0, 1]])), (0.5, array([[0, 1],
       [1, 0]]))], key=flip)"""
    )


def test_matrix_mixture_repr() -> None:
    mix = [
        (0.5, np.array([[1, 0], [0, 1]], dtype=np.dtype('complex64'))),
        (0.5, np.array([[0, 1], [1, 0]], dtype=np.dtype('complex64'))),
    ]
    half_flip = qubitron.MixedUnitaryChannel(mix, key='flip')
    assert (
        repr(half_flip)
        == """\
qubitron.MixedUnitaryChannel(mixture=[\
(0.5, np.array([[(1+0j), 0j], [0j, (1+0j)]], dtype=np.dtype('complex64'))), \
(0.5, np.array([[0j, (1+0j)], [(1+0j), 0j]], dtype=np.dtype('complex64')))], \
key='flip')"""
    )


def test_mix_no_unitaries_fails() -> None:
    with pytest.raises(ValueError, match='must have at least one unitary'):
        _ = qubitron.MixedUnitaryChannel(mixture=[], key='m')


def test_mix_bad_prob_fails() -> None:
    mix = [(0.5, np.array([[1, 0], [0, 0]]))]

    with pytest.raises(ValueError, match='Unitary probabilities must sum to 1'):
        _ = qubitron.MixedUnitaryChannel(mixture=mix, key='m')


def test_mix_mismatch_fails() -> None:
    op2 = np.zeros((4, 4))
    op2[1][1] = 1
    mix = [(0.5, np.array([[1, 0], [0, 0]])), (0.5, op2)]

    with pytest.raises(ValueError, match='Inconsistent unitary shapes'):
        _ = qubitron.MixedUnitaryChannel(mixture=mix, key='m')


def test_nonqubit_mixture_fails() -> None:
    mix = [(0.5, np.array([[1, 0, 0], [0, 1, 0]])), (0.5, np.array([[0, 1, 0], [1, 0, 0]]))]

    with pytest.raises(ValueError, match='Input mixture'):
        _ = qubitron.MixedUnitaryChannel(mixture=mix, key='m')


def test_validate() -> None:
    mix = [(0.5, np.array([[1, 0], [0, 0]])), (0.5, np.array([[0, 0], [0, 1]]))]
    with pytest.raises(ValueError, match='non-unitary'):
        _ = qubitron.MixedUnitaryChannel(mixture=mix, key='m', validate=True)
