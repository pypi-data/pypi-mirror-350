...
from qubitron.ops import named_qubit, qid_util, qubit_manager
...
class GreedyQubitManager(qubit_manager.QubitManager):
    ...
    def __init__(self, prefix: str, *, size: int = 0, maximize_reuse: bool = False):
        ...
        Args:
            prefix: The prefix to use for naming new clean ancillas allocated by the qubit manager.
                    The i'th allocated qubit is of the type `qubitron.NamedQubit(f'{prefix}_{i}')`.
        ...
        """
        self._prefix = prefix
        self._used_qubits: set[qubitron.Qid] = set()
        self._free_qubits: list[qubitron.Qid] = []
        self._size = 0
        self.maximize_reuse = maximize_reuse
        self.resize(size)

    def _allocate_qid(self, name: str, dim: int) -> qubitron.Qid:
    ...
    def qalloc(self, n: int, dim: int = 2) -> list[qubitron.Qid]:
        if not n:
            return []
    ...
    def qfree(self, qubits: Iterable[qubitron.Qid]) -> None:
    ...
    def qborrow(self, n: int, dim: int = 2) -> list[qubitron.Qid]:
