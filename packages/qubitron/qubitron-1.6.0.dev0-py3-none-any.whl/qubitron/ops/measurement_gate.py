...
from qubitron import _compat, protocols, value
...
class MeasurementGate(raw_types.Gate):
    ...
    Instead of constructing this directly, consider using the `qubitron.measure`
    helper method.
    """
    ...
    def with_key(self, key: str | qubitron.MeasurementKey) -> MeasurementGate:
    ...
    def _op_repr_(self, qubits: Sequence[qubitron.Qid]) -> str:
        ...
        arg_list = ', '.join(args)
        return f'qubitron.measure({arg_list})'
    ...
    def __repr__(self):
        ...
        if self.confusion_map:
            ...
            args.append(f'confusion_map={{{proper_map_str}}}')
        return f'qubitron.MeasurementGate({", ".join(args)})'
    ...
    def _act_on_(self, sim_state: qubitron.SimulationStateBase, qubits: Sequence[qubitron.Qid]) -> bool:
        from qubitron.sim import SimulationState
