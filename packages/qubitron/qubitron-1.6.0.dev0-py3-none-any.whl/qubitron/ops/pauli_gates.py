...
from qubitron.ops import common_gates, identity, raw_types

if TYPE_CHECKING:
    import qubitron
    from qubitron.ops.pauli_string import SingleQubitPauliStringGateOperation
    from qubitron.value.product_state import (  # pragma: no cover
        _XEigenState,
        _YEigenState,
        _ZEigenState,
    )
...
pauli_string = LazyLoader("pauli_string", globals(), "qubitron.ops.pauli_string")
...
class Pauli(raw_types.Gate, metaclass=abc.ABCMeta):
    ...
    def phased_pauli_product(
        self, other: qubitron.Pauli | identity.IdentityGate
    ) -> tuple[complex, qubitron.Pauli | identity.IdentityGate]:
        if self == other:
            return 1, identity.I
    ...
    def on(self, *qubits: qubitron.Qid) -> SingleQubitPauliStringGateOperation:
...
class _PauliX(Pauli, common_gates.XPowGate):
    ...
    def __pow__(self, exponent: qubitron.TParamVal) -> common_gates.XPowGate:
        return common_gates.XPowGate(exponent=exponent) if exponent != 1 else _PauliX()

    def _with_exponent(self, exponent: qubitron.TParamVal) -> common_gates.XPowGate:
    ...
    def basis(self) -> dict[int, _XEigenState]:
        from qubitron.value.product_state import _XEigenState
...
class _PauliY(Pauli, common_gates.YPowGate):
    ...
    def __pow__(self, exponent: qubitron.TParamVal) -> common_gates.YPowGate:
        return common_gates.YPowGate(exponent=exponent) if exponent != 1 else _PauliY()

    def _with_exponent(self, exponent: qubitron.TParamVal) -> common_gates.YPowGate:
        return self.__pow__(exponent)
    ...
    def basis(self) -> dict[int, _YEigenState]:
        from qubitron.value.product_state import _YEigenState
...
class _PauliZ(Pauli, common_gates.ZPowGate):
    ...
    def __pow__(self, exponent: qubitron.TParamVal) -> common_gates.ZPowGate:
        return common_gates.ZPowGate(exponent=exponent) if exponent != 1 else _PauliZ()

    def _with_exponent(self, exponent: qubitron.TParamVal) -> common_gates.ZPowGate:
        return self.__pow__(exponent)
    ...
    def basis(self) -> dict[int, _ZEigenState]:
        from qubitron.value.product_state import _ZEigenState
...
document(
    ...
    This is the `exponent=1` instance of the `qubitron.XPowGate`.

    The unitary matrix of `qubitron.X` is:
    $$
    \begin{bmatrix}
        0 & 1 \\
        1 & 0
...
document(
    ...
    This is the `exponent=1` instance of the `qubitron.YPowGate`.

    The unitary matrix of `qubitron.Y` is:
    $$
    \begin{bmatrix}
        0 & -i \\
        i & 0
...
document(
    ...
    This is the `exponent=1` instance of the `qubitron.ZPowGate`.

    The unitary matrix of `qubitron.Z` is:
    $$
    \begin{bmatrix}
        1 & 0 \\
        0 & -1
