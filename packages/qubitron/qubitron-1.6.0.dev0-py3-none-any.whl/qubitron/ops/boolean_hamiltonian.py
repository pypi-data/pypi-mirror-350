...
from qubitron.ops.linear_combinations import PauliSum
...
class BooleanHamiltonianGate(raw_types.Gate):
    ...
    ) -> qubitron.BooleanHamiltonianGate:
    ...
    def __repr__(self) -> str:
        return (
            f'qubitron.BooleanHamiltonianGate('
...
def _get_gates_from_hamiltonians(
    hamiltonian_polynomial_list: list[qubitron.PauliSum], qubit_map: dict[str, qubitron.Qid], theta: float
) -> Generator[qubitron.Operation, None, None]:
    ...
    def _apply_cnots(prevh: tuple[int, ...], currh: tuple[int, ...]):
        ...
        for gate in (qubitron.CNOT(qubits[c], qubits[t]) for c, t in cnots):
            yield gate
    ...
    for h in sorted_hamiltonian_keys:
        ...
        if len(h) >= 1:
            yield qubitron.Rz(rads=(theta * w)).on(qubits[h[-1]])
