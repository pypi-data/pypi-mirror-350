...
from qubitron.ops.raw_types import Operation

if TYPE_CHECKING:
    import qubitron

moment = LazyLoader("moment", globals(), "qubitron.circuits.moment")
...
document(
    ...
    `qubitron.OP_TREE` argument:

    - A single operation (a `qubitron.Operation`).
    - A list of operations (a `list[qubitron.Operation]`).
    - A list of lists of operations (a `list[list[qubitron.Operation]]`).
    - A list mixing operations and generators of operations
        (a `list[qubitron.Operation | Iterator[qubitron.Operation]]`).
...
) -> Iterator[Operation | qubitron.Moment]:
...
def flatten_to_ops_or_moments(root: OP_TREE) -> Iterator[Operation | qubitron.Moment]:
