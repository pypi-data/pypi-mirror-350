...
class IdentityGate(raw_types.Gate):
    ...
    `qubitron.I` is the single qubit identity gate.
    """
    ...
    def __repr__(self) -> str:
        ...
        if all(e == 2 for e in self._qid_shape):
            return f'qubitron.IdentityGate({len(self._qid_shape)})'
        return f'qubitron.IdentityGate(qid_shape={self._qid_shape!r})'

    def _decompose_(self, qubits) -> qubitron.OP_TREE:
        return []
    ...
    def _mul_with_qubits(self, qubits: tuple[qubitron.Qid, ...], other):
        ...
        if isinstance(other, numbers.Complex):
            from qubitron.ops.pauli_string import PauliString
...
def identity_each(*qubits: qubitron.Qid) -> qubitron.Operation:
    ...
    for qubit in qubits:
        if not isinstance(qubit, raw_types.Qid):
            raise ValueError(f'Not a qubitron.Qid: {qubit!r}.')
