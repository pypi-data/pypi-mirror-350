...
class ClassicallyControlledOperation(raw_types.Operation):
    ...
    ...     qubitron.measure(a, key='control_key'),
    ...     qubitron.X(b).with_classical_controls('control_key'))
    >>> print(circuit1)
    0: ─────────────M───────
                    ║
    1: ─────────────╫───X───
    ...
    ...     qubitron.measure(a, key='control_key1'),
    ...     qubitron.measure(b, key='control_key2'),
    ...     qubitron.X(c).with_classical_controls('control_key1', 'control_key2')])
    >>> print(circuit2)
                     ┌──┐
    0: ───────────────M─────────
                      ║
    ...
    def __init__(
        self,
        sub_operation: qubitron.Operation,
        conditions: Sequence[str | qubitron.MeasurementKey | qubitron.Condition | sympy.Basic],
    ):
        ...
        for c in conditions:
            ...
            conds.append(c)
        self._conditions: tuple[qubitron.Condition, ...] = tuple(conds)
        self._sub_operation: qubitron.Operation = sub_operation
    ...
    def _decompose_with_context_(self, context: qubitron.DecompositionContext | None = None):
    ...
    def __repr__(self):
        return (
            f'qubitron.ClassicallyControlledOperation('
    ...
    def _control_keys_(self) -> frozenset[qubitron.MeasurementKey]:
        local_keys: frozenset[qubitron.MeasurementKey] = frozenset(
    ...
    def _qasm_(self, args: qubitron.QasmArgs) -> str | None:
