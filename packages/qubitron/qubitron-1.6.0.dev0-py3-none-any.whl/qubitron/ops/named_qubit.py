...
class NamedQid(_BaseNamedQid):
    ...
    containing `qubitron.NamedQid('qid22', dimension=3)` and
    `qubitron.NamedQid('qid3', dimension=3)`, the wire for 'qid3' will
    ...
    _cache = weakref.WeakValueDictionary[tuple[str, int], 'qubitron.NamedQid']()

    def __new__(cls, name: str, dimension: int) -> qubitron.NamedQid:
    ...
    def __repr__(self) -> str:
        return f'qubitron.NamedQid({self._name!r}, dimension={self._dimension})'
    ...
    def range(*args, prefix: str, dimension: int) -> list[NamedQid]:
        ...
        each number in the range, e.g.:
            ...
            >>> qubitron.NamedQid.range(3, prefix='a', dimension=3)
            ... # doctest: +NORMALIZE_WHITESPACE
            [qubitron.NamedQid('a0', dimension=3), qubitron.NamedQid('a1', dimension=3),
                qubitron.NamedQid('a2', dimension=3)]
            >>> qubitron.NamedQid.range(2, 4, prefix='a', dimension=3)
            [qubitron.NamedQid('a2', dimension=3), qubitron.NamedQid('a3', dimension=3)]
...
class NamedQubit(_BaseNamedQid):
    ...
    the name are handled correctly. So, for example, if you print a circuit
    containing `qubitron.NamedQubit('qubit22')` and `qubitron.NamedQubit('qubit3')`, the
    ...
    # Holds weak references so instances can still be garbage collected.
    _cache = weakref.WeakValueDictionary[str, 'qubitron.NamedQubit']()

    def __new__(cls, name: str) -> qubitron.NamedQubit:
        """Initializes a `NamedQid` with a given name and dimension.
    ...
    def __repr__(self) -> str:
        return f'qubitron.NamedQubit({self._name!r})'

    @staticmethod
    def range(*args, prefix: str) -> list[NamedQubit]:
        r"""Returns a range of `qubitron.NamedQubit`s.

        The range returned starts with the prefix, and followed by a qubit for
        each number in the range, e.g.:

            >>> qubitron.NamedQubit.range(3, prefix='a')
            ... # doctest: +NORMALIZE_WHITESPACE
            [qubitron.NamedQubit('a0'), qubitron.NamedQubit('a1'),
                qubitron.NamedQubit('a2')]
            >>> qubitron.NamedQubit.range(2, 4, prefix='a')
            [qubitron.NamedQubit('a2'), qubitron.NamedQubit('a3')]
