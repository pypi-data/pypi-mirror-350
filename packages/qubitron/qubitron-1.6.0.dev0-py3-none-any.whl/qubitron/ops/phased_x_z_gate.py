...
from qubitron import linalg, ops, protocols, value
...
class PhasedXZGate(raw_types.Gate):
    ...
    The unitary matrix of `qubitron.PhasedXZGate(x_exponent=x, z_exponent=z, axis_phase_exponent=a)` is:
    $$
        \begin{bmatrix}
    ...
    This gate can be thought of as a `qubitron.PhasedXPowGate` followed by a `qubitron.ZPowGate`.
    ...
    Every single qubit gate can be written as a single `qubitron.PhasedXZGate`.
    """
    ...
    @classmethod
    def from_zyz_angles(cls, z0_rad: float, y_rad: float, z1_rad: float) -> qubitron.PhasedXZGate:
    ...
    @classmethod
    def from_zyz_exponents(cls, z0: float, y: float, z1: float) -> qubitron.PhasedXZGate:
    ...
    def _canonical(self) -> qubitron.PhasedXZGate:
        x = self.x_exponent
        z = self.z_exponent
    ...
    @staticmethod
    def from_matrix(mat: np.ndarray) -> qubitron.PhasedXZGate:
    ...
    def with_z_exponent(self, z_exponent: float | sympy.Expr) -> qubitron.PhasedXZGate:
        return PhasedXZGate(
    ...
    def _qasm_(self, args: qubitron.QasmArgs, qubits: tuple[qubitron.Qid, ...]) -> str | None:
        from qubitron.circuits import qasm_output
    ...
    def _decompose_(self, qubits: Sequence[qubitron.Qid]) -> Iterator[qubitron.OP_TREE]:
        q = qubits[0]
    ...
    def _is_parameterized_(self) -> bool:
        """See `qubitron.SupportsParameterization`."""
        return (
    ...
    def _resolve_parameters_(
        ...
        if isinstance(z_exponent, numbers.Complex):
            if isinstance(z_exponent, numbers.Real):
                ...
                raise ValueError(f'Complex exponent {z_exponent} not allowed in qubitron.PhasedXZGate')
        if isinstance(x_exponent, numbers.Complex):
            if isinstance(x_exponent, numbers.Real):
                x_exponent = float(x_exponent)
            else:
                raise ValueError(f'Complex exponent {x_exponent} not allowed in qubitron.PhasedXZGate')
        if isinstance(axis_phase_exponent, numbers.Complex):
            if isinstance(axis_phase_exponent, numbers.Real):
                ...
                raise ValueError(
                    f'Complex exponent {axis_phase_exponent} not allowed in qubitron.PhasedXZGate'
                )
        return PhasedXZGate(
            z_exponent=z_exponent, x_exponent=x_exponent, axis_phase_exponent=axis_phase_exponent
        )

    def _phase_by_(self, phase_turns, qubit_index) -> qubitron.PhasedXZGate:
    ...
    def _circuit_diagram_info_(self, args: qubitron.CircuitDiagramInfoArgs) -> str:
        """See `qubitron.SupportsCircuitDiagramInfo`."""
        return (
            f'PhXZ('
