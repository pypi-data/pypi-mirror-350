...
class XXPowGate(gate_features.InterchangeableQubitsGate, eigen_gate.EigenGate):
    ...
    See also: `qubitron.ops.MSGate` (the Mølmer–Sørensen gate), which is
    ...
    def _decompose_(self, qubits: tuple[qubitron.Qid, ...]) -> Iterator[qubitron.OP_TREE]:
    ...
    def __repr__(self) -> str:
        if self._global_shift == 0:
            if self._exponent == 1:
                return 'qubitron.XX'
            return f'(qubitron.XX**{proper_repr(self._exponent)})'
        return (
            f'qubitron.XXPowGate(exponent={proper_repr(self._exponent)}, '
...
class YYPowGate(gate_features.InterchangeableQubitsGate, eigen_gate.EigenGate):
    ...
    def __repr__(self) -> str:
        if self._global_shift == 0:
            if self._exponent == 1:
                return 'qubitron.YY'
            return f'(qubitron.YY**{proper_repr(self._exponent)})'
        return (
            f'qubitron.YYPowGate(exponent={proper_repr(self._exponent)}, '
            f'global_shift={self._global_shift!r})'
        )
...
class ZZPowGate(gate_features.InterchangeableQubitsGate, eigen_gate.EigenGate):
    ...
    def _decompose_into_clifford_with_qubits_(
        self, qubits: Sequence[qubitron.Qid]
    ) -> Sequence[qubitron.Operation | Sequence[qubitron.Operation]]:
    ...
    def __repr__(self) -> str:
        if self._global_shift == 0:
            if self._exponent == 1:
                return 'qubitron.ZZ'
            return f'(qubitron.ZZ**{proper_repr(self._exponent)})'
        return (
            f'qubitron.ZZPowGate(exponent={proper_repr(self._exponent)}, '
            f'global_shift={self._global_shift!r})'
        )
...
class MSGate(XXPowGate):
    ...
    def __repr__(self) -> str:
        if self._exponent == 1:
            return 'qubitron.ms(np.pi/2)'
        return f'qubitron.ms({self._exponent!r}*np.pi/2)'

    # the default namespace is already occupied by qubitron_ionq.MSGate
    @classmethod
...
def ms(rads: float) -> MSGate:
    """A helper to construct the `qubitron.MSGate` for the given angle specified in radians.
...
document(
    ...
    Useful for creating `qubitron.XXPowGate`s via `qubitron.XX**t`.

    This is the `exponent=1` instance of `qubitron.XXPowGate`.
    """,
)
YY = YYPowGate()
document(
    ...
    Useful for creating `qubitron.YYPowGate`s via `qubitron.YY**t`.

    This is the `exponent=1` instance of `qubitron.YYPowGate`.
    """,
)
ZZ = ZZPowGate()
document(
    ...
    Useful for creating `qubitron.ZZPowGate`s via `qubitron.ZZ**t`.

    This is the `exponent=1` instance of `qubitron.ZZPowGate`.
    """,
)
