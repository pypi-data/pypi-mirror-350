...
from qubitron.ops import control_values as cv, gate_features, raw_types
...
class GateOperation(raw_types.Operation):
    ...
    def __init__(self, gate: qubitron.Gate, qubits: Sequence[qubitron.Qid]) -> None:
        """Inits GateOperation.
    ...
    def with_gate(self, new_gate: qubitron.Gate) -> qubitron.Operation:
    ...
    def __repr__(self):
        ...
        return f'qubitron.GateOperation(gate={self.gate!r}, qubits=[{qubit_args_repr}])'
    ...
    def _group_interchangeable_qubits(
        self,
    ) -> tuple[qubitron.Qid | tuple[int, frozenset[qubitron.Qid]], ...]:
        if not isinstance(self.gate, gate_features.InterchangeableQubitsGate):
            return self.qubits
        groups: dict[int, list[qubitron.Qid]] = {}
    ...
    def _measurement_key_obj_(self) -> qubitron.MeasurementKey | None:
        getter = getattr(self.gate, '_measurement_key_obj_', None)
        if getter is not None:
            return getter()
        return NotImplemented

    def _measurement_key_objs_(self) -> frozenset[qubitron.MeasurementKey] | NotImplementedType | None:
        getter = getattr(self.gate, '_measurement_key_objs_', None)
        if getter is not None:
            return getter()
        return NotImplemented

    def _act_on_(self, sim_state: qubitron.SimulationStateBase):
    ...
    def _resolve_parameters_(self, resolver: qubitron.ParamResolver, recursive: bool) -> qubitron.Operation:
    ...
    def __pow__(self, exponent: Any) -> qubitron.Operation:
        ...
        Only works if the gate implements qubitron.ExtrapolatableEffect.
