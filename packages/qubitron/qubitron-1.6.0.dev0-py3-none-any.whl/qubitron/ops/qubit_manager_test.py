...
import qubitron
from qubitron.ops import qubit_manager as cqi
...
def test_clean_qubits() -> None:
    ...
    assert str(q) == '_c(1)'
    assert repr(q) == 'qubitron.ops.CleanQubit(1)'
    ...
    assert repr(q) == 'qubitron.ops.CleanQubit(2, dim=3)'

    q = cqi.CleanQubit(3, dim=4, prefix="a")
    assert str(q) == 'a_c(3) (d=4)'
    assert repr(q) == "qubitron.ops.CleanQubit(3, dim=4, prefix='a')"
...
def test_borrow_qubits() -> None:
    ...
    assert q.dimension == 2
    assert str(q) == '_b(10)'
    assert repr(q) == 'qubitron.ops.BorrowableQubit(10)'
    ...
    assert repr(q) == 'qubitron.ops.BorrowableQubit(20, dim=4)'

    q = cqi.BorrowableQubit(30, dim=4, prefix="a")
    assert str(q) == 'a_b(30) (d=4)'
    assert repr(q) == "qubitron.ops.BorrowableQubit(30, dim=4, prefix='a')"
