...
from qubitron import protocols, qis, value
...
class ControlledOperation(raw_types.Operation):
    ...
    def __init__(
        self,
        controls: Sequence[qubitron.Qid],
        ...
        if protocols.num_qubits(control_values) != len(controls):
            raise ValueError('qubitron.num_qubits(control_values) != len(controls)')
    ...
    @property
    def controls(self) -> tuple[qubitron.Qid, ...]:
    ...
    @property
    def gate(self) -> qubitron.ControlledGate | None:
    ...
    def _decompose_with_context_(self, context: qubitron.DecompositionContext | None = None):
        ...
        if isinstance(self.sub_operation.gate, matrix_gates.MatrixGate):
            # Default decompositions of 2/3 qubit `qubitron.MatrixGate` ignores global phase, which is
    ...
    def __repr__(self):
        ...
        return (
            f'qubitron.ControlledOperation('
