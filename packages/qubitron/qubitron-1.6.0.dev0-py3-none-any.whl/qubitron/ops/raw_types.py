...
from qubitron._compat import __qubitron_debug__, _method_cache_name, cached_method
from qubitron._import import LazyLoader

# Lazy imports to break circular dependencies.
ops = LazyLoader("ops", globals(), "qubitron.ops")
line_qubit = LazyLoader("line_qubit", globals(), "qubitron.devices.line_qubit")
...
class Qid(metaclass=abc.ABCMeta):
    ...
    example, `qubitron.LineQubit`'s `_comparison_key` method returns `self.x`. This
...
class Gate(metaclass=value.ABCMetaImplementAnyOneOf):
    ...
    def validate_args(self, qubits: Sequence[qubitron.Qid]) -> None:
    ...
    def on(self, *qubits: Qid) -> Operation:
        ...
        Returns: a `qubitron.Operation` which is this gate applied to the given
            qubits.
        """
        return ops.gate_operation.GateOperation(self, qubits)

    def on_each(self, *targets: Qid | Iterable[Any]) -> list[qubitron.Operation]:
        ...
        """
        operations: list[qubitron.Operation] = []
        ...
        if not __qubitron_debug__.get():
            return [
                ...
                for op in (
                    ...
                    else [self.on(cast('qubitron.Qid', q))]
                )
            ]
    ...
    def wrap_in_linear_combination(
        self, coefficient: qubitron.TParamValComplex = 1
        ...
        Args:
            coefficient: number coefficient to use in the resulting
                `qubitron.LinearCombinationOfGates` object.

        Returns:
            `qubitron.LinearCombinationOfGates` containing self with a
                coefficient of `coefficient`.
        """
        return ops.linear_combinations.LinearCombinationOfGates({self: coefficient})

    def __add__(self, other: Gate | qubitron.LinearCombinationOfGates) -> qubitron.LinearCombinationOfGates:
        if isinstance(other, Gate):
            return self.wrap_in_linear_combination() + other.wrap_in_linear_combination()
        return self.wrap_in_linear_combination() + other

    def __sub__(self, other: Gate | qubitron.LinearCombinationOfGates) -> qubitron.LinearCombinationOfGates:
        if isinstance(other, Gate):
            return self.wrap_in_linear_combination() - other.wrap_in_linear_combination()
        return self.wrap_in_linear_combination() - other

    def __neg__(self) -> qubitron.LinearCombinationOfGates:
        return self.wrap_in_linear_combination(coefficient=-1)

    def __mul__(self, other: complex) -> qubitron.LinearCombinationOfGates:
        return self.wrap_in_linear_combination(coefficient=other)

    def __rmul__(self, other: complex) -> qubitron.LinearCombinationOfGates:
        return self.wrap_in_linear_combination(coefficient=other)

    def __truediv__(self, other: complex) -> qubitron.LinearCombinationOfGates:
    ...
    def with_probability(self, probability: qubitron.TParamVal) -> qubitron.Gate:
        ...
        Returns:
            `qubitron.RandomGateChannel` that applies `self` with probability
    ...
    def controlled(
        ...
        Returns:
            A `qubitron.Gate` representing `self` controlled by the given control values
                and qubits. This is a `qubitron.ControlledGate` in the base
    ...
    # to keep backwards compatibility with versions of qubitron where num_qubits
    ...
    def _mul_with_qubits(self, qubits: tuple[qubitron.Qid, ...], other):
        """qubitron.GateOperation.__mul__ delegates to this method."""
        return NotImplemented

    def _rmul_with_qubits(self, qubits: tuple[qubitron.Qid, ...], other):
        """qubitron.GateOperation.__rmul__ delegates to this method."""
        return NotImplemented
...
class Operation(metaclass=abc.ABCMeta):
    ...
    @property
    def gate(self) -> qubitron.Gate | None:
        return None
    ...
    @abc.abstractmethod
    def with_qubits(self, *new_qubits: qubitron.Qid) -> Self:
    ...
    @property
    def untagged(self) -> qubitron.Operation:
        """Returns the underlying operation without any tags."""
        return self

    def with_tags(self, *new_tags: Hashable) -> qubitron.Operation:
    ...
    def transform_qubits(
        ...
        >>> op = qubitron.CZ(qubitron.LineQubit(5), qubitron.LineQubit(9))
        >>> op.transform_qubits(lambda q: qubitron.GridQubit(q.x // 4, q.x % 4))
        qubitron.CZ(qubitron.GridQubit(1, 1), qubitron.GridQubit(2, 1))
        ...
        >>> d = {a: qubitron.LineQubit(4), b: qubitron.LineQubit(5)}
        >>> op = qubitron.CNOT(a, b)
        >>> op.transform_qubits(d)
        qubitron.CNOT(qubitron.LineQubit(4), qubitron.LineQubit(5))
    ...
    def with_probability(self, probability: qubitron.TParamVal) -> qubitron.Operation:
    ...
    def validate_args(self, qubits: Sequence[qubitron.Qid]):
    ...
    @property
    def classical_controls(self) -> frozenset[qubitron.Condition]:
        """The classical controls gating this operation."""
        return frozenset()

    def with_classical_controls(
        self, *conditions: str | qubitron.MeasurementKey | qubitron.Condition | sympy.Expr
    ) -> qubitron.Operation:
        ...
        or a user-specified `qubitron.Condition`. A measurement key evaluates to
        True iff any qubit in the corresponding measurement operation evaluated
        to a non-zero value; `qubitron.Condition` supports more complex,
        ...
        Returns:
           ...
           are specified, returns self.
        """
        from qubitron.ops.classically_controlled_operation import ClassicallyControlledOperation

        if not conditions:
            return self
        return ClassicallyControlledOperation(self, conditions)

    def without_classical_controls(self) -> qubitron.Operation:
...
class TaggedOperation(Operation):
    ...
    def __init__(self, sub_operation: qubitron.Operation, *tags: Hashable):
    ...
    @property
    def sub_operation(self) -> qubitron.Operation:
    ...
    def with_tags(self, *new_tags: Hashable) -> qubitron.TaggedOperation:
    ...
    def __str__(self) -> str:
        ...
        return f'qubitron.TaggedOperation({repr(self.sub_operation)}, {tag_repr})'
    ...
    def _decompose_with_context_(
        self, context: qubitron.DecompositionContext | None = None
    ) -> qubitron.OP_TREE:
    ...
    def _act_on_(self, sim_state: qubitron.SimulationStateBase) -> bool:
    ...
    def _phase_by_(self, phase_turns: float, qubit_index: int) -> qubitron.Operation:
        return protocols.phase_by(
            self.sub_operation, phase_turns, qubit_index, default=NotImplemented
        )

    def __pow__(self, exponent: Any) -> qubitron.Operation:
...
class _InverseCompositeGate(Gate):
    ...
    def _decompose_with_context_(
        self, qubits: Sequence[qubitron.Qid], context: qubitron.DecompositionContext | None = None
    ) -> qubitron.OP_TREE:
    ...
    def _has_unitary_(self):
        from qubitron import devices, protocols
    ...
    def _circuit_diagram_info_(self, args: qubitron.CircuitDiagramInfoArgs):
...
def _validate_qid_shape(val: Any, qubits: Sequence[qubitron.Qid]) -> None:
