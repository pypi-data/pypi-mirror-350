...
from qubitron import _compat, linalg, protocols, value
from qubitron.ops import linear_combinations, pauli_string_phasor

if TYPE_CHECKING:
    import qubitron


def _all_pauli_strings_commute(pauli_sum: qubitron.PauliSum) -> bool:
    for x in pauli_sum:
        for y in pauli_sum:
...
class PauliSumExponential:
    ...
    def __init__(
        self, pauli_sum_like: qubitron.PauliSumLike, exponent: qubitron.TParamVal = 1, atol: float = 1e-8
    ):
    ...
    def with_qubits(self, *new_qubits: qubitron.Qid) -> PauliSumExponential:
    ...
    def __iter__(self) -> Iterator[qubitron.PauliStringPhasor]:
    ...
    def __repr__(self) -> str:
        ...
        return f'qubitron.{class_name}({self._pauli_sum!r}, {self._exponent!r})'
