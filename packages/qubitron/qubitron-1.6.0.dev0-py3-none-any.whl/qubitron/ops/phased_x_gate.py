...
class PhasedXPowGate(raw_types.Gate):
    ...
    The unitary matrix of `qubitron.PhasedXPowGate(exponent=t, phase_exponent=p)` is:
    $$
        \begin{bmatrix}
    ...
    This gate is like an `qubitron.XPowGate`, but which has been "phased",
    by applying a `qubitron.ZPowGate` before and after this gate. In the language
    ...
    def _decompose_(self, qubits: Sequence[qubitron.Qid]) -> qubitron.OP_TREE:
        assert len(qubits) == 1
        q = qubits[0]
        z = qubitron.Z(q) ** self._phase_exponent
        x = qubitron.XPowGate(exponent=self._exponent, global_shift=self.global_shift).on(q)
        return z**-1, x, z
    ...
    def _unitary_(self) -> np.ndarray | NotImplementedType | None:
        ...
        if self._is_parameterized_():
            return None
        z = protocols.unitary(qubitron.Z**self._phase_exponent)
        x = protocols.unitary(qubitron.X**self._exponent)
    ...
    def _resolve_parameters_(self, resolver: qubitron.ParamResolver, recursive: bool) -> PhasedXPowGate:
        """See `qubitron.SupportsParameterization`."""
    ...
    def __repr__(self) -> str:
        ...
        args_str = ', '.join(args)
        return f'qubitron.PhasedXPowGate({args_str})'
