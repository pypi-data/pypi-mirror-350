...
def test_generated_unitary_is_uniform(m: int, n: int) -> None:
    ...
    """
    gate = qubitron.UniformSuperpositionGate(m, n)
    matrix = np.array(qubitron.unitary(gate))
...
def test_incompatible_m_value_and_qubit_args(m: int, n: int) -> None:
    ...
    if not (isinstance(m, int)):
        with pytest.raises(ValueError, match="m_value must be a positive integer."):
            qubitron.UniformSuperpositionGate(m, n)
        ...
        with pytest.raises(ValueError, match="m_value must be a positive integer."):
            qubitron.UniformSuperpositionGate(int(m), int(n))
    elif n < np.log2(m):
        with pytest.raises(
            ValueError,
            match="num_qubits must be an integer greater than or equal to log2\\(m_value\\).",
...
def test_repr() -> None:
    assert (
        repr(qubitron.UniformSuperpositionGate(7, 3))
        == 'UniformSuperpositionGate(m_value=7, num_qubits=3)'
    )


def test_uniform_superposition_gate_json_dict() -> None:
    assert qubitron.UniformSuperpositionGate(7, 3)._json_dict_() == {'m_value': 7, 'num_qubits': 3}


def test_str() -> None:
    assert (
        str(qubitron.UniformSuperpositionGate(7, 3))
        == 'UniformSuperpositionGate(m_value=7, num_qubits=3)'
    )
...
def test_eq(m: int, n: int) -> None:
    a = qubitron.UniformSuperpositionGate(m, n)
    b = qubitron.UniformSuperpositionGate(m, n)
    c = qubitron.UniformSuperpositionGate(m + 1, n)
    d = qubitron.X
