...
class Device(metaclass=abc.ABCMeta):
    ...
    using a `qubitron.DeviceMetadata` object.   This class can also be
    ...
    def metadata(self) -> DeviceMetadata | None:
        ...
        Returns:
            `qubitron.DeviceMetadata` if specified by the device otherwise None.
        """
        return None
    ...
    def validate_circuit(self, circuit: qubitron.AbstractCircuit) -> None:
    ...
    def validate_moment(self, moment: qubitron.Moment) -> None:
...
class DeviceMetadata:
    ...
    def __init__(self, qubits: Iterable[qubitron.Qid], nx_graph: nx.Graph):
        """Construct a DeviceMetadata object.

        Args:
            qubits: Iterable of `qubitron.Qid`s that exist on the device.
            nx_graph: `nx.Graph` describing qubit connectivity
                ...
                coupling.
        """
        self._qubits_set: frozenset[qubitron.Qid] = frozenset(qubits)
        self._nx_graph = nx_graph

    @property
    def qubit_set(self) -> frozenset[qubitron.Qid]:
    ...
    @classmethod
    def _from_json_dict_(cls, qubits: Iterable[qubitron.Qid], nx_graph: nx.Graph, **kwargs):
