...
import qubitron
from qubitron.devices.noise_properties import NoiseModelFromNoiseProperties
...
DEFAULT_GATE_NS: dict[type, float] = {
    qubitron.ZPowGate: 25.0,
    qubitron.MeasurementGate: 4000.0,
    qubitron.ResetChannel: 250.0,
    qubitron.PhasedXZGate: 25.0,
    qubitron.FSimGate: 32.0,
    qubitron.PhasedFSimGate: 32.0,
    qubitron.ISwapPowGate: 32.0,
    qubitron.CZPowGate: 32.0,
    # qubitron.WaitGate is a special case.
}


def default_props(system_qubits: list[qubitron.Qid], qubit_pairs: list[tuple[qubitron.Qid, qubitron.Qid]]):
    return {
...
class ExampleNoiseProperties(SuperconductingQubitsNoiseProperties):
    ...
    def single_qubit_gates(cls) -> set[type]:
        return {qubitron.ZPowGate, qubitron.PhasedXZGate, qubitron.MeasurementGate, qubitron.ResetChannel}

    @classmethod
    def symmetric_two_qubit_gates(cls) -> set[type]:
        return {qubitron.FSimGate, qubitron.PhasedFSimGate, qubitron.ISwapPowGate, qubitron.CZPowGate}
...
def test_init_validation():
    ...
    with pytest.raises(ValueError, match='Symmetric errors can only apply to 2-qubit gates.'):
        _ = ExampleNoiseProperties(
            ...
            tphi_ns={q0: 1},
            readout_errors={q0: [0.1, 0.2]},
            gate_pauli_errors={OpIdentifier(qubitron.CCNOT, q0, q1, q2): 0.1},
        )
    ...
    with pytest.raises(ValueError, match='does not appear in the symmetric or asymmetric'):
        _ = ExampleNoiseProperties(
            ...
            readout_errors={q0: [0.1, 0.2]},
            gate_pauli_errors={
                OpIdentifier(qubitron.CNOT, q0, q1): 0.1,
                OpIdentifier(qubitron.CNOT, q1, q0): 0.1,
            },
        )
    ...
    with pytest.raises(ValueError, match='has errors but its symmetric id'):
        _ = ExampleNoiseProperties(
            ...
            tphi_ns={q0: 1},
            readout_errors={q0: [0.1, 0.2]},
            gate_pauli_errors={OpIdentifier(qubitron.CZPowGate, q0, q1): 0.1},
        )
    ...
    test_props = ExampleNoiseProperties(
        ...
        readout_errors={q0: [0.1, 0.2]},
        gate_pauli_errors={
            OpIdentifier(qubitron.ZPowGate, q0): 0.1,
            OpIdentifier(qubitron.CZPowGate, q0, q1): 0.1,
            OpIdentifier(qubitron.CZPowGate, q1, q0): 0.1,
        },
    )
    ...
    _ = ExampleNoiseProperties(
        ...
        readout_errors={q0: [0.1, 0.2]},
        gate_pauli_errors={
            OpIdentifier(qubitron.CCNOT, q0, q1, q2): 0.1,
            OpIdentifier(qubitron.CNOT, q0, q1): 0.1,
        },
        validate=False,
    )
...
def test_depol_validation():
    ...
    z_2q_props = ExampleNoiseProperties(
        ...
        tphi_ns={q0: 1},
        readout_errors={q0: [0.1, 0.2]},
        gate_pauli_errors={OpIdentifier(qubitron.ZPowGate, q0, q1): 0.1},
        validate=False,
    )
    ...
    cz_3q_props = ExampleNoiseProperties(
        ...
        tphi_ns={q0: 1},
        readout_errors={q0: [0.1, 0.2]},
        gate_pauli_errors={OpIdentifier(qubitron.CZPowGate, q0, q1, q2): 0.1},
        validate=False,
    )
    ...
    toffoli_props = ExampleNoiseProperties(
        ...
        tphi_ns={q0: 1},
        readout_errors={q0: [0.1, 0.2]},
        gate_pauli_errors={OpIdentifier(qubitron.CCXPowGate, q0, q1, q2): 0.1},
        validate=False,
    )
...
@pytest.mark.parametrize(
    'op',
    [
        qubitron.Z(qubitron.LineQubit(0)) ** 0.3,
        qubitron.PhasedXZGate(x_exponent=0.8, z_exponent=0.2, axis_phase_exponent=0.1).on(
...
def test_single_qubit_gates(op):
    ...
    assert isinstance(depol_op.gate, qubitron.DepolarizingChannel)
    ...
    assert isinstance(thermal_op.gate, qubitron.KrausChannel)
    thermal_choi = qubitron.kraus_to_choi(qubitron.kraus(thermal_op))
    assert np.allclose(
        thermal_choi,
        [
...
@pytest.mark.parametrize(
    'op', [qubitron.ISWAP(*qubitron.LineQubit.range(2)) ** 0.6, qubitron.CZ(*qubitron.LineQubit.range(2)) ** 0.3]
)
def test_two_qubit_gates(op):
    ...
    assert isinstance(thermal_op_0.gate, qubitron.KrausChannel)
    assert isinstance(thermal_op_1.gate, qubitron.KrausChannel)
    thermal_choi_0 = qubitron.kraus_to_choi(qubitron.kraus(thermal_op_0))
    thermal_choi_1 = qubitron.kraus_to_choi(qubitron.kraus(thermal_op_1))
...
def test_measure_gates():
    q00, q01, q10, q11 = qubitron.GridQubit.rect(2, 2)
    ...
    model = NoiseModelFromNoiseProperties(props)
    op = qubitron.measure(*qubits, key='m')
    circuit = qubitron.Circuit(qubitron.measure(*qubits, key='m'))
    noisy_circuit = circuit.with_noise(model)
    ...
    for q in qubits:
        ...
        assert isinstance(op.gate, qubitron.GeneralizedAmplitudeDampingChannel), q
