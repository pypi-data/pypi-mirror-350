...
from qubitron import ops, protocols, qis, value
...
class OpIdentifier:
    ...
    def __init__(self, gate_type: type[qubitron.Gate], *qubits: qubitron.Qid):
        self._gate_type = gate_type
        self._gate_family = ops.GateFamily(gate_type)
        self._qubits: tuple[qubitron.Qid, ...] = tuple(qubits)

    @property
    def gate_type(self) -> type[qubitron.Gate]:
    ...
    def __repr__(self) -> str:
        ...
        return f'qubitron.devices.noise_utils.OpIdentifier({proper_repr(self.gate_type)}, {qubits})'
    ...
    def _json_dict_(self) -> dict[str, Any]:
        if hasattr(self.gate_type, '__name__'):
            return {'gate_type': protocols.json_qubitron_type(self._gate_type), 'qubits': self._qubits}
        return {'gate_type': self._gate_type, 'qubits': self._qubits}

    @classmethod
    def _from_json_dict_(cls, gate_type, qubits, **kwargs) -> OpIdentifier:
        if isinstance(gate_type, str):
            gate_type = protocols.qubitron_type_from_json(gate_type)
        return cls(gate_type, *qubits)


@deprecated(deadline='v2.0', fix='use qubitron.qis.decay_constant_to_xeb_fidelity')
...
@deprecated(deadline='v2.0', fix='use qubitron.qis.decay_constant_to_pauli_error')
...
@deprecated(deadline='v2.0', fix='use qubitron.qis.pauli_error_to_decay_constant')
...
@deprecated(deadline='v2.0', fix='use qubitron.qis.xeb_fidelity_to_decay_constant')
...
@deprecated(deadline='v2.0', fix='use qubitron.qis.pauli_error_from_t1')
...
@deprecated(deadline='v2.0', fix='use qubitron.qis.average_error')
...
@deprecated(deadline='v2.0', fix='use qubitron.qis.decoherence_pauli_error')
