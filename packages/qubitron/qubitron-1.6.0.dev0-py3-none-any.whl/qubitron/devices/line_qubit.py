...
class _BaseLineQid(ops.Qid):
    ...
    def is_adjacent(self, other: qubitron.Qid) -> bool:
        ...
        Args:
            other: `qubitron.Qid` to test for adjacency.
...
class LineQid(_BaseLineQid):
    ...
    One can construct new `qubitron.LineQid`s by adding or subtracting integers:

    >>> qubitron.LineQid(1, dimension=2) + 3
    qubitron.LineQid(4, dimension=2)

    >>> qubitron.LineQid(2, dimension=3) - 1
    ...
    _cache = weakref.WeakValueDictionary[tuple[int, int], 'qubitron.LineQid']()

    def __new__(cls, x: int, dimension: int) -> qubitron.LineQid:
    ...
    def for_gate(val: Any, start: int = 0, step: int = 1) -> list[LineQid]:
        ...
        Args:
            val: Any value that supports the `qubitron.qid_shape` protocol.  Usually
                a gate.
            start: The x coordinate of the first `LineQid`.
            step: The amount to increment each x coordinate.
        """
        # Avoids circular import.
        from qubitron.protocols.qid_shape_protocol import qid_shape

        return LineQid.for_qid_shape(qid_shape(val), start=start, step=step)

    def __repr__(self) -> str:
        return f"qubitron.LineQid({self._x}, dimension={self._dimension})"
...
class LineQubit(_BaseLineQid):
    ...
    One can construct new `qubitron.LineQubit`s by adding or subtracting integers:

    >>> qubitron.LineQubit(1) + 3
    qubitron.LineQubit(4)

    >>> qubitron.LineQubit(2) - 1
    ...
    # Holds weak references so instances can still be garbage collected.
    _cache = weakref.WeakValueDictionary[int, 'qubitron.LineQubit']()

    def __new__(cls, x: int) -> qubitron.LineQubit:
        """Initializes a line qid at the given x coordinate.
    ...
    def __repr__(self) -> str:
        return f"qubitron.LineQubit({self._x})"
