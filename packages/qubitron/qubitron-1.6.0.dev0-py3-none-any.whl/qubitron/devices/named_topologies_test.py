...
def test_tilted_square_lattice(width, height) -> None:
    ...
    qubitron.testing.assert_equivalent_repr(topo)
...
def test_tilted_square_methods() -> None:
    ...
    assert all(isinstance(q, qubitron.GridQubit) for q in qubits)

    mapping = topo.nodes_to_gridqubits(offset=(3, 5))
    assert all(
        isinstance(q, qubitron.GridQubit) and q >= qubitron.GridQubit(0, 0) for q in mapping.values()
    )
...
def test_line_topology() -> None:
    ...
    assert all(isinstance(q, qubitron.LineQubit) for q in mapping.values())
    assert all(mapping[x] == qubitron.LineQubit(x + 3) for x in mapping)
...
def test_line_topology_nodes_as_qubits() -> None:
    for n in range(2, 10, 2):
        assert LineTopology(n).nodes_as_linequbits() == qubitron.LineQubit.range(n)
...
def test_draw_gridlike_qubits(tilted) -> None:
    graph = nx.grid_2d_graph(3, 3)
    graph = nx.relabel_nodes(graph, {(r, c): qubitron.GridQubit(r, c) for r, c in sorted(graph.nodes)})
    ax = MagicMock()
    pos = draw_gridlike(graph, tilted=tilted, ax=ax)
    ax.scatter.assert_called()
