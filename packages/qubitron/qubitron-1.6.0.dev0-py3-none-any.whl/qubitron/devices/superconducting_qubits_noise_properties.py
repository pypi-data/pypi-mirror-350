...
from qubitron import devices, ops, qis
from qubitron.devices import noise_utils
...
class SuperconductingQubitsNoiseProperties(devices.NoiseProperties, abc.ABC):
    ...
    Args:
        ...
        t1_ns: dict[qubitron.Qid, float] of qubits to their T_1 time, in ns.
        tphi_ns: dict[qubitron.Qid, float] of qubits to their T_phi time, in ns.
        readout_errors: dict[qubitron.Qid, np.ndarray] of qubits to their readout
    ...
    t1_ns: dict[qubitron.Qid, float]
    tphi_ns: dict[qubitron.Qid, float]
    readout_errors: dict[qubitron.Qid, list[float]]
    gate_pauli_errors: dict[noise_utils.OpIdentifier, float]

    validate: bool = True
    _qubits: list[qubitron.Qid] = field(init=False, default_factory=list)
    ...
    @property
    def qubits(self) -> list[qubitron.Qid]:
    ...
    def build_noise_models(self) -> list[qubitron.NoiseModel]:
        noise_models: list[qubitron.NoiseModel] = []
        ...
        if self.readout_errors:
            added_measure_errors: dict[noise_utils.OpIdentifier, qubitron.Operation] = {}
