...
from qubitron.devices import GridQubit, LineQubit
from qubitron.protocols.json_serialization import dataclass_json_dict
...
_GRIDLIKE_NODE = Union['qubitron.GridQubit', tuple[int, int]]
...
def draw_gridlike(
    ...
    Args:
        graph: A NetworkX graph whose nodes are (row, column) coordinates or qubitron.GridQubits.
...
class LineTopology(NamedTopology):
    ...
    def nodes_as_linequbits(self) -> list[qubitron.LineQubit]:
        """Get the graph nodes as qubitron.LineQubit"""
    ...
    def nodes_to_linequbits(self, offset: int = 0) -> dict[int, qubitron.LineQubit]:
        """Return a mapping from graph nodes to `qubitron.LineQubit`
...
class TiltedSquareLattice(NamedTopology):
    ...
    def nodes_as_gridqubits(self) -> list[qubitron.GridQubit]:
        """Get the graph nodes as qubitron.GridQubit"""
        return [GridQubit(r, c) for r, c in sorted(self.graph.nodes)]

    def nodes_to_gridqubits(self, offset=(0, 0)) -> dict[tuple[int, int], qubitron.GridQubit]:
        """Return a mapping from graph nodes to `qubitron.GridQubit`
...
def get_placements(
    ...
    Args:
        big_graph: The parent, super-graph. We often consider the case where this is a
            nx.Graph representation of a Device whose nodes are `qubitron.Qid`s like `GridQubit`s.
