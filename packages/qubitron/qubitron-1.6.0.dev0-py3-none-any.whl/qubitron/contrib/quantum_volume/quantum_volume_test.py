...
from qubitron.contrib.quantum_volume import CompilationResult


class FakeDevice(qubitron.Device):
    qubits = qubitron.GridQubit.rect(5, 5)


def test_generate_model_circuit():
    """Test that a model circuit is randomly generated."""
    model_circuit = qubitron.contrib.quantum_volume.generate_model_circuit(
        3, 3, random_state=np.random.RandomState(1)
    )

    assert len(model_circuit) == 3
    # Ensure there are no measurement gates.
    assert list(model_circuit.findall_operations_with_gate_type(qubitron.MeasurementGate)) == []


def test_generate_model_circuit_without_seed():
    """Test that a model circuit is randomly generated without a seed."""
    model_circuit = qubitron.contrib.quantum_volume.generate_model_circuit(3, 3)
...
def test_generate_model_circuit_seed():
    ...
    model_circuit_1 = qubitron.contrib.quantum_volume.generate_model_circuit(
        3, 3, random_state=np.random.RandomState(1)
    )
    model_circuit_2 = qubitron.contrib.quantum_volume.generate_model_circuit(
        3, 3, random_state=np.random.RandomState(1)
    )
    model_circuit_3 = qubitron.contrib.quantum_volume.generate_model_circuit(
...
def test_compute_heavy_set():
    ...
    model_circuit = qubitron.Circuit(
        [
            qubitron.Moment([]),
            qubitron.Moment([qubitron.X(a), qubitron.Y(b)]),
            qubitron.Moment([]),
            qubitron.Moment([qubitron.CNOT(a, c)]),
            qubitron.Moment([qubitron.Z(a), qubitron.H(b)]),
        ]
    )
    assert qubitron.contrib.quantum_volume.compute_heavy_set(model_circuit) == [5, 7]


def test_sample_heavy_set():
    """Test that we correctly sample a circuit's heavy set"""

    sampler = Mock(spec=qubitron.Simulator)
    # Construct a result that returns "1", "2", "3", "0"
    result = qubitron.ResultDict(
        params=qubitron.ParamResolver({}),
        measurements={'mock': np.array([[0, 1], [1, 0], [1, 1], [0, 0]])},
    )
    sampler.run = MagicMock(return_value=result)
    circuit = qubitron.Circuit(qubitron.measure(*qubitron.LineQubit.range(2)))
    compilation_result = CompilationResult(circuit=circuit, mapping={}, parity_map={})
    probability = qubitron.contrib.quantum_volume.sample_heavy_set(
...
def test_sample_heavy_set_with_parity():
    ...
    sampler.run = MagicMock(return_value=result)
    circuit = qubitron.Circuit(qubitron.measure(*qubitron.LineQubit.range(4)))
    compilation_result = CompilationResult(
        circuit=circuit,
        mapping={q: q for q in qubitron.LineQubit.range(4)},
        parity_map={qubitron.LineQubit(0): qubitron.LineQubit(1), qubitron.LineQubit(2): qubitron.LineQubit(3)},
    )
    probability = qubitron.contrib.quantum_volume.sample_heavy_set(
...
def test_compile_circuit_router():
    ...
    qubitron.contrib.quantum_volume.compile_circuit(
        qubitron.Circuit(),
...
def test_compile_circuit():
    ...
    a, b, c = qubitron.LineQubit.range(3)
    model_circuit = qubitron.Circuit([qubitron.Moment([qubitron.X(a), qubitron.Y(b), qubitron.Z(c)])])
    compilation_result = qubitron.contrib.quantum_volume.compile_circuit(
        model_circuit,
        device_graph=ccr.gridqubits_to_graph_device(FakeDevice().qubits),
    ...
    assert qubitron.contrib.routing.ops_are_consistent_with_device_graph(
        compilation_result.circuit.all_operations(),
        qubitron.contrib.routing.gridqubits_to_graph_device(FakeDevice().qubits),
    )
...
def test_compile_circuit_replaces_swaps():
    ...
    assert (
        len(
            list(compilation_result.circuit.findall_operations_with_gate_type(qubitron.ops.SwapPowGate))
        )
        > 0
    )
        ...
        len(
            list(
                compilation_result.circuit.findall_operations_with_gate_type(
                    qubitron.contrib.acquaintance.SwapPermutationGate
                )
            )
        )
        == 0
...
def test_compile_circuit_with_readout_correction():
    ...
    a, b, c = qubitron.LineQubit.range(3)
    ap, bp, cp = qubitron.LineQubit.range(3, 6)
    model_circuit = qubitron.Circuit([qubitron.Moment([qubitron.X(a), qubitron.Y(b), qubitron.Z(c)])])
    compilation_result = qubitron.contrib.quantum_volume.compile_circuit(
        model_circuit,
        device_graph=ccr.gridqubits_to_graph_device(FakeDevice().qubits),
    ...
    assert compilation_result.circuit == qubitron.Circuit(
        [
            qubitron.Moment([qubitron.X(a), qubitron.Y(b), qubitron.Z(c)]),
            qubitron.Moment([qubitron.X(a), qubitron.X(b), qubitron.X(c)]),
            qubitron.Moment([qubitron.CNOT(a, ap), qubitron.CNOT(b, bp), qubitron.CNOT(c, cp)]),
            qubitron.Moment([qubitron.X(a), qubitron.X(b), qubitron.X(c)]),
        ]
    )
...
def test_compile_circuit_multiple_routing_attempts():
    ...
    more_operations = qubitron.Circuit([qubitron.X.on_each(qubits), qubitron.Y.on_each(qubits)])
    more_qubits = qubitron.Circuit([qubitron.X.on_each(qubitron.LineQubit.range(4))])
    well_routed = qubitron.Circuit([qubitron.X.on_each(qubits)])
    router_mock = MagicMock(
        side_effect=[
    ...
    compiler_mock = MagicMock(side_effect=lambda circuit: circuit)
    model_circuit = qubitron.Circuit([qubitron.X.on_each(qubits)])
...
def test_calculate_quantum_volume_result():
    ...
    results = qubitron.contrib.quantum_volume.calculate_quantum_volume(
        num_qubits=3,
        depth=3,
        num_circuits=1,
        device_graph=ccr.gridqubits_to_graph_device(qubitron.GridQubit.rect(3, 3)),
        samplers=[qubitron.Simulator()],
        routing_attempts=2,
        random_state=1,
    )

    model_circuit = qubitron.contrib.quantum_volume.generate_model_circuit(3, 3, random_state=1)
    assert len(results) == 1
    assert results[0].model_circuit == model_circuit
    assert results[0].heavy_set == qubitron.contrib.quantum_volume.compute_heavy_set(model_circuit)
    # Ensure that calling to_json on the results does not err.
    buffer = io.StringIO()
    qubitron.to_json(results, buffer)
...
def test_calculate_quantum_volume_result_with_device_graph():
    ...
    graph"""
    device_qubits = [qubitron.GridQubit(i, j) for i in range(2) for j in range(3)]
...
def test_calculate_quantum_volume_loop():
    ...
    # attempts.
    qubitron.contrib.quantum_volume.calculate_quantum_volume(
        num_qubits=5,
        depth=5,
        num_circuits=1,
        routing_attempts=2,
