...
def generate_model_circuit(
    num_qubits: int, depth: int, *, random_state: qubitron.RANDOM_STATE_OR_SEED_LIKE = None
) -> qubitron.Circuit:
    ...
    qubits = qubitron.LineQubit.range(num_qubits)
    circuit = qubitron.Circuit()
    random_state = qubitron.value.parse_random_state(random_state)
    ...
    for _ in range(depth):
        ...
        for k in range(0, num_qubits - 1, 2):
            ...
            special_unitary = qubitron.testing.random_special_unitary(4, random_state=random_state)

            # Convert the decomposed unitary to Qubitron operations and add them to
            # the circuit.
            circuit.append(
                qubitron.MatrixGate(special_unitary).on(
...
def compute_heavy_set(circuit: qubitron.Circuit) -> list[int]:
...
class CompilationResult:
    circuit: qubitron.Circuit
    mapping: dict[qubitron.Qid, qubitron.Qid]
    parity_map: dict[qubitron.Qid, qubitron.Qid]
...
def sample_heavy_set(
    ...
    *,
    repetitions=10_000,
    sampler: qubitron.Sampler = qubitron.Simulator(),
) -> float:
    ...
    circuit_copy = circuit + [qubitron.measure(q) for q in sorted_qubits]
    ...
    results = results.agg(lambda meas: qubitron.value.big_endian_bits_to_int(meas), axis=1)
...
def process_results(
    mapping: dict[qubitron.Qid, qubitron.Qid],
    parity_mapping: dict[qubitron.Qid, qubitron.Qid],
    trial_result: qubitron.Result,
) -> pd.DataFrame:
    ...
    inverse_mapping: dict[qubitron.Qid, qubitron.Qid] = {v: k for k, v in mapping.items()}
...
def compile_circuit(
    ...
    *,
    ...
    compiler: Callable[[qubitron.Circuit], qubitron.Circuit] | None = None,
    ...
    parity_map: dict[qubitron.Qid, qubitron.Qid] = {}  # original -> parity
    if add_readout_error_correction:
        ...
        for idx, qubit in enumerate(sorted(compiled_circuit.all_qubits())):
            ...
            parity_qubit = qubitron.LineQubit(qubit_num)
            compiled_circuit.append(qubitron.X(qubit))
            compiled_circuit.append(qubitron.CNOT(qubit, parity_qubit))
            compiled_circuit.append(qubitron.X(qubit))
    ...
    if router is None and routing_algo_name is None:
        ...
        # Github issue: https://github.com/amyssnippet/Qubitron/issues/2967
    ...
    def replace_swap_permutation_gate(op: qubitron.Operation, _):
        if isinstance(op.gate, qubitron.contrib.acquaintance.SwapPermutationGate):
            return [op.gate.swap_gate.on(*op.qubits)]
        return op

    routed_circuit = qubitron.map_operations_and_unroll(
...
) -> list[tuple[qubitron.Circuit, list[int]]]:
...
def execute_circuits(
    *,
    device_graph: nx.Graph,
    samplers: list[qubitron.Sampler],
    circuits: list[tuple[qubitron.Circuit, list[int]]],
    routing_attempts: int,
    compiler: Callable[[qubitron.Circuit], qubitron.Circuit] | None = None,
