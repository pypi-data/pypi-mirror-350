...
from qubitron import devices, protocols, qis, value
from qubitron.sim import simulator_base
from qubitron.sim.simulation_state import SimulationState
...
class MPSSimulator(
    ...
    def __init__(
        ...
        grouping: dict[qubitron.Qid, int] | None = None,
    ):
    ...
    def _create_step_result(self, sim_state: qubitron.SimulationStateBase[MPSState]):
    ...
    def _create_simulator_trial_result(
        ...
        final_simulator_state: qubitron.SimulationStateBase[MPSState],
    ) -> MPSTrialResult:
...
class MPSTrialResult(simulator_base.SimulationTrialResultBase['MPSState']):
    ...
    def _repr_pretty_(self, p: Any, cycle: bool):
        ...
        if cycle:
            ...
            p.text('qubitron.MPSTrialResult(...)')
        else:
            p.text(str(self))
...
class MPSSimulatorStepResult(simulator_base.StepResultBase['MPSState']):
    ...
    def __init__(self, sim_state: qubitron.SimulationStateBase[MPSState]):
    ...
    def _repr_pretty_(self, p: Any, cycle: bool):
        """iPython (Jupyter) pretty print."""
        p.text("qubitron.MPSSimulatorStepResult(...)" if cycle else self.__str__())
...
class _MPSHandler(qis.QuantumStateRepresentation):
    ...
    def create(
        ...
        *,
        ...
        # circuit_to_tensors in qubitron.contrib.quimb.state_vector.
    ...
    def sample(
        self, axes: Sequence[int], repetitions: int = 1, seed: qubitron.RANDOM_STATE_OR_SEED_LIKE = None
    ) -> np.ndarray:
        """Samples the MPS.
...
class MPSState(SimulationState[_MPSHandler]):
    ...
    def partial_trace(self, keep_qubits: set[qubitron.Qid]) -> np.ndarray:
