...
def circuit_to_tensors(
    circuit: qubitron.Circuit, qubits: Sequence[qubitron.Qid] | None = None, initial_state: int | None = 0
) -> tuple[list[qtn.Tensor], dict[qubitron.Qid, int], None]:
...
def tensor_unitary(circuit: qubitron.Circuit, qubits: Sequence[qubitron.Qid] | None = None) -> np.ndarray:
...
def circuit_for_expectation_value(
    circuit: qubitron.Circuit, pauli_string: qubitron.PauliString
) -> qubitron.Circuit:
    ...
    return qubitron.Circuit(
        [
            circuit,
            qubitron.Moment(gate.on(q) for q, gate in pauli_string.items()),
            qubitron.inverse(circuit),
        ]
    )


def tensor_expectation_value(
    circuit: qubitron.Circuit, pauli_string: qubitron.PauliString, max_ram_gb=16, tol=1e-6
) -> float:
    ...
    if QUIMB_VERSION[0] < (1, 3):
        warnings.warn(  # pragma: no cover
            ...
            'See https://github.com/amyssnippet/Qubitron/issues/3263'
        )
    else:
