...
def get_grid_moments(
    problem_graph: nx.Graph, two_qubit_gate=qubitron.ZZPowGate
) -> Iterator[qubitron.Moment]:
        ...
        for row in range(row_start + row_start_offset, row_end + row_end_offset, row_step):
            for col in range(col_start + col_start_offset, col_end + col_end_offset, col_step):
                ...
                yield two_qubit_gate(exponent=weight, global_shift=-0.5).on(
                    qubitron.GridQubit(*node1), qubitron.GridQubit(*node2)
                )

    # Horizontal
    yield qubitron.Moment(
        _interaction(
            col_start_offset=0,
            col_end_offset=-1,
            col_step=2,
...
def simplify_expectation_value_circuit(circuit_sand: qubitron.Circuit):
    ...
    while True:
        circuit = qubitron.merge_k_qubit_unitaries(circuit, k=1)
        circuit = qubitron.drop_negligible_operations(circuit, atol=1e-6)
        circuit = qubitron.merge_k_qubit_unitaries(circuit, k=2)
