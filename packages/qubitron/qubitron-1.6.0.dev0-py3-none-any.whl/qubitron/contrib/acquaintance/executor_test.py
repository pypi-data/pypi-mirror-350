...
def test_executor_explicit():
    ...
    assert execution_strategy.device == qubitron.UNCONSTRAINED_DEVICE
    ...
    with pytest.raises(TypeError):
        bad_strategy = qubitron.Circuit(qubitron.X(qubits[0]))
...
) -> dict[tuple[qubitron.Qid, ...], qubitron.Gate]:

    return {
        Q: qubitron.DiagonalGate(np.random.random(2**acquaintance_size))
        for Q in combinations(qubitron.LineQubit.range(num_qubits), acquaintance_size)
    }
...
def test_executor_random(
    num_qubits: int, acquaintance_size: int, gates: dict[tuple[qubitron.Qid, ...], qubitron.Gate]
):
    qubits = qubitron.LineQubit.range(num_qubits)
    circuit = cca.complete_acquaintance_strategy(qubits, acquaintance_size)

    logical_circuit = qubitron.Circuit([g(*Q) for Q, g in gates.items()])
...
def test_acquaintance_operation():
    n = 5
    physical_qubits = tuple(qubitron.LineQubit.range(n))
    logical_qubits = tuple(qubitron.NamedQubit(s) for s in ascii_lowercase[:n])
    ...
    for logical_indices in (logical_qubits, int_indices):
        ...
        assert qubitron.circuit_diagram_info(op) == qubitron.CircuitDiagramInfo(wire_symbols=wire_symbols)
