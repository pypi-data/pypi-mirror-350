...
from qubitron.contrib.acquaintance.gates import acquaint, SwapNetworkGate
...
def qubit_pairs_to_qubit_order(qubit_pairs: Sequence[Sequence[qubitron.Qid]]) -> list[qubitron.Qid]:
...
def quartic_paired_acquaintance_strategy(
    qubit_pairs: Iterable[tuple[qubitron.Qid, qubitron.Qid]],
) -> tuple[qubitron.Circuit, Sequence[qubitron.Qid]]:
    ...
    qubit_pairs = tuple(
        cast(tuple['qubitron.Qid', 'qubitron.Qid'], tuple(qubit_pair)) for qubit_pair in qubit_pairs
    )
