...
from qubitron import circuits, devices, ops, protocols, transformers
from qubitron.contrib.acquaintance.gates import AcquaintanceOpportunityGate
from qubitron.contrib.acquaintance.mutation_utils import expose_acquaintance_gates
...
class ExecutionStrategy(metaclass=abc.ABCMeta):
    ...
    @property
    @abc.abstractmethod
    def device(self) -> qubitron.Device:
    ...
    @abc.abstractmethod
    def get_operations(
        self, indices: Sequence[LogicalIndex], qubits: Sequence[qubitron.Qid]
    ) -> qubitron.OP_TREE:
...
class StrategyExecutorTransformer:
    ...
    def __call__(
        self, circuit: circuits.AbstractCircuit, context: qubitron.TransformerContext | None = None
    ) -> circuits.Circuit:
        """Executes an acquaintance strategy using qubitron.map_operations_and_unroll and
        mutates initial mapping.

        Args:
            circuit: `qubitron.Circuit` input circuit to transform.
            context: `qubitron.TransformerContext` storing common configurable
    ...
    def _map_func(self, op: qubitron.Operation, index) -> qubitron.OP_TREE:
...
class AcquaintanceOperation(ops.GateOperation):
    ...
    def __init__(self, qubits: Sequence[qubitron.Qid], logical_indices: Sequence[LogicalIndex]) -> None:
...
class GreedyExecutionStrategy(ExecutionStrategy):
    ...
    def __init__(
        ...
        device: qubitron.Device | None = None,
    ) -> None:
