...
def get_grid_device_graph(*args, **kwargs) -> nx.Graph:
    ...
    return gridqubits_to_graph_device(qubitron.GridQubit.rect(*args, **kwargs))


def gridqubits_to_graph_device(qubits: Iterable[qubitron.GridQubit]):
...
def nx_qubit_layout(graph: nx.Graph) -> dict[qubitron.Qid, tuple[float, float]]:
    ...
    >>> plt.clf()
    >>> g = ccr.gridqubits_to_graph_device(qubitron.GridQubit.rect(4,5))
    >>> pos = ccr.nx_qubit_layout(g)
    >>> nx.draw_networkx(g, pos=pos)

    """
    pos: dict[qubitron.Qid, tuple[float, float]] = {}

    _node_to_i_cache = None
    for node in graph.nodes:
        if isinstance(node, qubitron.GridQubit):
            pos[node] = (node.col, -node.row)
        elif isinstance(node, qubitron.LineQubit):
