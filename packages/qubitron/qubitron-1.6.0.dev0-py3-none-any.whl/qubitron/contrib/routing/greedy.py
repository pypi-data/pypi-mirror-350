...
from qubitron import circuits, ops, value
from qubitron.contrib import circuitdag
from qubitron.contrib.routing.initialization import get_initial_mapping
from qubitron.contrib.routing.swap_network import SwapNetwork
from qubitron.contrib.routing.utils import get_time_slices, ops_are_consistent_with_device_graph
...
class _GreedyRouter:
    ...
    def log_to_phys(self, *qubits: qubitron.Qid) -> Iterable[ops.Qid]:
        """Returns an iterator over the physical qubits mapped to by the given
        logical qubits."""
        return (self._log_to_phys[q] for q in qubits)

    def phys_to_log(self, *qubits: qubitron.Qid) -> Iterable[ops.Qid | None]:
