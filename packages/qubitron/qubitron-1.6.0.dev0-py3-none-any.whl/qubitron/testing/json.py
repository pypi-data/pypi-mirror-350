...
# The actual tests live in qubitron.protocols.json_serialization_test.py.
#
...
#       qubitron/google/json_test_data
...
#       qubitron/google/json_test_data/spec.py
#  - resolver cache: a resolver cache for the exposed types - see for example
#       qubitron/json_resolver_cache.py or qubitron/google/json_resolver_cache.py
...
class ModuleJsonTestSpec:
    ...
    # The unqualified public name is different from the qubitron_type field of the json object,
    # usually due to namespacing.
    custom_class_name_to_qubitron_type: dict[str, str] = dataclasses.field(default_factory=dict)
    ...
    def _get_all_public_classes(self) -> Iterator[tuple[str, type]]:
        for module in self.packages:
            for name, obj in inspect.getmembers(module):
                ...
                name = self.custom_class_name_to_qubitron_type.get(name, name)
                yield name, obj
...
def assert_json_roundtrip_works(obj, text_should_be=None, resolvers=None):
    ...
    """
    buffer = io.StringIO()
    qubitron.protocols.to_json(obj, buffer)
    ...
    buffer.seek(0)
    restored_obj = qubitron.protocols.read_json(buffer, resolvers=resolvers)
