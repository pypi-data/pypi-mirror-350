...
def assert_consistent_resolve_parameters(val: Any):
    names = qubitron.parameter_names(val)
    symbols = qubitron.parameter_symbols(val)

    assert {symbol.name for symbol in symbols} == names

    if not qubitron.is_parameterized(val):
        assert not names
        assert not symbols
    else:
        ...
        try:
            resolved = qubitron.resolve_parameters(val, {name: 0 for name in names})
        except Exception:
            pass
        else:
            assert not qubitron.parameter_names(resolved)
            assert not qubitron.parameter_symbols(resolved)

        # Try single-step resolution of parameters to names that map to zero.
        # All names should be preserved.
        param_dict: qubitron.ParamDictType = {
        ...
        resolver = qubitron.ParamResolver(param_dict)
        resolved = qubitron.resolve_parameters_once(val, resolver)
        assert qubitron.parameter_names(resolved) == set(name + '_CONSISTENCY_TEST' for name in names)
