...
from qubitron.testing import lin_alg_utils
...
def assert_decompose_ends_at_default_gateset(val: Any, ignore_known_gates: bool = True):
    """Asserts that qubitron.decompose(val) ends at default qubitron gateset or a known gate."""
    ...
    for op in [*ops.flatten_to_ops(protocols.decompose(d) for d in dec_once)]:
        ...
        ), f'{val} decomposed to {op}, which is not part of default qubitron target gateset.'
