...
class SemiBadPhaser:
    ...
    def _unitary_(self):
        a1 = qubitron.unitary(GoodPhaser(self.e[0]))
        a2 = qubitron.unitary(BadPhaser(self.e[1]))
...
def test_assert_phase_by_is_consistent_with_unitary() -> None:
    qubitron.testing.assert_phase_by_is_consistent_with_unitary(GoodPhaser(0.5))

    qubitron.testing.assert_phase_by_is_consistent_with_unitary(GoodQuditPhaser(0.5))

    with pytest.raises(AssertionError, match='Phased unitary was incorrect for index #0'):
        qubitron.testing.assert_phase_by_is_consistent_with_unitary(BadPhaser(0.5))

    with pytest.raises(AssertionError, match='Phased unitary was incorrect for index #1'):
        qubitron.testing.assert_phase_by_is_consistent_with_unitary(SemiBadPhaser([0.5, 0.25]))

    # Vacuous success.
    qubitron.testing.assert_phase_by_is_consistent_with_unitary(NotPhaser())
