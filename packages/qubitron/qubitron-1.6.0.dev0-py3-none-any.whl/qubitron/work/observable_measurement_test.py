...
from qubitron.work import _MeasurementSpec, BitstringAccumulator, group_settings_greedy, InitObsSetting
from qubitron.work.observable_measurement import (
...
def test_with_parameterized_layers() -> None:
    ...
    circuit = qubitron.Circuit([qubitron.H.on_each(*qs), qubitron.CZ(qs[0], qs[1]), qubitron.CZ(qs[1], qs[2])])
...
def test_get_params_for_setting() -> None:
    ...
    init_state = qubitron.KET_PLUS(a) * qubitron.KET_ZERO(b)
    observable = qubitron.X(a) * qubitron.Y(b)
    setting = cw.InitObsSetting(init_state=init_state, observable=observable)
    padded_setting = _pad_setting(setting, qubits=qubits)
    assert padded_setting.init_state == qubitron.KET_PLUS(a) * qubitron.KET_ZERO(b) * qubitron.KET_ZERO(c)
    assert padded_setting.observable == qubitron.X(a) * qubitron.Y(b) * qubitron.Z(c)
    assert init_state == qubitron.KET_PLUS(a) * qubitron.KET_ZERO(b)
    assert observable == qubitron.X(a) * qubitron.Y(b)
    ...
    circuit = qubitron.Circuit(qubitron.I.on_each(*qubits))
    ...
    psi = qubitron.Simulator().simulate(circuit, param_resolver=params)
    ma = qubitron.Z(a).expectation_from_state_vector(psi.final_state_vector, qubit_map=psi.qubit_map)
    mb = qubitron.Z(b).expectation_from_state_vector(psi.final_state_vector, qubit_map=psi.qubit_map)
    mc = qubitron.Z(c).expectation_from_state_vector(psi.final_state_vector, qubit_map=psi.qubit_map)
...
def test_params_and_settings() -> None:
    ...
    (q,) = qubits
    tests = [
        (qubitron.KET_ZERO, qubitron.Z, 1),
        (qubitron.KET_ONE, qubitron.Z, -1),
        (qubitron.KET_PLUS, qubitron.X, 1),
        (qubitron.KET_MINUS, qubitron.X, -1),
        (qubitron.KET_IMAG, qubitron.Y, 1),
        (qubitron.KET_MINUS_IMAG, qubitron.Y, -1),
        (qubitron.KET_ZERO, qubitron.Y, 0),
    ]
    ...
    for init, obs, coef in tests:
        ...
        circuit = circuit[:-1]  # remove measurement so we can compute <Z>
        psi = qubitron.Simulator().simulate(circuit, param_resolver=params)
        z = qubitron.Z(q).expectation_from_state_vector(psi.final_state_vector, qubit_map=psi.qubit_map)
...
class _MockBitstringAccumulator(BitstringAccumulator):
    def __init__(self):
        super().__init__(
            meas_spec=None,
            simul_settings=None,
            qubit_to_index={q: i for i, q in enumerate(qubitron.LineQubit.range(5))},
        )
...
def _set_up_meas_specs_for_testing():
    ...
    bsa = cw.BitstringAccumulator(
        meas_spec, [], {q: i for i, q in enumerate(qubitron.LineQubit.range(3))}
    )
...
def test_measure_grouped_settings_read_checkpoint(tmpdir) -> None:
    ...
    setting = cw.InitObsSetting(init_state=qubitron.KET_ZERO(q), observable=qubitron.Z(q))
    grouped_settings = {setting: [setting]}
    circuit = qubitron.Circuit(qubitron.I.on_each(*qubits))
    ...
    results = qubitron.read_json(f'{tmpdir}/obs.json')
    (result,) = results  # one group
...
@pytest.mark.parametrize(
    ['circuit', 'observable'],
    [
        (qubitron.Circuit(qubitron.X(Q) ** 0.2), qubitron.Z(Q)),
        (qubitron.Circuit(qubitron.X(Q) ** -0.5, qubitron.Z(Q) ** 0.2), qubitron.Y(Q)),
        (qubitron.Circuit(qubitron.Y(Q) ** 0.5, qubitron.Z(Q) ** 0.2), qubitron.X(Q)),
    ],
)
...
def test_measure_observable_grouper(grouper) -> None:
    circuit = qubitron.Circuit(qubitron.X(Q) ** 0.2)
    observables = [qubitron.Z(Q), qubitron.Z(qubitron.NamedQubit('q2'))]
