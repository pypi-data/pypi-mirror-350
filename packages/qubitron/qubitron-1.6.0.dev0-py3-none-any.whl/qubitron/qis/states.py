...
def infer_qid_shape(*states: qubitron.QUANTUM_STATE_LIKE) -> tuple[int, ...]:
    ...
    if qid_shape is None:
        raise ValueError(
            ...
            'an issue at https://github.com/amyssnippet/Qubitron.'
        )
...
def dirac_notation(
    ...
    >>> state_vector = np.array([1/np.sqrt(2), 1/np.sqrt(2)], dtype=np.complex64)
    >>> print(qubitron.dirac_notation(state_vector))
    0.71|0⟩ + 0.71|1⟩
...
def to_valid_state_vector(
    state_rep: qubitron.STATE_VECTOR_LIKE,
...
def to_valid_density_matrix(
    density_matrix_rep: np.ndarray | qubitron.STATE_VECTOR_LIKE,
    num_qubits: int | None = None,
    *,  # Force keyword arguments
    qid_shape: tuple[int, ...] | None = None,
    dtype: DTypeLike | None = None,
