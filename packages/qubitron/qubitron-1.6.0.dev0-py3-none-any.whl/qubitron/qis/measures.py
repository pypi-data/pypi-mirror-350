...
from qubitron.qis.states import (
    infer_qid_shape,
    quantum_state,
    QuantumState,
...
def _validate_product_state(state: qubitron.ProductState, qid_shape: tuple[int, ...] | None) -> None:
...
def fidelity(
    state1: qubitron.QUANTUM_STATE_LIKE,
    state2: qubitron.QUANTUM_STATE_LIKE,
        ...
        if state1.shape == state2.shape:
            if qid_shape is None:
                ...
                raise ValueError(
                    ...
                    'using a wrapper function like qubitron.density_matrix.'
                )
...
def von_neumann_entropy(
    state: qubitron.QUANTUM_STATE_LIKE,
    qid_shape: tuple[int, ...] | None = None,
    validate: bool = True,
    atol: float = 1e-7,
) -> float:
...
def entanglement_fidelity(operation: qubitron.SupportsKraus) -> float:
