...
from qubitron import circuits, ops, study, vis
...
class TensoredConfusionMatrices:
    ...
    Use `qubitron.measure_confusion_matrix(sampler, qubits, repetitions)` to perform
    an experiment on `sampler` and construct the `qubitron.TensoredConfusionMatrices` object.
    """

    def __init__(
        self,
        confusion_matrices: np.ndarray | Sequence[np.ndarray],
        measure_qubits: Sequence[qubitron.Qid] | Sequence[Sequence[qubitron.Qid]],
        *,
        repetitions: int,
        timestamp: float,
    ):
        """Initializes `qubitron.TensoredConfusionMatrices`.
        ...
        measure_qubits = cast(
            Sequence[Sequence['qubitron.Qid']],
        ...
        self._cache: dict[tuple[qubitron.Qid, ...], np.ndarray] = {}
    ...
    def from_measurement(
        cls, gate: ops.MeasurementGate, qubits: Sequence[qubitron.Qid]
    ...
    @property
    def measure_qubits(self) -> tuple[tuple[qubitron.Qid, ...], ...]:
    ...
    def _get_vars(self, qubit_pattern: Sequence[qubitron.Qid]) -> list[int]:
        in_vars = [2 * self._qubits_to_idx[q] for q in qubit_pattern]
        out_vars = [2 * self._qubits_to_idx[q] + 1 for q in qubit_pattern]
        return in_vars + out_vars

    def _confusion_matrix(self, qubits: Sequence[qubitron.Qid]) -> np.ndarray:
    ...
    def confusion_matrix(self, qubits: Sequence[qubitron.Qid] | None = None) -> np.ndarray:
    ...
    def correction_matrix(self, qubits: Sequence[qubitron.Qid] | None = None) -> np.ndarray:
    ...
    def readout_mitigation_pauli_uncorrelated(
        self, qubits: Sequence[qubitron.Qid], measured_bitstrings: np.ndarray
    ) -> tuple[float, float]:
    ...
    def __repr__(self) -> str:
        return (
            f"qubitron.TensoredConfusionMatrices("
...
def measure_confusion_matrix(
    ...
    qubits: Sequence[qubitron.Qid] | Sequence[Sequence[qubitron.Qid]],
    ...
    """
    qubits = cast(
        Sequence[Sequence['qubitron.Qid']], [qubits] if isinstance(qubits[0], ops.Qid) else qubits
    )
    confusion_matrices = []
    for qs in qubits:
