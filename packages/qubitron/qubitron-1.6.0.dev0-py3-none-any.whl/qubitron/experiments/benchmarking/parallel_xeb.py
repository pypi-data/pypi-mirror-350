...
import qubitron.experiments.two_qubit_xeb as tqxeb
import qubitron.experiments.xeb_fitting as xeb_fitting
from qubitron import circuits, devices, ops, protocols, sim, value

if TYPE_CHECKING:
    import qubitron

_TARGET_T = Union['qubitron.Gate', 'qubitron.Operation', 'qubitron.AbstractCircuit']
_QUBIT_PAIR_T = tuple['qubitron.GridQubit', 'qubitron.GridQubit']
_CANONICAL_TARGET_T = Union['qubitron.Operation', dict[_QUBIT_PAIR_T, 'qubitron.Operation']]
...
class XEBWideCircuitInfo:
    ...
    def from_narrow_circuits(
        circuit_templates: Sequence[qubitron.Circuit],
...
def _target_to_operation(target: _TARGET_T) -> qubitron.Operation:
...
def _transform_moment_with_circuit_ops_to_moment_with_single_op(
    ...
    Args:
        moment: A qubitron moment composed of single and two qubit operations.
...
def simulate_circuit(
    simulator: qubitron.Simulator, circuit: qubitron.Circuit, cycle_depths: Sequence[int]
) -> Sequence[np.ndarray]:
    """Simulates the given circuit and returns the state probabilities for each cycle depth.

    Args:
        simulator: A qubitron simulator.
...
def sample_all_circuits(
    sampler: qubitron.Sampler, circuits: Sequence[qubitron.Circuit], repetitions: int
) -> Sequence[dict[str, np.ndarray]]:
    """Calls sampler.run_batch on the given circuits and estimates the state probabilities.

    Args:
        sampler: A qubitron sampler.
