...
from qubitron.experiments.xeb_simulation import simulate_2q_xeb_circuits
...
def benchmark_2q_xeb_fidelities(
    ...
    circuits: Sequence[qubitron.Circuit],
    ...
    `qubitron.experiments.fidelity_estimation.least_squares_xeb_fidelity_from_expectations`, but
...
class XEBCharacterizationOptions(ABC):
    @staticmethod
    @abstractmethod
    def should_parameterize(op: qubitron.Operation) -> bool:
        """Whether to replace `op` with a parameterized version."""

    @abstractmethod
    def get_parameterized_gate(self) -> qubitron.Gate:
...
def _try_defaults_from_unitary(gate: qubitron.Gate) -> dict[str, qubitron.TParamVal] | None:
...
def phased_fsim_angles_from_gate(gate: qubitron.Gate) -> dict[str, qubitron.TParamVal]:
    """For a given gate, return a dictionary mapping '{angle}_default' to its noiseless value
    for the five PhasedFSim angles."""
    defaults: dict[str, qubitron.TParamVal] = {
        'theta_default': 0.0,
        'zeta_default': 0.0,
        'chi_default': 0.0,
        'gamma_default': 0.0,
...
class XEBPhasedFSimCharacterizationOptions(XEBCharacterizationOptions):
    ...
    def get_initial_simplex_and_names(
        ...
        We also return a list of parameter names so the Qubitron `param_resovler`
    ...
    def with_defaults_from_gate(
        self, gate: qubitron.Gate, gate_to_angles_func=phased_fsim_angles_from_gate
    ):
...
QPair_T = tuple['qubitron.Qid', 'qubitron.Qid']
...
def characterize_phased_fsim_parameters_with_xeb(
    sampled_df: pd.DataFrame,
    parameterized_circuits: list[qubitron.Circuit],
    ...
    final_params: qubitron.ParamDictType = dict(zip(names, optimization_result.x))
