...
def test_state_tomography_diagonal() -> None:
    ...
    for state in range(2**n):
        ...
        for i, q in enumerate(qubits):
            bit = state & (1 << (n - i - 1))
            if bit:
                circuit.append(qubitron.X(q))
        res = qubitron.experiments.state_tomography(
            qubitron.Simulator(seed=87539319),
            qubits,
            circuit,
            repetitions=1000,
...
def test_state_tomography_ghz_state() -> None:
    circuit = qubitron.Circuit()
    circuit.append(qubitron.H(qubitron.LineQubit(0)))
    circuit.append(qubitron.CNOT(qubitron.LineQubit(0), qubitron.LineQubit(1)))
    circuit.append(qubitron.CNOT(qubitron.LineQubit(0), qubitron.LineQubit(2)))
    res = qubitron.experiments.state_tomography(
        qubitron.Simulator(seed=87539319),
...
def test_make_experiment_no_rots() -> None:
    exp = qubitron.experiments.StateTomographyExperiment(
        [qubitron.LineQubit(0), qubitron.LineQubit(1), qubitron.LineQubit(2)]
    )
    assert len(exp.rot_sweep) > 0


def compute_density_matrix(circuit: qubitron.Circuit, qubits: Sequence[qubitron.Qid]) -> np.ndarray:
...
@pytest.mark.parametrize(
    'circuit, qubits',
    (
        (qubitron.Circuit(qubitron.X(Q0) ** 0.25), (Q0,)),
        (qubitron.Circuit(qubitron.CNOT(Q0, Q1) ** 0.25), (Q0, Q1)),
        (qubitron.Circuit(qubitron.CNOT(Q0, Q1) ** 0.25), (Q1, Q0)),
        (qubitron.Circuit(qubitron.TOFFOLI(Q0, Q1, Q2)), (Q1, Q0, Q2)),
        (
            qubitron.Circuit(
                qubitron.H(Q0),
                qubitron.H(Q1),
...
def test_density_matrix_from_state_tomography_is_correct(circuit, qubits) -> None:
    sim = qubitron.Simulator(seed=87539319)
    tomography_result = qubitron.experiments.state_tomography(sim, qubits, circuit, repetitions=5000)
...
@pytest.mark.parametrize(
    'circuit',
    (
        qubitron.Circuit(qubitron.CNOT(Q0, Q1) ** 0.3),
        qubitron.Circuit(qubitron.H(Q0), qubitron.CNOT(Q0, Q1)),
        qubitron.Circuit(qubitron.X(Q0) ** 0.25, qubitron.ISWAP(Q0, Q1)),
    ),
)
def test_agrees_with_two_qubit_state_tomography(circuit) -> None:
    ...
    two_qubit_tomography_result = qubitron.experiments.two_qubit_state_tomography(
