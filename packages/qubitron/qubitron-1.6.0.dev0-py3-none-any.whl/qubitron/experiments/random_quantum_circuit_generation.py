...
from qubitron import circuits, devices, ops, protocols, value
...
QidPairT = tuple['qubitron.Qid', 'qubitron.Qid']
GridQubitPairT = tuple['qubitron.GridQubit', 'qubitron.GridQubit']
...
class GridInteractionLayer(Container[GridQubitPairT]):
    ...
    * *-
    ...
    def __repr__(self) -> str:
        return (
            'qubitron.experiments.GridInteractionLayer('
...
def random_rotations_between_two_qubit_circuit(
    ...
    two_qubit_op_factory: Callable[
        [qubitron.Qid, qubitron.Qid, np.random.RandomState], qubitron.OP_TREE
...
def generate_library_of_2q_circuits(
    ...
    *,
    ...
    q0: qubitron.Qid = devices.LineQubit(0),
    q1: qubitron.Qid = devices.LineQubit(1),
    random_state: qubitron.RANDOM_STATE_OR_SEED_LIKE = None,
    tags: Sequence[Any] = (),
) -> list[qubitron.Circuit]:
...
def _pairs_from_moment(moment: qubitron.Moment) -> list[QidPairT]:
...
def get_random_combinations_for_layer_circuit(
    n_library_circuits: int,
    n_combinations: int,
    layer_circuit: qubitron.Circuit,
    random_state: qubitron.RANDOM_STATE_OR_SEED_LIKE = None,
) -> list[CircuitLibraryCombination]:
...
def _coupled_qubit_pairs(qubits: list[qubitron.GridQubit]) -> list[GridQubitPairT]:
    pairs = []
...
class _RandomSingleQubitLayerFactory:
    ...
    def new_layer(self, previous_single_qubit_layer: qubitron.Moment) -> qubitron.Moment:
        def random_gate(qubit: qubitron.Qid) -> qubitron.Gate:
...
class _FixedSingleQubitLayerFactory:
    def __init__(self, fixed_single_qubit_layer: dict[qubitron.Qid, qubitron.Gate]) -> None:
...
def _single_qubit_gates_arg_to_factory(
    single_qubit_gates: Sequence[qubitron.Gate], qubits: Sequence[qubitron.Qid], prng: np.random.RandomState
