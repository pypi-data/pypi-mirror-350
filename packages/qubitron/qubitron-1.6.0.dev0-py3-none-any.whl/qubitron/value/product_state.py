...
class _NamedOneQubitState(metaclass=abc.ABCMeta):
    ...
    def on(self, qubit: qubitron.Qid) -> ProductState:
...
class ProductState:
    ...
    For example, the |00⟩ state is `qubitron.KET_ZERO(q0) * qubitron.KET_ZERO(q1)`.
    The |+⟩ state is a length-1 tensor product state and can be constructed
    with `qubitron.KET_PLUS(q0)`.
    """

    states: dict[qubitron.Qid, _NamedOneQubitState]
    ...
    @property
    def qubits(self) -> Sequence[qubitron.Qid]:
        return sorted(self.states.keys())

    def __mul__(self, other: qubitron.ProductState) -> qubitron.ProductState:
    ...
    def __repr__(self) -> str:
        ...
        return f'qubitron.ProductState({{{states_dict_repr}}})'

    def __getitem__(self, qubit: qubitron.Qid) -> _NamedOneQubitState:
        """Return the _NamedOneQubitState at the given qubit."""
        return self.states[qubit]

    def __iter__(self) -> Iterator[tuple[qubitron.Qid, _NamedOneQubitState]]:
    ...
    def state_vector(self, qubit_order: qubitron.QubitOrder | None = None) -> np.ndarray:
    ...
    def projector(self, qubit_order: qubitron.QubitOrder | None = None) -> np.ndarray:
...
class _PauliEigenState(_NamedOneQubitState):
    ...
    def __repr__(self) -> str:
        return f'qubitron.{self._symbol}.basis[{self.eigenvalue:+d}]'

    @abc.abstractmethod
    def stabilized_by(self) -> tuple[int, qubitron.Pauli]:
        pass
