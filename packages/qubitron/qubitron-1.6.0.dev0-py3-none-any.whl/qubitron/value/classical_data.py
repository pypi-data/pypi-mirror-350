...
from qubitron.value import digits, value_equality_attr
...
class MeasurementType(enum.IntEnum):
    ...
    `MEASUREMENT` represent measurements of a `Qubitron.Qid`
    ...
    def __repr__(self):
        return f'qubitron.MeasurementType.{self.name}'


class ClassicalDataStoreReader(abc.ABC):
    @abc.abstractmethod
    def keys(self) -> tuple[qubitron.MeasurementKey, ...]:
        """Gets the measurement keys in the order they were stored."""

    @property
    @abc.abstractmethod
    def records(self) -> Mapping[qubitron.MeasurementKey, list[tuple[int, ...]]]:
        """Gets the a mapping from measurement key to measurement records."""

    @property
    @abc.abstractmethod
    def channel_records(self) -> Mapping[qubitron.MeasurementKey, list[int]]:
        """Gets the a mapping from measurement key to channel measurement records."""

    @abc.abstractmethod
    def get_int(self, key: qubitron.MeasurementKey, index=-1) -> int:
    ...
    @abc.abstractmethod
    def get_digits(self, key: qubitron.MeasurementKey, index=-1) -> tuple[int, ...]:
...
class ClassicalDataStore(ClassicalDataStoreReader, abc.ABC):
    ...
    def record_measurement(
        self, key: qubitron.MeasurementKey, measurement: Sequence[int], qubits: Sequence[qubitron.Qid]
    ):
    ...
    @abc.abstractmethod
    def record_channel_measurement(self, key: qubitron.MeasurementKey, measurement: int):
...
class ClassicalDataDictionaryStore(ClassicalDataStore):
    ...
    def __init__(
        self,
        *,
        _records: dict[qubitron.MeasurementKey, list[tuple[int, ...]]] | None = None,
        _measured_qubits: dict[qubitron.MeasurementKey, list[tuple[qubitron.Qid, ...]]] | None = None,
        _channel_records: dict[qubitron.MeasurementKey, list[int]] | None = None,
        _measurement_types: dict[qubitron.MeasurementKey, qubitron.MeasurementType] | None = None,
    ):
        ...
        if _channel_records is None:
            _channel_records = {}
        self._records: dict[qubitron.MeasurementKey, list[tuple[int, ...]]] = _records
        self._measured_qubits: dict[qubitron.MeasurementKey, list[tuple[qubitron.Qid, ...]]] = (
            _measured_qubits
        )
        self._channel_records: dict[qubitron.MeasurementKey, list[int]] = _channel_records
        self._measurement_types: dict[qubitron.MeasurementKey, qubitron.MeasurementType] = (
            _measurement_types
        )
    ...
    @property
    def measured_qubits(self) -> Mapping[qubitron.MeasurementKey, list[tuple[qubitron.Qid, ...]]]:
        """Gets the a mapping from measurement key to the qubits measured."""
        return self._measured_qubits

    @property
    def measurement_types(self) -> Mapping[qubitron.MeasurementKey, qubitron.MeasurementType]:
    ...
    def __repr__(self):
        rep = 'qubitron.ClassicalDataDictionaryStore('
        if self.records:
