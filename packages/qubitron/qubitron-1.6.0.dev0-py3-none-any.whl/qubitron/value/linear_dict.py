...
def _format_coefficient(format_spec: str, coefficient: qubitron.TParamValComplex) -> str:
...
def _format_term(format_spec: str, vector: TVector, coefficient: qubitron.TParamValComplex) -> str:
...
def _format_terms(terms: Iterable[tuple[TVector, qubitron.TParamValComplex]], format_spec: str):
...
class LinearDict(Generic[TVector], MutableMapping[TVector, 'qubitron.TParamValComplex']):
    ...
    def __init__(
        self,
        terms: Mapping[TVector, qubitron.TParamValComplex] | None = None,
        ...
        self._terms: dict[TVector, qubitron.TParamValComplex] = {}
    ...
    def values(self) -> ValuesView[qubitron.TParamValComplex]:
        snapshot = self.copy().clean(atol=0)
        return snapshot._terms.values()

    def items(self) -> ItemsView[TVector, qubitron.TParamValComplex]:
        snapshot = self.copy().clean(atol=0)
    ...
    @overload
    def update(
        self, other: Mapping[TVector, qubitron.TParamValComplex], **kwargs: qubitron.TParamValComplex
    ) -> None:
        pass
    ...
    def update(
        self,
        other: Iterable[tuple[TVector, qubitron.TParamValComplex]],
        **kwargs: qubitron.TParamValComplex,
    ) -> None:
        pass

    @overload
    def update(self, *args: Any, **kwargs: qubitron.TParamValComplex) -> None:
        pass
    ...
    @overload
    def get(self, vector: TVector) -> qubitron.TParamValComplex:
        pass

    @overload
    def get(self, vector: TVector, default: TDefault) -> qubitron.TParamValComplex | TDefault:
        pass
    ...
    def __getitem__(self, vector: TVector) -> qubitron.TParamValComplex:
        return self._terms.get(vector, 0)

    def __setitem__(self, vector: TVector, coefficient: qubitron.TParamValComplex) -> None:
    ...
    def __imul__(self, a: qubitron.TParamValComplex) -> Self:
        for vector in self:
            self._terms[vector] *= a
        self.clean(atol=0)
        return self

    def __mul__(self, a: qubitron.TParamValComplex) -> Self:
        result = self.copy()
        result *= a
        return result.copy()

    def __rmul__(self, a: qubitron.TParamValComplex) -> Self:
        return self.__mul__(a)

    def __truediv__(self, a: qubitron.TParamValComplex) -> Self:
    ...
    def __eq__(self, other: Any) -> bool:
        ...
        numerical error in floating point coefficients. Use qubitron.approx_eq()
        instead.
        """
    ...
    def __repr__(self) -> str:
        ...
        return f'qubitron.{class_name}({coefficients!r})'
    ...
    def _resolve_parameters_(self, resolver: qubitron.ParamResolver, recursive: bool) -> LinearDict:
        result = self.copy()
        result.update(
            {
