...
from qubitron.protocols import json_serialization, measurement_key_protocol as mkp
from qubitron.value import measurement_key
...
class Condition(abc.ABC):
    ...
    @abc.abstractmethod
    def replace_key(self, current: qubitron.MeasurementKey, replacement: qubitron.MeasurementKey):
        """Replaces the control keys."""

    @abc.abstractmethod
    def resolve(self, classical_data: qubitron.ClassicalDataStoreReader) -> bool:
    ...
    def _qasm_(self, args: qubitron.QasmArgs, **kwargs) -> str | None:
        return self.qasm

    def _with_measurement_key_mapping_(self, key_map: Mapping[str, str]) -> qubitron.Condition:
        condition = self
        for k in self.keys:
            condition = condition.replace_key(k, mkp.with_measurement_key_mapping(k, key_map))
        return condition

    def _with_key_path_prefix_(self, path: tuple[str, ...]) -> qubitron.Condition:
        condition = self
        for k in self.keys:
...
class KeyCondition(Condition):
    ...
    def __repr__(self):
        if self.index != -1:
            return f'qubitron.KeyCondition({self.key!r}, {self.index})'
        return f'qubitron.KeyCondition({self.key!r})'
...
class BitMaskKeyCondition(Condition):
    ...
    The bits in the bitmask have the same order as the qubits passed to `qubitron.measure(...)`. That's
    ...
    key: qubitron.MeasurementKey = attrs.field(
        converter=lambda x: (
            x
    ...
    def create_equal_mask(
        key: qubitron.MeasurementKey, bitmask: int, *, index: int = -1
    ) -> BitMaskKeyCondition:
    ...
    def __repr__(self):
        ...
        return f'qubitron.BitMaskKeyCondition({parameters})'
...
class SympyCondition(Condition):
    ...
    def __repr__(self):
        return f'qubitron.SympyCondition({proper_repr(self.expr)})'
