...
from qubitron._compat import cached_method, proper_repr
from qubitron._doc import document

if TYPE_CHECKING:
    import qubitron

DURATION_LIKE = Union[None, datetime.timedelta, 'qubitron.Duration']
document(
    DURATION_LIKE,
    """A `qubitron.Duration` or value that can trivially converted to one.

    A `datetime.timedelta` is a `qubitron.DURATION_LIKE`. It is converted while
    preserving its duration.

    `None` is a `qubitron.DURATION_LIKE` that converts into a zero-length duration.
...
class Duration:
    ...
    def __init__(
        ...
        *,  # Force keyword args.
        ...
        Raises:
            TypeError: If the given value is not of a `qubitron.DURATION_LIKE` type.

        Examples:
            >>> print(qubitron.Duration(nanos=100))
            100 ns
            >>> print(qubitron.Duration(micros=1.5 * sympy.Symbol('t')))
            (1500.0*t) ns
        """
        ...
        if value is not None and value != 0:
            if isinstance(value, datetime.timedelta):
                ...
                raise TypeError(f'Not a `qubitron.DURATION_LIKE`: {repr(value)}.')
    ...
    def _resolve_parameters_(self, resolver: qubitron.ParamResolver, recursive: bool) -> Duration:
    ...
    def __repr__(self) -> str:
        ...
        return f'qubitron.Duration({unit}={proper_repr(amount)})'
