...
ParamDictType = dict['qubitron.TParamKey', 'qubitron.TParamValComplex']
ParamMappingType = Mapping['qubitron.TParamKey', 'qubitron.TParamValComplex']
document(ParamDictType, """Dictionary from symbols to values.""")
document(ParamMappingType, """Immutable map from symbols to values.""")

ParamResolverOrSimilarType = Union['qubitron.ParamResolver', ParamMappingType, None]
document(
...
class ParamResolver:
    ...
    def __new__(cls, param_dict: qubitron.ParamResolverOrSimilarType = None):
        if isinstance(param_dict, ParamResolver):
            return param_dict
        return super().__new__(cls)

    def __init__(self, param_dict: qubitron.ParamResolverOrSimilarType = None) -> None:
    ...
    def value_of(
        self, value: qubitron.TParamKey | qubitron.TParamValComplex, recursive: bool = True
        ...
        qubitron.flatten or other means so that formula resolution is avoided.
    ...
    def _value_of_recursive(self, value: qubitron.TParamKey) -> qubitron.TParamValComplex:
    ...
    def _resolve_parameters_(self, resolver: ParamResolver, recursive: bool) -> ParamResolver:
        new_dict: dict[qubitron.TParamKey, float | str | sympy.Symbol | sympy.Expr] = {
    ...
    def __getitem__(self, key: qubitron.TParamKey | qubitron.TParamValComplex) -> qubitron.TParamValComplex:
    ...
    def __repr__(self) -> str:
        ...
        return f'qubitron.ParamResolver({param_dict_repr})'
