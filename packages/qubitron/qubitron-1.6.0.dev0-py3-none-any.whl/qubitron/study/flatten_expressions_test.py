...
import qubitron
from qubitron.study import flatten_expressions
...
def test_flattener_value_of() -> None:
    ...
    assert qubitron.flatten([sympy.Symbol('c') / 2 + 1, sympy.Symbol('c/2') + 1])[0] == [
        sympy.Symbol('<c/2 + 1>'),
...
def test_expression_map_repr() -> None:
    qubitron.testing.assert_equivalent_repr(qubitron.ExpressionMap({'a': 'b'}))


def test_flatten_circuit() -> None:
    qubit = qubitron.LineQubit(0)
    a = sympy.Symbol('a')
    circuit = qubitron.Circuit(qubitron.X(qubit) ** a, qubitron.X(qubit) ** (1 + a / 2))

    c_flat, expr_map = qubitron.flatten(circuit)

    c_expected = qubitron.Circuit(qubitron.X(qubit) ** a, qubitron.X(qubit) ** sympy.Symbol('<a/2 + 1>'))
    assert c_flat == c_expected
    assert isinstance(expr_map, qubitron.ExpressionMap)
...
def test_transform_params() -> None:
    qubit = qubitron.LineQubit(0)
    a = sympy.Symbol('a')
    circuit = qubitron.Circuit(qubitron.X(qubit) ** (a / 4), qubitron.X(qubit) ** (1 + a / 2))
    params = {'a': 3}

    _, new_params = qubitron.flatten_with_params(circuit, params)
...
def test_transform_sweep() -> None:
    ...
    a = sympy.Symbol('a')
    circuit = qubitron.Circuit(qubitron.X(qubit) ** (a / 4), qubitron.X(qubit) ** (1 + a / 2))
    sweep = qubitron.Linspace(a, start=0, stop=3, length=4)

    _, new_sweep = qubitron.flatten_with_sweep(circuit, sweep)
    assert isinstance(new_sweep, qubitron.Sweep)
    resolvers = list(new_sweep)

    expected_resolvers = [
        qubitron.ParamResolver({'<a/4>': 0.0, '<a/2 + 1>': 1.0}),
        qubitron.ParamResolver({'<a/4>': 0.25, '<a/2 + 1>': 1.5}),
        qubitron.ParamResolver({'<a/4>': 0.5, '<a/2 + 1>': 2}),
        qubitron.ParamResolver({'<a/4>': 0.75, '<a/2 + 1>': 2.5}),
    ]
...
def test_resolver_new() -> None:
    flattener = flatten_expressions._ParamFlattener({'a': 'b'})
    flattener2 = qubitron.ParamResolver(flattener)
...
def test_transformed_sweep() -> None:
    a = sympy.Symbol('a')
    sweep = qubitron.Linspace('a', start=0, stop=3, length=4)
    expr_map = qubitron.ExpressionMap({a / 4: 'x0', 1 - a / 2: 'x1'})
...
def test_transformed_sweep_equality() -> None:
    a = sympy.Symbol('a')
    sweep = qubitron.Linspace('a', start=0, stop=3, length=4)
    expr_map = qubitron.ExpressionMap({a / 4: 'x0', 1 - a / 4: 'x1'})

    sweep2 = qubitron.Linspace(a, start=0, stop=3, length=4)
    expr_map2 = qubitron.ExpressionMap({a / 4: 'x0', 1 - a / 4: 'x1'})

    sweep3 = qubitron.Linspace(a, start=0, stop=3, length=20)
    expr_map3 = qubitron.ExpressionMap({a / 20: 'x0', 1 - a / 20: 'x1'})
