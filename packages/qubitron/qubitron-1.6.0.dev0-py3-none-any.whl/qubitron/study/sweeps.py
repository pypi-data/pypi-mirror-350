...
from qubitron.study import resolver

if TYPE_CHECKING:
    import qubitron

Params = Iterable[tuple['qubitron.TParamKey', 'qubitron.TParamVal']]
ProductOrZipSweepLike = dict['qubitron.TParamKey', Union['qubitron.TParamVal', Sequence['qubitron.TParamVal']]]
...
class Sweep(metaclass=abc.ABCMeta):
    ...
    @property
    @abc.abstractmethod
    def keys(self) -> list[qubitron.TParamKey]:
...
class Product(Sweep):
    ...
    def __repr__(self) -> str:
        ...
        return f'qubitron.Product({factors_repr})'
...
class Concat(Sweep):
    ...
    def __repr__(self) -> str:
        ...
        return f'qubitron.Concat({sweeps_repr})'
...
class Zip(Sweep):
    ...
    def __repr__(self) -> str:
        sweeps_repr = ', '.join(repr(s) for s in self.sweeps)
        return f'qubitron.Zip({sweeps_repr})'
...
class ZipLongest(Zip):
    ...
    def __repr__(self) -> str:
        sweeps_repr = ', '.join(repr(s) for s in self.sweeps)
        return f'qubitron_google.ZipLongest({sweeps_repr})'
...
class SingleSweep(Sweep):
    ...
    def __init__(self, key: qubitron.TParamKey) -> None:
...
class Points(SingleSweep):
    ...
    def __init__(
        self, key: qubitron.TParamKey, points: Sequence[float], metadata: Any | None = None
    ) -> None:
    ...
    def __repr__(self) -> str:
        ...
        return f'qubitron.Points({self.key!r}, {self.points!r}{metadata_repr})'
...
class Linspace(SingleSweep):
    ...
    def __repr__(self) -> str:
        metadata_repr = f', metadata={self.metadata!r}' if self.metadata is not None else ""
        return (
            f'qubitron.Linspace({self.key!r}, start={self.start!r}, '
...
class ListSweep(Sweep):
    ...
    def __init__(self, resolver_list: Iterable[resolver.ParamResolverOrSimilarType]):
        ...
        Raises:
            TypeError: If `resolver_list` is not a `qubitron.ParamResolver` or a
                dict.
        """
    ...
    def __repr__(self) -> str:
        return f'qubitron.ListSweep({self.resolver_list!r})'
