...
import qubitron
from qubitron import quirk_json_to_circuit, quirk_url_to_circuit
...
def test_parse_simple_cases():
    ...
    assert quirk_url_to_circuit('http://algassert.com/quirk') == qubitron.Circuit()
    assert quirk_url_to_circuit('https://algassert.com/quirk') == qubitron.Circuit()
    assert quirk_url_to_circuit('https://algassert.com/quirk#') == qubitron.Circuit()
    assert quirk_url_to_circuit('http://algassert.com/quirk#circuit={"cols":[]}') == qubitron.Circuit()

    assert quirk_url_to_circuit(
        'https://algassert.com/quirk#circuit={'
        '%22cols%22:[[%22H%22],[%22%E2%80%A2%22,%22X%22]]'
        '}'
    ) == qubitron.Circuit(qubitron.H(a), qubitron.X(b).controlled_by(a))
...
def test_parse_with_qubits():
    ...
    b = qubitron.GridQubit(0, 1)
    c = qubitron.GridQubit(0, 2)

    assert quirk_url_to_circuit(
        'http://algassert.com/quirk#circuit={"cols":[["H"],["•","X"]]}',
        qubits=qubitron.GridQubit.rect(4, 4),
    ) == qubitron.Circuit(qubitron.H(a), qubitron.X(b).controlled_by(a))

    assert quirk_url_to_circuit(
        'http://algassert.com/quirk#circuit={"cols":[["H"],["•",1,"X"]]}',
        qubits=qubitron.GridQubit.rect(4, 4),
    ) == qubitron.Circuit(qubitron.H(a), qubitron.X(c).controlled_by(a))

    with pytest.raises(IndexError, match="qubits specified"):
        _ = quirk_url_to_circuit(
            'http://algassert.com/quirk#circuit={"cols":[["H"],["•","X"]]}',
            qubits=[qubitron.GridQubit(0, 0)],
        )


def test_extra_cell_makers():
    assert qubitron.quirk_url_to_circuit(
        'http://algassert.com/quirk#circuit={"cols":[["iswap"]]}',
        extra_cell_makers=[
            qubitron.interop.quirk.cells.CellMaker(
                identifier='iswap', size=2, maker=lambda args: qubitron.ISWAP(*args.qubits)
            )
        ],
    ) == qubitron.Circuit(qubitron.ISWAP(*qubitron.LineQubit.range(2)))

    assert qubitron.quirk_url_to_circuit(
        'http://algassert.com/quirk#circuit={"cols":[["iswap"]]}',
        extra_cell_makers={'iswap': qubitron.ISWAP},
    ) == qubitron.Circuit(qubitron.ISWAP(*qubitron.LineQubit.range(2)))
    ...
    assert qubitron.quirk_url_to_circuit(
        ...
        extra_cell_makers=[
            ...
            qubitron.interop.quirk.cells.CellMaker(
                identifier='toffoli', size=3, maker=lambda args: qubitron.TOFFOLI(*args.qubits)
            ),
        ],
    ) == qubitron.Circuit(
        [qubitron.ISWAP(*qubitron.LineQubit.range(2)), qubitron.TOFFOLI(*qubitron.LineQubit.range(3))]
    )

    assert qubitron.quirk_url_to_circuit(
        'http://algassert.com/quirk#circuit={"cols":[["iswap"], ["toffoli"]]}',
        extra_cell_makers={'iswap': qubitron.ISWAP, 'toffoli': qubitron.TOFFOLI},
    ) == qubitron.Circuit(
        [qubitron.ISWAP(*qubitron.LineQubit.range(2)), qubitron.TOFFOLI(*qubitron.LineQubit.range(3))]
    )
...
def test_init():
    b, c, d, e, f = qubitron.LineQubit.range(1, 6)
    assert_url_to_circuit_returns(
        '{"cols":[],"init":[0,1,"+","-","i","-i"]}',
        qubitron.Circuit(
            qubitron.X(b),
            qubitron.ry(np.pi / 2).on(c),
            qubitron.ry(-np.pi / 2).on(d),
            qubitron.rx(-np.pi / 2).on(e),
            qubitron.rx(np.pi / 2).on(f),
        ),
    )
    ...
    with pytest.raises(ValueError, match="init must be a list"):
        _ = qubitron.quirk_url_to_circuit('http://algassert.com/quirk#circuit={"cols":[],"init":0}')

    with pytest.raises(ValueError, match="Unrecognized init state"):
        _ = qubitron.quirk_url_to_circuit('http://algassert.com/quirk#circuit={"cols":[],"init":[2]}')
...
def test_custom_matrix_gate():
    ...
    assert_url_to_circuit_returns(
        ...
        qubitron.Circuit(qubitron.MatrixGate(np.array([[0, 1], [1, 0]])).on(a)),
    )

    # With name.
    assert_url_to_circuit_returns(
        '{"cols":[["~cv0d"]],"gates":[{"id":"~cv0d","name":"test","matrix":"{{0,i},{1,0}}"}]}',
        qubitron.Circuit(qubitron.MatrixGate(np.array([[0, 1j], [1, 0]])).on(a)),
    )
    ...
    assert_url_to_circuit_returns(
        ...
        qubitron.Circuit(qubitron.X(a), qubitron.MatrixGate(np.diag([-1, 1j, 1, -1j])).on(b, a)),
        output_amplitudes_from_quirk=[
            {"r": 0, "i": 0},
            {"r": 0, "i": 1},
            {"r": 0, "i": 0},
...
def test_completes_weight_zero_billion_laughs():
    circuit = qubitron.quirk_url_to_circuit(
        'https://algassert.com/quirk#circuit={'
        '"cols":[["~z"]],'
        '"gates":['
        '{"id":"~a","circuit":{"cols":['
