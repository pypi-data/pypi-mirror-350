...
import qubitron
from qubitron import quirk_json_to_circuit
from qubitron.interop.quirk.cells.composite_cell import _iterator_to_iterable
...
def test_custom_circuit_gate():
    ...
    assert_url_to_circuit_returns(
        ...
        qubitron.Circuit(qubitron.H(a), qubitron.X(b).controlled_by(a), qubitron.Y(a)),
    )
    ...
    assert_url_to_circuit_returns(
        ...
        qubitron.Circuit(
            qubitron.interop.quirk.QuirkArithmeticGate('+=A1', target=[2], inputs=[[2]]).on(b, a)
        ),
    )
    ...
    assert_url_to_circuit_returns(
        ...
        '"circuit":{"cols":[["X"],["Y","Z"]]}}]}',
        qubitron.Circuit(
            qubitron.X(c).controlled_by(a), qubitron.Y(c).controlled_by(a), qubitron.Z(d).controlled_by(a)
        ),
    )
    ...
    assert_url_to_circuit_returns(
        ...
        '"circuit":{"cols":[["X"],["⊕","Z"]]}}]}',
        qubitron.Circuit(
            qubitron.X(c).controlled_by(a),
            qubitron.Y(c) ** 0.5,
            qubitron.Z(d).controlled_by(a, c),
            qubitron.Y(c) ** -0.5,
        ),
    )
    ...
    assert_url_to_circuit_returns(
        ...
        qubitron.Circuit(
            qubitron.interop.quirk.QuirkArithmeticGate('+=A2', target=[2, 2], inputs=[[2, 2]]).on(
                a, b, d, e
            ),
            qubitron.interop.quirk.QuirkArithmeticGate('+=A2', target=[2, 2], inputs=[[2, 2]]).on(
                b, c, d, e
    ...
    assert_url_to_circuit_returns(
        ...
        qubitron.Circuit(qubitron.X(a)) * 4,
    )
