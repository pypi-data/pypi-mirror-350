...
def assert_url_to_circuit_returns(
    json_text: str,
    circuit: qubitron.Circuit | None = None,
    *,
    ...
    Args:
        ...
        output_amplitudes_from_quirk: Optional data copied from Quirk's "export
            simulation data" function, for comparison to Qubitron's simulator
    ...
    if diagram is not None:
        qubitron.testing.assert_has_diagram(parsed, diagram)

    if circuit is not None:
        qubitron.testing.assert_same_circuits(parsed, circuit)

    if unitary is not None:
        np.testing.assert_allclose(qubitron.unitary(parsed), unitary, atol=1e-8)
    ...
    if maps:
        qubitron.testing.assert_equivalent_computational_basis_map(maps, parsed)
