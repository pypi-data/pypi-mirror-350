...
def test_measurement_gates() -> None:
    ...
    assert_url_to_circuit_returns(
        ...
        qubitron.Circuit(
            qubitron.measure(a, key='row=0,col=0'),
            qubitron.measure(b, key='row=1,col=0'),
            qubitron.measure(a, key='row=0,col=1'),
            qubitron.measure(b, key='row=1,col=1'),
        ),
    )
    assert_url_to_circuit_returns(
        ...
        qubitron.Circuit(
            ...
            qubitron.Moment(
                [
                    qubitron.measure(a, key='row=0,col=0'),
                    qubitron.measure(b, key='row=1,col=0'),
                    qubitron.measure(c, key='row=2,col=0'),
                ]
            ),
            qubitron.Y(a) ** -0.5,
            qubitron.X(b) ** 0.5,
