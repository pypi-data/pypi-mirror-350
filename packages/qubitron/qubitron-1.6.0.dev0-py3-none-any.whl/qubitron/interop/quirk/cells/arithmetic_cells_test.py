...
from qubitron.interop.quirk.cells import arithmetic_cells
...
def test_repr() -> None:
    ...
    qubitron.testing.assert_equivalent_repr(
        qubitron.interop.quirk.cells.arithmetic_cells.ArithmeticCell(
            '+=A2', qubitron.LineQubit.range(2), [qubitron.LineQubit.range(2, 5)]
        )
    )
...
def test_with_registers() -> None:
    ...
    op = cast(qubitron.ArithmeticGate, circuit[0].operations[0].gate)
    ...
    np.testing.assert_allclose(qubitron.unitary(qubitron.Circuit(op2())), np.array([[1]]), atol=1e-8)

    op2 = op.with_registers([2, 2, 2], 5, 5)
    np.testing.assert_allclose(
        qubitron.final_state_vector(qubitron.Circuit(op2(*qubitron.LineQubit.range(3))), initial_state=0),
        qubitron.one_hot(index=25 % 8, shape=8, dtype=np.complex64),
        atol=1e-8,
    )
...
def test_with_line_qubits_mapped_to() -> None:
    ...
    a2, b2, c2, d2, e2 = qubitron.NamedQubit.range(5, prefix='p')

    # After assigned to qubit register.
    cell = qubitron.interop.quirk.cells.arithmetic_cells.ArithmeticCell('+=A2', [a, b], [(c, d, e)])
    mapped_cell = qubitron.interop.quirk.cells.arithmetic_cells.ArithmeticCell(
        '+=A2', [a2, b2], [(c2, d2, e2)]
    )
    ...
    # Before assigned.
    cell = qubitron.interop.quirk.cells.arithmetic_cells.ArithmeticCell('+=A2', [a, b], [None])
    mapped_cell = qubitron.interop.quirk.cells.arithmetic_cells.ArithmeticCell('+=A2', [a2, b2], [None])
    assert cell != mapped_cell
    assert cell.with_line_qubits_mapped_to([a2, b2, c2, d2, e2]) == mapped_cell

    # After assigned to classical constant.
    cell = qubitron.interop.quirk.cells.arithmetic_cells.ArithmeticCell('+=A2', [a, b], [42])
    mapped_cell = qubitron.interop.quirk.cells.arithmetic_cells.ArithmeticCell('+=A2', [a2, b2], [42])
    assert cell != mapped_cell
    assert cell.with_line_qubits_mapped_to([a2, b2, c2, d2, e2]) == mapped_cell
