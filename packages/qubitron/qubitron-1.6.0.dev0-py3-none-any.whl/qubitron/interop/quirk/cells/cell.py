...
from qubitron import devices, ops, value
...
class Cell(metaclass=abc.ABCMeta):
    ...
    @classmethod
    def _replace_qubit(cls, old_qubit: qubitron.Qid, qubits: list[qubitron.Qid]) -> qubitron.Qid:
    ...
    @classmethod
    def _replace_qubits(
        cls, old_qubits: Iterable[qubitron.Qid], qubits: list[qubitron.Qid]
    ) -> tuple[qubitron.Qid, ...]:
    ...
    def with_input(self, letter: str, register: Sequence[qubitron.Qid] | int) -> Cell:
    ...
    def controlled_by(self, qubit: qubitron.Qid) -> Cell:
    ...
    def operations(self) -> qubitron.OP_TREE:
        ...
        Returns:
            A `qubitron.OP_TREE` of operations implementing the cell.
    ...
    def basis_change(self) -> qubitron.OP_TREE:
        ...
        Returns:
            A `qubitron.OP_TREE` of basis change operations.
        """
        return ()
...
class ExplicitOperationsCell(Cell):
    ...
    def controlled_by(self, qubit: qubitron.Qid) -> ExplicitOperationsCell:
        return ExplicitOperationsCell(
...
CellMakerArgs = NamedTuple(
    'CellMakerArgs', [('qubits', Sequence['qubitron.Qid']), ('value', Any), ('row', int), ('col', int)]
)
...
CellMaker = NamedTuple(
    ...
    [
        ('identifier', str),
        ('size', int),
        ('maker', Callable[[CellMakerArgs], Union[None, Cell, 'qubitron.Operation']]),
    ],
)
CellMaker.__doc__ = """Turns Quirk identifiers into Qubitron operations.
...
Attributes:
    ...
    maker: A function that takes a `qubitron.interop.quirk.cells.CellMakerArgs` and
        returns either a `qubitron.Operation` or a `qubitron.interop.quirk.cells.Cell`.
