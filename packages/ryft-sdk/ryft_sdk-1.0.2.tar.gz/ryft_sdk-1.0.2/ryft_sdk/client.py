...
class RyftClient:
    def __init__(self, api_key: Optional[str]):
        self.api_key = api_key or os.environ.get("RYFT_SECRET_KEY")
        if not self.api_key:
            raise ValueError(
                "API key is required. Please provide it as an argument or set the RYFT_API_KEY environment variable."
            )

        self.base_url = determine_base_url(self.api_key)
