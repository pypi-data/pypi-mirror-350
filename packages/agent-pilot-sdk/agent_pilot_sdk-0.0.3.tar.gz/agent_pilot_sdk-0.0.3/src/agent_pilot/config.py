...
class Config:
    ...
    def __init__(
        ...
        self.api_key = api_key or os.environ.get('PROBING_API_KEY')
        self.verbose = verbose if verbose is not None else os.getenv('PROBING_VERBOSE') is not None
        self.api_url = api_url or os.getenv('PROBING_API_URL') or DEFAULT_API_URL
        self.ssl_verify = not (
            disable_ssl_verify
        ...
        self.sample_rate = sample_rate if sample_rate is not None else os.getenv('PROBING_SAMPLE_RATE')
