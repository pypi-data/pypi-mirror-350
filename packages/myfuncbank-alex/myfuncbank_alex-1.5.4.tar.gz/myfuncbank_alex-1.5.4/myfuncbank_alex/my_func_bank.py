...
# ignore_tradingHour调用时显性设为True时，表示如果今天时交易日，不考虑是否已收盘，直接返回今天日期
def getLastTradeDate(current_time,ignore_tradingHour=False):
    # 判断今天是不是交易日

    try:
        lastTradeDate=w.tdaysoffset(0, current_time, "") #前推0个周期检测今天是否是交易日
        if current_time.date() == lastTradeDate.Data[0][0].date(): #今天是交易日
            if ignore_tradingHour:
...
def move_or_replace_sheet(src_file, dest_file, sheet_name):
    ...
    # ---- 防止pywin32 生成的缓存文件损坏或不完整导致调用EnsureDispatch函数报异常-----
    # 获取缓存路径
