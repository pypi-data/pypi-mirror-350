...
```sh
pip install -U 'mwpf_incr[stim]'
```

```python
from mwpf_incr import SinterMWPFIncrDecoder

sinter.collect(
    tasks = ...,
    num_workers = 1,
    decoders = ["mwpf_incr"],
    custom_decoders = { "mwpf_incr": SinterMWPFIncrDecoder(cluster_node_limit=50) },
)
...
custom_decoders = { "mwpf_incr": SinterMWPFIncrDecoder(cluster_node_limit=50).with_circuit(circuit) }
```
...
```python
from mwpf_incr import HyperEdge, SolverInitializer, SolverSerialJointSingleHair, SyndromePattern
