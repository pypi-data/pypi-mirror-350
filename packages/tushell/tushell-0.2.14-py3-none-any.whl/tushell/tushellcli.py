...
@click.command()
@click.option('--key', required=False, help='Memory key to store (required unless --json is used)')
...
def post_memory(key, value, file_, json_file):
    """Store fractal stone memory value by key. Accepts --value, --file, or --json, but only one at a time."""
    ...
    options = [v is not None for v in (value, file_, json_file)]
    if sum(options) > 1:
        click.echo("Error: Please provide only one of --value, --file, or --json.")
        return
    if sum(options) == 0:
        click.echo("Error: You must provide one of --value, --file, or --json.")
        return
