...
from metaworkflows.io.database import SparkDatabaseReader, SparkDatabaseWriter # Needs to be created
from metaworkflows.io.object_storage import SparkObjectStorageReader, SparkObjectStorageWriter # Needs to be created
...
class SparkEngine(BaseEngine):
    ...
    def initialize(self):
        ...
        try:
            ...
            logger.info(f"SparkSession initialized/retrieved. AppName: {self.spark_session.conf.get('spark.app.name')}")
    ...
    def _get_io_handler(self, connector_type: str, connection_ref: Optional[str], io_type: str): # io_type 'read' or 'write'
        conn_details = {}
        if connection_ref:
        ...
        specific_connector_type = conn_details.get("type", connector_type).lower()
        ...
        if specific_connector_type == "file":
        ...
        elif specific_connector_type in ["jdbc", "postgresql", "mysql"]: # Generalize JDBC
        ...
        elif specific_connector_type in ["gcp_cloud_storage", "gcs", "aws_s3", "s3"]: # Object storage
            ...
            raise ValueError(f"Unsupported connector type '{specific_connector_type}' for SparkEngine.")


    def read(self, connector_type: str, connection_ref: Optional[str], options: Dict[str, Any]) -> DataFrame:
        if not self.spark_session:
            raise RuntimeError("SparkSession is not initialized. Call initialize() first.")
        logger.info(f"SparkEngine reading from {connector_type} (ref: {connection_ref}) with options: {options}")
    ...
    def transform(self, step_config: Any, dataframes: Dict[str, DataFrame]) -> DataFrame:
        ...
        #    raise NotImplementedError("PySpark script transformation not yet implemented.")
            ...
            raise ValueError("Unsupported Spark transformation type in engine_specific config.")

    def write(self, dataframe: DataFrame, connector_type: str, connection_ref: Optional[str], options: Dict[str, Any]):
        if not self.spark_session:
            raise RuntimeError("SparkSession is not initialized. Call initialize() first.")
        logger.info(f"SparkEngine writing to {connector_type} (ref: {connection_ref}) with options: {options}")
    ...
    def custom_script(self, step_config: Any):
        ...
        transform_config = step_config.engine_specific
        if "script" in transform_config:
            script = transform_config["script"]
            #TODO: Implement logic to execute the script
            return script
        # Add other transformation types specific to Spark if needed (e.g., calling a PySpark script)
        # elif "pyspark_script" in transform_config:
        #    script_path = transform_config["pyspark_script"]["path"]
