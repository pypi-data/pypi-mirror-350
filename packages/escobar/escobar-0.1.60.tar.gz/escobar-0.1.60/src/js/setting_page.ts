...
// Declare the global PACKAGE_VERSION variable that will be injected by webpack
declare const PACKAGE_VERSION: string;
// Use a fallback value when PACKAGE_VERSION is not defined (e.g., during development)
const version = typeof PACKAGE_VERSION !== 'undefined' ? PACKAGE_VERSION : '0.1.58';
...
export function createSettingsPage(
  ...
  if (isJupyterHub) {
    console.log('Creating JupyterHub settings page');
    return new JupyterHubSettingsPage(settingsRegistry, currentSettings, onSave);
  } else {
    console.log('Creating Plugin settings page');
