...
function getJupyterHubUsername(): { username: string, fromJupyterHub: boolean } {
  console.log('DEBUG: Getting JupyterHub username...');
  ...
  // Try a more permissive regex that can handle special characters in the username
  const hubUserRegex = /\/user\/([^\/]+)\//;
  console.log('DEBUG: Using regex pattern:', hubUserRegex);
  ...
  if (hubUserMatch && hubUserMatch[1]) {
    const rawUsername = hubUserMatch[1];
    console.log('DEBUG: Raw username from URL pathname:', rawUsername);

    // Make sure to decode the username to handle special characters like @ in email addresses
    const decodedUsername = decodeURIComponent(rawUsername);
    console.log('DEBUG: Decoded username from URL pathname:', decodedUsername);
  ...
  console.log('DEBUG: Full URL match result:', fullUrlMatch);

  if (fullUrlMatch && fullUrlMatch[1]) {
    const rawUsername = fullUrlMatch[1];
    console.log('DEBUG: Raw username from full URL:', rawUsername);

    // Make sure to decode the username to handle special characters like @ in email addresses
    const decodedUsername = decodeURIComponent(rawUsername);
    console.log('DEBUG: Decoded username from full URL:', decodedUsername);
  ...
  try {
    ...
    console.log('DEBUG: Using baseURI regex pattern:', baseRegex);

    const baseMatch = baseUri.match(baseRegex);
    console.log('DEBUG: Base URI match result:', baseMatch);

    if (baseMatch && baseMatch[1]) {
      const rawUsername = baseMatch[1];
      console.log('DEBUG: Raw username from baseURI:', rawUsername);

      // Make sure to decode the username to handle special characters like @ in email addresses
      const decodedUsername = decodeURIComponent(rawUsername);
      console.log('DEBUG: Decoded username from baseURI:', decodedUsername);
  ...
  if (altMatch && altMatch[1]) {
    const rawUsername = altMatch[1];
    console.log('DEBUG: Raw username from alternative pattern (pathname):', rawUsername);

    const decodedUsername = decodeURIComponent(rawUsername);
    console.log('DEBUG: Decoded username from alternative pattern (pathname):', decodedUsername);
  ...
  console.log('DEBUG: Alternative full URL match result:', altFullMatch);

  if (altFullMatch && altFullMatch[1]) {
    const rawUsername = altFullMatch[1];
    console.log('DEBUG: Raw username from alternative pattern (full URL):', rawUsername);

    const decodedUsername = decodeURIComponent(rawUsername);
    console.log('DEBUG: Decoded username from alternative pattern (full URL):', decodedUsername);

    return { username: decodedUsername, fromJupyterHub: true };
  }

  // Try one more pattern specifically for email addresses in URLs
  console.log('DEBUG: Trying email-specific regex pattern...');
  const emailRegex = /user\/([^\/]+@[^\/]+)/;
  console.log('DEBUG: Using email regex pattern:', emailRegex);

  const emailMatch = window.location.href.match(emailRegex);
  console.log('DEBUG: Email regex match result:', emailMatch);

  if (emailMatch && emailMatch[1]) {
    const rawUsername = emailMatch[1];
    console.log('DEBUG: Raw username from email pattern:', rawUsername);

    const decodedUsername = decodeURIComponent(rawUsername);
    console.log('DEBUG: Decoded username from email pattern:', decodedUsername);

    return { username: decodedUsername, fromJupyterHub: true };
  }

  // Not in a JupyterHub environment or username not found, return default username
  console.log('DEBUG: No JupyterHub username found, using default: VoittaDefaultUser');
...
export class ChatWidget extends Widget {
  ...
  async init() {
    console.log('DEBUG: Initializing chat widget...');
    ...
    console.log('DEBUG: Current settings before username check:', JSON.stringify(this.settings));

    // First check if we're in JupyterHub mode
    const isJupyterHub = window.location.href.includes('/user/');
    console.log('DEBUG: Is JupyterHub environment (based on URL):', isJupyterHub);

    if (isJupyterHub) {
      // In JupyterHub mode, ALWAYS get the username from the URL
      console.log('DEBUG: In JupyterHub mode, getting username from URL');

      // Get JupyterHub username from URL
      const usernameInfo = getJupyterHubUsername();
      console.log('DEBUG: Username info from getJupyterHubUsername():', usernameInfo);

      // Force update the username in JupyterHub mode
      if (usernameInfo.fromJupyterHub) {
        console.log('DEBUG: Forcing username update from:', this.settings.username, 'to:', usernameInfo.username);
        this.settings.username = usernameInfo.username;
        this.settings.usernameFromJupyterHub = true;
        console.log(`DEBUG: Updated settings - username: ${this.settings.username}, fromJupyterHub: ${this.settings.usernameFromJupyterHub}`);
      } else {
        console.log('DEBUG: WARNING - In JupyterHub mode but could not extract username from URL');
      }
      ...
      console.log('DEBUG: Not in JupyterHub mode, but need to get or update username');
      console.log('DEBUG: Current username:', this.settings.username);
      ...
      if (usernameInfo.fromJupyterHub || !this.settings.username) {
        console.log('DEBUG: Updating username from:', this.settings.username, 'to:', usernameInfo.username);
        this.settings.username = usernameInfo.username;
        this.settings.usernameFromJupyterHub = usernameInfo.fromJupyterHub;
        console.log(`DEBUG: Updated settings - username: ${this.settings.username}, fromJupyterHub: ${this.settings.usernameFromJupyterHub}`);
      }
    } else {
      console.log(`DEBUG: Using saved username: ${this.settings.username} (custom user-defined name)`);
    }

    // Update global variable
    (window as any).escobarUsername = this.settings.username;
    console.log('DEBUG: Updated global escobarUsername to:', (window as any).escobarUsername);

    // Save to localStorage
    try {
      const settingsJson = JSON.stringify(this.settings);
      console.log('DEBUG: Saving settings to localStorage:', settingsJson);
      localStorage.setItem('escobar-settings', settingsJson);
      console.log('DEBUG: Saved settings to localStorage successfully');
    } catch (error) {
