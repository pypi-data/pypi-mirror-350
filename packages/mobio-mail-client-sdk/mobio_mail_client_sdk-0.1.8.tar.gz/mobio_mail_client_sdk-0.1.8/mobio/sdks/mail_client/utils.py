...
class EmailAddress:
    ...
    def to_json(self):
        return {"name": self.name, "email": self.email, "full": self.full}
...
def format_message_id_to_list(email_list: tuple) -> list:
    regex = "\\<(.*?)\\>"  # "<message_id>"
    message_id_raw = "".join(email_list)
    message_ids = re.findall(regex, message_id_raw)
    return message_ids