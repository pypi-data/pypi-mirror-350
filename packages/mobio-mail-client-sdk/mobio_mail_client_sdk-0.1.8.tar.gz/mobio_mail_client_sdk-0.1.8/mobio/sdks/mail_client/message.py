...
from .utils import decode_value, parse_email_addresses, parse_email_date, EmailAddress, format_message_id_to_list
...
class MailMessage:
    ...
    def message_id(self) -> str:
        ...
        if 'Message-ID' in self.obj:
            raw = format_message_id_to_list(self.obj['Message-ID'])
    ...
    def references(self) -> list:
        """Message references"""
        if 'References' in self.obj:
            raw = format_message_id_to_list(self.obj['References'])
            return raw
        return []
    ...
    def in_reply_to(self) -> str:
        """Message message_id"""
        if 'In-Reply-To' in self.obj:
            raw = format_message_id_to_list(self.obj['In-Reply-To'])
            return raw[0] if raw else ""
        return ''
