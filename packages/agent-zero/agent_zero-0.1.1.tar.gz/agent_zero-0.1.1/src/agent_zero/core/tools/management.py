...
from agent_zero.core.tools.functions.call_management import (
    transfer_call,
    end_call,
    get_weekday,
    await_call_transfer,
)
from agent_zero.core.tools.functions.scheduling import (
...
def get_functions_meta(function_names: Iterable[str]) -> List[Dict]:
    """
    Build an OpenAI `tools` list for the given function names.

    Args:
        function_names: An iterable of function names (strings) that should be
            exposed to the model.

    Returns:
        A list of dictionaries, each in the form expected by OpenAI's
        `function‑calling` interface, e.g.::
    ...
    Notes:
        • Only names present in the `_FUNCTION_PRESETS` table are used.
          Unknown names are silently ignored.

        • The order of the resulting list follows the order provided in
          *function_names* (duplicates are skipped).
    """
    seen: set[str] = set()
    tools: List[Dict] = []
    ...
    for name in function_names:
        if name in seen:
            continue  # skip duplicates while preserving order
        preset = _FUNCTION_PRESETS.get(name)
        if not preset:
            # Unknown function names are ignored to avoid runtime errors
            continue
        tools.append(
            {
                "type": "function",
